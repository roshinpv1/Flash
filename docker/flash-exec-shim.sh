#!/bin/sh
# shellcheck shell=sh
# /opt/flash/bin/flash — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `flash` user (UID 10000). When an operator runs `docker exec <c> flash ...`
...
# user runs `docker exec <c> flash login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Flash is not logged into Flash
# Portal" on every incoming message — even though `docker exec <c> flash
...
# Fix
# ---
# This shim sits at /opt/flash/bin/flash and is placed earliest on PATH.
# When invoked as root, it drops to the flash user (via s6-setuidgid)
...
# `docker exec --user flash`, kanban subagents, etc. — it short-circuits
...
# (/opt/flash/.venv/bin/flash), so the second hop cannot re-enter this
...
# state via the flash CLI. Default is to drop.

set -e

REAL=/opt/flash/.venv/bin/flash

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "flash-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user flash`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi
...
# Root, no opt-out. Drop to the flash user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "flash-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "flash-shim: re-run with --user flash or set HERMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the flash user's home before dropping privileges. Without
