...
#
# Wired into the image as /etc/cont-init.d/01-flash-setup by the
...
INSTALL_DIR="/opt/flash"

# Drop to flash via s6-setuidgid, but skip it when already non-root.
as_flash() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid flash "$@"; }
...
# starts non-root. The baked install tree under /opt/flash is intentionally
...
# `id -u flash` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# flash UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u flash)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-flash UID).
...
and the baked /opt/flash install tree is intentionally root-owned and
...
The image remaps the flash user to that UID/GID at boot and chowns the data
...
# `s6-setuidgid flash mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom HERMES_HOME paths whose parents only
# root can create (e.g. `HERMES_HOME=/home/flash/.flash` in a Compose
...
if [ -n "${HERMES_UID:-}" ] && validate_uid_gid "$HERMES_UID" && [ "$HERMES_UID" != "$(id -u flash)" ]; then
    echo "[stage2] Changing flash UID to $HERMES_UID"
    usermod -u "$HERMES_UID" flash
fi
if [ -n "${HERMES_GID:-}" ] && validate_uid_gid "$HERMES_GID" && [ "$HERMES_GID" != "$(id -g flash)" ]; then
    echo "[stage2] Changing flash GID to $HERMES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HERMES_GID" flash 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised flash user (UID 10000)
...
# flash process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes flash. Idempotent across container
...
#   - flash is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G flash 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] flash already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" flash 2>/dev/null; then
        echo "[stage2] Added flash to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group flash failed; docker backend may fail with EACCES"
    fi
    break
done
...
# the runtime flash UID, restore ownership to flash — but ONLY for the
# directories flash actually writes to. The full $HERMES_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of flash-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_flash_uid=$(id -u flash)
...
chown_flash_tree() {
    ...
    chown -R flash:flash "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$HERMES_HOME" 2>/dev/null)" != "$actual_flash_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $HERMES_HOME (targeted) to flash ($actual_flash_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $HERMES_HOME: chown the directory itself (not its contents)
    # so flash can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$HERMES_HOME"; then
        :
    else
        chown flash:flash "$HERMES_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $HERMES_HOME failed (rootless container?) — continuing"
    fi
    # Flash-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by flash (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$HERMES_HOME/$sub" ]; then
            chown_flash_tree "$HERMES_HOME/$sub"
        fi
    done
fi
...
# flash user. Hosted/container instances keep mutable state under
# $HERMES_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# HERMES_DISABLE_LAZY_INSTALLS=1. Keeping /opt/flash root-owned and
...
# even though installs are re-enabled. The dir is seeded + chowned to flash
...
# Always reset ownership of $HERMES_HOME/profiles to flash on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> flash …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as flash and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$HERMES_HOME/profiles" ]; then
    chown_flash_tree "$HERMES_HOME/profiles"
fi
...
# (jobs.json) must stay readable by the unprivileged flash runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$HERMES_HOME/cron" ]; then
    chown_flash_tree "$HERMES_HOME/cron"
fi
...
# flash pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged flash gateway cannot read,
...
if [ -d "$HERMES_HOME/platforms/pairing" ]; then
    chown_flash_tree "$HERMES_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$HERMES_HOME/pairing" ]; then
    chown_flash_tree "$HERMES_HOME/pairing"
fi

# Reset ownership of flash-owned top-level state files on every boot.
# The targeted data-volume chown above only covers flash-owned
...
# `docker exec <container> flash …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged flash runtime then hits
...
# entries of flash_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    if [ -e "$HERMES_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$HERMES_HOME/$f"; then
            ...
            chown flash:flash "$HERMES_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the flash runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$HERMES_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$HERMES_HOME/config.yaml"; then
        :
    else
        chown flash:flash "$HERMES_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$HERMES_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as flash user ---
# Run as flash via s6-setuidgid so dirs end up owned correctly (matters
...
# /opt/flash/.install_method (see Dockerfile), NOT written here into
...
# bind-mounted from the host (~/.flash:/opt/data) and sometimes shared with a
...
# 'flash update'. To heal homes already poisoned by older images, remove a
...
# the baked /opt/flash stamp, so deleting the data-dir copy is safe).
...
seed_one() {
    ...
    if [ ! -f "$HERMES_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$HERMES_HOME/$dest"; then
            ...
            as_flash cp "$INSTALL_DIR/$src" "$HERMES_HOME/$dest"
        fi
    fi
}
...
if [ -f "$HERMES_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$HERMES_HOME/.env"; then
        ...
        chown flash:flash "$HERMES_HOME/.env" 2>/dev/null || true
...
# config-schema migrations that `flash update` runs for non-Docker installs,
...
if [ -f "$HERMES_HOME/config.yaml" ]; then
    s6-setuidgid flash "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$HERMES_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$HERMES_HOME/auth.json"; then
        ...
        chown flash:flash "$HERMES_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$HERMES_HOME/gateway_state.json" ] && \
    ...
    if refuse_symlinked_path "seed" "$HERMES_HOME/gateway_state.json"; then
        ...
        chown flash:flash "$HERMES_HOME/gateway_state.json" 2>/dev/null || true
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_flash "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/flash/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Flash spawns for the
...
# main-wrapper.sh propagates it into the supervised ``flash`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-flash, dashboard, etc.).
