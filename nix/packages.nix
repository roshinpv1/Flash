# nix/packages.nix — Nyxo Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      nyxoAgent = pkgs.callPackage ./nyxo-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = nyxoAgent.nyxoTui;
        web = nyxoAgent.nyxoWeb;
        desktop = nyxoAgent.nyxoDesktop;
      };
    };
}
