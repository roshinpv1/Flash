# nix/nixosModules.nix — NixOS module for nyxo-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: nyxo runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NYXO_HOME/.env
# and read by nyxo at startup — no container recreation needed for env changes.
#
# Tool resolution: the nyxo wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.nyxo-agent;
    effectivePackage =
    ...
    nyxo-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-nyxo-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "nyxo-config-attrs";
      description = "Nyxo YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "nyxo-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # NYXO_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "nyxo-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NYXO_DOC_EOF'\n${value}\nNYXO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "nyxo-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/nyxo";
    ...
    # nyxo user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "nyxo-container-entrypoint" ''
      set -eu

      NYXO_UID="''${NYXO_UID:?NYXO_UID must be set}"
      NYXO_GID="''${NYXO_GID:?NYXO_GID must be set}"

      # ── Group: ensure a group with GID=$NYXO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NYXO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="nyxo"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NYXO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NYXO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NYXO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NYXO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="nyxo"
        TARGET_HOME="/home/nyxo"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NYXO_UID" -g "$NYXO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NYXO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NYXO_UID:$NYXO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NYXO_HOME is owned by the target user.
      ...
      if [ -n "''${NYXO_HOME:-}" ] && [ -d "$NYXO_HOME" ]; then
        find "$NYXO_HOME" \! -user "$NYXO_UID" -exec chown "$NYXO_UID:$NYXO_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/nyxo-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/nyxo-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nyxo ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nyxo
        chmod 0440 /etc/sudoers.d/nyxo
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NYXO_UID" --regid="$NYXO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NYXO_HOME/.env.
    ...
    # Default: /var/lib/nyxo/workspace → /data/workspace.
  ...
  in {
    options.services.nyxo-agent = with lib; {
      enable = mkEnableOption "Nyxo Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = nyxo-agent;
        description = "The nyxo-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/nyxo";
        description = "State directory. Contains .nyxo/ subdir (NYXO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Nyxo config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NYXO_HOME/.env at activation time.
          Nyxo reads this file on every startup via load_nyxo_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NYXO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NYXO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nyxo gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the nyxo user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nyxo plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Nyxo discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nyxo_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nyxo (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in nyxo-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the nyxo CLI to environment.systemPackages and export
          NYXO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.nyxo symlink to the service
            stateDir. These users are automatically added to the nyxo group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.nyxo-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the nyxo CLI to system PATH and export NYXO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.nyxo/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NYXO_HOME = "${cfg.stateDir}/.nyxo";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.nyxo-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the nyxo user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.nyxo-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed nyxo
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure nyxo is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.nyxo        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nyxo/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nyxo/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nyxo/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nyxo/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nyxo/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."nyxo-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.nyxo
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.nyxo ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.nyxo ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.nyxo -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.nyxo/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.nyxo/$_subdir"
            chmod 2770 "${cfg.stateDir}/.nyxo/$_subdir"
            find "${cfg.stateDir}/.nyxo/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # nyxo-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.nyxo/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.nyxo/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nyxo/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.nyxo/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.nyxo/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nyxo/.managed
          chmod 0644 ${cfg.stateDir}/.nyxo/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.nyxo/.container-mode <<'NYXO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    nyxo_bin=${containerDataDir}/current-package/bin/nyxo
    NYXO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nyxo/.container-mode
            chmod 0644 ${cfg.stateDir}/.nyxo/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.nyxo/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nyxo";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.nyxo" ]; then
                  rm -f "${symlinkPath}"
                  echo "nyxo-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.nyxo -> stateDir/.nyxo for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nyxo";
                target = "${cfg.stateDir}/.nyxo";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "nyxo-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nyxo/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.nyxo/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nyxo/auth.json
              fi
            ''}
          ''}
          ...
          # Nyxo reads $NYXO_HOME/.env at startup via load_nyxo_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.nyxo/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NYXO_NIX_ENV_EOF'
    ${envFileContent}
    NYXO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.nyxo/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.nyxo/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nyxo/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.nyxo-agent = {
          description = "Nyxo Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $NYXO_HOME/.env by the activation script. load_nyxo_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/nyxo"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the nyxo group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.nyxo-agent = {
          description = "Nyxo Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NYXO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NYXO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NYXO_UID="$NYXO_UID" \
                --env NYXO_GID="$NYXO_GID" \
                --env NYXO_HOME=${containerDataDir}/.nyxo \
                --env NYXO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/nyxo gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
