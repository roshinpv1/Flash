# nix/nixosModules.nix — NixOS module for flash-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: flash runs from /nix/store bind-mounted read-only into a
...
# and read by flash at startup — no container recreation needed for env changes.
#
# Tool resolution: the flash wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.flash-agent;
    effectivePackage =
    ...
    flash-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-flash-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "flash-config-attrs";
      description = "Flash YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "flash-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "flash-documents" { } (
      ''
        mkdir -p $out
    ...
    # flash user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "flash-container-entrypoint" ''
      set -eu
      ...
      if [ ! -f /var/lib/flash-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/flash-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/flash ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/flash
        chmod 0440 /etc/sudoers.d/flash
      fi
    ...
    # Default: /var/lib/flash/workspace → /data/workspace.
  ...
  in {
    options.services.flash-agent = with lib; {
      enable = mkEnableOption "Flash Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = flash-agent;
        description = "The flash-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/flash";
        description = "State directory. Contains .flash/ subdir (HERMES_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Flash config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Flash reads this file on every startup via load_flash_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `flash gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the flash user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the flash plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Flash discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          flash_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as flash (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in flash-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the flash CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.flash symlink to the service
            stateDir. These users are automatically added to the flash group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.flash-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the flash CLI to system PATH and export HERMES_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.flash/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.HERMES_HOME = "${cfg.stateDir}/.flash";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.flash-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the flash user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.flash-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed flash
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure flash is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.flash        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.flash/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.flash/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.flash/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.flash/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.flash/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."flash-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.flash
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.flash ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.flash ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.flash -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.flash/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.flash/$_subdir"
            chmod 2770 "${cfg.stateDir}/.flash/$_subdir"
            find "${cfg.stateDir}/.flash/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # flash-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.flash/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.flash/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.flash/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.flash/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.flash/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.flash/.managed
          chmod 0644 ${cfg.stateDir}/.flash/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.flash/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    flash_bin=${containerDataDir}/current-package/bin/flash
    HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.flash/.container-mode
            chmod 0644 ${cfg.stateDir}/.flash/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.flash/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.flash";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.flash" ]; then
                  rm -f "${symlinkPath}"
                  echo "flash-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.flash -> stateDir/.flash for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.flash";
                target = "${cfg.stateDir}/.flash";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "flash-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.flash/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.flash/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.flash/auth.json
              fi
            ''}
          ''}
          ...
          # Flash reads $HERMES_HOME/.env at startup via load_flash_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.flash/.env"
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.flash/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.flash/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.flash/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.flash-agent = {
          description = "Flash Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            HERMES_HOME = "${cfg.stateDir}/.flash";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $HERMES_HOME/.env by the activation script. load_flash_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/flash"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the flash group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.flash-agent = {
          description = "Flash Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env HERMES_HOME=${containerDataDir}/.flash \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/flash gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
