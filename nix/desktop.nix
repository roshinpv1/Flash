# nix/desktop.nix — Flash Desktop (Electron) app build + wrapper
#
# `flashAgent` is the fully-built `.#default` package — it ships the
# `flash` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `HERMES_DESKTOP_HERMES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Flash CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "flash-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "flash-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "flash-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/flash-desktop $out/bin
    cp -r ${renderer}/* $out/share/flash-desktop/
    ...
    substituteInPlace $out/share/flash-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/flash-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # HERMES_DESKTOP_HERMES to the absolute path of the nix-built `flash`
    # binary so the desktop's resolver step 4 ("existing Flash CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/flash-desktop \
      --add-flags "$out/share/flash-desktop" \
      --set HERMES_DESKTOP_HERMES "${lib.getExe flashAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Flash Agent";
    homepage = "https://github.com/FlashOrg/flash-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "flash-desktop";
  };
}
