...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      flash-agent = self'.packages.default;
      flashVenv = flash-agent.flashVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "flash-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${flashVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "flash-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "flash-build-package" { } ''
          echo "PASS: package built at ${flash-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "flash-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "flash-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${flash-agent}/bin/flash || (echo "FAIL: flash binary missing"; exit 1)
          test -x ${flash-agent}/bin/flash-agent || (echo "FAIL: flash-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${flash-agent}/bin/flash version 2>&1 | grep -qi "flash" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "flash-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in flash flash-agent flash-acp; do
            test -x ${flash-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "flash-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking flash --help ==="
          ${flash-agent}/bin/flash --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${flash-agent}/bin/flash --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "flash-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${flash-agent}/share/flash-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${flash-agent}/share/flash-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${flash-agent}/bin/flash || \
        ...
        bundled-plugins = pkgs.runCommand "flash-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${flash-agent}/share/flash-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${flash-agent}/share/flash-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${flash-agent}/bin/flash || \
        ...
        bundled-locales = pkgs.runCommand "flash-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${flash-agent}/share/flash-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${flash-agent}/share/flash-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${flash-agent}/share/flash-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HERMES_BUNDLED_LOCALES" ${flash-agent}/bin/flash || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HERMES_BUNDLED_LOCALES=${flash-agent}/share/flash-agent/locales \
            ${flashVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          ...
          BARE_DIR=$(cd "$HOME" && ${flashVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${flashVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
        ...
        bundled-tui = pkgs.runCommand "flash-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${flash-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${flash-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${flash-agent}/bin/flash || \
        ...
        flash-node = pkgs.runCommand "flash-node-version" { } ''
          set -e
          echo "=== Checking HERMES_NODE in wrapper ==="
          grep -q "HERMES_NODE" ${flash-agent}/bin/flash || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${flash-agent}/bin/flash)
        ...
        managed-guard = pkgs.runCommand "flash-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${flash-agent}/bin/flash config set model foo
          check_blocked "config edit" ${flash-agent}/bin/flash config edit
        ...
        extra-python-packages = let
          ...
          flashWithExtra = flash-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "flash-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${flashWithExtra}/bin/flash || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${flashWithExtra}/bin/flash || \
          ...
          if grep -q "PYTHONPATH" ${flash-agent}/bin/flash; then
        ...
        extra-dependency-groups = let
          flashWithGroups = flash-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "flash-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${flashWithGroups}"
          echo "venv: ${flashWithGroups.flashVenv}"
        ...
        messaging-variant = pkgs.runCommand "flash-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.flashVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "flash-config-roundtrip" {
          ...
          merge_and_load() {
            local flash_home="$1"
            export HERMES_HOME="$flash_home"
            ${configMergeScript} ${nixSettings} "$flash_home/config.yaml"
            ${flashVenv}/bin/python3 -c '
import json, sys
