...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nyxo-agent = self'.packages.default;
      nyxoVenv = nyxo-agent.nyxoVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nyxo-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${nyxoVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nyxo-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "nyxo-build-package" { } ''
          echo "PASS: package built at ${nyxo-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "nyxo-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "nyxo-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nyxo-agent}/bin/nyxo || (echo "FAIL: nyxo binary missing"; exit 1)
          test -x ${nyxo-agent}/bin/nyxo-agent || (echo "FAIL: nyxo-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nyxo-agent}/bin/nyxo version 2>&1 | grep -qi "nyxo" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nyxo-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nyxo nyxo-agent nyxo-acp; do
            test -x ${nyxo-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nyxo-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nyxo --help ==="
          ${nyxo-agent}/bin/nyxo --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nyxo-agent}/bin/nyxo --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nyxo-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nyxo-agent}/share/nyxo-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${nyxo-agent}/share/nyxo-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NYXO_BUNDLED_SKILLS" ${nyxo-agent}/bin/nyxo || \
            (echo "FAIL: NYXO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NYXO_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nyxo-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nyxo-agent}/share/nyxo-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nyxo-agent}/share/nyxo-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NYXO_BUNDLED_PLUGINS" ${nyxo-agent}/bin/nyxo || \
            (echo "FAIL: NYXO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NYXO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "nyxo-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${nyxo-agent}/share/nyxo-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${nyxo-agent}/share/nyxo-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${nyxo-agent}/share/nyxo-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "NYXO_BUNDLED_LOCALES" ${nyxo-agent}/bin/nyxo || \
            (echo "FAIL: NYXO_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: NYXO_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (NYXO_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && NYXO_BUNDLED_LOCALES=${nyxo-agent}/share/nyxo-agent/locales \
            ${nyxoVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with NYXO_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${nyxoVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${nyxoVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without NYXO_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "nyxo-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nyxo-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nyxo-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "NYXO_TUI_DIR" ${nyxo-agent}/bin/nyxo || \
            (echo "FAIL: NYXO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NYXO_TUI_DIR set in wrapper"
        ...
        # Verify NYXO_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        nyxo-node = pkgs.runCommand "nyxo-node-version" { } ''
          set -e
          echo "=== Checking NYXO_NODE in wrapper ==="
          grep -q "NYXO_NODE" ${nyxo-agent}/bin/nyxo || \
            (echo "FAIL: NYXO_NODE not set in wrapper"; exit 1)
          echo "PASS: NYXO_NODE present in wrapper"

          NYXO_NODE=$(sed -n "s/^export NYXO_NODE='\(.*\)'/\1/p" ${nyxo-agent}/bin/nyxo)
          test -x "$NYXO_NODE" || (echo "FAIL: NYXO_NODE=$NYXO_NODE not executable"; exit 1)
          echo "PASS: NYXO_NODE executable at $NYXO_NODE"

          NODE_MAJOR=$("$NYXO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All NYXO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NYXO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nyxo-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NYXO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NYXO_MANAGED guards ==="
          check_blocked "config set" ${nyxo-agent}/bin/nyxo config set model foo
          check_blocked "config edit" ${nyxo-agent}/bin/nyxo config edit
        ...
        extra-python-packages = let
          ...
          nyxoWithExtra = nyxo-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nyxo-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nyxoWithExtra}/bin/nyxo || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nyxoWithExtra}/bin/nyxo || \
          ...
          if grep -q "PYTHONPATH" ${nyxo-agent}/bin/nyxo; then
        ...
        extra-dependency-groups = let
          nyxoWithGroups = nyxo-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nyxo-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nyxoWithGroups}"
          echo "venv: ${nyxoWithGroups.nyxoVenv}"
        ...
        messaging-variant = pkgs.runCommand "nyxo-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.nyxoVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "nyxo-config-roundtrip" {
          ...
          merge_and_load() {
            local nyxo_home="$1"
            export NYXO_HOME="$nyxo_home"
            ${configMergeScript} ${nixSettings} "$nyxo_home/config.yaml"
            ${nyxoVenv}/bin/python3 -c '
import json, sys
