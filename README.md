...
**The self-improving AI agent built by [Flash Org](https://flashorg.com).** It's the only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, searches its own past conversations, and builds a deepening model of who you are across sessions. Run it on a $5 VPS, a GPU cluster, or serverless infrastructure that costs nearly nothing when idle. It's not tied to your laptop — talk to it from Telegram while it works on a cloud VM.

Use any model you want — [FlashPortal](https://portal.flashorg.com), OpenRouter, OpenAI, your own endpoint, and [many others](https://flash-agent.flashorg.com/docs/integrations/providers). Switch with `flash model` — no code changes, no lock-in.
...
> **Heads up:** Native Windows runs Flash without WSL — CLI, gateway, TUI, and tools all work natively. If you'd rather use WSL2, the Linux/macOS one-liner above works there too. Found a bug? Please [file issues](https://github.com/FlashOrg/flash-agent/issues).
...
The installer handles everything: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **and a portable Git Bash** (MinGit, unpacked to `%LOCALAPPDATA%\flash\git` — no admin required, completely isolated from any system Git install). Flash uses this bundled Git Bash to run shell commands.

If you already have Git installed, the installer detects it and uses that instead. Otherwise a ~45MB MinGit download is all you need — it won't touch or interfere with any system Git.

> **Android / Termux:** The tested manual path is documented in the [Termux guide](https://flash-agent.flashorg.com/docs/getting-started/termux). On Termux, Flash installs a curated `.[termux]` extra because the full `.[all]` extra currently pulls Android-incompatible voice dependencies.
>
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Flash `bin` folder (`%LOCALAPPDATA%\flash\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Flash bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
- Whitelist the **folder**, not the file hash — Flash updates `uv` and the hash changes every version
...
## Skip the API-key collection — FlashPortal

Flash works with whatever provider you want — that's not changing. But if you'd rather not collect five separate API keys for the model, web search, image generation, TTS, and a cloud browser, **[FlashPortal](https://portal.flashorg.com)** covers all of them under one subscription:
...
That logs you in via OAuth, sets Flashas your provider, and turns on the Tool Gateway. Check what's wired up any time with `flash portal info`. Full details on the [Tool Gateway docs page](https://flash-agent.flashorg.com/docs/user-guide/features/tool-gateway).
...
Flash has two entry points: start the terminal UI with `flash`, or run the gateway and talk to it from Telegram, Discord, Slack, WhatsApp, Signal, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.
...
If you're coming from OpenClaw, Flash can automatically import your settings, memories, skills, and API keys.
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Linux desktop-control MCP server for Flash and other MCP hosts, with AT-SPI accessibility trees, Wayland/X11 input, screenshots, and compositor window targeting.
- 🔌 [FlashClaw](https://github.com/AaronWong1999/flashclaw) — Community WeChat bridge: Run Flash Agent and OpenClaw on the same WeChat account.
