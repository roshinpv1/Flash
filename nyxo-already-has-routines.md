# Nyxo Agent Has Had "Routines" Since March
...
Nyxo equivalent — works today:
```bash
nyxo webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Nyxo implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Nyxo Agent |
|---|---|---|
...
## Things Nyxo Does That Routines Can't
...
Nyxo has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
```bash
nyxo gateway setup    # enable webhooks
nyxo webhook subscribe pr-review \
  --events "pull_request" \
