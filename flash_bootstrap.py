"""Windows UTF-8 bootstrap for Flash entry points.
...
should be imported at the very top of every Flash entry point
(``flash``, ``flash-agent``, ``flash-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Flash modules.

    Flash ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Flash source root
...
# Apply on import — entry points just need ``import flash_bootstrap``
# (or ``from flash_bootstrap import apply_windows_utf8_bootstrap``) at
