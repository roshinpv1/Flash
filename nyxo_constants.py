"""Shared constants for Nyxo Agent.
...
_UNSET = object()
_NYXO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_NYXO_HOME_OVERRIDE", default=_UNSET
)


def set_nyxo_home_override(path: str | Path | None) -> Token:
    """Set a context-local Nyxo home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _NYXO_HOME_OVERRIDE.set(value)


def reset_nyxo_home_override(token: Token) -> None:
    """Restore the previous context-local Nyxo home override."""
    _NYXO_HOME_OVERRIDE.reset(token)


def get_nyxo_home_override() -> str | None:
    """Return the active context-local Nyxo home override, if any."""
    override = _NYXO_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_nyxo_home() -> Path:
    """Return the platform-native default Nyxo home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "nyxo"
    return Path.home() / ".nyxo"


def get_nyxo_home() -> Path:
    """Return the Nyxo home directory (default: platform-native path).

    Reads NYXO_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``NYXO_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``NYXO_HOME`` explicitly (see the systemd
    template in ``nyxo_cli/gateway.py`` and the kanban dispatcher in
    ``nyxo_cli/kanban_db.py``).  See https://github.com/NousResearch/nyxo-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_nyxo_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[NYXO_HOME fallback] NYXO_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass NYXO_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_nyxo_home()


def get_default_nyxo_root() -> Path:
    """Return the root Nyxo directory for profile-level operations.

    In standard deployments this is the platform-native Nyxo home
    (``~/.nyxo`` on POSIX, ``%LOCALAPPDATA%\\nyxo`` on native Windows).

    In Docker or custom deployments where ``NYXO_HOME`` points outside
    ``~/.nyxo`` (e.g. ``/opt/data``), returns ``NYXO_HOME`` directly
    — that IS the root.

    In profile mode where ``NYXO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.nyxo/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_nyxo_home()
    env_home = os.environ.get("NYXO_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # NYXO_HOME is under ~/.nyxo (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — NYXO_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Nyxo is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``NYXO_OPTIONAL_SKILLS``.
    """
    override = os.getenv("NYXO_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_nyxo_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``NYXO_OPTIONAL_MCPS``.
    """
    override = os.getenv("NYXO_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_nyxo_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``NYXO_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<NYXO_HOME>/skills`` last-resort
    """
    override = os.getenv("NYXO_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_nyxo_home() / "skills"


def get_nyxo_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Nyxo subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to NYXO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to NYXO_HOME (e.g. ``"image_cache"``).
...
def iter_nyxo_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Nyxo-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\nyxo\\node``. POSIX installs use
    ``$NYXO_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_nyxo_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with nyxoManagedNodePathEntries() in
...
def find_nyxo_node_executable(command: str) -> str | None:
    """Return a Nyxo-managed Node/npm executable path, if installed."""
    names = _candidate_node_command_names(command)
    for directory in iter_nyxo_node_dirs():
        for name in names:
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Nyxo-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring Nyxo-managed installs.

    This is for Nyxo-owned subprocesses that should not be broken by a bad,
    missing, or elevation-triggering system Node/npm on PATH.
    """
    return find_nyxo_node_executable(command) or find_node_executable_on_path(command)


def with_nyxo_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Nyxo-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_nyxo_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``nyxo update`` — leaving a **dangling symlink** that ``which`` still
...
def display_nyxo_home() -> str:
    """Return a user-friendly display string for the current NYXO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.nyxo``
        profile:  ``~/.nyxo/profiles/coder``
        custom:   ``/opt/nyxo-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.nyxo``.  For code that needs a real ``Path``, use
    :func:`get_nyxo_home` instead.
    """
    home = get_nyxo_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``NYXO_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/nyxo-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{NYXO_HOME}/home`` when the profile-home directory exists."""
    nyxo_home = get_nyxo_home_override() or (env or {}).get("NYXO_HOME") or os.getenv("NYXO_HOME")
    if not nyxo_home:
        return None
    profile_home = os.path.join(nyxo_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("NYXO_REAL_HOME") or os.getenv("NYXO_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Nyxo profile HOME.

    ``NYXO_HOME`` scopes Nyxo state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={NYXO_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{NYXO_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{NYXO_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Nyxo' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["NYXO_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: NousResearch/nyxo-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under NYXO_HOME.

    Replaces the ``get_nyxo_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, nyxo_logging.py, nyxo_time.py, etc.).
    """
    return get_nyxo_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under NYXO_HOME."""
    return get_nyxo_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under NYXO_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_nyxo_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._nyxo_ipv4_patched = True  # type: ignore[attr-defined]
