...
def register_credential_file(
    ...
    *relative_path* is relative to ``HERMES_HOME`` (e.g. ``google_token.json``).
    ...
    """
    flash_home = _resolve_flash_home()
    ...
    host_path = flash_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, flash_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.flash/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="flash-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching flash_constants.get_flash_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_flash_dir()`` for backward compatibility with old directory layouts.
    """
    from flash_constants import get_flash_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_flash_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.flash``,
    SSH ``<remote_home>/.flash``, etc.) and whether translation is wanted.
