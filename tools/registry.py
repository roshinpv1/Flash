"""Central registry for all flash-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``flash tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``flash tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("flash_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``flash_plugins.{name}``), not the exact module string.
                # A handler defined in ``flash_plugins.pkg.handlers`` is
                # still owned by the ``flash_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("flash_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``flash tools enable foo``)
