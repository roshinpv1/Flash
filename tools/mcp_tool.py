...
transport, discovers their tools, and registers them into the flash-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.flash/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.flash/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Flash' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            flash_home = os.path.expanduser(
                os.getenv(
                    "HERMES_HOME", os.path.join(os.path.expanduser("~"), ".flash")
                )
            )
            candidates = [
                os.path.join(flash_home, "node", "bin", resolved_command),
                ...
                # image (which the Flash Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Flash MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Flash gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Flash' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Flash' logging (agent.log via flash_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'flash-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'flash-agent[all]'"
            )
        ...
        # ungraceful exit of this Flash process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Flash' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Flash is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `flash mcp login {server_name}` (or delete the tokens "
            f"file under ~/.flash/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    ...
    try:
        ...
        home_override = get_flash_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_flash_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # vanish here: OAuth token stores and any other get_flash_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Flash config file.
    ...
    ``os.environ`` (which includes ``~/.flash/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Flash' MEDIA: tag convention and out to messaging
            ...
            # Flash' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Flash's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Flash uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Flash registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``flash tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Flash registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple flash
