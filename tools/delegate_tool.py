...
# Must match flash_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``flash-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. flash-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.flash/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_flash_dir("cache/delegation", "delegation_cache")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'flash auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Flash config.
    ...
    ``HERMES_IGNORE_USER_CONFIG=1`` (``flash chat --ignore-user-config``) is
    ...
    if not prefer_legacy:
        try:
            from flash_cli.config import load_config_readonly
