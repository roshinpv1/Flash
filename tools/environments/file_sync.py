...
def iter_sync_files(container_base: str = "/root/.flash") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.flash to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, flash_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.flash/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="flash-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.flash/skills/a.md`` →
        ``~/.flash/skills/a.md``, a new remote file at
        ``/root/.flash/skills/b.md`` maps to ``~/.flash/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
