...
# Flash-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Flash-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Flash inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Flash-managed inference credential — Claude subscription auth is not a
    # working Flash provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Flash venv path instead of the project's own
# ``.venv`` — silently clobbering the Flash environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Flash venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_flash_internal_secret(key: str) -> bool:
    """Return True for Flash-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Flash-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`flash_subprocess_env` helper all call it, so the
...
def _inject_context_flash_home(env: dict) -> None:
    """Bridge the context-local Flash home override into subprocess env."""
    try:
        from flash_constants import get_flash_home_override

        value = get_flash_home_override()
        if value:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Flash-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_HERMES_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_flash_internal_secret(key):
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_HERMES_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_flash_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_flash_internal_secret(key):
            continue
        elif key not in _HERMES_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_flash_home(sanitized)
...
# legitimate child Flash spawns needs them, and they are the highest-value
...
# the conditional Tier-2 strip in flash_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_flash_internal_secret, but _ID has no secret suffix, so it must be
...
def flash_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Flash spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Flash-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_flash_internal_secret`.
    for key in list(env):
        if key.startswith(_HERMES_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_flash_internal_secret(key):
    ...
    _inject_context_flash_home(env)
    from flash_constants import apply_subprocess_home_env
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\flash\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\flash\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _flash_portable_git = os.path.join(_local_appdata, "flash", "git") if _local_appdata else ""
    if _flash_portable_git:
        for candidate in (
            os.path.join(_flash_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_flash_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Flash Agent requires Git for Windows on Windows.\n"
...
# Cached directory containing the ``flash`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_HERMES_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_flash_bin_dir() -> str | None:
    """Return the directory holding the ``flash`` console-script, or None.
    ...
    flash install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``flash``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``flash`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``flash`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("flash")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``flash`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "flash" or base.startswith("flash."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "flash.exe" if _IS_WINDOWS else "flash"
...
def _prepend_flash_bin_dir(existing_path: str) -> str:
    """Prepend the flash install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_flash_bin_dir()
    if not bin_dir:
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Flash
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_HERMES_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_flash_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_flash_internal_secret(k):
            continue
    ...
    if path_key is not None:
        ...
        # Ensure the flash install dir is reachable so plugins can shell out
        # to bare ``flash`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_flash_bin_dir(new_path)

    _inject_context_flash_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Flash trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = get_flash_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "flash_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._flash_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_flash_pgid", None)
                    if pgid is None:
                        raise
