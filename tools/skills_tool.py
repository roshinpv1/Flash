...
# Cache validation (mirrors flash_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.flash/skills/ (seeded from bundled skills/ on install).
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_flash_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.flash/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.flash/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``flash skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_flash_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from flash_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: flash plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.flash/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `flash skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.flash.* first (agentskills.io convention), fall back to top-level
        flash_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            flash_meta = metadata.get("flash", {}) or {}

        tags = _parse_tags(flash_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            flash_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
