...
`health_report` (surfaced by `flash computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Flash runs in).
_BLOCKED_KEY_COMBOS = {
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `flash computer-use install`. "
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of FlashOrg/flash-agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_flash_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `flash computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
