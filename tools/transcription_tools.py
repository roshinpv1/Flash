...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"flash-cmd-stt-{provider_name}-") as tmpdir:
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="flash-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `flash model` or set XAI_API_KEY",
        }
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    return managed_gateway.flash_user_token, urljoin(
