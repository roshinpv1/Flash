...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``nyxo tools`` repaint, tool-registration scans,
    ...
    2. ``~/.nyxo/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_nyxo_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.nyxo/.env`` first, then ``os.environ``.

    Wraps :func:`nyxo_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from nyxo_cli.config import get_env_value as _nyxo_get_env_value

        value = _nyxo_get_env_value(name)
...
def nyxo_xai_user_agent() -> str:
    """Return a stable Nyxo-specific User-Agent for xAI HTTP calls."""
    try:
        from nyxo_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Nyxo-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Nyxo-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``nyxo_cli.config.get_env_value`` so keys
    stored in ``~/.nyxo/.env`` (the standard Nyxo location) are honored —
