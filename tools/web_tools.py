...
Backend is selected during ``flash tools`` setup (web.backend in config.yaml).
When available, Flash can route Firecrawl calls through a Flash-hosted tool-gateway
for FlashSubscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for FlashSubscribers)
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Flash config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Flash' config/.env layer (``flash config set``, ``flash tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.flash/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``flash tools``).
    ...
    # pre-empted by a FlashOAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `flash tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    refresh against the Flashportal on every CLI startup (invoked at
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_flash_dir("cache/web", "web_cache")
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `flash plugins enable {disabled_key}` "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`flash plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from flash_cli.config import get_env_value as _gev
