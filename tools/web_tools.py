...
Backend is selected during ``nyxo tools`` setup (web.backend in config.yaml).
When available, Nyxo can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Nyxo config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Nyxo' config/.env layer (``nyxo config set``, ``nyxo tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nyxo/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nyxo tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `nyxo tools` repaint.
        try:
