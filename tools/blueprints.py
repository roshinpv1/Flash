...
`flash skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.flash.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.flash.blueprint`` is a mapping containing
    ...
    flash = meta.get("flash") if isinstance(meta, dict) else None
    blueprint = flash.get("blueprint") if isinstance(flash, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.flash.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.flash.blueprint`` block) they can hand
    to ``flash skills publish`` to share. ``body`` is the plain-language
