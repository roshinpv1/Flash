...
`nyxo skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.nyxo.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nyxo.blueprint`` is a mapping containing
    ...
    nyxo = meta.get("nyxo") if isinstance(meta, dict) else None
    blueprint = nyxo.get("blueprint") if isinstance(nyxo, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.nyxo.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.nyxo.blueprint`` block) they can hand
    to ``nyxo skills publish`` to share. ``body`` is the plain-language
