#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Flash Skills Hub.
...
Used by flash_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _flash_home() -> Path:
    return get_flash_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _flash_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            flash_meta = metadata.get("flash", {})
            if isinstance(flash_meta, dict):
                tags = flash_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "FlashOrg/flash-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                flash_meta = meta_block.get("flash", {})
                if isinstance(flash_meta, dict):
                    tags = flash_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Flash centralized index source
# ---------------------------------------------------------------------------

HERMES_INDEX_URL = "https://flash-agent.flashorg.com/docs/api/skills-index.json"
HERMES_INDEX_TTL = 6 * 3600  # 6 hours


def _flash_index_cache_file() -> Path:
    return _index_cache_dir() / "flash-index.json"


def _load_flash_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    flash_index_cache_file = _flash_index_cache_file()
    if flash_index_cache_file.exists():
        try:
            age = time.time() - flash_index_cache_file.stat().st_mtime
            if age < HERMES_INDEX_TTL:
                return json.loads(flash_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Flash index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Flash index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Flash index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        flash_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        flash_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class FlashIndexSource(SkillSource):
    """Skill source backed by the centralized Flash Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "flash-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "flash-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        FlashIndexSource(auth=auth), # Centralized index (search + resolved install paths)
