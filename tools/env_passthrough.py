...
def _is_flash_provider_credential(name: str) -> bool:
    """True if ``name`` is a Flash-managed provider credential (API key,
    ...
    Non-Flash API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Flash credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Flash-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_flash_internal_secret(name):
        return True
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Flash-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Flash-managed
    ...
    Non-Flash third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_flash_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Flash provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Flash-managed provider credentials must not be passed
                ...
                if _is_flash_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Flash "
