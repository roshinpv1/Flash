...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("NYXO_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NYXO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("NYXO_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("NYXO_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set NYXO_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind NYXO_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds NYXO_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("NYXO_CRON_SESSION"):
        return False
    if env_var_enabled("NYXO_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $NYXO_HOME, or by the resolved absolute
# active profile home path such as /home/nyxo/.nyxo/config.yaml. The
# resolved-absolute form is folded into the ~/.nyxo/ patterns at detection
...
# go stale when NYXO_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NYXO_ENV_PATH = (
    r'(?:~\/\.nyxo/|'
    r'(?:\$home|\$\{home\})/\.nyxo/|'
    r'(?:\$nyxo_home|\$\{nyxo_home\})/)'
    r'\.env\b'
)
# ~/.nyxo/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _NYXO_ENV_PATH; matches the NYXO_HOME override form as
# well as ~/.nyxo/.
_NYXO_CONFIG_PATH = (
    r'(?:~\/\.nyxo/|'
    r'(?:\$home|\$\{home\})/\.nyxo/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bnyxo\s+gateway\s+(stop|restart)\b', "stop/restart nyxo gateway (kills running agents)"),
    (r'\bnyxo\s+update\b', "nyxo update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `nyxo gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart nyxo-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart nyxo-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(nyxo|gateway|cli\.py)\b', "kill nyxo/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill nyxo` but not
    # `kill -9 $(pgrep -f nyxo)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Nyxo file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.nyxo/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Nyxo-managed security file (~/.nyxo/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_NYXO_CONFIG_PATH}|{_NYXO_ENV_PATH})', "in-place edit of Nyxo config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_NYXO_CONFIG_PATH}|{_NYXO_ENV_PATH})', "in-place edit of Nyxo config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_NYXO_CONFIG_PATH}|{_NYXO_ENV_PATH})', "in-place edit of Nyxo config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.nyxo/ form so the Nyxo config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/nyxo/.nyxo/config.yaml`) rather than
    # ~, $HOME, or $NYXO_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live NYXO_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_nyxo_home(command)
    return command
...
def _rewrite_resolved_nyxo_home(command: str) -> str:
    """Rewrite the resolved absolute Nyxo home prefix to ``~/.nyxo/``.

    Resolves the active ``NYXO_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.nyxo/`` so the static ``_NYXO_CONFIG_PATH`` / ``_NYXO_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from nyxo_constants import get_nyxo_home
        home = get_nyxo_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate NYXO_HOME (e.g. "/" or "") rewriting
        ...
        # /home/nyxo/.nyxo or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.nyxo/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["NYXO_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "NYXO_SPINNER_PAUSE" in os.environ:
            del os.environ["NYXO_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NYXO_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set NYXO_INTERACTIVE or NYXO_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("NYXO_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NYXO_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("NYXO_EXEC_ASK")
