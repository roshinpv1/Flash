...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``flash_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where flash previously hard-rejected an oversized read
...
_flash_config_resolved: str | None = None
_flash_config_resolved_loaded = False


def _get_flash_config_resolved() -> str | None:
    """Return the resolved absolute path of the Flash config file (cached)."""
    global _flash_config_resolved, _flash_config_resolved_loaded
    if _flash_config_resolved_loaded:
        return _flash_config_resolved
    _flash_config_resolved_loaded = True
    try:
        from flash_cli.config import get_config_path
        _flash_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _flash_config_resolved = str(Path(_expand_tilde("~/.flash/config.yaml")).resolve())
        except Exception:
            _flash_config_resolved = None
    return _flash_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Flash config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    flash_config = _get_flash_config_resolved()
    if flash_config and (resolved == flash_config or normalized == flash_config):
        return (
            f"Refusing to write to Flash config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.flash/config.yaml directly or use 'flash config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Flash mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Flash
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Flash state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.flash/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Flash process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.flash/…`` path.

    Returns ``None`` when the write is in-scope or outside Flash scope.
    ...
    # in a session that cd'd into ``~/.flash/profiles/other/`` is
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Flash-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Flash-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Flash profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Flash profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with flash_tools.write_file() for very "
            "large files."
        )
