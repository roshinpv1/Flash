...
This module provides the glue:
    - ``NyxoTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Nyxo Agent"
"""
...
from nyxo_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__nyxo_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses NYXO_HOME so each profile gets its own OAuth tokens.
    Layout: ``NYXO_HOME/mcp-tokens/``
    """
    try:
        from nyxo_constants import get_nyxo_home
        base = Path(get_nyxo_home())
    except ImportError:
        base = Path(os.environ.get("NYXO_HOME", str(Path.home() / ".nyxo")))
...
# ---------------------------------------------------------------------------
# NyxoTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class NyxoTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        NYXO_HOME/mcp-tokens/<server_name>.json         -- tokens
        NYXO_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NYXO_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Nyxo records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Nyxo.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://nyxo-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nyxo mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Nyxo Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`nyxo mcp login {server_name}` interactively first to complete "
