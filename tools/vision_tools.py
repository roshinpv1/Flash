...
Flash, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _normalize_to_supported_image(
    ...
    out_dir = get_flash_dir("cache/vision", "temp_vision_images")
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude — ``openrouter``, ``flash``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "flash", "vertex", "bedrock", "anthropic-vertex",
        "google-vertex",
    }
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_flash_dir("cache/vision", "temp_vision_images")
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider → openrouter → flash).
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Flash, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_flash_dir("cache/video", "temp_video_files")
