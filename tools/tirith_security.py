...
def _get_flash_home() -> str:
    """Return the Flash home directory, respecting HERMES_HOME env var."""
    return str(get_flash_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_flash_home(), ".tirith-install-failed")
...
def _flash_bin_dir() -> str:
    """Return $HERMES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_flash_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    try:
        ...
        dest = os.path.join(_flash_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    flash_bin = os.path.join(_flash_bin_dir(), "tirith")
    if os.path.isfile(flash_bin) and os.access(flash_bin, os.X_OK):
        _resolved_path = flash_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return flash_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchroflash; network download runs in a
    ...
    # Explicit path: synchroflash check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
