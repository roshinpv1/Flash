"""flash hooks — inspect and manage shell-script hooks.

Usage::

    flash hooks list
    flash hooks test <event> [--for-tool X] [--payload-file F]
    flash hooks revoke <command>
    flash hooks doctor

Consent records live under ``~/.flash/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.flash/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``flash hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: flash hooks {list|test|revoke|doctor}")
        print("Run 'flash hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.flash/config.yaml.")
        print("See `flash hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `flash hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `flash hooks test` and `flash hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Flash wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `flash hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `flash hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `flash hooks doctor`.")
