"""``flash dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.flash/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh FlashPortal access token from the existing login
     (``~/.flash/auth.json``), refreshing it if needed. Fails fast with a
     "run `flash setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.flash/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from flash_cli.auth import DEFAULT_FLASH_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``flash dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `flash auth add flash` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from flash_cli.config import get_env_path
    ...
    print(
        ...
        "  `flash dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    flash dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from flash_cli.config import get_env_value, is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `flash dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    default_portal = "https://portal.flashorg.com"
    ...
    # the portal, e.g. https://flash.example.com/auth/callback. At serve time
