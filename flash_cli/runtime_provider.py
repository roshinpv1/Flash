...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from flash_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from flash_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Flash' tool dispatch. Gated behind config key
...
def _flash_inference_base_url_override() -> str:
    """Return the trusted Flashruntime base URL override, if configured.

    Delegates to ``auth._flash_inference_env_override`` so every
    ``FLASH_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _flash_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = "chat_completions"
        base_url = _flash_inference_base_url_override() or base_url
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from flash_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from flash_cli.models import opencode_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/flash_cli/test_runtime_provider_resolution.py
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'flash model' or "
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "flash":
        ...
        base_url = (
            explicit_base_url
            or _flash_inference_base_url_override()
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("HERMES_FLASH_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_flash_runtime_credentials(
                timeout_seconds=float(_getenv("HERMES_FLASH_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
    ...
    *,
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "flash" and entry is not None:
            min_ttl = max(60, env_int("HERMES_FLASH_MIN_KEY_TTL_SECONDS", 1800))
            flash_state = {
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "flash-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from flash_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Flash canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Flash-compatible importers).
