"""
Flash Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``flash desktop`` / ``flash gui``)
     Built inside the agent checkout under ``$HERMES_HOME/flash-agent/``:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Flash.app`` or ``~/Applications/Flash.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Flash`` (NSIS per-user)
...
the app name ("Flash"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Flash``
  - Windows: ``%APPDATA%\\Flash``
  - Linux:   ``$XDG_CONFIG_HOME/Flash`` (default ``~/.config/Flash``)
...
``flash uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(flash_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return flash_home / "flash-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Flash"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Flash"
...
def source_built_gui_artifacts(flash_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``flash desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``flash-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(flash_home)
    ...
    return [
        ...
        flash_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Flash" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            ...
            home / "Applications" / "Flash.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Flash).
            local_base / "Programs" / "Flash",
            # Older / alternate layout some builds used.
            local_base / "flash-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Flash")
    else:
        ...
        paths += [
            data_base / "applications" / "flash.desktop",
            data_base / "applications" / "Flash.desktop",
        ]
    return paths


def agent_is_installed(flash_home: Path) -> bool:
    ...
    if (agent_root / "flash_cli").is_dir():
        return True
...
def gui_is_installed(flash_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(flash_home):
        if p.exists():
            return True
...
def gui_install_summary(flash_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = flash_home if flash_home is not None else get_flash_home()
...
def uninstall_gui(flash_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``flash-agent/flash_cli`` (agent source), ``venv/``, or any
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove flash' or "
            "'sudo dnf remove flash'). AppImage builds are a single file you "
