...
def get_flash_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `flash model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from flash_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline FlashPortal login for the Tool Gateway picker (`flash tools`)
# ---------------------------------------------------------------------------
...
def ensure_flash_portal_access(
    *,
    ...
    Used by ``flash tools`` when a user selects a Flash-managed Tool Gateway
    backend (e.g. "Firecrawl (FlashPortal)").  Unlike ``flash model``'s Flash
    login, this:
