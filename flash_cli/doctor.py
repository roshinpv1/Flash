"""
Doctor command for flash CLI.

Diagnoses issues with Flash Agent setup.
"""
...
from flash_cli.config import get_project_root, get_flash_home, get_env_path
from flash_cli.env_loader import load_flash_dotenv
from flash_constants import display_flash_home
from flash_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
HERMES_HOME = get_flash_home()
_DHH = display_flash_home()  # user-facing display path (e.g. ~/.flash or ~/.flash/profiles/coder)

# Load environment variables from ~/.flash/.env so API key checks work
_env_path = get_env_path()
load_flash_dotenv(flash_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from flash_cli.colors import Colors, color
from flash_cli.models import _HERMES_USER_AGENT
from flash_constants import OPENROUTER_MODELS_URL
...
from flash_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.flash/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from flash_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches flash_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``flash --version`` reporting a stale version while
    ...
    """
    try:
        from flash_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != flash_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'flash update', or set "
            "flash_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-flash and dashboard static services are up
    ...
    for static in ("main-flash", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `flash profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from flash_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `flash`.
    os.environ.setdefault("HERMES_INTERACTIVE", "1")

    # Handle `flash doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.flash/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Flash Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`flash doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and flash_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.flash/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'flash setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'flash setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'flash setup' to create one")
                issues.append("Run 'flash setup' to create .env")
    
    # Check ~/.flash/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from flash_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from flash_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'flash config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from flash_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.flash/.env or run 'flash setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'flash setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'flash config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from flash_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from flash_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'flash setup' to migrate config")
                else:
                    issues.append("Run 'flash doctor --fix' or 'flash setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'flash doctor --fix'")
        except Exception:
            pass
        ...
        try:
            import yaml
            from flash_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `flash setup` run)",
                )
                if should_fix:
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Flash' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = flash_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Flash a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Flash Agent Persona\n\n"
                "<!-- Edit this file to customize how Flash communicates. -->\n\n"
                "You are Flash, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = flash_home / "memories"
    ...
    state_db_path = flash_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from flash_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'flash doctor --fix' "
                        "(or 'flash sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from flash_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'flash doctor --fix' "
                        "(or 'flash sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = flash_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'flash doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "flash"
        ...
        _cmd_link = _cmd_link_dir / "flash"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(flash not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/flash → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/flash points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/flash → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/flash — run 'flash doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/flash exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/flash not found",
                    "(flash command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/flash → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/flash symlink — run 'flash doctor --fix'")
    ...
    try:
        from flash_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `flash update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `flash doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Flash one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'flash config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``flash doctor``.
    ...
    # never the legitimate source for `flash doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'flash setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: flash skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: flash memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: flash memory setup",
                    "No Honcho API key — run 'flash memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run flash memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: flash memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: flash memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from flash_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "flash -p" in content:
                            _m = _re.search(r"flash -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'flash doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
