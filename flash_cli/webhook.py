"""flash webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.flash/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _flash_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: flash gateway run
"""
...
def webhook_command(args):
    """Entry point for 'flash webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: flash webhook {subscribe|list|remove|test}")
        print("Run 'flash webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (flash gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: flash webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from flash webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (flash gateway run)")
