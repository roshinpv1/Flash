...
_TASK_NAME_DEFAULT = "Flash_Gateway"
_TASK_DESCRIPTION = "Flash Agent Gateway - Messaging Platform Integration"
...
def _preserve_flash_home_path(path: str | Path) -> str:
    """Render Flash-owned paths under the configured HERMES_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\flash`` as a symlink/junction to
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Flash profile."""
    from flash_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "flash_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``Flash_Gateway``
    Named profile X: ``Flash_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during flash_cli boot.
    from flash_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\flash\\gateway-service\\<task_name>.cmd``
    (or ``<HERMES_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Flash installs stay self-contained).
    """
    _assert_windows()
    from flash_cli.config import get_flash_home

    script_dir = Path(get_flash_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m flash_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "HERMES_HOME={flash_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports flash_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_flash_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_flash_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_flash_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [pythonw_path, "-m", "flash_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_flash_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_flash_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('HERMES_HOME')}) = {_quote_vbs_string(flash_home)}",
        ...
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_flash_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_flash_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    flash_home = str(Path(get_flash_home()))
    profile_arg = _profile_arg(flash_home)

    content = _build_gateway_cmd_script(python_path, working_dir, flash_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, flash_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Flash builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_flash_home_path(get_python_path())
    )
    project_root = _preserve_flash_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    flash_home = str(Path(get_flash_home()))
    profile_arg = _profile_arg(flash_home)

    argv = [python_exe, "-m", "flash_cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "HERMES_HOME": flash_home,
        "PYTHONIOENCODING": "utf-8",
        "HERMES_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_flash_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_flash_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    ``flash_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m flash_cli.main
    ...
    from flash_cli.config import get_flash_home
    from flash_cli.gateway import PROJECT_ROOT
    ...
    try:
        flash_home = str(Path(get_flash_home()).resolve())
    except Exception:
        flash_home = ""
    ...
    if flash_home:
        env_overlay["HERMES_HOME"] = flash_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m flash_cli.main gateway run``
    ...
    log_dir = Path(get_flash_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from flash_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `flash -p <profile> gateway install` must be safe.
    ...
    from flash_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"flash {profile_arg} gateway start" if profile_arg else "flash gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: flash gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Flash gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: flash gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: flash gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from flash_cli.config import get_flash_home
        print(f"    type {Path(get_flash_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_flash_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from flash_cli.config import get_flash_home

    flash_home = Path(get_flash_home())
    print()
    print("Next steps:")
    print("  flash gateway status                      # Check status")
    print(f"  type {flash_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Flash gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: flash gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: flash gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: flash gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `flash gateway status`."
        )
