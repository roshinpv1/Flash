"""Profile distributions — shareable, packaged Flash profiles via git.

A distribution is a Flash profile published as a git repository (or
...
* ``flash profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``flash skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``flash profile``, not a parallel tree):

    flash profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "flash-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            flash_requires=str(data.get("flash_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.flash_requires:
            out["flash_requires"] = self.flash_requires
        if self.author:
...
def check_flash_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Flash {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``flash profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Flash profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Flash distribution."
        )

    # Version check up-front so we fail fast
    check_flash_requires(manifest.flash_requires, flash_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.flash).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="flash_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `flash profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`flash profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="flash_dist_update_") as tmp:
        plan = plan_install(
