...
- ``flash_cli.providers``        -- canonical provider identity + overlays
- ``flash_cli.model_normalize``  -- per-provider name formatting
...
#   FlashOrg/Flash-3-Llama-3.1-70B, flash-4-405b, openrouter/flash3:70b
# Negative examples it must NOT match:
#   flash-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_FLASH_HERMES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])flash[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_flash_flash_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real FlashFlash 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_FLASH_HERMES_NON_AGENTIC_RE.search(model_name))


def _check_flash_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a FlashFlash 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``flash config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from flash_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'flash model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'flash doctor' — config issues detected:"
        ...
        from flash_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from flash_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from flash_cli.config import load_config
            from flash_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from flash_cli.providers import resolve_user_provider as _ruser
    ...
    # flash_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from flash_cli.models import normalize_opencode_base_url
    ...
    flash_warn = _check_flash_model_warning(new_model)
    if flash_warn:
        warnings.append(flash_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from flash_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from flash_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from flash_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    if "lmstudio" not in curated and (
        ...
        from flash_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Flash-mapped providers ---
    from flash_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from flash_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for flash_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(flash_id)
        if (
            _alias_target
            and _alias_target != flash_id
        ...
        pconfig = PROVIDER_REGISTRY.get(flash_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HERMES_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Flash may not route yet.
        ...
        from flash_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(flash_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(flash_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        flash_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `flash model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(flash_id)
        if not model_ids:
            model_ids = curated.get(flash_id, [])
            if flash_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(flash_id, model_ids)
    ...
    # Build reverse mapping: models.dev ID → Flash provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Flash IDs ("copilot").
    _mdev_to_flash = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HERMES_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Flash slug — e.g. "github-copilot" → "copilot"
        flash_slug = _mdev_to_flash.get(pid, pid)
        if flash_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(flash_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, flash_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or flash_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(flash_slug):
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", flash_slug, exc)
        ...
        # configured.
        if not has_creds and flash_slug == "anthropic":
            try:
                ...
                if (flash_creds and flash_creds.get("accessToken")) or \
        ...
        if flash_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(flash_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(flash_slug)
            ...
            if not model_ids:
                model_ids = curated.get(flash_slug, []) or curated.get(pid, [])
                if flash_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(flash_slug, model_ids)
        total = len(model_ids)
        if flash_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": flash_slug,
            "name": get_label(flash_slug),
            "is_current": flash_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "flash",
        ...
        seen_slugs.add(flash_slug.lower())
        _record_builtin_endpoint(flash_slug)
    ...
    # with `flash model`).
    try:
        from flash_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Flash's canonical write key (matches
            ...
            # Flash writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from flash_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Flash's own writer uses; a hyphen variant is accepted
            ...
            # active model. Flash's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, flash_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from flash_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`flash_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from flash_cli.models import fetch_openrouter_models
