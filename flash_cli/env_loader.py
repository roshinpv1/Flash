"""Helpers for loading Flash .env files consistently across entrypoints."""
...
# load_flash_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_flash_dotenv().
# Used by setup / `flash model` flows to label detected credentials so
...
# process.  ``load_flash_dotenv()`` is called at module-import time from
# several hot modules (cli.py, flash_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_flash_dotenv()`` call.  Returns
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `flash setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``flash_cli.config._sanitize_env_lines`` which
    already knows all valid Flash env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from flash_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_flash_dotenv(
    *,
    flash_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Flash environment files with user config taking precedence.

    Behavior:
    - `~/.flash/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(flash_home or os.getenv("HERMES_HOME", Path.home() / ".flash"))
    ...
    #   EnvironmentFile=-/path/to/.flash/.op.env
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Flash reads
    ...
    ``home_path`` are no-ops.  ``load_flash_dotenv()`` runs at import
    time from several hot modules (cli.py, flash_cli/main.py,
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `flash model`
