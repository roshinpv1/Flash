...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with flash-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.flash/plugins/<name>/``
3. **Project plugins** – ``./.flash/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``flash_agent.plugins``
   entry-point group.
...
from flash_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
# stderr in addition to ~/.flash/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Flash loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Flash' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by flash_cli.kanban_db when a task
    ...
    # separate `flash -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `flash kanban dispatch`),
...
ENTRY_POINTS_GROUP = "flash_agent.plugins"

_NS_PARENT = "flash_plugins"
...
class PluginManifest:
    ...
    #              in ~/.flash/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``flash plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Flash profile name (e.g. ``"default"``).

        Derived from ``HERMES_HOME`` via
        :func:`flash_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.flash/profiles/<name>``, or ``"custom"`` when
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Flash core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``flash honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``flash <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`flash_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_flash_dotenv()`` startup — after
        ``~/.flash/.env`` loads, before Flash reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Flash' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``flash model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: flash_cli.main imports plugins indirectly
        from flash_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.flash/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to flash_cli/. Two layouts are
        ...
        # 2. User plugins (~/.flash/plugins/)
        user_dir = get_flash_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.flash/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".flash" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with flash and must
            ...
            # all ~20 of them added several seconds to every `flash`
            # invocation — including plain `flash chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Flash ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `flash plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``flash plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`flash plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `flash plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``flash_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``flash_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="flash-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``flash tools`` TUI so plugin-provided toolsets appear
