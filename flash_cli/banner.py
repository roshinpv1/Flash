...
Pure display functions with no FlashCLI state dependency.
"""
import json
import logging
import os
...
from flash_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built flash — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/FlashOrg/flash-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/flashorg/flash-agent"
...
def _fetch_pypi_latest(package: str = "flash-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Flash update is available.
    ...
    cache_file = flash_home / ".update_check"
    ...
    # and `flash update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/flash/update/check`
    ...
    try:
        from flash_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $HERMES_HOME/flash-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = flash_home / "flash-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Flash git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$HERMES_HOME/flash-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``flash_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from flash_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/FlashOrg/flash-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Flash checkout. Cached per-process. Release URL always points at the
    canonical FlashOrg/flash-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Flash Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Flash Org[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Flash Org[/]")
    ...
    try:
        from flash_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from flash_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built flash; we know an update
