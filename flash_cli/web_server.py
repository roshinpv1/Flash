"""
Flash Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m flash_cli.main web          # Start on http://127.0.0.1:9119
    python -m flash_cli.main web --port 8080
"""
...
from flash_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `flash dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``flash gateway run`` — but the
    desktop app spawns a ``flash dashboard`` backend, not a gateway, so a cron
...
def _warm_gateway_module() -> None:
    try:
        import flash_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from flash_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire flash_cli.gateway import into a background thread so the event
    ...
    # since the app has no gateway running the scheduler. Server `flash
...
app = FastAPI(title="Flash Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from flash_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
_SESSION_HEADER_NAME = "X-Flash-Session-Token"
...
#
# This list is defined in ``flash_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from flash_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Flash-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``flash-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from flash_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from flash_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from flash_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Flash (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during flash update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``flash model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Flash provider slug — fine.
    ...
    1. Vendor-name → Flash-provider mapping: when the provider string is
       not a known Flash provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from flash_cli.config import get_compatible_custom_providers
    from flash_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from flash_cli.model_normalize import normalize_model_for_provider
    from flash_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from flash_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another Flash process is writing to it.
    """
    from flash_state import DEFAULT_DB_PATH, SessionDB
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Flash credential-store basenames (see
...
def _default_flash_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_flash_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``flash update``.
    ...
    a container — e.g. the flash-webui image sharing the Flash source tree),
    the dashboard's ``flash update`` button is the correct update path and
    ...
    """
    if _default_flash_root_is_opt_data():
        return True
    try:
        from flash_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Flash root is actually /opt/data
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Flash inside an xterm.js PTY. Browser
    ...
    with _profile_scope(profile) as scoped_home:
        home = scoped_home or get_flash_home()
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``flash:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.flash-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``flash_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from flash_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from flash_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_flash_home() at call time
    ...
    try:
        ...
        # Windows install the first import of flash_cli.gateway blocks the
        ...
        # and which providers are registered so ``flash status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Flash Org" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from flash_cli.dashboard_auth import list_providers as _list_providers
        ...
        flash_session_valid = "unknown"
        try:
            from flash_cli.auth import get_flash_session_validity
            flash_session_valid = get_flash_session_validity()
        except Exception:
            flash_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_flash": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "flash_session_valid": flash_session_valid,
        }
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Flash Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the flash home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_flash_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_flash_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    return {
        ...
        "subscription_url": "https://portal.flashorg.com/manage-subscription",
        "features": features,
    }
...
async def run_prompt_size():
    try:
        proc = _spawn_flash_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_flash_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_flash_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchroflash*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from flash_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.flash/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_flash_home() / "logs"
...
def _spawn_flash_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``flash <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``flash_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "flash_cli.main", *subcommand]
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["flash", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``flash gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``flash gateway restart`` children race each other on the
    ...
    return _spawn_flash_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``flash gateway restart`` in the background."""
    try:
...
async def update_flash():
    """Kick off ``flash update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Flash updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("flash-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "flash-update",
    ...
    try:
        proc = _spawn_flash_action(["update"], "flash-update")
    except Exception as exc:
        _log.exception("Failed to spawn flash update")
...
@app.get("/api/flash/update/check")
async def check_flash_update(force: bool = False):
    """Report whether a Flash update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/flash/update`` actually runs ``flash update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Flash version string
    ...
    try:
        ...
        if force:
            try:
                (get_flash_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.flash/config.yaml`` under ``tts.``.
    """
...
def get_profiles_sessions(
    ...
    from flash_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Flash surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Flash supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        flash_home / f"{name}.json",
        flash_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_flash_home()))
            return
...
# in flash_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from flash_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.flash/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from flash_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "flash":
            try:
                from flash_cli.flash_subscription import apply_flash_managed_defaults
                from flash_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``flash model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from flash_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``flash model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from flash_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from flash_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `flash model` provider — would be
        ...
        # Vertex — a `flash model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `flash model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Flash from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Flash to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Flash from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Flash to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Flash in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Flash through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Flash via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Flash through an IMAP/SMTP mailbox.",
        "docs_url": "https://flash-agent.flashorg.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Flash to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Flash inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Flash to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://flash-agent.flashorg.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://flash-agent.flashorg.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Flash through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Flash to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://flash-agent.flashorg.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Flash to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Flash as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://flash-agent.flashorg.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://flash-agent.flashorg.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in flash gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in flash gateway setup",
...
def _whatsapp_session_path() -> Path:
    from flash_constants import get_flash_dir

    return get_flash_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.flash-agent.flashorg.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"FlashDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Flash Agent").strip() or "Flash Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Flash appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.flash/.anthropic_oauth.json`` — Flash-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_flash_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_flash_oauth_credentials,
            _get_flash_oauth_file,
        )
    except ImportError:
        read_flash_oauth_credentials = None  # type: ignore
        _get_flash_oauth_file = None  # type: ignore

    flash_creds = None
    if read_flash_oauth_credentials:
        try:
            flash_creds = read_flash_oauth_credentials()
        except Exception:
            flash_creds = None
    if flash_creds and flash_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "flash_pkce",
            "source_label": f"Flash PKCE ({_get_flash_oauth_file() if _get_flash_oauth_file else None})",
            "token_preview": _truncate_token(flash_creds.get("accessToken")),
            "expires_at": flash_creds.get("expiresAt"),
            "has_refresh_token": bool(flash_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Flash even
    when they also have a separate Flash-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Flash never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "flash auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "flash auth add qwen-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "flash auth add xai-oauth",
        "docs_url": "https://flash-agent.flashorg.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "flash auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from flash_cli import auth as hauth
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Flash, so the disconnect
    ...
    user sees exactly what executes, and Flash then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Flash — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``flash model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `flash model` universe (zero-edit for new plugins).
    try:
        from flash_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"flash auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("flash_pkce", "claude_code", ...)
    ...
    sync with the `flash model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Flash-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_flash_oauth_file
                oauth_file = _get_flash_oauth_file()
            ...
            try:
                from flash_cli.auth import clear_provider_auth
        ...
        try:
            from flash_cli.auth import clear_provider_auth, invalidate_flash_auth_status_cache
...
# Guarded so flash web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Flash file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``flash auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_flash_oauth_file
    oauth_file = _get_flash_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "flash-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from flash_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``flash auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``flash model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Flash "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from flash_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `flash sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `flash sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_flash_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from flash_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_flash_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from flash_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_flash_home_override(str(home))
    try:
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Flash home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (flash_cli.mcp_config), so
# servers managed here show up under `flash mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from flash_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from flash_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from flash_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from flash_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from flash_cli.mcp_config import (
        _get_mcp_servers,
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``flash mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import FlashTokenStorage, force_interactive_oauth
        ...
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = FlashTokenStorage(name)
...
#
# Wraps the same JSON store the CLI uses (flash_cli.webhook); the webhook
...
async def list_webhooks():
    import flash_cli.webhook as wh
...
# spawn the real `flash gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_flash_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_flash_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from flash_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_flash_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_flash_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_flash_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"flash-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_flash_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `flash import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_flash_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from flash_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from flash_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <flash_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_flash_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_flash_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``flash`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_flash_action`` tracks one process/log per name, so a shared
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from flash_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`flash -p <name> skills install`)
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_flash_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "flash setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from flash_constants import set_flash_home_override, reset_flash_home_override

    token = set_flash_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from flash_constants import set_flash_home_override, reset_flash_home_override
    from flash_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from flash_constants import set_flash_home_override, reset_flash_home_override
    from flash_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``flash profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``flash profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``flash profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from flash_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_flash_home()`` at call
       time — the context-local override from ``set_flash_home_override``
    ...
    retargeted to the *current* ``get_flash_home()`` so writes land in the
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_flash_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_flash_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from flash_cli.skills_config import get_disabled_skills
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``flash tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``flash tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `flash tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``flash tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.flash/.env`` via ``save_env_value`` —
    the same store ``flash tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``flash tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``flash -p <profile> tools post-setup``.
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Flash,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``flash computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``flash --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from flash_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from flash_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from flash_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from flash_cli.dashboard_auth.audit import AuditEvent, audit_log
        from flash_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``flash --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HERMES_TUI_RESUME`` env var —
    matching what ``flash_cli.main._launch_tui`` does for the CLI path.
    ...
    ``get_flash_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``flash -p <name>`` performs. The in-process
    ...
    """
    from flash_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from flash_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="flash-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Flash Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Flash CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="flash-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_flash_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from flash_cli.console_engine import FlashConsoleEngine

        engine = FlashConsoleEngine(
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Flash Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Flash inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`flash_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from flash_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/flash/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /flash`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HERMES_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `flash serve` is the headless backend: it must NEVER serve the browser
    ...
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (flash serve): web UI disabled — use "
            "`flash dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/flash``)
    ...
    # under ``/flash`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Flash backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.flash/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.flash/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_flash_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.flash/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as flash_cli.plugins):
    1. User plugins:    ~/.flash/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.flash/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from flash_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_flash_home() / "plugins", "user"),
    ...
    # ``flash_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".flash" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from flash_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_flash_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"flash auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from flash_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from flash_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from flash_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from flash_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.flash/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"flash_dashboard_plugin_{plugin['name']}"
...
from flash_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    # the flash-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Flash Agent exposes a
    ...
    # as agent turns, and a single synchroflash GIL-holding call on a worker
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Flash backend listening on {host}:{actual_port}")
            else:
                print(f"  Flash Web UI → http://{host}:{actual_port}")
