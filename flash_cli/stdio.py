...
on Japanese locales, etc.).  Flash's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``flash_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Flash is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Flash-managed Git install directories so
    ...
    # already-running shells don't see the change — which means flash
    ...
    # install without Flash-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Flash's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Flash's
    ``flash config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Flash (User env var in Windows Settings, or
    export in a PowerShell profile) and Flash picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Flash-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\flash\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``flash`` from right after install) retain their old PATH.

    Any subprocess Flash spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Flash-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "flash", "git", "cmd"),
        os.path.join(local_appdata, "flash", "git", "bin"),
        os.path.join(local_appdata, "flash", "git", "usr", "bin"),
        # Flash venv Scripts directory — host of the flash.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes flash, but harmless to include.
        os.path.join(local_appdata, "flash", "flash-agent", "venv", "Scripts"),
