...
from flash_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from flash_cli import __version__ as _HERMES_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _HERMES_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"flash-cli/{_HERMES_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://flash-agent.flashorg.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://flash-agent.flashorg.com/docs/guides/oauth-over-ssh"
...
def get_anthropic_key() -> str:
    ...
    ``~/.flash/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from flash_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from flash_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.flash/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``flash auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `flash model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.flash/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_flash_home() / "auth.json"
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".flash" / "auth.json").resolve(strict=False)
        try:
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_flash_root()
    except Exception:
        return None
    profile_home = get_flash_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.flash/auth.json`` even when HERMES_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_flash_root()`` for a profile-shaped HERMES_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".flash" / "auth.json"
            try:
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_flash_portal_url(raw["providers"])
        return raw
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``flash auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``flash auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``flash auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Flash.
    ...
    # the user initiated inside Flash (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "flash_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `flash logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from flash_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'flash doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'flash model' for available providers, or run 'flash doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `flash auth add openrouter`
    ...
    # `flash auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.flash/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'flash model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.flash/.env.",
...
def _migrate_stale_flash_portal_url(providers: Dict[str, Any]) -> None:
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `flash model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.flash/auth.json
# =============================================================================
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `flash auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `flash auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `flash auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `flash auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.flash/.env, and return it.
    ...
    print("       App name:     anything (e.g. flash-agent)")
    ...
    # Persist so subsequent `flash auth spotify` runs skip the wizard.
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.flash/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Flash:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Flash is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.flash/auth.json (not ~/.codex/)
#
# Flash maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Flash auth store (~/.flash/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `flash auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `flash auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `flash auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `flash auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``flash setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``flash auth add openai-codex``
      ...
      ``flash auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Flash auth store (~/.flash/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``flash auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Flash auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Flash auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run flash auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `flash auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Flash auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Flash keeps its OWN
        ...
        # the Codex CLI (or another Flash process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Flash's own Codex token store.
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Flash processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.flash/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `flash model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `flash model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `flash model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `flash model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.flash/auth.json even when HERMES_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``flash auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.flash/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `flash model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `flash model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Flash (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `flash model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``flash model``
...
#
# File lives at ${HERMES_SHARED_AUTH_DIR}/flash_auth.json, defaulting to
# ``<flash-root>/shared/flash_auth.json`` where ``<flash-root>`` is what
# ``get_default_flash_root()`` returns — ``~/.flash`` on Linux/macOS,
# ``%LOCALAPPDATA%\flash`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's HERMES_HOME so named profiles (which
# typically live under ``<flash-root>/profiles/<name>/``) all see the
# same file.
#
...
def _flash_shared_auth_dir() -> Path:
    ...
    ``<flash-root>/shared/``, where ``<flash-root>`` is what
    :func:`flash_constants.get_default_flash_root` returns — so
    Linux/macOS classic installs land at ``~/.flash/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\flash\\shared\\``, and
    ...
    from flash_constants import get_default_flash_root
    return get_default_flash_root() / "shared"
...
def _quarantine_flash_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Flash
...
def _refresh_access_token(
    *,
    ...
    # retires the original RT, Flash's next refresh uses it, and the whole
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Flash models — they're not reliable for agentic tool-calling
...
def resolve_flash_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        # no override is set.
        env_portal_override = _flash_portal_env_override()
        ...
        with _flash_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "flash", state, state_source_path)
                return access_token
...
def persist_flash_credentials(
    ...
    *,
    ...
    Historically ``flash auth add flash`` wrote a ``manual:device_code`` pool
    ...
    ``flash auth add flash --label <name>``).  It gets embedded in the
    ...
    # these credentials via `flash auth add flash --type oauth`. Best-
...
# which does a synchroflash OAuth refresh POST to portal.flashorg.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`flash tools`, status panels,
# subscription-feature checks) call it many times per render — `flash tools` → "All Platforms"
...
# Enum values reported on the dashboard /api/status as ``flash_session_valid``.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `flash auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `flash auth` and
    # `flash model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``flash doctor`` runs the live
    ...
    try:
        from flash_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Flash' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `flash doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from flash_cli.copilot_auth import (
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from flash_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `flash logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from flash_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from flash_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'flash model' or 'flash setup' instead."""
    print("The 'flash login' command has been removed.")
    print("Use 'flash auth' to manage credentials,")
    print("'flash model' to select a provider, or 'flash setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.flash/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Flash-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Flash auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Flash will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Flash will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Flash gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Flash creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Flash auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Flash auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Flash creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``flash auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``flash auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Flash auth store (~/.flash/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Flash login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `flash model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``flash status`` that just want to know
...
def _flash_device_code_login(
    *,
    ...
    print(f"Starting Flash login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `flash model` again to finish setup.")
            raise SystemExit(1)
        raise
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Flash builds still see
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Flash will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `flash model` or configure an API key to use Flash.")
        else:
