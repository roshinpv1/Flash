...
Pure logic: walks a Flash config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `flash doctor` and a future `flash migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Flash config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Flash config and return retirement issues.
...
def apply_migration(
    ...
    from flash_cli.config import require_readable_config_before_write
