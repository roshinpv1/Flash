...
the tool registry or launching Flash (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``flash-cli`` to the non-existent ``flash``. ``resolve_toolset('flash')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``flash-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``flash`` instead of ``flash-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `flash tools` to reconfigure."
        )
    return warnings
