"""
Flash Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.flash/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Flash PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing flash-agent or flash PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Flash Agent" comment and following line
                if '# Flash Agent' in line or '# flash-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('flash' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing flash
                if 'flash' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the flash wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "flash",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains flash_cli reference)
                content = wrapper.read_text()
                if 'flash_cli' in content or 'flash-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(flash_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``flash`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Flash home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (flash_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``flash gateway run`` processes
    ...
    try:
        from flash_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from flash_cli.gateway import get_launchd_plist_path
...
#      like ``%LOCALAPPDATA%\flash\git\cmd``, ``%LOCALAPPDATA%\flash\git\bin``,
#      ``%LOCALAPPDATA%\flash\git\usr\bin``, ``%LOCALAPPDATA%\flash\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\flash\git\`` and Node to
#      ``%LOCALAPPDATA%\flash\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``flash dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\flash\gateway-service\`` and sometimes
...
def _flash_path_markers(flash_home: Path) -> list[str]:
    """Path-entry substrings that identify Flash-owned User-PATH entries."""
    root = str(flash_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare flash-agent install dir.
    markers = [root + "\\flash-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HERMES_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "flash".  We don't want to catch
    # unrelated entries like "cflash-foo" or "ephermeral", so we look for
    # backslash-flash as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(flash_home: Path) -> list[str]:
    """Strip Flash-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _flash_path_markers(flash_home)
            kept: list[str] = []
...
def remove_flash_env_vars_windows() -> list[str]:
...
def _is_default_flash_home(flash_home: Path) -> bool:
    """Return True when ``flash_home`` points at the default (non-profile) root."""
    try:
        from flash_constants import get_default_flash_root
        return flash_home.resolve() == get_default_flash_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``flash -p <name> gateway stop|uninstall`` because
    ...
    #    Use `python -m flash_cli.main` so we don't depend on a `flash`
    #    wrapper that may be half-removed mid-uninstall.
    flash_invocation = [_sys.executable, "-m", "flash_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                flash_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    Mirrors ``flash uninstall --gui``. Removes the desktop app's built
    ...
    """
    from flash_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(flash_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Flash Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Flash Chat GUI installation was found.")
        print(f"  Checked: {flash_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Flash agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(flash_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Flash agent at {flash_home / 'flash-agent'}")
        print(f"  • Your config, sessions, and secrets under {flash_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Flash agent is still installed. Run 'flash' to use the CLI,")
    print("or 'flash uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.flash/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.flash/ for future reinstall
    """
    ...
    is_default_profile = _is_default_flash_home(flash_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.flash data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Flash Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {flash_home / 'config.yaml'}")
    print(f"  Secrets: {flash_home / '.env'}")
    print(f"  Data:    {flash_home / 'cron/'}, {flash_home / 'sessions/'}, {flash_home / 'logs/'}")
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Flash data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Flash code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, flash_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Flash PATH entries from shell configs / Windows User PATH")
    print("  • Flash wrapper scripts and Flash-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Flash config/data: {flash_home}")
        if _is_default_flash_home(flash_home):
        ...
        print(f"  • Keep Flash config/data: {flash_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``flash`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in flash_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\flash\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(flash_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Flash-owned PATH entries in User environment")

        log_info("Removing HERMES_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_flash_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Flash-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing flash command...")
    ...
    #     (only when they still point into this Flash home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Flash-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(flash_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Flash-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(flash_home) would sweep the in-tree artifacts anyway,
    ...
    try:
        from flash_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(flash_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.flash/, just remove the flash-agent subdir
            if flash_home in project_root.parents or project_root.parent == flash_home:
    ...
    #     the step-5 rmtree(flash_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(flash_home)
    ...
    # 5. Optionally remove ~/.flash/ data directory (and named profiles)
    if full_uninstall:
        ...
        try:
            if flash_home.exists():
                shutil.rmtree(flash_home)
                log_success(f"Removed {flash_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {flash_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {flash_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://flash-agent.flashorg.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://flash-agent.flashorg.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Flash Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m flash_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import flash_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``flash_constants`` + ``flash_cli.colors``
    (and lazily ``flash_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m flash_cli.uninstall")
    parser.add_argument(
        "--mode",
