...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``flash_cli.relaunch`` should carry over when
...
Examples:
    ...
    flash console                Open the safe Flash command console
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="flash",
        description="Flash Agent - AI assistant with tool-calling capabilities",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Flash Agent",
    )
    ...
    # top-level parser: when the user writes `flash -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/flash_cli/
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Flash itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
