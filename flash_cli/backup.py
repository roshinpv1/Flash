"""
Backup and import commands for flash CLI.

`flash backup` creates a zip archive of the entire ~/.flash/ directory
(excluding the flash-agent repo and transient files).

`flash import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
from flash_constants import get_default_flash_root, get_flash_home, display_flash_home
...
# ``flash-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/flash-agent/``
...
_EXCLUDED_DIRS = {
    "flash-agent",     # the codebase repo — re-clone instead
...
# File names that ``flash import`` must never overwrite, matched by basename so
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to flash root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``flash-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/flash-agent/`` — must be preserved.
        if part == "flash-agent" and part != parts[0]:
            continue
        return True
...
def run_backup(args) -> None:
    """Create a zip backup of the Flash home directory."""
    flash_root = get_default_flash_root()

    if not flash_root.is_dir():
        print(f"Error: Flash home directory not found at {flash_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"flash-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"flash-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_flash_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(flash_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(flash_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``flash-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "flash-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(flash_root)
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_flash_home()}."
        )
    ...
    print(f"\nRestore with: flash import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Flash backup.
    ...
    # Look for telltale files that a flash home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Flash backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.flash/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a flash dir name
        if prefix in {".flash", "flash"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_flash_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (flash_root / "config.yaml").exists()
        has_env = (flash_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Flash configuration.")
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(flash_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_flash_home()}."
            )
        ...
        profiles_dir = flash_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # flash_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: flash profile list  (after installing flash)")

        # Guidance
        print()
        if not (flash_root / "flash-agent").is_dir():
            print("Note: The flash-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: flash update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  flash -p {pname} gateway install")

        print("Done. Your Flash configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and flash backup --quick)
# ---------------------------------------------------------------------------
...
# — `flash update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(flash_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(flash_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``flash update``.
    ...
    Args:
        ...
        flash_home: Override for the Flash home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(flash_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for flash backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_flash_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, flash_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``flash_root`` to ``out_path``.
...
def _pre_update_backup_dir(flash_home: Optional[Path] = None) -> Path:
    home = flash_home or get_flash_home()
...
def create_pre_update_backup(
    ...
    (``flash update``) should continue even if the backup fails.
    """
    flash_root = flash_home or get_default_flash_root()
    if not flash_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(flash_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, flash_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `flash claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `flash import` and the
