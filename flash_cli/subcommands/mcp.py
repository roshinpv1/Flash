...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Flash as an MCP server",
        description=(
            "Manage MCP server connections and run Flash as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'flash mcp add' to connect to a new server, or\n"
            "'flash mcp serve' to expose Flash conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Flash as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `flash mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `flash mcp install n8n`)",
    )
