"""``flash gateway`` and ``flash proxy`` subcommand parsers.
...
from flash_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy flash.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Flash gateway unit files "
            "(e.g. flash.service) left over from older installs. Profile "
            "units (flash-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.flash/.env. "
            "Requires being logged in (flash setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.flash/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="flash",
        help="Upstream provider: flash or xai (default: flash). See `flash proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
