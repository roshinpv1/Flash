...
billing screens drive. Companion to ``flash_cli/flash_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Flash already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``HERMES_PORTAL_BASE_URL`` → ``FLASH_PORTAL_BASE_URL`` → the
...
DEFAULT_PORTAL_BASE_URL = "https://portal.flashorg.com"
...
# flash_cli.auth.FLASH_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``HERMES_PORTAL_BASE_URL`` → ``FLASH_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("HERMES_PORTAL_BASE_URL") or os.getenv("FLASH_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into FlashPortal — run `flash portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
