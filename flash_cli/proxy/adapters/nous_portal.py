...
Reads the user's FlashOAuth state from ``~/.flash/auth.json`` through the
...
# Endpoints inference-api.flashorg.com actually serves. Anything else
...
class FlashPortalAdapter(UpstreamAdapter):
    ...
    def __init__(self) -> None:
        ...
        # and persistence are handled by resolve_flash_runtime_credentials().
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into FlashPortal. Run `flash auth add flash` first."
                )

            try:
                refreshed = resolve_flash_runtime_credentials(
    ...
    # to flash_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
