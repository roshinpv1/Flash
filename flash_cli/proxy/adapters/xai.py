...
# xAI's public API is OpenAI-compatible for the endpoints Flash commonly
# uses. The Responses endpoint is included because Flash' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Flash-managed OAuth credentials."""

    auth_hint = "flash auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`flash auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`flash auth reset xai-oauth` or re-authenticate with "
                    "`flash auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `flash auth add xai-oauth --type oauth`."
            )
