"""`flash checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.flash/checkpoints/``.  Actions:

    flash checkpoints               # same as `status`
    flash checkpoints status        # total size, project count, breakdown
    flash checkpoints list          # per-project checkpoint counts + workdir
    flash checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    flash checkpoints clear [-f]    # nuke the entire base (asks first)
    flash checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    flash checkpoints
    flash checkpoints prune --retention-days 3 --max-size-mb 200
    flash checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: flash checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``flash checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `flash checkpoints` → status
