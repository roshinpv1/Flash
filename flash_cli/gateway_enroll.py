"""``flash gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.flash/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or FlashPortal).
    ...
    self-hosted-IdP path; otherwise FlashPortal). Raises RuntimeError on failure.
    """
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `flash gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO FlashPortal); otherwise the FlashPortal token.
    try:
    ...
    #    save_env_value writes them to ~/.flash/.env (0600 dir) and never logs.
    to_write = {
