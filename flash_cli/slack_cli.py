"""``flash slack ...`` CLI subcommands.

Today only ``flash slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ flash slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Flash. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Flash deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from flash_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Flash in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Flash agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``flash_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$HERMES_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Flash")
    ...
    """
    name = getattr(args, "name", None) or "Flash"
    description = getattr(args, "description", None) or "Your Flash agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(get_flash_home()) / "slack-manifest.json"
            except Exception:
                target = Path(os.environ.get("HERMES_HOME") or str(Path.home() / ".flash")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Flash app\n"
