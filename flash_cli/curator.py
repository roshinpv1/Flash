"""CLI subcommand: `flash curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchroflash:
        print("llm pass running in background — check `flash curator status` later")
    if dry:
        if synchroflash:
            print(
                "dry-run: no changes applied. Read the report with "
                "`flash curator status` and run `flash curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`flash curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.flash/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`flash curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.flash/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.flash/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.flash/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by flash_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="flash curator")
    register_cli(parser)
