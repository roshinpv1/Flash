"""``flash logs`` — view and filter Flash log files.
...
Usage examples::
    ...
    flash logs                    # last 50 lines of agent.log
    flash logs -f                 # follow agent.log in real time
    flash logs errors             # last 50 lines of errors.log
    flash logs gateway -n 100    # last 100 lines of gateway.log
    flash logs gui -f            # follow gui.log (dashboard/pty/ws)
    flash logs desktop -f        # follow desktop.log (Electron app boot/backend)
    flash logs --level WARNING    # only WARNING+ lines
    flash logs --session abc123   # filter by session ID substring
    flash logs --component tools  # only tool-related lines
    flash logs --since 1h         # lines from the last hour
    flash logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_flash_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Flash runs — try 'flash chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_flash_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_flash_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_flash_home()}/logs/")
        return

    print(f"Log files in {display_flash_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'flash chat' to generate logs)")
