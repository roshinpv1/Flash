"""
Profile management for multiple isolated Flash instances.

Each profile is a fully independent HERMES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.flash/profiles/<name>/`` by default.

The "default" profile is ``~/.flash`` itself — backward compatible,
zero migration needed.

Usage::

    flash profile create coder          # fresh profile + bundled skills
    flash profile create coder --clone  # also copy config, .env, SOUL.md, skills
    flash profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    flash -p coder chat                 # or via flag
    flash profile use coder             # set as sticky default
    flash profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See flash_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `flash profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `flash
...
# `flash skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Flash infrastructure directories that only the default profile
         (``~/.flash``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_flash_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.flash) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "flash-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# into the export. The set below identifies the *known Flash profile
...
# (#58394). Add new artifacts here when introduced in ``flash_constants``.
...
_RESERVED_NAMES = frozenset({
    "flash", "default", "test", "tmp", "root", "sudo",
})

# Flash subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the flash root, NOT to the current HERMES_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HERMES_HOME points outside
    ``~/.flash``, profiles live under ``HERMES_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_flash_home() / "profiles"


def _get_default_flash_home() -> Path:
    """Return the default (pre-profile) HERMES_HOME path.

    In standard deployments this is ``~/.flash``.
    In Docker/custom deployments where HERMES_HOME is outside ``~/.flash``
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_flash_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Flash installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    ...
    if canon == "default":
        return _get_default_flash_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, flash subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a flash subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            flash_exe = shutil.which("flash") or "flash"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(flash_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Flash. If we
    ...
    try:
        ...
        # not be migrated. The next `flash doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``flash -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``flash profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``flash -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``flash profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Flash config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, flash_home)`` pairs a gateway should serve.
    ...
    The returned ``flash_home`` is the path to pass to
    ``set_flash_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_flash_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``flash update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.flash)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.flash/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `flash -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Flash profile.\n"
    ...
    if not soul_path.exists():
        try:
            from flash_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `flash update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`flash profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Flash (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `flash profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `flash -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``flash profile create
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_flash_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Flash *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Flash backend subcommand
    ...
    # Never terminate ourselves or a parent (e.g. `flash -p <canon> profile
    ...
    flash_markers = ("flash_cli.main", "flash-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Flash process: either an entrypoint marker in argv, or
            # a resolved executable named `flash`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_flash = (
                any(marker in joined for marker in flash_markers)
                or exe_name == "flash"
                or exe_name.startswith("flash")
            )
            if not is_flash:
                continue
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.flash).\n"
            "To remove everything, use: flash uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_flash_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from flash_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from flash_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.flash/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: flash profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to ``~/.flash``.
    Returns the profile name if HERMES_HOME points into ``~/.flash/profiles/<name>``.
    Returns ``"custom"`` if HERMES_HOME is set to an unrecognized path.
    """
    from flash_constants import get_flash_home
    flash_home = get_flash_home()
    resolved = flash_home.resolve()

    default_resolved = _get_default_flash_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      first and proved unable to anticipate every non-Flash file the
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.flash itself — its parent is ~/ and its
        # directory name is ".flash", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: flash profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.flash itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.flash). "
            "Specify a different name: flash profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="flash_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"flash_{old_name}"
    legacy_old_host = f"flash.{old_name}"
    new_host = f"flash_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_flash_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("flash_"):
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any flash modules
