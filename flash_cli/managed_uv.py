...
Flash owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
def managed_uv_path() -> Path:
    """Return the path where Flash keeps *its* uv binary.
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``flash update`` runs
    the call site from the *old*, already-imported ``flash_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``flash update`` so the managed copy stays current.
