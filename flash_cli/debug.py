"""``flash debug`` debug tools for Flash Agent.
...
def _pending_file() -> Path:
    ...
    """
    return get_flash_home() / "pastes" / "pending.json"
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "flash-agent/debug-share"},
    )
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "flash-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----FlashDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from flash_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_flash_home() / "logs" / filename) if filename else None
...
def run_debug_share(args):
    ...
    print("To delete now:  flash debug delete <url>")
