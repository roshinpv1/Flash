"""
Gateway subcommand for flash CLI.

Handles: flash gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_flash_home is imported lazily at call sites to avoid ImportError
# when flash_constants is cached from a pre-update version during `flash update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "flash CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``flash gateway status`` from falsely counting the ``flash`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``flash gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_flash_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"flash_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "flash_home=" in command_lc
            and f"flash_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``flash update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Flash profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Flash profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from flash_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "flash_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m flash_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``flash update`` users
    # with no running gateway until they re-invoke ``flash gateway``
    ...
    from flash_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from flash_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import flash_cli.  Empty on POSIX, where
...
def _flash_home_from_systemd_unit_file(system: bool = False) -> str | None:
...
def _sync_flash_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HERMES_HOME``.

    Under ``sudo``, ``HERMES_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_flash_home` falls back to ``/root/.flash`` — the wrong
    ...
    unit_home = (_flash_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}flash gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}flash gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from flash_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: flash gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `flash gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``flash gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from flash_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (flash_cli/gateway.py restart
...
from flash_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Flash Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``flash gateway run`` must remain interruptible from
...
_SERVICE_BASE = "flash-gateway"
SERVICE_DESCRIPTION = "Flash Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    ...
    home = get_flash_home().resolve()
    default = get_default_flash_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(flash_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when HERMES_HOME is a named profile.

    For ``~/.flash/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        flash_home: Optional explicit HERMES_HOME path. Defaults to the current
            ``get_flash_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Flash root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_flash_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from flash_constants import get_default_flash_root

    home = Path(flash_home or str(get_flash_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_flash_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(flash_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".flash"
    try:
        Path(flash_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(flash_home, default_root=target_root)
    except ValueError:
        return _profile_arg(flash_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this HERMES_HOME.

    Default ``~/.flash`` returns ``flash-gateway`` (backward compatible).
    Profile ``~/.flash/profiles/coder`` returns ``flash-gateway-coder``.
...
# Legacy service names from older Flash installs that predate the
# flash-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (flash-gateway-*.service) and unrelated third-party
# "flash" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("flash.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "flash_cli.main gateway",
    "flash_cli/main.py gateway",
    "gateway/run.py",
    " flash gateway ",
    "/flash gateway ",
)
...
def _find_legacy_flash_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Flash gateway units.

    Detects unit files installed by older Flash versions that used a
    different service name (e.g. ``flash.service`` before the rename to
    ``flash-gateway.service``). When both a legacy unit and the current
    ``flash-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``flash-gateway-coder.service`` and unrelated third-party
      ``flash-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``flash.service`` running an unrelated
...
def has_legacy_flash_units() -> bool:
    """Return True when any legacy Flash gateway unit files exist."""
    return bool(_find_legacy_flash_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Flash gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_flash_units()
    if not legacy:
        return
    print_warning("Legacy Flash gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current flash-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    flash gateway migrate-legacy")


def remove_legacy_flash_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Flash gateway unit files.

    Iterates over whatever ``_find_legacy_flash_units()`` returns — which is
    ...
    legacy = _find_legacy_flash_units()
    if not legacy:
        print("No legacy Flash gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Flash gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: flash gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo flash gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    flash gateway uninstall")
    print_info("    sudo flash gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: flash gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Flash often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.flash`` → ``ai.flash.gateway.plist`` (backward compatible).
    Profile ``~/.flash/profiles/coder`` → ``ai.flash.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.flash.gateway-{suffix}" if suffix else "ai.flash.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.flash/flash-agent  -> /home/alice/.flash/flash-agent
      /opt/flash                 -> /opt/flash  (kept as-is)
...
def _flash_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HERMES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_flash_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.flash                    → /home/alice/.flash
      /root/.flash/profiles/coder     → /home/alice/.flash/profiles/coder
      /opt/custom-flash               → /opt/custom-flash  (kept as-is)
    """
    current_flash = get_flash_home().resolve()
    current_default = (Path.home() / ".flash").resolve()
    target_default = Path(target_home_dir) / ".flash"

    # Default ~/.flash → remap to target user's default
    if current_flash == current_default:
        return str(target_default)

    # Profile or subdir of ~/.flash → preserve the relative structure
    try:
        relative = current_flash.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.flash) — keep as-is
        return str(current_flash)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    flash_node = flash_home / "node" / "bin"
    if _is_dir(flash_node):
        candidates.append(str(flash_node))
    flash_nm = flash_home / "node_modules" / ".bin"
    if _is_dir(flash_nm):
        candidates.append(str(flash_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m flash_cli.main``, so module
    ...
    dir, or a clone that ``flash update`` later relocates/removes — the path
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        flash_home = _flash_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(flash_home, home_dir)
        ...
        working_dir = str(flash_home) if flash_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m flash_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="HERMES_HOME={flash_home}"
Restart=always
RestartSec=5
...
"""
    ...
    flash_home = str(get_flash_home().resolve())
    profile_arg = _profile_arg(flash_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    #
    # Under ``sudo flash gateway … --system``, HERMES_HOME is often stripped
    # and falls back to ``/root/.flash``. Adopting the unit's pinned home
    ...
    _sync_flash_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``flash gateway restart``
    with ``HERMES_HOME=/tmp/flash-e2e-<pr>`` exported; the restart path's
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../flash_test`` path into
    ...
    if not system and (
        ...
        or '/flash_test"' in new_unit
        or "/flash_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # HERMES_HOME into the unit (manual E2E homes like /tmp/flash-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Flash install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo flash gateway uninstall --system")
    print_info("         flash gateway install")
    print_info("         flash gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (flash.service from pre-rename installs)
    # before installing the new flash-gateway.service. If both remain, they
    ...
    # ``sudo flash gateway install --system --force`` would bake /root/.flash
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}flash gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}flash gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}flash gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `flash gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `flash gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}flash gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}flash gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}flash gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}flash gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.flash.gateway-{suffix}" if suffix else "ai.flash.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``flash gateway start/stop/status`` call).
...
#      background process (the `nohup flash gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_flash_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_flash_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m flash_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "flash_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup flash gateway run --replace`
    ...
    """
    from flash_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_flash_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup flash gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.flash/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See flash-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_flash_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Flash install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  flash gateway status             # Check status")
    from flash_constants import display_flash_home as _dhh
...
def launchd_stop():
    ...
    # `flash gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Flash PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Flash PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Flash install")
    else:
        print("⚠ Service definition is stale relative to the current Flash install")
        print("  Run: flash gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: flash gateway stop")
            else:
    ...
    if deep:
        log_file = get_flash_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``flash gateway run`` is always a misconfiguration, so we
    ...
    try:
        from flash_constants import get_default_flash_root
        default_root = get_default_flash_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``flash gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``flash gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'flash gateway restart' to replace it,")
    print("  or 'flash gateway stop' first.")
    print("  Or use 'flash gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Flash gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'flash' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/flash/docker/entrypoint.sh before the Flash command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `flash gateway run` still
    ...
    # /restart) rather than through `flash gateway restart` which already
    ...
    # `flash gateway start/restart` — leaving the gateway vulnerable to
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from flash_constants import get_flash_home as _ghh
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. flash) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Flash delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Flash connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: flash pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Flash will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``flash setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run flash under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.flash/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_flash_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from flash_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'flash pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: flash pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'flash gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Flash will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Flash will store the returned account_id/token in ~/.flash/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `flash gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_flash_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `flash pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `flash pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "FlashAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``flash_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from flash_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.flash/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.flash/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: flash gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: flash gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: flash gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo flash gateway install --system"
                        )
                    print_info("  Or run in foreground:  flash gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: flash gateway run")
                print_info(
                    "  For persistence:   tmux new -s flash 'flash gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: flash gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup flash gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'flash gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`flash_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from flash_cli.service_manager import (
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``flash gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``flash gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    #
    # Prefer `sleep infinity` (matches the static main-flash service's
    # pattern in docker/s6-rc.d/main-flash/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: flash gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: flash gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s flash 'flash gateway run'"
                )
                print()
            ...
            print()
            print(
                "  flash gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s flash 'flash gateway run'         # persistent via tmux"
            )
            print(
                "  nohup flash gateway run > ~/.flash/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from flash_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  flash profile create <name>     # creates the s6 service slot")
                print("  flash -p <name> gateway start   # bring it up via s6")
                print("  flash status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: flash gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: flash gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: flash gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  flash profile delete <name>     # tears down the s6 service slot")
                print("  flash -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `flash -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: flash gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `flash gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `flash gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: flash gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  flash gateway install")
                else:
                    print("To install as a service:")
                    print("  flash gateway install")
                    print("  sudo flash gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  flash gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup flash gateway run > ~/.flash/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s flash 'flash gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  flash gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  flash gateway install  # Install as user service")
                    print(
                        "  sudo flash gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Flash gateway unit files from
        # pre-rename installs (e.g. flash.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_flash_units(interactive=not yes, dry_run=dry_run)
