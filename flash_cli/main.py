#!/usr/bin/env python3
"""
Flash CLI - Main entry point.

Usage:
    flash                     # Interactive chat (default)
    flash chat                # Interactive chat
    flash gateway             # Run gateway in foreground
    flash gateway start       # Start gateway as service
    flash gateway stop        # Stop gateway service
    flash gateway status      # Show gateway status
    flash gateway install     # Install gateway service
    flash gateway uninstall   # Uninstall gateway service
    flash setup               # Interactive setup wizard
    flash logout              # Clear stored authentication
    flash status              # Show status of all components
    flash cron                # Manage cron jobs
    flash cron list           # List cron jobs
    flash cron status         # Check if cron scheduler is running
    flash doctor              # Check configuration and dependencies
    flash honcho setup                    # Configure Honcho AI memory integration
    flash honcho status                   # Show Honcho config and connection status
    flash honcho sessions                 # List directory → session name mappings
    flash honcho map <name>               # Map current directory to a session name
    flash honcho peer                     # Show peer names and dialectic settings
    flash honcho peer --user NAME         # Set user peer name
    flash honcho peer --ai NAME           # Set AI peer name
    flash honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    flash honcho mode                     # Show current memory mode
    flash honcho mode [hybrid|honcho|local]  # Set memory mode
    flash honcho tokens                   # Show token budget settings
    flash honcho tokens --context N       # Set session.context() token cap
    flash honcho tokens --dialectic N     # Set dialectic result char cap
    flash honcho identity                 # Show AI peer identity representation
    flash honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    flash honcho migrate                  # Step-by-step migration guide: OpenClaw native → Flash + Honcho
    flash version             Show version
    flash update              Update to latest version
    flash uninstall           Uninstall Flash Agent
    flash acp                 Run as an ACP server for editor integration
    flash sessions browse     Interactive session picker with search

    flash claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: flash_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``flash_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``flash update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``flash_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, flash crashes on import and the user can't run
# ``flash update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'flash' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``flash tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``flash.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("flash")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"flash", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"flash")
        # Windows: the .exe name is already ``flash.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* flash_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".flash", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    cron jobs, pipes run ``flash … chat -q`` with stdio on a pipe. This
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`flash --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from flash_cli import __release_date__, __version__

    print(f"Flash Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``flash --version`` before config/logging imports on Termux."""
...
from flash_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from flash_cli.subcommands.cron import build_cron_parser
from flash_cli.subcommands.gateway import build_gateway_parser
from flash_cli.subcommands.profile import build_profile_parser
from flash_cli.subcommands.model import build_model_parser
from flash_cli.subcommands.setup import build_setup_parser
from flash_cli.subcommands.postinstall import build_postinstall_parser
from flash_cli.subcommands.whatsapp import build_whatsapp_parser
from flash_cli.subcommands.slack import build_slack_parser
from flash_cli.subcommands.login import build_login_parser
from flash_cli.subcommands.logout import build_logout_parser
from flash_cli.subcommands.auth import build_auth_parser
from flash_cli.subcommands.status import build_status_parser
from flash_cli.subcommands.webhook import build_webhook_parser
from flash_cli.subcommands.hooks import build_hooks_parser
from flash_cli.subcommands.doctor import build_doctor_parser
from flash_cli.subcommands.security import build_security_parser
from flash_cli.subcommands.dump import build_dump_parser
from flash_cli.subcommands.debug import build_debug_parser
from flash_cli.subcommands.backup import build_backup_parser
from flash_cli.subcommands.import_cmd import build_import_cmd_parser
from flash_cli.subcommands.config import build_config_parser
from flash_cli.subcommands.console import build_console_parser
from flash_cli.subcommands.version import build_version_parser
from flash_cli.subcommands.update import build_update_parser
from flash_cli.subcommands.uninstall import build_uninstall_parser
from flash_cli.subcommands.dashboard import build_dashboard_parser
from flash_cli.subcommands.gui import build_gui_parser
from flash_cli.subcommands.logs import build_logs_parser
from flash_cli.subcommands.prompt_size import build_prompt_size_parser
from flash_cli.subcommands.memory import build_memory_parser
from flash_cli.subcommands.acp import build_acp_parser
from flash_cli.subcommands.tools import build_tools_parser
from flash_cli.subcommands.insights import build_insights_parser
from flash_cli.subcommands.skills import build_skills_parser
from flash_cli.subcommands.pairing import build_pairing_parser
from flash_cli.subcommands.plugins import build_plugins_parser
from flash_cli.subcommands.mcp import build_mcp_parser
from flash_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (flash tools, flash setup, flash model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'flash {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any flash module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.flash/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `flash mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Flash' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo flash -p <name>` against the invoking user's home.
        ...
        candidate = home / ".flash" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`flash chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors flash_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # parent directory name (e.g. ~/.flash/profiles/coder or
    # /opt/data/profiles/coder).  If HERMES_HOME points to the flash root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.flash), we must
    # still read active_profile — the user may have switched profiles via
    # `flash profile use` and the gateway should honour that choice.
    # See issue #22502.
    flash_home_env = os.environ.get("HERMES_HOME", "")
    if profile_name is None and flash_home_env:
        if Path(flash_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the flash root.
    #
    ...
    # reserved ``gateway-default`` slot runs bare ``flash gateway run`` to mean
    ...
    if profile_name is None and not os.environ.get("HERMES_S6_SUPERVISED_CHILD"):
        try:
            ...
            active_path = get_default_flash_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            from flash_cli.profiles import resolve_profile_env

            flash_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            flash_home = _resolve_sudo_user_profile_env(profile_name)
            if not flash_home:
            ...
            # A bug in profiles.py must NEVER prevent flash from starting
            print(
...
# Load .env from ~/.flash/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from flash_cli.config import get_flash_home
from flash_cli.env_loader import load_flash_dotenv

load_flash_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE flash_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_flash_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from flash_cli import managed_scope
...
# Initialize centralized file logging early — all `flash` subcommands
...
try:
    from flash_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from flash_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if flash_constants not importable yet
...
from flash_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to flash_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (flash_cli.main._model_flow_*) keep resolving unchanged.
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_flash_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from flash_cli.config import get_env_path, get_flash_home, load_config
    from flash_cli.auth import get_auth_status

    # Determine whether Flash itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_flash_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from flash_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if Flash has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Flash to use their tokens.
    if _has_flash_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, flash_bin
        cli_args: the original CLI arguments (everything after 'flash')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    flash_bin = container_info["flash_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo flash {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo flash {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  flash --tui --resume {target}")
    if title:
        print(f'  flash --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @flash/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@flash" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/flash-ink/package.json",
    "packages/flash-ink/index.js",
    "packages/flash-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    flash_home = os.environ.get("HERMES_HOME") or str(Path.home() / ".flash")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": flash_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(flash_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(flash_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if flash_cli_dir is None:
        flash_cli_dir = Path(__file__).parent
    bundled = flash_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``flash update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Flash checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `flash update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Flash checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `flash --tui`\n"
        "If the checkout is still inconsistent, run `flash update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        # Keep the local @flash/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @flash/ink resolves through
        # packages/flash-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "flash-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from flash_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from flash_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="flash-tui-active-session-", suffix=".json"
    )
    ...
    # in the user's shell would otherwise make a plain `flash --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `flash update` so
    ...
    if code == 42:
        from flash_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``flash kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``flash kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from flash_cli.kanban_db import get_current_board
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.flash/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Flash — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Flash still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``flash … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'flash sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from flash_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from flash_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'flash doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Flash isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  flash setup")
        print()

        from flash_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'flash setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from flash_cli.banner import prefetch_update_check
    ...
    # user's ~/.flash/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from flash_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from flash_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from flash_cli.config import get_env_value, save_env_value
    from flash_constants import find_node_executable, with_flash_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Flash?")
        print()
    ...
    # has no creds.json.  Every subsequent `flash gateway` then paid a 30s
    ...
    session_dir = get_flash_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: flash gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `flash gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  flash gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Flash Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  flash gateway")
        ...
        print()
        print("  Or install as a service: flash gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'flash whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``flash whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``flash_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from flash_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from flash_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from flash_cli.config import stamp_install_method
    from flash_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Flash post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from flash_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``flash model``) and the setup wizard
    ...
    from flash_cli.providers import resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'flash model' for "
                "available providers, or run 'flash doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in flash_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.flash/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.flash/.env if the active provider is not 'custom'.
    ...
    """
    from flash_cli.config import get_env_value, save_env_value, load_config
...
#
# Flash uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `flash model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `flash model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`flash_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from flash_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``flash model`` flow, then route aux tasks to them here.
    """
    from flash_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from flash_cli.auth import _prompt_model_selection
    from flash_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from flash_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from flash_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from flash_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Flash URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from flash_cli.curses_ui import curses_radiolist
...
# downstream call sites read `flash_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `flash` invocation — including
# fast paths like `flash --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``flash setup`` / ``flash model``.
    ...
    recover from a malformed paste without editing ``~/.flash/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from flash_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from flash_cli.config import save_env_value
    from flash_cli.secret_prompt import masked_secret_prompt
    ...
    from flash_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `flash setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from flash_constants import display_flash_home as _dhh_fn

            print(
                f"    Flash will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Flash CLI with a provider."""
    from flash_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from flash_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from flash_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from flash_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from flash_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from flash_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``flash slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: flash slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `flash slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from flash_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from flash_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from flash_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from flash_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from flash_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `flash security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from flash_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from flash_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from flash_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from flash_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Flash home directory to a zip file."""
    if getattr(args, "quick", False):
        from flash_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from flash_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Flash backup from a zip file."""
    from flash_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from flash_cli.config import detect_install_method
...
def cmd_uninstall(args):
    """Uninstall Flash Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from flash_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from flash_cli.uninstall import run_gui_uninstall
    ...
    from flash_cli.uninstall import run_uninstall
...
# Critical files that Flash must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "flash_cli/main.py",
    "flash_cli/config.py",
    "flash_cli/__init__.py",
    "flash_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``flash`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="flash-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``flash update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``flash_cli/web_dist/`` (per vite.config.ts
    outDir: "../flash_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "flash_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``flash`` launcher present but ``flash_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the flash venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — flash venv python3, no nix-shell overhead
    ...
    # a flash venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``flash update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``flash web``).
    ...
    # build path usable on a stock `py -m flash_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_flash_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (flash web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``flash update`` can unconditionally call ``flash desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``flash desktop`` (interactive launch) skips the build when the
...
def _desktop_stamp_path() -> Path:
    ...
    from flash_constants import get_flash_home
    return get_flash_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``flash update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Flash.app/Contents/MacOS/Flash"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Flash.exe",
            release_dir / "win-ia32-unpacked" / "Flash.exe",
            release_dir / "win-arm64-unpacked" / "Flash.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "flash",
            release_dir / "linux-unpacked" / "Flash",
            release_dir / "linux-arm64-unpacked" / "flash",
            release_dir / "linux-arm64-unpacked" / "Flash",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Flash'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_flash_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Flash.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Flash.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Flash.exe: Access is
    ...
    an unrelated "Flash" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Flash is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Flash.app/Contents/MacOS/Flash  ->  app bundle = .../Flash.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``flash desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Flash Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Flash Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from flash_logging import setup_logging as _setup_logging_gui
    ...
    # with_flash_node_path() copies os.environ when called with no arg.
    env = with_flash_node_path()
    if getattr(args, "fake_boot", False):
        env["HERMES_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["HERMES_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "flash_root", None):
        env["HERMES_DESKTOP_HERMES_ROOT"] = str(Path(args.flash_root).expanduser().resolve())
    ...
    # `HERMES_DESKTOP_DISABLE_GPU=... flash desktop` keeps working.
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  flash gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Flash.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Flash.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Flash.exe, close any")
                    print("  running Flash desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> flash desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Flash is
    ...
    if source_mode:
        print("→ Launching Flash Desktop from source build...")
    ...
    print(f"→ Launching packaged Flash Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``flash dashboard`` processes other than ourselves.

    ``flash dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``flash update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Flash Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``flash serve`` as
    ...
    patterns = [
        "flash dashboard",
        "flash_cli.main dashboard",
        "flash_cli/main.py dashboard",
        # The headless backend (`flash serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "flash serve",
        "flash_cli.main serve",
        "flash_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from flash_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "flash.*dashboard"`) keeps us consistent
            # with `flash_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `flash update`.
    ...
    print("  Preview now:  flash curator run --dry-run")
    print("  Pause it:     flash curator pause")
    print(
        "  Docs:         https://flash-agent.flashorg.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``flash update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``flash update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: flash curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``flash dashboard`` processes.

    Called at the end of ``flash update`` (default ``reason``) and also
    from ``flash dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Flash Desktop Electron app spawns this dashboard as a
    ...
    if killed:
        ...
        print("    flash dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.flash-update-staging"
    backup = f"{dst}.flash-update-old"
...
def _update_via_zip(args):
    """Update Flash Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `flash update "
            f"--branch {branch}`, or update against main with `flash update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/FlashOrg/flash-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="flash-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"flash-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for flash-agent itself
        ...
        # GitHub ZIPs extract to flash-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"flash-agent-{branch}")
    ...
    from flash_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: flash skills list-modified  "
    ...
    try:
        from flash_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "flash-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Flash behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes flash completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Flash couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Flash couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Flash behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Flash couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Flash couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `flash update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/FlashOrg/flash-agent.git",
    "git@github.com:FlashOrg/flash-agent.git",
    "https://github.com/FlashOrg/flash-agent",
    "git@github.com:FlashOrg/flash-agent",
}
OFFICIAL_REPO_URL = "https://github.com/FlashOrg/flash-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_flash_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_flash_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Flash repository.")
        print("  This means you may miss updates from FlashOrg/flash-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/FlashOrg/flash-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/FlashOrg/flash-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``flash update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from flash_constants import get_default_flash_root

    default_home = get_default_flash_root()
...
# Install-scoped breadcrumb dropped right before ``flash update`` mutates the
...
# survives and the next ``flash`` launch finishes the install instead of
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``flash update`` left half-done.
    ...
    protocol stream (``flash acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if flash.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _flash_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Flash is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Flash from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `flash update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from flash_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``flash update`` is still progressing even if pip/uv itself is silent.
    """
...
def _flash_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_flash_instances(
    ...
    The Flash Desktop Electron app spawns ``flash.EXE`` as a backend child,
    so during ``flash update`` the user-invoked process and the desktop's
    ...
    shims (``flash.exe`` / ``flash-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``flash update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``flash.exe``) is a separate process from the Python
    ...
    for shim in _flash_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated flash.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``flash update``
    ...
    #      flash.exe sitting *under* a non-Flash parent (e.g. a Flash
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "flash.exe"
    lines = ["✗ Another flash.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Flash Desktop, exit any open `flash` REPLs, and")
    lines.append("  stop the gateway (`flash gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `flash update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_flash_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``flash.exe``.
    ...
    when ``flash update`` runs, ``flash.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``flash.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next flash invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Flash Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Flash Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _flash_exe_shims(scripts_dir):
        ...
        print(
            "    Close Flash Desktop, exit other `flash` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `flash update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_flash_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``flash.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``flash.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``flash`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_flash_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``flash.exe.old.*`` left by prior updates.

    Called early on every flash invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``flash update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `flash update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``flash.exe`` / ``flash-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_flash_exe`` for the rationale.
    """
    ...
    # hours later inside ``flash update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``flash.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``flash.exe`` shim is locked during ``flash update``, or when uv/distlib
    skips a launcher write. The symptom is ``flash-agent.exe`` and
    ``flash-acp.exe`` present but ``flash.exe`` missing, so ``flash`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `flash update --force` after "
            "closing other flash processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m flash_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``flash update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``flash.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``flash`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `flash update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `flash update --force` after closing other flash processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from flash_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from flash_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `flash update`, so we install root-only first
    ...
    nixos_env = with_flash_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `flash update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``flash update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.flash/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``flash update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``flash update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.flash/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``flash update --gateway``) the update is already
    ...
    try:
        ...
        # flash_cli.config.get_flash_home to simulate setup failure.
        from flash_cli.config import get_flash_home as _get_flash_home

        logs_dir = _get_flash_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.flash/logs/update.log`` only, never the terminal.

    During ``flash update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``flash update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``flash update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from flash_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from flash_cli.config import recommended_update_command
        from flash_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``flash update`` without requiring a reinstall.  The
    ...
    ``flash`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/flash.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v flash'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/flash, code at /usr/local/lib/flash-agent).
    fhs_link = Path("/usr/local/bin/flash")
    ...
    path_comment = (
        "# Flash Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``flash update``
    ...
    # ``flash_cli/config.py``). Fall back to false if the key is missing
    ...
    try:
        from flash_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_flash_home so the user sees ~/.flash/...
    try:
        from flash_constants import get_flash_home, display_flash_home

        home = get_flash_home()
        try:
            display_path = f"{display_flash_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  flash import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``flash update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``flash update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run flash from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.flash-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".flash-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The flash.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m flash_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``flash
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Flash processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Flash Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Flash desktop app / other Flash terminals, then re-run:"
    )
    lines.append("    flash update")
    lines.append("  (or use `flash update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    flash.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from flash_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Flash...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m flash_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: flash gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``flash gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from flash_cli import gateway_windows
        from flash_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``flash update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Flash Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Flash Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `flash update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Flash via pip (for PyPI installs)."""
    from flash_cli import __version__
    from flash_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "flash-agent"]
        ...
        cmd = [pipx, "upgrade", "flash-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "flash-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "flash-agent"]
    ...
    print("✓ Update complete! Restart flash to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Flash Agent...")
    print()

    # On Windows, abort early if another flash.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_flash_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `flash serve` backend) will keep .pyd
    ...
    # the flash.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Flash windows/gateways and re-run: flash update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        try:
            from flash_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in flash_cli/config.py bricked
        # every user who ran ``flash update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``flash
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``flash update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_flash_home added to flash_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``flash`` launch finishes the
        ...
        # build.  ``flash desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``flash desktop`` shouldn't be forced into a full
        # Electron build by ``flash update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from flash_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "flash_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `flash desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from flash_constants import display_flash_home as _dhh
        ...
        # current — so copy it straight over ~/.flash/cache/model_catalog.json
        ...
        # modules in this process.  Reload flash_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_flash_home() added since the last release.
        try:
            import importlib
            import flash_constants as _hc
        ...
        try:
            from flash_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from flash_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'flash config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: flash config migrate")
            else:
                print()
                print("Skipped. Run 'flash config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from flash_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `flash curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``flash update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from flash_cli.tools_config import install_cua_driver
        ...
        # When running as ``flash update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_flash_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... flash-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... flash-gateway*)
            ...
            # Discover all flash-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. flash-gateway.service or flash-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `flash update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: flash gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: flash gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: flash -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Flash gateway unit files are still installed.
        # When both flash.service (from a pre-rename install) and the
        # current flash-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `flash update` surfaces the issue until the user migrates.
        try:
            from flash_cli.gateway import (
                has_legacy_flash_units,
                _find_legacy_flash_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_flash_units():
                print()
                print("⚠ Legacy Flash gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_flash_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (flash.service) fight the current")
                print("  flash-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    flash gateway migrate-legacy")
                print()
        ...
        print("  flash model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``flash -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `flash profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_flash_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → flash -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.flash)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  flash profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     flash -p {name} chat")
                else:
        ...
        from flash_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from flash_constants import get_flash_home as _hh
        ...
        from flash_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `flash profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → flash -p {name}  ({wrapper})")
        print()
        ...
        from flash_cli.profiles import profile_exists, validate_alias_name
        ...
        from flash_cli.profiles import rename_profile
        ...
        from flash_cli.profiles import export_profile
        ...
        from flash_cli.profiles import import_profile
        ...
        import tempfile
        from flash_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="flash_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  flash -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      flash -p {plan.manifest.name} chat")
        ...
        from flash_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `flash profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"flash -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from flash_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("flash_requires"):
            print(f"Requires:     Flash {data['flash_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from flash_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.flash_requires:
        print(f"  Requires: Flash {mf.flash_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``flash dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``flash dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No flash dashboard processes running.")
        return 0

    print(f"{len(pids)} flash dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``flash dashboard register`` for OAuth.
    ...
    try:
        from flash_cli.web_server import should_require_auth
    ...
    try:
        from flash_cli.dashboard_auth import list_providers
        if list_providers():
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No flash dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `flash update`.
    ...
    try:
        from flash_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "flash_cli.main",
            "-p", "default",
        ...
        # HERMES_HOME falls back to $HOME/.flash = /opt/data/.flash — an
        ...
        # update-button guard also misfires).  get_default_flash_root()
        # returns the root for both layouts: ~/.flash for a standard install
        ...
        try:
            from flash_constants import get_default_flash_root
            env["HERMES_HOME"] = str(get_default_flash_root())
        except Exception:
    ...
    # the same logs directory as every other Flash surface.
    try:
        from flash_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "flash_cli" / "web_dist"
        )
    ...
    try:
        from flash_cli.mcp_startup import start_background_mcp_discovery
    ...
    from flash_cli.web_server import start_server
...
def cmd_gateway_enroll(args):
    ...
    from flash_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from flash_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from flash_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Flash log files."""
    from flash_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Flash command console."""
    from flash_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from flash_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``flash -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``flash_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``flash -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``flash`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from flash_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from flash_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `flash --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_flash_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_flash_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_flash_home()}/memories/\n")
    else:
        from flash_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Flash Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from flash_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from flash_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from flash_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from flash_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from flash_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from flash_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from flash_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from flash_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from flash_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for flash CLI."""
    # Cosmetic: make the process show up as 'flash' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from flash_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``flash.exe.old.*`` quarantine files left by previous
    # ``flash update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_flash_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``flash update``
    ...
    # ``flash skills install update``) merely defers recovery one launch;
    # under-matching (missing ``flash -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in flash_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from flash_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from flash_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://flash-agent.flashorg.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `flash model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.flash/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://flash-agent.flashorg.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from flash_cli import secrets_cli as _secrets_cli
    from flash_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from flash_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in flash_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in flash_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in flash_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in flash_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `flash whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in flash_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from flash_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in flash_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in flash_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in flash_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in flash_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in flash_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in flash_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from flash_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from flash_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in flash_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in flash_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in flash_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in flash_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in flash_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in flash_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.flash/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo flash uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from flash_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in flash_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in flash_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in flash_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in flash_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in flash_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from flash_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in flash_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``flash --help``, ``flash version``, ``flash logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from flash_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from flash_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Flash shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from flash_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from flash_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in flash_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in flash_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `flash computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `flash tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `flash computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Flash app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from flash_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: flash computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: flash computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: flash computer-use install --upgrade")
                return
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: flash computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: flash computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in flash_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <flash home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from flash_state import workspace_key as _ws_key
            ...
            from flash_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from flash_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from flash_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_flash_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from flash_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_flash_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_flash_home() / "sessions"
            ...
            # `flash sessions prune`: no time window and no filters at all
            ...
            # Launch flash --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in flash_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in flash_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in flash_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in flash_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in flash_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in flash_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in flash_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in flash_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Flash-Setup.exe success screen tells users to
    # run `flash desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in flash_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in flash_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in flash_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``flash -c Pokemon Agent Dev`` → ``flash -c 'Pokemon Agent Dev'``
    ...
    from flash_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'flash -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (flash hooks list, cron
