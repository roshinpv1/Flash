...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-flash-setup
...
def reconcile_profile_gateways(
    *,
    ...
    not under ``profiles/``). The dispatcher in ``flash_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``flash gateway start`` (no ``-p``) targets. Without it,
    bare ``flash gateway start`` inside the container would land on
    ...
    Args:
        flash_home: The container's HERMES_HOME (typically /opt/data).
            Profiles live under ``<flash_home>/profiles/<name>/``;
            the default profile lives at ``<flash_home>`` itself.
    ...
    # ``flash gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(flash_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(flash_home)
    ...
    profiles_root = flash_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `flash profile create` (config.yaml
            # is not — that comes later via `flash setup`). Use it as the
            ...
            # via ``flash -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(flash_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = flash_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the flash args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/flash/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/flash/docker/main-wrapper.sh
    ...
    ``flash`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``flash <subcommand>``, so an
    explicit leading ``flash`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `flash <subcommand>`; peel an explicit flash.
    if args and Path(args[0]).name == "flash":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``flash dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `flash -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit flash
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the flash user) won't hit EACCES. See
...
def _write_reconcile_log(
    flash_home: Path, actions: list[ReconcileAction],
) -> None:
...
def main() -> int:
    ...
    actions = reconcile_profile_gateways(
        flash_home=flash_home, scandir=scandir,
    )
    for a in actions:
        print(
