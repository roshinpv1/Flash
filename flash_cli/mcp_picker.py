"""MCP picker — interactive `flash mcp picker` (also the default `flash mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``flash mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `flash plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from flash_cli.cli_output import prompt_yes_no
from flash_cli.curses_ui import curses_single_select
from flash_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Flash session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from flash_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `flash mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: flash mcp install <name>    Picker: flash mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Flash is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Flash — run `flash update` "
...
def show_catalog() -> None:
    """`flash mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`flash mcp picker` (and default `flash mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`flash mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `flash mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
