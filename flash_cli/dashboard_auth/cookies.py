...
Three cookies in play:
  - flash_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - flash_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         FlashPortal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / flash
  ...
  - flash_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/flash) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/flash`` covers
...
setter wrote ``__Secure-flash_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "flash_session_at"
SESSION_RT_COOKIE = "flash_session_rt"
PKCE_COOKIE = "flash_session_pkce"
...
SSO_ATTEMPT_COOKIE = "flash_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /flash`` we want ``Path=/flash`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. FlashPortal
    issues a 24h rotating refresh token (flash #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/flash``)
