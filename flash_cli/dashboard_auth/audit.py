...
from ``flash_constants`` or other flash_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    ...
    Mirrors ``flash_constants.get_flash_home`` semantics: env var wins,
    else ``~/.flash``. A local copy avoids an import cycle with the
    middleware which lives below ``flash_cli``.
    """
    home = os.environ.get("HERMES_HOME") or str(Path.home() / ".flash")
