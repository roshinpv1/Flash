...
prefix (e.g. ``mission-control.tilos.com/flash/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /flash`` so the backend can
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``flash.example.com`` instead of
    ``https://flash.example.com``) — the single most common cause of
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "flash.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/flash"`` (no trailing slash) or ``""``
