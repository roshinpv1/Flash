"""On-demand supply-chain audit for Flash Agent installs.

Scans three surfaces a Flash user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Flash venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.flash/plugins``
...
def _discover_plugins(flash_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.flash/plugins``.
...
def run_audit(
    *,
    ...
    home = flash_home or Path(get_flash_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, flash_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(flash_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `flash security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, flash_home=home
    )
    if total == 0:
