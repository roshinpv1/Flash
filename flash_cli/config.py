"""
Configuration management for Flash Agent.

Config files are stored in ~/.flash/ for easy access:
- ~/.flash/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.flash/.env         - API keys and secrets

This module provides:
- flash config          - Show current configuration
- flash config edit     - Open config in editor
- flash config set      - Set a specific value
- flash config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``flash config set`` (which rewrites
    ...
    ``config.yaml`` in place: flash never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.flash/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``flash logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  flash config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Flash makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Flash itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``flash update``, the TUI build.
...
#   on every plugin install / ``flash update``.
...
#   ``HERMES_ENV`` — Flash runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Flash runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Flash runtime location "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `flash plugins enable
    # observability/langfuse` or `flash tools → Langfuse`); credentials gate
...
from flash_cli.colors import Colors, color
from flash_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_flash_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Flash is running in package-manager-managed mode.
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade flash-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``flash_cli/`` — i.e. the git checkout for source
    installs, ``/opt/flash`` inside the published image, the venv's
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Flash was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.flash``)
    --------------------------------------------------------------------------
    ...
    bind-mount it (``~/.flash:/opt/data``) so config/sessions/memory persist
    ...
    ``docker`` and ``flash update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``flashorg/flash-agent`` image bakes a ``docker``
        stamp into ``/opt/flash`` at build time.
    ...
    #    container, and honouring it wrongly blocks ``flash update``.
    try:
        method = (
            (get_flash_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``flash_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    ``/opt/flash`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Flash lives in a ``uv tool`` layout.

    ``uv tool install flash-agent`` places the install at
    ``.../uv/tools/flash-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``flash-agent`` happens to be uv-tool-installed on the machine while
    the *active* Flash is a regular pip/venv install, causing
    ``flash update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/flash-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade flash-agent"
    if method == "docker":
        return "docker pull flashorg/flash-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade flash-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade flash-agent"
        return "pip install --upgrade flash-agent"
    return "flash update"
...
# banner, the TUI/desktop session info panel, and ``flash update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://flash-agent.flashorg.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``flash update`` / ``flash update --check`` so the wording — and the
...
# Long-form text for ``flash update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# flash_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Flash, it doesn't update
...
✗ ``flash update`` doesn't apply inside the Docker container.

Flash Agent runs as a published image (flashorg/flash-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull flashorg/flash-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate flash-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm flashorg/flash-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/flashorg/flash-agent/tags
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``flash update`` inside Docker.
...
def format_managed_message(action: str = "modify this Flash installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Flash installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.flash-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Flash installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade flash-agent"
        )

    return (
        f"Cannot {action}: this Flash installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Flash."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, flash_bin
    ...
    container_mode_file = get_flash_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "flash-agent")
    exec_user = info.get("exec_user", "flash")
    flash_bin = info.get("flash_bin", "/data/current-package/bin/flash")
...
# Re-export from flash_constants — canonical definition lives there.
from flash_constants import get_flash_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_flash_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_flash_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_flash_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the HERMES_UID / HERMES_GID env vars set by Docker deployments.

    Docker containers running Flash commonly set these to map the in-container
    ...
    subdirectories created at runtime by ``ensure_flash_home()`` (especially
...
def _chown_to_flash_uid(path) -> None:
    ...
    directories created by :func:`ensure_flash_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_flash_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the flash group can
    ...
    try:
        ...
        pass
    _chown_to_flash_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Flash runs in a container with volume-mounted config files, forcing
...
def ensure_flash_home():
    """Ensure ~/.flash directory structure exists with secure permissions.
    ...
    """
    home = get_flash_home()
    # Named profiles must be created explicitly (e.g. ``flash profile create``).
    ...
    if is_managed():
        ...
        try:
            _ensure_flash_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_flash_home_managed(home: Path):
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Flash-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Flash
        ...
        # app, ACP) in a code workspace, Flash adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Flash auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Flash sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Flash
        ...
        # bundled Flash image, which drops to the `flash` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Flash sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Flash should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``flash chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.flash/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: flash sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .flash.md, or .cursorrules before Flash applies
    ...
    # single read_file page exceeds these limits, Flash truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Flash' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); flash =
                                      # Flash' compression threshold triggers
    ...
    # See tools/kanban_tools.py and flash_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``flash dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``flash kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``flash kanban decompose`` and the kanban
        ...
        # ``flash profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `flash model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `flash` (and `flash chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `flash --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `flash -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `flash --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/flash`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `flash pets`. Disabled until
        ...
        "pet": {
            ...
            # get_flash_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``portal_url`` defaults to https://portal.flashorg.com).
        "oauth": {
        ...
        # optional path prefix (e.g. ``https://example.com/flash``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Flash appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.flash/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.flash/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.flash/cache/delegation/
    ...
    # response. If not, Flash feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Flash auto-pauses the goal and
    ...
    "moa": {
        ...
        # <flash_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # cron daemon.
    #
    # See `flash curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `flash curator run --consolidate` overrides
        ...
        # normally restored on every `flash update`, so pruning them only
        ...
        # skipped), snapshot ~/.flash/skills/ into
        # ~/.flash/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `flash curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for flash-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Flash Agent*" header.
    ...
    # replace Flash' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.flash/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `flash doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``flash_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Flash to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `flash -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `flash` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `flash kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     flash-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Flash tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.flash/logs/.
    ...
    # update model picker lists without shipping a flash-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://flash-agent.flashorg.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `flash model` invocation; network failures
    ...
    "gateway": {
        ...
        # producing ~/.flash/sessions/sessions.json entirely.
        ...
        # `launchctl kickstart ai.flash.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.flash/.env,
        ...
        # behavior: files must live under the Flash cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Flash
        # cache (~/.flash/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.flash/state.db.
    ...
    "sessions": {
        ...
        # default of ``flash sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.flash/sessions/session_{sid}.json`` on every turn
    ...
    # ``flash update`` behaviour.
    "updates": {
        # Run a full ``flash backup``-style zip of HERMES_HOME before every
        # ``flash update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``flash import <path>``.  Off by default:
        # zipping a large HERMES_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``flash update
        ...
        "backup_keep": 5,
        # What `flash update` does with uncommitted local changes to the
    ...
    # `flash tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.flash/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.flash/.env (or your shell) and never in config.yaml.
            ...
            # ~/.flash/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.flash/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Flash disables it for our users unless they
        # explicitly opt in here. When false (default), Flash sets
    ...
    # Flash Desktop (Electron app) launch options. These only affect
    # `flash desktop`; they do not touch the CLI/gateway.
    "desktop": {
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Flash mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'flash model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for FlashSubscribers only, used to derive vendor hosts, e.g. flashorg.com -> firecrawl-gateway.flashorg.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit FlashSubscriber access token for tool-gateway requests (optional; otherwise read from the Flash auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Flash, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @flash:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #flash)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: flash-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'flash-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Flash API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Flash API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Flash API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.flash.config`` entries, then checks
    ...
    try:
        ...
        # should never break `flash update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Flash' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Flash versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``flash_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'flash setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Flash won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'flash doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.flash")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "flash update / flash -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            try:
                user_plugins_dir = get_flash_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `flash plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `flash curator status` has no
    ...
    #   3. Creates `~/.flash/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_flash_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_flash_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `flash config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `flash config set`"
                    )
    ...
    # behavior set it to true explicitly via `flash config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from flash_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: flash config set <key> <value>")
    ...
    # `flash update`, but only the version bump is persisted.
    ...
    # metadata.flash.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_flash_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``flash config set``
    ...
    backends) — while display paths (``flash status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _flash_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.flash/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.flash/config.yaml.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/flash/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_flash_dotenv()
...
#   flash         (OAuth — flash auth) — FlashPortal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.flash/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Flash writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.flash/.env.
    ...
    interactive menu render (`flash tools`, `flash setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `flash tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Flash env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.flash/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.flash/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.flash/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.flash/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Flash — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Flash vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.flash/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.flash/.env`` over ``os.environ``.

    Used for Flash-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Flash Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from flash_cli.auth import get_anthropic_key
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'flash doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  flash config edit     # Edit config file", Colors.DIM))
    print(color("  flash config set <key> <value>", Colors.DIM))
    print(color("  flash setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `flash config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `flash config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: flash config set <key> <value>")
            print()
            print("Examples:")
            print("  flash config set model anthropic/claude-sonnet-4")
            print("  flash config set terminal.backend docker")
            print("  flash config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'flash config migrate' to add them")
        ...
        print("  flash config           Show current configuration")
        print("  flash config edit      Open config in editor")
        print("  flash config set <key> <value>   Set a config value")
        print("  flash config check     Check for missing/outdated config")
        print("  flash config migrate   Update config with new options")
        print("  flash config path      Show config file path")
        print("  flash config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``flash config`` UI so users
