"""Shell completion script generation for flash CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_flash_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Flash Agent bash completion
# Add to ~/.bashrc:
#   eval "$(flash completion bash)"

_flash_profiles() {{
    local profiles_dir="$HOME/.flash/profiles"
...
_flash_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_flash_profiles)" -- "$cur"))
        return
    fi
...
complete -F _flash_completion flash
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef flash
# Flash Agent zsh completion
# Add to ~/.zshrc:
...
_flash_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.flash/profiles" ]]; then
        profiles+=($HOME/.flash/profiles/*(N/:t))
    fi
...
_flash() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_flash_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'flash command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Flash Agent fish completion",
        "# Add to your config:",
        "#   flash completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __flash_profiles",
        "    echo default",
        "    if test -d $HOME/.flash/profiles",
        "        for d in $HOME/.flash/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c flash -f -s p -l profile"
        " -d 'Profile name' -xa '(__flash_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__flash_profiles)' -d 'Profile name'"
                )
