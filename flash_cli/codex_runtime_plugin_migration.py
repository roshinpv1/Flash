"""Migrate Flash' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Flash' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Flash mcp_servers.<n>.command/args/env  → codex stdio transport
  Flash mcp_servers.<n>.url/headers       → codex streamable_http transport
  Flash mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Flash mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Flash-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Flash' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Flash config.")
...
# Flash keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Flash' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, flash_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Flash MCP server config to the codex inline-table dict
    ...
    has_command = bool(flash_cfg.get("command"))
    has_url = bool(flash_cfg.get("url"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Flash)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Flash' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Flash' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Flash
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="flash-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    codex-routed flash-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.flash`` symlink that happens to live under ``/private/var/folders``
...
def _build_flash_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Flash' own
    ...
    so a flash installed under /opt/, /usr/local/, or a venv all work.
    ...
    # (not get_flash_home()) on purpose: when the env var is unset we want
    ...
    flash_home = os.environ.get("HERMES_HOME") or ""
    if flash_home and _looks_like_test_tempdir(flash_home):
        flash_home = ""
    if flash_home:
        env["HERMES_HOME"] = flash_home
    # PYTHONPATH passes through so a worktree-launched flash finds the
...
def migrate(
    ...
    *,
    ...
    expose_flash_tools: bool = True,
) -> MigrationReport:
    """Translate Flash mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        flash_config: full ~/.flash/config.yaml dict
        ...
        expose_flash_tools: when True (default), register Flash' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Flash for tools
    ...
    flash_servers = (flash_config or {}).get("mcp_servers") or {}
    if not isinstance(flash_servers, dict):
        report.errors.append(
            "mcp_servers in Flash config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in flash_servers.items():
    ...
    # Inject Flash' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Flash for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/flash_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_flash_tools:
        translated["flash-tools"] = _build_flash_tools_mcp_entry()
        if "flash-tools" not in report.migrated:
            report.migrated.append("flash-tools")
