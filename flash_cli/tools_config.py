"""
Unified tool configuration for Flash Agent.

`flash tools` and `flash setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.flash/config.yaml under
...
from flash_cli.flash_subscription import (
    apply_flash_managed_defaults,
    get_flash_subscription_features,
)
from flash_cli.flash_account import format_flash_portal_entitlement_message
...
from flash_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `flash tools` → Video Generation, which walks
...
# `flash tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from flash_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `flash tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``flash tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from flash_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``flash tools`` checklist actually offers
    for ``platform``.
    ...
    so ``flash tools`` never claims to add or remove a toolset the user was
...
from flash_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Flash routes X searches through xAI's built-in x_search "
...
# provider+model picker like `flash model` — NOT this single-key prompt, so
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``flash update`` and
      by ``flash computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `flash update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `flash computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `flash update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `flash update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.flash/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Flash process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_flash_home()}/flash-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in flash-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/flashorg/flash-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.flash/config.yaml")
        ...
        # Run the full `flash auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.flash/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from flash_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: flash auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `flash auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: flash auth spotify")
        except Exception as exc:
        ...
        try:
            from flash_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: flash plugins enable observability/langfuse")
        _print_info("    Restart Flash for tracing to take effect.")
        _print_info("    Verify: flash plugins list")
        ...
        try:
            ...
            _print_info("    Run later: flash auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `flash auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: flash auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: flash auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``flash tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``flash tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: flash tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``flash-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``flash-discord``) is an opt-in to the platform's native default-off
    ...
    # "flash-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[flash-cli, spotify]`` after enabling Spotify via ``flash
        ...
        # (e.g. "flash-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``flash tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `flash tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"flash-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("flash-")}
    ...
    # shipped as a bundled plugin but user must opt in via `flash tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `flash tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by flash tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `flash` instead of
    # `flash-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `flash update`/`flash doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `flash tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (flash-cli, flash-telegram, etc.)
    ...
    # Opening `flash tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `flash tools` gets a silent no-op
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``flash model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `flash model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `flash model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `flash tools` and `flash setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Flash Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://flash-agent.flashorg.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from flash_constants import display_flash_home
    print(color(f"  Tool configuration saved to {display_flash_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'flash' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, flash mcp
