...
the existing module-level functions in flash_cli.gateway and
flash_cli.gateway_windows directly. This protocol is a thin facade
...
``flash gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``flash gateway start/stop/restart``).
    """
    ...
    # `flash gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged flash
    ...
    detection always fails. Since every Flash runtime call inside the
    container drops to flash via ``s6-setuidgid``, that silent failure
...
# in ``flash_cli.gateway`` (systemd/launchd) and ``flash_cli.gateway_windows``
...
# active (set via the ``flash -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in flash_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from flash_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from flash_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from flash_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from flash_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in flash_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from flash_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from flash_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from flash_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from flash_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``flash_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `flash profile create`
# runs inside the container (Phase 4). Static services (main-flash, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    flash_home = Path(os.environ.get("HERMES_HOME", "/opt/data"))
    if flash_home.parent.name == "profiles":
        root = flash_home.parent.parent
    else:
        root = flash_home
...
# UID/GID of the in-image ``flash`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the flash user.
    ...
    (the ``flash`` user — UID 10000 — running every Flash runtime
    ...
    is dispatched as the flash user.
    ...
    with flash ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           flash:flash, 0755 (parent must already exist)
    ``svc_dir/event/``                     flash:flash, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 flash:flash, 0755
    ``svc_dir/supervise/event/``           flash:flash, 03730
    ``svc_dir/supervise/control``          flash:flash, 0660    (FIFO)
    ...
    slot's supervise/ was flash-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the flash user already — directory is flash-
    ...
    # when the parent slot's supervise/ is flash-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``flash -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`flash profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-flash, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HERMES_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HERMES_HOME=/data/flash`` is honored at run
          ...
          4. Drops to the flash user and exec's
             ``flash -p <profile> gateway run`` (or just ``flash
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``flash gateway
        ...
        ``flash_cli.gateway`` maps that empty string to the
        ...
        allocator in ``flash_cli.profiles._allocate_gateway_port``).
        ...
        # (a stray ``flash gateway run`` from a shell, an agent action, or
        ...
        if profile == "default":
            gateway_cmd = "flash gateway run --replace"
        else:
            gateway_cmd = f"flash -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid flash {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        — so a container started with ``-e HERMES_HOME=/data/flash``
        gets its logs under /data/flash/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``flash logs`` reads and what
        ...
        return (
            ...
            # later then runs its log service as flash and crash-loops on
            ...
            f'chown flash:flash "$HERMES_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R flash:flash "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid flash s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``flash gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``flash -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with flash ownership
            ...
            # the flash user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is flash-owned (see ``_seed_supervise_skeleton``), and on
