"""
Interactive setup wizard for Flash Agent.
...
from flash_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://flash-agent.flashorg.com/docs"
...
# display_flash_home imported lazily at call sites (stale-module safety during flash update)
...
from flash_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from flash_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Flash Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Flash using environment variables or config commands:")
    print_info("  flash config set model.provider custom")
    print_info("  flash config set model.base_url http://localhost:8080/v1")
    print_info("  flash config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'flash setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``flash_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from flash_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'flash setup')")


def _print_setup_summary(config: dict, flash_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'flash setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from flash_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `flash tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from flash_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'flash setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'flash setup tts'"))
    else:
    ...
    # Spotify (OAuth via flash auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'flash setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {flash_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('flash setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('flash setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('flash setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('flash setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('flash setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('flash config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('flash config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('flash config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('flash', Colors.GREEN)}              Start chatting")
    print(f"   {color('flash gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('flash doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `flash tools` and `flash setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``flash model``)
    ...
    provider added to ``flash model`` is automatically available here.
    ...
    # Delegate to the shared flash model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from flash_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `flash auth add`, `flash setup` vision, and
    # `flash setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from flash_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``flash model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Flash auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via flash setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'flash setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Flash runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `flash setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `flash setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `flash setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.flash/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from flash_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_flash_home(Path(get_flash_home()))
    except Exception:
        pass
...
def _profile_name_from_flash_home(flash_home) -> str | None:
    """Return the active profile name when HERMES_HOME is a profile dir."""
    if flash_home.parent.name == "profiles":
        return flash_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Flash delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Flash to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from flash_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://flash-agent.flashorg.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://flash-agent.flashorg.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  flash config edit")
    print_info("   Open config in your editor:  flash config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from flash_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Flash from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'flash setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     flash config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_flash_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: flash gateway install --system")
                print_info("  Or run in foreground:  flash gateway")
        else:
            from flash_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   flash gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   flash gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `flash setup tools` and `flash tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from flash_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from flash_cli.config
    ...
    if section_key == "model":
        ...
        from flash_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_flash.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_flash migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_flash", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Flash.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Flash to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Flash at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Flash at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Flash at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Flash at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Flash equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Flash config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Flash.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Flash's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(flash_home: Path) -> bool:
    ...
    print_info("Flash can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: flash claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=flash_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: flash claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Flash configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Flash config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Flash (use flash claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    ``flash setup`` and hunt for the quick-setup option.
    ...
    try:
        from flash_cli.main import _model_flow_nous
        ...
        logger.debug("_model_flow_nous error during `flash portal`: %s", exc)
        print()
    ...
    print_info("  Run `flash portal info` to inspect routing.")
    print_info("  Run `flash` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      flash setup           — full or quick (auto-detected)
      flash setup model     — just model/provider
      flash setup tts       — just text-to-speech
      flash setup terminal  — just terminal backend
      flash setup gateway   — just messaging platforms
      flash setup tools     — just tool configuration
      flash setup agent     — just agent settings
    """
    from flash_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Flash Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Flash Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Flash Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, flash_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Flash configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'flash setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(flash_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, flash_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, flash_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {flash_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'flash config edit'")
    ...
    # Tune later with `flash setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, flash_home)
...
def _run_first_time_quick_setup(config: dict, flash_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'flash setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    flash setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  flash setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``flash tools`` (which
    ...
    try:
        ...
        from flash_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``flash setup agent`` /
    ``flash config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, flash_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `flash update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        flash tools")
        print_info("  Seed skills:         flash skills opt-in --sync")
        print_info("  Add MCP servers:     flash mcp add")
        print_info("  Enable plugins:      flash plugins")
        print_info("  Tune agent settings: flash setup agent")
        print()
        _print_setup_summary(config, flash_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, flash_home)


def _blank_slate_walkthrough(config: dict, flash_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`flash update` runs from re-injecting them. Opt back in any")
            print_info("time with `flash skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `flash tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `flash plugins list` / `flash plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `flash plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `flash mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `flash mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   flash tools")
    print_info("  Seed skills:         flash skills opt-in --sync")
    print_info("  Add MCP servers:     flash mcp add")
    print_info("  Tune agent settings: flash setup agent")
    print()
...
def _run_quick_setup(config: dict, flash_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'flash setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Flash to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'flash setup gateway'.")
