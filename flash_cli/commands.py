"""Slash command definitions and autocomplete for the Flash CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Flash config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Flash works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Flash is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.flash/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Flash Agent to the latest version", "Info"),
    CommandDef("version", "Show Flash Agent version", "Info", aliases=("v",)),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[flash|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`flash_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/flash`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Flash ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Flash
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``flash skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``flash skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``flash skills list`` and
...
# unrelated command landed. These claim their slots right after /flash,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /flash <command>).
...
# native slot, the alias spelling stays reachable via /flash reset).
...
# ``/flash <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/flash" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /flash credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /flash billing.
#   - moa: high-cost slash mode, available through /flash moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /flash debug on Slack.
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/flash <verb>`` subcommand.
    ...
    can still reach them via ``/flash <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/flash`` is always reserved as the first entry so the
    legacy ``/flash <subcommand>`` form keeps working for anything that
    ...
    # Reserve /flash as the catch-all top-level command.
    entries.append(("flash", "Talk to Flash or run a subcommand", "[subcommand] [args]"))
    seen.add("flash")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_VIA_HERMES_ONLY:
            # Intentionally Slack-via-/flash only (see _SLACK_VIA_HERMES_ONLY).
            return
    ...
    # everything except /flash, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://flash-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /flash handler.

    Maps both canonical names and aliases so /flash bg do stuff works
    the same as /flash background do stuff.

    Plugin-registered slash commands are included so ``/flash <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from flash_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
