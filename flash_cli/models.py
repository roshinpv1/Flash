...
Add, remove, or reorder entries here — both `flash setup` and
`flash` provider-selection will pick up the change automatically.
"""
...
from flash_cli import __version__ as _HERMES_VERSION
from flash_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HERMES_USER_AGENT = f"flash-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `flash model`
    flow without maintaining a separate static list.
    """
    from flash_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Flash-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Flash refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Flash that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Flash that doesn't ship
...
def _resolve_flash_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.flashorg.com"
...
# this list:  flash model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `flash model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Flash provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `flash model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``flash model``, the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``flash auth add openai-codex`` without
    ``flash model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``flash model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    flash-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from flash_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — flash-agent
...
_DEFAULT_FLASH_INFERENCE_BASE = "https://inference-api.flashorg.com"
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "flash":
        ...
        if base_url:
            # Flashbase_url typically looks like https://inference-api.flashorg.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``flash model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from flash_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from flash_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Flash' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``flash_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Flash should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``flash auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.flash/.env``.
    ...
    """
    try:
        from flash_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Flash release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Flash release). Falls back to the hardcoded catalog if no token
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``flash model`` shows.
...
def _provider_models_cache_path() -> Path:
    from flash_constants import get_flash_home
    return get_flash_home() / "provider_models_cache.json"
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Flash' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    if isinstance(request_headers, dict):
        ...
        from flash_cli.config import normalize_extra_headers
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from flash_constants import get_flash_home
    return get_flash_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `flash moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Flash will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Flash cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Flash actually ships.  (#46850)
