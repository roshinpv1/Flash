"""Per-provider model-selection wizard flows for ``flash setup`` / ``flash model``.

Extracted from ``flash_cli/main.py`` as part of the god-file decomposition
campaign (``~/.flash/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from flash_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``flash_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from flash_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``flash_cli.main`` at import time -> no import cycle.
"""
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from flash_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from flash_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.flash/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `flash setup` from scratch. OpenRouter
    ...
    from flash_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from flash_cli.auth import _save_model_choice, deactivate_provider
    from flash_cli.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `flash moa configure <name>` first.")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `flash auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``flash auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from flash_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from flash_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from flash_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from flash_cli.auth import _save_model_choice, deactivate_provider
    from flash_cli.config import get_env_value, load_config, save_config
    ...
    from flash_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `flash model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from flash_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from flash_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from flash_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Flash will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Flash will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from flash_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from flash_cli.auth import _save_model_choice, deactivate_provider
    from flash_cli.config import load_config, save_config
    from flash_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from flash_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from flash_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from flash_cli.config import save_env_value, load_config, save_config
    from flash_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from flash_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from flash_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Flash turns to `copilot --acp`.")
    print("  Flash currently starts its own ACP subprocess for each request.")
    print("  Flash uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from flash_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from flash_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from flash_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so flash knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from flash_cli.config import load_config, save_config, get_env_value
    from flash_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.flash/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``flash_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from flash_cli.main import _prompt_provider_choice
    from flash_cli.auth import ZAI_ENDPOINTS
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Flash typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Flash, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from flash_cli.main import _run_anthropic_oauth_flow
    from flash_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'flash model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
