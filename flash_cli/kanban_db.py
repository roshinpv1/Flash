...
``<root>`` is the **shared Flash root** (the parent of any active
...
``flash -p <profile>`` joins the same board as the dispatcher that
...
the profile's Flash home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``flash kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.flash``. In Docker / custom
deployments where ``HERMES_HOME`` points outside ``~/.flash`` (e.g.
``/opt/flash``), ``<root>`` is ``HERMES_HOME``. Legacy env-var
...
coding workloads.  See ``docs/flash-kanban-v1-spec.pdf`` for the full
...
from flash_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "flash_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``flash-agent``
...
def kanban_home() -> Path:
    """Return the shared Flash root that anchors the kanban board.

    Resolution order:

    1. ``HERMES_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_flash_root()``, which already returns ``<root>``
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``flash kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``flash kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``flash kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``flash kanban log`` unambiguous even when multiple
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (flash_cli/projects_db). When set,
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``flash gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Flash treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `flash kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from flash_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See flash_state._WAL_INCOMPAT_MARKERS for detection logic.
                from flash_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``flash kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from flash_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``flash --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from flash_cli import projects_db as _pdb
    ...
    # `flash --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``flash kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `flash kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``flash kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``flash kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``flash kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``flash kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Flash' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Flash-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``flash kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Flash checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Flash treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Flash creates/reuses that linked worktree. With
      no ``workspace_path``, Flash anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Flash uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Flash
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``flash kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``flash kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Flash profile.
    ...
    if not rows:
        return False
    try:
        from flash_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from flash_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Flash profile.
        # `_default_spawn` invokes ``flash -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Flash
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `flash kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from flash_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_flash_argv() -> list[str]:
    """Return the interpreter-bound Flash CLI invocation."""
    # ``flash_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``flash`` package — there is no
    # ``flash`` package to import.
    return [sys.executable, "-m", "flash_cli.main"]


def _absolute_flash_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Flash shim."""
...
def _flash_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Flash executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_flash_argv()
    return [_absolute_flash_path(path)]


def _resolve_flash_argv() -> list[str]:
    """Resolve the ``flash`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("flash")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\flash.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m flash_cli.main`` — fallback for setups where
       Flash is launched from a venv and the ``flash`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_flash_bin`` for the same reason. Kept
    local (not imported from gateway) because ``flash_cli`` sits below
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _flash_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _flash_path_argv(resolved_env_bin)
        return _module_flash_argv()

    flash_bin = _safe_which_no_cwd("flash") if _IS_WINDOWS else shutil.which("flash")
    if flash_bin:
        return _flash_path_argv(flash_bin)
    return _module_flash_argv()
...
def _resolve_worker_cli_toolsets(flash_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_flash_home_override(flash_home)
        try:
            cfg = load_config()
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``flash -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `flash -p <name>` the
    # _apply_profile_override() runs *before* flash_constants is imported.
    # If HERMES_HOME is absent from the child's env, get_flash_home() falls
    # back to Path.home() / ".flash" (the DEFAULT profile root), ignoring the
    ...
    # that even when the worker activates a profile (`flash -p <name>`
    # rewrites HERMES_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_flash_root()`
    ...
    # `flash -p <assignee>` activates the profile, but the env var is
    ...
    # older flash builds on PATH that predate the flag's precedence.
    ...
    # `flash kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`flash` executable not found on PATH. "
            "Install Flash Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``flash kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "flash-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Flash root exists

    Reads profile paths directly so this module has no import dependency on
    ``flash_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from flash_constants import get_default_flash_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``flash kanban assignees`` for the terminal.
