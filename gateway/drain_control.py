...
Why the epoch (NS-570). ``HERMES_HOME`` is a **durable** store — on Flash
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``flash gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    ...
    base = home if home is not None else get_flash_home()
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    durable Fly volume on Flash Cloud) and the lifecycle action that triggered
