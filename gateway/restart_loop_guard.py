...
Defenses 1 and 2 (the ``_HERMES_GATEWAY`` guard on ``flash gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.flash.gateway")``,
...
def _state_path():
    return get_flash_home() / "gateway" / "restart_loop.json"
