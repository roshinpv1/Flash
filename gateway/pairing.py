...
from flash_constants import get_flash_dir, get_flash_home
...
PAIRING_DIR = get_flash_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from flash_cli.config import save_env_value, remove_env_value
...
class PairingStore:
    ...
    directory (backward-compat for the ``flash pairing`` CLI).
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            self._dir = get_flash_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `flash` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> flash pairing approve ...` as root, "
                    "re-run with `docker exec -u flash <container> ...` and "
                    "chown the existing file to the flash user, or restart the "
