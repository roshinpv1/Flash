...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``flash setup`` display).
    ...
    # built-ins).  Used by ``flash gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `flash`
        # invocation -- including plain `flash chat`, which never touches any
        ...
        # `flash setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``flash setup``/``gateway status``, channel
