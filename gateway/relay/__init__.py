"""Relay/connector support package for the Flash gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``flash gateway enroll`` writes them to
    ``~/.flash/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``flash gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``flash gateway enroll`` CLI. Two modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         FlashPortal): when ``gateway.idp.token_url`` (or
      ...
      2. **FlashPortal** (default): ``resolve_flash_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — FlashPortal (default, unchanged behaviour).
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Flashaccess token. In
    that case the runtime resolves the agent's own Flashaccess token (the same
    ...
    up. The creds live ONLY in process memory — never written to ``~/.flash/.env``.
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'flash'}"
