...
There are two ways to add a platform to the Nyxo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nyxo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Nyxo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nyxo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nyxo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Nyxo core.
...
And add it to the `nyxo-gateway` composite:

```python
"nyxo-gateway": {
    "includes": [..., "nyxo-your-platform"]
}
```
...
## 12. Status Display (`nyxo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nyxo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nyxo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
