...
There are two ways to add a platform to the Flash gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.flash/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Flash code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `flash gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `flash_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Flash core.
...
And add it to the `flash-gateway` composite:

```python
"flash-gateway": {
    "includes": [..., "flash-your-platform"]
}
```
...
## 12. Status Display (`flash_cli/status.py`)
...
## 13. Gateway Setup Wizard (`flash_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ flash_cli/ toolsets.py \
  --include="*.py" -l | sort -u
