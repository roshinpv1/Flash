...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.flash to the active profile home."""
    ...
    if raw == "~/.flash":
        return flash_home
    if raw.startswith("~/.flash/"):
        return flash_home / raw.removeprefix("~/.flash/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return flash_home / path
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    ...
    scripts_root = (get_flash_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.flash" or raw_text.startswith("~/.flash/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__flash_ignore__") is True
        ):
            return False, None
