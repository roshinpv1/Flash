...
# Audio file extensions Flash recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Flash' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Flash-created Telegram private-chat
...
from flash_constants import get_default_flash_root, get_flash_dir, get_flash_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.flash/.env manually."
)
...
IMAGE_CACHE_DIR = get_flash_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_flash_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_flash_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; FlashAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_flash_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_flash_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_flash_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_flash_dir("cache/screenshots", "browser_screenshots")
...
_HERMES_HOME = get_flash_home()
_HERMES_ROOT = get_default_flash_root()
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Flash root.
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Flash profile and shared Flash root both contain control
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.flash
    ...
    for flash_root in (_HERMES_HOME, _HERMES_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(flash_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Flash
    secrets (``~/.flash/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Flash-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the Flash root (~/.flash/.env,
    ...
    # ``MEDIA:~/.flash/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?flash\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+flash[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Flash commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Flash sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Flash home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Flash home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.flash/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from flash_cli.commands import should_bypass_active_session
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _flash_run_generation is set on
