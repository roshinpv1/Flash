...
# Audio file extensions Nyxo recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Nyxo' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Nyxo-created Telegram private-chat
...
from nyxo_constants import get_default_nyxo_root, get_nyxo_dir, get_nyxo_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.nyxo/.env manually."
)
...
# Default location: {NYXO_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_nyxo_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; NyxoAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_nyxo_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_nyxo_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_nyxo_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_nyxo_dir("cache/screenshots", "browser_screenshots")
_NYXO_HOME = get_nyxo_home()
_NYXO_ROOT = get_default_nyxo_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "NYXO_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "NYXO_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "NYXO_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "NYXO_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _NYXO_HOME / "image_cache",
    _NYXO_HOME / "audio_cache",
    _NYXO_HOME / "video_cache",
    _NYXO_HOME / "document_cache",
    _NYXO_HOME / "browser_screenshots",
    ...
    _NYXO_HOME / "cache" / "images",
    _NYXO_HOME / "cache" / "audio",
    _NYXO_HOME / "cache" / "videos",
    _NYXO_HOME / "cache" / "documents",
    _NYXO_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Nyxo profile and shared Nyxo root both contain control
    ...
    # NYXO_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.nyxo
    ...
    for nyxo_root in (_NYXO_HOME, _NYXO_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(nyxo_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Nyxo
    secrets (``~/.nyxo/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``NYXO_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Nyxo-managed cache, under an operator-allowlisted root
    (``NYXO_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Nyxo root (~/.nyxo/.env,
    ...
    # ``MEDIA:~/.nyxo/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?nyxo\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+nyxo[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``NYXO_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Nyxo commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("NYXO_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "NYXO_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "NYXO_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Nyxo sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.nyxo/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from nyxo_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          NYXO_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          NYXO_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          NYXO_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("NYXO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("NYXO_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("NYXO_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _nyxo_run_generation is set on
