"""Shared constants for Flash Agent.
...
def set_flash_home_override(path: str | Path | None) -> Token:
    """Set a context-local Flash home override and return its reset token.
...
def reset_flash_home_override(token: Token) -> None:
    """Restore the previous context-local Flash home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_flash_home_override() -> str | None:
    """Return the active context-local Flash home override, if any."""
...
def _get_platform_default_flash_home() -> Path:
    """Return the platform-native default Flash home path."""
...
def get_flash_home() -> Path:
    """Return the Flash home directory (default: platform-native path).
    ...
    template in ``flash_cli/gateway.py`` and the kanban dispatcher in
    ``flash_cli/kanban_db.py``).  See https://github.com/FlashOrg/flash-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_flash_home()
    ...
    return _get_platform_default_flash_home()


def get_default_flash_root() -> Path:
    """Return the root Flash directory for profile-level operations.

    In standard deployments this is the platform-native Flash home
    (``~/.flash`` on POSIX, ``%LOCALAPPDATA%\\flash`` on native Windows).

    In Docker or custom deployments where ``HERMES_HOME`` points outside
    ``~/.flash`` (e.g. ``/opt/data``), returns ``HERMES_HOME`` directly
    — that IS the root.

    In profile mode where ``HERMES_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.flash/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_flash_home()
    ...
    try:
        ...
        # HERMES_HOME is under ~/.flash (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Flash is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_flash_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_flash_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_flash_home() / "skills"


def get_flash_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Flash subdirectory with backward compatibility.
...
def iter_flash_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Flash-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\flash\\node``. POSIX installs use
    ``$HERMES_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_flash_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with flashManagedNodePathEntries() in
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Flash-managed Node trees live under ``$HERMES_HOME/node`` (or a profile's
    ...
    immediately throws ``MODULE_NOT_FOUND``. ``find_flash_node_executable``
    used to trust file presence alone, so ``flash update`` would pick that
...
def flash_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Flash-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_flash_node_dirs(home):
        for name in names:
...
def heal_flash_managed_node() -> bool:
    """Redownload Flash-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not flash_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": str(get_flash_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_flash_node_executable(command: str) -> str | None:
    """Return a Flash-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_flash_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_flash_managed_node():
        for directory in iter_flash_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Flash-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Flash-managed installs.

    This is for Flash-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_flash_node_executable(command)
    if managed:
        return managed
    if flash_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_flash_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Flash-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_flash_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``flash update`` — leaving a **dangling symlink** that ``which`` still
...
def display_flash_home() -> str:
    ...
    Uses ``~/`` shorthand for readability::
        ...
        default:  ``~/.flash``
        profile:  ``~/.flash/profiles/coder``
        custom:   ``/opt/flash-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.flash``.  For code that needs a real ``Path``, use
    :func:`get_flash_home` instead.
    """
    home = get_flash_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/FlashOrg/flash-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    flash_home = get_flash_home_override() or (env or {}).get("HERMES_HOME") or os.getenv("HERMES_HOME")
    if not flash_home:
        return None
    profile_home = os.path.join(flash_home, "home")
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Flash profile HOME.

    ``HERMES_HOME`` scopes Flash state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Flash' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def is_container() -> bool:
    ...
    See: FlashOrg/flash-agent#47111
    """
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_flash_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, flash_logging.py, flash_time.py, etc.).
    """
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_flash_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._flash_ipv4_patched = True  # type: ignore[attr-defined]
