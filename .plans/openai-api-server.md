# OpenAI-Compatible API Server for Nyxo Agent
...
SSE streaming. By exposing this endpoint, nyxo-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  nyxo-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.nyxo/.env`
2. Run `nyxo gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with nyxo-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns nyxo-agent as a model) |
...
Authorization: Bearer nyxo-api-key-here
...
But nyxo-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "nyxo-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured nyxo-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of nyxo-agent's core prompt
...
Once implemented, nyxo-agent works as a drop-in backend for:
