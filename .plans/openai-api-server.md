# OpenAI-Compatible API Server for Flash Agent
...
SSE streaming. By exposing this endpoint, flash-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  flash-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.flash/.env`
2. Run `flash gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with flash-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns flash-agent as a model) |
...
Authorization: Bearer flash-api-key-here
...
But flash-agent sessions have persistent state (memory, skills, tool context).
...
The agent's `run_conversation()` is synchroflash and returns the full response.
...
Frontends send `"model": "flash-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured flash-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of flash-agent's core prompt
...
Once implemented, flash-agent works as a drop-in backend for:
