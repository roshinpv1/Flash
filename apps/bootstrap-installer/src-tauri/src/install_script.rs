...
//! Electron app's APP_ROOT/../.. trick, because Flash-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Flash-Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/FlashOrg/flash-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "flash-setup/0.0.1")
        .send()
        .await
