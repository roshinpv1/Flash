...
//!
//! Driven when the installer is launched as `Flash-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Flash desktop process to fully exit (so both the
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let flash = resolve_flash(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the flash CLI under {}. Is Flash installed? \
    ...
    // and the guard would only produce a false "Flash is still running" stop.
    //
    ...
    // below surfaces the correct "close all Flash windows" message.
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Flash" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Flash is still running. Close all Flash windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Flash manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Flash to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend flash.exe (or the desktop Flash.exe
            ...
            // force-kill every Flash.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Flash still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Flash.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Flash.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// `startFlash()` in the desktop gates local-backend startup on our
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_flash_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Flash.app was found under {}",
...
mod tests {
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Flash.app"]),
            Some(PathBuf::from("/Applications/Flash.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Flash.app");
        let tmp = base.join("Flash.app.flash-update-new");
        let old = base.join("Flash.app.flash-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Flash.app");
        let tmp = base.join("Flash.app.flash-update-new"); // intentionally absent
        let old = base.join("Flash.app.flash-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Flash.app");
        let tmp = base.join("Flash.app.flash-update-new"); // absent
        let old = base.join("Flash.app.flash-update-old");
        write_marker(&target, "OLD");
