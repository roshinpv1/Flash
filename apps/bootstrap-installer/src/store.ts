...
export const $flashHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => { }
    $logPath.set('~/.flash/logs/bootstrap-installer.log')
    $flashHome.set('~/.flash')
  ...
  try {
    const [logPath, flashHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_flash_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $flashHome.set(flashHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Flash" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Flash-Setup.exe --update);
...
export async function launchFlashDesktop(): Promise<void> {
  ...
  await invoke('launch_flash_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Flash repository' },
