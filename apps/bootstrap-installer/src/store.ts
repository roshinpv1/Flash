...
export const $nyxoHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, nyxoHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_nyxo_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $nyxoHome.set(nyxoHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Nyxo" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Nyxo-Setup.exe --update);
...
export async function launchNyxoDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_nyxo_desktop', { installRoot })
}
