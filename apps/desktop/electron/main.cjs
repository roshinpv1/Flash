...
const { canImportNyxoCli, verifyNyxoCli } = require('./backend-probes.cjs')
...
const { buildDesktopBackendEnv, normalizeNyxoHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.NYXO_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NYXO_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with NYXO_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[nyxo] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}

ipcMain.handle('nyxo:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[nyxo] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[nyxo] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[nyxo] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NYXO_HOME — the user-facing root for everything Nyxo-related. Mirrors
// scripts/install.ps1's $NyxoHome and scripts/install.sh's $NYXO_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\nyxo (matches install.ps1)
//   macOS / Linux: ~/.nyxo (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.nyxo directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\nyxo yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NYXO_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NYXO_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.nyxo / %LOCALAPPDATA%\nyxo.
function resolveNyxoHome() {
  if (process.env.NYXO_HOME) return normalizeNyxoHomeRoot(process.env.NYXO_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'nyxo-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NYXO_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\nyxo and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NYXO_HOME')
    if (fromRegistry) return normalizeNyxoHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'nyxo')
    const legacy = path.join(app.getPath('home'), '.nyxo')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.nyxo setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.nyxo')
}

const NYXO_HOME = resolveNyxoHome()

function nyxoManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_nyxo_node_dirs() in
  // nyxo_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(NYXO_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithNyxoManagedNode(...entries) {
  return [...nyxoManagedNodePathEntries(), ...entries, process.env.PATH]
    .filter(Boolean)
    .join(path.delimiter)
}

// ACTIVE_NYXO_ROOT — the canonical mutable Nyxo install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NYXO_ROOT = path.join(NYXO_HOME, 'nyxo-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NYXO_ROOT, 'venv')
...
// boots, going straight to `resolveNyxoBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NYXO_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_NYXO_ROOT, '.nyxo-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Nyxo profile the desktop launches its
// local backend as. When set, startNyxo() passes `nyxo --profile <name>
// dashboard …`, which deterministically pins NYXO_HOME (see
// _apply_profile_override in nyxo_cli/main.py) and bypasses the sticky
// ~/.nyxo/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors nyxo_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// nyxoDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NYXO_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by nyxo_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NYXO_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (nyxo_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.NYXO_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NYXO_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'nyxo:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.nyxo) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.nyxo')
}
// Seed the native About panel with the live Nyxo version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `nyxo update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveNyxoVersion(),
...
const MEDIA_PROTOCOL = 'nyxo-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(nyxo-media://…)`.
...
let mainWindow = null
let nyxoProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by nyxoProcess +
// connectionPromise + startNyxo(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.NYXO_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.NYXO_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startNyxo() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nyxo backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  nyxoLog.push(...lines)
  if (nyxoLog.length > 300) {
    nyxoLog.splice(0, nyxoLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-nyxo-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('nyxo:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the nyxo:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nyxo:bootstrap:event', ev)
}
...
// The Tauri updater writes NYXO_HOME/.nyxo-update-in-progress for the whole
...
// (`force_kill_other_nyxo`, taskkill /IM nyxo.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(NYXO_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Nyxo will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(NYXO_HOME)
  }
  if (marker) {
...
function isNyxoSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'nyxo_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.NYXO_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Nyxo deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Nyxo' terminal tool requires
...
function findGitBash() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nyxo', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'nyxo', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nyxo', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'nyxo', 'git', 'bin', 'git.exe'))
  }
...
function recentNyxoLog() {
  return nyxoLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's nyxo root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NYXO_ROOT.
// NYXO_DESKTOP_NYXO_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NYXO_DESKTOP_NYXO_ROOT && path.resolve(process.env.NYXO_DESKTOP_NYXO_ROOT),
    !IS_PACKAGED && isNyxoSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNyxoSourceRoot(ACTIVE_NYXO_ROOT) ? ACTIVE_NYXO_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NYXO_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('nyxo:updates:progress', merged)
  }
}
...
// NYXO_HOME/nyxo-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_nyxo_home). That binary owns
// ALL repo mutation — running `nyxo update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'nyxo-setup.exe' : 'nyxo-setup'
  const candidate = path.join(NYXO_HOME, name)
...
// Path to the venv shim whose lock decides whether `nyxo update` can write
// fresh entry points. On Windows this is the file the running backend
// `nyxo.exe` holds open; on POSIX it's never mandatory-locked.
function venvNyxoShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'nyxo.exe')
    : path.join(updateRoot, 'venv', 'bin', 'nyxo')
}
...
// only signals the direct child, so on Windows a backend `nyxo.exe` that
// spawned its own grandchildren (a `nyxo` REPL, a pty terminal session, the
...
// `nyxoProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `nyxo.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a nyxo
// REPL, a pty terminal, the gateway) keep `nyxo.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (nyxoProcess && Number.isInteger(nyxoProcess.pid)) pids.push(nyxoProcess.pid)
  ...
  if (nyxoProcess && !nyxoProcess.killed) {
    try {
      nyxoProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvNyxoShimPath(updateRoot)
...
// `nyxo update`). Instead we spawn the staged Nyxo-Setup binary with
// --update and quit, so it can run `nyxo update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri nyxo-setup. Unlike Windows
      ...
      // whole update itself: `nyxo update` (backend) + `nyxo desktop
    ...
    if (!updater) {
      ...
      // `nyxo desktop`, never the Tauri installer that self-copies
      // nyxo-setup.exe into NYXO_HOME). They DO have a working `nyxo`
      ...
      // checkout they're on — bare `nyxo update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'nyxo update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `nyxo update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `nyxo update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, nyxoRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating Nyxo — this window will close and the updater will open. Don’t reopen Nyxo yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // nyxo.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `nyxo update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: NYXO_HOME,
      env: {
        ...process.env,
        NYXO_HOME,
        PATH: pathWithNyxoManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvNyxo = path.join(venvBin, IS_WINDOWS ? 'nyxo.exe' : 'nyxo')
  const updaterArgs = fileExists(venvNyxo) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the nyxo CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `nyxo` on PATH.
function resolveNyxoCliBinary(updateRoot) {
  const venvNyxo = path.join(updateRoot, 'venv', 'bin', 'nyxo')
  if (fileExists(venvNyxo)) return venvNyxo
  return findOnPath('nyxo') || null
}
...
// macOS/Linux in-app update: backend (`nyxo update`) + OS-aware GUI rebuild
// (`nyxo desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const nyxo = resolveNyxoCliBinary(updateRoot)
  if (!nyxo) {
    emitUpdateProgress({ stage: 'manual', message: 'nyxo update', percent: null })
    return { ok: true, manual: true, command: 'nyxo update', nyxoRoot: updateRoot }
  }

  // Put the Nyxo-managed Node and the venv on PATH so `nyxo desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\nyxo\node, not node\bin.
  const env = {
    NYXO_HOME,
    PATH: pathWithNyxoManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `nyxo update` reaps stale `nyxo dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads NYXO_DESKTOP_CHILD_PID and excludes
  ...
  if (nyxoProcess && Number.isInteger(nyxoProcess.pid)) {
    desktopChildPids.push(nyxoProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.NYXO_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Nyxo (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(nyxo, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'nyxo update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'nyxo update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(nyxo, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Nyxo to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `nyxo desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Nyxo…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `nyxo-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Nyxo to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Nyxo desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Nyxo to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Nyxo.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Nyxo.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Nyxo to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.nyxo-update-new"; then
    rm -rf "$DST.nyxo-update-old" 2>/dev/null || true
    mv "$DST" "$DST.nyxo-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.nyxo-update-new" "$DST"
    rm -rf "$DST.nyxo-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Nyxo to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `nyxo
  ...
  return isNyxoSourceRoot(ACTIVE_NYXO_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.NYXO_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NYXO_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: nyxo desktop --force-build`
  )
  return candidates[0]
}
...
function resolveNyxoCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Nyxo.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNyxoCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'nyxo_cli.main', ...dashboardArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_NYXO_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Nyxo at ${ACTIVE_NYXO_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'nyxo_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      nyxoHome: NYXO_HOME,
      pythonPathEntries: [ACTIVE_NYXO_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NYXO_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveNyxoBackend(dashboardArgs) {
  // 1. Explicit override -- NYXO_DESKTOP_NYXO_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.NYXO_DESKTOP_NYXO_ROOT && path.resolve(process.env.NYXO_DESKTOP_NYXO_ROOT)
  if (overrideRoot && isNyxoSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nyxo source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `nyxo` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNyxoSourceRoot.)
  if (!IS_PACKAGED && isNyxoSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nyxo source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_NYXO_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\nyxo\nyxo-agent (Windows) or ~/.nyxo/nyxo-agent.
  ...
  //    to spawning nyxo. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `nyxo update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `nyxo` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NYXO_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NYXO_DESKTOP_IGNORE_EXISTING !== '1') {
    let nyxoCommand = null
    const nyxoOverride = process.env.NYXO_DESKTOP_NYXO

    if (nyxoOverride) {
      const resolvedOverride = findOnPath(nyxoOverride)
      if (resolvedOverride) {
        nyxoCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(nyxoOverride, { isWsl: IS_WSL })) {
        nyxoCommand = nyxoOverride
      } else {
        rememberLog(`Ignoring Windows Nyxo override under WSL: ${nyxoOverride}`)
      }
    } else {
      nyxoCommand = findOnPath('nyxo')
    }

    if (nyxoCommand) {
      if (looksLikeDesktopAppBinary(nyxoCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nyxo CLI: ${nyxoCommand}`)
        nyxoCommand = null
      }
    }

    if (nyxoCommand) {
      // Smoke-test the candidate before trusting it. A `nyxo` shim
      ...
      const shellForProbe = isCommandScript(nyxoCommand)
      if (verifyNyxoCli(nyxoCommand, { shell: shellForProbe })) {
        return {
          label: `existing Nyxo CLI at ${nyxoCommand}`,
          command: nyxoCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
      ...
      rememberLog(
        `Ignoring existing Nyxo CLI at ${nyxoCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed nyxo_cli module via system Python.
  ...
  if (python) {
    ...
    // having nyxo_cli installed -- common on dev boxes that have
    ...
    if (canImportNyxoCli(python)) {
      return {
        kind: 'python',
        label: `installed nyxo_cli module via ${python}`,
        command: python,
        args: ['-m', 'nyxo_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: nyxo_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNyxoBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Nyxo Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNyxoBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nyxo install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Nyxo recovery was handed off to Nyxo Setup. The desktop will restart when recovery completes.')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      nyxoHome: NYXO_HOME,
      logRoot: path.join(NYXO_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nyxo install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nyxo bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NYXO_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startNyxo() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // nyxo:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveNyxoBackend(backend.args))
  }
  ...
  if (!isNyxoSourceRoot(ACTIVE_NYXO_ROOT)) {
    throw new Error(
      `Nyxo install at ${ACTIVE_NYXO_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Nyxo' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\nyxo\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `nyxo` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nyxo on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isNyxoSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Nyxo venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Nyxo at ${ACTIVE_NYXO_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nyxo runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Nyxo backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Nyxo backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Nyxo backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Nyxo-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('nyxo:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNyxoCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nyxo:preview-file-changed', payload)
}
...
async function waitForNyxo(baseUrl, token) {
  ...
  throw new Error(`Nyxo backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nyxo:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nyxo:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Nyxo...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nyxo:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('nyxo:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('nyxo:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'nyxo:desktop:zoomLevel'
...
//
// Hosted Nyxo gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``nyxo_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / nyxo #37247).
//     Both are set as HttpOnly cookies (``nyxo_session_at`` ~15 min,
//     ``nyxo_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:nyxo-remote-oauth'
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nyxo backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root NYXO_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NYXO_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.NYXO_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by NYXO_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (nyxo #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Nyxo gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Nyxo gateway is selected, but no session token is saved. ' +
...
//   2. env override (NYXO_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.NYXO_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.NYXO_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'NYXO_DESKTOP_REMOTE_URL is set but NYXO_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Nyxo backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.NYXO_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nyxo
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startNyxo())
  ...
  // connect to Nyxo gateway". Mirror the renderer's connect here so the test
...
// startNyxo() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetNyxoConnection() nulls nyxoProcess.
  const dying = nyxoProcess && !nyxoProcess.killed ? nyxoProcess : null
  resetNyxoConnection()
...
// profile to startNyxo() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startNyxo() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForNyxo(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: nyxoLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited NYXO_HOME env (see _apply_profile_override
  // step 3 in nyxo_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveNyxoBackend(dashboardArgs))
  const nyxoCwd = resolveNyxoCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Nyxo backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: nyxoCwd,
        NYXO_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        NYXO_DESKTOP: '1',
        NYXO_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Nyxo backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Nyxo backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Nyxo backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForNyxo(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Nyxo backend for profile "${profile}"`,
    rememberLog
  })
...
async function startNyxo() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startNyxo() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Nyxo backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Nyxo backend at ${remote.baseUrl}`, 24)
      await waitForNyxo(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nyxo backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.nyxo/active_profile file) and
    // resolves NYXO_HOME the same way `nyxo -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Nyxo runtime', 28)
    const backend = await ensureRuntime(resolveNyxoBackend(dashboardArgs))
    const nyxoCwd = resolveNyxoCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Nyxo backend via ${backend.label}`, 84)
    rememberLog(`Starting Nyxo backend via ${backend.label}`)
    ...
    nyxoProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin NYXO_HOME for the child so Python's get_nyxo_home()
          // resolves to the SAME location our resolveNyxoHome() picked. Without
          // this pin, Python falls back to ~/.nyxo on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\nyxo, which differs from C:\Users\<u>\.nyxo.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets NYXO_HOME via setx; the desktop
    ...
    nyxoProcess.stdout.on('data', rememberLog)
    nyxoProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    nyxoProcess.once('error', error => {
      rememberLog(`Nyxo backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nyxo backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    nyxoProcess.once('exit', (code, signal) => {
      rememberLog(`Nyxo backend exited (${signal || code})`)
      nyxoProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Nyxo backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nyxo backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNyxoLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nyxo backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(nyxoProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nyxo backend to become ready', 90)
    await Promise.race([waitForNyxo(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null nyxoProcess when the child dies.
      childAlive: () => nyxoProcess !== null && nyxoProcess.exitCode === null && !nyxoProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nyxo backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Nyxo is
...
// pushes pet state over IPC (nyxo:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via nyxo:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Nyxo app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Nyxo icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see nyxo:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Nyxo in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('nyxo:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startNyxo().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('nyxo:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Nyxo…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('nyxo:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetNyxoConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Nyxo backend failed liveness probe; dropping stale connection.')
    resetNyxoConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('nyxo:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('nyxo:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('nyxo:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('nyxo:window:openNewSession', async () => {
...
ipcMain.handle('nyxo:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('nyxo:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('nyxo:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('nyxo:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('nyxo:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('nyxo:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('nyxo:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('nyxo:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('nyxo:pet-overlay:control', payload)
})
ipcMain.handle('nyxo:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startNyxo() call restarts the
...
ipcMain.handle('nyxo:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startNyxo() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('nyxo:bootstrap:cancel', async () => {
...
ipcMain.handle('nyxo:boot-progress:get', async () => bootProgressState)
ipcMain.handle('nyxo:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('nyxo:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('nyxo:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('nyxo:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('nyxo:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('nyxo:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('nyxo:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('nyxo:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('nyxo:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('nyxo:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NYXO_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('nyxo:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('nyxo:requestMicrophoneAccess', async () => {
...
ipcMain.handle('nyxo:api', async (_event, request) => {
...
ipcMain.handle('nyxo:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('nyxo:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('nyxo:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('nyxo:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('nyxo:readFileText', async (_event, filePath) => {
...
ipcMain.handle('nyxo:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('nyxo:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('nyxo:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('nyxo:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('nyxo:saveClipboardImage', async () => {
...
ipcMain.handle('nyxo:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('nyxo:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('nyxo:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('nyxo:titlebar-theme', (_event, payload) => {
...
ipcMain.on('nyxo:native-theme', (_event, mode) => {
...
ipcMain.on('nyxo:translucency', (_event, payload) => {
...
ipcMain.handle('nyxo:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('nyxo:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNyxoCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('nyxo:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNyxoCwd()
}))

ipcMain.handle('nyxo:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('nyxo:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('nyxo:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('nyxo:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('nyxo:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('nyxo:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: nyxoLog.slice(-200) }))
...
function terminalShellCommand() {
  // NYXO_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.NYXO_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Nyxo' TUI into
  ...
  env.TERM_PROGRAM = 'Nyxo'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a nyxo/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from NYXO_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.NYXO_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `nyxo:terminal:${id}:${suffix}`
}
...
ipcMain.handle('nyxo:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('nyxo:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('nyxo:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('nyxo:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Nyxo.')
  }
...
ipcMain.handle('nyxo:terminal:write', (_event, id, data) => {
...
ipcMain.handle('nyxo:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nyxo:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('nyxo:updates:check', async () =>
...
ipcMain.handle('nyxo:updates:apply', async (_event, payload) =>
...
ipcMain.handle('nyxo:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('nyxo:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Nyxo version (the one `release.py` bumps in
// nyxo_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nyxo version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNyxoVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'nyxo_cli', '__init__.py')
...
// Re-resolve the live Nyxo version and push it into the native About panel
// just before showing it, so an in-place `nyxo update` is reflected without
...
ipcMain.handle('nyxo:version', async () => ({
  appVersion: resolveNyxoVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  nyxoRoot: resolveUpdateRoot()
}))
...
// the actual removal via `nyxo uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (nyxo_cli/uninstall.py + nyxo_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NYXO_ROOT
  ...
  const fallback = () => ({
    nyxo_home: NYXO_HOME,
    agent_installed: isNyxoSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'nyxo_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NYXO_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Nyxo agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import nyxo_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_NYXO_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `nyxo-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `nyxo-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('nyxo:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('nyxo:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('nyxo:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('nyxo:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// nyxo:// deep links (e.g. nyxo://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const NYXO_PROTOCOL = 'nyxo'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${NYXO_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // nyxo://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('nyxo:deep-link', payload)
...
ipcMain.handle('nyxo:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${NYXO_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NYXO_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(NYXO_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `nyxo://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching nyxo:// URL is in our own argv.
