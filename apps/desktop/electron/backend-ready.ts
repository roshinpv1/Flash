...
// `flash serve` announces HERMES_BACKEND_READY; the legacy `flash dashboard`
...
// compile and import the whole `flash_cli.main` → `web_server` → FastAPI/
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Flash backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Flash backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
