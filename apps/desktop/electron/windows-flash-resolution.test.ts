// Regression guards for Windows `flash` resolution in main.ts.
//
...
//      Git-Bash `flash` shim shadowed the real flash.cmd/flash.exe; the
...
//      --repair by checking ONLY venv\Scripts\flash.exe (the console-script
...
//      installs.
//   3. unwrapWindowsVenvFlashCommand() returned the venv python with NO
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow flash.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\flash.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvFlash\) \?/,
    'recovery regressed to gating only on the flash.exe shim, which forces destructive --repair'
  )
})

test('unwrapWindowsVenvFlashCommand smoke-tests the venv python before trusting it', () => {
  const source = readMain()
  const fnStart = source.indexOf('function unwrapWindowsVenvFlashCommand(')
  assert.notEqual(fnStart, -1, 'unwrapWindowsVenvFlashCommand must exist in main.ts')
