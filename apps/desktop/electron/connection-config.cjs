 ...
 *     `X-Nyxo-Session-Token` header; WS uses `?token=`.
...
// nyxo_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - nyxo_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - nyxo_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / nyxo #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-nyxo_session_at', '__Secure-nyxo_session_at', 'nyxo_session_at']
const RT_COOKIE_VARIANTS = ['__Host-nyxo_session_rt', '__Secure-nyxo_session_rt', 'nyxo_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://nyxo.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a nyxo session ACCESS-token cookie
