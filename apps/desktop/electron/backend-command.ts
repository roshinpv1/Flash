...
// Backend subcommand routing for the desktop-managed Flash process.
//
// The desktop app launches its own headless backend via `flash serve` — it
...
// app hasn't updated yet, or an older `flash` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Flash profile to pin via `--profile`.
 */
 ...
 * `-m flash_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `flash_cli/subcommands/dashboard.py` source registers
