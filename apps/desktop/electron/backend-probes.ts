 ...
 * path) but no flash_cli in its site-packages: the resolver hands back
 * a backend the spawn step can't actually run, and the user gets a
 * dead-on-arrival "ModuleNotFoundError: No module named 'flash_cli'"
 ...
 * @returns {string}
 */
function flashRuntimeImportProbe() {
  return 'import yaml; import dotenv; import flash_cli.config'
}
 ...
 * dies on spawn.
 *
 * The probe intentionally imports flash_cli.config, not just the top-level
...
function canImportFlashCli(pythonPath: string, opts: { env?: Record<string, string> } = {}) {
  ...
  try {
    execFileSync(pythonPath, ['-c', flashRuntimeImportProbe()], {
...
/**
 * Return true iff `<flashCommand> --version` exits 0.
 *
 * Used to gate the "existing `flash` on PATH" rung. Without this, a
 * stale flash.cmd shim left behind by an uninstalled pip install (or
 * a half-built venv whose `flash` entry-point points at a deleted
 * Python) survives findOnPath() and gets selected as the backend.
 *
 * We intentionally avoid invoking the command with the dashboard args
 * here -- `--version` is the cheapest "is this binary alive" smoke
 * test that every flash_cli entry-point has supported since 0.1.
 *
 * @param {string} flashCommand - Resolved absolute path to a flash
...
function verifyFlashCli(flashCommand: string, opts?: { shell?: boolean }) {
  ...
  try {
    execFileSync(flashCommand, ['--version'], {
      stdio: 'ignore',
      timeout: PROBE_TIMEOUT_MS,
