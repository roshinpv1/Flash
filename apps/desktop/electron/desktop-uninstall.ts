 ...
 * the three user-facing uninstall modes to the `flash uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m flash_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `flash_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'flash_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Flash.app/Contents/MacOS/Flash  → …/Flash.app
 *   Windows: …\Flash\Flash.exe                 → …\Flash  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Flash.app/Contents/MacOS/Flash → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Flash.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Flash.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Flash$/i.test(dir) || /[\\/]flash-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/flash
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import flash_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, flashHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export HERMES_HOME=${q(flashHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import flash_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Flash install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "HERMES_HOME=${String(flashHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
