...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'nyxo_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'nyxo_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'nyxo_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Nyxo.app/Contents/MacOS/Nyxo', 'darwin'),
    '/Applications/Nyxo.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Nyxo.app/Contents/MacOS/Nyxo', 'darwin'),
    '/Users/x/Applications/Nyxo.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Nyxo\\Nyxo.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Nyxo'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\nyxo-desktop\\Nyxo.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\nyxo-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Nyxo.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_NyxoXXXX/nyxo', 'linux', { APPIMAGE: '/home/x/Apps/Nyxo.AppImage' }),
    '/home/x/Apps/Nyxo.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/nyxo/linux-unpacked/nyxo', 'linux', {}),
    '/opt/nyxo/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/nyxo', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Nyxo.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Nyxo.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.nyxo/nyxo-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.nyxo/nyxo-agent',
    uninstallArgs: ['-m', 'nyxo_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/nyxo/linux-unpacked',
    nyxoHome: '/home/x/.nyxo'
  })
  ...
  assert.match(script, /'-m' 'nyxo_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/nyxo\/linux-unpacked'/)
  assert.match(script, /export NYXO_HOME='\/home\/x\/\.nyxo'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.nyxo/nyxo-agent',
    agentRoot: '/home/x/.nyxo/nyxo-agent',
    uninstallArgs: ['-m', 'nyxo_cli.uninstall', '--mode', 'full'],
    appPath: null,
    nyxoHome: '/home/x/.nyxo'
  })
  // System python + source on PYTHONPATH so import nyxo_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.nyxo\/nyxo-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'nyxo_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'nyxo_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    nyxoHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'nyxo_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\nyxo',
    uninstallArgs: ['-m', 'nyxo_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Nyxo',
    nyxoHome: 'C:\\Users\\x\\AppData\\Local\\nyxo'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import nyxo_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\nyxo;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "nyxo_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Nyxo" >nul 2>&1/)
