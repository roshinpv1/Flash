...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'flash_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'flash_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'flash_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Flash.app/Contents/MacOS/Flash', 'darwin'),
    '/Applications/Flash.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Flash.app/Contents/MacOS/Flash', 'darwin'),
    '/Users/x/Applications/Flash.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Flash\\Flash.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Flash'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\flash-desktop\\Flash.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\flash-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Flash.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_FlashXXXX/flash', 'linux', { APPIMAGE: '/home/x/Apps/Flash.AppImage' }),
    '/home/x/Apps/Flash.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/flash/linux-unpacked/flash', 'linux', {}), '/opt/flash/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/flash', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Flash.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Flash.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.flash/flash-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.flash/flash-agent',
    uninstallArgs: ['-m', 'flash_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/flash/linux-unpacked',
    flashHome: '/home/x/.flash'
  })
  ...
  assert.match(script, /'-m' 'flash_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/flash\/linux-unpacked'/)
  assert.match(script, /export HERMES_HOME='\/home\/x\/\.flash'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.flash/flash-agent',
    agentRoot: '/home/x/.flash/flash-agent',
    uninstallArgs: ['-m', 'flash_cli.uninstall', '--mode', 'full'],
    appPath: null,
    flashHome: '/home/x/.flash'
  })

  // System python + source on PYTHONPATH so import flash_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.flash\/flash-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'flash_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'flash_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    flashHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'flash_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\flash',
    uninstallArgs: ['-m', 'flash_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Flash',
    flashHome: 'C:\\Users\\x\\AppData\\Local\\flash'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import flash_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\flash;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "flash_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Flash" >nul 2>&1/)
