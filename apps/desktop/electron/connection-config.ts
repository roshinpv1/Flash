 ...
 *     `X-Flash-Session-Token` header; WS uses `?token=`.
...
// flash_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - flash_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - flash_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / flash #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-flash_session_at', '__Secure-flash_session_at', 'flash_session_at']
const RT_COOKIE_VARIANTS = ['__Host-flash_session_rt', '__Secure-flash_session_rt', 'flash_session_rt']

// The Flashportal (NAS) does NOT use Flash gateway session cookies — it is a
...
// Flash Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://flash.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a flash session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live FlashPORTAL (Privy) session — a non-empty
 ...
 * the Flash gateway session cookies, so cloud sign-in / discovery liveness
