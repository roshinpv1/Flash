...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'flash-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/flash-runner-test',
    sourceRepoRoot: null,
    flashHome: '/tmp/flash-runner-test',
    logRoot: '/tmp/flash-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'flash-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'flash-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/flash-agent', '--flash-home', '/tmp/flash', '--branch', 'main']
  )
})
