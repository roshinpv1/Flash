 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Flash Agent by spawning
...
function bootstrapCacheDir(flashHome) {
  return path.join(flashHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.flash/flash-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(flashHome) {
  if (!flashHome) {
    return null
  }
  const candidate = path.join(flashHome, 'flash-agent', 'scripts', installScriptName())
...
function cachedScriptPath(flashHome, commit) {
  return path.join(bootstrapCacheDir(flashHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/FlashOrg/flash-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(flashHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(flashHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, flashHome }: any = {}) {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...
          HERMES_HOME: flashHome || process.env.HERMES_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, flashHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, flashHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--flash-home', flashHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, flashHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
      ...
      '--non-interactive',
      '--json',
      ...buildPosixPinArgs({ installStamp, activeRoot, flashHome, pinCommit })
    ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(flashHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, flashHome, emit })
