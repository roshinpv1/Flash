 ...
 *      internals; NYXO_HOME / NYXO_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.nyxo/nyxo-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'nyxo')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Nyxo12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/nyxo/nyxo', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Nyxo/nyxo', ROOT, 'linux'), null)
  // dev electron
  assert.equal(resolveUnpackedRelease('/home/u/.nyxo/nyxo-agent/node_modules/electron/dist/electron', ROOT, 'linux'), null)
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'nyxo'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'nyxo')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'nyxo://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]
  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'nyxo://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves NYXO_HOME + NYXO_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    NYXO_HOME: '/home/u/.nyxo',
    NYXO_DESKTOP_REMOTE_URL: 'http://box:9119',
    NYXO_DESKTOP_REMOTE_TOKEN: 'secret',
    NYXO_DESKTOP_NYXO_ROOT: '/home/u/dev/nyxo',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.nyxo/nyxo-agent/apps/desktop/release/linux-unpacked/Nyxo',
    args: ['nyxo://open/agent/42', "--note=it's fine"],
    env: { NYXO_HOME: '/home/u/.nyxo', NYXO_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export NYXO_HOME='\/home\/u\/\.nyxo'/)
  assert.match(script, /export NYXO_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Nyxo' 'nyxo:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `nyxo-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Nyxo/Nyxo',
    args: [],
    env: {},
    cwd: ''
  })
  const tmp = path.join(os.tmpdir(), `nyxo-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  try {
    execFileSync('bash', ['-n', tmp], { stdio: 'pipe' })
  } finally {
  ...
  assert.match(script, /exec '\/opt\/Nyxo\/Nyxo'\n/)
})
