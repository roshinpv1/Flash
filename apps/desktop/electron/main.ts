...
import { buildDesktopBackendEnv, normalizeFlashHomeRoot } from './backend-env'
import { canImportFlashCli, verifyFlashCli } from './backend-probes'
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[flash] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[flash] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('flash:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[flash] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[flash] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[flash] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[flash] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// HERMES_HOME — the user-facing root for everything Flash-related. Mirrors
// scripts/install.ps1's $FlashHome and scripts/install.sh's $HERMES_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\flash (matches install.ps1)
//   macOS / Linux: ~/.flash (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.flash directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\flash yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// HERMES_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// HERMES_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.flash / %LOCALAPPDATA%\flash.
function resolveFlashHome() {
  if (process.env.HERMES_HOME) {
    return normalizeFlashHomeRoot(process.env.HERMES_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'flash-home')
  }
  ...
  if (IS_WINDOWS) {
    ...
    // backend silently falls back to %LOCALAPPDATA%\flash and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('HERMES_HOME')

    if (fromRegistry) {
      return normalizeFlashHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'flash')
    const legacy = path.join(app.getPath('home'), '.flash')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.flash setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.flash')
}

const HERMES_HOME = resolveFlashHome()

function flashManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_flash_node_dirs() in
  // flash_constants.py — this Node main process cannot import the Python
...
function pathWithFlashManagedNode(...entries) {
  return [...flashManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_HERMES_ROOT — the canonical mutable Flash install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_HERMES_ROOT = path.join(HERMES_HOME, 'flash-agent')
...
// boots, going straight to `resolveFlashBackend()`. Missing or stale marker
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HERMES_ROOT, '.flash-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Flash profile the desktop launches its
// local backend as. When set, startFlash() passes `flash --profile <name>
// dashboard …`, which deterministically pins HERMES_HOME (see
// _apply_profile_override in flash_cli/main.py) and bypasses the sticky
// ~/.flash/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors flash_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// flashDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under HERMES_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by flash_logging.setup_logging — one log
...
//
// Mirror the Python logs (flash_logging.py RotatingFileHandler, maxBytes x
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Flash'
...
// 'flash:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.flashorg.flash) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.flashorg.flash')
}

// Seed the native About panel with the live Flash version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `flash update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'flash-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(flash-media://…)`.
...
// (the desktop's launch profile) stays managed by flashProcess +
// connectionPromise + startFlash(); this pool only holds EXTRA profile
...
// onto the error so subsequent startFlash() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning flash serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Flash backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (flashLog.length > 300) {
    flashLog.splice(0, flashLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-flash-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('flash:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the flash:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('flash:bootstrap:event', ev)
}
...
// The Tauri updater writes HERMES_HOME/.flash-update-in-progress for the whole
...
// (`force_kill_other_flash`, taskkill /IM flash.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Flash will start automatically when it completes…',
      12
    )
...
function findOnPath(command) {
  ...
  // shell-script shim named `flash` — must not shadow `flash.cmd`/`flash.exe`.
...
function unwrapWindowsVenvFlashCommand(command, backendArgs) {
  ...
  if (!/^flash(?:\.exe)?$/i.test(path.basename(resolved))) {
    return null
  }
  ...
  // died mid-`pip install` still has python.exe + flash.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Flash at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Flash Python at ${python}`,
    command: python,
    args: ['-m', 'flash_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      flashHome: HERMES_HOME,
...
// spawns `flash serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `flash` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'flash_cli', 'subcommands', 'dashboard.py'), 'utf8')
...
function isFlashSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'flash_cli', 'main.py'))
}
...
function findSystemPython() {
  ...
  // CPython but several Flash deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Flash' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\flash\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'flash', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'flash', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\flash\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'flash', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'flash', 'git', 'bin', 'git.exe'))
  }
...
function recentFlashLog() {
  return flashLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's flash root) ──────
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    !IS_PACKAGED && isFlashSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isFlashSourceRoot(ACTIVE_HERMES_ROOT) ? ACTIVE_HERMES_ROOT : null
  ].filter(Boolean)
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('flash:updates:progress', merged)
  }
}
...
// HERMES_HOME/flash-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_flash_home). That binary owns
// ALL repo mutation — running `flash update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'flash-setup.exe' : 'flash-setup'
...
// Path to the venv shim whose lock decides whether `flash update` can write
// fresh entry points. On Windows this is the file the running backend
// `flash.exe` holds open; on POSIX it's never mandatory-locked.
function venvFlashShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'flash.exe')
    : path.join(updateRoot, 'venv', 'bin', 'flash')
}
...
// `flashProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `flash.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a flash
// REPL, a pty terminal, the gateway) keep `flash.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (flashProcess && Number.isInteger(flashProcess.pid)) {
    pids.push(flashProcess.pid)
  }
  ...
  if (flashProcess && !flashProcess.killed) {
    try {
  ...
  const shim = venvFlashShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (flashProcess && Number.isInteger(flashProcess.pid)) {
      stragglers.push(flashProcess.pid)
    }
...
// `flash update`). Instead we spawn the staged Flash-Setup binary with
// --update and quit, so it can run `flash update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri flash-setup. Unlike Windows
      ...
      // whole update itself: `flash update` (backend) + `flash desktop
    ...
    if (!updater) {
      ...
      // `flash desktop`, never the Tauri installer that self-copies
      // flash-setup.exe into HERMES_HOME). They DO have a working `flash`
      ...
      // checkout they're on — bare `flash update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `flash update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `flash update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, flashRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Flash — this window will close and the updater will open. Don’t reopen Flash yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // flash.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running flash, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Flash install open ' +
        '(a second Flash window or a terminal running flash?). Close it and retry.'
    ...
    // `flash update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: HERMES_HOME,
      env: {
        ...process.env,
        HERMES_HOME,
        PATH: pathWithFlashManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvFlash = path.join(venvBin, IS_WINDOWS ? 'flash.exe' : 'flash')
  ...
  // not just the `flash.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvFlash) || fileExists(path.join(updateRoot, '.flash-bootstrap-complete'))
...
// Resolve the flash CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `flash` on PATH.
function resolveFlashCliBinary(updateRoot) {
  const venvFlash = path.join(updateRoot, 'venv', 'bin', 'flash')

  if (fileExists(venvFlash)) {
    return venvFlash
  }

  return findOnPath('flash') || null
}
...
// macOS/Linux in-app update: backend (`flash update`) + OS-aware GUI rebuild
// (`flash desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const flash = resolveFlashCliBinary(updateRoot)

  if (!flash) {
    emitUpdateProgress({ stage: 'manual', message: 'flash update', percent: null })

    return { ok: true, manual: true, command: 'flash update', flashRoot: updateRoot }
  }

  // Put the Flash-managed Node and the venv on PATH so `flash desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\flash\node, not node\bin.
  const env: Record<string, string> = {
    HERMES_HOME,
    PATH: pathWithFlashManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `flash update` reaps stale `flash serve` backends (a code update
  ...
  if (flashProcess && Number.isInteger(flashProcess.pid)) {
    desktopChildPids.push(flashProcess.pid)
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Flash (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(flash, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'flash update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'flash update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(flash, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Flash to retry.',
  ...
  // Linux in-app update terminal state (#45205). `flash desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Flash…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `flash-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Flash to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Flash desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Flash to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Flash.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Flash.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Flash to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.flash-update-new"; then
    rm -rf "$DST.flash-update-old" 2>/dev/null || true
    mv "$DST" "$DST.flash-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.flash-update-new" "$DST"
    rm -rf "$DST.flash-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Flash to load the new version.',
      percent: 100
    })
...
function isActiveRuntimeUsable() {
  ...
  return (
    isFlashSourceRoot(ACTIVE_HERMES_ROOT) &&
    fileExists(venvPython) &&
    canImportFlashCli(venvPython, {
      env: {
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `flash
...
function resolveRendererIndex() {
  ...
  rememberLog(
    ...
    `Rebuild with: flash desktop --force-build`
  )
...
function resolveFlashCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Flash.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveFlashCwd(), sanitized: Boolean(trimmed) }
  }
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Flash at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'flash_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      flashHome: HERMES_HOME,
...
function resolveFlashBackend(backendArgs) {
  // 1. Explicit override -- HERMES_DESKTOP_HERMES_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.HERMES_DESKTOP_HERMES_ROOT && path.resolve(process.env.HERMES_DESKTOP_HERMES_ROOT)

  if (overrideRoot && isFlashSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Flash source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `flash` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isFlashSourceRoot.)
  if (!IS_PACKAGED && isFlashSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Flash source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  //    %LOCALAPPDATA%\flash\flash-agent (Windows) or ~/.flash/flash-agent.
  ...
  //    to spawning flash. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `flash update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `flash` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    HERMES_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    let flashCommand = null
    const flashOverride = process.env.HERMES_DESKTOP_HERMES

    if (flashOverride) {
      const resolvedOverride = findOnPath(flashOverride)

      if (resolvedOverride) {
        flashCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(flashOverride, { isWsl: IS_WSL })) {
        flashCommand = flashOverride
      } else {
        rememberLog(`Ignoring Windows Flash override under WSL: ${flashOverride}`)
      }
    } else {
      flashCommand = findOnPath('flash')
    }

    if (flashCommand) {
      if (looksLikeDesktopAppBinary(flashCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Flash CLI: ${flashCommand}`)
        flashCommand = null
      }
    }

    if (flashCommand) {
      const unwrapped = unwrapWindowsVenvFlashCommand(flashCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `flash` shim
      ...
      const shellForProbe = isCommandScript(flashCommand)

      if (verifyFlashCli(flashCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvFlashCommand(flashCommand, backendArgs) || {
            label: `existing Flash CLI at ${flashCommand}`,
      ...
      rememberLog(
        `Ignoring existing Flash CLI at ${flashCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed flash_cli module via system Python.
  ...
  if (python) {
    ...
    // having flash_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\flash\flash-agent\venv.
    if (canImportFlashCli(python)) {
      return {
        kind: 'python',
        label: `installed flash_cli module via ${python}`,
        command: python,
        args: ['-m', 'flash_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: flash_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveFlashBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Flash Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveFlashBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Flash install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Flash recovery was handed off to Flash Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Flash install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Flash bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
      ...
      // Latch the failure so subsequent startFlash() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // flash:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveFlashBackend(backend.args))
  }
  ...
  if (!isFlashSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Flash install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
      'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Flash' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\flash\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `flash` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Flash on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isFlashSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Flash venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Flash at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Flash runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Flash backend URL protocol: ${parsed.protocol}`))
          ...
          if (looksHtml || contentType.includes('text/html')) {
            reject(
              new Error(
                ...
                'The endpoint is likely missing on the Flash backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Flash backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Flash-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('flash:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveFlashCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('flash:preview-file-changed', payload)
}
...
async function waitForFlash(baseUrl, token) {
  ...
  throw new Error(`Flash backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('flash:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('flash:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Flash...".
function sendPowerResume() {
  ...
  webContents.send('flash:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('flash:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('flash:window-state-changed', state)
}
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents.send('flash:zoom:changed', { level: next, percent: zoomLevelToPercent(next) })
  window.webContents
    .executeJavaScript(
...
const OAUTH_SESSION_PARTITION = 'persist:flash-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Flash Cloud agent…' : 'Sign in to Flash gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Flash backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// hasLiveOauthSession, which looks for flash_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Flash Cloud sign-in.'))
...
// Discover the hosted (Flash Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Flash Cloud. Open Settings → Gateway, choose Flash Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Flash Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Flash Cloud session has expired. Sign in to Flash Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Flash Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  // saved mode, preserving 'cloud' (a Flash Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Flash Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Flash Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Flash Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (flash #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Flash gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Flash gateway is selected, but no session token is saved. ' +
  ...
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        ...
        'Both must be provided to connect to a remote Flash backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Flash
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startFlash())
  ...
  // connect to Flash gateway". Mirror the renderer's connect here so the test
...
function resetFlashConnection({ soft = false } = {}) {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(flashProcess)
...
// startFlash() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetFlashConnection() nulls flashProcess.
  const dying = flashProcess && !flashProcess.killed ? flashProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('flash:connection:applied')
}
...
// profile to startFlash() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startFlash() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForFlash(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: flashLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // step 3 in flash_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveFlashBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const flashCwd = resolveFlashCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Flash backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Flash backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Flash backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Flash backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForFlash(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Flash backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_flash_home() recreates the deleted profile directory.
...
async function startFlash() {
  ...
  // process, every subsequent startFlash() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Flash backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Flash backend at ${remote.baseUrl}`, 24)
      await waitForFlash(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Flash backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.flash/active_profile file) and
    // resolves HERMES_HOME the same way `flash -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Flash runtime', 28)
    const backend = await ensureRuntime(resolveFlashBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const flashCwd = resolveFlashCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Flash backend via ${backend.label}`, 84)
    rememberLog(`Starting Flash backend via ${backend.label}`)
    ...
    flashProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin HERMES_HOME for the child so Python's get_flash_home()
          // resolves to the SAME location our resolveFlashHome() picked. Without
          // this pin, Python falls back to ~/.flash on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\flash, which differs from C:\Users\<u>\.flash.
    ...
    flashProcess.stdout.on('data', rememberLog)
    flashProcess.stderr.on('data', rememberLog)
    ...
    flashProcess.once('error', error => {
      rememberLog(`Flash backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Flash backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    flashProcess.once('exit', (code, signal) => {
      rememberLog(`Flash backend exited (${signal || code})`)
      flashProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Flash backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Flash backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentFlashLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Flash backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(flashProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Flash backend to become ready', 90)
    await Promise.race([waitForFlash(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null flashProcess when the child dies.
      childAlive: () => flashProcess !== null && flashProcess.exitCode === null && !flashProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Flash backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Flash is
...
// pushes pet state over IPC (flash:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via flash:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Flash app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Flash icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see flash:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Flash in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('flash:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startFlash().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('flash:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Flash…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('flash:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetFlashConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Flash backend failed liveness probe; dropping stale connection.')
    resetFlashConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('flash:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('flash:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('flash:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('flash:window:openNewSession', async () => {
...
ipcMain.handle('flash:zoom:get', event => {
...
ipcMain.on('flash:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('flash:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('flash:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('flash:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('flash:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('flash:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('flash:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('flash:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('flash:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('flash:pet-overlay:control', payload)
})
ipcMain.handle('flash:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startFlash() call restarts the
...
ipcMain.handle('flash:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startFlash() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('flash:bootstrap:cancel', async () => {
...
ipcMain.handle('flash:boot-progress:get', async () => bootProgressState)
ipcMain.handle('flash:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('flash:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('flash:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('flash:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('flash:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('flash:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Flash Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('flash:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('flash:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('flash:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on flash:connection:applied.
...
ipcMain.handle('flash:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('flash:profile:set', async (_event, name) => {
...
ipcMain.on('flash:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('flash:requestMicrophoneAccess', async () => {
...
ipcMain.handle('flash:api', async (_event, request) => {
  ...
  // backend calls ensure_flash_home() which recreates the profile directory,
...
ipcMain.handle('flash:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('flash:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('flash:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('flash:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('flash:readFileText', async (_event, filePath) => {
...
ipcMain.handle('flash:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('flash:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('flash:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('flash:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('flash:saveClipboardImage', async () => {
...
ipcMain.handle('flash:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('flash:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('flash:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('flash:titlebar-theme', (_event, payload) => {
...
ipcMain.on('flash:native-theme', (_event, mode) => {
...
ipcMain.on('flash:translucency', (_event, payload) => {
...
ipcMain.handle('flash:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('flash:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveFlashCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('flash:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveFlashCwd()
}))

ipcMain.handle('flash:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('flash:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('flash:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('flash:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('flash:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('flash:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: flashLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Flash' TUI into
  ...
  env.TERM_PROGRAM = 'Flash'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a flash/--tui launched in this pane know it's embedded in the desktop
...
function terminalChannel(id, suffix) {
  return `flash:terminal:${id}:${suffix}`
}
...
ipcMain.handle('flash:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('flash:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('flash:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('flash:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('flash:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('flash:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('flash:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('flash:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('flash:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('flash:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('flash:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('flash:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('flash:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('flash:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('flash:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('flash:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('flash:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('flash:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('flash:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('flash:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('flash:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('flash:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('flash:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('flash:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('flash:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('flash:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('flash:terminal:write', (_event, id, data) => {
...
ipcMain.handle('flash:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('flash:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('flash:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('flash:updates:check', async () =>
...
ipcMain.handle('flash:updates:apply', async (_event, payload) =>
...
ipcMain.handle('flash:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('flash:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Flash version (the one `release.py` bumps in
// flash_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Flash version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveFlashVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'flash_cli', '__init__.py')
...
// Re-resolve the live Flash version and push it into the native About panel
// just before showing it, so an in-place `flash update` is reflected without
...
ipcMain.handle('flash:version', async () => ({
  appVersion: resolveFlashVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  flashRoot: resolveUpdateRoot()
}))
...
// the actual removal via `flash uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (flash_cli/uninstall.py + flash_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const fallback = () => ({
    flash_home: HERMES_HOME,
    agent_installed: isFlashSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'flash_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Flash agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import flash_cli` resolves from
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `flash-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `flash-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('flash:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('flash:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('flash:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('flash:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// flash:// deep links (e.g. flash://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const HERMES_PROTOCOL = 'flash'
...
function handleDeepLink(url) {
  ...
  // flash://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('flash:deep-link', payload)
...
ipcMain.handle('flash:deep-link-ready', () => {
...
// second-instance argv. Without the lock a second `flash://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching flash:// URL is in our own argv.
