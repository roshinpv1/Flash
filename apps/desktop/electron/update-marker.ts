...
export function markerPath(flashHome) {
  return path.join(flashHome, '.flash-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(flashHome)
  let raw
 ...
 *
 * The Tauri-based flash-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(flashHome, pid, { now = Date.now } = {}) {
  const file = markerPath(flashHome)
