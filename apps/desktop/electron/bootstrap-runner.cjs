 ...
 *
 * Drives apps/desktop's first-launch install of Nyxo Agent by spawning
...
function bootstrapCacheDir(nyxoHome) {
  return path.join(nyxoHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.nyxo/nyxo-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(nyxoHome) {
  if (!nyxoHome) return null
  const candidate = path.join(nyxoHome, 'nyxo-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(nyxoHome, commit) {
  return path.join(bootstrapCacheDir(nyxoHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/nyxo-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, nyxoHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(nyxoHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(nyxoHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, nyxoHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass NYXO_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        NYXO_HOME: nyxoHome || process.env.NYXO_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, nyxoHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, nyxoHome }) {
  const args = ['--dir', activeRoot, '--nyxo-home', nyxoHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, nyxoHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, nyxoHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, nyxoHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, nyxoHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(nyxoHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, nyxoHome, emit })
