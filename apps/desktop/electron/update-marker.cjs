...
function markerPath(nyxoHome) {
  return path.join(nyxoHome, '.nyxo-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(nyxoHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(nyxoHome)
  let raw
  try {
