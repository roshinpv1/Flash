...
const STORAGE_KEY = 'flash:native-notifications'
...
// "Backgrounded" = the user isn't on Flash. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.flashDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.flashDesktop
