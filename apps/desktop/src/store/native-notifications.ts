...
const STORAGE_KEY = 'nyxo:native-notifications'
...
// "Backgrounded" = the user isn't on Nyxo. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.nyxoDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.nyxoDesktop
