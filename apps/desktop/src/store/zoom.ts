...
export function setZoomPercent(percent: number): void {
  window.flashDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.flashDesktop?.zoom) {
  void window.flashDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.flashDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
