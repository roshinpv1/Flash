...
import type { ProjectInfo, ProjectsPayload } from '@/types/flash'
...
const PROJECT_SCOPE_KEY = 'flash.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Flash gateway is not connected')
  }
...
// repo, returning where Flash should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<FlashGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.flashDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.flashDesktop?.writeClipboard?.(path)
  }
}
