...
import type { FlashReviewFile, FlashReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Flash' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'flash.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'flash.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'flash.desktop.reviewTreeMode'
const SELECTED_KEY = 'flash.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<FlashReviewFile[]>([])
...
export const $reviewShipInfo = atom<FlashReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['flashDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: FlashReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.flashDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.flashDesktop?.openExternal?.(url)
    }
