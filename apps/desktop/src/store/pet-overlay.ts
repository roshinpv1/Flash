 ...
 * app's bounds and stays visible while Flash is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'flash.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'flash.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.flashDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.flashDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.flashDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.flashDesktop?.petOverlay?.close()
}
