...
describe('Flash REST parity helpers (hub / mcp / maintenance)', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'flashDesktop')
  })
  ...
  it('runs debug share synchroflashly with an upload-tolerant timeout', async () => {
    await runDebugShare()
