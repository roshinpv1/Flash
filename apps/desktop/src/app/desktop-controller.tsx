...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../flash'
...
import { useFlashConfig } from './session/hooks/use-flash-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.flashDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.flashDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.flashDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.flashDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // flash:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.flashDesktop?.onDeepLink?.(payload => {
    ...
    void window.flashDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.flashDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshFlashConfig, sttEnabled, voiceMaxRecordingSeconds } = useFlashConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshFlashConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshFlashConfig])
