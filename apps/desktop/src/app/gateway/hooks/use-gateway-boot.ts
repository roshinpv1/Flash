...
interface GatewayBootOptions {
  ...
  onGatewayReady: (gateway: FlashGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: FlashConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Flash...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Flash…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Flash gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshFlashConfig().catch(() => undefined)
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startFlash re-emits progress — ignore so the
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshFlashConfig()
