...
// bug that drives the REAL useGatewayBoot hook + REAL FlashGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshFlashConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForFlash) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startFlash()'s remote branch awaits waitForFlash() for 45s before it
    ...
    // After ~45s waitForFlash gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Flash backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
