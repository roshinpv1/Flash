...
import { getSessionMessages, listAllProfileSessions } from '@/nyxo'
...
import type { SessionInfo, SessionMessage } from '@/types/nyxo'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.nyxoDesktop?.openExternal) {
        await window.nyxoDesktop.openExternal(href)
      } else {
