...
const HERMES_PATHS_MIME = 'application/x-flash-paths'
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.flashDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (flash --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[flash-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.flashDesktop?.terminal?.write(sessionId, `${command}\r`)
