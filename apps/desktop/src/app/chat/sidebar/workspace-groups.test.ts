...
import type { NyxoWorktreeInfo } from '@/global'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/nyxo-agent/apps/desktop'), makeSession('/a/nyxo-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/nyxo-agent/apps/desktop'), makeSession('/a/nyxo-agent-wt-rtl/apps/desktop')])
    ).toEqual(['nyxo-agent/apps/desktop', 'nyxo-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/nyxo-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<NyxoWorktreeInfo> & Pick<NyxoWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): NyxoWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/nyxo-agent'), makeSession('/www/nyxo-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('nyxo-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['nyxo-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/nyxo-agent') {
        return info({ repoRoot: '/www/nyxo-agent', worktreeRoot: '/www/nyxo-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/nyxo-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/nyxo-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['nyxo-agent', 'rtl'])
  })
