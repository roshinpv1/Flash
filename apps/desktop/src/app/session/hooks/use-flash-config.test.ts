...
import { getFlashConfig } from '@/flash'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useFlashConfig } from './use-flash-config'

vi.mock('@/flash', () => ({
  getFlashConfig: vi.fn(),
  getFlashConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'flash.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getFlashConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getFlashConfig>>)

describe('useFlashConfig refreshFlashConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshFlashConfig()
    })
