...
import { getFlashConfig, getFlashConfigDefaults } from '@/flash'
...
interface FlashConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useFlashConfig({ activeSessionIdRef, refreshProjectBranch }: FlashConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshFlashConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getFlashConfig(), getFlashConfigDefaults().catch(() => ({}))])
  ...
  return { refreshFlashConfig, sttEnabled, voiceMaxRecordingSeconds }
}
