...
import { getProfiles, transcribeAudio } from '@/nyxo'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.nyxoDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `nyxo gateway` process performs the actual transfer, so we
