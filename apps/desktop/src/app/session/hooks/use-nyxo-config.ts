...
import { getNyxoConfig, getNyxoConfigDefaults } from '@/nyxo'
...
interface NyxoConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useNyxoConfig({ activeSessionIdRef, refreshProjectBranch }: NyxoConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshNyxoConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getNyxoConfig(), getNyxoConfigDefaults().catch(() => ({}))])
  ...
  return { refreshNyxoConfig, sttEnabled, voiceMaxRecordingSeconds }
}
