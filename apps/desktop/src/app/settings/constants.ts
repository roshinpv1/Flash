...
export const EMPTY_SELECT_VALUE = '__flash_empty__'
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // flash_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Flash needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Flash stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Flash can read from one file request.',
  approvals: {
    mode: 'How Flash handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Flash updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
