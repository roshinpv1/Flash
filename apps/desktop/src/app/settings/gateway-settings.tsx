...
// Flash Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.flashDesktop.applyConnectionConfig(payload())
        : await window.flashDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.flashDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.flashDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.flashDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.flashDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.flashDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.flashDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.flashDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Flash Cloud panel: one portal sign-in, then a discovered-agent picker
                ...
                {cloudDiscover === 'loading' ? (
                  ...
                  <div className="flex items-start gap-2 py-3 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    <span>
                      ...
                      <ExternalLink href="https://portal.flashorg.com/agents" showExternalIcon={false}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/flash"
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.flashDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
