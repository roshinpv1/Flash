...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Flash doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in flash_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `flash model` uses — instead of
...
export type FlashConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `flash model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Flashonly: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Flashonly: paid models a free-tier user cannot select (shown disabled). */
...
// folders. Mirrors flash_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Flash, 'hub' = installed. */
 ...
 *  not Flash); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/flash/update/check` — the backend's own update state.
...
/** One Flash-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the FlashTool Gateway as a result of
   *  switching the main provider to Flash. Empty unless provider === 'flash'
