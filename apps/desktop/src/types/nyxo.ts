...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Nyxo doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in nyxo_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `nyxo model` uses — instead of
...
export type NyxoConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `nyxo model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `nyxo tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Nyxo); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/nyxo/update/check` — the backend's own update state.
