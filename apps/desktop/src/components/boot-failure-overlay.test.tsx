...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Flash gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.flashDesktop
  Object.defineProperty(window, 'flashDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'flashDesktop', { configurable: true, value: original })
}
