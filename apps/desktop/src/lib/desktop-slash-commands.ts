...
/** Why a known Nyxo command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Nyxo profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Nyxo Agent version', surface: exec() },
...
function isKnownNyxoSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Nyxo' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownNyxoSlashCommand(normalized)
}
