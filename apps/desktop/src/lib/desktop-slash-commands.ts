...
/** Why a known Flash command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Flash profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Flash Agent version', surface: exec() },
...
function isKnownFlashSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Flash' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownFlashSlashCommand(normalized)
}
