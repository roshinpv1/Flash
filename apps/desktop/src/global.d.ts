...
declare global {
  interface Window {
    flashDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<FlashConnection>
      ...
      // Flash Cloud: one portal login powers discovery + silent per-agent
      ...
      api: <T>(request: FlashApiRequest) => Promise<T>
      notify: (payload: FlashNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<FlashReadFileTextResult>
      selectPaths: (options?: FlashSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<FlashPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<FlashPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: FlashTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<FlashReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<FlashGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<FlashGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<FlashRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: FlashReviewScope, baseRef?: null | string) => Promise<FlashReviewList>
          diff: (
            repoPath: string,
            filePath: string,
          ...
          shipInfo: (repoPath: string) => Promise<FlashReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: FlashTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<FlashTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: FlashWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: FlashPreviewFileChanged) => void) => () => void
...
export interface FlashTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface FlashTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `flash update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  flashRoot?: string
...
export interface FlashTitleBarTheme {
  background: string
  foreground: string
}

export interface FlashWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Flash Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Flash Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Flash Cloud org (slug or id) to
...
// A discovered Flash Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface FlashApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface FlashPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface FlashReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface FlashPreviewWatch {
  id: string
  path: string
}
...
export interface FlashGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface FlashGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface FlashRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface FlashRepoStatus {
  branch: null | string
...
// turn began.
export type FlashReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface FlashReviewFile {
  path: string
  added: number
  removed: number
...
export interface FlashReviewList {
  files: FlashReviewFile[]
...
export interface FlashReviewPr {
  url: string
  state: string
  number: number
}
...
export interface FlashReviewShipInfo {
  ghReady: boolean
  pr: FlashReviewPr | null
}

export interface FlashReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface FlashReadDirResult {
  entries: FlashReadDirEntry[]
  error?: string
}

export interface FlashPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface FlashSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
