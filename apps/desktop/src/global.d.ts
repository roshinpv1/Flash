...
declare global {
  interface Window {
    nyxoDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<NyxoConnection>
      ...
      profile: {
        ...
        // backend under the new NYXO_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: NyxoApiRequest) => Promise<T>
      notify: (payload: NyxoNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<NyxoReadFileTextResult>
      selectPaths: (options?: NyxoSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<NyxoPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<NyxoPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: NyxoTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<NyxoReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, NyxoWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: NyxoTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<NyxoTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: NyxoWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: NyxoPreviewFileChanged) => void) => () => void
...
export interface NyxoTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface NyxoTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `nyxo update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  nyxoRoot?: string
...
export interface NyxoConnection {
  baseUrl: string
...
export interface NyxoTitleBarTheme {
  background: string
  foreground: string
}

export interface NyxoWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface NyxoApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface NyxoNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface NyxoPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface NyxoReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface NyxoPreviewWatch {
  id: string
  path: string
}

export interface NyxoWorktreeInfo {
...
export interface NyxoReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface NyxoReadDirResult {
  entries: NyxoReadDirEntry[]
  error?: string
}

export interface NyxoPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface NyxoSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
