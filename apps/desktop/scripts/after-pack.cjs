 ...
 *
 * Stamps the Nyxo icon + identity onto the packed Windows Nyxo.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `nyxo desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Nyxo')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Nyxo'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Nyxo.exe keeps the stock Electron icon`)
  }
}
