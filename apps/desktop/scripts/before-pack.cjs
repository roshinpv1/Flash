 ...
 * product name (`Nyxo`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Nyxo'
 *
 * This is a hard failure with no obvious cause for the user — `nyxo desktop`
 ...
 * (the mac-unpacked Nyxo.app bundle) and Windows (win-unpacked), so we
