...
// no longer bundles the Flash Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Flash.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Flash'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Flash.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'flash'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// resolveFlashHome(). On Windows it's %LOCALAPPDATA%\flash; elsewhere
// it's ~/.flash. The fresh-install sandbox launchFresh() sets its own
// HERMES_HOME and never touches this.
const DEFAULT_HERMES_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'flash')
  }
  return path.join(os.homedir(), '.flash')
})()
const VENV_ROOT = path.join(DEFAULT_HERMES_HOME, 'flash-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'flash-desktop-fresh-install')
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Flash-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Flash-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Flash-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Flash-${version}-${os}-${arch}.${ext}'
...
function launchFresh() {
  ...
  const flashHome = path.join(sandbox, 'flash-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(flashHome, { recursive: true })
  ...
  console.log(`  HERMES_HOME: ${flashHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(flashHome, 'flash-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Flash Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of flash_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'flash-agent', 'flash_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Flash
