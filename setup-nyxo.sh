...
# 5. Symlinks the 'nyxo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Nyxo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/nyxo-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink nyxo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nyxo command..."

NYXO_BIN="$SCRIPT_DIR/venv/bin/nyxo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NYXO_BIN" "$COMMAND_LINK_DIR/nyxo"
echo -e "${GREEN}✓${NC} Symlinked nyxo → $COMMAND_LINK_DISPLAY_DIR/nyxo"
...
# ============================================================================
# Seed bundled skills into ~/.nyxo/skills/
# ============================================================================

NYXO_SKILLS_DIR="${NYXO_HOME:-$HOME/.nyxo}/skills"
mkdir -p "$NYXO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nyxo/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NYXO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  nyxo status        # Check configuration"
if is_termux; then
    echo "  nyxo gateway       # Run gateway in foreground"
else
    echo "  nyxo gateway install # Install gateway service (messaging + cron)"
fi
echo "  nyxo cron list     # View scheduled jobs"
echo "  nyxo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nyxo_cli.main setup
fi
