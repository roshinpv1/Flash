...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Flashaccess token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **FlashPortal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `flash-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
- **Auth:** `Authorization: Bearer <agent Flashaccess token>`. NAS validates via
...
**dashboard app** (`flash_cli/web_server.py`) — the agent's always-reachable
...
If `callback_url` / `portal_url` is blank or the agent has no Flashlogin,
