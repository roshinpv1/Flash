# Flash Observer Hooks

Flash observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Flash' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Flash catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Flash only builds
