...
The canonical transcript store is SQLite via `SessionDB` (from `nyxo_state`). The
...
Counts consecutive restarts via a JSON file (`{NYXO_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `nyxo update`, `nyxo gateway restart`,
or `/restart`.
