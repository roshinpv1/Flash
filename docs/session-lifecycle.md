...
The canonical transcript store is SQLite via `SessionDB` (from `flash_state`). The
...
This prevents unwanted auto-resets after `flash update`, `flash gateway restart`,
or `/restart`.
