...
Nyxo middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Nyxo'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Nyxo calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Nyxo retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Nyxo guardrails,
...
- `telemetry_schema_version`: currently `nyxo.observer.v1`
- `middleware_schema_version`: currently `nyxo.middleware.v1`
...
Nyxo stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Nyxo runs
them as a nested chain in registration order. Middleware failures are fail-open:
Nyxo logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Nyxo applies middleware in this order:
...
For each tool call, Nyxo applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Nyxo pre-execution path against the effective arguments:
...
```bash
nyxo plugins enable <plugin-name>
```

For isolated local testing, use one `NYXO_HOME` for plugin enablement and the
agent run:

```bash
export NYXO_HOME=/tmp/nyxo-middleware-test
mkdir -p "$NYXO_HOME"
nyxo plugins enable <plugin-name>
nyxo chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run nyxo plugins enable <plugin-name>
uv run nyxo chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["nyxo_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/nyxo-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Nyxo expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Nyxo treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Nyxo preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Nyxo does not convert downstream
