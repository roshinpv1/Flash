...
Supersedes: PR #31781 (prompt_toolkit `flash profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `flash profile wizard` in the terminal.
...
A profile = a full `~/.flash/profiles/<name>/` directory with its own:
...
import time**. The context-local `set_flash_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_flash_action`
runs `python -m flash_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_flash_action(["-p", profile, "skills", "install", identifier], "skills-install")
```
