...
When running Flash inside Docker, the default `network_mode: host` gives the
...
The Flash [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=flash,flash-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://flash-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Flash trust model and vulnerability reporting
