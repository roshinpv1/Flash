...
When running Nyxo inside Docker, the default `network_mode: host` gives the
...
The Nyxo [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   NYXO_UID=$(id -u) NYXO_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=nyxo,nyxo-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://nyxo-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Nyxo trust model and vulnerability reporting
