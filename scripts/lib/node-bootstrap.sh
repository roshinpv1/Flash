...
#   2. ~/.flash/node/ from a prior Flash-managed install
...
# Redirect a Flash-managed Node's `npm install -g` to the command link dir
...
# ---------------------------------------------------------------------------
# Heal a broken Flash-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
heal_managed_node() {
    ...
    _nb_log "Flash-managed Node is broken — redownloading to $HERMES_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        export PATH="$HERMES_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Flash-managed)"
            HERMES_NODE_AVAILABLE=true
            return 0
        fi
    fi
