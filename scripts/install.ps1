# ============================================================================
# Flash Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$FlashHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\flash" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\flash-agent" } else { "$env:LOCALAPPDATA\flash\flash-agent" }),
    ...
    # builds apps/desktop into a launchable Flash.exe.
    #
    # Why opt-in:
    #   * Flash-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Flash.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Flash.exe
    ...
    #     `flash desktop` already builds on demand.
...
# 5.1's progress UI repaints synchroflashly on every received byte, which
...
$RepoUrlSsh = "git@github.com:FlashOrg/flash-agent.git"
$RepoUrlHttps = "https://github.com/FlashOrg/flash-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Flash Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Flash Org.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $FlashHome)) {
        New-Item -ItemType Directory -Force -Path $FlashHome | Out-Null
    }
    $envFile = Join-Path $FlashHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $FlashHome "node"
...
function Install-Uv {
    # Flash owns its own uv at $FlashHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (flash_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `flash update` stay in sync.
    $managedUv = Join-Path $FlashHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $FlashHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $FlashHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $FlashHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $FlashHome "bin"
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Flash-Setup.exe
...
function Install-Git {
    ...
    which Flash uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\flash\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Flash needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Flash copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\flash\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope) so Flash can find it in a fresh
    ...
    # Download PortableGit into $FlashHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $FlashHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Flash features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Flash needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Flash can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $FlashHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $FlashHome\git\usr\bin\bash.exe
    $candidates += "$FlashHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$FlashHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Warn "Could not locate bash.exe -- Flash may not find Git Bash."
...
function Test-Node {
    ...
    # Prefer a Flash-managed Node from a previous run over a too-old system one.
    $managedNode = "$FlashHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$FlashHome\node;$env:Path"
        Write-Success "Node.js $version found (Flash-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Flash-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $FlashHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $FlashHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\flash-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$FlashHome\node") { Remove-Item -Recurse -Force "$FlashHome\node" }
                Move-Item $extractedDir.FullName "$FlashHome\node"
                ...
                $version = & "$FlashHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $FlashHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\flash-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `flash
                    # update` path (#4735).
                    ...
                    $stashName = "flash-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``flash update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Flash behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running flash processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/FlashOrg/flash-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/FlashOrg/flash-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/FlashOrg/flash-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\flash-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\flash-agent-extract"
    ...
    # next `flash update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Flash-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running flash process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running flash processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Flash_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (flash.exe) plus its child tree.
            & taskkill /F /T /IM flash.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM flash.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m flash_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not flash.exe.
            ...
            # user (see flash_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `flash update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `flash.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install flash-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and flash.exe broken with
    ...
    # here, not 30 seconds later when the user runs `flash`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register flash.exe in dist-info/RECORD but fail to
        ...
        # `flash` command while flash-agent.exe / flash-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m flash_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `flash dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\flash_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `flash dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `flash dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: flash_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up flash command..."
    
    if ($NoVenv) {
        $flashBin = "$InstallDir"
    } else {
        $flashBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so flash is globally available
    # On Windows, the flash.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$flashBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$flashBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $flashBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HERMES_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\flash instead
    # of the Unix default ~/.flash
    $currentFlashHome = [Environment]::GetEnvironmentVariable("HERMES_HOME", "User")
    if (-not $currentFlashHome -or $currentFlashHome -ne $FlashHome) {
        [Environment]::SetEnvironmentVariable("HERMES_HOME", $FlashHome, "User")
        Write-Success "Set HERMES_HOME=$FlashHome"
    }
    $env:HERMES_HOME = $FlashHome
    
    # Update current session
    $env:Path = "$flashBin;$env:Path"
    
    Write-Success "flash command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.flash-bootstrap-complete which tells the Flash
    ...
    $markerPath = Join-Path $InstallDir ".flash-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the HERMES_HOME directory structure ($FlashHome, default %LOCALAPPDATA%\flash)
    New-Item -ItemType Directory -Force -Path "$FlashHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$FlashHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$FlashHome\.env"
    ...
    # Create config.yaml
    $configPath = "$FlashHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Flash's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$FlashHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in flash_cli/default_soul.py. The runtime
        ...
        $soulContent = @"
You are Flash Agent, an intelligent AI assistant created by Flash Org. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $FlashHome"
    
    # Seed bundled skills into $FlashHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $FlashHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $FlashHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$FlashHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $FlashHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Flash-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'flash setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\flash-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\flash-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\flash-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Flash` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Flash.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_flash_desktop command
    # resolves apps/desktop/release/win-unpacked/Flash.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Flash-managed
    ...
    # The --dir mode produces an unpacked Flash.exe in
    ...
    # are NOT trying to work around). The Flash icon + product name are
    # stamped onto Flash.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\flash-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Flash.exe",
        "$desktopDir\release\win-arm64-unpacked\Flash.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Flash.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Flash icon + identity are stamped onto Flash.exe by the
    ...
    #    Flash.exe. We deliberately do NOT point them at `flash desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Flash.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Flash.lnk')
        )
        ...
        # with the Flash icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.flash/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'flash setup'."
        return
    }
    ...
    # Run flash setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m flash_cli.main setup
    } else {
        python -m flash_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $flashCmd = "$InstallDir\venv\Scripts\flash.exe"
    if (-not (Test-Path $flashCmd)) {
        $flashCmd = "flash"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$FlashHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'flash whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: flash gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$FlashHome\logs\gateway.log"
            Start-Process -FilePath $flashCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$FlashHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: flash gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: flash gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$FlashHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$FlashHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$FlashHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$FlashHome\flash-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   flash              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   flash setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   flash config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   flash config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   flash gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   flash update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Flash repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Flash-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Flash to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://flash-agent.flashorg.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
