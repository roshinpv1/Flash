#!/usr/bin/env python3
"""Live test harness for Nyxo Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any nyxo imports.
ORIGINAL_HOME = os.environ.get("NYXO_HOME")
ORIGINAL_AUTH = Path.home() / ".nyxo" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.nyxo/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.nyxo/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="nyxo_ts_live_"))
    nyxo_home = home_dir / ".nyxo"
    nyxo_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, nyxo_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".nyxo" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, nyxo_home / ".env")
        ...
        load_nyxo_dotenv(nyxo_home=str(Path.home() / ".nyxo"))
    ...
    (nyxo_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return nyxo_home
...
def reset_module_state():
    """Drop cached modules so the new NYXO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "nyxo_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original NYXO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["NYXO_HOME"] = ORIGINAL_HOME
    else:
