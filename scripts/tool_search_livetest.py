#!/usr/bin/env python3
"""Live test harness for Flash Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any flash imports.
ORIGINAL_HOME = os.environ.get("HERMES_HOME")
ORIGINAL_AUTH = Path.home() / ".flash" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.flash/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.flash/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="flash_ts_live_"))
    flash_home = home_dir / ".flash"
    flash_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, flash_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".flash" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, flash_home / ".env")
        ...
        load_flash_dotenv(flash_home=str(Path.home() / ".flash"))
    ...
    (flash_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return flash_home
