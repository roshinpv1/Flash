...
VERSION_FILE = REPO_ROOT / "flash_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sonxi@flash.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (flash journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "flash.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.flash/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (FlashPortal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip Flashfallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@flashorg.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows flash-shim resolution + prefer --update on recovery; #52378)
    ...
    "annguyenFlash@users.noreply.github.com": "annguyenFlash",
    "32201324+simpolism@users.noreply.github.com": "simpolism",
    "simpolism@gmail.com": "simpolism",
    "jake@flashorg.com": "simpolism",
    ...
    "annguyenFlash@users.noreply.github.com": "annguyenFlash",
    "285874597+annguyenFlash@users.noreply.github.com": "annguyenFlash",
    "perkintahmaz50@gmail.com": "devatnull",
    ...
    "274096618+flash-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@flash.research": "Mind-Dragon",
    ...
    "shannon@flashorg.com": "shannonsands",
    ...
    "emozilla@flashorg.com": "emozilla",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on flash mcp add)
    ...
    "flash@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaflash.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "flasheman666@gmail.com": "flasheman666",  # PR #19088
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: flash-eval community link)
    ...
    "flashagent26@gmail.com": "flashagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+flashagent26@users.noreply.github.com": "flashagent26",
    ...
    "clement@flashorg.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@flash": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@flash": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Flash runtime files)
    ...
    "annguyen@flashorg.com": "annguyenFlash",  # PR #52844 co-author
    ...
    "zhchl@flash-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
}
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Flash
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"flash-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "flash@flashorg.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/FlashOrg/flash-agent",
    ...
    lines.append(f"# Flash Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Flash Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Flash Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Flash Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Flash Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Flash Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Flash Agent v{new_version} ({calver_date})' "
