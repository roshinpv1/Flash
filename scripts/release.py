...
VERSION_FILE = REPO_ROOT / "nyxo_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+nyxo-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "nyxo@nousresearch.com": "NousResearch",
    ...
    "nyxo@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyanyxo.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (NYXO_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (NYXO_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: nyxo-eval community link)
    ...
    "nyxoagent26@gmail.com": "nyxoagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+nyxoagent26@users.noreply.github.com": "nyxoagent26",
    ...
    "vigo@nyxo": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Nyxo runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level NYXO_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Nyxo
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"nyxo-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "nyxo@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/nyxo-agent",
    ...
    lines.append(f"# Nyxo Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Nyxo Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Nyxo Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Nyxo Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Nyxo Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Nyxo Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Nyxo Agent v{new_version} ({calver_date})' "
