...
# .flash-sandbox/ in the worktree git root).
#
# Usage:
#   scripts/dev-sandbox.sh python -m flash_cli.main
#   scripts/dev-sandbox.sh flash desktop
...
#   HERMES_DEV_SANDBOX_DIR=.staging-sandbox scripts/dev-sandbox.sh --persistent flash desktop
...
Options:
  --persistent    Keep the sandbox dir across restarts (under the worktree
                  git root, in .flash-sandbox/). Without this flag the
                  sandbox is a temp dir that is removed on exit.
  --delete        Delete the existing persistent sandbox in .flash-sandbox.
...
Examples:
  dev-sandbox.sh flash desktop
  dev-sandbox.sh --persistent flash desktop
...
SANDBOX_DIR_NAME="${HERMES_DEV_SANDBOX_DIR:-.flash-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t flash-sandbox.XXXXXX)"
fi

export HERMES_HOME="$SANDBOX_ROOT/flash-home"
