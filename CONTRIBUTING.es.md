# Contribuir a Flash Agent

¡Gracias por contribuir a Flash Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Flash funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Flash. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
Si tu habilidad es especializada, contribuida por la comunidad o de nicho, es mejor para un **Skills Hub** — súbela a un registro de habilidades y compártela en el [Discord de Flash Org](https://discord.gg/FlashOrg). Los usuarios pueden instalarla con `flash skills install`.
...
├── cli.py                    # Clase FlashCLI — TUI interactiva, integración prompt_toolkit
...
│   ├── auth.py                   # Resolución de proveedor, OAuth, FlashPortal
...
| `~/.flash/auth.json` | Credenciales OAuth (FlashPortal) |
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Flash o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Flash usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.
...
Flash se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Flash tiene acceso al terminal. La seguridad importa.
...
- Incluye: SO, versión de Python, versión de Flash (`flash version`), traza de error completa
