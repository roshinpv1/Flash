...
``flash gateway restart`` (or ``launchctl kickstart ai.flash.gateway``
or ``systemctl restart flash-gateway``).  When the cron fires, the
...
``flash cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``flash gateway``, ``launchctl ... flash-gateway``,
``systemctl ... flash-gateway``, ``pkill`` against the gateway) so it
...
``flash gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `flash gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:flash\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a flash-gateway label. macOS launchd
    # labels look like `ai.flash.gateway` / `flash-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated flash services (e.g.
    # `launchctl unload ai.flash.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bflash[.\-]?gateway)"
    # Branch C: systemctl ops on a flash-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bflash[.\-]?gateway)"
    # Branch D: pkill / kill targeting the flash gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    (``~/.flash/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_flash_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `flash gateway restart` from a shell outside "
