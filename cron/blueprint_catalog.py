...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``flash://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``flash://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"flash://blueprint/{quote(blueprint.key)}{qs}"
