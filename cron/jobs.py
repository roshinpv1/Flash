...
Jobs are stored in ~/.flash/cron/jobs.json
Output is saved to ~/.flash/cron/output/{job_id}/{timestamp}.md
"""
...
# `coder` lives in `~/.flash/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_flash_home()` (the active profile home), NOT `get_default_flash_root()`
...
# `_get_flash_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
HERMES_DIR = get_flash_home().resolve()
...
# gateway process and the (separate) ``flash cron status`` process share it
...
# threshold in `flash cron status` (flash_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``flash`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Flash timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `flash_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                flash_tz = _flash_now().tzinfo
                dt = dt.replace(tzinfo=flash_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _flash_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Flash configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Flash timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _flash_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Flash/system local time
    ...
    if kind == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall flash-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `flash cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``flash cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `flash cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _flash_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_flash_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.flash/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `flash cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _flash_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _flash_now().isoformat(),
        },
    )
...
# execution. Unlike the quick-snapshot store (`flash_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``flash_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _flash_now().strftime("%Y-%m-%d_%H-%M-%S")
