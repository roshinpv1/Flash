...
Uses a file-based lock (~/.flash/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `flash update` reloads
# the module) fail with ModuleNotFoundError for flash_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from flash_constants import get_flash_home
from flash_cli._subprocess_compat import windows_hide_flags
from flash_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy flash_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from flash_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``flash tools`` config for the ``cron`` platform.
    ...
    try:
        from flash_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``flash update`` / ``flash gateway stop`` / systemd restart, or an
...
_flash_home: Path | None = None


def _get_flash_home() -> Path:
    """Resolve Flash home dynamically while preserving test monkeypatch hooks.
    ...
    """
    return _flash_home or get_flash_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    flash_home = _get_flash_home()
    lock_dir = flash_home / "cron"
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Flash-managed secrets are not inherited
    ...
    """
    scripts_dir = _get_flash_home() / "scripts"
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _flash_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_flash_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_flash_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        # load_flash_dotenv (not a bare load_dotenv) and reset the secret-
        ...
        # forced re-pull off the network. load_flash_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from flash_cli.env_loader import (
            load_flash_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_flash_dotenv(flash_home=_get_flash_home())
        ...
        try:
            import yaml
            _cfg_path = str(_get_flash_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                "default with `flash model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from flash_constants import apply_ipv4_preference
...
**Job ID:** {job_id}
**Run Time:** {_flash_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_flash_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_flash_home())
        )
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _flash_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _flash_now().strftime('%H:%M:%S'), len(due_jobs))
