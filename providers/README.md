...
Registry and ABC for every inference provider Flash knows about.
...
- `flash_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `flash_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `flash_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `flash_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `flash_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
