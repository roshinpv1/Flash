"""ACP agent server — exposes Flash Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_flash_interactive_context,
    set_flash_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from flash_cli import __version__ as HERMES_VERSION
except Exception:
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Flash running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Flash cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Flash/OpenAI-compatible user content payload."""
...
class FlashACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Flash AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Flash version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Flash maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from flash_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from flash_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Flash estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_flash_session_id: str,
        previous_flash_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.flash.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_flash_session_id,
                previous_flash_session_id=previous_flash_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_flash_session_id: Optional[str] = None,
        previous_flash_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Flash changes it.

        When the internal Flash head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_flash_session_id`` so the
        attached ``_meta.flash.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # flash_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_flash_session_id or session_id,
            previous_flash_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["flash-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="flash-agent", version=HERMES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Flash' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Flash setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Flash
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Flash on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Flash' local kawaii waiting/status
        ...
        # tools.approval (set_flash_interactive_context) rather than
        ...
        def _run_agent() -> dict:
            ...
            interactive_token = set_flash_interactive_context(True)
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_flash_interactive_context(interactive_token)
        ...
        try:
            # Snapshot the internal Flash DB session id before the turn so we
            ...
            pre_turn_flash_id = getattr(state.agent, "session_id", None)
        ...
        # _meta.flash.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_flash_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_flash_id
            and pre_turn_flash_id
            and post_turn_flash_id != pre_turn_flash_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_flash_session_id=post_turn_flash_id,
                    previous_flash_session_id=pre_turn_flash_id,
                )
            except Exception:
                logger.debug(
        ...
        # Flash' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Flash Agent v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Flash has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
