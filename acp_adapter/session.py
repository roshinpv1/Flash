"""ACP session manager — maps ACP sessions to Flash AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.flash/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Flash itself is running in WSL.

    Windows ACP clients can launch ``flash acp`` inside WSL while still sending
    ...
    """
    from flash_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Flash from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["flash-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Flash agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Flash AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Flash runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.flash/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        later (e.g. by the test fixture ``_isolate_flash_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=flash_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Flash daemon's process cwd.
