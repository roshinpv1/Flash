"""ACP auth helpers — detect and advertise Flash authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Flash runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Flash can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Flash.
    ...
    not have Flash provider credentials configured yet, so Flash always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Flash using the currently configured "
