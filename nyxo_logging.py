"""Centralized logging setup for Nyxo Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.nyxo/logs/`` (profile-aware via ``get_nyxo_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``nyxo_cli.web_server``,
    ``nyxo_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Nyxo (TUI, gateway, ``hy_memory`` server, MCP
...
from nyxo_constants import get_config_path, get_nyxo_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nyxo_session_injector", False):
    ...
    _session_record_factory._nyxo_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``nyxo logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``nyxo logs --component gateway``.
    "gateway": ("gateway", "nyxo_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Nyxo logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nyxo_home
        Override for the Nyxo home directory.  Falls back to
        ``get_nyxo_home()`` (profile-aware).
    log_level
    ...
    home = nyxo_home or get_nyxo_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_nyxo_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._nyxo_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nyxo group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from nyxo_cli.config import is_managed
