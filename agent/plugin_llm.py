...
Plugins built on Nyxo Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Nyxo
...
:class:`~nyxo_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Nyxo' main config
...
handles every provider, fallback chain, and per-task override Nyxo
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`nyxo_cli.plugins.PluginContext`
