...
Plugins built on Flash Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Flash
...
:class:`~flash_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Flash' main config
...
handles every provider, fallback chain, and per-task override Flash
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`flash_cli.plugins.PluginContext`
