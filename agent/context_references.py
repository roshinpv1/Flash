...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    flash_home = get_flash_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(flash_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(flash_home / rel for rel in _SENSITIVE_HERMES_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Flash path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.flash/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Flash path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
