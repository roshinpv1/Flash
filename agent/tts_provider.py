...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Flash with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.flash/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Flash
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``flash tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``flash setup`` for
