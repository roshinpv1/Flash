...
def _flash_home_path() -> Path:
    """Resolve the active HERMES_HOME (profile-aware) without circular imports."""
    try:
        from flash_constants import get_flash_home  # local import to avoid cycles
        return get_flash_home()
    except Exception:
        return Path(os.path.expanduser("~/.flash"))


def _flash_root_path() -> Path:
    """Resolve the Flash root dir (always the parent of any profile, never per-profile)."""
    try:
        from flash_constants import get_default_flash_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    flash_home = _flash_home_path()
    flash_root = _flash_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(flash_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(flash_root / ".anthropic_oauth.json"),
...
def is_write_denied(path: str) -> bool:
    ...
    flash_dirs = []
    for base in (_flash_home_path(), _flash_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in flash_dirs:
                flash_dirs.append(real)
        except Exception:
            continue

    for base_real in flash_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Flash path.

    Three categories are blocked:

      * Internal Flash cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Flash
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.flash/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Flash root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in flash_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Flash cache file "
    ...
    for hd in flash_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Flash credential store "
    ...
    for hd in flash_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Flash MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Flash MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Flash read (see
...
#
# Flash profiles are separate HERMES_HOME dirs under
...
#
# Reference: May 2026 incident where a flash-security profile session
# edited skills under both ``~/.flash/profiles/flash-security/skills/``
# AND ``~/.flash/skills/`` (the default profile's skills) without realizing
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _flash_home_path().resolve()
        root_real = _flash_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Flash scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Flash entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Flash profile {info['target_profile']!r}, but the "
...
#
#   <HERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.flash/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.flash/…`` segment and warns
# regardless of which Flash profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.flash/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.flash`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.flash/…`` and returns the
    index where the inner Flash-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .flash / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".flash":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Flash state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.flash``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.flash`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Flash resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Flash process never "
...
# full ``…/sandboxes/<backend>/<task>/home/.flash/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.flash/…``.  The root:root
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Flash state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Flash "
