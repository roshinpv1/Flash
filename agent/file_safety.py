...
def _nyxo_home_path() -> Path:
    """Resolve the active NYXO_HOME (profile-aware) without circular imports."""
    try:
        from nyxo_constants import get_nyxo_home  # local import to avoid cycles
        return get_nyxo_home()
    except Exception:
        return Path(os.path.expanduser("~/.nyxo"))


def _nyxo_root_path() -> Path:
    """Resolve the Nyxo root dir (always the parent of any profile, never per-profile)."""
    try:
        from nyxo_constants import get_default_nyxo_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    nyxo_home = _nyxo_home_path()
    nyxo_root = _nyxo_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(nyxo_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(nyxo_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved NYXO_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("NYXO_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    nyxo_dirs = []
    for base in (_nyxo_home_path(), _nyxo_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in nyxo_dirs:
                nyxo_dirs.append(real)
        except Exception:
            continue

    for base_real in nyxo_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Nyxo path.

    Three categories are blocked:

      * Internal Nyxo cache files under ``NYXO_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under NYXO_HOME and the global Nyxo
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.nyxo/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active NYXO_HOME (profile-aware) AND the global
    # Nyxo root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (NYXO_HOME points at
    ...
    for hd in nyxo_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Nyxo cache file "
    ...
    for hd in nyxo_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Nyxo credential store "
    ...
    for hd in nyxo_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Nyxo MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Nyxo MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Nyxo profiles are separate NYXO_HOME dirs under
...
#
# Reference: May 2026 incident where a nyxo-security profile session
# edited skills under both ``~/.nyxo/profiles/nyxo-security/skills/``
# AND ``~/.nyxo/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under NYXO_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from NYXO_HOME.
    ...
    """
    try:
        home_real = _nyxo_home_path().resolve()
        root_real = _nyxo_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Nyxo scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Nyxo entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Nyxo profile {info['target_profile']!r}, but the "
...
#
#   <NYXO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.nyxo/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.nyxo/…`` segment and warns
# regardless of which Nyxo profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.nyxo/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.nyxo`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.nyxo/…`` and returns the
    index where the inner Nyxo-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .nyxo / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".nyxo":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nyxo state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.nyxo``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.nyxo`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Nyxo resolver to
    succeed, so it works correctly even when called from contexts where
    NYXO_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Nyxo process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real NYXO_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.nyxo/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.nyxo/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host NYXO_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Nyxo state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Nyxo "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real NYXO_HOME. Use the "
