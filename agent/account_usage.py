...
def build_flash_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from flash_cli.flash_account import flash_portal_topup_url
        ...
        details.append(f"Top up: {flash_portal_topup_url(account_info)}")
...
def flash_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    try:
        ...
        tok = (get_provider_auth_state("flash") or {}).get("access_token")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_flash_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_flash_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_flash_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /credits)") that build_flash_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=flash_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Flash's native OAuth setup now stores
