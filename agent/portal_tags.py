...
Every Nyxo request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Nyxo Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=nyxo-agent",
        "client=nyxo-client-v<__version__>",
    ]

The version is sourced live from ``nyxo_cli.__version__`` so it auto-aligns
...
``nyxo_cli.__version__`` is the canonical source of truth.
"""
...
def _nyxo_version() -> str:
    """Return the current Nyxo release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``nyxo_cli`` cannot be imported (should
...
def nyxo_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=nyxo-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=nyxo-client-v{_nyxo_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=nyxo-agent", nyxo_client_tag()]
