...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=flash-agent",
        "client=flash-client-v<__version__>",
    ]

The version is sourced live from ``flash_cli.__version__`` so it auto-aligns
...
``flash_cli.__version__`` is the canonical source of truth.
"""
...
def _flash_version() -> str:
    """Return the current Flash release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``flash_cli`` cannot be imported (should
