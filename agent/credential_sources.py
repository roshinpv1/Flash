"""Unified removal contract for every credential source Flash reads from.

Flash seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.flash/.env
    claude_code   — ~/.claude/.credentials.json
    flash_pkce   — ~/.flash/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (flash, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `flash auth add`
...
unify here is **removal**:
    ...
    ``flash auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, flash device_code (partial), flash_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"flash"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.flash/.env  → clear it
    ...
    """
    from flash_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Flash.",
            f"  The pool entry is now suppressed — Flash will ignore "
            f"{env_var} until you run `flash auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Flash stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `flash auth add anthropic` to re-enable if needed.",
    ])


def _remove_flash_pkce(provider: str, removed) -> RemovalResult:
    """~/.flash/.anthropic_oauth.json is ours — delete it outright."""
    from flash_constants import get_flash_home

    result = RemovalResult()
    oauth_file = get_flash_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Flash Anthropic OAuth credentials")
...
def _remove_flash_device_code(provider: str, removed) -> RemovalResult:
    ...
    them to go through `flash auth add flash` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``flash auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `flash model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Flash auth store is not enough — _seed_from_singletons() would
    ...
    via ``flash auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `flash auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `flash auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Flash stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `flash auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="flash_pkce",
        remove_fn=_remove_flash_pkce,
        description="~/.flash/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="flash", source_id="device_code",
        remove_fn=_remove_flash_device_code,
        description="auth.json providers.flash",
    ))
    register(RemovalStep(
