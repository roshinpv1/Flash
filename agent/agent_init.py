...
from flash_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: flash config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    """
    return get_flash_home() / ".codex_gpt55_autoraise_notice"
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .flash.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HERMES_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.flash/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from flash_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.flash/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from flash_logging import setup_logging, setup_verbose_logging
    setup_logging(flash_home=_ra()._flash_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that flash_logging.setup_logging() does not
        ...
        # handler level inside flash_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from flash_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from flash_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `flash model` to "
                        "select a provider, or run `flash setup` for first-time "
                        "configuration."
                    )
    ...
    # Session logs go into ~/.flash/sessions/ alongside gateway sessions
    flash_home = get_flash_home()
    agent.logs_dir = flash_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.flash/sessions/session_{sid}.json)
    ...
    try:
        from flash_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from flash_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "flash"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Flash' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "flash", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, flash, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from flash_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from flash_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Flash Agent.  Choose a model with at least "
    ...
    # Nous Flash 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from flash_cli.model_switch import _check_flash_model_warning
            ...
            if _flash_warn:
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_flash_warn)
        except Exception:
            pass
