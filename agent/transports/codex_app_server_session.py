...
Owns one Codex thread per Flash session. Drives `turn/start`, consumes
...
# Flash' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Flash/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Flash session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="flash",
            client_title="Flash Agent",
            client_version=_get_flash_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Flash' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Flash' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own flash-tools callback we
            # auto-accept — the user already approved Flash' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Flash approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Flash returns 'once', 'session', 'always', or 'deny'.
...
def _get_flash_version() -> str:
    """Best-effort Flash version string for codex's userAgent line."""
    try:
