...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_path = os.path.join(
        flash_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(flash_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Hermes -HostProfileId flash -HostVersion 1.0.0 "
