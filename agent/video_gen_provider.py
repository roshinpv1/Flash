...
as ``kind: backend``) or ``~/.nyxo/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``nyxo tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NYXO_HOME/cache/videos/``, creating parents as needed."""
    from nyxo_constants import get_nyxo_home

    path = get_nyxo_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NYXO_HOME/cache/videos/``.
