...
as ``kind: backend``) or ``~/.nyxo/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nyxo tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``nyxo tools`` model picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``nyxo tools``
...
def _images_cache_dir() -> Path:
    """Return ``$NYXO_HOME/cache/images/``, creating parents as needed."""
    from nyxo_constants import get_nyxo_home

    path = get_nyxo_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$NYXO_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$NYXO_HOME/cache/images/``.
