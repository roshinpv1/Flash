...
row taxonomy is inferred from the atlas shape so Flash can render both legacy
...
# rows (1536x1872), while older Flash/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Flash' activity state names. They are not always identical to the
...
# Legacy Flash/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Flash activity names -> accepted row-name aliases in descending
