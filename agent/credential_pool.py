...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "flash":
            # Flashstores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._flash_invoke_jwt_is_usable(
                        token,
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): flash, openai-codex, and xai-oauth rotate the
    ...
    ``flash_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``flash model`` / ``flash auth`` which
        ...
        Mirrors the Flash/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Flash process
    ...
    def _sync_flash_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Flashpool entry from auth.json if tokens differ.

        FlashOAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_flash_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the FlashPortal.
        """
        if self.provider != "flash" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Flashstate from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Flashentry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Flash, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Flash/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``flash`` invocation that defaults to the active provider
        (e.g. setup wizard, ``flash auth status``) would land on
        ...
        # device-code sources (flash, openai-codex, xAI) use ``device_code``.
        ...
        try:
            with _auth_store_lock():
                ...
                # flash_cli.auth._save_xai_oauth_tokens (#43589); the pool
                ...
                if self.provider == "flash":
                    ...
                    _store_provider_state(auth_store, "flash", state, set_active=False)
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Flash processes: otherwise
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("flash_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Flash
            ...
            elif self.provider == "flash":
                synced = self._sync_flash_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_flash_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_flash_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("flash_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as flash — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Flashquarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/flash — another Flash process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Flashquarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist(removed_ids=removed_ids)
                    return None
            # For flash: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "flash":
                synced = self._sync_flash_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Flashrefresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_flash_refresh_error(exc):
                    logger.debug("Flashrefresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "flash") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_flash_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_flash_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal FlashOAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.FLASH_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.FLASH_DEVICE_CODE_SOURCE}",
                    }
                    removed_ids = [
                        item.id for item in self._entries
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "flash":
            # Flashrefresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Flash profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For flash entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_flash_runtime_credentials(), making this entry's
            ...
            # re-authed via `flash model` / `flash auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `flash model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``flash auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `flash auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `flash auth remove <provider> <N>` is stable across all source types.
    try:
        from flash_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Flash PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `flash setup` ("Claude
        ...
        # Claude Code CLI, flash_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.flash/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `flash setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"flash_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_flash_oauth_credentials

        for source_name, creds in (
            ("flash_pkce", read_flash_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_flash_credentials(label=...) when the user ran
            # `flash auth add flash --label <name>`).  Fall back to the
        ...
        # Flash auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from flash_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.flash/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `flash auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `flash auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Flash auth store.  Without this gate the removal is instantly
        ...
        # Flash owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``flash model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``flash auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with flash/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from flash_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.flash/.env over os.environ — the user's config file is the
    # authoritative source for Flash credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_flash_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `flash auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.flash/.env.
    ...
    try:
        from flash_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from flash_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.flash/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `flash auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Flash
