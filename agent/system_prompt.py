...
upstream prefix cache warm.  See ``flash-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, flash subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_flash_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Flash never re-renders parts of this string mid-
    ...
    # Pointer to the flash-agent skill + docs for user questions about Flash itself.
    ...
    flash_subscription_prompt = _r.build_flash_subscription_prompt(agent.valid_tool_names)
    if flash_subscription_prompt:
        stable_parts.append(flash_subscription_prompt)
    ...
    # Coding posture (base Flash, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Flash profile the agent is running
    # under so it doesn't conflate ~/.flash/skills/ (default profile) with
    # ~/.flash/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Flash profile: default. Other profiles (if any) live "
            "under ~/.flash/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Flash profile: {active_profile}. This session reads "
            f"and writes ~/.flash/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.flash/skills/, ~/.flash/plugins/, "
            f"~/.flash/cron/, ~/.flash/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Flash never rebuilds or reinjects parts of it
