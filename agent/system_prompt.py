...
upstream prefix cache warm.  See ``nyxo-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Nyxo never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NYXO_HOME persona while keeping
    ...
    # Pointer to the nyxo-agent skill + docs for user questions about Nyxo itself.
    stable_parts.append(NYXO_AGENT_HELP_GUIDANCE)
    ...
    # NYXO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Nyxo, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Nyxo profile the agent is running
    # under so it doesn't conflate ~/.nyxo/skills/ (default profile) with
    # ~/.nyxo/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Nyxo profile: default. Other profiles (if any) live "
            "under ~/.nyxo/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Nyxo profile: {active_profile}. This session reads "
            f"and writes ~/.nyxo/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.nyxo/skills/, ~/.nyxo/plugins/, "
            f"~/.nyxo/cron/, ~/.nyxo/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Nyxo never rebuilds or reinjects parts of it
