...
Usage (via cron with --no-agent):
    ...
    flash cron create flash-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HERMES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name flash-issues --repo FlashOrg/flash-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Flash .env file
(``${HERMES_HOME:-~/.flash}/.env``) to avoid the 60 req/hr
