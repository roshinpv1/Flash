...
Usage (via cron with --no-agent):
    ...
    nyxo cron create nyxo-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$NYXO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name nyxo-issues --repo NousResearch/nyxo-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Nyxo .env file
(``${NYXO_HOME:-~/.nyxo}/.env``) to avoid the 60 req/hr
