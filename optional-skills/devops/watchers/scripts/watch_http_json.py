...
Usage (via cron with --no-agent):
    ...
    nyxo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$NYXO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Nyxo-Watcher/1.0"})
