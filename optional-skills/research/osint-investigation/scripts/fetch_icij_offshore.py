...
$NYXO_OSINT_CACHE/icij/ (default: ~/.cache/nyxo-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("NYXO_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "nyxo-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "nyxo-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $NYXO_OSINT_CACHE/icij or ~/.cache/nyxo-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
