...
which is the same trust boundary Flash uses for .env and OAuth tokens.
"""
...
def flash_home() -> Path:
    return Path(os.environ.get("HERMES_HOME") or (Path.home() / ".flash"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else flash_home() / "unbroker"
