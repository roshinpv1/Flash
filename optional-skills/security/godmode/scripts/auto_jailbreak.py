...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.flash")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERMES_HOME", Path.home() / ".flash")) / "skills" / "red-teaming" / "godmode"
...
HERMES_HOME = Path(os.getenv("HERMES_HOME", Path.home() / ".flash"))
...
def _get_current_model() -> tuple:
    """Read current model and provider from Flash config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.flash/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Flash for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Flash for changes to take effect.")
