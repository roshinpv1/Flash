...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NYXO_HOME", os.path.expanduser("~/.nyxo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NYXO_HOME", Path.home() / ".nyxo")) / "skills" / "red-teaming" / "godmode"
...
NYXO_HOME = Path(os.getenv("NYXO_HOME", Path.home() / ".nyxo"))
CONFIG_PATH = NYXO_HOME / "config.yaml"
PREFILL_PATH = NYXO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Nyxo models — already uncensored, just needs clean prompt
    "nyxo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nyxo" in model_lower or "nous" in model_lower:
        return "nyxo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Nyxo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nyxo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Nyxo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Nyxo for changes to take effect.")
