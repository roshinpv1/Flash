...
## 5. GODMODE FAST — Flash 4 405B (Zero Refusal)

**Model:** `flashorg/flash-4-405b`
**Strategy:** Flash models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Flash Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.flash/config.yaml`:
