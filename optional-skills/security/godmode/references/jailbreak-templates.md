...
## 5. GODMODE FAST — Nyxo 4 405B (Zero Refusal)

**Model:** `nousresearch/nyxo-4-405b`
**Strategy:** Nyxo models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Nyxo Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.nyxo/config.yaml`:
