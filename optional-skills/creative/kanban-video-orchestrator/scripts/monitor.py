#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `nyxo kanban list` and
...
def nyxo_available() -> bool:
    return shutil.which("nyxo") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed nyxo CLI."""
    try:
        out = subprocess.run(
            ["nyxo", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `nyxo kanban list`
    out = subprocess.run(
        ["nyxo", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not nyxo_available():
        print("ERROR: 'nyxo' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
