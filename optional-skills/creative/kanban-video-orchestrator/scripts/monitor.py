#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `flash kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed flash CLI."""
    try:
        out = subprocess.run(
            ["flash", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `flash kanban list`
    out = subprocess.run(
        ["flash", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["flash", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not flash_available():
        print("ERROR: 'flash' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
