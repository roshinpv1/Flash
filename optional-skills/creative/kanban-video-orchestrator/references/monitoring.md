...
nyxo kanban watch --tenant <project-slug>

# Snapshot of the board
nyxo kanban list --tenant <project-slug>
nyxo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nyxo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nyxo dashboard

# Inspect a specific task (includes comments + events)
nyxo kanban show <task-id>

# Follow a single task's event stream
nyxo kanban tail <task-id>
```

Verify available subcommands with `nyxo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nyxo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nyxo kanban block <id>` then `nyxo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nyxo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nyxo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nyxo profile list` (profile exists?), `nyxo status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(nyxo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nyxo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nyxo kanban block <task-id>
nyxo kanban archive <task-id>

# Diagnose what happened
nyxo kanban show <task-id>      # task body, comments, recent events
nyxo kanban tail <task-id>      # follow the live event stream
nyxo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nyxo kanban create` for the director
...
while true; do
    clear
    nyxo kanban list --tenant <slug>
    echo "---"
    nyxo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nyxo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nyxo profile list` and start
