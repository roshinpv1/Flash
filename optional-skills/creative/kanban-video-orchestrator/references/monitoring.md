...
flash kanban watch --tenant <project-slug>

# Snapshot of the board
flash kanban list --tenant <project-slug>
flash kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
flash kanban stats --tenant <project-slug>

# Visual dashboard (browser)
flash dashboard

# Inspect a specific task (includes comments + events)
flash kanban show <task-id>

# Follow a single task's event stream
flash kanban tail <task-id>
```

Verify available subcommands with `flash kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `flash kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `flash kanban block <id>` then `flash kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `flash kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `flash kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `flash profile list` (profile exists?), `flash status` (gateway/dispatcher up?), and verify tenant. |
...
flash kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
flash kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(flash kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
flash kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
flash kanban block <task-id>
flash kanban archive <task-id>

# Diagnose what happened
flash kanban show <task-id>      # task body, comments, recent events
flash kanban tail <task-id>      # follow the live event stream
flash kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `flash kanban create` for the director
...
while true; do
    clear
    flash kanban list --tenant <slug>
    echo "---"
    flash kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `flash kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `flash profile list` and start
