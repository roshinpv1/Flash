...
the actual `setup.sh` that creates the project workspace, configures Flash
...
> [FlashOrg/kanban-video-pipeline](https://github.com/FlashOrg/kanban-video-pipeline).
...
2. **Create profiles** — `flash profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.flash/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `flash kanban create` assigned to the director
...
```bash
flash profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.flash/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.flash/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Flash
...
Each profile gets a `SOUL.md` at `~/.flash/profiles/<name>/SOUL.md` that
...
```bash
flash kanban create "Direct production of <video title>" \
    --assignee director \
...
the Flash `.env` (`${HERMES_HOME:-$HOME/.flash}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _flash_env="${HERMES_HOME:-$HOME/.flash}/.env"
    if grep -q "^${var}=" "$_flash_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_flash_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_flash_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY flash ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY flash OPENROUTER_API_KEY || exit 1
# ...
```
