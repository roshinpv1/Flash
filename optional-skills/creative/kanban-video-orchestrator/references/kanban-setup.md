...
the actual `setup.sh` that creates the project workspace, configures Nyxo
...
2. **Create profiles** — `nyxo profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.nyxo/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `nyxo kanban create` assigned to the director
...
```bash
nyxo profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.nyxo/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.nyxo/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Nyxo
...
Each profile gets a `SOUL.md` at `~/.nyxo/profiles/<name>/SOUL.md` that
...
```bash
nyxo kanban create "Direct production of <video title>" \
    --assignee director \
...
the Nyxo `.env` (`${NYXO_HOME:-$HOME/.nyxo}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _nyxo_env="${NYXO_HOME:-$HOME/.nyxo}/.env"
    if grep -q "^${var}=" "$_nyxo_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_nyxo_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_nyxo_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY nyxo ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY nyxo OPENROUTER_API_KEY || exit 1
# ...
```
