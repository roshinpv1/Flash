...
Maps each role archetype to the Flash skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public flash-agent
...
## Flash skills relevant to video production

### Visual / rendering skills (`flash-agent/skills/creative/`)
...
### Design / pre-production skills (`flash-agent/skills/creative/`)
...
### Audio / media skills (`flash-agent/skills/creative/` + `skills/media/`)
...
These are **not** Flash skills but external CLIs / APIs that profiles invoke.
...
## Built-in Flash tools for media review

These are native Flash tools — not invoked via terminal but through their own
...
key is present in `${HERMES_HOME:-~/.flash}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Flash profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HERMES_HOME:-~/.flash}/.env` → environment variable.
...
When a new Flash-public video skill ships:
