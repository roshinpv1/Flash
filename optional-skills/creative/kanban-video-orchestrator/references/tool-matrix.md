...
Maps each role archetype to the Nyxo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nyxo-agent
...
## Nyxo skills relevant to video production

### Visual / rendering skills (`nyxo-agent/skills/creative/`)
...
### Design / pre-production skills (`nyxo-agent/skills/creative/`)
...
### Audio / media skills (`nyxo-agent/skills/creative/` + `skills/media/`)
...
These are **not** Nyxo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Nyxo tools for media review

These are native Nyxo tools — not invoked via terminal but through their own
...
key is present in `${NYXO_HOME:-~/.nyxo}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Nyxo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${NYXO_HOME:-~/.nyxo}/.env` → environment variable.
...
When a new Nyxo-public video skill ships:
