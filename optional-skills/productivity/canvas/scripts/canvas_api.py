#!/usr/bin/env python3
"""Canvas LMS API CLI for Flash Agent.
...
def _check_config():
    ...
    if missing:
        flash_env = os.path.join(
            os.environ.get("HERMES_HOME", os.path.expanduser("~/.flash")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {flash_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Flash Agent"
    )
