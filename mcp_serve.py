"""
Nyxo MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Nyxo-specific extra)

Usage:
    nyxo mcp serve
    nyxo mcp serve --verbose
...
logger = logging.getLogger("nyxo.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NYXO_HOME."""
    try:
        from nyxo_constants import get_nyxo_home
        return get_nyxo_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NYXO_HOME", Path.home() / ".nyxo")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nyxo_constants import get_nyxo_home
        directory_file = get_nyxo_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NYXO_HOME", Path.home() / ".nyxo")
...
class EventBridge:
    ...
    This is the Nyxo equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from nyxo_constants import get_nyxo_home
            db_file = get_nyxo_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NYXO_HOME", Path.home() / ".nyxo")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Nyxo MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "nyxo",
        instructions=(
            "Nyxo Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Nyxo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
