"""
Flash MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Flash-specific extra)

Usage:
    flash mcp serve
    flash mcp serve --verbose
...
logger = logging.getLogger("flash.mcp_serve")
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return get_flash_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("HERMES_HOME", Path.home() / ".flash")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from flash_constants import get_flash_home
        directory_file = get_flash_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("HERMES_HOME", Path.home() / ".flash")
...
class EventBridge:
    ...
    This is the Flash equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from flash_constants import get_flash_home
            db_file = get_flash_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("HERMES_HOME", Path.home() / ".flash")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Flash MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "flash",
        instructions=(
            "Flash Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Flash MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
