...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_flash_home() under the active profile.
...
#   - cli.py, flash_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like flash-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("flash-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (flash-*) include _HERMES_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("flash-")
    ...
    # window). Core Flash tools (toolsets._HERMES_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from flash_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to flash-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from flash_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from flash_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        try:
            ...
            from flash_cli.middleware import run_tool_execution_middleware
