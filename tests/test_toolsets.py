...
class TestToolsetConsistency:
    ...
    def test_flash_platforms_share_core_tools(self):
        """All flash-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on flash-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["flash-cli", "flash-telegram", "flash-discord", "flash-whatsapp", "flash-slack", "flash-signal", "flash-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_flash_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("flash-whatsapp")

    def test_flash_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("flash-api-server")
