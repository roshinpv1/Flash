...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``flash_cli.config.load_config``
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "flash_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        flash_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://flash.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "flash_session_pkce" in result.cookie_payload
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["flash_session_pkce"] != b.cookie_payload[
            "flash_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://flash.fly.dev/auth/callback")
