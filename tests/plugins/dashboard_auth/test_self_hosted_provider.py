...
_ISSUER = "https://auth.example.com/application/o/flash"
_CLIENT_ID = "flash-dashboard"
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://flash.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://flash.example/auth/callback"
    ...
    def test_code_verifier_length(self, provider):
        ...
        pkce = result.cookie_payload["flash_session_pkce"]
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://flash.example/auth/callback")
        b = provider.start_login(redirect_uri="https://flash.example/auth/callback")
        assert (
            a.cookie_payload["flash_session_pkce"]
            != b.cookie_payload["flash_session_pkce"]
        )
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://flash.example/auth/callback")
...
class TestPluginRegister:
    ...
    def patch_config(self, monkeypatch):
        def _set(oauth_block):
            ...
            monkeypatch.setattr("flash_cli.config.load_config", lambda: cfg)
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("flash_cli.config.load_config", _broken)
        ctx = MagicMock()
