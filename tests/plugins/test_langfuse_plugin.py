...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_flash_config(self, monkeypatch):
        ...
        # Drop any cached import of flash_cli.config.
        sys.modules.pop("flash_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "flash_cli.config" not in sys.modules, (
            "langfuse plugin imported flash_cli.config — regression toward "
...
# Flash logs.  The fix in `_get_langfuse()` validates the documented
