...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_flash_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``flash pairing`` CLI continues to work without a profile context."""
        from flash_constants import get_flash_home
        monkeypatch.setattr("flash_constants.get_flash_home", lambda: tmp_path)
