...
class TestPayloadFilters:
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.flash/data/watchlist.json",
                            },
                            {
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_flash_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.flash/scripts paths must resolve through HERMES_HOME for profiles."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.flash/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
