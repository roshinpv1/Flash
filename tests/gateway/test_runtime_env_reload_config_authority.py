...
NYXO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env NYXO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    nyxo_home = tmp_path / ".nyxo"
    nyxo_home.mkdir()
    (nyxo_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (nyxo_home / ".env").write_text("NYXO_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["NYXO_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("NYXO_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["NYXO_MAX_ITERATIONS"] = "200"
