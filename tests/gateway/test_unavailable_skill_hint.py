...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`flash_cli.skills_config.save_disabled_skills` stores.
"""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `flash skills config` writes).
    monkeypatch.setattr(
    ...
    assert "flash skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "flash_constants.get_optional_skills_dir",
