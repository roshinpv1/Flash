...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.flash/media/'
...
class TestMediaDeliveryPathValidation:
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.flash/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_flash_credentials(self, tmp_path, monkeypatch):
        """~/.flash/.env and ~/.flash/auth.json stay blocked even in
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.flash/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        flash_dir = fake_home / ".flash"
        (flash_dir / "mcp-tokens").mkdir(parents=True)
        secret = flash_dir / rel
    ...
    def test_denylist_blocks_flash_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        flash_dir = fake_home / ".flash"
        flash_dir.mkdir(parents=True)
        config_file = flash_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_flash_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Flash root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".flash" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        flash_root = fake_home / ".flash"
        config_file = flash_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        ...
        flash_dir = fake_home / ".flash"
        flash_dir.mkdir(parents=True)
        token = flash_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._HERMES_HOME", flash_dir)
        monkeypatch.setattr("gateway.platforms.base._HERMES_ROOT", flash_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.flash/pairing/ (platform pairing tokens) are
    ...
    def test_flash_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        flash_dir = fake_home / ".flash"
        cache_dir = flash_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_flash_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.flash
        ...
        flash_dir = fake_home / ".flash"
        flash_dir.mkdir(parents=True)
        artifact = flash_dir / "adhoc_report.pdf"
    ...
    def test_root_home_flash_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.flash/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        ...
        under a denied prefix (e.g. /opt/data -> /root/.flash) emits
        ...
        flash_root = denied_root / ".flash"
        prof_cache = flash_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._HERMES_ROOT", flash_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        flash_root = denied_root / ".flash"
        prof_dir = flash_root / "profiles" / "myprof"
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Flash
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../flash/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.flash/cache/media/sensitive_host_path_abc123.bin"
