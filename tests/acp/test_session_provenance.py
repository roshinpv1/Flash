...
def test_root_session_no_compression(db):
    ...
    assert prov["currentNyxoSessionId"] == "root1"
    assert prov["rootNyxoSessionId"] == "root1"
    assert prov["parentNyxoSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_nyxo_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentNyxoSessionId"] == "old"
    assert prov["rootNyxoSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousNyxoSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootNyxoSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootNyxoSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_nyxo_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousNyxoSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"nyxo"}
    assert "sessionProvenance" in meta["nyxo"]
    assert meta["nyxo"]["sessionProvenance"]["currentNyxoSessionId"] == "root1"
