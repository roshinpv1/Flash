...
def test_root_session_no_compression(db):
    ...
    assert prov["currentFlashSessionId"] == "root1"
    assert prov["rootFlashSessionId"] == "root1"
    assert prov["parentFlashSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_flash_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentFlashSessionId"] == "old"
    assert prov["rootFlashSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousFlashSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootFlashSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootFlashSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_flash_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousFlashSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"flash"}
    assert "sessionProvenance" in meta["flash"]
    assert meta["flash"]["sessionProvenance"]["currentFlashSessionId"] == "root1"
