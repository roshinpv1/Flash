"""Tests for nyxo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Nyxo entry point
(nyxo, nyxo-agent, nyxo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Nyxo entry point imports nyxo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported nyxo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("nyxo_bootstrap", None)
    import nyxo_bootstrap  # noqa: WPS433
    return nyxo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Nyxo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Nyxo entry point must import nyxo_bootstrap as its
    ...
    # Entry points that invoke Nyxo as a process.  Each one must
    # import nyxo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "nyxo_cli/main.py",   # nyxo CLI (console_script)
        "run_agent.py",          # nyxo-agent (console_script)
        "acp_adapter/entry.py",  # nyxo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import nyxo_bootstrap' and that
        ...
        half-finished ``nyxo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``nyxo_bootstrap`` as a top-level module) leaves nyxo
        ...
        """
        # Resolve relative to the nyxo-agent repo root.  Tests live
        # at tests/test_nyxo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # nyxo start even when nyxo_bootstrap hasn't been
        ...
        assert first_import_name == "nyxo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'nyxo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import nyxo_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Nyxo's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("NYXO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["NYXO_PYTHON_SRC_ROOT"] = env
            elif "NYXO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["NYXO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/nyxo")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NYXO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["NYXO_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/nyxo", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Nyxo root.  The guard must relocate Nyxo to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/nyxo"])
        assert result[0] == "/opt/nyxo"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Nyxo root.
        assert result.index("/opt/nyxo") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/nyxo", "/opt/nyxo", ""])
        assert result.count("/opt/nyxo") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["NYXO_PYTHON_SRC_ROOT"] = "/env/nyxo"
            hb.harden_import_path()
            assert sys.path[0] == "/env/nyxo"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NYXO_PYTHON_SRC_ROOT", None)
