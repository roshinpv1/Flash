...
- Storage: a job created under profile ``coder`` writes to
  ``~/.flash/profiles/coder/cron/jobs.json`` — NOT the shared default root.
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    monkeypatch.setattr(
        flash_constants, "_get_platform_default_flash_home", lambda: root
    )
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    assert flash_constants.get_flash_home().resolve() == profile_home.resolve()
    assert flash_constants.get_default_flash_root().resolve() == root.resolve()

    # cron/jobs.py computes HERMES_DIR from get_flash_home() at import, so a
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_flash_home", None, raising=False)
    assert scheduler._get_flash_home().resolve() == profile_home.resolve()
    assert scheduler._get_flash_home().resolve() != root.resolve()
