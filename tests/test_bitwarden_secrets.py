...
binary download are exercised manually by `flash secrets bitwarden
...
def flash_home(tmp_path, monkeypatch):
    """Point Flash at an isolated home directory."""
    home = tmp_path / ".flash"
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    # Some modules cache get_flash_home; clear if needed.
    import flash_constants
    if hasattr(flash_constants, "_HERMES_HOME_CACHE"):
        flash_constants._HERMES_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(flash_home, monkeypatch):
...
def test_install_bws_happy_path(flash_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(flash_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(flash_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from flash_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under flash_home/cache/."""
    home = tmp_path / ".flash"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
