...
Before the fix, ``flash whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``flash gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(flash))
...
def _env_value(flash_home: Path, key: str) -> str | None:
    env_file = flash_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from flash_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("flash_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``flash whatsapp`` with an existing paired session and
