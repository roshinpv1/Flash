...
def flash_home(tmp_path, monkeypatch):
    ...
    # /etc/flash on the dev/CI box can't influence the test.
...
def test_user_config_overrides_default(flash_home, monkeypatch):
    from flash_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(flash_home, monkeypatch):
    from flash_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(flash_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_flash_dotenv(flash_home=str(home))
