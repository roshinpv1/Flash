"""Tests for IRC gateway configuration via `flash setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Flash install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Flash install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#flash")
            monkeypatch.setenv("IRC_NICKNAME", "flash-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `flash setup gateway` experience with IRC present."""
