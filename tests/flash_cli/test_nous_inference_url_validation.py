...
client. ``_validate_flash_inference_url_from_network()`` blocks any URL
...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.flashorg.com/v1"
        assert _validate_flash_inference_url_from_network(url) == url

    def test_trailing_slash_stripped(self):
        url = "https://inference-api.flashorg.com/v1/"
        assert _validate_flash_inference_url_from_network(url) == url.rstrip("/")

    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="flash_cli.auth"):
            assert (
                _validate_flash_inference_url_from_network("https://attacker.com/v1")
                is None
            )
        assert any("attacker.com" in rec.message for rec in caplog.records)

    def test_subdomain_of_allowlist_host_rejected(self):
        """*.flashorg.com is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.flashorg.com would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_flash_inference_url_from_network(
                "https://evil.inference-api.flashorg.com/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="flash_cli.auth"):
            assert (
                _validate_flash_inference_url_from_network(
                    "http://inference-api.flashorg.com/v1"
                )
                is None
            )
    ...
    def test_file_scheme_rejected(self):
        assert (
            _validate_flash_inference_url_from_network("file:///etc/passwd") is None
        )
    ...
    def test_empty_string_rejected(self):
        assert _validate_flash_inference_url_from_network("") is None

    def test_whitespace_only_rejected(self):
        assert _validate_flash_inference_url_from_network("   ") is None

    def test_none_rejected(self):
        assert _validate_flash_inference_url_from_network(None) is None

    def test_non_string_rejected(self):
        assert _validate_flash_inference_url_from_network(12345) is None  # type: ignore[arg-type]
        assert _validate_flash_inference_url_from_network({"url": "x"}) is None  # type: ignore[arg-type]

    def test_malformed_url_rejected(self):
        """Even garbled input must fall back safely, not raise."""
        assert (
            _validate_flash_inference_url_from_network("not://a real url at all")
            is None
        )
...
class TestCallSiteWiring:
    ...
    ``_validate_flash_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_flash_runtime_credentials`` and one
    ...
    def _read_auth_source(self):
        import flash_cli.auth as _auth_mod
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_flash_inference_url_from_network() instead."
            )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        refresh_count = source.count(
            '_validate_flash_inference_url_from_network(refreshed.get("inference_base_url"))'
        )
        mint_count = source.count(
            '_validate_flash_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        ...
        boundary."""
        from pathlib import Path
        import flash_cli.proxy.adapters.flash_portal as _flash_adapter
        source = Path(_flash_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_flash_inference_url_from_network" in source
...
class TestEnvOverrideNotGated:
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_flash_runtime_credentials, the env override is
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.flashorg.com`` persisted before the
    ...
    staging endpoint (real incident: opus-4.8 routed to flash, flash pinned
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-inference-api.flashorg.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "flash-cli",
        ...
        monkeypatch.setattr(auth, "_assert_flash_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_flash_invoke_jwt", lambda *a, **k: None)
    ...
    def test_refresh_keeps_valid_url(self, monkeypatch):
        ...
        good = "https://inference-api.flashorg.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "flash-cli",
...
class TestEnvOverrideWins:
    ...
    STAGING = "https://stg-inference-api.flashorg.com/v1"

    def _patch_no_refresh(self, monkeypatch, auth, state):
        import contextlib

        # No refresh fires: the stored access token is a usable invoke JWT.
        monkeypatch.setattr(auth, "_flash_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(auth, "_write_shared_flash_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_flash_pool_from_auth_store", lambda *a, **k: None)
...
class TestProxyAdapterEnvOverride:
    ...
    def test_proxy_adapter_consults_env_override(self):
        ...
        source = Path(_flash_adapter.__file__).read_text(encoding="utf-8")
        assert "_flash_inference_env_override()" in source, (
