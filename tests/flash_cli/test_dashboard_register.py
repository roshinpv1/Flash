"""Tests for ``flash dashboard register``.
...
The portal HTTP call and the Flashtoken resolution are both mocked — this
...
import flash_cli.dashboard_register as dr
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.flashorg.com",
        ...
        with patch(
            ...
            "flash_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "flash_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://flash.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://flash.example.com/auth/callback"
        )
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            ...
            "flash_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "flash_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        saved = self._run(
            args=_ns(portal_url="https://portal.flashorg.com"),
            portal="https://portal.flashorg.com",
            existing_portal=None,
        )
        assert (
            saved["HERMES_DASHBOARD_PORTAL_URL"] == "https://portal.flashorg.com"
        )
...
class TestPublicUrlPersistence:
    ...
    https://flash.example.com/auth/callback). At serve time the dashboard auth
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://flash.example.com"
    ...
    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://flash.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://flash.example.com:8443"
    ...
    def test_public_url_equal_to_existing_is_noop(self, capsys):
        ...
        saved = self._run(
            args=_ns(redirect_uri="https://flash.example.com/auth/callback"),
            existing_public="https://flash.example.com",
        )
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://flash.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "flash_cli.config.get_env_value", return_value=None
        ), patch(
            "flash_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://flash.example.com/auth/callback",
                )
            )
        assert saved["HERMES_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://flash.example.com"
...
class TestPortalResolution:
    ...
    def test_falls_back_to_stored_login_portal(self):
        with patch(
            ...
            return_value={"portal_base_url": "https://portal.staging-flashorg.com"},
        ):
            assert (
                dr._resolve_portal_base_url(None)
                == "https://portal.staging-flashorg.com"
            )
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        err = urllib.error.HTTPError(
            url="https://portal.flashorg.com/api/oauth/self-hosted-client",
            code=code,
            msg="err",
            hdrs=None,
