...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        flash = "flash_cli.main:main"
        flash-agent = "run_agent:main"
        flash-acp = "acp_adapter.entry:main"
    """
        )
    )
...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("flash", "flash-agent", "flash-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with patch("flash_cli.main._is_windows", return_value=True), \
             patch("flash_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("flash_cli.main._run_quarantined_install") as mock_install:
            from flash_cli.main import _verify_console_scripts_installed

            _verify_console_scripts_installed(["uv", "pip"], env={})

        mock_install.assert_not_called()

    def test_triggers_reinstall_when_flash_exe_missing(
        self, temp_pyproject, fake_scripts_dir
    ):
        (fake_scripts_dir / "flash-agent.exe").write_bytes(b"fake")
        (fake_scripts_dir / "flash-acp.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("flash_cli.main._is_windows", return_value=False), \
             patch("flash_cli.main._run_quarantined_install") as mock_install:
            from flash_cli.main import _verify_console_scripts_installed
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from flash_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["flash", "flash-agent", "flash-acp"]
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("flash_cli.main._is_windows", return_value=False), \
             patch("flash_cli.main._run_quarantined_install") as mock_install, \
             patch("flash_cli.main._verify_console_scripts_installed") as mock_verify:
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("flash_cli.main._is_windows", return_value=True):
            names = {path.name for path in main_mod._flash_exe_shims(fake_scripts_dir)}

        assert {"flash.exe", "flash-agent.exe", "flash-acp.exe"} <= names
        assert "flash-gateway.exe" in names
