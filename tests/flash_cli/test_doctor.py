...
import flash_cli.doctor as doctor
import flash_cli.gateway as gateway_cli
from flash_cli import doctor as doctor_mod
from flash_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `flash doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "HERMES_HOME", flash_home)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_flash_home(self, tmp_path, provider=""):
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_flash_home(tmp_path, provider)
        ...
        try:
            from flash_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_flash_auth_status", lambda: {})
...
def test_run_doctor_accepts_flash_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        ...
        leaves doctor probing the REAL ~/.flash. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_flash_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from flash_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("flash_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "flash_cli.doctor", raising=False)
        from flash_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Flash/ Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from flash_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_flash_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Flash/ Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".flash"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "FlashPortal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (flash_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "HERMES_HOME", flash_home)
        monkeypatch.setattr(doctor_mod, "get_flash_home", lambda: flash_home)
        ...
        return buf.getvalue(), flash_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, flash_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (flash_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (flash_home / ".env").read_text(encoding="utf-8")
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`flash doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Flash install.
