"""Tests for the top-level `./flash` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./flash` should use `flash_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "flash"
    called = []

    fake_main_module = types.ModuleType("flash_cli.main")

    def fake_main():
        called.append("flash_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "flash_cli.main", fake_main_module)
