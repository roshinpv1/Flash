...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = flash_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=flash-agent\n"
        "exec_user=flash\n"
        "flash_bin=/data/current-package/bin/flash\n"
    )
    return flash_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("flash_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "flash-agent"
    assert info["exec_user"] == "flash"
    assert info["flash_bin"] == "/data/current-package/bin/flash"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("flash_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_flash_dev(container_env, monkeypatch):
...
def test_get_container_exec_info_not_skipped_when_flash_dev_zero(container_env, monkeypatch):
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        flash_home = Path(tmpdir) / ".flash"
        flash_home.mkdir()
        (flash_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("flash_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_flash_home": lambda: flash_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=flash-custom\n"
        "exec_user=myuser\n"
        "flash_bin=/opt/flash/bin/flash\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "flash-custom"
    assert info["exec_user"] == "myuser"
    assert info["flash_bin"] == "/opt/flash/bin/flash"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from flash_cli.main import _exec_in_container
