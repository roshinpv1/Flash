...
from flash_cli.slack_cli import _build_full_manifest
from flash_cli.subcommands.slack import build_slack_parser


def _parse_slack_args(argv):
    """Build the real `flash slack` parser and parse argv against it."""
...
class TestSlackFullManifest:
    """Generated full Slack app manifest used by `flash slack manifest`."""

    def test_app_home_messages_are_writable(self):
        manifest = _build_full_manifest("Flash", "Your Flash agent on Slack")
    ...
    def test_group_dm_surface_present_without_assistant_mode(self):
        ...
        manifest = _build_full_manifest(
            "Flash", "Your Flash agent on Slack", include_assistant=False
        )
