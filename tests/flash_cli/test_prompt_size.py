"""Tests for the ``flash prompt-size`` diagnostic (issue #34667)."""
...
from flash_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(flash_home, memory_text="", user_text=""):
    mem_dir = flash_home / "memories"
...
def _seed_skill(flash_home, name, description):
    skill_dir = flash_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "FLASH_API_KEY",
                "ANTHROPIC_API_KEY"):
