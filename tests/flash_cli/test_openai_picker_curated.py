...
Bug 1 — OpenAI picker dumped the raw ``/v1/models`` catalog
    ...
    moderation, gpt-3.5, …). The ``flash model`` CLI shows only the curated
...
Bug 2 — OpenRouter appeared authenticated whenever OPENAI_API_KEY was set
    OpenRouter's FlashOverlay carried ``extra_env_vars=("OPENAI_API_KEY",)``.
...
from flash_cli import models as M
