...
the SAME provider universe as ``flash model`` (the CLI/TUI picker).
...
def test_every_flash_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `flash model` universe."""
    ...
    assert not missing, (
        "providers shown in `flash model` but not configurable in the desktop "
