"""Tests for ``flash debug`` CLI command and debug utilities."""
...
def flash_home(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO flash_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from flash_cli.debug import _upload_paste_rs
        ...
        with patch("flash_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "flash_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from flash_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from flash_cli.debug import upload_to_pastebin

        with patch("flash_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("flash_cli.debug._upload_paste_rs",
             ...
             patch("flash_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, flash_home):
        from flash_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, flash_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (flash_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, flash_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = flash_home / "logs" / "agent.log"
        from flash_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, flash_home):
        ...
        (flash_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, flash_home):
    ...
    def test_unknown_log_returns_none(self, flash_home):
        from flash_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, flash_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from flash_cli.debug import _capture_log_snapshot

        logs_dir = flash_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, flash_home):
    ...
    def test_falls_back_when_primary_empty(self, flash_home):
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def flash_home_with_secret(self, tmp_path, monkeypatch):
    ...
    def test_default_redacts_tail_and_full_text(self, flash_home_with_secret):
        from flash_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, flash_home_with_secret):
        from flash_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, flash_home_with_secret, monkeypatch
    ):
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, flash_home_with_secret
    ):
        from flash_cli.debug import _capture_log_snapshot

        log_path = flash_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, flash_home_with_secret):
        from flash_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from flash_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, flash_home):
        from flash_cli.debug import collect_debug_report

        with patch("flash_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- flash dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- flash dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, flash_home):
        from flash_cli.debug import collect_debug_report

        with patch("flash_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, flash_home):
        from flash_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, flash_home):
        from flash_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, flash_home):
        from flash_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, flash_home):
        from flash_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, flash_home, capsys):
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch("flash_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("flash_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, flash_home, capsys):
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch(
                 "flash_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, flash_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, flash_home, capsys):
        ...
        with patch("flash_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- flash dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- flash dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- flash dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- flash dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- flash dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, flash_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from flash_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch("flash_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("flash_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, flash_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, flash_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, flash_home_with_secret, capsys
    ):
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch("flash_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("flash_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "flash debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, flash_home):
        from flash_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from flash_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from flash_cli.debug import delete_paste
        ...
        with patch("flash_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.flash/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchroflashly on the next ``flash debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, flash_home):
        ...
        import ast
        import inspect
        from flash_cli.debug import _schedule_auto_delete
        ...
        # (it should just write pending.json synchroflashly).
        import os as _os
    ...
    def test_records_pending_to_json(self, flash_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from flash_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, flash_home):
    ...
    def test_merges_with_existing_pending(self, flash_home):
        """Subsequent calls merge into existing pending.json."""
        from flash_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, flash_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, flash_home):
        from flash_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, flash_home):
        from flash_cli.debug import (
        ...
        with patch("flash_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, flash_home):
        from flash_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("flash_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, flash_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, flash_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, flash_home):
        from flash_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("flash_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, flash_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: flash debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from flash_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("flash_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("flash_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, flash_home, capsys):
        from flash_cli.debug import run_debug_share
        ...
        with patch("flash_cli.dump.run_dump"), \
             ...
             patch("flash_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, flash_home, capsys):
        from flash_cli.debug import run_debug_share
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch("flash_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("flash_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, flash_home, capsys):
        from flash_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``flash debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, flash_home):
        from flash_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("flash_cli.dump.run_dump"), patch(
            "flash_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("flash_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, flash_home):
        from flash_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (flash_home / "logs" / "desktop.log").unlink()

        with patch("flash_cli.dump.run_dump"), patch(
            "flash_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, flash_home):
        from flash_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, flash_home):
        from flash_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, flash_home):
        from flash_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, flash_home):
        from flash_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, flash_home):
        from flash_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, flash_home):
        from flash_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, flash_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from flash_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, flash_home):
        ...
        from flash_cli.debug import build_flash_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_flash_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "flash-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_flash_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
...
class TestRunDebugShareNous:
    ...
    def test_flash_success_prints_view_url(self, flash_home, capsys):
        from flash_cli.debug import run_debug_share
        ...
        with patch("flash_cli.dump.run_dump"), patch(
            "flash_cli.diagnostics_upload.share_to_flash", return_value=res
        ) as share:
    ...
    def test_flash_failure_suggests_local(self, flash_home, capsys):
        from flash_cli.debug import run_debug_share

        with patch("flash_cli.dump.run_dump"), patch(
            "flash_cli.diagnostics_upload.share_to_flash",
    ...
    def test_flash_does_not_touch_pastebin(self, flash_home):
        ...
        with patch("flash_cli.dump.run_dump"), patch(
            "flash_cli.diagnostics_upload.share_to_flash", return_value=res
        ), patch("flash_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [flash|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``FlashCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("flash_cli.debug.run_debug_share", _fake_run):
            self._handler()(cmd_original)
        return captured

    def test_bare_debug_defaults_to_paste(self):
        c = self._captured("/debug")
        assert c["flash"] is False and c["local"] is False
    ...
    def test_flash_word_sets_flash(self):
        c = self._captured("/debug flash")
        assert c["flash"] is True and c["local"] is False
    ...
    def test_local_wins_over_flash(self):
        ...
        c = self._captured("/debug flash local")
        assert c["local"] is True and c["flash"] is False
...
class TestShareConsentGate:
    """`flash debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, flash=False,
                    no_redact=False, yes=False)
        base.update(over)
        return SimpleNamespace(**base)

    def test_aborts_on_user_decline(self, flash_home, capsys, monkeypatch):
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch("flash_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())

        mock_upload.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_proceeds_on_user_accept(self, flash_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, flash_home, capsys, monkeypatch):
    ...
    def test_non_interactive_requires_yes(self, flash_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, flash_home, capsys, monkeypatch):
    ...
    def test_flash_path_also_gated(self, flash_home, capsys, monkeypatch):
        """The --flash S3 path enforces the same consent gate (sibling site)."""
        from flash_cli.debug import run_debug_share
        ...
        with patch("flash_cli.dump.run_dump"), \
             patch("flash_cli.diagnostics_upload.share_to_flash") as mock_flash:
            run_debug_share(self._args(flash=True))

        mock_flash.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_local_never_prompts(self, flash_home, capsys, monkeypatch):
