...
# ── deprecated `flash login` fails gracefully, not with argparse error ────
#
# `flash login` is a removed command; its handler (`login_command` in
# `flash_cli/auth.py`) prints a deprecation notice pointing at `flash auth` /
# `flash model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `flash --help`.
