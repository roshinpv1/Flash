...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /flash-gateway.service"`` and runs ``systemctl --user daemon-reload``
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "flash"
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Flash gateway as root" in out
    assert "/opt/flash/docker/entrypoint.sh" in out
...
def test_run_gateway_refuses_when_service_supervising(monkeypatch, capsys):
    ...
    assert "flash gateway restart" in out
    assert "--force" in out
...
def test_run_gateway_refuses_existing_process_before_importing_gateway_run(monkeypatch, capsys):
    ...
    assert "flash gateway run --replace" in out
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("flash_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("flash_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_running_under_gateway_supervisor_markers(monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "org.flashorg.flash.gateway")
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    A Telegram-hosted agent may run `flash gateway restart` via the terminal
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "flash-gateway.service"
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "flash-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "flash-gateway.service"
    system_unit = tmp_path / "system" / "flash-gateway.service"
    ...
    assert "flash gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_non_root_never_offers_system(monkeypatch, capsys):
    ...
    # handed a `sudo flash …` self-elevation recipe.
    captured = {}
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_flash_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_flash_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "flash-gateway.service"
...
def test_scan_gateway_pids_detects_windows_flash_exe_case_variants(monkeypatch):
    monkeypatch.setattr(gateway, "is_windows", lambda: True)
    ...
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["wmic.exe", "process", "get", "ProcessId,CommandLine"]:
            return SimpleNamespace(
                returncode=0,
                stdout=(
                    "CommandLine=C:\\Program Files\\Flash\\Flash.EXE gateway run --replace\n"
                    "ProcessId=2468\n\n"
                ),
                stderr="",
            )
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "flash_cli.gateway"
