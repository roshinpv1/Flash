...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Flash-Session-Token": web_server._SESSION_TOKEN},
    )
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 flash-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
    ...
    ("flash-agent-prod-abc.fly.dev", False, True),
])
...
def test_start_server_insecure_public_no_longer_bypasses_gate(monkeypatch):
    ...
    """
    from flash_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
def test_start_server_surfaces_flash_skip_reason_when_unconfigured(monkeypatch):
    ...
    from flash_cli.dashboard_auth import clear_providers
    from plugins.dashboard_auth import flash as flash_plugin
    ...
    flash_plugin.register(MagicMock())  # populates LAST_SKIP_REASON
