"""Tests for flash_cli configuration management."""
...
class TestEnsureFlashHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from flash_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from flash_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".flash" / "profiles" / "coder"
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``flash logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from flash_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="flash_cli.config"):
                config = load_config()
            ...
            # stands out at flash startup before logging is configured)
            captured = capsys.readouterr()
            assert "flash config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert "flash config:" in after_edit, "edited file should re-warn"
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from flash_cli.config import _deep_merge
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from flash_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `flash setup`) lets a
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `flash setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Flash
    process on next startup via the dotenv → ``os.environ`` chain in
    ``flash_cli/env_loader.py``.
    ...
    def _flash_home(self, tmp_path, monkeypatch):
    ...
    def test_flash_integration_keys_still_writable(self, allowed_key):
...
class TestCodexAppServerAutoConfig:
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "flash"
