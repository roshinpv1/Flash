"""Tests for the Flash plugin system (flash_cli.plugins)."""
...
from flash_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<flash_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<flash_home>/plugins/``; we derive
    ``<flash_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        ...
        import os
        flash_home_str = os.environ.get("HERMES_HOME")
        if flash_home_str:
            flash_home = Path(flash_home_str)
        else:
            flash_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.flash/plugins/ are discovered."""
        plugins_dir = tmp_path / "flash_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "flash_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        plugins_dir = project_dir / ".flash" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        flash_home = tmp_path / "flash_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under flash_plugins.<name>."""
        plugins_dir = tmp_path / "flash_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "flash_test"))

        # Clean up any prior namespace module
        sys.modules.pop("flash_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "flash_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="flash_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallDirective:
    ...
    def test_approve_directive_returned(self, monkeypatch):
        from flash_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "flash_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_block_returns_message(self, monkeypatch):
        from flash_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "flash_cli.plugins.invoke_hook",
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from flash_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "flash_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("flash_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("flash_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from flash_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from flash_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("flash_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `flash plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("flash_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        flash_home = tmp_path / "flash_test"
        plugins_dir = flash_home / "plugins"
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("flash_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("flash_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("flash_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("flash_cli.plugins.PluginContext.dispatch_tool.__module__", "flash_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from flash_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".flash" / "profiles" / "coder"
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".flash" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(prof))
        ctx = self._ctx()
