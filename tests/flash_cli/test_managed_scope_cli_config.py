...
flash_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `flash config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_flash_home = get_flash_home()`` at import time (module
