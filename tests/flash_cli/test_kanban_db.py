"""Tests for the Kanban DB layer (flash_cli.kanban_db)."""
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from flash_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Flash profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="flash-task", assignee="daily")
...
def test_scratch_workspace_created_under_flash_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import flash_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Flash-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Flash' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Flash' DB/metadata/logs on task completion.
    """
...
# spawned with `flash -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def _set_home(self, monkeypatch, tmp_path, flash_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(flash_home))
        monkeypatch.delenv("HERMES_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_flash(
        self, tmp_path, monkeypatch
    ):
        # Standard install: HERMES_HOME == ~/.flash, no profile active.
        default_home = tmp_path / ".flash"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.flash/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.flash/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.flash/kanban.db.
        default_home = tmp_path / ".flash"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Worker's perspective (profile activated by `flash -p coder`).
    ...
    def test_docker_custom_flash_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: HERMES_HOME points outside ~/.flash.
        # `get_default_flash_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".flash"`.
        custom_root = tmp_path / "opt" / "flash"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: HERMES_HOME=/opt/flash/profiles/coder;
        # `get_default_flash_root()` walks up to /opt/flash because
    ...
    def test_explicit_override_via_flash_kanban_home(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_flash_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # HERMES_KANBAN_DB pins the file path directly and beats both
        # HERMES_KANBAN_HOME and the `get_default_flash_root()` path.
    ...
    def test_flash_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see flash_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("flash_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="flash_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'flash kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_flash_argv()
#
# Workers spawned by the dispatcher must use a `flash` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["flash", ...]`
...
def test_resolve_flash_argv_prefers_path_shim(monkeypatch):
    """When `flash` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import flash_cli.kanban_db as kb

    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/flash")
    argv = kb._resolve_flash_argv()
    assert argv == ["/usr/local/bin/flash"]


def test_resolve_flash_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import flash_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HERMES_BIN", ".\\flash.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_flash_argv() == [os.path.abspath(".\\flash.exe")]


def test_resolve_flash_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "flash.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_flash_argv() == [sys.executable, "-m", "flash_cli.main"]


def test_resolve_flash_argv_honors_flash_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like HERMES_BIN lets service managers pin the executable."""
    import shutil
    import flash_cli.kanban_db as kb

    shim = tmp_path / "bin" / "flash"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("HERMES_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_flash_argv() == [str(shim)]


def test_resolve_flash_argv_flash_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare HERMES_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import flash_cli.kanban_db as kb

    cwd_flash = tmp_path / "flash"
    cwd_flash.write_text("wrong\n", encoding="utf-8")
    cwd_flash.chmod(cwd_flash.stat().st_mode | stat.S_IXUSR)
    path_flash = tmp_path / "bin" / "flash"
    path_flash.parent.mkdir()
    path_flash.write_text("right\n", encoding="utf-8")
    path_flash.chmod(path_flash.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_flash.parent))
    monkeypatch.setenv("HERMES_BIN", "flash")

    assert kb._resolve_flash_argv() == [str(path_flash)]


def test_resolve_flash_argv_flash_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare HERMES_BIN does not accept current-directory shadow executables."""
    import sys
    import flash_cli.kanban_db as kb

    (tmp_path / "flash.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_flash_argv_flash_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
...
def test_resolve_flash_argv_flash_bin_unresolved_bare_name_falls_back(monkeypatch):
...
def test_resolve_flash_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m flash_cli.main`.

    Pins the correct module name (NOT `flash` — there is no top-level
    `flash` package). Regression for #23198: the original PR shipped
    `python -m flash` which fails with `No module named flash` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_flash_argv()
    assert argv == [sys.executable, "-m", "flash_cli.main"]


def test_resolve_flash_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `flash_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m flash_cli.main --version`
    ...
    assert "Flash Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="flash_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from flash_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("flash_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from flash_cli.kanban_db import connect, write_txn
    import flash_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from flash_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from flash_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("flash_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("flash_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("flash_cli.kanban_db.os.name", "nt")
    with patch("flash_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("flash_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("flash_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "flash_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("flash_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "flash_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("flash_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("flash_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("flash_cli.kanban_db._record_worker_exit"):
            with patch("flash_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
