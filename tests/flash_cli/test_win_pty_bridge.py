"""Unit tests for flash_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/flash_cli/test_pty_bridge.py.  Drives
...
``flash_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from flash_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from flash_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo flash-ok"])
        try:
            output = _read_until(bridge, b"flash-ok")
            assert b"flash-ok" in output
        finally:
            bridge.close()
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from flash_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from flash_cli.win_pty_bridge import _MAX_COLS, _clamp
