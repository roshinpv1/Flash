"""Tests for the per-profile Projects store (flash_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Flash Agent", folders=["/tmp/flash"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "flash-agent"
    assert proj.name == "Flash Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/flash"
    assert [f.path for f in proj.folders] == ["/tmp/flash"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "flash-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Flash Agent")
    pdb.create_project(conn, name="Flash Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["flash-agent", "flash-agent-2"]
...
def test_db_path_under_flash_home():
