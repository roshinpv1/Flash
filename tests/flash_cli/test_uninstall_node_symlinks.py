"""Tests for flash_cli.uninstall.remove_node_symlinks.
...
only when they still resolve into the Flash-managed node dir.
"""
...
def _make_flash_node(flash_home: Path) -> Path:
    """Create a fake $HERMES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = flash_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_flash_node(fake_home):
    flash_home = fake_home / ".flash"
    node_bin = _make_flash_node(flash_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(flash_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(flash_home) == []


def test_removes_dangling_symlink_into_flash_node(fake_home):
    """A link into the Flash node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Flash links that exist; ignores the ones that don't."""
    flash_home = fake_home / ".flash"
    node_bin = _make_flash_node(flash_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Flash-managed; node is a real user binary.
