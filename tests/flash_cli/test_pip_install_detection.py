...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("flash_cli.config.get_managed_system", return_value=None), \
         patch("flash_cli.config.get_flash_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("flash_cli.config.get_managed_system", return_value="NixOS"), \
         patch("flash_cli.config.get_flash_home", return_value=tmp_path):
        from flash_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    host install is correctly identified as 'git' and 'flash update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("flash_cli.config.get_managed_system", return_value=None), \
         patch("flash_cli.config.get_flash_home", return_value=home):
        from flash_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    with patch("flash_cli.config.get_managed_system", return_value=None), \
         patch("flash_cli.config.get_flash_home", return_value=home), \
         patch("flash_cli.config._running_in_container", return_value=False):
        from flash_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("flash_cli.config.get_managed_system", return_value=None), \
         patch("flash_cli.config.get_flash_home", return_value=home), \
         patch("flash_cli.config._running_in_container", return_value=True):
        from flash_cli.config import detect_install_method
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("flash_cli.config.get_flash_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``flash update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("flash_cli.config.get_managed_system", return_value=None), \
         patch("flash_cli.config.get_flash_home", return_value=tmp_path), \
         patch("flash_constants.is_container", return_value=True):
        from flash_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from flash_cli import banner

    hh = tmp_path / ".flash"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("flash_cli.config.get_flash_home", return_value=hh), \
         patch("flash_constants.get_flash_home", return_value=hh):
        buf = io.StringIO()
