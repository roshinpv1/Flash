"""Tests for the multi-board kanban layer (``flash kanban boards …``).
...
* CLI surface: ``flash kanban boards list/create/switch/rm``.
...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset flash_constants cache so get_default_flash_root() re-reads.
    try:
        import flash_constants
        flash_constants._cached_default_flash_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "flash-agent", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``flash kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "flash_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
