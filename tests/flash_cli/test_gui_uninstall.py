"""Tests for flash_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import flash_cli.gui_uninstall as gu


def _make_agent(flash_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = flash_home / "flash-agent"
    (agent_root / "flash_cli").mkdir(parents=True)
    (agent_root / "flash_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(flash_home: Path) -> None:
    """Create the source-built GUI artifacts a `flash desktop` run produces."""
    desktop = flash_home / "flash-agent" / "apps" / "desktop"
    ...
    (flash_home / "flash-agent" / "node_modules").mkdir(parents=True)
    (flash_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(flash_home: Path) -> None:
    (flash_home / "config.yaml").write_text("x: 1\n")
    (flash_home / ".env").write_text("KEY=secret\n")
    (flash_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    flash_home = tmp_path / ".flash"
    flash_home.mkdir()
    assert gu.agent_is_installed(flash_home) is False
    _make_agent(flash_home)
    assert gu.agent_is_installed(flash_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    flash_home = tmp_path / ".flash"
    (flash_home / "flash-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(flash_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    flash_home = tmp_path / ".flash"
    _make_gui_build(flash_home)
    artifacts = gu.source_built_gui_artifacts(flash_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(flash_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(flash_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    flash_home = tmp_path / ".flash"
    agent_root = _make_agent(flash_home)
    _make_gui_build(flash_home)
    _make_user_data(flash_home)
    ...
    removed = gu.uninstall_gui(flash_home)
    ...
    assert not (flash_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "flash_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (flash_home / "config.yaml").exists()
    assert (flash_home / ".env").exists()
    assert (flash_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    flash_home = tmp_path / ".flash"
    _make_agent(flash_home)
    userdata = tmp_path / "Flash-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(flash_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(flash_home)
    ...
    assert summary["flash_home"] == str(flash_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Flash"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Flash"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Flash"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Flash"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    """
    import flash_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_flash_home", lambda: flash_home)
    ...
    from flash_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    flash_home = tmp_path / ".flash"
    (flash_home / "flash-agent" / "flash_cli").mkdir(parents=True)
    (flash_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not flash_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m flash_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_flash_home", lambda: flash_home)
    ...
    assert not (desktop / "release").exists()
    assert not (flash_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "flash_cli").exists()
    assert (flash_home / "config.yaml").exists()
