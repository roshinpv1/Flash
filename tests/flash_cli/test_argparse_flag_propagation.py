...
subparser, placing the flag BEFORE the subcommand (e.g. 'flash --yolo chat')
...
def _build_parser():
    """Build the flash argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("flash_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "flash_cli.banner", fake_banner)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `flash gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: flash_cli.main is a very heavy
    # import (previously 11 separate `python -m flash_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `flash <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `flash -t web chat` silently dropped the
    ...
    Uses the real `flash_cli._parser.build_top_level_parser()` rather than
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`flash {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `flash --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`flash {flag} chat` lost the flag — got "
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `flash <flag> <value> chat`:\n  "
