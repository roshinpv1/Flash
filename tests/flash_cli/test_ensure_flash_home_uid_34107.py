"""Regression tests for #34107 — Docker UID/GID handling in ensure_flash_home.

When Flash runs in Docker with ``HERMES_UID=1000`` / ``HERMES_GID=911``,
...
The fix is a ``_chown_to_flash_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_flash_uid_gid
# ---------------------------------------------------------------------------


class TestResolveFlashUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from flash_cli.config import _resolve_flash_uid_gid
...
class TestChownToFlashUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_flash_uid(d)  # must not raise
