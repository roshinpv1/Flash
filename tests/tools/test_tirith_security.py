...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._flash_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(flash_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(flash_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_sync_resolve_persists_failure(self, mock_which, mock_install,
                                            mock_disk_check, mock_mark):
        """Synchroflash _resolve_tirith_path persists failure to disk."""
        from tools.tirith_security import _resolve_tirith_path
        _tirith_mod._resolved_path = None
    ...
    def test_install_failed_recovers_from_flash_bin(self):
        """After _INSTALL_FAILED, manual install in HERMES_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        flash_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(flash_bin, "w") as f:
            f.write("#!/bin/sh\n")
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._flash_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == flash_bin
            assert _tirith_mod._resolved_path == flash_bin
            mock_clear.assert_called_once()
...
class TestFlashHomeIsolation:
    def test_flash_bin_dir_respects_flash_home(self):
        """_flash_bin_dir must use HERMES_HOME, not hardcoded ~/.flash."""
        from tools.tirith_security import _flash_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"HERMES_HOME": tmpdir}):
            result = _flash_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_flash_home(self):
        """_failure_marker_path must use HERMES_HOME, not hardcoded ~/.flash."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"HERMES_HOME": "/custom/flash"}):
            result = _failure_marker_path()
        assert result == "/custom/flash/.tirith-install-failed"
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        flash_home = os.getenv("HERMES_HOME")
        assert flash_home is not None, "HERMES_HOME should be set by conftest"
        assert "flash_test" in flash_home, "Should point to test temp dir"

    def test_get_flash_home_fallback(self):
        """Without HERMES_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_flash_home
        with patch.dict(os.environ, {}, clear=True):
            ...
            expected = os.path.join(os.path.expanduser("~"), ".flash")
            result = _get_flash_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._flash_bin_dir",
