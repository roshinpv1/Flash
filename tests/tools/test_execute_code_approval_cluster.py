...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad NYXO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: NYXO_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_nyxo_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "NYXO_HOME": "/h", "NYXO_PROFILE": "p",
        "NYXO_CONFIG": "/c.yaml", "NYXO_ENV": "/e",
        # other NYXO_* → dropped (broad prefix removed)
        "NYXO_BASE_URL": "https://x", "NYXO_INTERACTIVE": "1",
        "NYXO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("NYXO_HOME", "NYXO_PROFILE", "NYXO_CONFIG", "NYXO_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "NYXO_BASE_URL", "NYXO_INTERACTIVE", "NYXO_KANBAN_DB",
...
def test_env_scrub_logs_dropped_nyxo_vars(caplog):
    """Dropping a non-allowlisted, non-secret NYXO_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset NYXO_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "NYXO_HOME": "/h",          # allowlisted → kept, not logged
        "NYXO_BASE_URL": "https://x",   # dropped → logged
        "NYXO_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "NYXO_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "NYXO_HOME" in out and "PATH" in out
    assert "NYXO_BASE_URL" not in out and "NYXO_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "NYXO_BASE_URL" in msgs and "NYXO_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "NYXO_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped NYXO_* vars."""
    import logging
