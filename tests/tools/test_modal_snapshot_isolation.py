...
def _restore_tool_modules():
    original_nyxo_home = os.environ.get("NYXO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_nyxo_home is None:
            os.environ.pop("NYXO_HOME", None)
        else:
            os.environ["NYXO_HOME"] = original_nyxo_home
        _reset_modules(("tools", "nyxo_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    nyxo_cli = types.ModuleType("nyxo_cli")
    nyxo_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["nyxo_cli"] = nyxo_cli
    nyxo_home = tmp_path / "nyxo-home"
    os.environ["NYXO_HOME"] = str(nyxo_home)
    sys.modules["nyxo_cli.config"] = types.SimpleNamespace(
        get_nyxo_home=lambda: nyxo_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="nyxo-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": nyxo_home / "modal_snapshots.json",
