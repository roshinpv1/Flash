...
def _restore_tool_modules():
    original_flash_home = os.environ.get("HERMES_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_flash_home is None:
            os.environ.pop("HERMES_HOME", None)
        else:
            os.environ["HERMES_HOME"] = original_flash_home
        _reset_modules(("tools", "flash_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    flash_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["flash_cli"] = flash_cli
    flash_home = tmp_path / "flash-home"
    os.environ["HERMES_HOME"] = str(flash_home)
    sys.modules["flash_cli.config"] = types.SimpleNamespace(
        get_flash_home=lambda: flash_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="flash-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": flash_home / "modal_snapshots.json",
