...
class TestScanMemoryContent:
    ...
    def test_flash_config_mod_blocked(self):
        result = _scan_memory_content("edit .flash/config.yaml to change settings")
        assert "Blocked" in result
        assert "flash_config_mod" in result
        result = _scan_memory_content("update .flash/SOUL.md with new personality")
        assert "Blocked" in result
        assert "flash_config_mod" in result
    ...
    def test_flash_config_mod_no_false_positives(self):
        """Merely mentioning flash config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .flash/config.yaml for settings") is None
        assert _scan_memory_content("Read .flash/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .flash/config.yaml file contains runtime options") is None
