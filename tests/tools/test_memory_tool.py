...
class TestScanMemoryContent:
    ...
    def test_nyxo_config_mod_blocked(self):
        result = _scan_memory_content("edit .nyxo/config.yaml to change settings")
        assert "Blocked" in result
        assert "nyxo_config_mod" in result
        result = _scan_memory_content("update .nyxo/SOUL.md with new personality")
        assert "Blocked" in result
        assert "nyxo_config_mod" in result
    ...
    def test_nyxo_config_mod_no_false_positives(self):
        """Merely mentioning nyxo config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .nyxo/config.yaml for settings") is None
        assert _scan_memory_content("Read .nyxo/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .nyxo/config.yaml file contains runtime options") is None
