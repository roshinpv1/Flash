...
Tests cover:
- managed_flash_tools_enabled() subscription-based gate
...
# managed_flash_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedFlashToolsEnabled:
    """Subscription-based gate: True for paid Flashsubscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "flash_cli.flash_account.get_flash_portal_account_info",
            lambda: FlashPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_flash_tools_enabled() is False
    ...
    def test_enabled_for_paid_subscriber(self, monkeypatch):
        ...
        assert managed_flash_tools_enabled() is True
    ...
    def test_force_fresh_is_forwarded(self, monkeypatch):
        ...
        assert managed_flash_tools_enabled(force_fresh=True) is True
        assert calls == [True]
...
class TestFlashToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "flash_cli.flash_account.get_flash_portal_account_info",
            lambda force_fresh=False: FlashPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
                paid_service_access=False,
        ...
        message = flash_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, flash_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_flash_tools_enabled",
            lambda: flash_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, flash_enabled=True)
        assert result["selected_backend"] == "managed"

    def test_auto_falls_back_to_direct(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=False, flash_enabled=True)
    ...
    def test_auto_managed_ready_but_flash_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, flash_enabled=False)
        assert result["selected_backend"] == "direct"

    def test_auto_nothing_when_only_managed_and_flash_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=False, managed_ready=True, flash_enabled=False)
        assert result["selected_backend"] is None

    # --- direct mode ---

    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, flash_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, flash_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---

    def test_managed_selects_managed_when_ready_and_enabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, flash_enabled=True)
        assert result["selected_backend"] == "managed"

    def test_managed_none_when_not_ready(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=False, flash_enabled=True)
        assert result["selected_backend"] is None

    def test_managed_blocked_when_flash_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, flash_enabled=False)
        assert result["selected_backend"] is None
