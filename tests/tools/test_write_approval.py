...
and the shared slash-command handlers (flash_cli/write_approval_commands.py).
...
@pytest.fixture
def flash_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="flash_wa_test_")
    home = os.path.join(d, ".flash")
    os.makedirs(home)
...
def test_default_gate_is_off(flash_home):
...
def test_invalid_subsystem_is_off(flash_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(flash_home):
...
def test_memory_gate_on_no_interactive_stages(flash_home):
...
def test_memory_gate_on_then_apply(flash_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(flash_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(flash_home, monkeypatch):
    ...
    monkeypatch.setattr("flash_cli.config.load_config", _boom)
...
def test_skill_gate_off_allows_create(flash_home):
...
def test_skill_gate_on_always_stages(flash_home):
...
def test_skill_gate_on_then_apply_writes_file(flash_home):
...
def test_skill_create_diff_is_full_content(flash_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(flash_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(flash_home):
...
def test_handle_approve_all(flash_home):
    from flash_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(flash_home):
    from flash_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(flash_home):
    from flash_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(flash_home):
    from flash_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(flash_home):
...
def test_handle_approval_invalid(flash_home):
    from flash_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(flash_home):
    from flash_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(flash_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(flash_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(flash_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(flash_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(flash_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(flash_home):
