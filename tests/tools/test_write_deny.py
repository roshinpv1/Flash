...
class TestWriteDenyExactPaths:
    ...
    def test_flash_env(self):
        # ``.env`` under the active HERMES_HOME (profile-aware, not just
        # ``~/.flash``) must be write-denied. The hermetic test conftest
        # points HERMES_HOME at a tempdir — resolve via get_flash_home()
        # to match the denylist.
        from flash_constants import get_flash_home
        path = str(get_flash_home() / ".env")
        assert _is_write_denied(path) is True

    def test_flash_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.flash/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "flash_root"
        ...
        from flash_constants import get_flash_home, get_default_flash_root
        assert get_flash_home() == profile_home
        assert get_default_flash_root() == root
...
class TestWriteAllowed:
    ...
    def test_flash_control_files_requested_writable(self):
        from flash_constants import get_flash_home
