...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, flash_subprocess_env
...
# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (flash_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_flash_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """flash_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
...
def test_flash_subprocess_env_bound_contextvar_wins(monkeypatch):
...
def test_flash_subprocess_env_unengaged_preserves_fallback(monkeypatch):
