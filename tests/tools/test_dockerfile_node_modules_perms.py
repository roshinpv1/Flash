...
Older images made /opt/flash/ui-tui and /opt/flash/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/flash/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R flash:flash /opt/flash/ui-tui" not in text
    assert "chown -R flash:flash /opt/flash/node_modules" not in text
