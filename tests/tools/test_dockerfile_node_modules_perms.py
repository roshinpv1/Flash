...
Older images made /opt/nyxo/ui-tui and /opt/nyxo/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV NYXO_TUI_DIR=/opt/nyxo/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R nyxo:nyxo /opt/nyxo/ui-tui" not in text
    assert "chown -R nyxo:nyxo /opt/nyxo/node_modules" not in text
