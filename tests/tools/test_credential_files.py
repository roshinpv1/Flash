...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        flash_home.mkdir()
        (flash_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(flash_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.flash/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        flash_home = tmp_path / ".flash"
        flash_home.mkdir()
        (flash_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        flash_home = tmp_path / ".flash"
        flash_home.mkdir()
        (flash_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        flash_home = tmp_path / ".flash"
        flash_home.mkdir()
        (flash_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.flash/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        flash_home = tmp_path / ".flash"
        (flash_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"HERMES_HOME": str(flash_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.flash")

        assert mounts[0]["container_path"] == "/home/user/.flash/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.flash/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.flash/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above flash_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside flash_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        ...
        flash_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(flash_home))
        (flash_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_flash_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of HERMES_HOME must be allowed."""
        flash_home = tmp_path / ".flash"
        flash_home.mkdir()
        subdir = flash_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside flash_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside flash_home pointing outside
        symlink = flash_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, flash_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(flash_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(flash_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (flash_home / "oauth.json").write_text("{}")
        self._write_config(flash_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        flash_home = tmp_path / ".flash"
        flash_home.mkdir()
        (flash_home / "cache" / "documents").mkdir(parents=True)
        (flash_home / "cache" / "audio").mkdir(parents=True)
        (flash_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(flash_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.flash/cache/documents" in paths
        assert "/root/.flash/cache/audio" in paths
        assert "/root/.flash/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.flash/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_flash_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        flash_home = tmp_path / ".flash"
        flash_home.mkdir()
        # Use legacy dir name with content — get_flash_dir prefers
        # populated old over new.
        legacy_doc = flash_home / "document_cache"
        legacy_img = flash_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(flash_home / "document_cache") in host_paths
        assert str(flash_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.flash/cache/documents" in container_paths
        assert "/root/.flash/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        flash_home = tmp_path / ".flash"
        img_dir = flash_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.flash/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.flash")
            == "/home/agent/.flash/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        flash_home = tmp_path / ".flash"
        (flash_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(flash_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(flash_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        flash_home = tmp_path / ".flash"
        doc_dir = flash_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        flash_home = tmp_path / ".flash"
        ss_dir = flash_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.flash/cache/screenshots/session_abc/screen1.png"
