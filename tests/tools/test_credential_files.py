...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        nyxo_home.mkdir()
        (nyxo_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(nyxo_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.nyxo/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        (nyxo_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        (nyxo_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        (nyxo_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.nyxo/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        nyxo_home = tmp_path / ".nyxo"
        (nyxo_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"NYXO_HOME": str(nyxo_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.nyxo")

        assert mounts[0]["container_path"] == "/home/user/.nyxo/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.nyxo/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.nyxo/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape NYXO_HOME."""
        monkeypatch.setenv("NYXO_HOME", str(tmp_path / ".nyxo"))
        (tmp_path / ".nyxo").mkdir()

        # Create a sensitive file one level above nyxo_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside nyxo_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside NYXO_HOME must still be registered."""
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
        (nyxo_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_nyxo_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of NYXO_HOME must be allowed."""
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        subdir = nyxo_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside NYXO_HOME pointing outside must be rejected."""
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))

        # Create a sensitive file outside nyxo_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside nyxo_home pointing outside
        symlink = nyxo_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes NYXO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, nyxo_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape NYXO_HOME."""
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
        ...
        sensitive.write_text("{}")
        self._write_config(nyxo_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(nyxo_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside NYXO_HOME via config must still mount."""
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))

        (nyxo_home / "oauth.json").write_text("{}")
        self._write_config(nyxo_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        (nyxo_home / "cache" / "documents").mkdir(parents=True)
        (nyxo_home / "cache" / "audio").mkdir(parents=True)
        (nyxo_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.nyxo/cache/documents" in paths
        assert "/root/.nyxo/cache/audio" in paths
        assert "/root/.nyxo/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.nyxo/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        nyxo_home = tmp_path / ".nyxo"
        nyxo_home.mkdir()
        # Use legacy dir name — get_nyxo_dir prefers old if it exists
        (nyxo_home / "document_cache").mkdir()
        (nyxo_home / "image_cache").mkdir()
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(nyxo_home / "document_cache") in host_paths
        assert str(nyxo_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.nyxo/cache/documents" in container_paths
        assert "/root/.nyxo/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        nyxo_home = tmp_path / ".nyxo"
        img_dir = nyxo_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.nyxo/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.nyxo")
            == "/home/agent/.nyxo/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        nyxo_home = tmp_path / ".nyxo"
        (nyxo_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(nyxo_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        nyxo_home = tmp_path / ".nyxo"
        doc_dir = nyxo_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        nyxo_home = tmp_path / ".nyxo"
        ss_dir = nyxo_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.nyxo/cache/screenshots/session_abc/screen1.png"
