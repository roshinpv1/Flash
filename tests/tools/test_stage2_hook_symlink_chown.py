...
def _chown_flash_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    *,
    ...
    flash_home = target if flash_home is None else flash_home
    script = (
        "set -eu\n"
        f'HERMES_HOME="{flash_home}"\n'
        f"{_chown_flash_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_flash_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "flash-home"
    try:
...
def test_stage2_uses_symlink_safe_helper_for_flash_home_trees(stage2_text: str) -> None:
    assert 'chown_flash_tree "$HERMES_HOME/$sub"' in stage2_text
    assert 'chown_flash_tree "$HERMES_HOME/profiles"' in stage2_text
    assert 'chown_flash_tree "$HERMES_HOME/cron"' in stage2_text
    assert 'chown -R flash:flash "$HERMES_HOME/$sub"' not in stage2_text
    assert 'chown -R flash:flash "$HERMES_HOME/profiles"' not in stage2_text
    assert 'chown -R flash:flash "$HERMES_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_flash_home(
    stage2_text: str,
) -> None:
