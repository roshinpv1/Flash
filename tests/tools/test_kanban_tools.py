...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `flash chat` sessions (no HERMES_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("flash-cli")), quiet=True)
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["flash_cli/kanban.py"],
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    under an authoritative-looking name like ``flash-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "flash-system",
    })
    ...
    try:
        ...
        # caller-supplied "flash-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
# restarting Flash. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Flash home with two distinct kanban boards seeded.
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.flash; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".flash"
