"""Contract tests for the Docker image's immutable /opt/flash install tree."""
...
def test_dockerfile_makes_opt_flash_readonly_for_flash_user() -> None:
    ...
    assert "chown -R root:root /opt/flash" not in text
    assert "chmod -R a+rX /opt/flash" not in text
    assert "chmod -R a-w /opt/flash" not in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "HERMES_TUI_DIR=/opt/flash/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_flash() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+flash:flash\s+/opt/flash/\.venv",
        r"chown\s+-R\s+flash:flash\s+/opt/flash/ui-tui",
        r"chown\s+-R\s+flash:flash\s+/opt/flash/gateway",
        r"chown\s+-R\s+flash:flash\s+/opt/flash/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/flash must stay immutable; "
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/flash tree.
    ...
    assert "ENV HERMES_LAZY_INSTALL_TARGET=/opt/flash" not in text
    ...
    # succeed as the unprivileged flash runtime user.
