...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_flash_cli_expands_web(self):
        """flash-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"flash-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("flash-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['flash-cli'] yields ['web']."""
        parent_toolsets = {"flash-cli"}
