...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Flash-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (FlashOrg/flash-agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (FlashOrg/flash-agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (FlashOrg/flash-agent#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Flash patch.
    ...
    def test_future_renamed_subcommand_is_honored(self):
        ...
        instead of `mcp` keeps working without a Flash patch."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (FlashOrg/flash-agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (FlashOrg/flash-agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (FlashOrg/flash-agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Flash never declared a cua-driver
    ...
    def test_session_id_format(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        backend = CuaDriverBackend()
        # flash-{12 hex chars} — short enough to surface in logs
        # without being a privacy hazard, unique enough for concurrent runs.
        assert backend._session_id.startswith("flash-")
    ...
    def test_session_id_unique_per_backend(self):
        ...
        assert a != b, "each Flash run should mint its own session id"
