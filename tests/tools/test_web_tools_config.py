...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_nyxo_home_override(self, tmp_path):
        """Auth lookup should read from NYXO_HOME/auth.json, not ~/.nyxo/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".nyxo").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``nyxo tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "nyxo_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
