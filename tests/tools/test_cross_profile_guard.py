...
def fake_flash(tmp_path, monkeypatch):
    """Build a two-profile Flash layout and point HERMES_HOME at
    the flash-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setattr(fs, "_flash_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_flash):
        from tools.file_tools import write_file_tool
        target = fake_flash["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_flash):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_flash["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "flash-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_flash):
    ...
    def test_non_flash_path_unaffected(self, fake_flash, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_flash):
    ...
    def test_cross_profile_patch_bypass(self, fake_flash):
        from tools.file_tools import patch_tool
        target = fake_flash["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_flash):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'flash-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_flash, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_flash["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_flash["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.flash/profiles/<name>/."""
        # Don't set HERMES_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_flash_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_flash_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_flash):
        """When active profile is 'flash-security', the prompt warns
        ...
        assert "Active Flash profile" in src
        assert "cross_profile=True" in src
        assert "~/.flash/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Flash profile: default" in src
        assert "Active Flash profile: {active_profile}" in src
