"""Regression guard: a genuine Flash429 must re-enter the retry loop so the
top-of-loop Flashrate-limit guard can activate the fallback chain.
...
class TestGenuineFlash429ReentersLoop:
...
class TestSourceUsesReentrantAssignment:
    ...
    form in the genuine-Flash-429 branch.  Protects against an accidental
    ...
    def test_genuine_branch_does_not_skip_to_max_retries(self):
        ...
        assert match is not None, (
            "genuine-Flash-429 branch not found in conversation_loop — "
            "update this test if the branch was refactored"
        )
        block = match.group(0)
        assert "retry_count = max(0, max_retries - 1)" in block, (
            "genuine-Flash-429 branch must re-enter the retry loop "
            "(retry_count = max(0, max_retries - 1)); "
