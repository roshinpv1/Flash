...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_flash_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "flash",
...
class TestBuildApiKwargsNousPortal:
    def test_includes_flash_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == flash_portal_tags()
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_flash_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_flash_auth", return_value=flash_auth), \
