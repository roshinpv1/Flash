...
(SQLITE_PROTOCOL — typical on NFS/SMB), Flash must fall back to
...
def _reset_last_init_error():
    ...
    flash_state._set_last_init_error(None)
    yield
    flash_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    flash_state._wal_fallback_warned_paths.clear()
    yield
    flash_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see flash_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        flash_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("flash_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        flash_state._set_last_init_error("OperationalError: generic SQLite error")
