"""Tests for flash_constants module."""
...
from flash_constants import (
    ...
    find_flash_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_flash_root,
    get_flash_dir,
    get_flash_home,
    heal_flash_managed_node,
    flash_managed_node_tree_present,
    iter_flash_node_dirs,
    is_container,
    node_tool_runnable,
    parse_reasoning_effort,
    secure_parent_dir,
    with_flash_node_path,
)


class TestGetDefaultFlashRoot:
    """Tests for get_default_flash_root() — Docker/custom deployment awareness."""

    def test_no_flash_home_returns_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME is not set, returns ~/.flash."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_flash_root() == tmp_path / ".flash"

    def test_flash_home_is_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME = ~/.flash, returns ~/.flash."""
        native = tmp_path / ".flash"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(native))
        assert get_default_flash_root() == native

    def test_flash_home_is_profile(self, tmp_path, monkeypatch):
        """When HERMES_HOME is a profile under ~/.flash, returns ~/.flash."""
        native = tmp_path / ".flash"
    ...
    def test_flash_home_is_docker(self, tmp_path, monkeypatch):
        """When HERMES_HOME points outside ~/.flash (Docker), returns HERMES_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
        assert get_default_flash_root() == docker_home

    def test_flash_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any HERMES_HOME outside ~/.flash is treated as the root."""
        custom = tmp_path / "my-flash-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(custom))
        assert get_default_flash_root() == custom
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(profile))
        assert get_default_flash_root() == docker_root

    def test_no_flash_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\flash, not ~/.flash."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(flash_constants.sys, "platform", "win32")

        assert get_default_flash_root() == local_appdata / "flash"

    def test_no_flash_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/flash without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        ...
        assert get_default_flash_root() == home / "AppData" / "Local" / "flash"


class TestGetFlashHome:
    """Tests for get_flash_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When HERMES_HOME is unset on Windows, use %LOCALAPPDATA%\\flash."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(flash_constants.sys, "platform", "win32")
        monkeypatch.setattr(flash_constants, "_profile_fallback_warned", False)

        assert get_flash_home() == local_appdata / "flash"


class TestFlashManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_flash_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(flash_constants.sys, "platform", "win32")
        monkeypatch.setenv("HERMES_HOME", str(home))
        monkeypatch.setattr(flash_constants, "node_tool_runnable", lambda path: True)

        assert find_flash_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(flash_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(flash_constants, "heal_flash_managed_node", lambda: False)
        monkeypatch.setattr(
            flash_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert flash_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_flash_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "flash"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_flash_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Flash-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(flash_constants, "heal_flash_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_flash_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(flash_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(flash_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".flash"
...
class TestGetFlashDir:
    """Tests for ``get_flash_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_flash_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_flash_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_flash_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_flash_dir("cache/audio", "audio_cache")
