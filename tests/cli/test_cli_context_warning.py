...
def _isolate(tmp_path, monkeypatch):
    """Isolate NYXO_HOME so tests don't touch real config."""
    home = tmp_path / ".nyxo"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal NyxoCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = NyxoCLI.__new__(NyxoCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["nyxo-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Nyxo' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Nyxo' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Nyxo' minimum context length."""
