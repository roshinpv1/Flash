...
def cli_obj(_isolate):
    """Create a minimal FlashCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = FlashCLI.__new__(FlashCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["flash-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Flash' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Flash' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Flash' minimum context length."""
