...
def session_db(tmp_path):
    ...
    os.environ["HERMES_HOME"] = str(tmp_path / ".flash")
    os.makedirs(tmp_path / ".flash", exist_ok=True)
    from flash_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".flash" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal FlashCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        FlashCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        FlashCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_branch_rotates_flash_session_id_env_and_context(self, cli_instance, session_db):
