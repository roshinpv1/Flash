...
Every ``docker exec`` here runs as the unprivileged ``flash`` user
...
def test_dashboard_not_running_by_default(
    ...
    r = docker_exec(container_name, "pgrep", "-f", "flash dashboard")
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `flash doctor` and any other health check).
...
def test_dashboard_opt_in_starts(
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'flash dashboard'", deadline_s=30.0,
    )
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "flash dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as flash, so the
    # flash user can kill it (same UID).
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled flash
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``flash`` in the
    ...
    assert "flash" in provider_names, (
        "Bundled dashboard_auth/flash provider should register when "
