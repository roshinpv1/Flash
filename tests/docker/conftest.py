...
IMAGE_TAG = os.environ.get("HERMES_TEST_IMAGE", "flash-agent-harness:latest")
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged flash user
# ---------------------------------------------------------------------------
#
# Background: every Flash runtime path inside the container drops to UID
# 10000 (the ``flash`` user) via ``s6-setuidgid flash``. ``docker exec``
...
# fails (PermissionError swallowed) for flash, so a test that ran as root
...
# helpers below run every probe under ``-u flash`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: flash).
