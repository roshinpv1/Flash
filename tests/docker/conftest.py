...
Override the image with ``NYXO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("NYXO_TEST_IMAGE", "nyxo-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``NYXO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("NYXO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged nyxo user
# ---------------------------------------------------------------------------
#
# Background: every Nyxo runtime path inside the container drops to UID
# 10000 (the ``nyxo`` user) via ``s6-setuidgid nyxo``. ``docker exec``
...
# fails (PermissionError swallowed) for nyxo, so a test that ran as root
...
# helpers below run every probe under ``-u nyxo`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: nyxo).
