...
is migrated by docker_config_migrate.py on boot, running as the flash
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    docker_config_migrate.py, running as the flash user."""
    # Start container
    ...
    r = docker_exec_sh(
        container_name,
        "test -f /opt/flash/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by flash (migration ran as flash)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "flash", (
        f"config.yaml not owned by flash (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
