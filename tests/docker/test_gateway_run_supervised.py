...
Before the s6 migration, ``docker run nousresearch/nyxo-agent gateway
...
crash and a supervised dashboard alongside (when ``NYXO_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``NYXO_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``NYXO_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``nyxo`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``nyxo gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "NYXO_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The NYXO_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``nyxo gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``nyxo gateway
    ...
    # Count python processes running `nyxo gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*nyxo.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `nyxo gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-nyxo`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``NYXO_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${NYXO_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Nyxo Gateway Starting…"
    ...
    assert "⚕" in combined or "Nyxo Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Nyxo Gateway Starting" in file_contents, (
