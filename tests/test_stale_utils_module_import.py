...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.flash/flash-agent/utils.py)
...
2. ``flash update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``flash_cli/main.py`` (the ``flash update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``flash_constants`` after the
