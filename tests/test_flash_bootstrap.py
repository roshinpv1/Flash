"""Tests for flash_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Flash entry point
(flash, flash-agent, flash-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Flash entry point imports flash_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported flash_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("flash_bootstrap", None)
    import flash_bootstrap  # noqa: WPS433
    return flash_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Flash CLI
...
class TestEntryPointsImportBootstrap:
    """Every Flash entry point must import flash_bootstrap as its
    ...
    # Entry points that invoke Flash as a process.  Each one must
    # import flash_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "flash_cli/main.py",   # flash CLI (console_script)
        "run_agent.py",          # flash-agent (console_script)
        "acp_adapter/entry.py",  # flash-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import flash_bootstrap' and that
        ...
        half-finished ``flash update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``flash_bootstrap`` as a top-level module) leaves flash
        ...
        """
        # Resolve relative to the flash-agent repo root.  Tests live
        # at tests/test_flash_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # flash start even when flash_bootstrap hasn't been
        ...
        assert first_import_name == "flash_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'flash_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import flash_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Flash's own top-level modules — covering both
    ...
    def _run(self, hb, path_seed, env=None):
        ...
        try:
            ...
            hb.harden_import_path(src_root="/opt/flash")
            return sys.path[:]
        finally:
            sys.path[:] = original
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/flash", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Flash root.  The guard must relocate Flash to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/flash"])
        assert result[0] == "/opt/flash"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Flash root.
        assert result.index("/opt/flash") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/flash", "/opt/flash", ""])
        assert result.count("/opt/flash") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HERMES_PYTHON_SRC_ROOT"] = "/env/flash"
            hb.harden_import_path()
            assert sys.path[0] == "/env/flash"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HERMES_PYTHON_SRC_ROOT", None)
