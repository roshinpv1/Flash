...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate NYXO_HOME under tmp_path."""
    monkeypatch.setenv("NYXO_HOME", str(tmp_path / ".nyxo"))
    (tmp_path / ".nyxo").mkdir()

    # Force the constants/image cache helpers to re-read NYXO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("nyxo_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_nyxo_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under NYXO_HOME — gateway cleanup
