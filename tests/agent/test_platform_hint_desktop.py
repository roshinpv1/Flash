...
class TestDesktopHintEntry:
    ...
    def test_desktop_hint_does_not_inherit_tui_cron_local_only_block(self):
        ...
        (flash_cli/web_server.py under HERMES_DESKTOP=1). We deliberately
...
class TestEmbeddedTuiPaneClarifier:
    """When ``HERMES_DESKTOP_TERMINAL=1``, a standalone ``flash --tui`` is
...
class TestContradictionGone:
    ...
    contained both ``You are running in the Flash terminal UI (TUI).`` and
    ``Runtime surface: you're running inside the Flash desktop GUI app.``.
