...
def test_verify_on_stop_auto_uses_flash_platform_override(clear_verify_env):
...
def test_verify_on_stop_default_path_through_load_config(tmp_path, clear_verify_env):
    ...
    # cannot exercise.
    clear_verify_env.setenv("HERMES_HOME", str(tmp_path / ".flash"))
...
def test_ad_hoc_pass_satisfies_no_suite_stop_loop(tmp_path, monkeypatch):
    ...
    (tmp_path / "package.json").write_text("{}", encoding="utf-8")
    changed = str(tmp_path / "src" / "app.ts")
    script = Path(tempfile.gettempdir()) / f"flash-ad-hoc-stop-{tmp_path.name}.py"
