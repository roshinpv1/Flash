...
def test_flash_client_tag_includes_current_version():
    """The client tag must reflect flash_cli.__version__ verbatim."""
    from flash_cli import __version__
    from agent.portal_tags import flash_client_tag

    assert flash_client_tag() == f"client=flash-client-v{__version__}"
...
def test_flash_client_tag_format():
    ...
    assert tag.startswith("client=flash-client-v")
...
def test_flash_portal_tags_contains_product_and_client():
    ...
    assert "product=flash-agent" in tags
    assert flash_client_tag() in tags
    assert len(tags) == 2


def test_flash_portal_tags_returns_fresh_list():
...
def test_flash_provider_profile_uses_helper():
    ...
    assert body["tags"] == flash_portal_tags()
