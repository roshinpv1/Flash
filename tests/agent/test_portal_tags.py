...
def test_nyxo_client_tag_includes_current_version():
    """The client tag must reflect nyxo_cli.__version__ verbatim."""
    from nyxo_cli import __version__
    from agent.portal_tags import nyxo_client_tag

    assert nyxo_client_tag() == f"client=nyxo-client-v{__version__}"


def test_nyxo_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import nyxo_client_tag

    tag = nyxo_client_tag()
    assert tag.startswith("client=nyxo-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import nyxo_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=nyxo-agent" in tags
    assert nyxo_client_tag() in tags
    assert len(tags) == 2
