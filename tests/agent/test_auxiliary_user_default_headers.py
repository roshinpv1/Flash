...
def _isolate(tmp_path, monkeypatch):
    """Redirect NYXO_HOME so load_config() reads our test config.yaml."""
    nyxo_home = tmp_path / ".nyxo"
    nyxo_home.mkdir()
    monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
    (nyxo_home / "config.yaml").write_text("model:\n  default: test-model\n")


def _write_config(tmp_path, config_dict):
    import yaml
    (tmp_path / ".nyxo" / "config.yaml").write_text(yaml.dump(config_dict))
