...
def hermes_home(monkeypatch):
    """Isolate NYXO_HOME for ``reload_skills`` tests.
    ...
    module-level ``NYXO_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="hermes-reload-skills-")
    monkeypatch.setenv("NYXO_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "NYXO_HOME", home, raising=False)
