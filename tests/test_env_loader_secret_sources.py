"""Tests for the secret-source tracking in ``flash_cli.env_loader``.

These cover the small public surface that lets `flash model` / `flash setup`
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_flash_dotenv()`` is called at module-import time from several
    hot modules (cli.py, flash_cli/main.py, run_agent.py, ...).  The
    ...
    # cli.py, flash_cli/main.py, run_agent.py, trajectory_compressor.py,
