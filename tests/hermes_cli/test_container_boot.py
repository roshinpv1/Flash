"""Tests for flash_cli.container_boot — the cont-init.d-time
...
from flash_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running flash containers, those
    ...
    """
    monkeypatch.setattr(
        "flash_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under flash_home/profiles/<name>/."""
    p = flash_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `flash profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    if state is not None:
        (flash_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (flash_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (flash_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        flash_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from flash_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        flash_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    so `flash gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        flash_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    the root profile. It must call `flash gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    ...
    assert "flash gateway run" in run
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/flash/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/flash/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Through s6 /init + the main-wrapper that re-execs `flash`.
        ("/init", "/opt/flash/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
            "/opt/flash/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "127.0.0.1",
            "--no-open",
        ),
        # Wrapper that kept the explicit `flash` argv0.
        ("/init", "/opt/flash/docker/main-wrapper.sh", "flash", "dashboard"),
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from flash_cli.container_boot import _is_dashboard_container
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/flash/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/flash/docker/main-wrapper.sh", "flash", "gateway", "run"),
        ("chat",),
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/flash/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/flash/docker/main-wrapper.sh", "gateway", "run"),
    )
