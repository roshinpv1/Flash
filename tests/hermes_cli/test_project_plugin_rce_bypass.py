...
1. ``flash_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.flash/plugins/`` source via
...
2. ``flash_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.flash/plugins``
...
class TestEndToEndPocBlocked:
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "flash_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "flash_dashboard_plugin_evil" not in sys.modules
