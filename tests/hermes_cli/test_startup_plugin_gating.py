...
``flash_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``flash --help``, ``flash version``,
``flash logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``flash --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from flash_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["flash", "--tui", "chat"], "chat"),
        (["flash", "-w", "logs"], "logs"),
        (["flash", "chat", "hello world"], "chat"),
        (["flash", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["flash", "-m", "gpt5", "chat"], "chat"),
        (["flash", "--model", "gpt5", "chat", "hi"], "chat"),
        (["flash", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["flash", "-z", "hello world"], None),
        (["flash", "-z", "hello", "chat"], "chat"),
        (["flash", "--model=gpt5", "chat"], "chat"),     # inline form
        (["flash", "--", "chat"], "chat"),               # -- terminator
        (["flash", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["flash", "some-plugin-cmd"], "some-plugin-cmd"),
        (["flash", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["flash"],                          # bare → chat
        ["flash", "--help"],                # top-level help
        ["flash", "-h"],
        ["flash", "version"],               # known built-in
        ["flash", "logs"],
        ["flash", "gateway", "run"],
        ["flash", "--tui"],
        ["flash", "-w", "--tui"],
        ["flash", "chat", "hi"],
        ["flash", "help"],                  # accepted built-in-ish
        ["flash", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["flash", "meet", "join"],          # potential google_meet plugin
        ["flash", "honcho", "status"],      # potential memory plugin
        ["flash", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``flash help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"flash_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
