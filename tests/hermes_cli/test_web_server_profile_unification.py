...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_flash_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_flash_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import flash_cli.mcp_config as mcp_config
        from flash_constants import get_flash_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        monkeypatch.setattr(
            "flash_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_flash_home()))
            return None
        ...
        assert resp.json()["flash_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_flash_home(self, isolated_profiles, monkeypatch):
        import flash_cli.web_server as web_server

        monkeypatch.setattr(
            "flash_cli.main._make_tui_argv",
