...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "flash gateway stop",
        "flash  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "HERMES GATEWAY RESTART",           # uppercase
    ])
    def test_flash_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.flash.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.flash.gateway.plist",
        "launchctl stop ai.flash.gateway",
        "systemctl restart flash-gateway",
        "systemctl stop flash-gateway.service",
        "systemctl start flash-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill flash gateway process",
        "pkill -f flash.*gateway",
        "pkill -f gateway.*flash",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "flash cron list",
        "flash update",
        "flash config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `flash gateway start` is benign — starting a gateway from inside a
        ...
        "flash gateway start",
        "flash gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway flash
        # services must not be falsely blocked (the old `.*flash` matched any
        # flash token).
        "launchctl unload ai.flash.update-checker.plist",
        "launchctl restart ai.flash.daemon",
        "systemctl restart flash-meta.service",
        "systemctl restart flash-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_flash_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade flash then run flash gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.flash.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify flash gateway stop/restart refuse when _HERMES_GATEWAY=1."""
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart flash-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart flash-gateway",
        "systemctl --user restart flash-gateway",
        "systemctl stop flash-gateway.service",
        "flash gateway restart",
        "launchctl kickstart gui/501/ai.flash.gateway",
        "pkill -f flash.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart flash-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-flash systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart flash-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart flash-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run flash gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("flash gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeflash gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.flash.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    flash_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run flash gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        result = json.loads(cronjob(
            ...
            prompt="please run flash gateway restart nightly",
        ))
