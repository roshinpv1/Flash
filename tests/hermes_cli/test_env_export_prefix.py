...
hand-rolled parsers — ``flash_cli.config.load_env``,
``flash_cli.main._has_any_provider_configured``, and
...
def test_skills_tool_load_env_strips_export_prefix(tmp_path, monkeypatch):
    ...
    # skills_tool.load_env reads get_flash_home()/.env directly.
    import importlib

    import tools.skills_tool as skills_tool

    importlib.reload(skills_tool)
    with patch.object(skills_tool, "get_flash_home", return_value=tmp_path):
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import flash_cli.main as hmain
    ...
    # load_flash_dotenv may have populated at import/reload time, forcing the
