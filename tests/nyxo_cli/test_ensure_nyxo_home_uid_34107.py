"""Regression tests for #34107 — Docker UID/GID handling in ensure_nyxo_home.

When Nyxo runs in Docker with ``NYXO_UID=1000`` / ``NYXO_GID=911``,
the entrypoint chowns the top-level ``NYXO_HOME`` once at startup. But
...
The fix is a ``_chown_to_nyxo_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_nyxo_uid_gid
# ---------------------------------------------------------------------------


class TestResolveNyxoUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("NYXO_UID", "1000")
        monkeypatch.setenv("NYXO_GID", "911")
        from nyxo_cli.config import _resolve_nyxo_uid_gid
        uid, gid = _resolve_nyxo_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("NYXO_UID", raising=False)
        monkeypatch.delenv("NYXO_GID", raising=False)
        from nyxo_cli.config import _resolve_nyxo_uid_gid
        uid, gid = _resolve_nyxo_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("NYXO_UID", "not-a-number")
        monkeypatch.setenv("NYXO_GID", "911")
        from nyxo_cli.config import _resolve_nyxo_uid_gid
        uid, gid = _resolve_nyxo_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("NYXO_UID", "")
        monkeypatch.setenv("NYXO_GID", "")
        from nyxo_cli.config import _resolve_nyxo_uid_gid
        uid, gid = _resolve_nyxo_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToNyxoUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_nyxo_uid(d)  # must not raise
