...
Covers:
- Defense 1: gateway stop/restart refuse when _NYXO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "NYXO GATEWAY RESTART",           # uppercase
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "systemctl restart nyxo-gateway",
        "systemctl stop nyxo-gateway.service",
        "systemctl start nyxo-gateway",
    ])
...
class TestGatewaySelfTargetingGuard:
    """Verify hermes gateway stop/restart refuse when _NYXO_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_NYXO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _NYXO_GATEWAY=1.

    Issue #37453: systemctl --user restart nyxo-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart nyxo-gateway", force=True
        ))
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _NYXO_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart nyxo-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart nyxo-gateway"]
