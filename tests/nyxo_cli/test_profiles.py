"""Comprehensive tests for nyxo_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and NYXO_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * NYXO_HOME  -> tmp_path/.hermes  (so get_nyxo_home() agrees)
...
class TestValidateProfileName:
    ...
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Nyxo install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_nyxo_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes nyxo-agent, .worktrees,
        ...
        (default_home / "nyxo-agent" / ".git").mkdir(parents=True)
        (default_home / "nyxo-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "nyxo-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "nyxo-agent").exists()
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("nyxo_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("nyxo_cli.profiles._cleanup_gateway_service"), \
             patch("nyxo_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestGetActiveProfileName:
    ...
    def test_default_nyxo_home_returns_default(self, profile_env):
        # NYXO_HOME points to tmp_path/.hermes which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom NYXO_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("NYXO_HOME", str(custom))
        # With Docker-aware roots, a custom NYXO_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("nyxo_cli.profiles.shutil.which", lambda name: "/opt/hermes/bin/hermes")
        from nyxo_cli.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from nyxo_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from nyxo_cli.profiles import remove_wrapper_script
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from nyxo_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from nyxo_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from nyxo_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("nyxo_cli.profiles.check_alias_collision", return_value="skip"):
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("nyxo-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/nyxo-agent", "default/.worktrees", "default/profiles",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultNyxoHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_nyxo_home()."""
    ...
    def test_default_nyxo_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_nyxo_home()
        assert home == tmp_path / ".hermes"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (NYXO_HOME outside ~/.hermes), profiles go under NYXO_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_nyxo_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_nyxo_home() returns NYXO_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (NYXO_HOME under ~/.hermes), profiles root is still ~/.hermes/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under NYXO_HOME."""
        from nyxo_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under NYXO_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from nyxo_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_nyxo_home()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with NYXO_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("NYXO_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_nyxo_home()
