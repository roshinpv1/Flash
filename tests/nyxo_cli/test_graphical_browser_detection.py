"""Tests for `_can_open_graphical_browser()` in nyxo_cli.auth.
...
from nyxo_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("nyxo_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("nyxo_cli.auth.sys.platform", "darwin")
