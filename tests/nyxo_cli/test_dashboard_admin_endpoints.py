...
def _client():
    ...
    # Keep the state DB under the isolated NYXO_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_nyxo_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_nyxo_home):
        ...
        (get_nyxo_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_nyxo_action", fake_spawn_action)
    ...
    def test_enable_platform_reports_restart_failure_after_save(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_nyxo_action", fail_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "nyxo_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("nyxo-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # nyxo-index source advertises availability + featured search.
            idx = _Src("nyxo-index")
        ...
        assert {"official", "github", "nyxo-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nyxo_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/nyxo/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/nyxo/update`` runs ``nyxo update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/nyxo/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/nyxo/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        ...
        monkeypatch.setattr("nyxo_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `nyxo tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_nyxo_action", _fake_spawn)
        r = self.client.post(
