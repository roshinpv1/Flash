...
nyxo_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    cli.py binds ``_nyxo_home = get_nyxo_home()`` at import time (module
    singleton), so monkeypatching NYXO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap NYXO_HOME.)
    """
    import cli
