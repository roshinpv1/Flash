...
files. In Nyxo the equivalent isolation is:
...
argparse → cmd_chat → cli.main() → NyxoCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("NYXO_IGNORE_USER_CONFIG", "NYXO_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``NYXO_IGNORE_USER_CONFIG=1``.
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._nyxo_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("NYXO_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``NyxoCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting NYXO_IGNORE_RULES=1 flips NyxoCLI.ignore_rules True."""
        monkeypatch.setenv("NYXO_IGNORE_RULES", "1")

        # Import NyxoCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of NyxoCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.NyxoCLI)
        # Replicate the exact logic from cli.py NyxoCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("NYXO_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("NYXO_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.NyxoCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``nyxo_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("NYXO_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("NYXO_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("NYXO_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("NYXO_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("NYXO_IGNORE_USER_CONFIG") == "1"
        assert "NYXO_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "NYXO_IGNORE_USER_CONFIG" not in os.environ
        assert "NYXO_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        ...
        import inspect
        import nyxo_cli.main as hm
        src = inspect.getsource(hm)
        assert "NYXO_IGNORE_USER_CONFIG" in src
        assert "NYXO_IGNORE_RULES" in src
