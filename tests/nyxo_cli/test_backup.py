...
def _make_hermes_tree(root: Path) -> None:
    ...
    (root / "nyxo_state.db").write_bytes(b"fake-state")
    ...
    # nyxo-agent repo (should be EXCLUDED)
    (root / "nyxo-agent").mkdir(exist_ok=True)
    (root / "nyxo-agent" / "run_agent.py").write_text("# big file\n")
    (root / "nyxo-agent" / ".git").mkdir()
    (root / "nyxo-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from nyxo_cli.backup import _should_exclude
        assert _should_exclude(Path("nyxo-agent/run_agent.py"))
        assert _should_exclude(Path("nyxo-agent/.git/HEAD"))
    ...
    def test_includes_nested_hermes_agent_in_skills(self):
        """skills/autonomous-ai-agents/nyxo-agent/ must NOT be excluded —
        only the root-level nyxo-agent/ repo is skipped."""
        from nyxo_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/nyxo-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/nyxo-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under NYXO_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NYXO_HOME", str(hermes_home))
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import nyxo_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_excludes_hermes_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include nyxo-agent/ directory."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "nyxo-agent" in n]
            assert agent_files == [], f"nyxo-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under NYXO_HOME must be
    ...
    def test_includes_nested_hermes_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../nyxo-agent/ but NOT root nyxo-agent/."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)

        # Add a nested nyxo-agent directory inside skills (like the real layout)
        nested = hermes_home / "skills" / "autonomous-ai-agents" / "nyxo-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Nyxo Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root nyxo-agent must be excluded
            root_agent = [n for n in names if n.startswith("nyxo-agent/")]
            assert root_agent == [], f"root nyxo-agent leaked: {root_agent}"
            # Nested skill nyxo-agent must be included
            assert "skills/autonomous-ai-agents/nyxo-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/nyxo-agent/sub/item.txt" in names
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside NYXO_HOME."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Nyxo backup."""
        from nyxo_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only nyxo_state.db (old wrong name) is rejected."""
        from nyxo_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["nyxo_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NYXO_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from nyxo_cli.backup import run_backup, run_import
        ...
        monkeypatch.setenv("NYXO_HOME", str(dst_home))
        ...
        # nyxo-agent should NOT be present
        assert not (dst_home / "nyxo-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from nyxo_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from nyxo_cli.backup import _detect_prefix
...
class TestBackupEdgeCases:
    def test_nonexistent_nyxo_home(self, tmp_path, monkeypatch):
        """Backup exits when hermes home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".hermes"
        monkeypatch.setenv("NYXO_HOME", str(fake_home))
    ...
    def test_empty_nyxo_home(self, tmp_path, monkeypatch):
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from nyxo_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake NYXO_HOME with critical state files."""
    ...
    def test_list_snapshots(self, hermes_home):
        from nyxo_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from nyxo_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from nyxo_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from nyxo_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from nyxo_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_manifest_rel_traversal(self, hermes_home):
        ...
        entry rather than restoring outside NYXO_HOME."""
        from nyxo_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped NYXO_HOME: {escape_dst} exists"
        )
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # nyxo-agent repo excluded
        assert not any(n.startswith("nyxo-agent/") for n in names)
    ...
    def test_skips_symlinked_files(self, hermes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside NYXO_HOME."""
        from nyxo_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point NYXO_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("NYXO_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for nyxo_cli.config + nyxo_constants so they pick up NYXO_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("nyxo_cli.config") or mod == "nyxo_constants":
    ...
    def test_backup_flag_creates_backup(self, hermes_home, capsys):
        ...
        from nyxo_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, hermes_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("nyxo_cli.config"):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        from nyxo_cli.backup import create_pre_migration_backup
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Nyxo
        backup — `hermes import` should accept it."""
        from nyxo_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_missing_nyxo_home_returns_none(self, tmp_path):
    ...
    def test_does_not_touch_pre_update_backups(self, hermes_home):
        ...
        from nyxo_cli.backup import create_pre_update_backup, create_pre_migration_backup
...
# location, NOT under NYXO_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------
...
class TestMemoryProviderExternalPaths:
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        # External provider state living OUTSIDE NYXO_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under NYXO_HOME,
        ...
        # External state did NOT leak into NYXO_HOME.
