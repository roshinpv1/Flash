"""Tests for IRC gateway configuration via `nyxo setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Nyxo install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Nyxo install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#nyxo")
            monkeypatch.setenv("IRC_NICKNAME", "nyxo-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `nyxo setup gateway` experience with IRC present."""
