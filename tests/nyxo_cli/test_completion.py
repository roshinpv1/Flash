"""Tests for nyxo_cli/completion.py — shell completion script generation."""
...
from nyxo_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real nyxo structure."""
    p = argparse.ArgumentParser(prog="nyxo")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_nyxo_completion()" in out
        assert "complete -F _nyxo_completion nyxo" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _nyxo nyxo' in out
        assert '_nyxo "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_nyxo_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[nyxo]}} == _nyxo ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c nyxo -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from nyxo_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_nyxo_profiles()" in out
        assert 'profiles_dir="$HOME/.nyxo/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'nyxo profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _nyxo_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_nyxo_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _nyxo_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_nyxo_profiles()" in out
        assert "$HOME/.nyxo/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__nyxo_profiles" in out
        count = out.count("(__nyxo_profiles)")
