...
Older Nyxo desktop app shells (<= 0.15.x) spawn the backend as::
...
died before it became ready and the desktop GUI showed only "Nyxo couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real nyxo_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "nyxo_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
