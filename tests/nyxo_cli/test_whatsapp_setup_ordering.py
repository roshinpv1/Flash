...
Before the fix, ``nyxo whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``nyxo gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    nyxo = home / ".nyxo"
    nyxo.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("NYXO_HOME", str(nyxo))
...
def _env_value(nyxo_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from nyxo_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("nyxo_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``nyxo whatsapp`` with an existing paired session and
