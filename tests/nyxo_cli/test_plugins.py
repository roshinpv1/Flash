"""Tests for the Nyxo plugin system (nyxo_cli.plugins)."""
...
from nyxo_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<nyxo_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<nyxo_home>/plugins/``; we derive
    ``<nyxo_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <NYXO_HOME>/config.yaml.
        # Config is always read from NYXO_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        nyxo_home_str = os.environ.get("NYXO_HOME")
        if nyxo_home_str:
            nyxo_home = Path(nyxo_home_str)
        else:
            nyxo_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.nyxo/plugins/ are discovered."""
        plugins_dir = tmp_path / "nyxo_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("NYXO_HOME", str(tmp_path / "nyxo_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NYXO_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".nyxo" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under nyxo_plugins.<name>."""
        plugins_dir = tmp_path / "nyxo_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("NYXO_HOME", str(tmp_path / "nyxo_test"))

        # Clean up any prior namespace module
        sys.modules.pop("nyxo_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "nyxo_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nyxo_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `nyxo plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("nyxo_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "nyxo_test" / "plugins"
        # Set NYXO_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("nyxo_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("nyxo_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("nyxo_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("nyxo_cli.plugins.PluginContext.dispatch_tool.__module__", "nyxo_cli.plugins"):
...
class TestPluginDebugLogging:
    """NYXO_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("NYXO_PLUGINS_DEBUG", raising=False)
        from nyxo_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With NYXO_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("NYXO_PLUGINS_DEBUG", "1")
        from nyxo_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from NYXO_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """NYXO_HOME at the root resolves to 'default'."""
        home = tmp_path / ".nyxo"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """NYXO_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".nyxo" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".nyxo" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(prof))
        ctx = self._ctx()
