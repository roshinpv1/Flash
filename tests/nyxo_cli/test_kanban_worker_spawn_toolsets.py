...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    NYXO_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_nyxo_argv", lambda: ["nyxo"])
    ...
    assert pid == 4242
    assert captured["env"]["NYXO_HOME"] == str(profile)
    assert captured["env"]["NYXO_KANBAN_TASK"] == "t_spawn_tools"
