...
the SAME provider universe as ``nyxo model`` (the CLI/TUI picker).
...
def test_every_nyxo_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `nyxo model` universe."""
    ...
    assert not missing, (
        "providers shown in `nyxo model` but not configurable in the desktop "
