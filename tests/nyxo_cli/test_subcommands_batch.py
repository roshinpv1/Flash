...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="nyxo")
...
# ── deprecated `nyxo login` fails gracefully, not with argparse error ────
#
# `nyxo login` is a removed command; its handler (`login_command` in
# `nyxo_cli/auth.py`) prints a deprecation notice pointing at `nyxo auth` /
# `nyxo model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `nyxo --help`.
