"""Tests for nyxo_cli configuration management."""
...
class TestGetNyxoHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"NYXO_HOME": "/custom/path"}):
            home = get_nyxo_home()
            assert home == Path("/custom/path")


class TestEnsureNyxoHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from nyxo_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from nyxo_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from nyxo_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="nyxo_cli.config"):
                config = load_config()
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NYXO_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from nyxo_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """NYXO_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).
        ...
        via config.yaml; NYXO_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from nyxo_cli.config import OPTIONAL_ENV_VARS
        assert "NYXO_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``NYXO_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Nyxo
    process on next startup via the dotenv → ``os.environ`` chain in
    ``nyxo_cli/env_loader.py``.
    ...
    def _nyxo_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NYXO_SPOTIFY_CLIENT_ID",
            "NYXO_QWEN_BASE_URL",
            "NYXO_MAX_ITERATIONS",
        ],
    )
    def test_hermes_integration_keys_still_writable(self, allowed_key):
        """``NYXO_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``NYXO_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``NYXO_*``) keep working. The denylist
