"""Tests for nyxo_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("nyxo_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from nyxo_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("nyxo_cli.config.load_config")
    ...
    @patch("nyxo_cli.config.load_config")
    @patch.dict("os.environ", {"NYXO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NYXO_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """NYXO_SESSION_PLATFORM should be used when NYXO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_nyxo_platform_takes_precedence(self, tmp_path, monkeypatch):
        """NYXO_PLATFORM should win over NYXO_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("NYXO_HOME", str(tmp_path))
        monkeypatch.setenv("NYXO_PLATFORM", "telegram")
        monkeypatch.setenv("NYXO_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from nyxo_cli.skills_config import _get_categories
        skills = [
