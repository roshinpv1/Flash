"""Tests for nyxo_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_nyxo_home):
    """Drop the example-dashboard fixture into the per-test NYXO_HOME
    ...
    ``NYXO_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, nyxo-achievements, model providers
    ...
    from nyxo_cli import web_server
    ...
    #      and start serving requests against a torn-down NYXO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Nyxo vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Nyxo env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints NYXO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import nyxo_cli.web_server as ws

        monkeypatch.setenv("NYXO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("NYXO_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nyxo_home):
        """Create a TestClient and isolate the state DB under the test NYXO_HOME."""
        try:
        ...
        import nyxo_state
        from nyxo_constants import get_nyxo_home
        from nyxo_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(nyxo_state, "DEFAULT_DB_PATH", get_nyxo_home() / "state.db")
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        import nyxo_cli.web_server as web_server
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from nyxo_constants import get_nyxo_home
        from nyxo_cli.config import load_config, load_env
        ...
        config_path = get_nyxo_home() / "hindsight" / "config.json"
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("nyxo_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from nyxo_cli.web_server import app
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull nousresearch/nyxo-agent:latest" in data["message"]
        ...
        assert any("docker pull nousresearch/nyxo-agent:latest" in line for line in status_data["lines"])
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from nyxo_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from nyxo_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "nyxo_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Nyxo provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from nyxo_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from nyxo_cli.config import save_env_value
        from nyxo_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://nyxo-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Nyxo Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("NyxoDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "deep_link": "https://t.me/newbot/NyxoSetupBot/hermes_pair123_bot",
                "qr_payload": "https://t.me/newbot/NyxoSetupBot/hermes_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "deep_link": "https://t.me/newbot/NyxoSetupBot/hermes_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/NyxoSetupBot/hermes_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "deep_link": "https://t.me/newbot/NyxoSetupBot/hermes_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/NyxoSetupBot/hermes_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "deep_link": "https://t.me/newbot/NyxoSetupBot/hermes_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/NyxoSetupBot/hermes_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "deep_link": "https://t.me/newbot/NyxoSetupBot/hermes_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/NyxoSetupBot/hermes_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "deep_link": "https://t.me/newbot/NyxoSetupBot/hermes_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/NyxoSetupBot/hermes_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/nyxo_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import nyxo_cli.nous_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from nyxo_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from nyxo_cli.web_server import _parse_model_ids
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from nyxo_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from nyxo_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_nyxo_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from nyxo_constants import get_nyxo_home
        get_nyxo_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from nyxo_constants import get_nyxo_home
        import nyxo_cli.profiles as profiles_mod

        hermes_home = get_nyxo_home()
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_nyxo_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_nyxo_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_nyxo_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_nyxo_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_nyxo_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_nyxo_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_nyxo_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_nyxo_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_nyxo_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_nyxo_home() / "profiles" / "builder"
        token = set_nyxo_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_nyxo_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import nyxo_cli.skills_config as skills_config
        import nyxo_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from nyxo_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from nyxo_cli.web_server import _denormalize_config_from_web
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("NYXO_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from nyxo_cli.web_server import _normalise_theme_definition
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nyxo_home):
        ...
        monkeypatch.setattr(
            nyxo_state, "DEFAULT_DB_PATH", get_nyxo_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``nyxo_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in nyxo_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nyxo_home):
        ...
        # Pin the SessionDB to the isolated NYXO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_nyxo_home, _install_example_plugin):
        ...
        installs it into the per-test ``NYXO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into NYXO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, nyxo-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (nyxo-achievements is loaded by default).
        resp = self.client.get("/api/plugins/nyxo-achievements/overview")
        assert resp.status_code == 401
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import nyxo_cli.main as main_mod
        ...
        assert env["NYXO_TUI_DASHBOARD"] == "1"
        assert env["NYXO_TUI_INLINE"] == "1"
        assert env["NYXO_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_nyxo_home
    ):
        import nyxo_cli.main as main_mod

        config_path = Path(os.environ["NYXO_HOME"]) / "config.yaml"
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from nyxo_cli.pty_bridge import PtyUnavailableError
        ...
        import nyxo_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        NYXO_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from nyxo_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("NYXO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nyxo_home, _install_example_plugin):
        ...
        the per-test ``NYXO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_nyxo_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("NYXO_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under NYXO_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_nyxo_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("NYXO_DESKTOP", raising=False)
