"""Tests for nyxo_cli/goals.py — persistent cross-turn goals."""
...
def nyxo_home(tmp_path, monkeypatch):
    """Isolated NYXO_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves NYXO_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from nyxo_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from nyxo_cli.goals import judge_goal
...
class TestGoalManager:
    def test_no_goal_initial(self, nyxo_home):
    ...
    def test_set_then_status(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, nyxo_home):
    ...
    def test_evaluate_after_turn_done(self, nyxo_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, nyxo_home):
    ...
    def test_evaluate_after_turn_inactive(self, nyxo_home):
    ...
    def test_continuation_prompt_shape(self, nyxo_home):
        ...
        mgr.set("port goal command to nyxo")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to nyxo" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from nyxo_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, nyxo_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, nyxo_home):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, nyxo_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from nyxo_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, nyxo_home):
        from nyxo_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_no_goal_to_migrate_returns_false(self, nyxo_home):
        from nyxo_cli.goals import migrate_goal_to_session, load_goal
        assert migrate_goal_to_session("empty-parent", "child2") is False
        assert load_goal("child2") is None

    def test_does_not_clobber_existing_child_goal(self, nyxo_home):
        from nyxo_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_same_id_is_noop(self, nyxo_home):
        from nyxo_cli.goals import save_goal, migrate_goal_to_session, GoalState
        save_goal("same", GoalState(goal="g"))
        assert migrate_goal_to_session("same", "same") is False

    def test_cleared_goal_not_migrated(self, nyxo_home):
        from nyxo_cli.goals import save_goal, clear_goal, migrate_goal_to_session, load_goal, GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_add_subgoal_requires_active_goal(self, nyxo_home):
        import pytest
        from nyxo_cli.goals import GoalManager
    ...
    def test_add_empty_subgoal_rejected(self, nyxo_home):
        import pytest
        from nyxo_cli.goals import GoalManager
    ...
    def test_remove_subgoal(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, nyxo_home):
        import pytest
        from nyxo_cli.goals import GoalManager
    ...
    def test_clear_subgoals(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_subgoals_persist_across_reloads(self, nyxo_home):
...
class TestContinuationPromptWithSubgoals:
    def test_empty_subgoals_uses_original_template(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_with_subgoals_includes_them(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, nyxo_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
...
class TestStatusLineSubgoalCount:
    def test_status_line_no_subgoals(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_status_line_with_subgoals(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
...
class TestWaitBarrier:
    ...
    def test_wait_on_requires_active_goal(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
        mgr = GoalManager(session_id="wb-noactive")
        with pytest.raises(RuntimeError):
            mgr.wait_on(12345)

    def test_wait_on_rejects_bad_pid(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_barrier_auto_clears_when_process_exits_and_loop_resumes(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_dead_pid_never_parks(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_stop_waiting_clears_barrier(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_pause_and_resume_clear_barrier(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_barrier_persists_and_reloads(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_old_state_row_loads_without_barrier_fields(self, nyxo_home):
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_pid_parks_loop(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_judge_wait_seconds_parks_loop(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, nyxo_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_is_session_waiting_running_unmatched(self, nyxo_home):
        s, reg = self._inject("proc_t1", watch_patterns=["READY"])
        assert reg.is_session_waiting("proc_t1") is True

    def test_registry_releases_on_watch_match_while_alive(self, nyxo_home):
    ...
    def test_registry_releases_on_exit_plain_session(self, nyxo_home):
    ...
    def test_registry_unknown_session_never_waits(self, nyxo_home):
        from tools.process_registry import process_registry
        assert process_registry.is_session_waiting("proc_does_not_exist") is False

    def test_goal_parks_on_session_and_releases_on_trigger(self, nyxo_home):
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalManager
    ...
    def test_wait_on_session_validation(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_session_directive_parsed_from_judge(self, nyxo_home):
        from nyxo_cli.goals import _parse_judge_response
    ...
    def test_old_state_loads_without_session_field(self, nyxo_home):
        from nyxo_cli.goals import GoalState
...
class TestParseContract:
    def test_plain_goal_no_contract(self):
        from nyxo_cli.goals import parse_contract
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from nyxo_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from nyxo_cli.goals import GoalContract
...
class TestGoalManagerContract:
    def test_set_with_contract(self, nyxo_home):
        from nyxo_cli.goals import GoalManager, GoalContract
    ...
    def test_set_without_contract_no_marker(self, nyxo_home):
        from nyxo_cli.goals import GoalManager
    ...
    def test_continuation_prompt_includes_contract(self, nyxo_home):
        from nyxo_cli.goals import GoalManager, GoalContract
    ...
    def test_set_contract_after_the_fact(self, nyxo_home):
        from nyxo_cli.goals import GoalManager, GoalContract
        ...
        assert mgr.has_contract()
        # Survives reload.
        from nyxo_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, nyxo_home):
        from nyxo_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalContract
    ...
    def test_contract_plus_subgoals_combine(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
    ...
    def test_draft_returns_none_on_bad_json(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalContract
    ...
    def test_contract_goal_can_still_complete_on_evidence(self, nyxo_home):
        from unittest.mock import patch
        from nyxo_cli import goals
        from nyxo_cli.goals import GoalContract
