"""Tests for the top-level `./nyxo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nyxo` should use `nyxo_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nyxo"
    called = []

    fake_main_module = types.ModuleType("nyxo_cli.main")

    def fake_main():
        called.append("nyxo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nyxo_cli.main", fake_main_module)
