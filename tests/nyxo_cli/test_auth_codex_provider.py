"""Tests for Codex auth — tokens stored in Nyxo auth store (~/.nyxo/auth.json)."""
...
def _setup_nyxo_auth(nyxo_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Nyxo auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_nyxo_auth(nyxo_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
    ...
    monkeypatch.setattr("nyxo_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    nyxo_home = tmp_path / "nyxo"
    _setup_nyxo_auth(nyxo_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((nyxo_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``nyxo auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``nyxo auth add openai-codex``.
    ...
    (nyxo_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `nyxo auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``nyxo auth add openai-codex`` for accounts B and C
    ...
    (nyxo_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``nyxo auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``nyxo auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Nyxo auth store, not ~/.codex/."""
    nyxo_home = tmp_path / "nyxo"
    codex_home = tmp_path / "codex-cli"
    nyxo_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "nyxo-at", "refresh_token": "nyxo-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Nyxo store
    assert not (codex_home / "auth.json").exists()

    # Nyxo auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "nyxo-at"


def test_resolve_returns_nyxo_auth_store_source(tmp_path, monkeypatch):
    nyxo_home = tmp_path / "nyxo"
    _setup_nyxo_auth(nyxo_home)
    monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run nyxo auth".
    """
    from nyxo_cli.auth import (
    ...
    assert "nyxo auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from nyxo_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from nyxo_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "nyxo_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "nyxo_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("nyxo_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("nyxo_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch nyxo_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("nyxo_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
