"""Tests for the `nyxo memory reset` CLI command.
...
- Profile-scoped reset (uses NYXO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake NYXO_HOME with memory files."""
    nyxo_home = tmp_path / ".nyxo"
    memories = nyxo_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("NYXO_HOME", str(nyxo_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nNyxo repo is at ~/.nyxo/nyxo-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `nyxo memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        nyxo_home = tmp_path / ".nyxo"
        (nyxo_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("NYXO_HOME", str(nyxo_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's NYXO_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_nyxo_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
