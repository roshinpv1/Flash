...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NYXO_HOME with .container-mode file."""
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    monkeypatch.setenv("NYXO_HOME", str(hermes_home))
    monkeypatch.delenv("NYXO_DEV", raising=False)

    container_mode = hermes_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nyxo-agent\n"
        "exec_user=hermes\n"
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nyxo_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nyxo-agent"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("nyxo_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_hermes_dev(container_env, monkeypatch):
    """Returns None when NYXO_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("NYXO_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hermes_dev_zero(container_env, monkeypatch):
    """NYXO_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("NYXO_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("nyxo_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_nyxo_home": lambda: hermes_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NYXO_DEV", None)
            info = get_container_exec_info()
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nyxo_cli.main import _exec_in_container
