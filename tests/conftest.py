"""Shared fixtures for the flash-agent test suite.
...
2. **Isolated HERMES_HOME.** HERMES_HOME points to a per-test tempdir so
   code reading ``~/.flash/*`` via ``get_flash_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".flash"`` instead of the canonical
   ``get_flash_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.flash/kanban.db instead of the per-test HERMES_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
    ...
    # dashboard-auth `flash` plugin auto-registers itself on plugin discovery,
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    reads ``~/.flash/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.flash/*`` via ``get_flash_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.flash/*`` via
    #    ``Path.home() / ".flash"`` instead of ``get_flash_home()``
    #    is a bug to fix at the callsite.
    fake_flash_home = tmp_path / "flash_test"
    fake_flash_home.mkdir()
    (fake_flash_home / "sessions").mkdir()
    (fake_flash_home / "cron").mkdir()
    (fake_flash_home / "memories").mkdir()
    (fake_flash_home / "skills").mkdir()
    monkeypatch.setenv("HERMES_HOME", str(fake_flash_home))
    ...
    #    ~/.flash/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import flash_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_flash_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal flash config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchroflash tests.
...
# environment and finds the developer's live ``flash-gateway`` process
...
# correlated with a ``tests/flash_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> flash-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart flash-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting flash/python patterns are also blocked.
    """
    ...
    def _matches_flash_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
    ...
    def _is_blocked_systemctl(cmd) -> bool:
        ...
        if "systemctl" not in cmd_str:
            return False
        if not _matches_flash_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch flash-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m flash_cli.main".
                if (
                    "flash" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live flash-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting flash/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `flash update` (or the
        # equivalent `python -m flash_cli.main update`).  These commands
        ...
        if "update" in low and (
            # flash update / flash update --gateway / setsid bash -c ... flash update
            ("flash" in low and "update" in low.split())
            or
            # python -m flash_cli.main update --gateway
            ("flash_cli" in low and "update" in low.split())
            or
            # venv/bin/flash update  (absolute path variant used in tests)
            (".venv/bin/flash" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`flash update` against the real checkout, fetching "
