"""Shared fixtures for the nyxo-agent test suite.
...
2. **Isolated NYXO_HOME.** NYXO_HOME points to a per-test tempdir so
   code reading ``~/.nyxo/*`` via ``get_nyxo_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".nyxo"`` instead of the canonical
   ``get_nyxo_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No NYXO_SESSION_* inheritance** — the agent's current gateway
...
# NYXO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_NYXO_BEHAVIORAL_VARS = frozenset({
    "NYXO_YOLO_MODE",
    "NYXO_INTERACTIVE",
    "NYXO_QUIET",
    "NYXO_TOOL_PROGRESS",
    "NYXO_TOOL_PROGRESS_MODE",
    "NYXO_MAX_ITERATIONS",
    ...
    "NYXO_SESSION_SOURCE",
    "NYXO_SESSION_KEY",
    "NYXO_GATEWAY_SESSION",
    "NYXO_CRON_SESSION",
    "_NYXO_GATEWAY",
    "NYXO_PLATFORM",
    "NYXO_MODEL",
    "NYXO_INFERENCE_MODEL",
    "NYXO_INFERENCE_PROVIDER",
    "NYXO_TUI_PROVIDER",
    "NYXO_MANAGED",
    "NYXO_MANAGED_DIR",
    "NYXO_DEV",
    ...
    "NYXO_TIMEZONE",
    "NYXO_REDACT_SECRETS",
    "NYXO_BACKGROUND_NOTIFICATIONS",
    "NYXO_EXEC_ASK",
    "NYXO_HOME_MODE",
    "NYXO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.nyxo/kanban.db instead of the per-test NYXO_HOME.
    "NYXO_KANBAN_DB",
    "NYXO_KANBAN_BOARD",
    "NYXO_KANBAN_HOME",
    "NYXO_KANBAN_WORKSPACES_ROOT",
    "NYXO_KANBAN_LOGS_ROOT",
    "NYXO_KANBAN_TASK",
    "NYXO_KANBAN_WORKSPACE",
    "NYXO_KANBAN_RUN_ID",
    "NYXO_KANBAN_CLAIM_LOCK",
    "NYXO_KANBAN_DISPATCH_IN_GATEWAY",
    "NYXO_TENANT",
    ...
    "NYXO_DASHBOARD_OAUTH_CLIENT_ID",
    "NYXO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NYXO_HOME to per-test tempdirs so code that
    reads ``~/.nyxo/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral NYXO_* vars that could change test semantics.
    for name in _NYXO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect NYXO_HOME to a per-test tempdir. Code that reads
    #    ``~/.nyxo/*`` via ``get_nyxo_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.nyxo/*`` via
    #    ``Path.home() / ".nyxo"`` instead of ``get_nyxo_home()``
    #    is a bug to fix at the callsite.
    fake_nyxo_home = tmp_path / "nyxo_test"
    fake_nyxo_home.mkdir()
    (fake_nyxo_home / "sessions").mkdir()
    (fake_nyxo_home / "cron").mkdir()
    (fake_nyxo_home / "memories").mkdir()
    (fake_nyxo_home / "skills").mkdir()
    monkeypatch.setenv("NYXO_HOME", str(fake_nyxo_home))
    ...
    #    ~/.nyxo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import nyxo_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_nyxo_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal nyxo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``nyxo-gateway`` process
...
# correlated with a ``tests/nyxo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> nyxo-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart nyxo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting nyxo/python patterns are also blocked.
    """
    ...
    def _matches_nyxo_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _NYXO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_nyxo_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch nyxo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m nyxo_cli.main".
                if (
                    "nyxo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live nyxo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting nyxo/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `nyxo update` (or the
        # equivalent `python -m nyxo_cli.main update`).  These commands
        ...
        if "update" in low and (
            # nyxo update / nyxo update --gateway / setsid bash -c ... nyxo update
            ("nyxo" in low and "update" in low.split())
            or
            # python -m nyxo_cli.main update --gateway
            ("nyxo_cli" in low and "update" in low.split())
            or
            # venv/bin/nyxo update  (absolute path variant used in tests)
            (".venv/bin/nyxo" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`nyxo update` against the real checkout, fetching "
