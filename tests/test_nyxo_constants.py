"""Tests for nyxo_constants module."""
...
from nyxo_constants import (
    ...
    find_nyxo_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_nyxo_root,
    get_nyxo_home,
    iter_nyxo_node_dirs,
    is_container,
...
class TestGetDefaultNyxoRoot:
    """Tests for get_default_nyxo_root() — Docker/custom deployment awareness."""

    def test_no_nyxo_home_returns_native(self, tmp_path, monkeypatch):
        """When NYXO_HOME is not set, returns ~/.nyxo."""
        monkeypatch.delenv("NYXO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_nyxo_root() == tmp_path / ".nyxo"

    def test_nyxo_home_is_native(self, tmp_path, monkeypatch):
        """When NYXO_HOME = ~/.nyxo, returns ~/.nyxo."""
        native = tmp_path / ".nyxo"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(native))
        assert get_default_nyxo_root() == native

    def test_nyxo_home_is_profile(self, tmp_path, monkeypatch):
        """When NYXO_HOME is a profile under ~/.nyxo, returns ~/.nyxo."""
        native = tmp_path / ".nyxo"
    ...
    def test_nyxo_home_is_docker(self, tmp_path, monkeypatch):
        """When NYXO_HOME points outside ~/.nyxo (Docker), returns NYXO_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(docker_home))
        assert get_default_nyxo_root() == docker_home

    def test_nyxo_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any NYXO_HOME outside ~/.nyxo is treated as the root."""
        custom = tmp_path / "my-nyxo-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(custom))
        assert get_default_nyxo_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (NYXO_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NYXO_HOME", str(profile))
        assert get_default_nyxo_root() == docker_root

    def test_no_nyxo_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\nyxo, not ~/.nyxo."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NYXO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(nyxo_constants.sys, "platform", "win32")

        assert get_default_nyxo_root() == local_appdata / "nyxo"

    def test_no_nyxo_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/nyxo without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("NYXO_HOME", raising=False)
        ...
        assert get_default_nyxo_root() == home / "AppData" / "Local" / "nyxo"


class TestGetNyxoHome:
    """Tests for get_nyxo_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When NYXO_HOME is unset on Windows, use %LOCALAPPDATA%\\nyxo."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NYXO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(nyxo_constants.sys, "platform", "win32")
        monkeypatch.setattr(nyxo_constants, "_profile_fallback_warned", False)

        assert get_nyxo_home() == local_appdata / "nyxo"


class TestNyxoManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_nyxo_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        assert find_nyxo_node_executable("npm") == str(npm_cmd)
    ...
    def test_with_nyxo_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "nyxo"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_nyxo_node_path({"PATH": "system-node"})
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(nyxo_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(nyxo_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".nyxo"
