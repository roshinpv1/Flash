...
"""
    ...
    assert "Tyler Williams > Active Projects: Nyxo Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `nyxo cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # nyxo_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.nyxo/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "nyxo_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Nyxo``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Nyxo prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Nyxo to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Nyxo config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Nyxo uses tools well"
    # All-lowercase matches → lowercase ``nyxo``; this preserves the
    # real filesystem path ``~/.nyxo`` (Nyxo home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "nyxo should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Nyxo remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "nyxo prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Nyxo was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "nyxo uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Nyxo said to ask Nyxo about Nyxo settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.nyxo`` (the real Nyxo home), not the broken
    ``.Nyxo``.
    ...
    rewritten to ``~/.Nyxo/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.nyxo/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .nyxo directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.nyxo'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "nyxo config path: ~/.nyxo/"
