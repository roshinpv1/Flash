"""Shell completion script generation for nyxo CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nyxo_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Nyxo Agent bash completion
# Add to ~/.bashrc:
#   eval "$(nyxo completion bash)"

_nyxo_profiles() {{
    local profiles_dir="$HOME/.nyxo/profiles"
...
_nyxo_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nyxo_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nyxo_completion nyxo
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef nyxo
# Nyxo Agent zsh completion
# Add to ~/.zshrc:
...
_nyxo_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nyxo/profiles" ]]; then
        profiles+=($HOME/.nyxo/profiles/*(N/:t))
    fi
...
_nyxo() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nyxo_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nyxo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Nyxo Agent fish completion",
        "# Add to your config:",
        "#   nyxo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nyxo_profiles",
        "    echo default",
        "    if test -d $HOME/.nyxo/profiles",
        "        for d in $HOME/.nyxo/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nyxo -f -s p -l profile"
        " -d 'Profile name' -xa '(__nyxo_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nyxo_profiles)' -d 'Profile name'"
                )
