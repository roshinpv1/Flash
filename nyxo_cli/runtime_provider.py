...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from nyxo_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from nyxo_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Nyxo' tool dispatch. Gated behind config key
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from nyxo_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from nyxo_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("NYXO_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def canonical_custom_identity(
    *,
    ...
    2. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``NYXO_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("NYXO_INFERENCE_PROVIDER", "").strip()
    ...
    *,
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from nyxo_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Nyxo canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Nyxo-compatible importers).
