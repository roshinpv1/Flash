...
manual BotFather token copy-paste. Nyxo talks only to the Flashonboarding
...
DEFAULT_API_URL = "https://setup.nyxo-agent.flash.com"
...
DEFAULT_MANAGER_BOT = "NyxoSetupBot"

DEFAULT_BOT_NAME = "Nyxo Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``nyxo_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``nyxo_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"nyxo_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Nyxo Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Nyxo Telegram onboarding service.")
