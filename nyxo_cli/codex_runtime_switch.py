...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Nyxo'
...
The actual runtime resolution happens in nyxo_cli.runtime_provider's
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Nyxo' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Nyxo for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from nyxo_cli.codex_runtime_plugin_migration import migrate
            ...
            # Permissions + Nyxo tool callback are always-on production
            ...
            if "flash-tools" in mig_report.migrated:
                msg_lines.append(
                    "Nyxo tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Nyxo runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Nyxo tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Nyxo runtime.")
