"""
Status command for nyxo CLI.

Shows the status of all Nyxo Agent components.
"""
...
from nyxo_cli.auth import AuthError, resolve_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``nyxo config``'s
...
def show_status(args):
    """Show status of all Nyxo Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Nyxo Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: nyxo model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: nyxo auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: nyxo auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: nyxo model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from nyxo_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from nyxo_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_nyxo_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_nyxo_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'nyxo doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'nyxo setup' to configure", Colors.DIM))
    print()
