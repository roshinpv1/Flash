...
on Japanese locales, etc.).  Nyxo's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nyxo_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Nyxo is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NYXO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NYXO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Nyxo-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nyxo
    ...
    # install without Nyxo-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Nyxo's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Nyxo's
    ``nyxo config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Nyxo (User env var in Windows Settings, or
    export in a PowerShell profile) and Nyxo picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Nyxo-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nyxo\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nyxo`` from right after install) retain their old PATH.

    Any subprocess Nyxo spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Nyxo-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nyxo", "git", "cmd"),
        os.path.join(local_appdata, "nyxo", "git", "bin"),
        os.path.join(local_appdata, "nyxo", "git", "usr", "bin"),
        # Nyxo venv Scripts directory — host of the nyxo.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nyxo, but harmless to include.
        os.path.join(local_appdata, "nyxo", "nyxo-agent", "venv", "Scripts"),
