"""
flash fallback — manage the fallback provider chain.
...
Subcommands:
  flash fallback [list]   Show the current fallback chain (default when no subcommand)
  flash fallback add      Pick provider + model via the same picker as `flash model`,
                           then append the selection to the chain
  flash fallback remove   Pick an entry to delete from the chain
  flash fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.flash/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nyxo_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `flash model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  flash fallback add")
        print()
        return
    ...
    print("  Docs: https://nyxo-agent.flash.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `flash model`, then append the selection to the chain."""
    from nyxo_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `flash model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `flash fallback list` to view, or `flash fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``flash fallback [subcommand]``."""
