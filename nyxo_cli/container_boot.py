...
``$NYXO_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-nyxo-setup
(the stage2 hook) has chowned the volume and seeded $NYXO_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NYXO_HOME``,
    not under ``profiles/``). The dispatcher in ``nyxo_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``nyxo gateway start`` (no ``-p``) targets. Without it,
    bare ``nyxo gateway start`` inside the container would land on
    ...
    ``$NYXO_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        nyxo_home: The container's NYXO_HOME (typically /opt/data).
            Profiles live under ``<nyxo_home>/profiles/<name>/``;
            the default profile lives at ``<nyxo_home>`` itself.
    ...
    # ``nyxo gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(nyxo_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(nyxo_home)
    ...
    profiles_root = nyxo_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `nyxo profile create` (config.yaml
            # is not — that comes later via `nyxo setup`). Use it as the
            ...
            # via ``nyxo -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(nyxo_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = nyxo_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("NYXO_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the nyxo args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/nyxo/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/nyxo/docker/main-wrapper.sh
    ...
    ``nyxo`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``nyxo <subcommand>``, so an
    explicit leading ``nyxo`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `nyxo <subcommand>`; peel an explicit nyxo.
    if args and Path(args[0]).name == "nyxo":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``nyxo dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted NYXO_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `nyxo -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit nyxo
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the nyxo user) won't hit EACCES. See
...
def _write_reconcile_log(
    nyxo_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $NYXO_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted NYXO_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        nyxo_home=nyxo_home, scandir=scandir,
    )
    for a in actions:
        print(
