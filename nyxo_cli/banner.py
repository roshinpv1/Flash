...
Pure display functions with no NyxoCLI state dependency.
"""
...
from nyxo_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _fetch_pypi_latest(package: str = "nyxo-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Nyxo update is available.

    Two paths: if ``NYXO_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = nyxo_home / ".update_check"
    embedded_rev = os.environ.get("NYXO_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # NYXO_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `nyxo update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/nyxo/update/check`
    ...
    if embedded_rev:
        ...
        # $NYXO_HOME/nyxo-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = nyxo_home / "nyxo-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Nyxo git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$NYXO_HOME/nyxo-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``nyxo_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from nyxo_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Nyxo Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else NYXO_CADUCEUS
    except Exception:
        _bskin = None
        _hero = NYXO_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from nyxo_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from nyxo_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built nyxo; we know an update
    ...
    # Pip-install warning — `pip install nyxo-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else NYXO_AGENT_LOGO
        console.print(_logo)
        console.print()
