...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_nyxo_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$NYXO_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# in nyxo_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from nyxo_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from nyxo_cli.auth import resolve_nous_runtime_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from nyxo_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
