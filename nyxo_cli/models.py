...
Add, remove, or reorder entries here — both `nyxo setup` and
`nyxo` provider-selection will pick up the change automatically.
"""
...
_NYXO_USER_AGENT = f"nyxo-cli/{_NYXO_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nyxo model`
    flow without maintaining a separate static list.
    """
    from nyxo_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $NYXO_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nyxo-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NYXO_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Nyxo refreshes the cache from
...
# ---------------------------------------------------------------------------
# FlashPortal free-model helper
# ---------------------------------------------------------------------------
# The FlashPortal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# FlashPortal account tier detection
# ---------------------------------------------------------------------------
...
def partition_flash_models_by_tier(
    ...
    """Split Flashmodels into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Nyxo that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Nyxo that doesn't ship
...
def check_flash_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current FlashPortal user is on a free (unpaid) tier.
    ...
    try:
        from nyxo_cli.flash_account import get_flash_portal_account_info
...
# ---------------------------------------------------------------------------
# FlashPortal recommended models
#
...
FLASH_RECOMMENDED_MODELS_PATH = "/api/flash/recommended-models"
_FLASH_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
...
def _flash_recommended_disk_path() -> "Path":
    ...
    return get_nyxo_home() / "cache" / "flash_recommended_cache.json"
...
def fetch_flash_recommended_models(
    ...
    *,
    ...
    """
    base = (portal_base_url or "https://portal.flash.com").rstrip("/")
    now = time.monotonic()
    cached = _flash_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _FLASH_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{FLASH_RECOMMENDED_MODELS_PATH}"
    try:
...
# this list:  nyxo model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nyxo model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `nyxo model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("flash",           "FlashPortal",              "FlashPortal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Nyxo provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `nyxo model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``nyxo model``, the
...
# sensible default, but FlashPortal is a per-token *metered aggregator* whose
...
# path. The *interactive* default (GUI onboarding / ``nyxo model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in nyxo_cli/web_server.py and ``partition_flash_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``nyxo auth add openai-codex`` without
    ``nyxo model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``nyxo model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nyxo-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (FlashPortal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nyxo_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — nyxo-agent
...
def get_curated_flash_model_ids() -> list[str]:
    """Return the curated FlashPortal model-id list.
    ...
    """
    try:
        from nyxo_cli.model_catalog import get_curated_flash_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, FlashPortal).
    """
...
_DEFAULT_FLASH_INFERENCE_BASE = "https://inference-api.flash.com"


def _resolve_flash_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for FlashPortal pricing.

    The Flashinference ``/v1/models`` endpoint exposes pricing without
    ...
    """
    try:
        from nyxo_cli.auth import resolve_flash_runtime_credentials
        creds = resolve_flash_runtime_credentials()
        if creds:
            return (creds.get("api_key", ""), creds.get("base_url", ""))
    except Exception:
        pass
    return ("", _DEFAULT_FLASH_INFERENCE_BASE)
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "flash":
        ...
        if base_url:
            # Flashbase_url typically looks like https://inference-api.flash.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nyxo model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nyxo_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Flash, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from nyxo_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Nyxo' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nyxo_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Nyxo should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nyxo auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nyxo/.env``.
    ...
    """
    try:
        from nyxo_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from nyxo_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Flash),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Nyxo release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Nyxo release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from nyxo_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "flash":
        # Try live FlashPortal /models endpoint
        try:
            from nyxo_cli.auth import fetch_flash_models, resolve_flash_runtime_credentials
            creds = resolve_flash_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Nyxo release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``nyxo model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $NYXO_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from nyxo_constants import get_nyxo_home
    return get_nyxo_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$NYXO_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from nyxo_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "NyxoAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NYXO_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Nyxo' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _NYXO_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from nyxo_constants import get_nyxo_home
    return get_nyxo_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Nyxo will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Nyxo cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Nyxo actually ships.  (#46850)
