...
``<root>`` is the **shared Nyxo root** (the parent of any active
...
``nyxo -p <profile>`` joins the same board as the dispatcher that
...
the profile's Nyxo home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NYXO_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NYXO_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``nyxo kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.nyxo``. In Docker / custom
deployments where ``NYXO_HOME`` points outside ``~/.nyxo`` (e.g.
``/opt/nyxo``), ``<root>`` is ``NYXO_HOME``. Legacy env-var
overrides still work:

* ``NYXO_KANBAN_DB`` — pin the database file path directly.
* ``NYXO_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NYXO_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NYXO_KANBAN_DB``,
``NYXO_KANBAN_WORKSPACES_ROOT``, and ``NYXO_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/nyxo-kanban-v1-spec.pdf`` for the full
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active NYXO_HOME so dispatcher- and
...
# ``NYXO_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``NYXO_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("NYXO_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``NYXO_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("NYXO_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``NYXO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "NYXO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "nyxo_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``nyxo-agent``
...
def kanban_home() -> Path:
    """Return the shared Nyxo root that anchors the kanban board.

    Resolution order:

    1. ``NYXO_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_nyxo_root()``, which already returns ``<root>``
       when ``NYXO_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NYXO_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NYXO_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NYXO_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from nyxo_constants import get_default_nyxo_root
    return get_default_nyxo_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``nyxo kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NYXO_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``nyxo kanban boards
    ...
    env = os.environ.get("NYXO_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``nyxo kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NYXO_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NYXO_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NYXO_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NYXO_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``NYXO_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("NYXO_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``nyxo kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``NYXO_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``NYXO_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("NYXO_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``nyxo gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Nyxo treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NYXO_KANBAN_DB`` env → ``NYXO_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `nyxo kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See nyxo_state._WAL_INCOMPAT_MARKERS for detection logic.
                from nyxo_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``nyxo kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``NYXO_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from nyxo_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``nyxo --skills ...``. Use this to pin a task to a
    ...
    # `nyxo --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``nyxo kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `nyxo kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``nyxo kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``nyxo kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``nyxo kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``nyxo kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``NYXO_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Nyxo' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Nyxo-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``nyxo kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Nyxo checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Nyxo treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Nyxo creates/reuses that linked worktree. With
      no ``workspace_path``, Nyxo anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Nyxo uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``NYXO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Nyxo
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``nyxo kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``nyxo kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Nyxo profile.
    ...
    if not rows:
        return False
    try:
        from nyxo_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from nyxo_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Nyxo profile.
        # `_default_spawn` invokes ``nyxo -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Nyxo
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `nyxo kanban tail` — without
...
def _module_nyxo_argv() -> list[str]:
    """Return the interpreter-bound Nyxo CLI invocation."""
    # ``nyxo_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``nyxo`` package — there is no
    # ``nyxo`` package to import.
    return [sys.executable, "-m", "nyxo_cli.main"]


def _absolute_nyxo_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Nyxo shim."""
...
def _nyxo_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Nyxo executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_nyxo_argv()
    return [_absolute_nyxo_path(path)]


def _resolve_nyxo_argv() -> list[str]:
    """Resolve the ``nyxo`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$NYXO_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("nyxo")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\nyxo.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m nyxo_cli.main`` — fallback for setups where
       Nyxo is launched from a venv and the ``nyxo`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_nyxo_bin`` for the same reason. Kept
    local (not imported from gateway) because ``nyxo_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("NYXO_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _nyxo_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _nyxo_path_argv(resolved_env_bin)
        return _module_nyxo_argv()

    nyxo_bin = _safe_which_no_cwd("nyxo") if _IS_WINDOWS else shutil.which("nyxo")
    if nyxo_bin:
        return _nyxo_path_argv(nyxo_bin)
    return _module_nyxo_argv()
...
def _resolve_worker_cli_toolsets(nyxo_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``NYXO_KANBAN_TASK`` is set.
    """
    if not nyxo_home:
        return None
    try:
        ...
        token = set_nyxo_home_override(nyxo_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for NYXO_HOME=%r (%s)",
            nyxo_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``nyxo -p <profile> chat -q ...`` subprocess.
    ...
    ``NYXO_KANBAN_DB`` / ``NYXO_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject NYXO_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `nyxo -p <name>` the
    # _apply_profile_override() runs *before* nyxo_constants is imported.
    # If NYXO_HOME is absent from the child's env, get_nyxo_home() falls
    # back to Path.home() / ".nyxo" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from nyxo_cli.profiles import resolve_profile_env
    try:
        env["NYXO_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via NYXO_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NYXO_HOME never had profiles created.
        pass
    if task.tenant:
        env["NYXO_TENANT"] = task.tenant
    env["NYXO_KANBAN_TASK"] = task.id
    env["NYXO_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["NYXO_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["NYXO_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["NYXO_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["NYXO_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["NYXO_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`nyxo -p <name>`
    # rewrites NYXO_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_nyxo_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NYXO_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NYXO_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NYXO_KANBAN_BOARD"] = resolved_board
    # NYXO_PROFILE is the author the kanban_comment tool defaults to.
    # `nyxo -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NYXO_PROFILE"] = profile_arg

    cmd = [
        *_resolve_nyxo_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped NYXO_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("NYXO_HOME"))
    if worker_toolsets:
    ...
    # `nyxo kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`nyxo` executable not found on PATH. "
            "Install Nyxo Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``nyxo kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled NYXO_PROFILE values like "nyxo-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Nyxo root exists

    Reads profile paths directly so this module has no import dependency on
    ``nyxo_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from nyxo_constants import get_default_nyxo_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``nyxo kanban assignees`` for the terminal.
