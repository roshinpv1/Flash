...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or flash -c.",
    ...
    "Ctrl+Z suspends Nyxo to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "flash -c resumes your most recent CLI session. flash -c \"project name\" resumes by title.",
    "flash -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "flash -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "flash chat -t web,terminal enables only specific toolsets for a focused session.",
    "flash chat -s github-pr-workflow preloads a skill at launch.",
    "flash chat -q \"query\" runs a single non-interactive query and exits.",
    "flash chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "flash chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "flash --yolo bypasses all dangerous command approval prompts for the entire session.",
    "flash chat --source telegram tags the session for filtering in flash sessions list.",
    "flash -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "flash doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "flash dump outputs a compact setup summary — great for bug reports.",
    "flash config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "flash config edit opens config.yaml in your default editor.",
    "flash config check scans for missing or stale configuration options.",
    "flash sessions browse opens an interactive session picker with search.",
    "flash sessions stats shows session counts by platform and database size.",
    "flash sessions prune --older-than 30 cleans up old sessions.",
    "flash skills search react --source skills-sh searches the skills.sh public directory.",
    "flash skills check scans installed hub skills for upstream updates.",
    "flash skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "flash skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "flash mcp add github --command npx adds MCP servers from the command line.",
    "flash mcp serve runs Nyxo itself as an MCP server for other agents.",
    "flash auth add lets you add multiple API keys for credential pool rotation.",
    "flash completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "flash logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "flash backup creates a zip backup of your entire Nyxo home directory.",
    "flash profile create coder creates an isolated profile that becomes its own command.",
    "flash profile create work --clone copies your current config and keys to a new profile.",
    "flash update syncs new bundled skills to ALL profiles automatically.",
    "flash gateway install sets up Nyxo as a system service (systemd/launchd).",
    "flash memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "flash webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: flash tools disables unused tools, flash skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "flash models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as flash -w).",
    ...
    "NYXO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Nyxo tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'flash profile create coder' creates the 'coder' command.",
    "flash profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Nyxo prints a resume command with session ID and stats.",
    "flash sessions export backup.jsonl exports all sessions for backup or analysis.",
    "flash -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "flash skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .flash/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: flash cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Nyxo runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "flash gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Nyxo via a local macOS server.",
    ...
    "Context files (.flash.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Nyxo loads project context from .flash.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "flash mcp opens an interactive picker of Flash-approved MCPs you can install in one keystroke.",
    "flash mcp catalog lists Flash-approved MCP servers shipped with the repo.",
    "flash mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — flash tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.flash/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "flash chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "flash plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "flash acp runs Nyxo as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "Cron scripts live in ~/.flash/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Nyxo your own.",
    ...
    "Before a gateway session reset, Nyxo auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.flash/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Nyxo state via NYXO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "NYXO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NYXO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.flash/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'flash profile create ls' would be rejected.",
    "flash profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "flash claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NYXO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via flash tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.flash/.env.",
    "Skills from trusted repos (Flash) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.flash/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Nyxo auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Nyxo config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Nyxo is working.',
    ...
    '/reload reloads ~/.flash/.env into the running session — pick up new API keys without restarting.',
    ...
    "NYXO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.flash/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.flash/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'flash curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "flash curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'flash curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'flash auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Flashsubscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'NYXO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NYXO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'flash dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'flash dashboard embeds the full Nyxo TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.flash/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.flash/dashboard-plugins/ — no npm build required.',
    ...
    'NYXO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'NYXO_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'NYXO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NYXO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.flash/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.flash/skills/ so drop-in skills appear without restarting the session.',
    ...
    'flash -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'flash chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'flash chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'flash chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "flash chat --source tool tags programmatic chats so they don't clutter flash sessions list.",
    'flash dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'flash sessions rename <ID> "new title" renames any past session; flash sessions delete <ID> removes one.',
    'flash import restores a session export or profile archive produced by sessions export or profile export.',
    'flash fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'flash pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'flash setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'flash status --deep runs the full health sweep across every component; plain flash status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NYXO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NYXO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.flash/plugins/ — trust-gated by design.',
    "NYXO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NYXO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NYXO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NYXO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NYXO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NYXO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NYXO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NYXO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NYXO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NYXO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NYXO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Nyxo block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from flash sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.flash/dashboard-plugins/.',
]
