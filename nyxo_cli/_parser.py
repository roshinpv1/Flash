...
# argparse runs (it sets ``NYXO_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``nyxo_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via NYXO_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to NYXO_ACCEPT_HOOKS=1 or "
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Nyxo Agent",
    )
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also NYXO_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Nyxo itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
