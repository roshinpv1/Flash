...
Used by ``nyxo profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``nyxo_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Nyxo Agent kanban board.
...
Rules:
  ...
  - Never write "Nyxo Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_nyxo_home())
        else:
