"""
Unified tool configuration for Nyxo Agent.
...
from nyxo_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _NYXO_CORE_TOOLS (available at runtime if enabled),
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from nyxo_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from nyxo_cli.plugins import discover_plugins, get_plugin_toolsets
...
from nyxo_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Nyxo routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set NYXO_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "NYXO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "NYXO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "NYXO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "NYXO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "NYXO_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Nyxo process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_nyxo_home()}/nyxo-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in nyxo-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/flash/nyxo-agent:latest"
            )
            return
        ...
        try:
            from nyxo_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from nyxo_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _print_info("    Restart Nyxo for tracing to take effect.")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "flash-cli" (which include all _NYXO_CORE_TOOLS) cause disabled
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Nyxo Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://nyxo-agent.flash.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from nyxo_constants import display_nyxo_home
    print(color(f"  Tool configuration saved to {display_nyxo_home()}/config.yaml", Colors.DIM))
