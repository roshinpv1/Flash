"""
Nyxo Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.nyxo/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Nyxo PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nyxo-agent or nyxo PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Nyxo Agent" comment and following line
                if '# Nyxo Agent' in line or '# nyxo-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nyxo' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nyxo
                if 'nyxo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nyxo wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nyxo",
        Path("/usr/local/bin/nyxo"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains nyxo_cli reference)
                content = wrapper.read_text()
                if 'nyxo_cli' in content or 'nyxo-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(nyxo_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``nyxo`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Nyxo home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (nyxo_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nyxo gateway run`` processes
    ...
    try:
        from nyxo_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nyxo_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NYXO_HOME`` and ``NYXO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\nyxo\git\cmd``, ``%LOCALAPPDATA%\nyxo\git\bin``,
#      ``%LOCALAPPDATA%\nyxo\git\usr\bin``, ``%LOCALAPPDATA%\nyxo\node``.
...
#   4. On the ``nyxo dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\nyxo\gateway-service\`` and sometimes
...
def _nyxo_path_markers(nyxo_home: Path) -> list[str]:
    """Path-entry substrings that identify Nyxo-owned User-PATH entries."""
    root = str(nyxo_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare nyxo-agent install dir.
    markers = [root + "\\nyxo-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NYXO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nyxo".  We don't want to catch
    # unrelated entries like "cnyxo-foo" or "ephermeral", so we look for
    # backslash-nyxo as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(nyxo_home: Path) -> list[str]:
    """Strip Nyxo-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _nyxo_path_markers(nyxo_home)
            kept: list[str] = []
...
def remove_nyxo_env_vars_windows() -> list[str]:
    """Delete NYXO_HOME and NYXO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NYXO_HOME", "NYXO_GIT_BASH_PATH"):
                try:
...
def _is_default_nyxo_home(nyxo_home: Path) -> bool:
    """Return True when ``nyxo_home`` points at the default (non-profile) root."""
    try:
        from nyxo_constants import get_default_nyxo_root
        return nyxo_home.resolve() == get_default_nyxo_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NYXO_HOME directory.

    We shell out to ``nyxo -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NYXO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m nyxo_cli.main` so we don't depend on a `nyxo`
    #    wrapper that may be half-removed mid-uninstall.
    nyxo_invocation = [_sys.executable, "-m", "nyxo_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nyxo_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's NYXO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``nyxo uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$NYXO_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from nyxo_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(nyxo_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Nyxo Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Nyxo Chat GUI installation was found.")
        print(f"  Checked: {nyxo_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Nyxo agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(nyxo_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Nyxo agent at {nyxo_home / 'nyxo-agent'}")
        print(f"  • Your config, sessions, and secrets under {nyxo_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Nyxo agent is still installed. Run 'nyxo' to use the CLI,")
    print("or 'nyxo uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nyxo/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nyxo/ for future reinstall
    """
    project_root = get_project_root()
    nyxo_home = get_nyxo_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NYXO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_nyxo_home(nyxo_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.nyxo data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Nyxo Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {nyxo_home / 'config.yaml'}")
    print(f"  Secrets: {nyxo_home / '.env'}")
    print(f"  Data:    {nyxo_home / 'cron/'}, {nyxo_home / 'sessions/'}, {nyxo_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NYXO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Nyxo data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Nyxo code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``nyxo`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$NYXO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in nyxo_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\nyxo\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(nyxo_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Nyxo-owned PATH entries in User environment")

        log_info("Removing NYXO_HOME / NYXO_GIT_BASH_PATH User env vars...")
        removed_env = remove_nyxo_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Nyxo-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing nyxo command...")
    ...
    #     (only when they still point into this Nyxo home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Nyxo-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(nyxo_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Nyxo-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(nyxo_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE NYXO_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from nyxo_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(nyxo_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nyxo/, just remove the nyxo-agent subdir
            if nyxo_home in project_root.parents or project_root.parent == nyxo_home:
    ...
    #     under NYXO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(nyxo_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(nyxo_home)
    ...
    # 5. Optionally remove ~/.nyxo/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NYXO_HOME dirs live under
        ...
        try:
            if nyxo_home.exists():
                shutil.rmtree(nyxo_home)
                log_success(f"Removed {nyxo_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {nyxo_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {nyxo_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://nyxo-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://nyxo-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Nyxo Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m nyxo_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import nyxo_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``nyxo_constants`` + ``nyxo_cli.colors``
    (and lazily ``nyxo_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m nyxo_cli.uninstall")
    parser.add_argument(
        "--mode",
