"""``flash debug`` debug tools for Nyxo Agent.

Currently supports:
    flash debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[flash debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.flash/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``flash debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``flash debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Nyxo version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `flash debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nyxo-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``flash debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.flash/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``flash debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nyxo-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----NyxoDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from nyxo_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_nyxo_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``flash dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``flash dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``flash debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "flash debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    print(f"To delete now:  flash debug delete <url>")

    print(f"\nShare these links with the Nyxo team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: flash debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'flash debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``flash debug`` call.
    ...
    # best-effort — any failure is swallowed so ``flash debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: flash debug <command>")
        print()
        print("Commands:")
