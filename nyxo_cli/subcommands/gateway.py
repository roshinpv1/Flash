"""``nyxo gateway`` and ``nyxo proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if NYXO_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy nyxo.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Nyxo gateway unit files "
            "(e.g. nyxo.service) left over from older installs. Profile "
            "units (nyxo-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Requires being logged in (nyxo setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.nyxo/.env and forwarded at provision. "
