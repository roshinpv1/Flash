"""
Single source of truth for provider identity in Nyxo Agent.
...
2. **Nyxo overlays** — transport type, auth patterns, aggregator flags,
...
# -- Nyxo overlay ----------------------------------------------------------
# Nyxo-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class NyxoOverlay:
    """Nyxo-specific provider metadata layered on top of models.dev."""
...
NYXO_OVERLAYS: Dict[str, NyxoOverlay] = {
    "openrouter": NyxoOverlay(
    ...
    "openai-codex": NyxoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": NyxoOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": NyxoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": NyxoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="NYXO_QWEN_BASE_URL",
    ),
    "lmstudio": NyxoOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": NyxoOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": NyxoOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": NyxoOverlay(
    ...
    "minimax-oauth": NyxoOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": NyxoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": NyxoOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": NyxoOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": NyxoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": NyxoOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    overlay = NYXO_OVERLAYS.get(canonical)
    ...
    if overlay is not None:
        # Nyxo-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in NYXO_OVERLAYS
    if provider == "bedrock":
