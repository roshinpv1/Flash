...
Three cookies in play:
  ...
  - nyxo_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-nyxo_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "nyxo_session_at"
SESSION_RT_COOKIE = "nyxo_session_rt"
PKCE_COOKIE = "nyxo_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /nyxo`` we want ``Path=/nyxo`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (nyxo #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/nyxo``)
