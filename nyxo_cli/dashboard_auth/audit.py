...
Profile-aware location: ``$NYXO_HOME/logs/dashboard-auth.log``.
...
from ``nyxo_constants`` or other nyxo_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$NYXO_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``nyxo_constants.get_nyxo_home`` semantics: env var wins,
    else ``~/.nyxo``. A local copy avoids an import cycle with the
    middleware which lives below ``nyxo_cli``.
    """
    home = os.environ.get("NYXO_HOME") or str(Path.home() / ".nyxo")
