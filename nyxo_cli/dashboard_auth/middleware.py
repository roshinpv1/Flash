...
from nyxo_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from nyxo_cli.dashboard_auth.cookies import read_session_cookies
from nyxo_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /nyxo``, the
    ``login_url`` is prefixed (``/nyxo/login?next=...``) so the
    ...
    """
    from nyxo_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["nyxo.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``nyxo_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            from nyxo_cli.dashboard_auth.cookies import (
                detect_https,
                set_session_cookies,
            )
            from nyxo_cli.dashboard_auth.prefix import prefix_from_request
        ...
        from nyxo_cli.dashboard_auth.cookies import clear_session_cookies
        from nyxo_cli.dashboard_auth.prefix import prefix_from_request
