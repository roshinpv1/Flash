...
3. Running inside a container with no persistent volume mount over the
   NYXO_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("flash.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Nyxo as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("NYXO_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(flash_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = flash_home or Path(
        os.environ.get("NYXO_HOME", os.path.expanduser("~/.flash"))
    )
    try:
...
def run_security_audit(
    *, flash_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(flash_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(flash_home=flash_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
