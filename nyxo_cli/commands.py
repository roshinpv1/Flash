"""Slash command definitions and autocomplete for the Nyxo CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Nyxo config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Nyxo works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Nyxo is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.nyxo/skills/ for newly installed or removed skills",
    ...
    CommandDef("credits", "Show Flashcredit balance and top up", "Info"),
    CommandDef("billing", "Manage Flashterminal billing — buy credits, auto-reload, limits", "Info",
               cli_only=True),
    ...
    CommandDef("update", "Update Nyxo Agent to the latest version", "Info"),
    CommandDef("version", "Show Nyxo Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nyxo_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nyxo`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Nyxo ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Nyxo
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nyxo skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nyxo skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nyxo skills list`` and
...
# unrelated command landed. These claim their slots right after /nyxo,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /nyxo <command>).
...
# native slot, the alias spelling stays reachable via /nyxo reset).
...
# ``/nyxo <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/nyxo" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /nyxo credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /nyxo billing.
#   - debug: the log/report upload surface; reached via /nyxo debug on Slack.
_SLACK_VIA_NYXO_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nyxo <verb>`` subcommand.
    ...
    can still reach them via ``/nyxo <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nyxo`` is always reserved as the first entry so the
    legacy ``/nyxo <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nyxo as the catch-all top-level command.
    entries.append(("nyxo", "Talk to Nyxo or run a subcommand", "[subcommand] [args]"))
    seen.add("nyxo")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NYXO_ONLY:
            # Intentionally Slack-via-/nyxo only (see _SLACK_VIA_NYXO_ONLY).
            return
    ...
    # everything except /nyxo, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://nyxo-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nyxo handler.

    Maps both canonical names and aliases so /nyxo bg do stuff works
    the same as /nyxo background do stuff.

    Plugin-registered slash commands are included so ``/nyxo <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from nyxo_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
