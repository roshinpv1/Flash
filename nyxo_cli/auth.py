"""
Multi-provider authentication system for Nyxo Agent.

Supports OAuth device code flows (FlashPortal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.nyxo/auth.json with cross-process file locking.
...
from nyxo_cli.config import get_nyxo_home, get_config_path, read_raw_config
from nyxo_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_FLASH_PORTAL_URL = "https://portal.flash.com"
DEFAULT_FLASH_INFERENCE_URL = "https://inference-api.flash.com/v1"
DEFAULT_FLASH_CLIENT_ID = "nyxo-cli"
...
SPOTIFY_DOCS_URL = "https://nyxo-agent.flash.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://nyxo-agent.flash.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://nyxo-agent.flash.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from nyxo_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.nyxo/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``nyxo auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `nyxo model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "flash":
            return _format_flash_entitlement_auth_error(error)
...
def _format_flash_entitlement_auth_error(error: AuthError) -> str:
    try:
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("NYXO_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.nyxo/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_nyxo_home() / "auth.json"
    # Seat belt: if pytest is running and NYXO_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch NYXO_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".nyxo" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set NYXO_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom NYXO_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_nyxo_root()
    except Exception:
        return None
    profile_home = get_nyxo_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.nyxo/auth.json`` even when NYXO_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_nyxo_root()`` for a profile-shaped NYXO_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".nyxo" / "auth.json"
            try:
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "flash_portal" in systems:
            providers["flash"] = systems["flash_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "flash" if providers else None}
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``flash``) that were only authenticated at
    global scope. Once the user runs ``nyxo auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``nyxo auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``nyxo auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Nyxo.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `nyxo logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from nyxo_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'nyxo doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'nyxo model' for available providers, or run 'nyxo doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `nyxo auth add openrouter`
    ...
    # `nyxo auth list` showing the credential while requests go out with no
    ...
    raise AuthError(
        "No inference provider configured. Run 'nyxo model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.nyxo/.env.",
...
_ALLOWED_FLASH_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.flash.com",
})


def _validate_flash_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "flash: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
...
def _flash_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_flash_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: nyxo auth add flash",
        provider="flash",
        code=reason,
...
def _log_flash_invoke_jwt_selected(
    *,
    access_token: Any,
...
def _flash_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_flash_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _flash_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_flash_invoke_jwt(
    ...
    *,
    ...
    _set_flash_agent_key_from_invoke_jwt(state)
    _log_flash_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )
...
def _flash_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("NYXO_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `nyxo model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.nyxo/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("NYXO_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set NYXO_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("NYXO_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NYXO_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("NYXO_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Nyxo times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``NYXO_LOG_LEVEL=INFO`` (or check
            # ``~/.nyxo/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>nyxo auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `nyxo auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `nyxo auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `nyxo auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `nyxo auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.nyxo/.env, and return it.
    ...
    print("       App name:     anything (e.g. nyxo-agent)")
    ...
    # Persist so subsequent `nyxo auth spotify` runs skip the wizard.
    save_env_value("NYXO_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("NYXO_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved NYXO_SPOTIFY_CLIENT_ID to ~/.nyxo/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "NYXO_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Nyxo:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Nyxo is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.nyxo/auth.json (not ~/.codex/)
#
# Nyxo maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Nyxo auth store (~/.nyxo/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `nyxo auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `nyxo auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `nyxo auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `nyxo auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``nyxo setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``nyxo auth add openai-codex``
      ...
      ``nyxo auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Nyxo auth store (~/.nyxo/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``nyxo auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Nyxo auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Nyxo auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run nyxo auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `nyxo auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Nyxo auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Nyxo keeps its OWN
        ...
        # the Codex CLI (or another Nyxo process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Nyxo's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("NYXO_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("NYXO_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Nyxo processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.nyxo/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `nyxo model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `nyxo model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `nyxo model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `nyxo model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.nyxo/auth.json even when NYXO_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.nyxo/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `nyxo model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``NYXO_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `nyxo model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Nyxo (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `nyxo model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``nyxo model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("NYXO_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
# so a new `nyxo --profile <name> auth add flash --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${NYXO_SHARED_AUTH_DIR}/flash_auth.json, defaulting to
# ``<nyxo-root>/shared/flash_auth.json`` where ``<nyxo-root>`` is what
# ``get_default_nyxo_root()`` returns — ``~/.nyxo`` on Linux/macOS,
# ``%LOCALAPPDATA%\nyxo`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's NYXO_HOME so named profiles (which
# typically live under ``<nyxo-root>/profiles/<name>/``) all see the
# same file.
#
...
def _flash_shared_auth_dir() -> Path:
    ...
    Honors ``NYXO_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<nyxo-root>/shared/``, where ``<nyxo-root>`` is what
    :func:`nyxo_constants.get_default_nyxo_root` returns — so
    Linux/macOS classic installs land at ``~/.nyxo/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\nyxo\\shared\\``, and
    Docker / custom ``NYXO_HOME`` deployments at
    ``<NYXO_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("NYXO_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from nyxo_constants import get_default_nyxo_root
    return get_default_nyxo_root() / "shared"
...
def _flash_shared_store_path() -> Path:
    ...
    # real user's Nyxo root, refuse rather than silently corrupt cross-profile
    # state. Tests must set NYXO_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from nyxo_constants import get_default_nyxo_root
        real_home_shared = (
            get_default_nyxo_root() / "shared" / FLASH_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set NYXO_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _flash_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_try_import_shared_flash_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _flash_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No NYXO_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _quarantine_flash_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_flash_state(reason)
    invalidate_flash_auth_status_cache()


def _quarantine_flash_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    if removed:
        pool["flash"] = retained
        _oauth_trace(
            "flash_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_flash_state(
    *,
    ...
    try:
        with _flash_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            # Build a full state dict so refresh_flash_oauth_from_state has every
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_flash_state(updated_state)

            refreshed = refresh_flash_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_flash_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-flash-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="flash", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="flash", relogin_required=True) from exc
    ...
    # called POST /api/oauth/token with Nyxo's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Nyxo's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Nyxo install sharing "
            "~/.nyxo/auth.json) called POST /api/oauth/token with Nyxo's "
            "refresh token without persisting the rotated token back.\n"
            "Flashrefresh tokens are single-use — only Nyxo may call the "
            "refresh endpoint. For health checks, use `nyxo auth status` "
            "instead.\n"
            "Re-authenticate with: nyxo auth add flash"
        )
        relogin = True

    raise AuthError(description, provider="flash", code=code, relogin_required=relogin)


def fetch_flash_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="flash", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Nyxo models — they're not reliable for agentic tool-calling
            if "nyxo" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _flash_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_flash_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "flash")

        if not state:
            raise AuthError(
                "Nyxo is not logged into FlashPortal.",
                provider="flash",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("NYXO_PORTAL_BASE_URL")
        ...
        with _flash_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_flash_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state(auth_store, "flash", state)
                    _save_auth_store(auth_store)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_flash_refresh_error(exc):
                        _quarantine_flash_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_flash_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
...
def refresh_flash_oauth_pure(
    ...
    *,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _flash_invoke_jwt_status(
        ...
        _assert_flash_inference_jwt_usable(state)
        _select_flash_invoke_jwt(state)

    return state


def refresh_flash_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
...
def persist_flash_credentials(
    ...
    *,
    ...
    - ``credential_pool.flash``: used by the runtime ``pool.select()`` path.

    Historically ``nyxo auth add flash`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.flash``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.flash`` then calls ``load_pool("flash")`` so
    ...
    ``nyxo auth add flash --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("flash")`` instead of the
    ...
    # these credentials via `nyxo auth add flash --type oauth`. Best-
...
def _sync_flash_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.flash changes; never fail login."""
    try:
        from agent.credential_pool import load_pool
...
def resolve_flash_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Nyxo is not logged into FlashPortal.",
        ...
        # the FLASH_INFERENCE_BASE_URL env override wins (documented dev/staging
        ...
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "flash_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if force_refresh or invoke_jwt_status is not None:
                with _flash_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_flash_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        invoke_jwt_status = _flash_invoke_jwt_status(
                            access_token,
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_flash_oauth_pure):
            ...
            _assert_flash_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_flash_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_flash_runtime_credentials_final")
...
def _empty_flash_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_flash_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_flash_auth_status()
...
# ── Process-level memo for get_flash_auth_status() ──
# get_flash_auth_status() validates state by calling resolve_flash_runtime_credentials(),
# which does a synchroflash OAuth refresh POST to portal.flash.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`nyxo tools`, status panels,
# subscription-feature checks) call it many times per render — `nyxo tools` → "All Platforms"
...
# `nyxo auth login/logout/add/remove` invalidate naturally on the next call.
...
def invalidate_flash_auth_status_cache() -> None:
    ...
    """
    global _flash_auth_status_cache
    _flash_auth_status_cache = None
...
def get_flash_auth_status() -> Dict[str, Any]:
    ...
    ``invalidate_flash_auth_status_cache()`` explicitly.
    """
    global _flash_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _flash_auth_status_cache
    ...
    status = _compute_flash_auth_status()
    _flash_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_flash_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_flash_auth_status(). See that function."""
    ...
    return _snapshot_flash_pool_status()


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `nyxo auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `nyxo auth` and
    # `nyxo model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    raw_args = os.getenv("NYXO_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``nyxo doctor`` runs the live
    ...
    try:
        from nyxo_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Nyxo' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `nyxo doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from nyxo_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `nyxo logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"flash", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from nyxo_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from nyxo_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'nyxo model' or 'nyxo setup' instead."""
    print("The 'nyxo login' command has been removed.")
    print("Use 'nyxo auth' to manage credentials,")
    print("'nyxo model' to select a provider, or 'nyxo setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.nyxo/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Nyxo-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Nyxo auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Nyxo will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("NYXO_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Nyxo will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Nyxo gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Nyxo creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Nyxo auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Nyxo auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Nyxo creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=nyxo-agent` lets xAI attribute Nyxo-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Nyxo — please report at "
            "https://github.com/Flash/nyxo-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Nyxo with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Nyxo auth store (~/.nyxo/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Nyxo login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `nyxo model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``nyxo status`` that just want to know
...
def _flash_device_code_login(
    *,
    ...
    print(f"Starting Nyxo login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_flash_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `nyxo model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_flash_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``NYXO_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_flash`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).

    Returns True iff the new token carries ``billing:manage``.
    """
    prior = get_provider_auth_state("flash") or {}
    pconfig = PROVIDER_REGISTRY["flash"]
    ...
    # Mirror to shared store + reseed the pool (best-effort), same as _login_flash.
    try:
...
def _login_flash(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if shared:
            try:
                shared_path = _flash_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
        ...
        # overwrites it to "flash".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=flash but model
        ...
        try:
            ...
            from nyxo_cli.models import (
                get_curated_flash_model_ids, get_pricing_for_provider,
                check_flash_free_tier, partition_flash_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Nyxo builds still see
        ...
        if not selected_model:
            ...
            # overwrote to "flash".  config.yaml model.provider is left
            ...
            print("  Run `nyxo model` again to switch to FlashPortal.")
            return

        config_path = _update_config_for_provider(
            "flash", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=flash)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Nyxo will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `nyxo model` or configure an API key to use Nyxo.")
        else:
