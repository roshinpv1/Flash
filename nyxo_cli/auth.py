...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: nyxo auth add nous",
        provider="nous",
        code=reason,
...
# so a new `nyxo --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${NYXO_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<nyxo-root>/shared/nous_auth.json`` where ``<nyxo-root>`` is what
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "instead.\n"
            "Re-authenticate with: nyxo auth add nous"
        )
        relogin = True
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``nyxo auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``nyxo auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `nyxo auth add nous --type oauth`. Best-
...
# which does a synchronous OAuth refresh POST to portal.flash.com. That can take
