"""
Backup and import commands for nyxo CLI.

`nyxo backup` creates a zip archive of the entire ~/.nyxo/ directory
(excluding the nyxo-agent repo and transient files).

`nyxo import` restores from a backup zip, overlaying onto the current
NYXO_HOME root.
"""
...
from nyxo_constants import get_default_nyxo_root, get_nyxo_home, display_nyxo_home
...
# ``nyxo-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/nyxo-agent/``
...
# NYXO_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "nyxo-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under NYXO_HOME) —
...
# File names that ``nyxo import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE NYXO_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside NYXO_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nyxo root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``nyxo-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/nyxo-agent/`` — must be preserved.
        if part == "nyxo-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NYXO_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Nyxo home directory."""
    nyxo_root = get_default_nyxo_root()

    if not nyxo_root.is_dir():
        print(f"Error: Nyxo home directory not found at {nyxo_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nyxo-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nyxo-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_nyxo_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(nyxo_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(nyxo_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``nyxo-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "nyxo-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nyxo_root)
    ...
    # outside NYXO_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_nyxo_home()}."
        )
    ...
    print(f"\nRestore with: nyxo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Nyxo backup.
    ...
    # Look for telltale files that a nyxo home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Nyxo backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nyxo/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nyxo dir name
        if prefix in {".nyxo", "nyxo"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nyxo_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nyxo_root / "config.yaml").exists()
        has_env = (nyxo_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Nyxo configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under NYXO_HOME.
            ...
            target = nyxo_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(nyxo_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_nyxo_home()}."
            )
        ...
        profiles_dir = nyxo_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # nyxo_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: nyxo profile list  (after installing nyxo)")

        # Guidance
        print()
        if not (nyxo_root / "nyxo-agent").is_dir():
            print("Note: The nyxo-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: nyxo update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nyxo -p {pname} gateway install")

        print("Done. Your Nyxo configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nyxo backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NYXO_HOME).
...
# — `nyxo update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(nyxo_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nyxo_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside NYXO_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``nyxo update``.
    ...
    Args:
        ...
        nyxo_home: Override for the Nyxo home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(nyxo_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for nyxo backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_nyxo_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, nyxo_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``nyxo_root`` to ``out_path``.
...
def _pre_update_backup_dir(nyxo_home: Optional[Path] = None) -> Path:
    home = nyxo_home or get_nyxo_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NYXO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NYXO_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``nyxo update``) should continue even if the backup fails.
    """
    nyxo_root = nyxo_home or get_default_nyxo_root()
    if not nyxo_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(nyxo_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, nyxo_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nyxo claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NYXO_HOME under ``backups/`` before a
    ``nyxo claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nyxo import <archive>``.  Writes to
    ``<NYXO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nyxo import` and the
