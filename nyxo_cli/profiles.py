"""
Profile management for multiple isolated Nyxo instances.

Each profile is a fully independent NYXO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nyxo/profiles/<name>/`` by default.

The "default" profile is ``~/.nyxo`` itself — backward compatible,
zero migration needed.

Usage::

    nyxo profile create coder          # fresh profile + bundled skills
    nyxo profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nyxo profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nyxo -p coder chat                 # or via flag
    nyxo profile use coder             # set as sticky default
    nyxo profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See nyxo_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `nyxo profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `nyxo
...
# `nyxo skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Nyxo infrastructure directories that only the default profile
         (``~/.nyxo``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_nyxo_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.nyxo) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nyxo-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "nyxo", "default", "test", "tmp", "root", "sudo",
})

# Nyxo subcommands that cannot be used as profile names/aliases
_NYXO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nyxo root, NOT to the current NYXO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NYXO_HOME points outside
    ``~/.nyxo``, profiles live under ``NYXO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_nyxo_home() / "profiles"


def _get_default_nyxo_home() -> Path:
    """Return the default (pre-profile) NYXO_HOME path.

    In standard deployments this is ``~/.nyxo``.
    In Docker/custom deployments where NYXO_HOME is outside ``~/.nyxo``
    (e.g. ``/opt/data``), returns NYXO_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_nyxo_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Nyxo installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NYXO_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nyxo_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, nyxo subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _NYXO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nyxo subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            nyxo_exe = shutil.which("nyxo") or "nyxo"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(nyxo_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Nyxo. If we
    ...
    try:
        ...
        # not be migrated. The next `nyxo doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``nyxo -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``nyxo profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nyxo profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``NYXO_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Nyxo config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_nyxo_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, nyxo_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own NYXO_HOME.
    ...
    The returned ``nyxo_home`` is the path to pass to
    ``set_nyxo_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_nyxo_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nyxo update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nyxo)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nyxo/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `nyxo -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Nyxo profile.\n"
    ...
    if not soul_path.exists():
        try:
            from nyxo_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `nyxo update`'s
    ...
    # Cloned configs can be older than the running Nyxo (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `nyxo profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `nyxo -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NYXO_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``nyxo profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NYXO_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_nyxo_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nyxo).\n"
            "To remove everything, use: nyxo uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``NYXO_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from nyxo_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NYXO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NYXO_HOME")
    try:
        os.environ["NYXO_HOME"] = str(profile_dir)
        from nyxo_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NYXO_HOME"] = old_home
        elif "NYXO_HOME" in os.environ:
            del os.environ["NYXO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nyxo/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nyxo profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NYXO_HOME.

    Returns ``"default"`` if NYXO_HOME is not set or points to ``~/.nyxo``.
    Returns the profile name if NYXO_HOME points into ``~/.nyxo/profiles/<name>``.
    Returns ``"custom"`` if NYXO_HOME is set to an unrecognized path.
    """
    from nyxo_constants import get_nyxo_home
    nyxo_home = get_nyxo_home()
    resolved = nyxo_home.resolve()

    default_resolved = _get_default_nyxo_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nyxo itself — its parent is ~/ and its
        # directory name is ".nyxo", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nyxo profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.nyxo itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nyxo). "
            "Specify a different name: nyxo profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="nyxo_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"nyxo_{old_name}"
    legacy_old_host = f"nyxo.{old_name}"
    new_host = f"nyxo_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_nyxo_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("nyxo_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NYXO_HOME path string.

    Called early in the CLI entry point, before any nyxo modules
    are imported, to set the NYXO_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
