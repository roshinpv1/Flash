"""Windows ConPTY bridge for the `nyxo dashboard` chat tab.

Drop-in counterpart to ``nyxo_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``nyxo_cli.web_server`` consumes: ``spawn``, ``read``,
