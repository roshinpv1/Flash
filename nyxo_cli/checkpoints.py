"""`nyxo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nyxo/checkpoints/``.  Actions:

    nyxo checkpoints               # same as `status`
    nyxo checkpoints status        # total size, project count, breakdown
    nyxo checkpoints list          # per-project checkpoint counts + workdir
    nyxo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nyxo checkpoints clear [-f]    # nuke the entire base (asks first)
    nyxo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nyxo checkpoints
    nyxo checkpoints prune --retention-days 3 --max-size-mb 200
    nyxo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: nyxo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nyxo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nyxo checkpoints` → status
