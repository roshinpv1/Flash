"""``nyxo slack ...`` CLI subcommands.

Today only ``nyxo slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ nyxo slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Nyxo. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Nyxo deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from nyxo_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Nyxo in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Nyxo agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``nyxo_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$NYXO_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Nyxo")
    ...
    """
    name = getattr(args, "name", None) or "Nyxo"
    description = getattr(args, "description", None) or "Your Nyxo agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("NYXO_HOME") or str(Path.home() / ".nyxo")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Nyxo app\n"
