...
Pure logic: walks a Nyxo config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `nyxo doctor` and a future `nyxo migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Nyxo config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Nyxo config and return retirement issues.
