"""
MCP Server Management CLI — ``nyxo mcp`` subcommand.

Implements ``nyxo mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.nyxo/config.yaml under the ``mcp_servers`` key.
"""
...
from nyxo_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_nyxo_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from nyxo_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.nyxo/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``nyxo mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return NyxoTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # nyxo_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  nyxo mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  nyxo mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  nyxo mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_nyxo_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: nyxo mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_nyxo_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `nyxo mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  nyxo mcp add <name> --url <endpoint>')
        _info('  nyxo mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `nyxo mcp remove` + `nyxo mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `nyxo mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'nyxo mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``nyxo mcp`` subcommands."""
    ...
    if action == "picker":
        from nyxo_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from nyxo_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from nyxo_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `nyxo plugin`.
        from nyxo_cli.mcp_picker import run_picker
        run_picker()
        ...
        _info("nyxo mcp add <name> --command <cmd>         Add a stdio server")
        _info("nyxo mcp add <name> --preset <preset>       Add from a known preset")
        _info("nyxo mcp remove <name>                      Remove a server")
        _info("nyxo mcp list                               List configured servers")
        _info("nyxo mcp test <name>                        Test connection")
        _info("nyxo mcp configure <name>                   Toggle tools")
        _info("nyxo mcp login <name>                       Re-authenticate OAuth")
        print()
