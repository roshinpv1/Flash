"""CLI handlers for the ``nyxo proxy`` subcommand."""
...
from nyxo_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "nyxo proxy requires aiohttp. Install one of:\n"
        "  pip install 'nyxo-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "flash"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"nyxo auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Nyxo proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Nyxo proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: nyxo proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``nyxo proxy <subcommand>``."""
    ...
    print(
        "nyxo proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  nyxo proxy start [--provider flash|xai] [--host 127.0.0.1] [--port 8645]\n"
