...
# xAI's public API is OpenAI-compatible for the endpoints Nyxo commonly
# uses. The Responses endpoint is included because Nyxo' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Nyxo-managed OAuth credentials."""

    auth_hint = "nyxo auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`nyxo auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`nyxo auth reset xai-oauth` or re-authenticate with "
                    "`nyxo auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `nyxo auth add xai-oauth --type oauth`."
            )
