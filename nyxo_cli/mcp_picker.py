"""MCP picker — interactive `nyxo mcp picker` (also the default `nyxo mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``nyxo mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `nyxo plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from nyxo_cli.cli_output import prompt_yes_no
from nyxo_cli.curses_ui import curses_single_select
from nyxo_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Nyxo session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from nyxo_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `nyxo mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: nyxo mcp install <name>    Picker: nyxo mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Nyxo is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Nyxo — run `nyxo update` "
...
def show_catalog() -> None:
    """`nyxo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`nyxo mcp picker` (and default `nyxo mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`nyxo mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `nyxo mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
