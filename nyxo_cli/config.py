"""
Configuration management for Nyxo Agent.

Config files are stored in ~/.nyxo/ for easy access:
- ~/.nyxo/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nyxo/.env         - API keys and secrets

This module provides:
- nyxo config          - Show current configuration
- nyxo config edit     - Open config in editor
- nyxo config set      - Set a specific value
- nyxo config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``nyxo config set`` (which rewrites
    ...
    ``config.yaml`` in place: nyxo never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.nyxo/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nyxo logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nyxo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nyxo makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nyxo itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nyxo update``, the TUI build.
...
#   on every plugin install / ``nyxo update``.
...
# * ``NYXO_HOME`` / ``NYXO_PROFILE`` / ``NYXO_CONFIG`` /
#   ``NYXO_ENV`` — Nyxo runtime location flags. Writing these into
...
#
# IMPORTANT: ``NYXO_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NYXO_LANGFUSE_PUBLIC_KEY,
# NYXO_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONFLASHERSITE",
    # Node
    ...
    # Nyxo runtime location — never via dashboard env writer.
    # NOT a NYXO_* blanket: integration credentials (NYXO_GEMINI_*,
    # NYXO_LANGFUSE_*, NYXO_SPOTIFY_*, ...) ARE allowed.
    "NYXO_HOME", "NYXO_PROFILE", "NYXO_CONFIG", "NYXO_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Nyxo runtime location "
            "(NYXO_HOME, NYXO_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "NYXO_TOOL_PROGRESS", "NYXO_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `nyxo plugins enable
    # observability/langfuse` or `nyxo tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NYXO_LANGFUSE_ENV",
    "NYXO_LANGFUSE_RELEASE",
    "NYXO_LANGFUSE_SAMPLE_RATE",
    "NYXO_LANGFUSE_MAX_CHARS",
    "NYXO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from nyxo_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NYXO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nyxo_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Nyxo is running in package-manager-managed mode.

    Two signals: the NYXO_MANAGED env var (set by the systemd service),
    or a .managed marker file in NYXO_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``nyxo_cli/`` — i.e. the git checkout for source
    installs, ``/opt/nyxo`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$NYXO_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nyxo was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$NYXO_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. NYXO_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.nyxo``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$NYXO_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.nyxo:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$NYXO_HOME``, a
    ...
    ``docker`` and ``nyxo update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $NYXO_HOME.
    try:
    ...
    #    host install whose shared $NYXO_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``nyxo update``.
    try:
        method = (
            (get_nyxo_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``nyxo_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$NYXO_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/nyxo`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Nyxo lives in a ``uv tool`` layout.

    ``uv tool install nyxo-agent`` places the install at
    ``.../uv/tools/nyxo-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``nyxo-agent`` happens to be uv-tool-installed on the machine while
    the *active* Nyxo is a regular pip/venv install, causing
    ``nyxo update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/nyxo-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade nyxo-agent"
        return "pip install --upgrade nyxo-agent"
    return "nyxo update"
...
# Long-form text for ``nyxo update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# nyxo_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Nyxo, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nyxo update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nyxo installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NYXO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Nyxo installation is managed by NixOS "
            f"(NYXO_MANAGED={env_hint}).\n"
            "Edit services.nyxo-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Nyxo installation is managed by Homebrew "
            f"(NYXO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nyxo-agent"
        )

    return (
        f"Cannot {action}: this Nyxo installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Nyxo."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NYXO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nyxo_bin
    or None if container mode is not active, we're already inside the
    container, or NYXO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NYXO_DEV") == "1":
        return None
    ...
    container_mode_file = get_nyxo_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nyxo-agent")
    exec_user = info.get("exec_user", "nyxo")
    nyxo_bin = info.get("nyxo_bin", "/data/current-package/bin/nyxo")
...
# Re-export from nyxo_constants — canonical definition lives there.
from nyxo_constants import get_nyxo_home  # noqa: F811,E402
...
def get_env_path() -> Path:
    ...
    return get_nyxo_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_nyxo_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NYXO_UID / NYXO_GID env vars set by Docker deployments.

    Docker containers running Nyxo commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NYXO_HOME once, but
    subdirectories created at runtime by ``ensure_nyxo_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NYXO_UID", "").strip()
    gid_str = os.environ.get("NYXO_GID", "").strip()
    try:
...
def _chown_to_nyxo_uid(path) -> None:
    """Chown ``path`` to ``NYXO_UID:NYXO_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_nyxo_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_nyxo_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nyxo group can
    share state with the gateway service.

    The mode can be overridden via the NYXO_HOME_MODE environment variable
    (e.g. NYXO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NYXO_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NYXO_UID``/``NYXO_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NYXO_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_nyxo_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Nyxo runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NYXO_CONTAINER") or os.environ.get("NYXO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NYXO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NYXO_HOME, upgrading legacy empty templates.
...
def ensure_nyxo_home():
    """Ensure ~/.nyxo directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_nyxo_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_nyxo_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NYXO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Nyxo-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nyxo
        ...
        # identity slot (SOUL.md). Empty by default. The NYXO_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Nyxo adds a coding operating brief
    ...
    "terminal": {
        ...
        #             NYXO_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NYXO_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Nyxo auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nyxo sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nyxo
        ...
        # bundled Nyxo image, which drops to the `nyxo` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Nyxo sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Nyxo should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nyxo chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nyxo/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nyxo sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .nyxo.md, or .cursorrules before Nyxo applies
    ...
    # single read_file page exceeds these limits, Nyxo truncates the
    ...
    # See tools/kanban_tools.py and nyxo_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``nyxo dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``nyxo kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nyxo kanban decompose`` and the kanban
        ...
        # ``nyxo profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `nyxo model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `nyxo` (and `nyxo chat`) launches by default:
        ...
        # REPL and `--tui` (or NYXO_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `nyxo --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nyxo -c` muscle memory.  Default off so existing
        # users aren't surprised.  NYXO_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nyxo --tui` drops a one-time hint
        ...
        # Flashcredits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # e.g. `model · 68% · ~/projects/nyxo`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `nyxo pets`. Disabled until
        ...
        "pet": {
            ...
            # get_nyxo_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled FlashPortal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``NYXO_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NYXO_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``NYXO_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``NYXO_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/nyxo``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Nyxo appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nyxo/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nyxo/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Flash, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Nyxo feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nyxo auto-pauses the goal and
    ...
    # always goes to ~/.nyxo/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NYXO_SKILL_DIR} and ${NYXO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nyxo curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `nyxo curator run --consolidate` overrides
        ...
        # normally restored on every `nyxo update`, so pruning them only
        ...
        # skipped), snapshot ~/.nyxo/skills/ into
        # ~/.nyxo/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nyxo curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nyxo-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nyxo Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NYXO_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Nyxo' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.nyxo/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NYXO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `nyxo doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nyxo_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nyxo to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $NYXO_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing FlashPortal token — there is no token key here.
        "chronos": {
        ...
        # Also overridable via NYXO_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `nyxo -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `nyxo` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `nyxo kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nyxo-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nyxo tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.nyxo/logs/.
    ...
    # curated model lists for OpenRouter and FlashPortal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a nyxo-agent release.
    ...
    "gateway": {
        ...
        # the NYXO_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nyxo/.env,
        ...
        # behavior: files must live under the Nyxo cache, under
        ...
        # user. Bridged to NYXO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nyxo
        # cache (~/.nyxo/cache/{documents,images,audio,video,screenshots})
        ...
        # to NYXO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NYXO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NYXO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nyxo/state.db.
    ...
    "sessions": {
        ...
        # default of ``nyxo sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nyxo/sessions/session_{sid}.json`` on every turn
    ...
    # ``nyxo update`` behaviour.
    "updates": {
        # Run a full ``nyxo backup``-style zip of NYXO_HOME before every
        # ``nyxo update``.  Backups land in ``<NYXO_HOME>/backups/`` and
        # can be restored with ``nyxo import <path>``.  Defaults to true
        # after the #48200 incident: a ``nyxo update --yes`` run that
        ...
        "backup_keep": 5,
        # What `nyxo update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<NYXO_HOME>/lsp/bin/`` on first use.
    ...
    # `nyxo tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nyxo/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.nyxo/.env (or your shell) and never in config.yaml.
            ...
            # ~/.nyxo/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Nyxo disables it for our users unless they
        # explicitly opt in here. When false (default), Nyxo sets
...
# selection step (FlashPortal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "FLASH_BASE_URL": {
        "description": "FlashPortal base URL override",
        "prompt": "FlashPortal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nyxo model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for FlashSubscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for FlashSubscribers only, used to derive vendor hosts, e.g. flash.com -> firecrawl-gateway.flash.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for FlashSubscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit FlashSubscriber access token for tool-gateway requests (optional; otherwise read from the Nyxo auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Nyxo, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nyxo:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NYXO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nyxo)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nyxo-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nyxo-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nyxo API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nyxo API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nyxo API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NYXO_TOOL_PROGRESS and NYXO_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nyxo.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nyxo update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nyxo versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nyxo_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nyxo setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nyxo won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nyxo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NYXO_HOME", "~/.nyxo")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NYXO_TOOL_PROGRESS")
            old_mode = get_env_value("NYXO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NYXO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NYXO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NYXO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NYXO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NYXO_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_nyxo_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nyxo plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nyxo curator status` has no
    ...
    #   3. Creates `~/.nyxo/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nyxo_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_nyxo_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from nyxo_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nyxo config set <key> <value>")
    ...
    # metadata.nyxo.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``nyxo config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nyxo_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nyxo/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nyxo/config.yaml.
    ...
    (which change ``NYXO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/nyxo/config.yaml invalidates the
...
#   flash         (OAuth — nyxo auth) — FlashPortal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.nyxo/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nyxo/.env.
    ...
    interactive menu render (`nyxo tools`, `nyxo setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nyxo tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Nyxo env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nyxo/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nyxo/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nyxo/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nyxo/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Nyxo — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nyxo vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nyxo/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Nyxo Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from nyxo_cli.auth import get_anthropic_key
    ...
    # Warn on stale NYXO_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("NYXO_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale NYXO_MAX_ITERATIONS={_env_ghost} "
                f"(run 'nyxo doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  nyxo config edit     # Edit config file", Colors.DIM))
    print(color("  nyxo config set <key> <value>", Colors.DIM))
    print(color("  nyxo setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `nyxo config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `nyxo config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nyxo config set <key> <value>")
            print()
            print("Examples:")
            print("  nyxo config set model anthropic/claude-sonnet-4")
            print("  nyxo config set terminal.backend docker")
            print("  nyxo config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nyxo config migrate' to add them")
        ...
        print("  nyxo config           Show current configuration")
        print("  nyxo config edit      Open config in editor")
        print("  nyxo config set <key> <value>   Set a config value")
        print("  nyxo config check     Check for missing/outdated config")
        print("  nyxo config migrate   Update config with new options")
        print("  nyxo config path      Show config file path")
        print("  nyxo config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nyxo config`` UI so users
