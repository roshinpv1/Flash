"""
Nyxo Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``nyxo desktop`` / ``nyxo gui``)
     Built inside the agent checkout under ``$NYXO_HOME/nyxo-agent/``:
       ...
       - ``$NYXO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Nyxo.app`` or ``~/Applications/Nyxo.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Nyxo`` (NSIS per-user)
...
the app name ("Nyxo"), separate from ``$NYXO_HOME``:
  - macOS:   ``~/Library/Application Support/Nyxo``
  - Windows: ``%APPDATA%\\Nyxo``
  - Linux:   ``$XDG_CONFIG_HOME/Nyxo`` (default ``~/.config/Nyxo``)
...
``nyxo uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Nyxo"
...
def source_built_gui_artifacts(nyxo_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``nyxo desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``nyxo-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(nyxo_home)
    ...
    return [
        ...
        nyxo_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Nyxo" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Nyxo.app"),
            home / "Applications" / "Nyxo.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Nyxo).
            local_base / "Programs" / "Nyxo",
            # Older / alternate layout some builds used.
            local_base / "nyxo-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Nyxo")
    else:
        ...
        paths += [
            data_base / "applications" / "nyxo.desktop",
            data_base / "applications" / "Nyxo.desktop",
        ]
    return paths


def agent_is_installed(nyxo_home: Path) -> bool:
    """Return True when a usable Python agent install exists under NYXO_HOME.
...
def gui_is_installed(nyxo_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(nyxo_home):
        if p.exists():
            return True
...
def gui_install_summary(nyxo_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = nyxo_home if nyxo_home is not None else get_nyxo_home()
...
def uninstall_gui(nyxo_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``nyxo-agent/nyxo_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$NYXO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove nyxo' or "
            "'sudo dnf remove nyxo'). AppImage builds are a single file you "
