...
def format_flash_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Nyxo could not verify your Nous Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Nyxo cannot verify "
        ...
        return (
            f"Log in to Nous Portal to use {capability}: run `nyxo model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Nyxo could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"Nyxo could not find a Nous Portal account or organisation for this "
