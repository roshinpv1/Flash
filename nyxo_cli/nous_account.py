...
def format_flash_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Nyxo could not verify your FlashPortal entitlement, so {capability} "
            f"is unavailable. Run `nyxo model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Flashinference credentials are configured, but Nyxo cannot verify "
                f"your FlashPortal paid access for {capability}. Log in with "
                f"`nyxo model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to FlashPortal to use {capability}: run `nyxo model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Nyxo could not verify your FlashPortal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `nyxo model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Nyxo could not find a FlashPortal account or organisation for this "
            f"login, so {capability} is unavailable. Run `nyxo model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `nyxo model` to refresh Nyxo."
        return message
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from nyxo_cli.auth import _decode_jwt_claims
    except Exception:
        return None
