"""nyxo hooks — inspect and manage shell-script hooks.

Usage::

    nyxo hooks list
    nyxo hooks test <event> [--for-tool X] [--payload-file F]
    nyxo hooks revoke <command>
    nyxo hooks doctor

Consent records live under ``~/.nyxo/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nyxo/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nyxo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nyxo hooks {list|test|revoke|doctor}")
        print("Run 'nyxo hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nyxo/config.yaml.")
        print("See `nyxo hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nyxo hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nyxo hooks test` and `nyxo hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Nyxo wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nyxo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nyxo hooks doctor` would execute
