...
# coverage category (nyxo_cli.flash_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `nyxo tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("NYXO_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_flash_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `nyxo model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("NYXO_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("NYXO_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from nyxo_cli.setup import prompt_checklist
    except Exception:
        return set()
