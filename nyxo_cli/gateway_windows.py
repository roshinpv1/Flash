...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m nyxo_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Nyxo_Gateway"
_TASK_DESCRIPTION = "Nyxo Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Nyxo profile."""
    from nyxo_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "nyxo_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("NYXO_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("NYXO_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("NYXO_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["NYXO_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["NYXO_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["NYXO_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("NYXO_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("NYXO_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("NYXO_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Nyxo_Gateway``
    Named profile X: ``Nyxo_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during nyxo_cli boot.
    from nyxo_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\nyxo\\gateway-service\\<task_name>.cmd``
    (or ``<NYXO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Nyxo installs stay self-contained).
    """
    _assert_windows()
    from nyxo_cli.config import get_nyxo_home

    script_dir = Path(get_nyxo_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``NYXO_HOME`` whenever it
    ...
    source checkout only if ``NYXO_HOME`` cannot be resolved yet.
    """
    from nyxo_cli.config import get_nyxo_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NYXO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m nyxo_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "NYXO_HOME={nyxo_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "NYXO_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports nyxo_constants-based logic during startup.
    ...
    prog_args = [pythonw_path, "-m", "nyxo_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('NYXO_HOME')}) = {_quote_vbs_string(nyxo_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('NYXO_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
...
def _write_task_script() -> Path:
    ...
    nyxo_home = str(Path(get_nyxo_home()).resolve())
    profile_arg = _profile_arg(nyxo_home)

    content = _build_gateway_cmd_script(python_path, working_dir, nyxo_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, nyxo_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Nyxo builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "nyxo_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m nyxo_cli.main gateway run``
    ...
    log_dir = Path(get_nyxo_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("NYXO_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("NYXO_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from nyxo_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `nyxo -p <profile> gateway install` must be safe.
    ...
    from nyxo_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"nyxo {profile_arg} gateway start" if profile_arg else "nyxo gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: nyxo gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Nyxo gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: nyxo gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: nyxo gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from nyxo_cli.config import get_nyxo_home
        print(f"    type {Path(get_nyxo_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_nyxo_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from nyxo_cli.config import get_nyxo_home

    nyxo_home = Path(get_nyxo_home()).resolve()
    print()
    print("Next steps:")
    print("  nyxo gateway status                      # Check status")
    print(f"  type {nyxo_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Nyxo gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: nyxo gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_nyxo_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: nyxo gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: nyxo gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from nyxo_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from nyxo_cli.gateway import kill_gateway_processes
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `nyxo gateway status`."
        )
