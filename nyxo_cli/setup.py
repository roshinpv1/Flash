"""
Interactive setup wizard for Nyxo Agent.
...
_DOCS_BASE = "https://nyxo-agent.flash.com/docs"
...
# display_nyxo_home imported lazily at call sites (stale-module safety during nyxo update)
...
from nyxo_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nyxo_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Nyxo Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Nyxo using environment variables or config commands:")
    print_info("  nyxo config set model.provider custom")
    print_info("  nyxo config set model.base_url http://localhost:8080/v1")
    print_info("  nyxo config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nyxo setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``NYXO_NONINTERACTIVE=1`` (see ``nyxo_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("NYXO_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``NYXO_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nyxo_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nyxo setup')")


def _print_setup_summary(config: dict, nyxo_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nyxo setup' to configure"))
    ...
    if subscription_features.video_gen.managed_by_flash:
        ...
        try:
            ...
            from nyxo_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_flash:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nyxo setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nyxo setup tts'"))
    else:
    ...
    # Spotify (OAuth via nyxo auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nyxo setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {nyxo_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('nyxo setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nyxo setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nyxo setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nyxo setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nyxo setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nyxo config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nyxo config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nyxo config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nyxo', Colors.GREEN)}              Start chatting")
    print(f"   {color('nyxo gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nyxo doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nyxo tools` and `nyxo setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nyxo model``)
    ...
    provider added to ``nyxo model`` is automatically available here.
    ...
    # Delegate to the shared nyxo model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from nyxo_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nyxo model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from nyxo_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "flash-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.nyxo/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Nyxo auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via nyxo setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nyxo setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Nyxo runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nyxo setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `nyxo setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NYXO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NYXO_MAX_ITERATIONS")
    ...
    print_info("  Run `nyxo setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NYXO_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from nyxo_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_nyxo_home(Path(get_nyxo_home()))
    except Exception:
        pass
...
def _profile_name_from_nyxo_home(nyxo_home) -> str | None:
    """Return the active profile name when NYXO_HOME is a profile dir."""
    if nyxo_home.parent.name == "profiles":
        return nyxo_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Nyxo delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Nyxo to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nyxo_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nyxo-agent.flash.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nyxo-agent.flash.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nyxo config edit")
    print_info("   Open config in your editor:  nyxo config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from nyxo_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Nyxo from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nyxo setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nyxo config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_nyxo_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo nyxo gateway install --system")
                print_info("  Or run in foreground:  nyxo gateway")
        else:
            from nyxo_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   nyxo gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   nyxo gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nyxo setup tools` and `nyxo tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``nyxo_cli.auth`` — lists every supported
    ...
    """
    try:
        from nyxo_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nyxo_cli.config
    ...
    if section_key == "model":
        ...
        from nyxo_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_nyxo migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nyxo", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Nyxo.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Nyxo to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Nyxo at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Nyxo at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Nyxo at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Nyxo at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Nyxo equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Nyxo config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Nyxo.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Nyxo's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(nyxo_home: Path) -> bool:
    ...
    print_info("Nyxo can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nyxo claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nyxo claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Nyxo configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Nyxo config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Nyxo (use nyxo claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``nyxo setup --portal`` and ``nyxo portal``. This is the
    ...
    Nyxo session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⚕ Nyxo Setup — FlashPortal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `nyxo portal` == quick setup's Flashstep.
    try:
    ...
    print_info("  Run `nyxo portal info` to inspect routing.")
    print_info("  Run `nyxo` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nyxo setup           — full or quick (auto-detected)
      nyxo setup model     — just model/provider
      nyxo setup tts       — just text-to-speech
      nyxo setup terminal  — just terminal backend
      nyxo setup gateway   — just messaging platforms
      nyxo setup tools     — just tool configuration
      nyxo setup agent     — just agent settings
    """
    from nyxo_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Nyxo Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Nyxo Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Nyxo Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, nyxo_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Nyxo configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nyxo setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(nyxo_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Nyxo?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, nyxo_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, nyxo_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {nyxo_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nyxo config edit'")
    ...
    # Tune later with `nyxo setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, nyxo_home)


def _run_first_time_quick_setup(config: dict, nyxo_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``nyxo setup <section>``
    or switch providers with ``nyxo model``.
    """
    from nyxo_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nyxo setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nyxo setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nyxo setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``nyxo tools`` (which
    ...
    try:
        ...
        from nyxo_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``nyxo setup agent`` /
    ``nyxo config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, nyxo_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `nyxo update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        nyxo tools")
        print_info("  Seed skills:         nyxo skills opt-in --sync")
        print_info("  Add MCP servers:     nyxo mcp add")
        print_info("  Enable plugins:      nyxo plugins")
        print_info("  Tune agent settings: nyxo setup agent")
        print()
        _print_setup_summary(config, nyxo_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, nyxo_home)


def _blank_slate_walkthrough(config: dict, nyxo_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`nyxo update` runs from re-injecting them. Opt back in any")
            print_info("time with `nyxo skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `nyxo tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `nyxo plugins list` / `nyxo plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `nyxo plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `nyxo mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `nyxo mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   nyxo tools")
    print_info("  Seed skills:         nyxo skills opt-in --sync")
    print_info("  Add MCP servers:     nyxo mcp add")
    print_info("  Tune agent settings: nyxo setup agent")
    print()
...
def _run_quick_setup(config: dict, nyxo_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nyxo setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Nyxo to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nyxo setup gateway'.")
