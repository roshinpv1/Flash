"""Profile distributions — shareable, packaged Nyxo profiles via git.

A distribution is a Nyxo profile published as a git repository (or
...
* ``nyxo profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nyxo skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nyxo profile``, not a parallel tree):

    nyxo profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "nyxo-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nyxo_requires=str(data.get("nyxo_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nyxo_requires:
            out["nyxo_requires"] = self.nyxo_requires
        if self.author:
...
def check_nyxo_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Nyxo {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nyxo profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Nyxo profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Nyxo distribution."
        )

    # Version check up-front so we fail fast
    check_nyxo_requires(manifest.nyxo_requires, nyxo_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nyxo).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="nyxo_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nyxo profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nyxo profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="nyxo_dist_update_") as tmp:
        plan = plan_install(
