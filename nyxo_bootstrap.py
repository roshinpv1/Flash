"""Windows UTF-8 bootstrap for Nyxo entry points.
...
should be imported at the very top of every Nyxo entry point
(``nyxo``, ``nyxo-agent``, ``nyxo-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Nyxo modules.

    Nyxo ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Nyxo source root
    ...
    """
    root = src_root or os.environ.get("NYXO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``NYXO_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("NYXO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import nyxo_bootstrap``
# (or ``from nyxo_bootstrap import apply_windows_utf8_bootstrap``) at
