#!/usr/bin/env python3
"""Audit WebUI dependencies on the flash-agent source tree.
...
def _import_kind(module_name: str) -> str:
    ...
    if module_name == "flash_state" or module_name.startswith("flash_state."):
        return "state_import"
    if module_name == "flash_constants" or module_name.startswith("flash_constants."):
        return "constants_import"
    if module_name.startswith("flash_cli.runtime_provider"):
    ...
    if module_name.startswith(("flash_cli.models", "agent.account_usage")):
        return "provider_model_catalog_import"
    if module_name.startswith(("flash_cli.auth", "flash_cli.config", "agent.credential_pool")):
        return "auth_config_credential_import"
    if module_name.startswith(("agent.skill_utils", "flash_cli.plugins", "flash_cli.profiles", "flash_cli.goals")):
...
def build_report(root: Path) -> dict[str, object]:
    ...
    docker_findings = _iter_text_matches(
        ...
        (
            ("agent_source_volume", r"flash-agent-src"),
            ("agent_source_path", r"/opt/flash(?:\b|[-/])?"),
        ),
    )
    ...
    classes = (
        DependencyClass(
            ...
            current_dependency=(
                ...
                "the flash-agent-src volume and /opt/flash."
            ),
            replacement_surface=(
                "Remove the WebUI source mount after startup install and runtime "
                "imports move to flash-agent endpoints or a versioned client package."
            ),
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI startup discovers HERMES_WEBUI_AGENT_DIR or ~/.flash/flash-agent "
                "and installs the agent checkout extras."
            ),
            replacement_surface=(
                "Replace source-tree pip installs with a packaged flash-agent client "
        ...
        DependencyClass(
            ...
            current_dependency=(
                "Browser chat, approvals, tools, and scheduled work import Flash Agent "
            ...
            replacement_surface=(
                ...
                "Flash Agent APIs or a versioned client package before removing source mounts."
            ),
            findings=_findings_by_kind(
                runtime_findings,
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI imports flash_state.SessionDB and related state helpers directly "
                "to read or write agent session state."
            ),
            replacement_surface=(
                "Move cross-container state reads/writes behind flash-agent session/state "
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI imports flash_cli.runtime_provider and agent adapter helpers "
                "to resolve providers and normalize gateway calls."
            ),
            replacement_surface=(
                "Route provider resolution and gateway invocation through flash-agent "
        ...
        DependencyClass(
            ...
            current_dependency=(
                "WebUI imports flash_cli and agent helpers for config/auth status, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Path to the flash-webui checkout; defaults to this script's repo.",
    )
    parser.add_argument(
        "--format",
