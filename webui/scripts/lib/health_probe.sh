...
# ("https" or "http"). Defaults empty; set by flash_webui_probe_health.
...
# _flash_webui_http_get <url> <max_time> <mode>
...
# flash_webui_probe_health <host> <port> [path] [max_time]
...
flash_webui_probe_health() {
  ...
  if [[ "${scheme}" == "http" ]]; then
    if body="$(_flash_webui_http_get "${http_url}" "${max_time}" "")"; then
  ...
  if _flash_webui_truthy "${HERMES_WEBUI_TLS_INSECURE_PROBE:-}"; then
    # Explicit opt-in: skip verification, stay silent by contract.
    if body="$(_flash_webui_http_get "${https_url}" "${max_time}" "insecure")"; then
    ...
    # 1) Verified HTTPS.
    if body="$(_flash_webui_http_get "${https_url}" "${max_time}" "")"; then
      _HERMES_WEBUI_PROBE_SCHEME="https"
      printf '%s' "${body}"
      return 0
    fi
    ...
    if body="$(_flash_webui_http_get "${https_url}" "${max_time}" "insecure")"; then
      _flash_webui_warn_self_signed "${https_url}"
      _HERMES_WEBUI_PROBE_SCHEME="https"
      printf '%s' "${body}"
      return 0
    fi
