...
# WSL-friendly autostart launcher for Flash WebUI.
#
# Safe defaults:
...
# - writes logs under ~/.flash/webui/logs unless HERMES_WEBUI_LOG_DIR is set

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_REPO="$(cd "${SCRIPT_DIR}/../.." && pwd)"
HERMES_WEBUI_REPO="${HERMES_WEBUI_REPO:-${DEFAULT_REPO}}"
HERMES_WEBUI_LOG_DIR="${HERMES_WEBUI_LOG_DIR:-${HOME}/.flash/webui/logs}"
...
HERMES_WEBUI_HEALTH_URL="${HERMES_WEBUI_HEALTH_URL:-$(flash_webui_probe_scheme)://${HERMES_WEBUI_HEALTH_HOST}:${HERMES_WEBUI_PORT}/health}"
HERMES_WEBUI_PID_FILE="${HERMES_WEBUI_PID_FILE:-${HERMES_WEBUI_LOG_DIR}/flash-webui.pid}"
HERMES_WEBUI_LOCK_FILE="${HERMES_WEBUI_LOCK_FILE:-/tmp/flash-webui-autostart.lock}"
AUTOSTART_LOG="${HERMES_WEBUI_LOG_DIR}/webui_autostart.log"
WEBUI_LOG="${HERMES_WEBUI_LOG_DIR}/flash_webui.log"
...
webui_healthy() {
  ...
  flash_webui_probe_health "${HERMES_WEBUI_HEALTH_HOST}" "${HERMES_WEBUI_PORT}" "/health" 3 >/dev/null 2>&1
}
...
validate_repo() {
  if [[ ! -d "${HERMES_WEBUI_REPO}" ]]; then
    log "Flash WebUI repo not found: ${HERMES_WEBUI_REPO}"
    exit 1
  fi
...
maybe_require_agent_process() {
  # Flash WebUI usually launches the agent in-process, so this check is opt-in.
  # Set HERMES_WEBUI_REQUIRE_AGENT_PROCESS=1 only if your setup depends on a
  # separately running Flash gateway/agent before WebUI starts.
  if [[ "${HERMES_WEBUI_REQUIRE_AGENT_PROCESS:-0}" != "1" ]]; then
    return 0
  fi
  if ! pgrep -f "flash" >/dev/null 2>&1; then
    log "HERMES_WEBUI_REQUIRE_AGENT_PROCESS=1 but no Flash process is running; skipping start"
    exit 1
  fi
}
...
start_webui() {
  ...
  if webui_healthy; then
    log "Flash WebUI already running at ${HERMES_WEBUI_HEALTH_URL}"
    exit 0
  fi

  if pid_is_alive; then
    log "Flash WebUI already running with pid $(cat "${HERMES_WEBUI_PID_FILE}")"
    exit 0
  fi

  rm -f "${HERMES_WEBUI_PID_FILE}"
  log "Starting Flash WebUI from ${HERMES_WEBUI_REPO} on ${HERMES_WEBUI_HOST}:${HERMES_WEBUI_PORT}"
  ...
  if webui_healthy; then
    log "Flash WebUI started and passed health check"
    exit 0
  fi

  if pid_is_alive; then
    log "Flash WebUI process started with pid $(cat "${HERMES_WEBUI_PID_FILE}"); health check not ready yet"
    exit 0
  fi

  log "Flash WebUI failed to stay running; see ${WEBUI_LOG}"
  exit 1
}
