...
DEFAULT_LAUNCHD_LABEL="${HERMES_WEBUI_LAUNCHD_LABEL:-com.parantoux.flash-webui}"
...
Commands:
  start [bootstrap args...]   Start Flash WebUI as a background daemon
...
_load_flash_dotenv() {
  # Also load ~/.flash/.env so that ${VAR} references in config.yaml can
  # resolve against provider credentials defined in the Flash env file.
  ...
  [[ "${HERMES_WEBUI_NO_DOTENV:-0}" == "1" ]] && return 0
  local flash_home="${HERMES_HOME:-${HOME}/.flash}"
  local flash_env="${flash_home}/.env"
  [[ -f "${flash_env}" ]] || return 0
  ...
  _apply_env_file_safely "${flash_env}"
...
start_cmd() {
  ...
  if existing_pid="$(_current_pid 2>/dev/null)"; then
    echo "[ctl] Flash WebUI is already running (PID ${existing_pid})"
    return 0
  fi
  local launchd_pid
  if launchd_pid="$(_launchd_webui_pid 2>/dev/null)"; then
    echo "[ctl] Refusing to start a second Flash WebUI while launchd job ${HERMES_WEBUI_LAUNCHD_LABEL:-${DEFAULT_LAUNCHD_LABEL}} is running (PID ${launchd_pid})." >&2
  ...
  if ! _is_alive "${pid}"; then
    echo "[ctl] Flash WebUI failed to stay running. Log: ${LOG_FILE}" >&2
    rm -f "${PID_FILE}" "${STATE_FILE}"
    return 1
  fi
  echo "[ctl] Started Flash WebUI (PID ${pid})"
...
stop_cmd() {
  ...
  if ! pid="$(_pid_from_file 2>/dev/null)"; then
    echo "[ctl] Flash WebUI is stopped"
    rm -f "${PID_FILE}" "${STATE_FILE}"
    return 0
  fi
  ...
  echo "[ctl] Stopping Flash WebUI (PID ${pid})"
...
_health_line() {
  ...
  scheme="$(flash_webui_probe_scheme)"
  url="${scheme}://${host}:${port}/health"
  if ! command -v curl >/dev/null 2>&1 && ! command -v wget >/dev/null 2>&1; then
    echo "unknown (curl/wget not found; ${url})"
    return 0
  fi
  if result="$(flash_webui_probe_health "${host}" "${port}" "/health" 2)"; then
