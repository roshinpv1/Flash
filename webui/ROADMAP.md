# Flash Web UI — Roadmap

> Web companion to the Flash Agent CLI. Same workflows, browser-native.
>
...
| Surface | Status |
|---|---|
| **Flash CLI parity** | ✅ Complete — every CLI workflow has a web equivalent |
...
- [x] Flash Control Center modal (centralized actions)
...
- [x] Vetted, versioned library repo with CI safety gates — [flash-webui/flash-webui-extensions](https://github.com/flash-webui/flash-webui-extensions) ("in the registry == vetted")
...
- [x] Subpath mount support (reverse proxy at `/flash/`)
...
The web app is the engine; a small fleet of native shells wrap it so Flash runs
...
|---|---|---|---|
| **Flash for Mac** | macOS (Intel + Apple Silicon) | Swift + WKWebView, SSH tunnel, Sparkle 2 auto-update, signed + notarized DMG | [flash-webui/flash-swift-mac](https://github.com/flash-webui/flash-swift-mac) |
| **Flash Desktop** | Windows + Linux + macOS | Rust / Tauri (WebView2 / WebKitGTK), per-platform installers | [flash-webui/flash-desktop-rust](https://github.com/flash-webui/flash-desktop-rust) |
| **Flash for Android** | Android | Native Android, Play-ready releases | [flash-webui/flash-android](https://github.com/flash-webui/flash-android) |
| **Flash for iOS** | iOS (iPhone) | Native Swift, connects over Tailscale (QR / hostname pairing) | [flash-webui/flash-swift-ios](https://github.com/flash-webui/flash-swift-ios) |
...
Flash WebUI is maintained by an autonomous agent system: inbound issues and PRs
...
Issue state drifts — re-derive with `gh issue list --repo nesquena/flash-webui`.
...
| Native MCP server expose | #733 | Flash WebUI as an MCP server for direct agent integration |
| Teams / agents management panel | #719 | Editable names, roles, assignments |
| WebUI profile ↔ Flash runtime model alignment | #749 | Design parity between WebUI profiles and the runtime model |
