# Flash Web UI: Browser Testing Plan
...
SETUP: Clear localStorage (DevTools > Application > Local Storage > delete flash-webui-session) or open in incognito.
STEPS:
...
EXPECT:
  ...
  - Sidebar footer shows a single "Flash WebUI" control-center button
...
EXPECT:
  ...
  - After a few seconds, Flash responds
...
EXPECT:
  ...
  - Within 10-30 seconds, Flash responds with a three-word greeting
...
EXPECT:
  ...
  - Flash responds describing or reading the file content
FAIL: Upload fails, file badge never appears, Flash does not mention the file.
...
STEPS:
  1. Type: "Run the command: rm -rf /tmp/flash_test_delete_me"
  2. Send the message
EXPECT:
...
SETUP: Create a safe test: type "Run: touch /tmp/flash_approval_test.txt"
STEPS:
  1. Send the message
...
EXPECT:
  ...
  - Verify: open a terminal and run: ls /tmp/flash_approval_test.txt
...
STEPS:
  1. Click the "Flash" button in the sidebar footer
  2. In the Control Center modal, click "Transcript"
EXPECT:
  - Browser downloads a .md file named flash-{session_id}.md
...
EXPECT:
  ...
  - The shared page uses the current Flash theme/skin and shows only a read-only transcript snapshot
...
STEPS:
  1. In a terminal: tail -f ~/.flash/webui/bootstrap-8787.log
     (or tail -f ~/.flash/webui.log when launched through `ctl.sh`)
...
4. For T7.x (approval tests): the agent running inside Flash needs to detect a
   dangerous command. Ask Flash to "run: rm -rf /tmp/test_flash_approval" and watch
...
EXPECT:
  - A small clipboard icon appears in the message header row (right side of "You" or "Flash")
...
EXPECT:
  - "You" and "Flash" labels appear in slightly muted blue/gold (not full-brightness)
...
EXPECT:
  - Browser downloads a file named flash-{session_id}.json
...
EXPECT:
  ...
  - Flash streams a fresh response
...
EXPECT:
  - The "Flash" button is visible in the sidebar footer
...
SETUP: Ask Flash something that produces a code response (e.g. "Show me a Python hello world").
EXPECT:
...
STEPS:
  1. While Flash is responding, type a new message and press Enter
EXPECT:
...
- Click the "Flash WebUI" button in the sidebar footer. Control Center overlay opens with vertical section tabs on the left.
...
- With flash-agent not installed → only default profile shown, graceful fallback.

---

## Slash command parity (manual checklist)

For each batch-1 command, run via webui slash menu AND via `flash` CLI in the
