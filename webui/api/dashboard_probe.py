"""Safe server-side probe for the official Flash Agent dashboard.

The official `flash dashboard` binds to 127.0.0.1:9119 by default and exposes
...
def _looks_like_official_dashboard(payload: object) -> bool:
    ...
    if not isinstance(version, str) or not version.strip():
        return False
    # Verified against current Flash Agent `flash_cli.web_server.get_status()`:
    # /api/status returns version plus these Flash-specific fields. Requiring at
    # least one avoids treating any generic {version: ...} local service as the
    # official dashboard.
    return any(key in payload for key in ("release_date", "flash_home", "config_path", "gateway_running"))
...
def probe_official_dashboard(
    ...
    """Best-effort check that `flash dashboard` is running on host:port."""
    try:
        ...
        request = urllib.request.Request(
            f"{base}/api/status",
            headers={"Accept": "application/json", "User-Agent": "flash-webui-dashboard-probe"},
        )
        ...
        logger.debug("official Flash dashboard probe failed", exc_info=True)
        return {"running": False}
