"""Flash Web UI -- Session model and in-memory session store."""
import collections
import copy
import datetime
import hashlib
...
def process_wakeup_credential_state_fingerprint(session) -> str:
    ...
    """
    try:
        flash_home = _get_profile_home(getattr(session, 'profile', None))
    except Exception:
        flash_home = Path(os.environ.get('HERMES_HOME') or HOME).expanduser()
    files = []
    for name in ('auth.json', 'config.yaml', 'config.yml', '.env'):
        path = flash_home / name
        if name == 'auth.json':
...
def _get_profile_home(profile) -> Path:
    """Resolve the flash agent home directory for the given profile.

    Prefers the profile-specific helper from api.profiles; falls back to the
    HERMES_HOME environment variable or ~/.flash, expanding ~ correctly.
    """
    try:
        from api.profiles import get_flash_home_for_profile
        return Path(get_flash_home_for_profile(profile))
    except ImportError:
        return Path(os.environ.get('HERMES_HOME') or '~/.flash').expanduser()
...
def _sync_sidecar_from_state_db_if_newer(session) -> bool:
    """Read-side self-heal when WebUI sidecar lags Flash state.db.
...
def _profile_default_model_state(profile=None):
    ...
    try:
        from api.profiles import get_flash_home_for_profile
        config_path = Path(get_flash_home_for_profile(profile)) / "config.yaml"
...
def _active_state_db_path() -> Path:
    """Return state.db for the active Flash profile, degrading to HERMES_HOME."""
    try:
...
def _resolve_cli_sessions_context(source_filter=None, include_claude_code: bool = True):
    ...
    #   - default profile  -> ~/.flash/state.db
    #   - named profile X  -> ~/.flash/profiles/X/state.db
    ...
    return flash_home, db_path, cli_profile, cache_key
...
def _all_profiles_cli_contexts() -> tuple[list[tuple[Path, Path, str | None]], tuple]:
    ...
    def _add_context(profile_name) -> None:
        try:
            flash_home = Path(get_flash_home_for_profile(profile_name)).expanduser().resolve()
        except Exception:
            return
        home_key = _path_cache_key(flash_home)
        ...
        contexts.append((flash_home, db_path, profile_value))
...
def _load_cli_sessions_uncached(
    ...
    *,
    ...
    # re-parsing flash_home/cron/jobs.json PER untitled cron row — up to ~200
    ...
    def _cron_job_names():
        if _cron_job_names_cache[0] is None:
            names: dict[str, str] = {}
            try:
                _jobs_path = flash_home / 'cron' / 'jobs.json'
    ...
    for row in read_importable_agent_session_rows(
        ...
        # re-injected from Flash state.db (#4397).
...
def get_cli_sessions(
    ...
    *,
    ...
    if all_profiles:
        ...
        flash_home, db_path, cli_profile, cache_key = _resolve_cli_sessions_context(
            source_filter,
            **resolve_kwargs,
        )
...
def get_state_db_session_messages(
    ...
    *,
    ...
    """Read messages for a Flash session from state.db.
    ...
    through another Flash surface such as the Gateway API Server.  When
