"""
Flash Web UI -- HTTP helper functions.
"""
import functools
...
def _safe_write(handler, body: bytes) -> None:
    ...
    try:
        ...
        import logging
        logging.getLogger("flash.webui").debug(
...
def _build_redact_fn():
    """Return a redactor backed by flash-agent plus local fallback patterns."""
    # Fallback mirrors the agent's known credential prefixes so WebUI API
    # responses remain a hard redaction boundary even without flash-agent.
    # Keep this active even when flash-agent is importable so API responses do
    ...
    def _combined_redact(text: str) -> str:
        ...
        try:
            ...
            # Older flash-agent builds that predate the force kwarg.
...
PROFILE_COOKIE_NAME = 'flash_profile'
...
def get_profile_cookie(handler) -> str | None:
    ...
    the current auth session so clients cannot forge ``flash_profile`` to
