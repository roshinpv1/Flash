"""
Flash Web UI -- Gateway session watcher.
...
requiring any changes to flash-agent.
"""
import hashlib
import json
import logging
...
def _get_state_db_path(flash_home: Path | None = None) -> Path:
    """Resolve state.db path for the active profile."""
    if flash_home is not None:
        return Path(flash_home).expanduser().resolve() / 'state.db'
    try:
        from api.profiles import get_active_flash_home
        flash_home = Path(get_active_flash_home()).expanduser().resolve()
    except Exception:
        flash_home = Path(os.getenv('HERMES_HOME', str(HOME / '.flash'))).expanduser().resolve()
    return flash_home / 'state.db'
...
class GatewayWatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._flash_home = Path(flash_home).expanduser().resolve() if flash_home else None
        self._state_db_path = (
            Path(state_db_path).expanduser().resolve()
            if state_db_path is not None
            else _get_state_db_path(self._flash_home) if self._flash_home is not None else _get_state_db_path()
        )
...
def _resolve_watcher_target(
    *,
    ...
    resolved_home = Path(flash_home).expanduser().resolve() if flash_home is not None else None

    try:
        from api.profiles import get_active_profile_name, get_flash_home_for_profile

        if not resolved_profile:
            resolved_profile = get_active_profile_name() or "default"
        if resolved_home is None and resolved_profile:
            resolved_home = Path(get_flash_home_for_profile(resolved_profile)).expanduser().resolve()
    except Exception:
...
def _watcher_registry_key(profile_name: str | None = None, flash_home: Path | None = None) -> str:
    """Return the stable registry key for a watcher target."""
    if flash_home is not None:
        return str(Path(flash_home).expanduser().resolve())
...
def start_watcher(*, profile_name: str | None = None, flash_home: Path | None = None):
    ...
    with _watcher_lock:
        ...
        if watcher is None or not watcher.is_alive():
            if watcher is not None:
                watcher.stop()
            watcher = GatewayWatcher(profile_name=resolved_profile, flash_home=resolved_home)
            watcher.start()
            _watchers[key] = watcher
        return watcher


def stop_watcher(*, profile_name: str | None = None, flash_home: Path | None = None):
    """Stop either one profile watcher or the entire registry."""
    with _watcher_lock:
        if profile_name is None and flash_home is None:
...
def restart_watcher_for_profile(name: str):
    ...
    from api.profiles import get_flash_home_for_profile

    flash_home = Path(get_flash_home_for_profile(name)).expanduser().resolve()
    key = _watcher_registry_key(name, flash_home)
    watcher = GatewayWatcher(profile_name=name, flash_home=flash_home)
    watcher.start()
    with _watcher_lock:
...
def get_watcher(*, profile_name: str | None = None, flash_home: Path | None = None) -> GatewayWatcher | None:
    ...
    with _watcher_lock:
        watcher = _watchers.get(key)
    if watcher is None or not watcher.is_alive():
        watcher = start_watcher(profile_name=resolved_profile, flash_home=resolved_home)
    return watcher
