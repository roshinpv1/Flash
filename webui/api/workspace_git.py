...
def _windows_hide_flags() -> int:
    ...
    never takes a hard dependency on the optional ``flash_cli`` package (a
...
_HERMES_BRANCH_SWITCH_STASH_PREFIX = "flash-webui branch switch"
    ...
    *,
    ...
    try:
        if disable_filter_attributes:
            fd, attributes_path = tempfile.mkstemp(prefix="flash-webui-git-attrs-")
            os.close(fd)
        ...
        if hardened_destructive_path:
            hooks_path = tempfile.mkdtemp(prefix="flash-webui-git-hooks-")
...
def _flash_branch_switch_stashes(ctx: GitContext) -> list[dict]:
...
def _restore_branch_switch_stash_locked(ctx: GitContext, branch: str) -> dict:
    ...
    if _dirty_worktree(ctx):
        return {}
    for item in _flash_branch_switch_stashes(ctx):
...
def _selected_temp_index_env(ctx: GitContext, specs: list[str]) -> tuple[dict[str, str], str]:
    ...
    fd, index_path = tempfile.mkstemp(prefix="flash-webui-git-index-")
    os.close(fd)
