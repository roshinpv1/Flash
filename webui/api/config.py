"""
Flash Web UI -- Shared configuration, constants, and global state.
...
HOME = _paths.HOME
_flash_home_has_webui_state = _paths._flash_home_has_webui_state
_platform_default_flash_home = _paths._platform_default_flash_home
...
_DEFAULT_HERMES_HOME = _platform_default_flash_home()
...
# ── Flash agent directory discovery ─────────────────────────────────────────
def _discover_agent_dir() -> Path:
    """
    Locate the flash-agent checkout using a multi-strategy search.

    Priority:
      1. HERMES_WEBUI_AGENT_DIR env var  -- explicit override always wins
      2. HERMES_HOME / flash-agent      -- e.g. ~/.flash/flash-agent
      3. Sibling of this repo            -- ../flash-agent
      4. Parent of this repo             -- ../../flash-agent (nested layout)
      5. Common install paths            -- ~/.flash/flash-agent (again as fallback)
      6. HOME / flash-agent             -- ~/flash-agent (simple flat layout)
    """
    ...
    # 2. HERMES_HOME / flash-agent
    flash_home = os.getenv("HERMES_HOME", str(_DEFAULT_HERMES_HOME))
    candidates.append(Path(flash_home).expanduser() / "flash-agent")

    # 3. Sibling: <repo-root>/../flash-agent
    candidates.append(REPO_ROOT.parent / "flash-agent")

    # 4. Parent is the agent repo itself (repo cloned inside flash-agent/)
    if _looks_like_agent_source_root(REPO_ROOT.parent):
        candidates.append(REPO_ROOT.parent)

    # 5. ~/.flash/flash-agent (explicit common path)
    candidates.append(_DEFAULT_HERMES_HOME / "flash-agent")

    # 6. ~/flash-agent
    candidates.append(HOME / "flash-agent")

    # 7. XDG_DATA_HOME / flash-agent  (e.g. ~/.local/share/flash-agent)
    xdg_data = Path(os.getenv("XDG_DATA_HOME", str(HOME / ".local" / "share")))
    candidates.append(xdg_data.expanduser() / "flash-agent")

    # 8. System-wide install paths (e.g. /opt/flash-agent, /usr/local/flash-agent)
    for sys_prefix in ("/opt", "/usr/local", "/usr/local/share"):
        candidates.append(Path(sys_prefix) / "flash-agent")
...
def _looks_like_agent_source_root(path: Path) -> bool:
    """Return True when a directory resembles a flash-agent source root."""
...
def _looks_like_pip_style_agent_source_root(path: Path) -> bool:
    ...
    if (path / "flash").exists():
        return True
    flash_cli_dir = path / "flash_cli"
    return (
        (flash_cli_dir / "__init__.py").exists()
        or (flash_cli_dir / "main.py").exists()
    )


def _discover_python(agent_dir: Path) -> str:
    """
    Locate a Python executable that has the Flash agent dependencies installed.
...
# ── Inject agent dir into sys.path so Flash modules are importable ──────────

# When users (or CI builds) run `pip install --target .` or
# `pip install -t .` inside the flash-agent checkout, third-party
# package directories (openai/, pydantic/, requests/, etc.) end up
# alongside real Flash source files.  Putting _AGENT_DIR at the
...
# and Flash-specific modules (run_agent, flash/, etc.) still
...
def _get_config_path() -> Path:
    ...
    try:
        from api.profiles import get_active_flash_home

        return get_active_flash_home() / "config.yaml"
    except ImportError:
...
_DEFAULT_AGENT_PERSONALITIES = {
    # Mirrors the Flash Agent CLI built-ins so WebUI's config-derived
...
def get_config_for_profile_home(profile_home: "Path | str | None") -> dict:
    ...
    the per-request thread-local profile context (set from the ``flash_profile``
    ...
    **default** profile's ``config.yaml`` for toolsets, prefill context, and
    ...
    try:
        ...
        if Path(get_active_flash_home()).expanduser() == target:
            return get_config()
    except Exception:
        pass
...
def _save_yaml_config_file(config_path: Path, config_data: dict) -> None:
    try:
        ...
        raise RuntimeError("PyYAML is required to write Flash config.yaml") from exc
...
def print_startup_config() -> None:
    ...
    if not _HERMES_FOUND:
        print(
            f"{err}  Could not find the Flash agent directory.\n"
            "      The server will start but agent features will not work.\n"
            "\n"
            "      To fix, set one of:\n"
            "        export HERMES_WEBUI_AGENT_DIR=/path/to/flash-agent\n"
            "        export HERMES_HOME=/path/to/.flash\n"
            "\n"
            "      Or clone flash-agent as a sibling of this repo:\n"
            "        git clone <flash-agent-repo> ../flash-agent\n",
            flush=True,
        )


def verify_flash_imports() -> tuple:
    """
    Attempt to import the key Flash modules.
...
_LEGACY_CLI_TOOLSET_ALIASES = {
    # Older Flash configs used "flash" as the CLI composite toolset. Modern
    # Flash Agent exposes that split as these two registered composites; keep
    # WebUI sessions usable when pointed at an older shared config.yaml.
    "flash": ("flash-cli", "flash-api-server"),
}
...
_FALLBACK_MODELS = [
    ...
    # are visible even when the tool-support filter in flash_cli strips
...
# Provider display names for known Flash provider IDs
_PROVIDER_DISPLAY = {
    "nous": "Nous Portal",
...
#
# This table is authoritative for the WebUI.  When ``flash_cli.models``
...
# not on ``sys.path`` (CI, installs without flash-agent cloned
# alongside the WebUI).
_PROVIDER_ALIASES = {
    "glm": "zai",
    "z-ai": "zai",
    ...
    # ``flash_cli.auth.PROVIDER_REGISTRY``. Routing it through ``custom``
...
def _resolve_provider_alias(name: str) -> str:
    ...
    additional aliases the agent provides (when flash_cli is on
    ...
    try:
        from flash_cli.models import _PROVIDER_ALIASES as _agent_aliases
...
def _canonicalise_provider_id(name: object) -> str:
    ...
    (the flash_cli alias target which the WebUI doesn't index by).
...
def _provider_is_known_or_configured(
    ...
    """True when ``provider_id`` is a provider Flash recognizes (static registry)
    ...
    This distinguishes a provider Flash knows how to route (e.g. ``ollama-cloud``,
...
_PROVIDER_MODELS = {
    ...
    # (flash_cli.models.provider_model_ids("copilot")) is authoritative and is
    ...
    # 'gemini' is the flash_cli provider ID for Google AI Studio
    ...
    # flash_cli.models.provider_model_ids("bedrock") when available (#2720).
    "bedrock": [
...
def _seed_provider_models_from_core() -> None:
    """Enrich existing provider model lists with missing IDs from flash_cli.
    ...
    if flash_cli is not importable (standalone WebUI deployments).

    Must be called AFTER ``_get_label_for_model`` is defined (module-level
    invocation is at the bottom of this module, not here).
    """
    try:
        from flash_cli.models import _PROVIDER_MODELS as _core_pm
    except ImportError:
        return
...
_LOCAL_SERVER_PROVIDERS = {
    "lmstudio",     # canonical (in flash_cli.models.CANONICAL_PROVIDERS)
...
def get_effective_default_model(config_data: dict | None = None) -> str:
    """Resolve the effective Flash default model from config, then env overrides."""
...
# Mirrors flash_constants.parse_reasoning_effort so WebUI can validate without
...
# Keep this WebUI-visible set aligned with flash-agent#29248.
...
def _heuristic_reasoning_efforts(model_id: str, provider_id: str) -> list[str]:
    """Fallback when flash_cli is unavailable."""
...
def _models_dev_reasoning_efforts(model_id: str, provider_id: str) -> list[str] | None:
    """Return reasoning efforts from Flash Agent model metadata when known.
...
def _lmstudio_reasoning_probe_options_fallback(
    ...
    *,
    ...
    """Query LM Studio reasoning options without relying on flash_cli."""
    ...
    headers = {
        ...
        "User-Agent": "flash-webui-reasoning-probe",
    }
    if api_key:
...
def _lmstudio_model_reasoning_options(
    ...
    *,
    ...
    """Prefer flash_cli, but keep WebUI reasoning probes working without it.

    SECURITY: when an ``api_key`` is being sent, always use the built-in
    no-redirect fallback probe rather than ``flash_cli``. The bundled CLI probe
    ...
    try:
        ...
        logger.warning(
            "flash_cli.lmstudio_model_reasoning_options has an unexpected signature; "
...
def _resolve_model_reasoning_efforts_impl(
    ...
    if provider in {"copilot", "github-copilot"}:
        try:
            from flash_cli.models import github_model_reasoning_efforts
        except Exception:
...
def coerce_reasoning_effort_for_model(
    ...
    # list says. resolve_model_reasoning_efforts() draws from flash_cli /
...
def _normalize_openai_family_model_id(model_id: str | None) -> str:
    """Return a model id in the form expected by flash_cli fast-mode resolution."""
    model = str(model_id or "").strip()
    if not model:
        return ""
...
def _legacy_openai_service_tier_overrides(model_id: str | None, provider: str | None) -> dict:
    """Compatibility fallback for standalone WebUI installs without flash_cli.

    Normal operation delegates to Flash Agent model metadata.  This fallback
...
def _resolve_main_model_fast_mode_overrides(model_id: str | None, provider: str | None = None) -> dict:
    ...
    try:
        from flash_cli.models import resolve_fast_mode_overrides
    except Exception:
        logger.debug("Failed to import flash_cli.models.resolve_fast_mode_overrides; using WebUI compatibility fallback.")
        return _legacy_openai_service_tier_overrides(model_id, provider)
    try:
...
def set_flash_default_model(model_id: str, provider: str | None = None, advanced: dict | None = None) -> dict:
    """Persist the Flash default model in config.yaml and reload runtime config."""
    ...
    with _cfg_lock:
        ...
        # prefix is a WebUI-internal routing hint that the flash-agent CLI does
        # not understand — if we wrote `@nous:anthropic/claude-opus-4.6` to
        # config.yaml, a user who ran `flash` in the terminal right after
...
# Keep in sync with flash_cli/config.py DEFAULT_CONFIG["auxiliary"] and
# flash_cli/web_server.py _AUX_TASK_SLOTS.
...
def _endpoint_advertised_model_ids(provider_id: str | None) -> frozenset | None:
    ...
    # PROFILE-SPECIFIC config path (_get_config_path -> get_active_flash_home),
...
def _credential_pool_profile_tag() -> str:
    ...
    The credential pool is per-Flash-profile (it lives in that profile's
...
def _pool_entry_payloads(provider_id: str) -> list[dict[str, Any]]:
    ...
    if bool(getattr(_thread_ctx, "block_process_env_fallback", False)):
        try:
            from flash_cli.auth import read_credential_pool as _read_credential_pool
...
def _get_auth_store_path() -> Path:
    """Return the auth.json path for the active Flash profile."""
    try:
        from api.profiles import get_active_flash_home as _gah
...
def _read_live_provider_model_ids(provider_id: str) -> list[str]:
    """Return live model IDs from Flash CLI for a provider, or [] on failure.
    ...
    groups should ask ``flash_cli.models.provider_model_ids()`` first (#1240).
    Provider aliases are tried as a secondary lookup because WebUI keeps a few
    display-facing IDs (for example ``google`` / ``x-ai``) that Flash CLI may
    ...
    if not pid:
        return []
    try:
        from flash_cli.models import provider_model_ids as _provider_model_ids
    except Exception:
        return []
    ...
    for candidate in candidates:
        try:
            ...
            logger.debug("Failed to load %s models from flash_cli", candidate)
            continue
        result: list[str] = []
        for mid in live_ids:
...
def _models_from_live_provider_ids(provider_id: str, live_ids: list[str]) -> list[dict]:
    """Convert Flash CLI model ids into WebUI picker model entries."""
...
def get_available_models(*, prefer_cache: bool = False, force_refresh: bool = False) -> dict:
    ...
    Discovery order:
      ...
      2. Check for known API keys in env or ~/.flash/.env
    ...
    def _build_available_models_uncached() -> dict:
        ...
        _flash_auth_used = False
        try:
            from flash_cli.models import list_available_providers as _lap
            from flash_cli.auth import get_auth_status as _gas
            ...
            # with `get_auth_status(<id>).logged_in` on some flash_cli versions
            ...
            logger.debug("Failed to detect auth providers from flash")

        if not _flash_auth_used:
            try:
                from api.profiles import get_active_flash_home as _gah2

                flash_env_path = _gah2() / ".env"
            except ImportError:
                flash_env_path = _DEFAULT_HERMES_HOME / ".env"
            env_keys = {}
            if flash_env_path.exists():
                try:
                    for line in flash_env_path.read_text(encoding="utf-8").splitlines():
                        line = line.strip()
                        if line and not line.startswith("#") and "=" in line:
                            k, v = line.split("=", 1)
                            env_keys[k.strip()] = v.strip().strip('"').strip("'")
                except Exception:
                    logger.debug("Failed to parse flash env file")
            all_env = {**env_keys}
            ...
            if all_env.get("OPENAI_API_KEY"):
                # flash-agent registers its OPENAI_API_KEY/OPENAI_BASE_URL provider
            ...
            # LM Studio: detect via LM_API_KEY + LM_BASE_URL in ~/.flash/.env
                ...
                try:
                    ...
                    if addr.is_private or addr.is_loopback or addr.is_link_local:
                        if "ollama" in host or "127.0.0.1" in host or "localhost" in host:
                            ...
                            # ``flash_cli.auth.PROVIDER_REGISTRY`` — see #1384.
                            provider = "custom"
                except ValueError:
                    pass
        ...
        if isinstance(_custom_providers_cfg, list):
            ...
            for _cp in _custom_providers_cfg:
                ...
                if _slug and _cp_base_url:
                    ...
                    # discovery (same as flash-agent model_switch.py Section 4
        ...
        if detected_providers:
            ...
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    #   (1) curated tool-supporting catalog via flash_cli.models.fetch_openrouter_models()
                    ...
                    try:
                        ...
                        logger.warning("Failed to load OpenRouter curated catalog from flash_cli")
                    ...
                    try:
                        ...
                        logger.warning("Failed to load Ollama Cloud models from flash_cli")
                    ...
                    try:
                        ...
                        logger.warning("Failed to load OpenAI Codex models from flash_cli")
                    ...
                    # live-fetch through flash_cli.models.provider_model_ids()
                    ...
                    try:
                        ...
                        logger.warning("Failed to load Nous Portal models from flash_cli")
                        live_ids = []
                    ...
                    # CI environments where flash_cli isn't importable —
                    ...
                    try:
                        ...
                        logger.debug("flash_cli LM Studio lookup unavailable; using urlopen fallback")
                    ...
                    # built-in Flash providers (e.g. providers.anthropic.models
                    ...
                    # config-models allowlist branch and asks Flash CLI for the
...
try:
    ...
    pass  # flash_cli not available -- default profile only
...
try:
    ...
    pass  # flash_cli not available (standalone deployment)
except Exception:
