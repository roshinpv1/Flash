...
The flash-agent ``tools.process_registry.ProcessRegistry`` exposes a thread-safe
...
def start_session_channel_reaper() -> bool:
    ...
    with _THREAD_LIFECYCLE_LOCK:
        ...
        _REAPER_THREAD = threading.Thread(
            target=_reaper_loop,
            name="flash-webui-session-channel-reaper",
            daemon=True,
        )
...
def _start_server_side_wakeup_turn(
    ...
    threading.Thread(
        target=_runner,
        name=f"flash-webui-process-wakeup-{str(session_id)[:8]}",
        daemon=True,
    ).start()
...
def start_drain_thread() -> bool:
    ...
    with _THREAD_LIFECYCLE_LOCK:
        ...
        _DRAIN_THREAD = threading.Thread(
            target=_drain_loop,
            name="flash-webui-bg-task-complete-drain",
            daemon=True,
        )
        _DRAIN_THREAD.start()
        return True
