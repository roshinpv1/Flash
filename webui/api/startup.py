...
def fix_credential_permissions() -> None:
    ...
    flash_home = Path(os.environ.get('HERMES_HOME', str(Path.home() / '.flash')))
    if not flash_home.is_dir():
        return
    for name in _SENSITIVE_FILES:
        fpath = flash_home / name
        if not fpath.exists():
            continue
        try:
...
def _agent_dir() -> Path | None:
    flash_home = Path(os.environ.get('HERMES_HOME', str(Path.home() / '.flash')))
    for raw in [os.environ.get('HERMES_WEBUI_AGENT_DIR', '').strip(), str(flash_home / 'flash-agent')]:
        if not raw:
            continue
...
def _trusted_agent_dir(agent_dir: Path) -> bool:
    ...
    the dir to be ~/.flash/flash-agent), so custom HERMES_WEBUI_AGENT_DIR
