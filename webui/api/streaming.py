"""
Flash Web UI -- SSE streaming engine and agent thread runner.
...
def _resolve_custom_provider_runtime_overrides(
    ...
    Flash Agent treats named custom providers as routing hints around an
...
def _prewarm_skill_tool_modules():
    ...
    in the flash-agent package which may not be on ``sys.path`` at
...
# Lazy import to avoid circular deps -- flash-agent is on sys.path via api/config.py
try:
...
- Match the normal Flash messaging style, but do not let long tool-running WebUI turns appear silent.
...
def _load_webui_prefill_context(
    ...
    Supports the same bounded JSON-file shape used by Flash Agent.  WebUI also
...
def _webui_delivery_context_prompt(config_data: Optional[dict] = None) -> str:
    ...
    display_flash_home = None
    try:
        from flash_constants import get_flash_home, display_flash_home as _dh
        display_flash_home = _dh
    except Exception:
        get_flash_home = None  # type: ignore[assignment]

    connected = ["local (files on this machine)"]
    try:
        if get_flash_home is not None:
            state_path = get_flash_home() / "gateway_state.json"
    ...
    try:
        home_display = display_flash_home() if display_flash_home else "~/.flash"
    except Exception:
        home_display = "~/.flash"
...
def _cancelled_turn_hint(agent_name: str | None = None) -> str:
    name = str(agent_name or _preferred_agent_display_name()).strip() or 'Flash'
...
def _classify_provider_error(err_str: str, exc=None, *, silent_failure: bool = False) -> dict:
    ...
    Keep this string-based until flash-agent exposes stable structured
    ...
    _is_not_found = (
        # model_not_found hints mention Settings / `flash model` below.
        '404' in err_str
        or 'not found' in _err_lower
        or 'does not exist' in _err_lower
        or 'model not found' in _err_lower
        or 'model_not_found' in _err_lower  # hint below points to Settings / `flash model`
    ...
    if _is_credential_pool_empty:
        return {
            ...
            'hint': 'The credential pool for this provider has no usable keys left (all entries exhausted or unconfigured). Add or refresh a key for this provider in your Flash config / credential pool, or switch providers via `flash model`.',
        }
    if _is_quota:
        return {
            'label': 'Out of credits',
            'type': 'quota_exhausted',
            'hint': 'Your provider account is out of credits or usage. Top up, wait for the plan window to reset, or switch providers via `flash model`.',
        }
    if _is_rate_limit:
        return {
    ...
    if _is_auth:
        return {
            ...
            'type': 'auth_mismatch',
            'hint': 'The selected model may not be supported by your configured provider or your API key is invalid. Run `flash model` in your terminal to update credentials, then restart the WebUI.',
        }
    if _is_not_found:
        return {
            'label': 'Model not found',
            'type': 'model_not_found',
            'hint': 'The selected model was not found by the provider. Check the model ID in Settings or run `flash model` to verify it exists for your provider.',
        }
    ...
    if silent_failure:
        return {
            ...
            'type': 'no_response',
            'hint': 'The provider returned no content and no error. This often means a usage/rate limit was hit silently. Check provider status, switch providers via `flash model`, or try again in a moment.',
        }
...
def _is_synthetic_max_iteration_summary_request(message) -> bool:
    """Return True for Flash Agent's internal max-iteration summary prompt."""
...
# Structured markers the Flash Agent stamps on synthetic scaffolding turns that
...
def _maybe_inject_max_iteration_summary_fallback(messages, result) -> list:
    ...
    couldn't generate a summary."``). Flash Agent surfaces that string as the
    ...
    flash-agent. Returns the (possibly new) messages list; does nothing when
...
def _aiagent_import_error_detail() -> str:
    ...
    The bare ImportError ("AIAgent not available -- check that flash-agent is
    ...
    var if set, the sys.path entries that mention 'flash', and the most-common
    ...
    lines = ["AIAgent not available -- check that flash-agent is on sys.path"]
    lines.append("")
    ...
    relevant = [p for p in _sys.path if "flash" in p.lower() or "agent" in p.lower()]
    if relevant:
        lines.append("  sys.path entries mentioning flash/agent:")
        for entry in relevant[:6]:
            lines.append(f"    - {entry}")
        if len(relevant) > 6:
            lines.append(f"    ... and {len(relevant) - 6} more")
    else:
        lines.append("  sys.path: (no entries mention flash or agent)")

    lines.append("")
    lines.append("  Most common fix: install the agent in editable mode so its modules")
    lines.append("  appear on sys.path:")
    lines.append("")
    lines.append("    cd /path/to/flash-agent")
...
def _stale_completion_max_age_seconds() -> float:
    ...
    contaminate an unrelated later turn. See nesquena/flash-webui#4029.
...
def _drain_webui_process_notifications(session_id: str) -> list[str]:
    ...
    while True:
        ...
        # (nesquena/flash-webui#4029). Drop (consume, do not requeue) any
...
def _build_native_multimodal_message(workspace_ctx: str, msg_text: str, attachments, workspace: str, *, cfg: dict = None):
    ...
    bytes to Flash as OpenAI-style image_url data URLs so vision-capable main
    ...
    # land in ~/.flash/webui/attachments/<sid>/ (outside workspace_root by
...
def _fallback_title_from_exchange(user_text: str, assistant_text: str) -> Optional[str]:
    ...
    if topic_name:
        if not _contains_latin(topic_name):
            ...
            if any(k in combined for k in ('flash', 'codex', 'ai')):
...
def _merge_display_messages_after_agent_result(previous_display, previous_context, result_messages, msg_text, source: str = "webui"):
    ...
    If Flash Agent returns a normal append-only history, append that delta to
    ...
    # Drop Flash Agent internal verify-loop scaffolding (synthetic "premature
...
def _attempt_credential_self_heal(
    ...
    Steps:
    1. Re-read ``~/.flash/auth.json`` to pick up fresh credentials that
       may have been written by a concurrent ``flash model`` CLI invocation.
...
def _run_agent_streaming(
    ...
    *,
    ...
    # rewritten yet.  See https://github.com/nesquena/flash-webui/issues/1968.
                            ...
                            if _sm_u:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        from api.profiles import get_flash_home_for_profile as _ghp_u
                                    ...
                                    # Older flash-agent: legacy 2-arg form.
                                    try:
                                        from api.routes import (
    ...
    try:
        ...
        try:
            ...
            _profile_home_path = get_flash_home_for_profile(getattr(s, 'profile', None))
        ...
        with _ENV_LOCK:
            ...
            if _profile_home:
                ...
                # _set_flash_home() does this for process-wide switches
        ...
        # reads `~/.flash/config.yaml` via `get_flash_home()`, which uses
        ...
        # `_servers` by `(profile_home, name)` upstream in flash-agent; that
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            """Bridge Flash clarify prompts to the WebUI."""
        ...
        try:
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                # Modern Flash Agent builds can call both tool_progress_callback
                ...
                if event_type == 'tool.completed':
                    ...
                    # `result` kwarg from modern Flash builds; fall
            ...
            # Resolve API key via Flash runtime provider (matches gateway behaviour).
            ...
            # flash_cli.runtime_provider directly. Fall back to config.yaml
            ...
            # provider recovery). Match Flash CLI/gateway semantics:
            ...
            # degrades gracefully when run against an older flash-agent build.
            ...
            # after the operator raises Flash' global turn budget.
            ...
            try:
                ...
                if _raw_max_iterations is None and isinstance(_cfg, dict):
                    # Back-compat for older Flash config files that used a
            ...
            _agent_kwargs = dict(
                ...
                # Identify browser-originated sessions as WebUI so Flash Agent
            ...
            # Params added in newer flash-agent — skip if not supported
            ...
            # (matches flash-agent CLI behavior — passes via ephemeral_system_prompt)
            ...
            # normal send never trips a TypeError on an older flash-agent whose
            ...
            with _agent_lock:
                ...
                with _stream_writeback_stage(_writeback_timings, "merge_result"):
                    ...
                    # #5494 — parity with flash-agent's handle_max_iterations() return
                ...
                if _agent_sid and _agent_sid != session_id:
                    ...
                    # get_flash_home_for_profile(getattr(s, 'profile', None))
                    ...
                    if _is_quota:
                        ...
                        if not _assistant_added:
                            ...
                            _err_hint = (
                                ...
                                'your API key is invalid. Run `flash model` in your terminal to '
                ...
                if (not getattr(s, 'context_length', 0)) or _skip_cc_cl:
                    try:
                        ...
                        # Older flash-agent builds whose get_model_context_length
                        ...
                        # Older flash-agent builds may not expose this helper.
                ...
                if not ephemeral:
                    ...
                    # every completed turn.  This mirrors Flash CLI where
            ...
            if not usage.get('context_length'):
                try:
                    ...
                    try:
                        ...
                        # Older flash-agent builds: fall back to legacy 2-arg form.
                        _fb_cl = _get_cl(
                            getattr(agent, 'model', resolved_model or '') or '',
                            _cfg_base_url,
                        )
            ...
            # /goal parity: after a successful assistant turn, run the Flash
            ...
            with _ENV_LOCK:
                ...
                if old_flash_home is None: os.environ.pop('HERMES_HOME', None)
                else: os.environ['HERMES_HOME'] = old_flash_home
        ...
        # The user hint still points to Settings / `flash model` from _classify_provider_error().
        if _exc_is_quota:
            ...
            _exc_label, _exc_type, _exc_hint = (
                ...
                'Run `flash model` in your terminal to switch providers, then restart the WebUI.',
            )
...
def _handle_chat_steer(handler, body: dict) -> bool:
    ...
    if not hasattr(agent, "steer"):
        # Older flash-agent that pre-dates the steer() method
