"""Default-off Flash Gateway bridge for browser-originated chat turns."""
...
def webui_chat_backend_mode(config_data=None, environ: dict[str, str] | None = None) -> str:
    ...
    values opt browser chat into the Flash API server bridge; generic truthy
...
def _gateway_http_error_event(exc: urllib.error.HTTPError, err_body: str, *, api_key_configured: bool) -> dict:
    ...
    if exc.code == 401:
        return {
            ...
            "hint": (
                "Set HERMES_WEBUI_GATEWAY_API_KEY to the same value as the Flash Gateway "
                "API_SERVER_KEY, or disable HERMES_WEBUI_CHAT_BACKEND=gateway."
                if not api_key_configured
                else "Check that HERMES_WEBUI_GATEWAY_API_KEY matches the Flash Gateway API_SERVER_KEY."
            ),
        }
    return {
...
def _gateway_tool_progress_event(payload: dict) -> tuple[str, dict] | None:
    """Translate Flash Gateway tool-progress SSE payloads to WebUI events."""
    if not isinstance(payload, dict):
        return None
...
def _run_gateway_runs_api_streaming(
    ...
    *, put_gateway_event, cancel_event,
    ...
    if api_key:
        ...
        headers["X-Flash-Session-Key"] = f"webui:{session_id}"
...
def _run_gateway_chat_streaming(
    ...
    *,
    ...
    """Bridge a WebUI chat turn through Flash Gateway's API server.
    ...
    try:
        ...
        if _use_runs_api:
            ...
            if approval_reason is not None:
                ...
                if not s._approval_notice_emitted:
                    approval_message = "Approvals require a newer gateway. Upgrade the connected Flash gateway to enable this."
                    approval_type = "approval_gateway_unsupported"
                    if approval_reason == "unreachable":
                        approval_type = "approval_gateway_offline"
                        approval_message = "Gateway connection failed. Check that the connected Flash gateway is running and reachable."
            ...
            with urllib.request.urlopen(req, timeout=_gateway_read_timeout_secs()) as resp:
                for raw_line in _iter_sse_lines_cancellable(resp, cancel_event):
                    ...
                    if _payload_event in {"flash.approval.request", "approval.request"}:
                        approval_data = _gateway_runs_approval_event(payload)
                        if approval_data:
                        ...
                        sse_event = "message"
                        continue
                    if sse_event == "flash.tool.progress":
                        translated = _gateway_tool_progress_event(payload)
                        if translated:
                            event_name, event_payload = translated
                            if event_name == "reasoning":
        ...
        if not assistant_text:
            put_gateway_event("apperror", {
                ...
                "hint": "Check that Flash Gateway API server is running and reachable.",
            })
            return
        with _get_session_agent_lock(session_id):
            s = get_session(session_id)
        ...
        try:
            ...
            profile_home = get_flash_home_for_profile(getattr(s, "profile", None))
