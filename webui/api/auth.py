"""
Flash Web UI -- optional authentication.
...
COOKIE_NAME = 'flash_session'
CSRF_HEADER_NAME = 'X-Flash-CSRF-Token'
...
def sign_profile_cookie_value(profile_name: str, session_cookie_value: str | None) -> str:
    ...
    ``flash_profile=<other-profile>`` and bypassing profile visibility guards.
    """
...
def csrf_token_for_session(cookie_value: str) -> str | None:
    ...
    in ``X-Flash-CSRF-Token`` on unsafe API requests. The token is derived
