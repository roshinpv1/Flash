"""Helpers for WebUI-managed Flash Agent git worktrees."""
...
def _setup_agent_worktree(repo_root: str) -> dict:
    try:
        ...
        from api.config import _AGENT_DIR  # Flash Agent source root
        ...
        # site-packages will preempt the real flash-agent/cli.py module.
        cli_path = str(Path(_AGENT_DIR) / "cli.py")
        spec = importlib.util.spec_from_file_location(
            "flash_cli_worktree", cli_path,
        )
        if spec is None or spec.loader is None:
            raise RuntimeError(
                f"Could not locate flash-agent worktree helper at {cli_path}"
            )
        cli_mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(cli_mod)
        _setup_worktree = cli_mod._setup_worktree
    except Exception as exc:
        raise RuntimeError("Flash Agent worktree helper is unavailable") from exc
    output = StringIO()
    with redirect_stdout(output), redirect_stderr(output):
        info = _setup_worktree(repo_root)
    emitted = output.getvalue().strip()
    if emitted:
        logger.debug("Flash Agent worktree helper output: %s", emitted)
    if not info:
        raise RuntimeError("Flash Agent failed to create a git worktree")
    return info
...
def create_worktree_for_workspace(workspace: str | Path) -> dict:
    ...
    if not path or not branch:
        raise RuntimeError("Flash Agent returned incomplete worktree metadata")
    return {
