"""Flash Web UI -- provider management endpoints.
...
# extension — the same technique used throughout the Flash codebase.
...
def _codex_usage_headers(access_token):
    headers = {
        ...
        "User-Agent": "codex_cli_rs/0.0.0 (Flash WebUI)",
...
_PROVIDER_ENV_VAR: dict[str, str] = {
    ...
    # by default and the runtime in flash_cli/runtime_provider.py only consumes
    ...
    # runtime reads (flash_cli/auth.py:182, api_key_env_vars=("LM_API_KEY",)).
...
# through the Flash CLI, not via API keys.  The WebUI cannot set these.
...
def _get_flash_home() -> Path:
    """Return the active Flash home directory."""
    try:
...
def _write_env_file(env_path: Path, updates: dict[str, str | None]) -> None:
    ...
    with _ENV_LOCK:
        ...
        # ``~/.flash/.env`` is also written by ``flash_cli.config.save_env_value``
...
def _provider_has_key(provider_id: str) -> bool:
    ...
    Checks (in order):
    1. ``~/.flash/.env`` for the known env var
    ...
    # `flash auth add` which store keys in auth.json (not config.yaml).
...
def _account_usage_subprocess_env(home: Path, provider: str, api_key: str | None) -> dict[str, str]:
    ...
    if bool(getattr(_thread_ctx, "block_process_env_fallback", False)):
        ...
        try:
            from api.profiles import _profile_secret_env_names, get_active_flash_home
            _strip.update(_profile_secret_env_names(get_active_flash_home()))
        except Exception:
...
def _close_account_usage_probe_workers_async(*, provider_id: str | None = None) -> None:
    with _account_usage_worker_pool_lock:
        if provider_id:
            active_home = str(_get_flash_home())
            workers_to_close = []
...
def get_provider_quota(provider_id: str | None = None, *, refresh: bool = False) -> dict[str, Any]:
    ...
    providers reuse Flash Agent's /usage account-limits abstraction so WebUI
...
def _cost_snapshots_dir() -> Path:
    ...
    Uses the Flash home directory (profile-aware) so snapshots are
    isolated per profile, matching the existing STATE_DIR convention.
    """
    return _get_flash_home() / _COST_SNAPSHOTS_DIR_NAME
...
def _append_cost_snapshot(provider: str, usage: int | float | None, limit: int | float | None) -> list[dict[str, Any]]:
    ...
    # deployments that share one Flash home/state directory.
...
def get_providers() -> dict[str, Any]:
    ...
    for pid in sorted(known_ids):
        ...
        if not has_key and is_plugin_model_provider(pid):
            try:
                from flash_cli.auth import get_auth_status as _gas_plugin
        ...
        if is_oauth:
            key_source = "oauth"
            # Check if actually authenticated via flash_cli.
            ...
            # returns logged_in=False (e.g. token not in the flash credential pool,
            ...
            try:
                ...
                # key just because the flash_cli auth module is unavailable.
                logger.debug("flash_cli auth check failed for %s", pid, exc_info=True)
        ...
        # The model picker already prefers flash_cli + Codex local cache for
        ...
        # case below — when flash_cli is unavailable or its lookup raises,
        ...
        if pid == "nous":
            try:
                ...
                logger.debug("Failed to load Nous Portal models from flash_cli")
        ...
        if pid == "lmstudio":
            try:
                from flash_cli.models import provider_model_ids as _pmi
                ...
                logger.debug("Failed to load LM Studio models from flash_cli")
    ...
    if isinstance(custom_providers_cfg, list):
        for cp in custom_providers_cfg:
            ...
            # Fallback: check credential pool (key added via flash auth add)
            if not cp_has_key:
                try:
                    from api.config import _has_explicit_pool_credentials
...
def set_provider_key(provider_id: str, api_key: str | None) -> dict[str, Any]:
    ...
    Writes the key to ``~/.flash/.env`` using the standard env var name.
    ...
    if _provider_is_oauth(provider_id):
        return {
            ...
            "error": f"'{_PROVIDER_DISPLAY.get(provider_id, provider_id)}' uses OAuth authentication. "
                     f"Use `flash model` in the terminal to configure it.",
        }
...
def remove_provider_key(provider_id: str) -> dict[str, Any]:
    ...
    Removes the key from ``~/.flash/.env`` (via ``set_provider_key``)
