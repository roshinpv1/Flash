"""
Flash Web UI -- Self-update checker.

Checks if the webui and flash-agent git repos are behind their latest
...
def _read_agent_source_version(agent_dir: Path) -> str | None:
    """Read Flash Agent's package version from a copied source tree."""
    init_file = agent_dir / 'flash_cli' / '__init__.py'
    try:
...
def _gateway_health_base_url() -> str:
    """Return the configured/default Flash Agent gateway base URL."""
    raw = (
        os.environ.get('GATEWAY_HEALTH_URL')
        or os.environ.get('HERMES_GATEWAY_HEALTH_URL')
        or 'http://flash-agent:8642'
    ).strip()
...
def _version_from_gateway_health_payload(payload: object) -> str | None:
    """Extract a version string from a Flash Agent gateway health payload."""
    if not isinstance(payload, dict):
        return None
    for key in ('version', 'agent_version', 'flash_version'):
...
def _detect_agent_version() -> str:
    """Detect the running Flash Agent version for UI display."""
    ...
    if agent_dir is not None:
        ...
        if agent_dir.exists():
            ...
            # still lives in flash_cli/__init__.py, so prefer that before giving
...
def _normalize_remote_url(remote_url):
    ...
    treats the argument as a character set and can truncate ``flash-webui`` to
    ``flash-webu``.
    """
    if not remote_url:
        return remote_url
...
def _github_release_tags(url='https://api.github.com/repos/nesquena/flash-webui/tags?per_page=100', *, timeout=3.0):
    """Return GitHub release tags newest-first, including commit SHAs when available."""
    request = urllib.request.Request(
        url,
        headers={
            'Accept': 'application/vnd.github+json',
            'User-Agent': 'flash-webui',
        },
    )
...
def _check_webui_published_release_update():
    ...
    repo_url = 'https://github.com/nesquena/flash-webui'
    return {
        'name': 'webui',
        'behind': behind,
...
def _commit_subjects_for_update_with_limit(info: dict, *, limit: int = 24) -> tuple[list[str], bool]:
    ...
    if target not in ('webui', 'agent'):
        target = 'webui' if info.get('repo_url', '').endswith('flash-webui') else target
...
def _fallback_update_bullets(details: list[dict]) -> list[str]:
    bullets = []
    for item in details:
        label = item.get('label') or item.get('name') or 'Flash'
...
def _worth_knowing_bullets(details: list[dict]) -> list[str]:
    ...
    if items:
        return items
    targets = [
        f"{item.get('label') or item.get('name') or 'Flash'} ({item.get('behind') or 0} update{'s' if (item.get('behind') or 0) != 1 else ''})"
        for item in details
        if item.get('behind')
    ]
    if len(targets) > 1:
...
def _update_summary_prompt(details: list[dict]) -> tuple[str, str]:
    system = (
        "You write human-readable release summaries for Flash users. "
    ...
    def _do():
        ...
        with _apply_lock:
            ...
            try:
                ...
                #     (e.g. "/root/flash-webui/server.py"), sys.executable is
...
def _agent_gateway_restart_failure_message(target: str, restart_result: dict) -> str:
    if restart_result.get("message"):
        return (
            ...
            f'{restart_result["message"]}. Run `flash gateway restart` manually.'
        )
    return (
        f'{target} updated, but gateway restart did not complete. '
        'Run `flash gateway restart` manually.'
    )
...
def _apply_update_inner(target, channel=DEFAULT_UPDATE_CHANNEL):
    ...
    stashed = False
    if status_out:
        _, ok = _run_git(['stash', 'push', '-m', 'flash-update-autostash'], path)
        if not ok:
