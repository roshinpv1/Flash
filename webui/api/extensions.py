...
_REGISTRY_URL = "https://flash-webui.github.io/flash-webui-extensions/registry.json"
_REGISTRY_ALLOWED_DOWNLOAD_HOSTS = frozenset({"flash-webui.github.io"})
...
def _extension_state_dir() -> Path:
    ...
    try:
        ...
        return Path(os.getenv("HERMES_WEBUI_STATE_DIR", str(Path.home() / ".flash" / "webui"))).expanduser()
...
def inject_extension_tags(index_html: str) -> str:
    ...
    runtime_tag = (
        ...
        "if(window.FlashExtensionSettings)window.FlashExtensionSettings.primeFromStatus(window.__HERMES_EXTENSION_CONFIG__);"
        "</script>"
    ).format(runtime_json)
