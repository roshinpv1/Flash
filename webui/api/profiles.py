"""
Flash Web UI -- Profile state management.
Wraps flash_cli.profiles to provide profile switching for the web UI.
...
cached paths in flash-agent modules (skills_tool, skill_manager_tool,
...
# ── Constants (match flash_cli.profiles upstream) ─────────────────────────
...
# reads its own profile from the flash_profile cookie instead of the
...
def _unwrap_profile_home_to_base(home: Path) -> Path:
    """Return the base Flash home when *home* is already a named profile dir."""
...
def _isolated_profile_opt_in() -> bool:
    ...
    Flash Agent launcher exports ``HERMES_HOME=~/.flash/profiles/<name>`` for any
...
def _warn_if_profile_shape_without_isolated_opt_in() -> None:
    ...
    if _ISOLATED_PROFILE_SHAPE_WITHOUT_OPT_IN_WARNING_EMITTED:
        return
    flash_home = _INITIAL_HERMES_HOME
    if not flash_home:
        return
    p = Path(flash_home).expanduser()
...
def _is_isolated_profile_mode() -> bool:
    ...
    Returns True only when BOTH conditions hold:
      ...
      2. HERMES_HOME at startup points at a concrete profile subdirectory
         (e.g., ~/.flash/profiles/user1) rather than the base home.
    ...
    named profile — the Flash Agent launcher sets
    ``HERMES_HOME=~/.flash/profiles/<name>`` for any active named profile, so the
    ...
    p = Path(flash_home).expanduser()
    # SECONDARY requirement: HERMES_HOME must look like ~/.flash/profiles/<name>
...
def _resolve_base_flash_home() -> Path:
    """Return the BASE ~/.flash directory — the root that contains profiles/.
    ...
    always point to the top-level .flash regardless of which profile is active.
    ...
    flash_home = os.getenv('HERMES_HOME', '').strip()
    if flash_home:
        p = Path(flash_home).expanduser()
    ...
    # fallback (prefer the populated legacy %USERPROFILE%\.flash over an
    # empty %LOCALAPPDATA%\flash). Import the shared path helper directly
    ...
    from api.paths import _platform_default_flash_home

    return _platform_default_flash_home()

_DEFAULT_HERMES_HOME = _resolve_base_flash_home()


def _read_active_profile_file() -> str:
    """Read the sticky active profile from ~/.flash/active_profile."""
...
#
# Flash Agent allows the root/default profile (~/.flash itself) to have a
# display name other than the legacy literal 'default'.  When that happens,
# WebUI must NOT resolve the display name as ~/.flash/profiles/<name> — that
# directory doesn't exist, and every site that does `if name == 'default':`
# will fall through to the wrong filesystem path.
#
# `_is_root_profile(name)` answers "does this name resolve to ~/.flash?" and
# is the canonical replacement for scattered `if name == 'default':` checks
# in switch_profile, get_active_flash_home, _validate_profile_name, etc.
#
# Cost note: list_profiles_api() shells out via flash_cli (non-trivial), so
...
def _is_root_profile(name: str) -> bool:
    """True if *name* resolves to the Flash Agent root profile (~/.flash).
    ...
    # avoid holding it across a flash_cli subprocess call.
    try:
...
def get_active_profile_name() -> str:
    ...
    Priority:
      ...
      2. Thread-local (set per-request from flash_profile cookie) — issue #798
...
def set_request_profile(name: str) -> None:
    ...
    Called by server.py at the start of each request when a flash_profile
...
def _resolve_profile_home_for_name(name: str) -> Path:
    """Resolve a logical profile name to its Flash home path.
...
def get_active_flash_home() -> Path:
...
class cron_profile_context_for_home:
    ...
    def __enter__(self):
        ...
        try:
            ...
            # cron.scheduler snapshots _flash_home at import time and run_job()
            ...
            try:
                import cron.scheduler as _cs
                self._prev_cs = (
                    getattr(_cs, '_flash_home', None),
                    getattr(_cs, '_LOCK_DIR', None),
                    getattr(_cs, '_LOCK_FILE', None),
                )
                _cs._flash_home = self._home
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            ...
            if getattr(self, '_prev_cs', None) is not None:
                try:
                    import cron.scheduler as _cs
                    _cs._flash_home, _cs._LOCK_DIR, _cs._LOCK_FILE = self._prev_cs
                except (ImportError, AttributeError):
                    pass
        finally:
            _pop_cron_profile_context_depth()
...
def get_flash_home_for_profile(name: str) -> Path:
...
def get_profile_runtime_env(home: Path) -> dict[str, str]:
    ...
    environment variables (matching ``flash -p <profile>``), so streaming must
...
# Match Flash Agent gateway behavior: profile-scoped WebUI runs should
...
def filter_runtime_env_for_gateway_parity(env: dict[str, str]) -> dict[str, str]:
    """Return a profile runtime env filtered to mimic Flash gateway semantics."""
...
# NOT in flash_cli.auth.PROVIDER_REGISTRY (so the registry-derived scrub set
# would miss them). Fail-closed list — verified against the installed agent:
#   CUSTOM_API_KEY            flash_cli/models.py (generic custom provider key)
#   AZURE_ANTHROPIC_KEY       flash_cli/runtime_provider.py (Azure-hosted Anthropic)
#   AZURE_FOUNDRY_API_KEY     flash_cli/runtime_provider.py (Azure Foundry key)
#   AZURE_* identity family   agent/azure_identity_adapter.py (service-principal /
#                             workload-identity model auth)
#   AWS_BEARER_TOKEN_BEDROCK  flash_cli/model_switch.py (Bedrock bearer token)
...
_NON_REGISTRY_AGENT_CREDENTIAL_ENV_NAMES: tuple[str, ...] = (
    ...
    # where flash_cli.auth is absent) — the registry union is best-effort.
    "ANTHROPIC_TOKEN",
...
def _agent_registry_credential_env_names() -> set[str]:
    ...
    1. ``flash_cli.auth.PROVIDER_REGISTRY[*].api_key_env_vars`` — every provider
...
# #5567: flash-agent v0.18.0+ exposes a CONTEXT-LOCAL Flash-home override
# (`flash_constants.set_flash_home_override`) that `get_flash_home()` — and
# therefore `flash_cli.config.get_config_path()` / `load_config()` — consults
...
_flash_home_override_available = None


def _resolve_flash_home_override():
    """Return the flash_constants module iff it exposes the v0.18.0+ context-local
    home override (set/reset), else None. Cached; import-safe on older agents."""
    global _flash_home_override_available
    import sys as _sys
    if _flash_home_override_available is False:
        return None
    mod = _sys.modules.get('flash_constants')
    if mod is None and _flash_home_override_available is None:
        try:
            import flash_constants as mod  # noqa: F811
        except Exception:
            _flash_home_override_available = False
            return None
    if mod is not None and hasattr(mod, 'set_flash_home_override') and hasattr(
        mod, 'reset_flash_home_override'
    ):
        _flash_home_override_available = True
        return mod
    _flash_home_override_available = False
    return None
...
def profile_env_for_background_worker(
    ...
    try:
        ...
        profile_home_path = Path(get_flash_home_for_profile(profile))
    ...
    # worker body because several production Flash readers still call
    ...
    _scope_token = None
    _has_scope = False
    # #5567: context-local Flash-home override (flash-agent v0.18.0+). None on
    ...
    try:
        ...
        if _secret_scope_mod is not None:
            try:
                ...
                pass
        # #5567: install the context-local Flash-home override so the agent
        # config reader (get_flash_home -> get_config_path/load_config) resolves
        ...
        _home_override_mod = _resolve_flash_home_override()
        if _home_override_mod is not None:
            try:
                _home_override_token = _home_override_mod.set_flash_home_override(
        ...
        with _ENV_LOCK:
            ...
            had_flash_home = "HERMES_HOME" in os.environ
            old_flash_home = os.environ.get("HERMES_HOME")
        ...
        if _home_override_mod is not None and _home_override_token is not None:
            try:
                _home_override_mod.reset_flash_home_override(_home_override_token)
            except Exception:
                pass
        ...
        with _ENV_LOCK:
            ...
            if had_flash_home:
                os.environ["HERMES_HOME"] = old_flash_home or ""
            else:
                os.environ.pop("HERMES_HOME", None)
...
def profile_env_for_active_request_readonly(
    ...
    on a named profile sets a ``flash_profile`` cookie, which ``server.py``
    ...
    environment first. It also sets a context-local Flash-home override so
    ...
    try:
        ...
        reset_flash_home_override = None
        set_flash_home_override = None
    ...
    try:
        ...
        if _secret_scope_mod is not None:
            try:
                ...
                pass
        if set_flash_home_override is not None:
            home_override_token = set_flash_home_override(profile_home_path)
        yield
    finally:
        if _has_scope and _secret_scope_mod is not None:
            try:
                ...
                pass
        if home_override_token is not None and reset_flash_home_override is not None:
            try:
                reset_flash_home_override(home_override_token)
            except Exception:
                (logger_override or logger).debug(
                    "Failed to reset Flash-home override for active request profile %s in %s",
                    profile,
                    purpose,
                    exc_info=True,
                )
...
def profile_env_for_active_request(
    ...
    Some request-scoped readers still delegate into Flash helpers that resolve
    credentials directly from process env or ``get_flash_home()``. Those paths
...
def init_profile_state() -> None:
    ...
    Reads ~/.flash/active_profile, sets HERMES_HOME env var, patches
...
def _build_profile_rows_fast() -> list | None:
    ...
    ``flash_cli.profiles.list_profiles()`` calls ``find_alias_for_profile()``
    ...
    rows: list = []
    default_home = _get_default_flash_home()
    if default_home.is_dir():
        # Upstream hardcodes the base home's display name to "default" even when
        # the directory is literally ".flash" — match that exactly.
...
def list_profiles_api() -> list:
    ...
    In isolated profile mode (HERMES_HOME points to ~/.flash/profiles/<name>),
    ...
    if _is_isolated_profile_mode():
        active = _isolated_profile_name()
        flash_home = Path(_INITIAL_HERMES_HOME).expanduser()
        try:
            ...
            for p in infos:
                try:
                    same_home = Path(p.path).expanduser().resolve() == flash_home.resolve()
                except OSError:
                    same_home = False
            ...
            pass
        # Fallback: construct profile dict with actual active name and flash_home path
        enabled_count, total_count = _get_profile_skills_stats(flash_home)
        return [{
            'name': active,
            'path': str(flash_home),
            ...
            'model': None,
            'provider': None,
            'has_env': (flash_home / '.env').exists(),
            'visible': _profile_visible_from_meta(flash_home),
            'skill_count': enabled_count,
            'enabled_skills': enabled_count,
            'total_skills': total_count,
        }]
    ...
    if rows is None:
        ...
        # or the default-only dict if flash_cli isn't importable at all.
        logger.debug(
...
def _default_profile_dict() -> dict:
    """Fallback profile dict when flash_cli is not importable."""
...
def _validate_profile_name(name: str):
    """Validate profile name format (matches flash_cli.profiles upstream)."""
    if name == 'default':
...
def _resolve_named_profile_home(name: str) -> Path:
    ...
    final filesystem path and enforces containment under ~/.flash/profiles.
    """
...
def _create_profile_fallback(name: str, clone_from: str = None,
    ...
    """Create a profile directory without flash_cli (Docker/standalone fallback)."""
...
# the key must be written to the profile's .env file so that Flash Agent's
...
def create_profile_api(name: str, clone_from: str = None,
    ...
    # flash_cli and the webui runtime do not always agree on the exact root,
    ...
    # When flash_cli is not importable, list_profiles_api() also falls back
...
def delete_profile_api(name: str) -> dict:
    ...
    try:
        from flash_cli.profiles import delete_profile
