"""
Flash Web UI -- Optional state.db sync bridge.

Mirrors WebUI session metadata (token usage, title, model) into the
flash-agent state.db so that /insights, session lists, and cost
...
def _get_state_db(profile: Optional[str] = None):
    ...
    ``get_active_flash_home()`` lookup for backward compatibility,
    ...
    Returns None if flash_state is not importable, the explicit
    ...
    if profile is not None:
        ...
        try:
            ...
            if not (_is_root_profile(profile) or _PROFILE_ID_RE.fullmatch(profile)):
                ...
                return None
            flash_home = Path(_resolve_profile_home_for_name(profile)).expanduser().resolve()
        except Exception:
            logger.warning(
        ...
        try:
            ...
            logger.debug("Failed to resolve flash home, using default")
            flash_home = Path(os.getenv('HERMES_HOME', str(Path.home() / '.flash')))
