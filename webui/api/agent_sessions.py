"""Shared helpers for reading Flash Agent sessions from state.db."""
import logging
import sqlite3
...
def normalize_agent_session_source(raw_source: str | None) -> dict:
    """Return stable source metadata for Flash Agent session rows.
...
def is_cli_session_row(row: dict) -> bool:
    ...
    # that Flash discovers on disk and lists alongside CLI/TUI sessions. The
...
def _is_continuation_session(parent: dict | None, child: dict | None) -> bool:
    ...
    by ``flash -c`` also records a new child session; for sidebar projection it
...
def read_importable_agent_session_rows(
    ...
    Flash Agent can create rows in ``state.db.sessions`` before a session has
    ...
    sidebar. This mirrors Flash Agent CLI's session-list behaviour: interactive
    ...
    with closing(conn):
        ...
        # Older Flash Agent versions may not have source tracking. Without a
        ...
        if 'source' not in session_cols:
            log.warning(
                ...
                "(older flash-agent?). Agent sessions unavailable. "
                "Upgrade flash-agent to fix this.",
                db_path,
            )
            return []
        ...
        if limit is not None:
            ...
            # aggregating every historical Flash state.db session before
...
def read_session_lineage_metadata(db_path: Path, session_ids: list[str] | set[str]) -> dict[str, dict]:
    ...
    WebUI sessions are persisted as JSON files, but Flash Agent also mirrors
