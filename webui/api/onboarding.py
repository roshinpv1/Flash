"""Flash Web UI -- first-run onboarding helpers."""
...
_SUPPORTED_PROVIDER_SETUPS = {
    ...
    "lmstudio": {
        "label": "LM Studio",
        # Canonical env var matches the agent CLI runtime (flash_cli/auth.py:182,
...
_UNSUPPORTED_PROVIDER_NOTE = (
    ...
    "when your Flash config selects the corresponding provider."
)


def _get_active_flash_home() -> Path:
    try:
...
def probe_provider_endpoint(
    ...
    headers = {
        ...
        "User-Agent": "flash-webui-onboarding-probe",
    }
    if api_key:
...
def _provider_api_key_present(
    ...
    # xai, etc.), ask the flash_cli auth registry — it knows every provider's env
...
def _provider_oauth_authenticated(provider: str, flash_home: "Path") -> bool:
    ...
    requested Flash home. Known OAuth providers may store auth either in the
    legacy providers[provider_id] singleton state or in credential_pool entries
    used by current Flash runtime auth resolution.
    """
    provider = (provider or "").strip().lower()
...
def _status_from_runtime(cfg: dict, imports_ok: bool) -> dict:
    ...
    env_values = _load_env_file(_get_active_flash_home() / ".env")
    ...
    if provider_configured:
        ...
        if provider in _SUPPORTED_PROVIDER_SETUPS:
            ...
            if meta.get("key_optional"):
                ...
                if not provider_ready and meta.get("oauth_provider"):
                    provider_ready = _provider_oauth_authenticated(
                        str(meta.get("oauth_provider")), _get_active_flash_home()
                    )
        else:
            ...
            provider_ready = (
                ...
                or _provider_oauth_authenticated(provider, _get_active_flash_home())
            )
    ...
    if not _HERMES_FOUND or not imports_ok:
        ...
        note = (
            "Flash is not fully importable from the Web UI yet. Finish bootstrap or fix the "
        ...
        provider_name = _PROVIDER_DISPLAY.get(
            provider, provider.title() if provider else "Flash"
        )
        note = f"Flash is minimally configured and ready to chat via {provider_name}."
    elif provider_configured:
        state = "provider_incomplete"
        if provider == "custom" and not base_url:
            note_key = "onboarding_notice_custom_base_url_required"
            note = (
                "Flash has a saved provider/model selection, but the custom "
            ...
            note = (
                ...
                "Run 'flash auth' or 'flash model' in a terminal to complete "
                "setup, then reload the Web UI."
            )
        else:
            note_key = "onboarding_notice_provider_api_key_required"
            note = (
                "Flash has a saved provider/model selection but still needs the "
                "API key required to chat."
            )
    else:
        state = "needs_provider"
        note_key = "onboarding_notice_provider_choice_required"
        note = "Flash is installed, but you still need to choose a provider and save working credentials."
    ...
    return {
        ...
        "env_path": str(_get_active_flash_home() / ".env"),
    }
...
def _build_setup_catalog(cfg: dict) -> dict:
    ...
    # of a key input when the user has already authenticated via 'flash auth'.
    current_is_oauth = (
        current_provider not in _SUPPORTED_PROVIDER_SETUPS and bool(current_provider)
    ) or _provider_oauth_authenticated(current_provider, _get_active_flash_home())
...
def get_onboarding_status() -> dict:
    ...
    cfg = get_config()
    imports_ok, missing, errors = verify_flash_imports()
    ...
    # Auto-complete for existing Flash users: if config.yaml already exists
    # AND the provider is configured (or the system is chat_ready), treat onboarding
    # as done.  These users configured Flash via the CLI before the Web UI existed;
    ...
    # a git branch switch in the flash-agent repo).  Without this, a CLI-configured
    ...
    return {
        ...
        "settings": {
            ...
            "bot_name": settings.get("bot_name") or "Flash",
        },
        "system": {
            "flash_found": bool(_HERMES_FOUND),
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if Path(config_path).exists() and not body.get("confirm_overwrite"):
        return {
            "error": "config_exists",
            "message": (
                "Flash is already configured (config.yaml exists). "
    ...
    env_path = _get_active_flash_home() / ".env"
    ...
    if not api_key and not _provider_api_key_present(provider, cfg, env_values):
        ...
        oauth_ready = bool(provider_meta.get("oauth_provider")) and _provider_oauth_authenticated(
            str(provider_meta.get("oauth_provider")), _get_active_flash_home()
        )
    ...
    # Reload the flash_cli provider/config cache so the next streaming call
    # picks up the new key without requiring a server restart.
    try:
        from api.profiles import _reload_dotenv
        _reload_dotenv(_get_active_flash_home())
    except Exception:
    ...
    try:
        # flash_cli may cache config at import time; ask it to reload if possible.
        from flash_cli.config import reload as _cli_reload
        _cli_reload()
    except Exception:
        logger.debug("Failed to reload flash_cli config")
...
def apply_self_hosted_provider_setup(body: dict) -> dict:
    ...
    if api_key and env_var:
        _write_env_file(_get_active_flash_home() / ".env", {env_var: api_key})
