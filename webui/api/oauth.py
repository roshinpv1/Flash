...
tokens stay server-side and are persisted to the active Flash profile's
...
AUTH_JSON_PATH = Path.home() / ".flash" / "auth.json"
...
def _oauth_start_key(provider: str, flash_home: Path) -> tuple[str, str]:
    """Return the canonical single-flight key for onboarding OAuth starts."""
    return (_normalize_onboarding_oauth_provider(provider), str(Path(flash_home)))


def _oauth_start_lock(provider: str, flash_home: Path) -> threading.Lock:
    """Return the per provider/profile lock that serializes OAuth flow creation."""
    key = _oauth_start_key(provider, flash_home)
...
def _pending_oauth_flow_for_locked(provider: str, flash_home: Path) -> tuple[str, dict[str, Any]] | None:
    ...
    canonical_home = str(Path(flash_home))
    now = time.time()
    for flow_id, flow in _OAUTH_FLOWS.items():
        if flow.get("provider") != canonical_provider:
            continue
        if str(flow.get("flash_home") or "") != canonical_home:
            continue
...
def _pending_oauth_flow_for(provider: str, flash_home: Path) -> tuple[str, dict[str, Any]] | None:
    """Return an existing live onboarding OAuth flow for the provider/profile, if any."""
    with _OAUTH_FLOWS_LOCK:
        return _pending_oauth_flow_for_locked(provider, flash_home)
...
def _get_active_flash_home() -> Path:
    """Return the active Flash profile home directory, falling back to ~/.flash when profile resolution fails."""
    try:
        from api.profiles import get_active_flash_home

        return Path(get_active_flash_home())
    except Exception as exc:
        # Per Opus advisor on stage-296: log the silent fallback so a corrupt
        # profile state ending up writing tokens to ~/.flash (instead of the
        # active profile) is observable in logs rather than failing silently.
        logger.warning(
            "Falling back to ~/.flash for OAuth credential storage: "
...
def _persist_codex_credentials(flash_home: Path, token_data: dict[str, Any]) -> Path:
    ...
    auth_path = Path(flash_home) / "auth.json"
...
def _save_codex_credentials(token_data):
    ...
    return _persist_codex_credentials(_get_active_flash_home(), token_data)
...
def _clear_anthropic_env_values(flash_home: Path) -> None:
...
def _link_anthropic_credentials(flash_home: Path) -> None:
    """Link Flash to use Claude Code's credential store.

    Clears ANTHROPIC_TOKEN and ANTHROPIC_API_KEY from the Flash .env so
    ...
    """
    _clear_anthropic_env_values(flash_home)
...
def _run_anthropic_credential_worker(flow_id: str) -> None:
    ...
    while True:
        ...
        try:
            ...
            flash_home = Path(flow["flash_home"])
            _link_anthropic_credentials(flash_home)
            with _OAUTH_FLOWS_LOCK:
                current = _OAUTH_FLOWS.get(flow_id)
                if not current or current.get("status") != "pending":
                    ...
                    cancelled = False
            if cancelled:
                _remove_anthropic_link_marker(flash_home)
            return
        except Exception as exc:
...
def _remove_anthropic_link_marker(flash_home: Path) -> None:
    ...
    while True:
        ...
        try:
            ...
            with _OAUTH_FLOWS_LOCK:
                ...
                if not current or current.get("status") != "pending":
                    return
            _persist_codex_credentials(Path(live["flash_home"]), tokens)
...
def _start_anthropic_flow(flash_home: Path) -> dict[str, Any]:
    ...
    with _OAUTH_FLOWS_LOCK:
        existing = _pending_oauth_flow_for_locked("anthropic", flash_home)
...
def start_onboarding_oauth_flow(body: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if provider in _ANTHROPIC_PROVIDER_ALIASES:
        return _start_anthropic_flow(_get_active_flash_home())

    # Codex flow
    flash_home = _get_active_flash_home()
    ...
    with _oauth_start_lock("openai-codex", flash_home):
        with _OAUTH_FLOWS_LOCK:
            existing = _pending_oauth_flow_for_locked("openai-codex", flash_home)
            if existing is not None:
                flow_id, flow = existing
                return _public_start_payload(flow_id, flow)
