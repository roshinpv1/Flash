"""Expose flash-agent's COMMAND_REGISTRY to the webui frontend.

This module is the single integration point with flash_cli.commands.
If flash-agent is unavailable the endpoint degrades to an empty list
...
def list_commands(_registry=None) -> list[dict[str, Any]]:
    ...
    Returns empty list if flash_cli is not installed (graceful
    degradation -- the frontend has its own fallback minimum set).

    Args:
        _registry: Optional injected registry for testing. When None
            (production), imports COMMAND_REGISTRY from flash_cli.
    """
    if _registry is None:
        try:
            from flash_cli.commands import COMMAND_REGISTRY as _registry
        except ImportError:
            logger.warning("flash_cli.commands not importable -- /api/commands returns []")
            return []
...
def _run_codex_runtime_command(arg_string: str) -> str:
    """Execute Flash' shared Codex runtime switch for the active profile."""
    try:
        from flash_cli.codex_runtime_switch import apply, parse_args
    except Exception as exc:
...
def _run_credits_command() -> str:
    """Render Flash' shared credits view for the WebUI slash-command path."""
    try:
    ...
    if not getattr(view, "logged_in", False):
        return "Not logged into Nous. Run `flash auth login nous` in Flash CLI, then try /credits again."
...
def resolve_moa_config(preset: str | None = None) -> dict:
    try:
        ...
        raise RuntimeError("MoA runtime unavailable (flash-agent not installed or too old)") from exc
    try:
...
def execute_plugin_command(command: str) -> str:
    ...
    transport errors, matching Flash' existing slash-command UX.
    """
