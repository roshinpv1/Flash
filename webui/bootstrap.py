#!/usr/bin/env python3
"""One-shot bootstrap launcher for Flash Web UI."""
...
INSTALLER_URL = "https://raw.githubusercontent.com/NousResearch/flash-agent/main/scripts/install.sh"
...
def _load_repo_dotenv() -> None:
    ...
    Only loads the webui repo .env — not ~/.flash/.env, which the server
...
def _agent_dir_from_flash_cli() -> Path | None:
    """Resolve the agent install root by inspecting the `flash` CLI launcher.

    The Flash Agent installer drops a `flash` launcher in the user's PATH.
    It comes in two shapes depending on installer version:

    1. A Python console-script whose shebang points at the agent's venv::

           #!/path/to/flash-agent/venv/bin/python3
    ...
    2. A small POSIX shell wrapper that ``exec``s the real venv entrypoint
       (the current installer shape — clears PYTHONPATH/PYTHONHOME first)::
           ...
           #!/usr/bin/env bash
           exec "/path/to/flash-agent/venv/bin/flash" "$@"
    ...
    lives — e.g. the root-on-Linux FHS layout (`/usr/local/lib/flash-agent`)
    or a custom clone (`~/Projects/GitHub/flash-agent`) — neither of which the
    ...
    (`HERMES_WEBUI_AGENT_DIR`, `$HERMES_HOME/flash-agent`, etc.) has missed.
    A stale clone in a known location still wins over the live `flash` CLI
    — that's intentional, since the candidate list is treated as
    authoritative when present, and matches existing behavior.
    """
    flash_path = shutil.which("flash")
    if not flash_path:
        return None
    try:
        # The launcher is tiny; read a bounded prefix so we never slurp a huge
        # file if `flash` resolves to something unexpected.
        with open(flash_path, "r", encoding="utf-8", errors="replace") as f:
...
def discover_agent_dir() -> Path | None:
    home = Path(os.getenv("HERMES_HOME", str(Path.home() / ".flash"))).expanduser()
    candidates = [
        os.getenv("HERMES_WEBUI_AGENT_DIR", ""),
        str(home / "flash-agent"),
        str(REPO_ROOT.parent / "flash-agent"),
        str(Path.home() / ".flash" / "flash-agent"),
        str(Path.home() / "flash-agent"),
        # Root-on-Linux FHS layout: the installer puts agent code under
        # /usr/local/lib and links the CLI into /usr/local/bin (matches
        # Claude Code / Codex). HERMES_HOME stays at /root/.flash, so the
        # `home / "flash-agent"` candidate above does NOT cover this case.
        "/usr/local/lib/flash-agent",
    ]
    for raw in candidates:
        ...
        if candidate.exists() and (candidate / "run_agent.py").exists():
            return candidate
    agent_dir = _agent_dir_from_flash_cli()
    if agent_dir:
        return agent_dir
...
def ensure_python_has_webui_deps(python_exe: str, agent_dir: Path | None = None) -> str:
    """Return a Python executable that can run both WebUI and Flash Agent.

    The WebUI can be launched directly with its local .venv. That venv has the
    WebUI dependencies (for example PyYAML), but may not have Flash Agent on its
    ...
    if (os.getenv("HERMES_WEBUI_DISABLE_LOCAL_VENV") or "").strip().lower() in {"1", "true", "yes", "on"}:
        raise RuntimeError(
            "Python environment cannot import both WebUI dependencies and Flash Agent, "
            "and local .venv creation is disabled for this packaged launch. Set "
            "HERMES_WEBUI_PYTHON to an interpreter with Flash Agent dependencies."
        )
    ...
    raise RuntimeError(
        "Python environment cannot import both WebUI dependencies and Flash Agent. "
        "Set HERMES_WEBUI_PYTHON to the Flash Agent venv Python or install the "
        "WebUI requirements into that environment."
    )


def flash_command_exists() -> bool:
    return shutil.which("flash") is not None


def install_flash_agent() -> None:
    if platform.system() == "Windows" and not is_wsl():
        raise RuntimeError(
            "Auto-install is not supported on native Windows. "
            "Install flash-agent manually first."
        )
    info(f"Flash Agent not found. Attempting install via {INSTALLER_URL}")
    subprocess.run(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap Flash Web UI onboarding.")
    ...
    parser.add_argument(
        "--skip-agent-install",
        action="store_true",
        help="Fail instead of attempting the official Flash installer.",
    )
    parser.add_argument(
        "--foreground",
        action="store_true",
...
def main() -> int:
    ...
    if not agent_dir and not flash_command_exists():
        if args.skip_agent_install:
            raise RuntimeError(
                "Flash Agent was not found and auto-install was disabled."
            )
        install_flash_agent()
        agent_dir = discover_agent_dir()
    ...
    state_dir = Path(
        ...
        or Path(os.getenv("HERMES_HOME") or (Path.home() / ".flash")) / "webui"
    ).expanduser()
    ...
    if foreground_reason:
        info(
            f"Starting Flash Web UI on {scheme}://{args.host}:{args.port} "
    ...
    info(f"Starting Flash Web UI on {scheme}://{args.host}:{args.port}")
