...
# reasoning: needed when using docker compose as the file will exist in the stopped container, and changing the value from environment variables or configuration file must be propagated from the root init phase to the flashwebui runtime phase
it=$itdir/flashwebui_user_uid
...
# In two-container setups (flash-agent + flash-webui), the shared flash-home
# volume may be owned by the agent container's UID — detect from there first.
if [ -z "${WANTED_UID+x}" ] || [ "${WANTED_UID}" = "1024" ]; then
  # Priority 1: flash-home shared volume — covers two-container Zeabur/Compose setups (#668)
  for _probe_dir in "/home/flashwebui/.flash" "$HERMES_HOME" "/opt/data"; do
...
if [ -z "${WANTED_GID+x}" ] || [ "${WANTED_GID}" = "1024" ]; then
  # Priority 1: flash-home shared volume
  for _probe_dir in "/home/flashwebui/.flash" "$HERMES_HOME" "/opt/data"; do
    if [ -d "$_probe_dir" ]; then
...
chown_home_flashwebui() {
  # macOS Docker bind mounts can expose flash-agent git object packs as
  ...
  # flash-agent-src volume read-only on the WebUI side because the WebUI only
  ...
  # source — prune the entire flash-agent path from the chown walk so a
  # read-only or partially-read-only mount doesn't break the rest of the home
  # ownership alignment.
  find /home/flashwebui \
    -path "/home/flashwebui/.flash/flash-agent" -prune \
    -o -name ".git" -prune \
    -o -exec chown -h "${WANTED_UID}:${WANTED_GID}" {} +
}

# The production image does not ship sudo. The entrypoint starts as root only
# long enough to align the flashwebui UID/GID with mounted volumes, prepare
# root-owned paths, and then drop privileges for the server process.
if [ "A${whoami}" == "Aroot" ]; then
  echo "-- Running as root for one-time container init; will switch to flashwebui"

  # We are altering the UID/GID of the flashwebui user to the desired ones and restarting as that user
  # using usermod for the already created flashwebui user, knowing it is not already in use
  ...
  if [ "A${_readonly_root}" == "Atrue" ]; then
    _current_flashwebui_gid=$(id -g flashwebui 2>/dev/null || echo "")
    _current_flashwebui_uid=$(id -u flashwebui 2>/dev/null || echo "")
    if [ "A${_current_flashwebui_gid}" == "A${WANTED_GID}" ] && [ "A${_current_flashwebui_uid}" == "A${WANTED_UID}" ]; then
      echo "  -- Skipping groupmod/usermod — flashwebui already has UID ${WANTED_UID} GID ${WANTED_GID} and root fs is read-only"
    else
      error_exit "Cannot modify /etc/group or /etc/passwd (read-only root fs). Set UID=${_current_flashwebui_uid} and GID=${_current_flashwebui_gid} to match, or run without read_only=true. See issue #1470."
    fi
  else
    groupmod -o -g "${WANTED_GID}" flashwebui || error_exit "Failed to set GID of flashwebui user"
    usermod -o -u "${WANTED_UID}" flashwebui || error_exit "Failed to set UID of flashwebui user"
  fi

  chown_home_flashwebui || error_exit "Failed to set owner of /home/flashwebui"

  echo ""; echo "-- Preparing /app for the flashwebui runtime user"
  mkdir -p /app || error_exit "Failed to create /app directory"
  chown flashwebui:flashwebui /app || error_exit "Failed to set owner of /app to flashwebui user"
  rsync -av --chown=flashwebui:flashwebui /apptoo/ /app/ || error_exit "Failed to sync /apptoo to /app with correct ownership"
  ...
  chown flashwebui:flashwebui "$HERMES_WEBUI_DEFAULT_WORKSPACE" 2>/dev/null || echo "!! WARNING: Could not chown $HERMES_WEBUI_DEFAULT_WORKSPACE (continuing)"

  export UV_CACHE_DIR=${UV_CACHE_DIR:-/uv_cache}
  mkdir -p "${UV_CACHE_DIR}" || error_exit "Failed to create ${UV_CACHE_DIR} directory"
  chown flashwebui:flashwebui "${UV_CACHE_DIR}" || error_exit "Failed to set owner of ${UV_CACHE_DIR} to flashwebui user"
  ...
  ENV_FILE="/tmp/flashwebui_root_env.txt"
  if ! ( : > "$ENV_FILE" ) 2>/dev/null; then
    ENV_FILE="${itdir:-/tmp/flashwebui_init}/flashwebui_root_env.txt"
    mkdir -p "$(dirname "$ENV_FILE")" 2>/dev/null
    if ! ( : > "$ENV_FILE" ) 2>/dev/null; then
      ENV_FILE="/app/.flashwebui_root_env"
    fi
  ...
  # visible to flashwebui before re-entering as the runtime user.
  for gid in $(id -G); do
    ...
    if [ -n "$group_name" ]; then
      usermod -a -G "$group_name" flashwebui 2>/dev/null || echo "!! WARNING: Could not add flashwebui to supplemental group $group_name ($gid)"
    fi
  done

  # restart the script as flashwebui set with the correct UID/GID this time
  echo "-- Restarting as flashwebui user with UID ${WANTED_UID} GID ${WANTED_GID}"
  exec su -s /bin/bash -c "exec \"${script_fullname}\"" flashwebui || error_exit "subscript failed"
fi

# If we are here, the script is started as an unprivileged runtime user.
# Because the whoami value for the flashwebui user can be any existing user, we cannot check against it;
# instead we check if the UID/GID are the expected ones.
if [ "$WANTED_GID" != "$new_gid" ]; then error_exit "flashwebui MUST be running as UID ${WANTED_UID} GID ${WANTED_GID}, current UID ${new_uid} GID ${new_gid}"; fi
if [ "$WANTED_UID" != "$new_uid" ]; then error_exit "flashwebui MUST be running as UID ${WANTED_UID} GID ${WANTED_GID}, current UID ${new_uid} GID ${new_gid}"; fi

########## 'flashwebui' specific section below

# We are therefore running as flashwebui
echo ""; echo "== Running as flashwebui"

# Load environment variables one by one if they do not exist from the root init phase
tmp_root_env="${_HW_ROOT_ENV_PATH:-/tmp/flashwebui_root_env.txt}"
...
echo ""; echo "-- Verifying /app is writable by the flashwebui runtime user"
...
echo ""; echo "== Checking required environment variables for flash-webui"

echo ""; echo "-- HERMES_WEBUI_STATE_DIR: Where to store sessions, workspaces, and other state (default: ~/.flash/webui)"
...
echo ""; echo "==================="
echo ""; echo "== Installing uv and creating a new virtual environment for flash-webui"

export PATH="/home/flashwebui/.local/bin/:$PATH"
...
export UV_CACHE_DIR=${UV_CACHE_DIR:-/uv_cache}
mkdir -p "${UV_CACHE_DIR}" || error_exit "Failed to create ${UV_CACHE_DIR} directory"
test -w "${UV_CACHE_DIR}" || error_exit "${UV_CACHE_DIR} is not writable by flashwebui"
...
echo "";echo "== Activating flash webui's virtual environment"
source /app/venv/bin/activate || error_exit "Failed to activate flashwebui virtual environment"
...
if [ -f /app/venv/.deps_installed ]; then
  ...
  echo ""; echo "== Installing flash-webui dependencies"
  ...
  echo ""; echo "== Adding flash-agent's pyproject.toml base dependencies to the virtual environment"
  _agent_paths=(
    "/home/flashwebui/.flash/flash-agent"
    "/opt/flash"
  )
  _agent_src=""
  ...
  if [ -n "$_agent_src" ]; then
    if [ -w "$_agent_src" ]; then
      echo ""
      echo "!! WARNING: flash-agent source mount is writable from the WebUI container."
    ...
    # `egg_info` build step, however, touches `flash_agent.egg-info/` inside the
    ...
    # then can't create `flash_agent.egg-info/` next to the package — it dies
    # with "Permission denied" even though `_stage_src` itself was created
    # writable by flashwebui. Re-add owner-write on the staged tree after the
    # copy so the build dir is genuinely writable, not just owned by us.
    _stage_src="/tmp/flash-agent-build"
    rm -rf "$_stage_src"
    mkdir -p "$_stage_src"
    if command -v rsync >/dev/null 2>&1; then
      rsync -a \
        ...
        || error_exit "Failed to stage flash-agent source to writable build dir"
    else
      ...
      cp -a "$_agent_src"/. "$_stage_src"/ \
        || error_exit "Failed to copy flash-agent source to writable build dir"
    ...
    chmod -R u+w "$_stage_src" \
      || error_exit "Failed to make staged flash-agent source writable (rsync/cp preserved :ro mount perms)"
    uv pip install "$_stage_src[all]" --trusted-host pypi.org --trusted-host files.pythonhosted.org \
      || error_exit "Failed to install flash-agent's requirements"
    rm -rf "$_stage_src"
  else
    echo ""
    echo "!! WARNING: flash-agent source not found."
    ...
    echo "!!   -v /path/to/flash-agent:/home/flashwebui/.flash/flash-agent"
    echo "!! Or see the two-container compose example:"
    echo "!!   https://github.com/nesquena/flash-webui/blob/master/docker-compose.two-container.yml"
    echo ""
  fi
...
echo ""; echo "== Running flash-webui"
cd /app; python server.py || error_exit "flash-webui failed or exited with an error"
