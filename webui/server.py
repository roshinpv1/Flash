"""Flash Web UI server entry point."""
import logging
import os
import re
import signal
...
if os.environ.get("HERMES_WEBUI_TEST_NETWORK_BLOCK", "").strip() in ("1", "true", "yes"):
    ...
    def _blocked_create_connection(address, *a, **kw):
        ...
        raise OSError(
            f"flash test network isolation (server.py): outbound to {address!r} blocked"
        )
    ...
    def _blocked_socket_connect(self, address):
        ...
        raise OSError(
            f"flash test network isolation (server.py): socket.connect to {address!r} blocked"
        )
...
class Handler(BaseHTTPRequestHandler):
    ...
    server_version = ('FlashWebUI/' + _ver_suffix) if _ver_suffix != 'unknown' else 'FlashWebUI'
...
def main() -> None:
    from api.config import print_startup_config, verify_flash_imports, _HERMES_FOUND
    ...
    ok, missing, errors = verify_flash_imports()
    if not ok and _HERMES_FOUND:
        print(f'[!!] Warning: Flash agent found but missing modules: {missing}', flush=True)
    ...
    print(f'  Flash Web UI listening on {scheme}://{HOST}:{PORT}', flush=True)
