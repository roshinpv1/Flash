...
per-request flash_profile cookie via the thread-local):
...
def test_profile_active_includes_default_workspace_from_resolver(monkeypatch):
    ...
    monkeypatch.setattr(profiles, "get_active_flash_home", lambda: "/home/u/.flash/profiles/work")
...
def test_profile_active_default_workspace_resolves_from_named_profile(monkeypatch, tmp_path):
    ...
    Mirrors the real boot path — the per-request flash_profile cookie sets the
    ...
    default.
    """
    # ── Single-user layout: base ~/.flash with a named 'work' profile ──
    ...
    # Simulate the request carrying a flash_profile=work cookie.
