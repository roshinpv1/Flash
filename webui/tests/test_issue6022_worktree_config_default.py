...
def _mk_repo_dirs(tmp_path):
    repo = tmp_path / "repo"
    worktree = repo / ".worktrees" / "flash-6022"
    repo.mkdir()
...
def test_absent_key_with_config_default_on_creates_worktree_session(tmp_path, monkeypatch):
    ...
    assert session["worktree_branch"] == "flash/flash-6022"
...
def test_worktree_default_resolves_named_profile_home(monkeypatch, tmp_path):
    ...
    seen = {}
    monkeypatch.setattr(
        profiles, "get_flash_home_for_profile", lambda name: tmp_path / name
    )
