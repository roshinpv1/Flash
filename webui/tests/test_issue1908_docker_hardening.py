...
def test_production_dockerfile_does_not_grant_passwordless_sudo():
    ...
    assert "adduser flashwebui sudo" not in DOCKERFILE
    assert "adduser flashwebuitoo sudo" not in DOCKERFILE
    assert "flashwebuitoo" not in DOCKERFILE, "production image should not keep a sudo-capable staging user"
...
def test_init_script_does_not_depend_on_sudo_at_runtime():
    ...
    assert re.search(r"\bsu\b.*\bflashwebui\b", INIT_SCRIPT), (
        "docker_init.bash must drop from root to flashwebui before launching the server"
    )
...
def test_init_script_uses_private_scratch_permissions():
    ...
    assert re.search(r"chmod\s+700\s+\"?\$itdir\"?", INIT_SCRIPT), (
        "/tmp/flashwebui_init should be mode 700"
    )
...
def test_docker_docs_explain_production_privilege_model():
    ...
    assert "root" in hardening_section and "flashwebui" in hardening_section
