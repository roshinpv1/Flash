...
def test_remove_clean_worktree_succeeds(tmp_path):
    ...
    subprocess.run(
        ["git", "-C", str(main), "worktree", "add", str(wt_path), "-b", "flash/testclean"],
    ...
    s = Session(
        ...
        worktree_branch="flash/testclean",
...
def test_remove_locked_worktree_succeeds_without_force(tmp_path):
    ...
    subprocess.run(
        ["git", "-C", str(main), "worktree", "add", str(wt_path), "-b", "flash/testlocked"],
        check=True, capture_output=True,
    )
    # Reproduce the agent's creation-time lock.
    subprocess.run(
        ["git", "-C", str(main), "worktree", "lock", "--reason", "flash pid=12345", str(wt_path)],
        check=True, capture_output=True,
    )
    ...
    s = Session(
        ...
        workspace=str(wt_path),
        worktree_path=str(wt_path),
        worktree_branch="flash/testlocked",
        worktree_repo_root=str(main),
    )
...
def test_remove_never_locked_worktree_unlock_is_fail_soft(tmp_path):
    ...
    subprocess.run(
        ["git", "-C", str(main), "worktree", "add", str(wt_path), "-b", "flash/testneverlocked"],
        check=True, capture_output=True,
    )
    ...
    s = Session(
        ...
        workspace=str(wt_path),
        worktree_path=str(wt_path),
        worktree_branch="flash/testneverlocked",
        worktree_repo_root=str(main),
    )
...
def test_remove_clean_worktree_does_not_force(tmp_path, monkeypatch):
    ...
    s = Session(
        ...
        worktree_branch="flash/testcleanforce",
...
def test_remove_dirty_worktree_without_force_is_rejected(tmp_path, monkeypatch):
    ...
    s = Session(
        ...
        workspace=str(worktree_path),
        worktree_path=str(worktree_path),
        worktree_branch="flash/testdirty",
        worktree_repo_root=str(repo_root),
    )
    monkeypatch.setattr(worktrees, "worktree_status_for_session", lambda session: {
        "exists": True,
...
def test_remove_untracked_worktree_without_force_is_rejected(tmp_path, monkeypatch):
    ...
    s = Session(
        ...
        workspace=str(worktree_path),
        worktree_path=str(worktree_path),
        worktree_branch="flash/testuntracked",
        worktree_repo_root=str(repo_root),
    )
    monkeypatch.setattr(worktrees, "worktree_status_for_session", lambda session: {
        "exists": True,
...
def test_remove_ahead_worktree_without_force_is_rejected(tmp_path, monkeypatch):
    ...
    s = Session(
        ...
        workspace=str(worktree_path),
        worktree_path=str(worktree_path),
        worktree_branch="flash/testahead",
        worktree_repo_root=str(repo_root),
    )
    monkeypatch.setattr(worktrees, "worktree_status_for_session", lambda session: {
        "exists": True,
...
def test_remove_force_warns_and_uses_git_force(tmp_path, monkeypatch):
    ...
    s = Session(
        ...
        workspace=str(worktree_path),
        worktree_path=str(worktree_path),
        worktree_branch="flash/testforce",
        worktree_repo_root=str(repo_root),
    )
    monkeypatch.setattr(worktrees, "worktree_status_for_session", lambda session: {
        "exists": True,
...
def test_remove_worktree_route_succeeds(tmp_path, monkeypatch):
    ...
    subprocess.run(
        ["git", "-C", str(main), "worktree", "add", str(wt_path), "-b", "flash/testroute"],
        check=True, capture_output=True,
    )
    ...
    s = Session(
        ...
        workspace=str(wt_path),
        worktree_path=str(wt_path),
        worktree_branch="flash/testroute",
        worktree_repo_root=str(main),
    )
    s.save()
