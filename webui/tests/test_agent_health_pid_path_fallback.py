...
<flash_root>/gateway.pid.  Profile-scoped gateways (running via
``gateway run --profile <name>`` or with ``active_profile`` set) write
their PID file under <flash_root>/profiles/<name>/gateway.pid instead of
...
pytest.importorskip("flash_constants", reason="flash-agent not installed")
...
def _call(monkeypatch, root: Path, profile_dir: Path | None = None) -> Path | None:
    ...
    if profile_dir is not None:
        monkeypatch.setattr(profiles, "get_active_flash_home", lambda: str(profile_dir))
...
def test_returns_root_path_when_profile_lookup_raises(tmp_path, monkeypatch):
    """get_active_flash_home() raising must be caught; root path returned."""
    root = tmp_path / "flash"
    root.mkdir()
    ...
    monkeypatch.setattr(profiles, "get_active_flash_home", _raise)
