...
def _make_flash_home(base: Path, profile_name: str | None = None) -> Path:
    """Create a temp HERMES_HOME (with optional profile dir) and return it."""
    flash_home = base / ".flash"
    flash_home.mkdir(parents=True, exist_ok=True)
    if profile_name:
        (flash_home / "profiles" / profile_name).mkdir(parents=True, exist_ok=True)
    return flash_home


# ── tests ────────────────────────────────────────────────────────────────────

class TestGetFlashHomeForProfile:
    """get_flash_home_for_profile() must return the profile path regardless of
    ...
    def test_existing_profile_returns_profile_dir(self, _patch_default_home):
        ...
        result = get_flash_home_for_profile("ayan")
    ...
    def test_nonexistent_profile_still_returns_profile_path(self, _patch_default_home):
        ...
        result = get_flash_home_for_profile("newprofile")
    ...
    def test_none_returns_default(self, _patch_default_home):
        ...
        result = get_flash_home_for_profile(None)
    ...
    def test_empty_string_returns_default(self, _patch_default_home):
        ...
        result = get_flash_home_for_profile("")
        assert result == fake_home
    ...
    def test_default_string_returns_default(self, _patch_default_home):
        ...
        result = get_flash_home_for_profile("default")
        assert result == fake_home
