...
def test_workspace_panel_saves_to_localstorage():
    """_setWorkspacePanelMode must call localStorage.setItem with flash-webui-workspace-panel."""
    assert "flash-webui-workspace-panel" in BOOT_JS, \
        "boot.js must use localStorage key 'flash-webui-workspace-panel' to persist panel state"
...
def test_workspace_panel_save_inside_set_mode():
    ...
    assert "flash-webui-workspace-panel" in fn_body, \
...
def test_workspace_panel_restored_on_boot():
    """Boot IIFE must read flash-webui-workspace-panel from localStorage and restore the mode."""
    ...
    assert "flash-webui-workspace-panel" in iife_body, \
        "Boot IIFE must read 'flash-webui-workspace-panel' from localStorage to restore panel state on load"
...
def test_workspace_panel_restore_before_sync():
    ...
    restore_pos = iife_body.find("flash-webui-workspace-panel-pref")
