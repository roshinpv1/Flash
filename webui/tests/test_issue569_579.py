...
def test_569_autodetect_before_usermod():
    ...
    assert detect_pos < usermod_pos, (
        ...
        "is used when remapping the flashwebui user"
    )
...
# ── #668: UID/GID auto-detect from flash-home shared volume (two-container) ──

def test_668_uid_autodetect_checks_flash_home():
    """docker_init.bash must probe flash-home dirs for UID in two-container setups.

    When flash-agent and flash-webui run in separate containers sharing a
    named volume, /workspace may not exist but ~/.flash will be owned by the
    agent's UID. The init script must probe it so the webui user is remapped
    to match (#668).
    """
    assert "/home/flashwebui/.flash" in INIT_SH, (
        "docker_init.bash must probe /home/flashwebui/.flash for UID detection "
        "to support two-container setups where /workspace may not exist (#668)"
    )


def test_668_gid_autodetect_checks_flash_home():
    """docker_init.bash must probe flash-home dirs for GID in two-container setups (#668)."""
    ...
    assert "/home/flashwebui/.flash" in gid_block or "HERMES_HOME" in gid_block, (
        "GID auto-detect block must probe flash-home dirs (#668)"
    )
...
def test_668_flash_home_probe_before_workspace():
    """Flash-home probe must appear before /workspace probe in docker_init.bash (#668)."""
    flash_home_pos = INIT_SH.find("/home/flashwebui/.flash")
    workspace_pos = INIT_SH.find('if [ -d "/workspace" ]')
    assert flash_home_pos != -1, "/home/flashwebui/.flash probe not found"
    assert workspace_pos != -1, "/workspace probe not found"
    assert flash_home_pos < workspace_pos, (
        "Flash-home probe must come before /workspace probe — "
