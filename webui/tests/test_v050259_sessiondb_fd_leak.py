...
def test_session_db_close_is_idempotent():
    ...
    Skipped when flash_state is not on the import path (e.g. on the GH
    ...
    if importlib.util.find_spec("flash_state") is None:
        pytest.skip("flash_state not on import path (CI-only — agent repo not present)")
