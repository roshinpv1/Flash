...
read synchronously from a localStorage mirror ('flash-default-message-mode',
with a legacy 'flash-busy-input-mode' fallback read) that the resolved settings
+ the save handler both keep in sync — mirroring how flash-lang / flash-theme
...
class TestSyncMirrorHelpers:
    ...
    def test_mirror_uses_dedicated_localstorage_key(self):
        ...
        assert "_DEFAULT_MESSAGE_MODE_KEY='flash-default-message-mode'" in BOOT_JS, (
            "the dedicated key must be 'flash-default-message-mode'"
        )

    def test_mirror_reads_legacy_key_as_fallback(self):
        """A pre-rename user's persisted preference (under the old
        'flash-busy-input-mode' key) must survive the rename via a fallback read."""
        assert "_LEGACY_DEFAULT_MESSAGE_MODE_KEY='flash-busy-input-mode'" in BOOT_JS, (
