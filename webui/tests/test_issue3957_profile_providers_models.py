...
def test_active_request_scope_sets_context_local_flash_home(monkeypatch, tmp_path):
    """Request scope keeps agent-side Flash-home readers on the active profile."""
    base = tmp_path / ".flash"
    ...
    fake_constants.set_flash_home_override = _set_override
    fake_constants.reset_flash_home_override = _reset_override
    fake_constants.get_flash_home = lambda: current_home["value"]
    monkeypatch.setitem(sys.modules, "flash_constants", fake_constants)

    profiles.set_request_profile("work")
    try:
        with profiles.profile_env_for_active_request_readonly("test"):
            assert fake_constants.get_flash_home() == work_home
    finally:
        profiles.clear_request_profile()


def test_active_request_scope_restores_state_when_home_reset_fails(monkeypatch, tmp_path):
    """Readonly scope still clears thread-local state if Flash-home reset raises."""
    base = tmp_path / ".flash"
    work_home = base / "profiles" / "work"
    work_home.mkdir(parents=True)
...
def test_active_request_readonly_scope_blocks_pool_env_seed(monkeypatch, tmp_path):
    ...
    # the root via get_default_flash_root(), which reads os.environ["HERMES_HOME"]
