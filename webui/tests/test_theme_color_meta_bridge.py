...
- index.html declares a single `id="flash-theme-color"` meta tag for runtime updates.
- Inline pre-paint script reads localStorage `flash-theme` and seeds the meta tag
...
(flash-webui/flash-swift-mac) read instead of pixel-sampling the page —
...
class TestIndexHtmlMetaTags:
    ...
    def test_runtime_theme_color_meta_has_stable_id(self):
        """A third theme-color meta tag (no media query) carries id="flash-theme-color"
        so boot.js can update it on theme/skin change. The id is the contract the
        Mac Swift app reads via `document.getElementById('flash-theme-color')`.
        """
        src = INDEX.read_text(encoding="utf-8")
        assert 'id="flash-theme-color"' in src
        # Must be on a meta tag (not some other element)
        assert '<meta name="theme-color" id="flash-theme-color"' in src
...
class TestBootJsThemeColorSync:
    ...
    def test_sync_helper_updates_all_theme_color_tags(self):
        ...
        """
        src = BOOT.read_text(encoding="utf-8")
        assert "getElementById('flash-theme-color')" in src
        assert "querySelectorAll('meta[name=\"theme-color\"]')" in src
