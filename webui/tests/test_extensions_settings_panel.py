...
def test_extensions_installed_settings_route_through_shared_accessor():
    ...
    assert "window.FlashExtensionSettings.settingsForExtension(id)" in settings_block
    ...
    assert "window.FlashExtensionSettings.settingsForExtension(id).reset()" in bind_block
    assert "window.FlashExtensionSettings.storageForExtension(id).clear()" in bind_block
...
def test_extensions_gallery_links_sources_and_humanizes_permissions():
    ...
    assert "flash-webui/flash-webui-extensions/tree/main" in helper_block
...
def test_extensions_docs_mentions_settings_panel_without_install_or_proxy_claims():
    ...
    assert "`window.FlashExtensionSettings`" in DOCS_EXTENSIONS
