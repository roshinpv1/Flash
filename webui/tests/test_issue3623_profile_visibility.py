...
def _install_fake_flash_profiles(monkeypatch, rows):
    flash_cli = types.ModuleType("flash_cli")
    profiles_mod = types.ModuleType("flash_cli.profiles")
    profiles_mod.list_profiles = lambda: rows
    monkeypatch.setitem(sys.modules, "flash_cli", flash_cli)
    monkeypatch.setitem(sys.modules, "flash_cli.profiles", profiles_mod)
...
def test_profile_yaml_visible_false_is_exposed_as_hidden(monkeypatch, tmp_path):
    ...
    _install_fake_flash_profiles(monkeypatch, rows)
