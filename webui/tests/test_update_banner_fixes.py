...
class TestUpdateChecker:
    def test_build_compare_url_requires_all_pieces(self):
        ...
        assert upd._build_compare_url(
            'https://github.com/nesquena/flash-webui', 'abc1234', 'def5678'
        ) == 'https://github.com/nesquena/flash-webui/compare/abc1234...def5678'
        assert upd._build_compare_url(None, 'abc1234', 'def5678') is None
        assert upd._build_compare_url('https://github.com/nesquena/flash-webui', None, 'def5678') is None
        assert upd._build_compare_url('https://github.com/nesquena/flash-webui', 'abc1234', None) is None
    ...
    def test_build_compare_url_rejects_unsafe_remote_urls(self):
        ...
        assert upd._build_compare_url('file:///tmp/flash-webui', 'abc1234', 'def5678') is None
        assert upd._build_compare_url('https:github.com/nesquena/flash-webui', 'abc1234', 'def5678') is None
        assert upd._build_compare_url('https://github.com/nesquena/flash-webui', 'abc1234', 'def5678')
    ...
    def test_check_repo_includes_compare_url_from_normalized_remote_and_merge_base(self, tmp_path, monkeypatch):
        ...
        def fake_run(args, cwd, timeout=10):
            ...
            if args[:2] == ['remote', 'get-url']:
                return 'git@github.com:NousResearch/flash-agent.git', True
            return '', True

        monkeypatch.setattr(upd, '_run_git', fake_run)
        result = upd._check_repo(tmp_path, 'agent')

        assert result['repo_url'] == 'https://github.com/NousResearch/flash-agent'
        assert result['current_sha'] == 'abcdef1'
        assert result['latest_sha'] == 'def5678'
        assert result['compare_url'] == 'https://github.com/NousResearch/flash-agent/compare/abcdef1...def5678'
    ...
    def test_check_repo_omits_compare_url_when_merge_base_missing(self, tmp_path, monkeypatch):
        ...
        def fake_run(args, cwd, timeout=10):
            ...
            if args[:3] == ['rev-parse', '--short', 'origin/master']:
                return 'def5678', True
            if args[:2] == ['remote', 'get-url']:
                return 'https://github.com/nesquena/flash-webui.git', True
            return '', True
    ...
    def test_repo_url_strips_only_dot_git_suffix(self, tmp_path, monkeypatch):
        ...
        assert result['repo_url'] == 'https://github.com/nesquena/flash-webui'
    ...
    def test_release_check_ignores_post_release_branch_commits(self, tmp_path, monkeypatch):
        ...
        def fake_run(args, cwd, timeout=10):
            ...
            if args[:2] == ['remote', 'get-url']:
                return 'https://github.com/NousResearch/flash-agent.git', True
            if args[:2] == ['rev-parse', '--abbrev-ref']:
                return 'origin/main', True
            if args[:2] == ['rev-list', '--count']:
                return '16', True
    ...
    def test_detect_agent_version_reads_copied_source_tree(self, tmp_path, monkeypatch):
        ...
        agent_dir = tmp_path / 'flash-agent'
        package_dir = agent_dir / 'flash_cli'
    ...
    def test_detect_agent_version_falls_back_to_gateway_health(self, monkeypatch):
        ...
        class FakeResponse:
            ...
            def read(self):
                return b'{"status":"ok","platform":"flash-agent","version":"0.14.1"}'
        ...
        monkeypatch.setenv('GATEWAY_HEALTH_URL', 'http://flash-agent:8642/health')
        monkeypatch.setattr(upd.urllib.request, 'urlopen', fake_urlopen)

        assert upd._detect_agent_version() == '0.14.1'
        assert seen == [('http://flash-agent:8642/health', 0.75)]
...
class TestAgentUpdateRequiresGatewayRestart:
    ...
    def test_agent_gateway_restart_real_profile_seam_unchanged_default_pid_fails(
        ...
        monkeypatch.setattr(gateway_restart.shutil, "which", lambda cmd: "/mock/bin/flash")
        ...
        assert [call[0] for call in calls["popen"]] == [
            ["/mock/bin/flash", "--profile", "default", "gateway", "restart"],
            ["/mock/bin/flash", "--profile", "default", "gateway", "restart"],
        ]
    ...
    def test_agent_gateway_restart_isolated_default_home_omits_profile_flag(
        ...
        assert [call[0] for call in calls["popen"]] == [
            ["/mock/bin/flash", "gateway", "restart"],
            ["/mock/bin/flash", "gateway", "restart"],
        ]
    ...
    def test_apply_update_agent_without_gateway_restart_result_fails(self, tmp_path, monkeypatch):
        ...
        assert 'flash gateway restart' in result['message']
...
class TestUpdateSummaryRouteModelSelection:
    ...
    def test_summary_route_auxiliary_model_uses_active_profile_env(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles, 'get_flash_home_for_profile', lambda profile: profile_home)
        monkeypatch.setattr(
            profiles,
        ...
        fake_runtime_provider = types.ModuleType('flash_cli.runtime_provider')
        fake_runtime_provider.resolve_runtime_provider = lambda requested=None: {
            'api_key': 'fake-key',
            'provider': requested or 'openai',
            'base_url': 'https://example.test/v1',
        }
        fake_flash_cli = types.ModuleType('flash_cli')
        fake_flash_cli.__path__ = []
        fake_flash_cli.runtime_provider = fake_runtime_provider
        monkeypatch.setitem(sys.modules, 'flash_cli', fake_flash_cli)
        monkeypatch.setitem(sys.modules, 'flash_cli.runtime_provider', fake_runtime_provider)
...
if(!instruction || instruction.indexOf('docker pull ghcr.io/nesquena/flash-webui:latest') === -1) throw new Error('manual webui update must include pull guidance: '+instruction);
...
_showUpdateBanner({{
  webui: {{
    ...
    current_version: 'v0.51.833',
    latest_version: 'v0.51.913',
    compare_url: 'https://github.com/nesquena/flash-webui/compare/current-sha...latest-sha',
  }},
  agent: null,
}});
if(state.updateMsg.textContent.indexOf('WebUI') === -1) throw new Error('manual update must still render banner text');
if(state.updateMsg.textContent.indexOf('docker pull ghcr.io/nesquena/flash-webui:latest') === -1) throw new Error('manual update must render pull guidance');
...
(async () => {{
  ...
  if(state.checkUpdatesStatus.textContent.indexOf('docker pull ghcr.io/nesquena/flash-webui:latest') === -1) throw new Error('settings manual update must render pull guidance: '+state.checkUpdatesStatus.textContent);
...
class TestUpdateCompareSource:
    def test_simulated_update_check_payload_includes_both_safe_compare_urls(self):
        src = read('api/routes.py')
        assert '"repo_url": "https://github.com/nesquena/flash-webui"' in src
        assert '"compare_url": "https://github.com/nesquena/flash-webui/compare/abc1234...def5678"' in src
        assert '"repo_url": "https://github.com/NousResearch/flash-agent"' in src
        assert '"compare_url": "https://github.com/NousResearch/flash-agent/compare/aaa0001...bbb0002"' in src
...
class TestWhatsNewSummaryToggle:
    ...
    def test_update_banner_summary_flow_keeps_diff_links_after_summary(self):
        ...
        assert 'flash-whats-new-generated-summaries' in src
    ...
    def test_update_banner_summary_cache_has_byte_cap_constant_within_bounds(self):
        src = read('static/ui.js')
        assert "const WHATS_NEW_SUMMARY_STORAGE_KEY='flash-whats-new-generated-summaries';" in src
        ...
        key_index = src.find("const WHATS_NEW_SUMMARY_STORAGE_KEY='flash-whats-new-generated-summaries';")
...
""".strip()
    ...
    def test_update_summary_deduplicates_repeated_agent_summary_bullets(self):
        ...
        duplicate_menu_item = (
            'The `flash tools` menus should open noticeably faster, especially when checking available tools or auth state.'
        )
