...
def test_start_writes_pid_under_flash_home_runs_foreground_no_browser_and_logs(tmp_path):
    ...
    pid_file = flash_home / "webui.pid"
    log_file = flash_home / "webui.log"
    ...
    try:
        ...
        assert_path_in_text(flash_home / "webui", fake_output)
...
def test_start_can_ignore_repo_dotenv_for_authoritative_test_env(tmp_path):
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    pid = wait_for_pid_file(tmp_path / ".flash" / "webui.pid")
    try:
        fake_output = wait_for_file_text(fake_log, contains="host=127.0.0.1 port=8787")
        assert_path_in_text(tmp_path / ".flash" / "webui", fake_output)
    ...
    try:
        result = run_ctl(
            ...
            env={
                ...
                "HERMES_WEBUI_LAUNCHD_LABEL": "com.parantoux.flash-webui",
            },
        )
        assert result.returncode == 2
        combined = result.stdout + result.stderr
        assert "Refusing to start a second Flash WebUI" in combined
        assert "launchctl kickstart -k" in combined
        assert not (tmp_path / ".flash" / "webui.pid").exists()
    finally:
        sleeper.terminate()
        try:
            sleeper.wait(timeout=3)
...
def test_start_allows_alternate_port_while_launchd_job_runs_on_default(tmp_path):
    ...
    try:
        ...
        combined = result.stdout + result.stderr
        assert "Refusing to start a second Flash WebUI" not in combined, combined
        assert result.returncode == 0, combined
        pid_file = tmp_path / ".flash" / "webui.pid"
        if pid_file.exists():
