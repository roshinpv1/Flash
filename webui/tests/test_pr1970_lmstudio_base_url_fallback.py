...
def _stub_flash_cli(monkeypatch):
...
def test_lmstudio_fallback_works_when_flash_cli_unavailable(tmp_path, monkeypatch):
    """The lmstudio branch must populate models from the urlopen fallback even
    when `from flash_cli.models import provider_model_ids` raises ImportError.
    ...
    a CI-vs-local divergence where local environments with flash_cli installed
    worked, and CI (clean editable install) failed with empty model groups.

    Caught in CI on stage-337; fix splits the flash_cli try from the urlopen
    ...
    # Block flash_cli import the way a CI runner without the package would.
    blocked_modules = [name for name in list(sys.modules) if name == "flash_cli" or name.startswith("flash_cli.")]
    for name in blocked_modules:
        monkeypatch.delitem(sys.modules, name, raising=False)

    class _Blocker:
        def find_module(self, name, path=None):
            if name == "flash_cli" or name.startswith("flash_cli."):
                return self
            return None

        def load_module(self, name):
            raise ImportError(f"flash_cli blocked for test: {name}")
...
""",
        ...
        # Fallback must succeed despite flash_cli being unimportable.
        assert "lmstudio" in groups, (
            f"lmstudio group missing when flash_cli unavailable; groups={list(groups)}"
        )
