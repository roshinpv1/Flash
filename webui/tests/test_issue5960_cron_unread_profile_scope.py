...
def test_profile_switch_clears_persisted_old_profile_cron_markers_only():
    ...
    switch = _extract_function(SESSIONS_JS, "_switchProfileForSessionLoad")
    script = f"""
const store={{'flash-session-completion-unread':JSON.stringify({{}})}};
global.localStorage={{
...
(async()=>{{
  ...
  const after={{
    ...
    generation:_cronPollGeneration,
    renders,
    persisted:JSON.parse(store['flash-session-completion-unread']),
  }};
...
const persisted=JSON.parse(store['flash-session-completion-unread']);
process.stdout.write(JSON.stringify({{
