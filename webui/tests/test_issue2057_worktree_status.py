...
def git_worktree(tmp_path):
    ...
    worktree = tmp_path / "flash-status"
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.email", "test@example.com")
    _git(repo, "config", "user.name", "Flash Test")
    ...
    _git(repo, "worktree", "add", "-b", "flash/status", str(worktree), "main")
    _git(worktree, "push", "-u", "origin", "flash/status")
