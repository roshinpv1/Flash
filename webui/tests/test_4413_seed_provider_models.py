"""Regression tests for #4413 — provider-model seeder from flash_cli.
...
sys.path.insert(0, str(REPO.parent / ".flash" / "flash-agent"))
...
def _patch_core_pm(fake_pm: dict):
    """Patch ``flash_cli.models._PROVIDER_MODELS`` with *fake_pm*."""
    # Build a fake flash_cli.models module with just _PROVIDER_MODELS.
    fake_module = mock.MagicMock()
    fake_module._PROVIDER_MODELS = fake_pm
    return mock.patch.dict(sys.modules, {"flash_cli.models": fake_module})
...
class TestSeederAddsMissingModels:
    ...
    def test_no_op_without_flash_cli(self, restore_providers):
        """Seeder must be a no-op (not raise) when flash_cli is unavailable."""
        with mock.patch.dict(sys.modules, {"flash_cli.models": None}):
...
class TestExceptionNarrowing:
    """The seeder's flash_cli import should only catch ImportError, not all
    ...
    def test_does_not_swallow_runtime_errors(self, restore_providers):
        ...
        with _patch_core_pm(fake_pm):
            with mock.patch(
                ...
                # catches ImportError for the optional flash_cli import.
