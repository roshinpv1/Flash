"""Regression coverage for #716 Flash agent/gateway heartbeat monitor."""
...
def _runtime_status(**overrides):
    payload = {
        ...
        "pid": 12345,
        "argv": ["flash", "gateway", "--token", "secret-token"],
        "command": "flash gateway --token secret-token",
        "executable": "/home/user/.flash/flash-agent/venv/bin/python",
        "env": {"API_KEY": "secret"},
    }
...
def test_agent_health_uses_root_gateway_state_when_flash_home_is_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(
        sys.modules,
        "flash_constants",
        types.SimpleNamespace(get_default_flash_root=lambda: root_home),
    )
...
def test_active_profile_gateway_running_pid_uses_active_profile_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(profiles, "get_active_flash_home", lambda: active_home)
