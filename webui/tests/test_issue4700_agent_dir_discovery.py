...
Discovery of a pip-style flash-agent checkout must accept ``cron/jobs.py``
...
def _isolate_discovery_inputs(config, monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(config, "_DEFAULT_HERMES_HOME", tmp_path / "default-flash-home")
...
def test_discover_agent_dir_prefers_later_run_agent_root_over_earlier_pip_lookalike(
    ...
    _make_plugins_lookalike_root(tmp_path / "flash-home" / "flash-agent")
    later_legacy = _make_legacy_agent_root(tmp_path / "flash-agent")
    _isolate_discovery_inputs(config, monkeypatch, tmp_path)
...
def test_explicit_legacy_agent_dir_override_still_beats_pip_style_fallback(
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "flash-home"))
    _make_pip_style_agent_root(tmp_path / "flash-home" / "flash-agent")
...
def test_routes_shadow_helper_can_recover_once_agent_dir_resolves(monkeypatch, tmp_path):
    ...
    agent_dir = _make_pip_style_agent_root(tmp_path / "flash-agent")
