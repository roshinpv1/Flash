...
class TestMainForegroundRouting:
    ...
    def stub_main_dependencies(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(bs, "flash_command_exists", lambda: True)
    ...
    def test_foreground_defaults_state_dir_to_flash_home_webui(self, stub_main_dependencies, clean_env, monkeypatch, tmp_path):
        bs = stub_main_dependencies
        flash_home = tmp_path / ".flash" / "profiles" / "webui"
        ...
        assert os.environ["HERMES_WEBUI_STATE_DIR"] == str(flash_home / "webui")
...
def test_package_python_discovers_agent_before_skip_install_gate(import_bootstrap, clean_env, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "flash-home"))
    ...
    with patch.object(bs, "install_flash_agent") as mock_install, patch.object(bs.venv, "EnvBuilder") as mock_builder, pytest.raises(SystemExit):
        bs.main()
...
def test_package_python_without_agent_stays_fail_closed(import_bootstrap, clean_env, monkeypatch, tmp_path):
    ...
    with patch.object(bs, "_agent_dir_from_python", return_value=None) as mock_probe, patch.object(bs, "install_flash_agent") as mock_install, patch.object(bs.venv, "EnvBuilder") as mock_builder, patch.object(os, "execv") as mock_execv, pytest.raises(RuntimeError, match="Flash Agent was not found"):
        bs.main()
