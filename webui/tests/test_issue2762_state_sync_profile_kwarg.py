...
``get_active_flash_home()`` to pick the DB path. TLS gets set on the HTTP
...
def two_profile_homes(tmp_path, monkeypatch):
    ...
    ``flash_state.SessionDB`` itself (so the schema matches what the
    ...
    pytest.importorskip("flash_state")
    ...
    monkeypatch.setattr(profiles_mod, 'get_active_flash_home', lambda: hiyuki_home)
...
def _read_session(db_path: Path, session_id: str):
    ...
    try:
        # Real state.db schema (see api/state_sync.py + flash_cli StateDB):
...
def two_profile_message_homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(profiles_mod, "get_flash_home_for_profile", fake_profile_home)
    monkeypatch.setattr(profiles_mod, "get_active_flash_home", lambda: hiyuki_home)
...
def test_get_state_db_honors_explicit_profile_kwarg(two_profile_homes):
    ...
    # Some installs ship without the flash_state package; the function
    # returns None gracefully and there's nothing to assert.
    try:
        import flash_state  # noqa: F401
    except ImportError:
        pytest.skip("flash_state package not available in this test env")
