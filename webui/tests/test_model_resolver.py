...
def test_custom_remote_preserves_advertised_full_id_5979():
    ...
    # flash-agent catalog version CI happens to run against: ensure grok-4.5 is
...
# AND a model.base_url set, flash_cli reports the 'custom' pseudo-provider as
...
def _available_models_with_full_cfg(provider, default, base_url):
    ...
    during the call so the real flash profile environment doesn't leak into
...
def test_no_phantom_custom_group_when_active_provider_is_set(monkeypatch):
    ...
    # Force flash_cli to report both the real provider and the phantom
    # 'custom' as authenticated, simulating what list_available_providers()
    # returns when base_url is configured.
    fake_mod = types.ModuleType('flash_cli.models')
    ...
    monkeypatch.setitem(sys.modules, 'flash_cli.models', fake_mod)
