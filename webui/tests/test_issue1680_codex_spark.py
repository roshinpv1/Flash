...
def _install_fake_flash_models(monkeypatch, provider_model_ids):
    flash_cli = types.ModuleType("flash_cli")
    flash_cli.__path__ = []
    models = types.ModuleType("flash_cli.models")
    models._PROVIDER_ALIASES = {}
    models.provider_model_ids = provider_model_ids
    monkeypatch.setitem(sys.modules, "flash_cli", flash_cli)
    monkeypatch.setitem(sys.modules, "flash_cli.models", models)
...
def test_openai_codex_group_uses_provider_model_ids_for_spark(monkeypatch, tmp_path):
    ...
    copied _PROVIDER_MODELS["openai-codex"] without asking flash_cli.
    """
    calls = []
    ...
    _install_fake_flash_models(monkeypatch, provider_model_ids)
    ...
    # sys.modules['flash_cli.models'] without restoring it, list_available_providers
    ...
    # gpt-5.3-codex-spark when flash_cli.provider_model_ids returns it".
    if calls != ["openai-codex"]:
        import pytest
        pytest.skip(f"flash_cli stub not active for openai-codex (likely test-isolation pollution from sibling test). Got calls={calls}")
