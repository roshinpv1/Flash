...
(flash-voice-silence-ms, flash-voice-continuous) with sensible defaults.
"""
...
class TestVoiceModeSilenceMsConfig:
    ...
    def test_silence_ms_reads_local_storage_with_fallback(self):
        ...
        assert re.search(
            r"parseInt\s*\(\s*localStorage\.getItem\s*\(\s*'flash-voice-silence-ms'\s*\)",
            src,
        ), "Voice mode must read the 'flash-voice-silence-ms' localStorage key via parseInt."
...
class TestVoiceModeContinuousConfig:
    ...
    def test_continuous_reads_local_storage(self):
        ...
        assert (
            ...
            "'flash-voice-continuous' with default false. "
            "Without this, users with natural mid-sentence pauses get cut off."
        )

    def test_continuous_true_behavior(self):
        """When flash-voice-continuous is 'true', the recognition stays open
