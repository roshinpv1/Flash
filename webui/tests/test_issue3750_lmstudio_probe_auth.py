...
def test_reasoning_probe_falls_back_without_flash_cli(
    tmp_path,
    monkeypatch,
    lmstudio_probe_server,
):
...
def test_credentialed_probe_never_calls_flash_cli(
    tmp_path,
    monkeypatch,
    lmstudio_probe_server,
):
    """When a credential is configured, the probe must use the built-in
    no-redirect fallback and NEVER the bundled flash_cli probe (which follows
...
""",
    ...
    monkeypatch.setitem(sys.modules, "flash_cli", type("FlashCli", (), {})())
    monkeypatch.setitem(sys.modules, "flash_cli.models", _CliModule)
    ...
    # … and the redirect-following flash_cli probe was never invoked.
    assert cli_calls == [], "credentialed probe must bypass flash_cli"
...
def test_keyless_probe_logs_signature_mismatch_before_fallback(
    ...
    """Keyless probes still prefer flash_cli; an incompatible CLI signature
...
def test_credentialed_probe_does_not_follow_redirects_end_to_end(
    ...
    flash_cli importable: when the CONFIGURED LM Studio endpoint returns a 302,
    the credentialed probe must not forward the key to the redirect target.
    This is the production path (flash_cli present) — credentialed probes route
    ...
    # Sanity: this test only proves the redirect guard if flash_cli is
