...
- Stale README references to `/root/.flash` are gone (the agent images
  use `/home/flash/.flash`)
"""
...
def test_two_container_compose_aligns_agent_uid_with_webui():
    ...
    assert "HERMES_UID=${UID:-1000}" in src, (
        "two-container: flash-agent must set HERMES_UID=${UID:-1000} so it "
        ...
        "shared flash-home volume."
    )
    assert "HERMES_GID=${GID:-1000}" in src, (
        "two-container: flash-agent must set HERMES_GID=${GID:-1000}"
    )
...
def test_three_container_compose_aligns_all_three_services():
    ...
    assert src.count("HERMES_UID=${UID:-1000}") >= 2, (
        "three-container: both flash-agent and flash-dashboard must set "
        "HERMES_UID=${UID:-1000}"
    )
...
def test_docs_docker_md_exists_and_covers_failure_modes():
    ...
    assert "sunnysktsang/flash-suite" in src, (
        "docs/docker.md should point Podman 3.4 / multi-arch users to the "
        "community all-in-one image as a documented escape hatch."
    )


# ── 5: stale /root/.flash references removed from README ──────────────────


def test_readme_no_stale_root_flash_path():
    """REGRESSION: the README's two-container Docker section used to claim
    'the agent writes to /root/.flash' which is wrong — current agent
    images use /home/flash/.flash. Stale paths confuse users reading
    the README to debug their own setup."""
    src = (REPO / "README.md").read_text(encoding="utf-8")
    assert "/root/.flash" not in src, (
        "README.md must not reference /root/.flash — the current agent "
        "image uses /home/flash/.flash. Stale paths in docs are worse "
        "than no docs at all."
    )
...
def test_multi_container_webui_points_at_gateway_service():
    ...
    for fname in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        path = REPO / fname
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
        env = data["services"]["flash-webui"]["environment"]
        assert "HERMES_API_URL=http://flash-agent:8642" in env, (
            f"{fname}: flash-webui must point at flash-agent over the compose "
...
def test_agent_service_does_not_recommend_invalid_home_mode():
    ...
    for fname in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        for service_name in ("flash-agent", "flash-dashboard"):
