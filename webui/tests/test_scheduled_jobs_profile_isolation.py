...
AGENT_ROOT = pathlib.Path(os.environ.get("HERMES_AGENT_ROOT", pathlib.Path.home() / "flash-agent"))
...
def test_cron_profile_context_pins_profile_home(tmp_path, monkeypatch):
    ...
    pytest.importorskip("cron.jobs")  # auto-skip when flash-agent is unavailable
...
def test_cron_run_does_not_silently_swallow_profile_resolution_errors():
    ...
    when get_active_flash_home() raises.
    ...
    than risk that, since get_active_flash_home() raising at all from
    ...
    # wrap get_active_flash_home() in a bare except that falls back to None.
    idx = src.find("def _handle_cron_run(handler, body):")
    assert idx != -1, "_handle_cron_run not found"
    body = src[idx : idx + 4000]

    # The spawn site must call get_active_flash_home() unguarded (no
    ...
    assert "_profile_home = None" not in pre_spawn, (
        ...
        "get_active_flash_home() raises. That re-introduces bug #1573 — "
...
def test_streaming_cronjob_wrapper_context_survives_threadpool_context_copy(tmp_path, monkeypatch):
    ...
    WebUI sets the active profile on the streaming thread, then the Flash agent
