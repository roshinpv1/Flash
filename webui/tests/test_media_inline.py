...
class TestMediaEndpointUnit(unittest.TestCase):
        ...
        with mock.patch.object(
            ...
            self.assertFalse(
                routes._path_is_within_root(
                    ...
                    pathlib.Path("C:/Users/agent/.flash"),
                )
            )
    ...
    def test_active_workspace_carveout_gated_against_flash_roots(self):
        ...
        ($HOME, ~/.flash, a profile root, etc.). A state.db sitting under such a
        ...
        with tempfile.TemporaryDirectory() as home:
            flash_home = pathlib.Path(home) / ".flash"
            flash_home.mkdir(parents=True)
            secret = flash_home / "state.db"
            ...
            with mock.patch.dict(os.environ, {"HERMES_HOME": str(flash_home)}), \
                 mock.patch.object(routes, "get_last_workspace", lambda: str(flash_home)), \
            ...
            self.assertEqual(
                ...
                "Flash home (carve-out must be gated against internal roots)",
            )
    ...
    def test_active_workspace_under_state_dir_serves_but_sessions_denied(self):
        ...
        with tempfile.TemporaryDirectory() as home:
            flash_home = pathlib.Path(home) / ".flash"
            state_dir = flash_home / "webui-state"
    ...
    def test_media_allowed_roots_env_var_serves_outside_flash_root(self):
        ...
        with tempfile.TemporaryDirectory() as home, tempfile.TemporaryDirectory() as extra:
            ...
            with mock.patch.dict(
                ...
                routes, "get_last_workspace", lambda: str(flash_home / "workspace")
            ), mock.patch(
            ...
            self.assertEqual(
                handler.status, 200,
                "MEDIA_ALLOWED_ROOTS media outside Flash roots must still serve",
            )
    ...
    def test_handle_media_session_authorizes_html_artifact_outside_roots(self):
        ...
        with tempfile.TemporaryDirectory() as home, tempfile.TemporaryDirectory() as outside:
            flash_home = pathlib.Path(home) / ".flash"
            flash_home.mkdir(parents=True)
            ws = flash_home / "workspace"
            ws.mkdir()
            html = pathlib.Path(outside) / "report.html"
            html.write_text("<h1>Report</h1>", encoding="utf-8")
            session = SimpleNamespace(messages=[{"role": "assistant", "content": f"MEDIA:{html}"}])
            with mock.patch.dict(os.environ, {"HERMES_HOME": str(flash_home), "MEDIA_ALLOWED_ROOTS": ""}), \
                 mock.patch.object(routes, "get_last_workspace", lambda: str(ws)), \
...
class TestMediaEndpointIntegration(unittest.TestCase):
    ...
    def test_nonexistent_file_returns_404(self):
        missing = _media_fixture_dir() / "__flash_nonexistent_12345.png"
    ...
    def test_valid_png_served_with_image_mime(self):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".png", prefix="flash_test_", dir=_media_fixture_dir(), delete=False
        ) as f:
            f.write(png_bytes)
            tmp_path = f.name
        try:
    ...
    def test_audio_media_endpoint_inline_and_range(self):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".wav", prefix="flash_test_", dir=_media_fixture_dir(), delete=False
        ) as f:
            f.write(audio_bytes)
            tmp_path = f.name
        try:
    ...
    def test_html_media_endpoint_inline_requires_csp_sandbox(self):
        ...
        html_bytes = b"<!doctype html><title>Flash</title><script>window.ok=1</script>"
        with tempfile.NamedTemporaryFile(
            suffix=".html", prefix="flash_test_", dir=_media_fixture_dir(), delete=False
        ) as f:
            f.write(html_bytes)
            tmp_path = f.name
        try:
    ...
    def test_webui_state_secret_files_denied(self):
        ...
        they live under an allowed root (the whole Flash home is allowed).
