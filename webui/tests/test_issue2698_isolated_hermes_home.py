...
When HERMES_HOME points at a specific profile directory like ~/.flash/profiles/user1
...
@pytest.fixture
def temp_flash_home():
    """Create a temporary .flash directory structure for testing."""
    with tempfile.TemporaryDirectory() as tmpdir:
        home = Path(tmpdir) / ".flash"
        home.mkdir()
...
def temp_single_profile():
    """Create a temporary .flash/profiles/user1 structure for isolated mode."""
    with tempfile.TemporaryDirectory() as tmpdir:
        home = Path(tmpdir) / ".flash"
        home.mkdir()
        profiles_root = home / "profiles"
...
class TestIsolatedProfileModeDetection:
    ...
    def test_normal_mode_when_flash_home_is_base(self, temp_flash_home):
        """Normal mode when HERMES_HOME points to base ~/.flash."""
        with mock.patch.dict(os.environ, {"HERMES_HOME": str(temp_flash_home)}):
            with mock.patch("api.profiles._DEFAULT_HERMES_HOME", temp_flash_home):
                with mock.patch("api.profiles._INITIAL_HERMES_HOME", str(temp_flash_home)):
                    isolated = _is_isolated_profile_mode()
                    assert isolated is False

    def test_isolated_mode_when_flash_home_is_profile_subdir(self, temp_single_profile):
        """Isolated mode when HERMES_HOME points to ~/.flash/profiles/user1."""
    ...
    def test_flash_base_home_does_not_disable_isolation(self, temp_single_profile):
...
class TestListProfilesInIsolatedMode:
    ...
    def test_list_returns_all_profiles_in_normal_mode(self, temp_flash_home):
        """Normal mode lists all profiles."""
        # Create a few test profiles
        profiles_root = temp_flash_home / "profiles"
                ...
                with mock.patch("api.profiles._is_isolated_profile_mode", return_value=False):
                    ...
                    with mock.patch("api.profiles._get_profile_skills_stats", return_value=(0, 0)):
                        ...
                        def mock_build_profiles():
                            return [
                                {'name': 'default', 'path': str(temp_flash_home), 'is_default': True,
            ...
            with mock.patch("api.profiles._DEFAULT_HERMES_HOME", base_home):
                with mock.patch("api.profiles._INITIAL_HERMES_HOME", str(temp_single_profile)):
                    with mock.patch("api.profiles._is_isolated_profile_mode", return_value=True):
                        with mock.patch("api.profiles._resolve_base_flash_home", return_value=base_home):
                            # Mock _get_profile_skills_stats to avoid importing agent.skill_utils
                            with mock.patch("api.profiles._get_profile_skills_stats", return_value=(0, 0)):
                                profiles = list_profiles_api()
...
class TestIsolatedRuntimePinning:
    ...
    def test_init_profile_state_pins_runtime_home_to_isolated_profile(
        ...
        assert get_active_profile_name() == "user1"
        assert get_active_flash_home() == temp_single_profile
        assert Path(os.environ["HERMES_HOME"]) == temp_single_profile
        assert reloaded == [temp_single_profile]

    def test_get_active_flash_home_keeps_profiles_default_pinned(self, temp_flash_home, monkeypatch):
        """An isolated profiles/default path must not collapse back to the base home."""
        isolated_default = temp_flash_home / "profiles" / "default"
        ...
        monkeypatch.setenv("HERMES_BASE_HOME", "")
        monkeypatch.setattr(_profiles_mod, "_DEFAULT_HERMES_HOME", temp_flash_home)
        ...
        assert get_active_flash_home() == isolated_default

    def test_explicit_profile_resolution_for_isolated_default_uses_pinned_home(self, temp_flash_home, monkeypatch):
        """Explicit default-profile resolution must stay on the isolated home."""
        from api.profiles import get_flash_home_for_profile, _resolve_profile_home_for_name
        ...
        assert get_flash_home_for_profile("default") == isolated_default
        assert get_flash_home_for_profile("default") != temp_flash_home
    ...
    def test_explicit_foreign_profile_resolution_stays_on_isolated_home(self, temp_single_profile, monkeypatch):
        ...
        assert get_flash_home_for_profile("other_profile") == temp_single_profile

    def test_list_profiles_prefers_matching_isolated_default_home(self, temp_flash_home, monkeypatch):
        ...
        # Inject a STUB flash_cli.profiles into sys.modules rather than
        # mock.patch("flash_cli.profiles.list_profiles", ...): that string
        # target forces a real import of flash_cli, which is NOT installed in
        # CI (the bundled agent isn't on the WebUI test path) -> ModuleNotFoundError.
        # list_profiles_api() does `from flash_cli.profiles import list_profiles`
        # internally, so a stub module satisfies it at the WebUI boundary. (#4454 CI)
        stub_profiles = types.ModuleType("flash_cli.profiles")
        stub_profiles.list_profiles = lambda: [
            _Info("default", temp_flash_home, True),
            _Info("default", isolated_default, False),
        ]
        stub_pkg = sys.modules.get("flash_cli") or types.ModuleType("flash_cli")
        monkeypatch.setitem(sys.modules, "flash_cli", stub_pkg)
        monkeypatch.setitem(sys.modules, "flash_cli.profiles", stub_profiles)
    ...
    def test_list_profiles_isolated_fallback_does_not_reenter_root_profile_lookup(self, temp_single_profile, monkeypatch):
        ...
        with mock.patch.dict(sys.modules, {"flash_cli": None, "flash_cli.profiles": None}):
            rows = list_profiles_api()
        ...
        with mock.patch.dict(os.environ, env_dict, clear=False):
            with mock.patch("api.profiles._DEFAULT_HERMES_HOME", base_home):
                with mock.patch("api.profiles._INITIAL_HERMES_HOME", str(temp_single_profile)):
                    ...
                    # for other reasons (missing flash_cli), but the isolation
                    ...
                    try:
                        ...
                        pass  # expected in test env without flash_cli

    def test_switch_to_same_default_profile_keeps_pinned_home(self, temp_flash_home, monkeypatch, tmp_path):
        ...
        (temp_flash_home / "config.yaml").write_text(f"workspace: {base_workspace}\n", encoding="utf-8")
...
class TestNormalModePreservation:
    ...
    def test_normal_mode_profile_operations_work(self, temp_flash_home):
            ...
            with mock.patch("api.profiles._DEFAULT_HERMES_HOME", temp_flash_home):
                with mock.patch("api.profiles._is_isolated_profile_mode", return_value=False):
                    ...
                    try:
                        ...
                        # The actual call might fail due to missing flash_cli,
