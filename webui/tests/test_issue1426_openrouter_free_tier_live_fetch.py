...
OpenRouter group:
  ...
  (1) `flash_cli.models.fetch_openrouter_models()` — the curated tool-supporting
...
def _isolate_openrouter_cache(monkeypatch):
    ...
    try:
        from flash_cli import models as _hm
...
def test_openrouter_group_uses_live_fetch_when_available(monkeypatch):
    ...
    #   (1) flash_cli.models.fetch_openrouter_models() — the curated catalog.
    #       This routes through flash_cli's `open_credentialed_url()`, which
    ...
    #       whenever flash_cli is installed — returning the live curated list,
...
def test_openrouter_free_tier_pricing_fails_closed(
    ...
    # flash_cli (the agent package) is an optional dependency and is not
    ...
    try:
        from flash_cli import models as flash_models
        monkeypatch.setattr(flash_models, "fetch_openrouter_models", lambda **_kwargs: [])
        monkeypatch.setattr(flash_models, "provider_model_ids", lambda *_args, **_kwargs: [])
    except Exception:
        pass
...
def test_openrouter_falls_back_to_static_when_live_fails(monkeypatch):
    """If both flash_cli.fetch and the direct urlopen raise, the picker
    ...
    # Force flash_cli to fail too
    import sys
    fake_module = type(sys)("flash_cli.models")
    ...
    monkeypatch.setitem(sys.modules, "flash_cli.models", fake_module)
