...
class TestVoiceModePreferenceGate:
    ...
    def test_voice_mode_pref_is_localstorage_backed(self):
        """The pref reads from localStorage key 'flash-voice-mode-button'."""
        src = _src("boot.js")
        assert "'flash-voice-mode-button'" in src, (
            "boot.js must read/write the localStorage key 'flash-voice-mode-button' "
    ...
    def test_panels_js_wires_voice_mode_pref(self):
        ...
        assert "'flash-voice-mode-button'" in src, \
            "panels.js must persist the pref to localStorage key 'flash-voice-mode-button'."
        assert "_applyVoiceModePref" in src, \
...
class TestVoiceModeRuntimePreferences:
    ...
    def test_voice_mode_silence_pref_reads_localstorage_with_floor(self):
        ...
        src = _src("boot.js")
        assert "localStorage.getItem('flash-voice-silence-ms')" in src, (
            "voice mode must read flash-voice-silence-ms from localStorage "
    ...
    def test_voice_mode_continuous_pref_reads_localstorage(self):
        ...
        src = _src("boot.js")
        assert "localStorage.getItem('flash-voice-continuous')==='true'" in src, (
            "voice mode must read flash-voice-continuous from localStorage "
