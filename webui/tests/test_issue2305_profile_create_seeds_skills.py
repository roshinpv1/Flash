...
# Do NOT touch real ~/.flash, real credentials, or real profile directories.
#
# Test strategy:
#   - Mock _DEFAULT_HERMES_HOME to a tmp_path so _resolve_base_flash_home()
#     picks up the isolated root.
#   - Inject a mock 'flash_cli.profiles' module directly into sys.modules so
#     that the `from flash_cli.profiles import seed_profile_skills` inside
#     create_profile_api resolves to the mock (not the real module).
#   - Stub flash_cli.profiles.create_profile to create the profile dir.
#   - Stub flash_cli.profiles.seed_profile_skills to record calls.
...
@pytest.fixture
def fake_flash_home(tmp_path, monkeypatch):
    # Point _DEFAULT_HERMES_HOME at an isolated temp directory so that
    # profile-path resolution does not touch the real ~/.flash.
    fake_home = tmp_path / '.flash'
...
def _install_flash_cli_profiles_mock(create_impl, seed_impl):
    # Inject a mock 'flash_cli.profiles' module directly into sys.modules.
    # This is the only way to intercept `from flash_cli.profiles import X`
    # inside create_profile_api — patch.dict(sys.modules, ...) only modifies
    # existing keys and cannot add new ones.
    mock = ModuleType('flash_cli.profiles')
    mock.create_profile = create_impl
    mock.seed_profile_skills = seed_impl
    sys.modules['flash_cli'] = ModuleType('flash_cli')
    sys.modules['flash_cli.profiles'] = mock
    return mock


def _remove_flash_cli():
    for key in list(sys.modules):
        if key == 'flash_cli' or key.startswith('flash_cli.'):
            del sys.modules[key]


# Module references saved at import time so we can restore the real flash_cli
# after each test that overwrites sys.modules['flash_cli.profiles'].  This
# prevents the `FallbackDoesNotCrash` tests from finding a deleted entry and
# incorrectly skipping.
_real_flash_cli = sys.modules.get('flash_cli')
_real_flash_cli_profiles = sys.modules.get('flash_cli.profiles')


def _restore_real_flash_cli():
    if _real_flash_cli is not None:
        sys.modules['flash_cli'] = _real_flash_cli
    if _real_flash_cli_profiles is not None:
        sys.modules['flash_cli.profiles'] = _real_flash_cli_profiles


# ── Tests ──────────────────────────────────────────────────────────────────────

class TestNoCloneSeedsSkills:
    def test_seed_called_once_with_resolved_path(self, fake_flash_home):
        calls = []

        def fake_create(name, **kw):
            _make_profile_dir(_isolated_profiles_root(fake_flash_home), name)

        def fake_seed(profile_path, quiet=None):
            calls.append({'profile_path': profile_path, 'quiet': quiet})

        _remove_flash_cli()
        _install_flash_cli_profiles_mock(fake_create, fake_seed)
        ...
        # Path must be the resolved profile directory under the fake flash home.
        expected_path = _isolated_profiles_root(fake_flash_home) / 'testprofile'
...
class TestCloneSkipsSeeding:
    def test_seed_not_called_when_clone_from_is_set(self, fake_flash_home):
        calls = []
...
class TestSeedFailureIsBestEffort:
    def test_seed_raising_logs_warning_and_still_returns_profile(self, fake_flash_home, caplog):
...
class TestFlashCliUnavailableFallbackDoesNotCrash:
    def test_fallback_create_still_produces_profile_dict(self, fake_flash_home):
        # Simulate flash_cli being present but create_profile raising ImportError
        ...
        #
        # We cannot permanently delete flash_cli.profiles from sys.modules (it
        ...
        real_mod = sys.modules.get('flash_cli.profiles')
        if real_mod is None:
            # flash_cli.profiles was already cleaned up by a prior test in this
            # process — skip rather than failing with a confusing assertion.
            pytest.skip('flash_cli.profiles not in sys.modules (cleaned up by prior test)')

        orig_create = real_mod.create_profile
        real_mod.create_profile = MagicMock(side_effect=ImportError('flash_cli profiles unavailable'))
        try:
            with patch.object(profiles_mod, 'list_profiles_api', return_value=[]):
        ...
        expected_path = _isolated_profiles_root(fake_flash_home) / 'isolatedprofile'
        assert Path(result['path']) == expected_path

    def test_seed_unavailable_logs_debug_without_crashing(self, fake_flash_home, caplog):
        import logging as std_logging
        ...
        real_mod = sys.modules.get('flash_cli.profiles')
        real_flash_cli = sys.modules.get('flash_cli')
        if real_mod is None or real_flash_cli is None:
            pytest.skip('flash_cli.profiles not in sys.modules (cleaned up by prior test)')

        # We need flash_cli.profiles.seed_profile_skills to not exist so that
        # `from flash_cli.profiles import seed_profile_skills` raises ImportError.
        ...
        fake_flash_cli = ModuleType('flash_cli')
        sys.modules['flash_cli'] = fake_flash_cli
        sys.modules['flash_cli.profiles'] = mock

        try:
            with caplog.at_level(std_logging.DEBUG):
                with patch.object(profiles_mod, 'list_profiles_api', return_value=[]):
                    result = profiles_mod.create_profile_api('noflashprofile')
        finally:
            # Restore the real modules so subsequent tests can use them.
            _remove_flash_cli()
            sys.modules['flash_cli'] = real_flash_cli
            sys.modules['flash_cli.profiles'] = real_mod

        # Profile is still created.
        assert result['name'] == 'noflashprofile'
