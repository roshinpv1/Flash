...
# The standalone fallback anchors under the OS temp dir (NOT ~/.flash) so test
# state is never created inside the production Flash home — matching conftest's
...
# ~/.flash either. Same logic: trip only if under literal ~/.flash and the temp
# root isn't itself under production.
_PROD_HERMES_HOME = (pathlib.Path.home() / '.flash').resolve()
