...
def test_extension_settings_runtime_normalizes_persists_resets_and_clears():
    script = textwrap.dedent(
        ...
        const settings = window.FlashExtensionSettings.settingsForExtension('demo.ext');
        ...
        assert.strictEqual(window.flashExt.settings.forExtension('demo.ext').get('mode'), 'compact');
        assert.deepStrictEqual(settings.setAll({{flag: true, mode: 'compact', count: 2}}).values, {{flag: true, mode: 'compact', count: 2}});
        assert.deepStrictEqual(JSON.parse(store.get('flash.ext.settings.demo.ext')), {{flag: true}});
        store.set('flash.ext.settings.demo.ext', JSON.stringify({{flag: false, unknown: 'kept', bad: 'x'}}));
        assert.deepStrictEqual(settings.values, {{flag: false, mode: 'compact', count: 2}});
        assert.deepStrictEqual(settings.overrides, {{}});
        assert.strictEqual(store.has('flash.ext.settings.demo.ext'), false);
        store.set('flash.ext.settings.demo.ext', 'not-json');
        assert.deepStrictEqual(settings.values, {{flag: false, mode: 'compact', count: 2}});
        assert.deepStrictEqual(settings.overrides, {{}});
        assert.strictEqual(store.has('flash.ext.settings.demo.ext'), false);
        store.set('flash.ext.settings.demo.ext', JSON.stringify(['bad']));
        assert.deepStrictEqual(settings.values, {{flag: false, mode: 'compact', count: 2}});
        assert.deepStrictEqual(settings.overrides, {{}});
        assert.strictEqual(store.has('flash.ext.settings.demo.ext'), false);
        ...
        const storage = window.FlashExtensionSettings.storageForExtension('demo.ext');
        assert.strictEqual(window.flashExt.storage.forExtension('demo.ext').set('note', 'local'), true);
        assert.strictEqual(storage.get('note'), 'local');
        assert.strictEqual(store.has('flash.ext.storage.demo.ext'), true);
        assert.strictEqual(store.has('flash.ext.settings.demo.ext'), false);
        assert.strictEqual(storage.clear(), true);
        assert.strictEqual(store.has('flash.ext.storage.demo.ext'), false);

        const deniedSettings = window.FlashExtensionSettings.settingsForExtension('denied.ext');
        assert.strictEqual(deniedSettings.setAll({{flag: true}}).ok, false);
        assert.strictEqual(store.has('flash.ext.settings.denied.ext'), false);

        const deniedStorage = window.FlashExtensionSettings.storageForExtension('denied.ext');
        assert.strictEqual(deniedStorage.set('note', 'blocked'), false);
        assert.strictEqual(store.has('flash.ext.storage.denied.ext'), false);

        window.FlashExtensionSettings.primeFromStatus({{
          extensions: [{{
            id: 'unknown.ext',
            name: 'Unknown',
            storage_owned: true,
        ...
        const unknownSettings = window.FlashExtensionSettings.settingsForExtension('unknown.ext');
        assert.strictEqual(unknownSettings.setAll({{flag: true}}).ok, false);
        assert.strictEqual(store.has('flash.ext.settings.unknown.ext'), false);
        ...
        const reprobe = window.FlashExtensionSettings.settingsForExtension('demo.ext');
