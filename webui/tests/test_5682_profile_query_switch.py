...
global.localStorage = {
  store: { 'flash-webui-session': 'saved-local' },
  getItem(key) {
...
global.switchToProfile = async (name) => {
  ...
  localStorage.setItem('flash-webui-session', 'fresh-local');
  return true;
};
(async () => {
  const savedLocalBefore = localStorage.getItem('flash-webui-session');
  ...
  if (blocksSavedLocal && profileSwitchCompleted && profileSwitchChangedProfile && localStorage.getItem('flash-webui-session') === savedLocalBefore) localStorage.removeItem('flash-webui-session');
  const savedLocalAfterSuppress = localStorage.getItem('flash-webui-session');
  const savedLocalAfterReload = localStorage.getItem('flash-webui-session');
...
(async () => {
  ...
  console.log(JSON.stringify({
    ...
    savedLocalBefore,
    savedLocalAfter: localStorage.getItem('flash-webui-session'),
    cleanupGuardPos,
  }));
})().catch(err => {
  console.error(err);
...
(async () => {
  ...
  const blocksSavedLocal = _profileQueryBlocksSavedLocalRestore(intent, null);
  if (blocksSavedLocal && profileSwitchCompleted && localStorage.getItem('flash-webui-session') === savedLocalBefore) localStorage.removeItem('flash-webui-session');
...
if (blocksImplicit) localStorage.removeItem('flash-webui-session');
const implicitAfter = localStorage.getItem('flash-webui-session');
localStorage.setItem('flash-webui-session', 'saved-local');
const allowsExplicit = _profileQueryBlocksSavedLocalRestore(validProfile, 'session-123');
if (allowsExplicit) localStorage.removeItem('flash-webui-session');
const explicitAfter = localStorage.getItem('flash-webui-session');
console.log(JSON.stringify({
  blocksImplicit,
  allowsExplicit,
...
global.assistantDisplayName = () => 'Flash';
