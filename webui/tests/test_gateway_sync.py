...
def get(path, *, profile=None):
    headers = {}
    if profile:
        headers["Cookie"] = f"flash_profile={profile}"
...
def _get_test_state_dir():
    ...
    formula via _pytest_port (temp-rooted, never ~/.flash).
    """
...
def test_agent_session_source_normalization_contract():
    """Raw Flash Agent sources map to stable WebUI source categories."""
...
def test_delete_imported_messaging_session_preserves_agent_memory(cleanup_test_sessions):
    """WebUI delete must not delete Flash Agent memory for external channels."""
    conn = _ensure_state_db()
...
def test_session_prefers_state_db_messages_over_stale_local_snapshot(cleanup_test_sessions):
    ...
    try:
        ...
        s = Session(
            ...
            workspace=str(pathlib.Path.home() / '.flash'),
            model='openai/gpt-5',
