...
def _init_repo(path):
    ...
    _git(path, "config", "user.email", "flash-tests@example.invalid")
    _git(path, "config", "user.name", "Flash Tests")
    return path
...
def test_git_fetch_pull_and_push_with_upstream(tmp_path):
    ...
    _git(clone, "config", "user.email", "flash-tests@example.invalid")
    _git(clone, "config", "user.name", "Flash Tests")
...
def test_git_stash_and_checkout_is_explicit(tmp_path):
    ...
    assert result["stash_name"].startswith("flash-webui branch switch")
    assert result["current_branch"] == "target"
    assert git_status(repo)["totals"]["changed"] == 0
    assert "flash-webui branch switch to target" in _git(repo, "stash", "list")
...
def test_git_stash_and_checkout_restores_branch_changes_when_returning(tmp_path):
    ...
    assert "On main: flash-webui branch switch" not in stash_list
    assert "On feature: flash-webui branch switch" in stash_list
...
def test_git_stash_and_checkout_reports_restore_conflicts_without_dropping_stash(tmp_path):
    ...
    assert "On main: flash-webui branch switch" in _git(repo, "stash", "list")
...
def test_git_fetch_blocks_repo_local_credential_helper_execution(tmp_path):
    ...
    class AuthRequiredHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            self.send_header("WWW-Authenticate", 'Basic realm="flash-test"')
            self.end_headers()
...
def test_git_env_scrub_removes_redirecting_vars_and_preserves_temp_index(monkeypatch):
    ...
    env = _clean_git_env({"GIT_INDEX_FILE": "/tmp/flash-index"})
    ...
    assert env["GIT_INDEX_FILE"] == "/tmp/flash-index"
