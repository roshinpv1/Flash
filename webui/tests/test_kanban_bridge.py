...
surfaces Flash Agent Kanban data under /api/kanban/* while keeping the Agent
kanban database as the only source of truth.

CI for flash-webui does not install flash-agent, so these tests inject a tiny
fake ``flash_cli.kanban_db`` module and verify the bridge contract without
...
class FakeKanbanDB:
    ...
    def worker_log_path(self, task_id):
        ...
        return Path(f"/tmp/flash-kanban/{task_id}.log")
...
def _load_bridge(monkeypatch):
    ...
    fake_flash_cli.kanban_db = fake_kanban
    monkeypatch.setitem(sys.modules, "flash_cli", fake_flash_cli)
    monkeypatch.setitem(sys.modules, "flash_cli.kanban_db", fake_kanban)
...
def test_board_pointer_drift_falls_back_to_default(monkeypatch):
    bridge = _load_bridge(monkeypatch)
    fake_kanban = sys.modules["flash_cli.kanban_db"]
    fake_kanban.boards = {
...
def test_handle_kanban_get_returns_503_when_flash_cli_missing(monkeypatch):
    """If flash_cli is unavailable (webui-only deploy), the bridge must
    ...
    """
    bridge = _load_bridge(monkeypatch)
    # Force _kb() to raise ImportError as if flash_cli was uninstalled
    monkeypatch.setattr(
        bridge, "_kb",
        lambda: (_ for _ in ()).throw(ImportError("No module named 'flash_cli'")),
    )
...
def test_handle_kanban_post_returns_503_when_flash_cli_missing(monkeypatch):
    """Same fallback contract for POST verb."""
    bridge = _load_bridge(monkeypatch)
    monkeypatch.setattr(
        bridge, "_kb",
        lambda: (_ for _ in ()).throw(ImportError("flash_cli missing")),
    )
    captured = {}
...
def test_handle_kanban_patch_returns_503_when_flash_cli_missing(monkeypatch):
