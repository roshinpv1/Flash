...
class TestProviderCatalog:
    ...
    def test_google_uses_gemini_key(self):
        """Google Gemini must use 'gemini' as provider ID (matches Flash CLI)."""
...
class TestApplyBaseURL:
    ...
    def test_requires_base_url_writes_user_url(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("api.onboarding._get_active_flash_home", lambda: tmp_path)
    ...
    def test_base_url_stripped_for_anthropic(self, tmp_path, monkeypatch):
        """Anthropic should NOT have base_url in config (Flash knows the URL)."""
        config_path = str(tmp_path / "config.yaml")
