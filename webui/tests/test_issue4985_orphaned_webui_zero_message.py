...
def _real_pipeline(tmp_path, monkeypatch):
    ...
    # ``api.profiles.get_active_flash_home``. Force the module-level
    # default so the helper returns ``flash_home/state.db`` even when the
    # active profile is something other than 'default'.
    monkeypatch.setattr(profiles, "_DEFAULT_HERMES_HOME", flash_home)
...
def test_real_all_sessions_post1171_untitled_uncounted_row_is_filtered_by_1171(
    ...
    _write_state_db(tmp_path / "flash_home", sessions={})
...
def test_tombstone_does_not_block_new_session_with_same_id(
    ...
    db_path = tmp_path / "flash_home" / "state.db"
