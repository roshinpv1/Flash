...
async def find_flash_tab():
    """Find a Flash session tab that has the scrollbar drag fix deployed."""
    tabs = json.loads(urllib.request.urlopen("http://localhost:9222/json").read())
    flash = [t for t in tabs if "8787" in t.get("url", "") and t["type"] == "page" and "/session/" in t.get("url", "")]
    if not flash:
        raise RuntimeError("No Flash session tab found on :8787")
    for t in flash:
    ...
    ws_url = flash[0]["webSocketDebuggerUrl"]
    async with websockets.connect(ws_url, max_size=1*1024*1024) as ws:
