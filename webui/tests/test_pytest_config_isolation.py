"""Regression coverage for pytest isolation of Flash config paths."""
import os
from pathlib import Path


def test_pytest_overrides_inherited_flash_config_path():
    """A live-agent HERMES_CONFIG_PATH must never leak into WebUI tests.

    Flash agents commonly run with HERMES_CONFIG_PATH pointing at the real
    ~/.flash/config.yaml. The test harness must replace it with the isolated
