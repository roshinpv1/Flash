...
`~/.flash/config.yaml`, never the non-default profile's MCP servers.
...
def test_discover_mcp_tools_called_after_flash_home_mutation():
...
def test_discover_mcp_tools_called_after_env_lock_release():
    ...
    flash-agent), and holding the env lock across that would serialize all
