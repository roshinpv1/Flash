...
def test_malformed_trusted_proxy_cidr_rejects_existing_trusted_session(monkeypatch):
    ...
    handler = _Handler(
        headers={"Cookie": f"flash_session={cookie}", "Remote-User": "alice"},
        client_address=("10.0.0.5", 12345),
    )
...
def test_allowlisted_peer_header_creates_trusted_session(monkeypatch):
    ...
    assert any(cookie.startswith("flash_session=") for cookie in pending)
    assert not any(cookie.startswith("flash_profile=") for cookie in pending)
...
def test_group_map_binds_profile(monkeypatch):
    _trusted_env(
        ...
        group_map={"flash_devops": "devops"},
    )
    handler = _Handler(
        headers={
            "Remote-User": "alice",
            "Remote-Groups": "flash_devops,ai_users",
        }
    )
    ...
    assert any(cookie.startswith("flash_profile=") for cookie in handler._pending_set_cookies)
...
def test_bound_profile_mismatch_rejected(monkeypatch):
    _trusted_env(monkeypatch, groups_header="Remote-Groups", group_map={"flash_devops": "devops"})
    cookie = auth.create_session(
        auth_type="trusted",
        username="alice",
        bound_profile="devops",
    )
    handler = _Handler(headers={"Cookie": f"flash_session={cookie}", "Remote-User": "alice", "Remote-Groups": "flash_devops"})
...
def test_first_trusted_profile_switch_rejection_keeps_session_cookies(monkeypatch):
    ...
    assert handler.status == 403
    assert handler.json_body()["error"] == "Profile is bound to the current session"
    assert any(cookie.startswith("flash_session=") for cookie in set_cookies)
    assert any(cookie.startswith("flash_profile=") for cookie in set_cookies)
    assert len([cookie for cookie in set_cookies if cookie.startswith("flash_session=")]) == 1
...
def test_profile_switch_accepts_bound_profile(monkeypatch):
    ...
    assert any(value.startswith("flash_profile=") for value in handler.header_values("Set-Cookie"))
...
def test_trusted_reconciliation_cache_resets_between_requests(monkeypatch):
    ...
    handler.headers = {"Cookie": f"flash_session={alice_cookie}", "Remote-User": "bob"}
...
def test_reset_clears_pending_cookies_across_keepalive_requests(monkeypatch):
    ...
    auth._queue_pending_cookie(handler, "flash_session=stale-value; Path=/")
    assert handler._pending_set_cookies == ["flash_session=stale-value; Path=/"]
...
def test_existing_trusted_session_without_header_is_invalidated(monkeypatch):
    _trusted_env(monkeypatch)
    cookie = auth.create_session(auth_type="trusted", username="alice")
    handler = _Handler(headers={"Cookie": f"flash_session={cookie}"})
...
def test_untrusted_existing_trusted_session_is_rejected_by_all_consumers(monkeypatch):
    ...
    protected = _Handler(
        headers={**headers, "Cookie": f"flash_session={protected_cookie}"},
        client_address=("10.0.0.5", 12345),
    )
    ...
    status = _Handler(
        headers={**headers, "Cookie": f"flash_session={status_cookie}"},
        client_address=("10.0.0.5", 12345),
    )
    monkeypatch.setattr(auth, "_passkey_feature_flag_enabled", lambda: False)
    monkeypatch.setattr("api.passkeys.registered_credentials", lambda: [])
    ...
    switch = _Handler(
        headers={**headers, "Cookie": f"flash_session={switch_cookie}"},
        client_address=("10.0.0.5", 12345),
    )
    switch.command = "POST"
    monkeypatch.setattr(routes, "_check_csrf", lambda _handler: True)
...
def test_trusted_session_rehydrates_bound_profile_cookie(monkeypatch):
    ...
    profile_cookie = next(
        cookie_header for cookie_header in handler._pending_set_cookies if cookie_header.startswith("flash_profile=")
    )
...
def test_first_trusted_shell_response_includes_csrf_token(monkeypatch):
    ...
    cookie_value = handler._trusted_auth_session_cookie_value
    assert any(cookie.startswith("flash_session=") for cookie in handler.header_values("Set-Cookie"))
...
def test_logout_clears_auth_and_profile_cookies(monkeypatch):
    ...
    set_cookies = handler.header_values("Set-Cookie")
    assert any(cookie.startswith("flash_session=") and "Max-Age=0" in cookie for cookie in set_cookies)
    assert any(cookie.startswith("flash_profile=") and "Max-Age=0" in cookie and "SameSite=Lax" in cookie for cookie in set_cookies)
    assert auth.verify_session(cookie) is False
...
def test_logout_identity_rotation_preserves_csrf_validation(monkeypatch):
    ...
    set_cookies = handler.header_values("Set-Cookie")
    assert any(cookie_header.startswith("flash_session=") and "Max-Age=0" in cookie_header for cookie_header in set_cookies)
    assert any(cookie_header.startswith("flash_profile=") and "Max-Age=0" in cookie_header for cookie_header in set_cookies)
