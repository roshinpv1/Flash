...
def test_config_state_dir_defaults_to_flash_home_webui(tmp_path):
    flash_home = tmp_path / ".flash" / "profiles" / "isolated"
    flash_home.mkdir(parents=True)

    state_dir = _state_dir_for_env(HERMES_HOME=flash_home)
    assert state_dir == (flash_home / "webui").resolve()


def test_config_state_dir_unchanged_for_normal_install_flash_home_unset():
    """Backward-compat: with HERMES_HOME unset, STATE_DIR stays at the platform
    default `<~/.flash>/webui` — a normal install's state must NOT relocate
    ...
    elsewhere = _state_dir_for_env(HERMES_HOME="/tmp/flash-4449-elsewhere-base")
    assert elsewhere == Path("/tmp/flash-4449-elsewhere-base/webui").resolve()
