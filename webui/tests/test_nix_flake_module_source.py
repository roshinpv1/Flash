...
def test_flake_check_covers_runtime_layout_and_wrapper_help():
    ...
    assert "test -d ${package}/flash-webui/api" in FLAKE_NIX
    assert "test -d ${package}/flash-webui/static" in FLAKE_NIX
    assert "${package}/bin/flash-webui --help >/dev/null" in FLAKE_NIX
...
def test_nixos_module_uses_explicit_agent_dir_passthru_when_available():
    ...
    legacy_agent_share_dir = "${cfg.agent.package}" + "/share/" + "flash-agent"

    assert "cfg.agent.package.passthru ? flashAgentDir" in MODULE_NIX
    assert "${cfg.agent.package.passthru.flashAgentDir}" in MODULE_NIX
    assert hardcoded_python_site_packages not in MODULE_NIX
    assert legacy_agent_share_dir not in MODULE_NIX


def test_flake_checks_package_with_only_flash_venv_metadata():
    assert "packageOnlyAgentPackage" in FLAKE_NIX
    assert "passthru.flashVenv = packageOnlyAgentVenv;" in FLAKE_NIX
...
def test_readme_wires_published_agent_flake_package():
    assert 'flash-agent.url = "github:NousResearch/flash-agent";' in README
    assert "agent.package = flash-agent.packages.${pkgs.stdenv.hostPlatform.system}.default;" in README
    assert 'flashHome = "/var/lib/flash/.flash";' in README


def test_nixos_module_does_not_chown_existing_flash_home():
    assert "d ${cfg.flashHome}" not in MODULE_NIX
