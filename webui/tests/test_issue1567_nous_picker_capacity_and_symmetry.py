...
1. **Endpoint disagreement (the bug):** The Settings → Providers card and the
   ...
   ``list_available_providers().authenticated``. On some flash_cli versions
...
def _install_fake_flash_cli(
    ...
    *,
    ...
    """Install fake ``flash_cli`` modules with controllable Nous behavior.

    The two flags ``list_authenticated`` and ``auth_status_logged_in`` model
    the divergence between ``flash_cli.models.list_available_providers()``
    and ``flash_cli.auth.get_auth_status()`` that #1567 calls out as a
    real-world pattern on some flash_cli versions.
    """
...
class TestApiModelsLargeCatalog:
    ...
    def test_picker_caps_large_catalog_and_exposes_extras(self, monkeypatch, tmp_path):
        ...
        catalog = _build_big_catalog()
        _install_fake_flash_cli(monkeypatch, nous_ids=catalog)
    ...
    def test_picker_does_not_cap_small_catalog(self, monkeypatch, tmp_path):
        ...
        _install_fake_flash_cli(monkeypatch, nous_ids=small_catalog)
        monkeypatch.setattr(profiles, "get_active_flash_home", lambda: tmp_path)
...
class TestNousDetectionSymmetry:
    ...
    def test_picker_omits_nous_when_both_auth_signals_false(self, monkeypatch, tmp_path):
        ...
        flash_cli-unavailable case only."""
        _scrub_provider_env(monkeypatch)
        _install_fake_flash_cli(
            monkeypatch,
...
class TestNousLiveFetchEmpty:
    """When authenticated but live-fetch returns [] (transient flash_cli
    ...
    def test_flash_cli_unavailable_falls_back_to_static_4(self, monkeypatch, tmp_path):
        """When flash_cli is unavailable (raises) — distinct from returning [] —
        ...
        list at import time if flash_cli was available then, so we assert the
        ...
        try:
            ...
            nous_groups = [g for g in data["groups"] if g["provider_id"] == "nous"]
            assert nous_groups, (
                "When flash_cli raises, Nous group MUST still appear with "
