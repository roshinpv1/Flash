...
runs `flash setup` in a terminal and the Flash auth store switches the active
...
def _write_auth_store(flash_home, provider_id: str) -> None:
...
def _configure_isolated_sources(tmp_path, monkeypatch, provider_id: str) -> None:
    ...
    # Keep the test hermetic without requiring flash-agent to be installed in
    # CI: inject the tiny flash_cli surface get_available_models() imports.
    ...
    _write_auth_store(flash_home, provider_id)
...
def test_disk_models_cache_invalidates_when_auth_store_active_provider_changes(
    ...
    # External terminal `flash setup` changes auth.json, not WebUI's in-process cache.
    flash_home = config._models_cache_path.parent.parent / "flash-home"
    _write_auth_store(flash_home, "opencode-go")
    _reset_memory_cache()
