...
def test_boot_mirrors_server_settings_before_tts_apply_and_preserves_failure_fallback():
    ...
    apply_idx = BOOT_JS.index("_applyTtsEnabled(localStorage.getItem('flash-tts-enabled')==='true')", success_call_idx)
    catch_idx = BOOT_JS.index("}catch(e){", success_call_idx)
    failure_apply_idx = BOOT_JS.index("_applyTtsEnabled(localStorage.getItem('flash-tts-enabled')==='true')", catch_idx)
    ...
    for storage_key in [
        ...
        "flash-tts-rate",
        "flash-tts-pitch",
        "flash-voice-mode-button",
        "flash-voice-continuous",
        "flash-voice-silence-ms",
        "flash-raw-audio-mode",
    ]:
...
assert.strictEqual(localStorage.getItem('flash-tts-enabled'), 'true');
assert.strictEqual(localStorage.getItem('flash-tts-pitch'), '0.8');
{{
  let settings = {{tts_enabled: false, tts_pitch: 1, persisted_speech_keys: []}};
  {speech_setting_block}
  assert.strictEqual(_speechSetting('tts_enabled', 'flash-tts-enabled', false, 'bool'), 'true');
  assert.strictEqual(_speechSetting('tts_pitch', 'flash-tts-pitch', 1), '0.8');
}}
_mirrorSpeechSettingsFromServer({{tts_enabled: false, tts_pitch: 1, persisted_speech_keys: ['tts_enabled', 'tts_pitch']}});
assert.strictEqual(localStorage.getItem('flash-tts-enabled'), 'false');
assert.strictEqual(localStorage.getItem('flash-tts-pitch'), '1');
{{
  let settings = {{tts_enabled: false, tts_pitch: 1, persisted_speech_keys: ['tts_enabled', 'tts_pitch']}};
  {speech_setting_block}
  assert.strictEqual(_speechSetting('tts_enabled', 'flash-tts-enabled', false, 'bool'), false);
  assert.strictEqual(_speechSetting('tts_pitch', 'flash-tts-pitch', 1), 1);
}}
"""
...
localStorage.clear();
localStorage.setItem('flash-tts-pitch', '0.8');
...
def test_settings_panel_persists_speech_fields_and_keeps_immediate_cache_writes():
    ...
    assert "_speechSetting('tts_engine','flash-tts-engine','browser')" in panel_block
