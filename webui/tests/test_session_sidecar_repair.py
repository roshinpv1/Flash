...
def _make_stale_session(session_id="stale_sid", pending_msg="Hello flash", stream_id="stream_1"):
...
def _write_core_transcript(flash_home, session_id, messages, **extra):
    """Write a core transcript JSON file for a session."""
    core_path = flash_home / "sessions" / f"session_{session_id}.json"
...
class TestRepairStalePendingNoDeadlock:
    ...
    def test_returns_false_when_lock_already_held(self, flash_home, monkeypatch):
    ...
    def test_no_deadlock_when_get_session_triggers_repair(self, flash_home, monkeypatch):
    ...
    def test_lock_contended_skip_retries_on_next_cache_miss(self, flash_home, monkeypatch):
...
class TestDraftRecovery:
    ...
    def test_pending_message_recovered_as_user_turn(self, flash_home, monkeypatch):
        ...
        with lock:
            core_path = flash_home / "sessions" / f"session_{s.session_id}.json"
    ...
    def test_pending_message_recovered_into_context_messages(self, flash_home, monkeypatch):
    ...
    def test_error_marker_no_preserved_as_draft(self, flash_home, monkeypatch):
    ...
    def test_pending_attachments_recovered(self, flash_home, monkeypatch):
    ...
    def test_pending_fields_cleared_after_recovery(self, flash_home, monkeypatch):
...
class TestStreamIdRecheck:
    ...
    def test_bails_when_stream_id_rotated(self, flash_home, monkeypatch):
    ...
    def test_bails_when_stream_came_alive(self, flash_home, monkeypatch):
    ...
    def test_proceeds_when_stream_is_dead(self, flash_home, monkeypatch):
...
class TestGetProfileHome:
    ...
    def test_expands_tilde_when_profiles_unavailable(self, monkeypatch):
        """When api.profiles import fails, fallback uses HERMES_HOME or ~/.flash
        ...
        assert str(result) == str(Path.home() / ".flash")

    def test_uses_flash_home_env_var(self, monkeypatch):
        """When HERMES_HOME is set, fallback uses it with expansion."""
        monkeypatch.setitem(sys.modules, "api.profiles", None)
        monkeypatch.setenv("HERMES_HOME", "/custom/flash")
        result = _get_profile_home(None)
        assert result == Path(os.environ["HERMES_HOME"]).expanduser()

    def test_expands_tilde_in_flash_home(self, monkeypatch):
        """If HERMES_HOME contains ~, it gets expanded."""
        monkeypatch.setitem(sys.modules, "api.profiles", None)
        monkeypatch.setenv("HERMES_HOME", "~/my-flash")
        result = _get_profile_home(None)
        assert "~" not in str(result)
        assert str(result) == str(Path.home() / "my-flash")
...
class TestCancelInProgressGuard:
    ...
    def test_bails_when_cancel_flag_set(self, flash_home, monkeypatch):
    ...
    def test_proceeds_when_cancel_flag_not_set(self, flash_home, monkeypatch):
    ...
    def test_proceeds_when_cancel_flag_absent(self, flash_home, monkeypatch):
...
class TestEmptyMessagesGuard:
    ...
    def test_pending_cleared_when_messages_nonempty_direct(self, flash_home, monkeypatch):
    ...
    def test_repeated_pending_prompt_uses_current_checkpoint_identity(self, flash_home, monkeypatch):
    ...
    def test_bails_when_pending_user_message_none(self, flash_home, monkeypatch):
    ...
    def test_proceeds_when_messages_empty(self, flash_home, monkeypatch):
...
class TestNonEmptyMessagesPendingCleared:
    ...
    def test_pending_cleared_when_messages_nonempty(self, flash_home, monkeypatch):
        ...
        _write_core_transcript(flash_home, s.session_id, core_messages)
    ...
    def test_journaled_partial_output_is_recovered_before_interrupted_marker(self, flash_home, monkeypatch):
        ...
        append_run_event(
            ...
            {
                "name": "terminal",
                "preview": "gh pr list --repo nesquena/flash-webui",
                "args": {"command": "gh pr list --repo nesquena/flash-webui"},
            },
        )
        append_run_event(
            s.session_id,
    ...
    def test_journal_recovery_does_not_materialize_reasoning_only_events(self, flash_home, monkeypatch):
    ...
    def test_journal_recovery_keeps_consecutive_tools_on_one_anchor(self, flash_home, monkeypatch):
    ...
    def test_core_sync_branch_recovers_visible_journal_output(self, flash_home, monkeypatch):
        ...
        core_path = _write_core_transcript(flash_home, s.session_id, core_messages)
        ...
        core_tool_calls = [
            {
                "name": "terminal",
                "preview": "gh pr list --repo nesquena/flash-webui",
                "snippet": "gh pr list --repo nesquena/flash-webui",
                "assistant_msg_idx": 1,
                "done": True,
            },
        ]
...
class TestLastResortSyncDelegation:
    ...
    def test_uses_shared_get_profile_home(self, flash_home, monkeypatch):
    ...
    def test_uses_shared_apply_core_sync_or_error_marker(self, flash_home, monkeypatch):
    ...
    def test_core_sync_from_last_resort(self, flash_home, monkeypatch):
...
class TestRepairStalePendingIntegration:
    ...
    def test_repairs_when_core_exists(self, flash_home, monkeypatch):
    ...
    def test_repairs_when_core_missing(self, flash_home, monkeypatch):
    ...
    def test_recovers_when_messages_nonempty(self, flash_home, monkeypatch):
    ...
    def test_skips_when_stream_alive(self, flash_home, monkeypatch):
    ...
    def test_skips_when_worker_alive_without_sse_stream(self, flash_home, monkeypatch):
        ...
        result = _repair_stale_pending(s)
        assert result is False
        assert s.pending_user_message == "Hello flash"
        assert s.active_stream_id == "detached_stream"
        assert s.messages == []

    def test_skips_when_no_pending(self, flash_home, monkeypatch):
...
class TestCoreSyncMetadata:
    ...
    def test_syncs_token_and_cost_fields(self, flash_home, monkeypatch):
        ...
        core_path = _write_core_transcript(
            flash_home, s.session_id, core_messages,
    ...
    def test_core_empty_messages_falls_through_to_recovery(self, flash_home, monkeypatch):
        ...
        core_path = _write_core_transcript(flash_home, s.session_id, [])
...
class TestRetryJournalRecoveryInPlace:
    ...
    def _make_pending_session(self, flash_home, stream_id="lazy_stream",
    ...
    def test_promotes_marker_when_journal_now_available(self, flash_home, monkeypatch):
        stream_id = "lazy_stream_promote"
        s = self._make_pending_session(flash_home, stream_id=stream_id)
    ...
    def test_increments_attempts_when_journal_still_empty(self, flash_home, monkeypatch):
    ...
    def test_demotes_to_neutral_after_max_attempts(self, flash_home, monkeypatch):
        stream_id = "lazy_stream_giveup_attempts"
        s = self._make_pending_session(
            flash_home, stream_id=stream_id,
    ...
    def test_demotes_to_neutral_after_giveup_seconds(self, flash_home, monkeypatch):
        stream_id = "lazy_stream_giveup_age"
        first_seen = int(time.time()) - (models._JOURNAL_RETRY_GIVEUP_SECONDS + 60)
        s = self._make_pending_session(
            flash_home, stream_id=stream_id, first_seen_ts=first_seen,
        )
        ok = models._retry_journal_recovery_in_place(s)
        assert ok is False
        marker = s.messages[-1]
    ...
    def test_noop_when_no_pending_marker(self, flash_home, monkeypatch):
        s = _make_session(messages=[
            {"role": "user", "content": "x"},
    ...
    def test_short_circuit_helper_detects_pending_marker(self, flash_home, monkeypatch):
        s = _make_session(messages=[
            {"role": "user", "content": "x"},
            {"role": "assistant", "content": "y"},
        ])
    ...
    def test_short_circuit_helper_stops_at_normal_assistant(self, flash_home, monkeypatch):
        s = _make_session(messages=[
            {"role": "user", "content": "x"},
        ])
...
class TestGetSessionLazyRetryHook:
    ...
    def test_triggers_retry_on_cache_hit(self, flash_home, monkeypatch):
        sid = "lazy_get_cache"
    ...
    def test_triggers_retry_on_cold_load(self, flash_home, monkeypatch):
        sid = "lazy_get_cold"
    ...
    def test_short_circuit_when_no_pending_marker(self, flash_home, monkeypatch):
    ...
    def test_metadata_only_skips_retry(self, flash_home, monkeypatch):
        sid = "lazy_get_meta"
...
class TestLazyRetryBackwardsCompat:
    ...
    def test_legacy_marker_without_flag_unchanged(self, flash_home, monkeypatch):
...
class TestJournalToolDedupeScoping:
    ...
    def test_repeated_tool_in_earlier_turn_does_not_block_recovery(self, flash_home, monkeypatch):
    ...
    def test_untagged_tool_still_matches_for_core_transcript_invariant(self, flash_home, monkeypatch):
    ...
    def test_tagged_tool_with_different_stream_does_not_match(self, flash_home, monkeypatch):
...
class TestWslPageCacheRace:
    ...
    def test_first_read_raises_oserror_second_read_succeeds(self, flash_home, monkeypatch):
        sid = "wsl_race_sid"
        ...
        core_path = flash_home / "sessions" / f"session_{sid}.json"
        result = _apply_core_sync_or_error_marker(
    ...
    def test_journal_grows_between_reads(self, flash_home, monkeypatch):
        sid = "wsl_grow_sid"
    ...
    def test_concurrent_get_session_calls_idempotent(self, flash_home, monkeypatch):
