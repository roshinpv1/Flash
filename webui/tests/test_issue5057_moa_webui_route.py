...
def _install_fake_moa_config(monkeypatch, *, default_preset="moa-default", usage_text="Usage: /moa <prompt>"):
    ...
    # monkeypatch.setattr restores the REAL flash_cli.__path__ on teardown;
    ...
    moa_config = ModuleType("flash_cli.moa_config")
    ...
    monkeypatch.setitem(sys.modules, "flash_cli.moa_config", moa_config)


def _install_fake_flash_config(monkeypatch, cfg_data=None):
    flash_cli_pkg = sys.modules.get("flash_cli") or ModuleType("flash_cli")
    # monkeypatch.setattr restores the REAL flash_cli.__path__ on teardown;
    # emptying it in place would strand the package for the rest of the suite.
    monkeypatch.setattr(flash_cli_pkg, "__path__", [], raising=False)
    config_mod = ModuleType("flash_cli.config")
    ...
    monkeypatch.setitem(sys.modules, "flash_cli", flash_cli_pkg)
    monkeypatch.setitem(sys.modules, "flash_cli.config", config_mod)
...
def test_resolve_moa_config_returns_expected_shape(monkeypatch):
    ...
    _install_fake_flash_config(monkeypatch, cfg_data={"moa": {}})
...
def test_resolve_moa_config_degrades_without_config(monkeypatch):
    ...
    _install_fake_flash_config(monkeypatch, cfg_data=None)
    from api.commands import resolve_moa_config
    result = resolve_moa_config()
...
def test_resolve_moa_config_raises_when_moa_unavailable(monkeypatch):
    monkeypatch.setitem(sys.modules, "flash_cli.moa_config", None)
    from api.commands import resolve_moa_config
...
def test_no_subprocess_in_moa_code_paths():
    ...
    assert "FlashCLI" not in func_body
...
def test_moa_config_is_per_turn_not_persisted():
    ...
    # TypeError on an older flash-agent whose run_conversation() predates the
