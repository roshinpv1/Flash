...
def test_critical_boot_storage_access_is_guarded():
    ...
    theme_script = re.search(r"<script>\(function\(\)\{[\s\S]*?flash-theme[\s\S]*?\}\)\(\)</script>", index)
    font_script = re.search(r"<script>\(function\(\)\{[\s\S]*?flash-font-size[\s\S]*?\}\)\(\)</script>", index)
    assert theme_script and "try" in theme_script.group(0)
    assert font_script and "try" in font_script.group(0)
    assert "try{localStorage.removeItem('flash-webui-server-stopped')" in boot
    assert "try { localStorage.setItem('flash-lang', resolved); } catch" in i18n
    assert "try { stored = localStorage.getItem('flash-lang'); } catch" in i18n
