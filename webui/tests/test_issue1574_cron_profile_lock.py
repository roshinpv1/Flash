...
def _install_fake_cron(monkeypatch, run_job, events):
    ...
    cron_jobs.HERMES_DIR = Path("/tmp/flash")
    ...
    cron_scheduler._flash_home = Path("/tmp/flash")
    cron_scheduler._LOCK_DIR = cron_scheduler._flash_home / "cron"
...
def _write_spawn_fake_agent(root: Path, *, run_job_body: str):
    ...
    (cron_dir / "scheduler.py").write_text(
        "from pathlib import Path\n"
        "_flash_home = Path('/tmp/flash')\n"
        "_LOCK_DIR = _flash_home / 'cron'\n"
...
def _activate_spawn_fake_agent(fake_agent_root: Path):
    ...
    parts = [
        ...
        if p and ("flash-agent" not in p or p == fake_path)
    ]
    os.environ["PYTHONPATH"] = os.pathsep.join([fake_path, *[p for p in parts if p != fake_path]])
    sys.path[:] = [
        p
        for p in sys.path
        if not p or "flash-agent" not in p or p == fake_path
    ]
...
def _real_flash_agent_editable_install_present() -> bool:
    """Detect a developer-machine editable install of flash-agent.
    ...
    resolve; on a maintainer's dev machine an editable install of flash-agent
    ...
    try:
        ...
        _write_spawn_fake_agent(
            ...
            run_job_body=(
                ...
                "    return True, str(scheduler._flash_home), 'final', None\n"
            ),
        )
        _activate_restore = _activate_spawn_fake_agent(fake_agent_root)
        try:
...
def test_manual_cron_subprocess_drains_large_result_before_join(tmp_path):
    ...
    if _real_flash_agent_editable_install_present():
        import pytest as _pytest
        _pytest.skip(
            "skipped on dev machines with an editable flash-agent install — "
