...
in the Flash agent CLI.  WebUI must expose them in Settings → Providers and
...
def _install_fake_flash_cli(monkeypatch, *, authenticated: bool = True, model_ids: list[str] | None = None):
...
class TestPluginModelProvidersSettings:
    def test_get_providers_includes_plugin_model_provider(self, monkeypatch, tmp_path):
        ...
        _install_fake_flash_cli(monkeypatch, model_ids=["deepseek-v4-flash/latest"])
...
class TestPluginFallbackModelsInStaticCatalog:
    ...
    def test_static_catalog_surfaces_plugin_fallback_models(
        ...
        _install_fake_yandex_plugin(monkeypatch)
        _install_fake_flash_cli(monkeypatch, authenticated=True, model_ids=[])
        monkeypatch.setattr(profiles, "get_active_flash_home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    def test_static_catalog_uses_provider_fallback_models_when_declared(
        ...
        # Mock the plugin provider's API key as authenticated via flash_cli.auth
        fake_pkg = types.ModuleType("flash_cli")
        fake_pkg.__path__ = []
        fake_models = types.ModuleType("flash_cli.models")
        fake_models.list_available_providers = lambda: []
