...
class TestProfileCreateAPIWithEndpoint:
    ...
    def _cleanup(self):
        """Remove the test profile from wherever flash_cli placed it."""
        home_flash = pathlib.Path.home() / ".flash"
        # Walk all profile roots: real ~/.flash, and any subdirs that might be HERMES_HOME
        roots_to_check = set()
        roots_to_check.add(home_flash)
        for root, dirs, _ in os.walk(str(home_flash)):
            if "profiles" in dirs:
                roots_to_check.add(pathlib.Path(root))
            if root.count(os.sep) - str(home_flash).count(os.sep) > 4:
