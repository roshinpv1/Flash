...
_PANELS_DASHBOARD_DRIVER = textwrap.dedent(
    ...
    global.t = (key) => key === 'tab_dashboard' ? 'Flash Dashboard' : String(key);
    ...
    (async () => {
      ...
      if (action === 'render') {
        _renderTabVisibilityChips();
        const chip = container.children.find((node) => node.getAttribute('data-tab-panel') === '__flash_dashboard__');
...
def test_frontend_static_contracts():
    ...
    assert "flash-webui-hidden-tabs" in PANELS_JS
    assert "_TAB_ORDER_LS_KEY" in PANELS_JS
    assert "flash-webui-tab-order" in PANELS_JS
    ...
    assert "flash-webui-hidden-tabs" not in INDEX_HTML[:head_end]
...
def test_dashboard_chip_renders_in_tab_visibility_grid():
    ...
    for mode in ("auto", "always", "never"):
        ...
        assert out["chipText"] == "Flash Dashboard", out
