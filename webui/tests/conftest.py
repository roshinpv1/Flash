...
PATH DISCOVERY:
  No hardcoded paths. Discovery order:
    ...
    3. Common install paths (~/.flash/flash-agent)
...
if not (3, 11) <= sys.version_info[:2] <= (3, 13):
    pytest.exit(
        "Flash WebUI tests require Python 3.11, 3.12, or 3.13. "
...
HERMES_HOME = pathlib.Path(os.getenv('HERMES_HOME', str(HOME / '.flash')))
...
# Test state must NEVER live inside the real Flash home (~/.flash or any
...
# ~/.flash/profiles/<...>/ (observed: 144 leaked webui-test-* dirs in the real
# profile home). We now anchor the default under the OS temp dir, in a dedicated
# `flash-webui-tests/` namespace, fully outside any production tree.
...
# inside the REAL Flash home tree — a misconfigured HERMES_WEBUI_TEST_STATE_DIR
# pointing at ~/.flash would let tests wipe/clobber production profiles,
# sessions, and credentials on teardown. We anchor on the literal user-home
# `~/.flash` (NOT $HERMES_HOME): HERMES_HOME is frequently overridden to a
...
_PROD_HERMES_HOME = (HOME / '.flash').resolve()
...
# production home. If TMPDIR is (mis)configured under ~/.flash, a "temp" path is
...
if _under_prod and not _under_temp:
    raise RuntimeError(
        ...
        f"Flash home {_PROD_HERMES_HOME}. Tests must never touch production files. "
...
# ~/.flash state tree before the server subprocess fixture starts.
...
# Flash Agent sessions may inherit HERMES_CONFIG_PATH pointing at the live
# ~/.flash/config.yaml.  Override it before any product modules are imported so
...
def _isolate_flash_config_path():
...
def _rederive_default_flash_home():
    ...
    corrupts that global to a now-deleted tmpdir, making get_flash_home_for_profile
    resolve later tests' profiles under the dead path. Re-deriving keeps it honest.
    """
    prof_mod = sys.modules.get('api.profiles')
    if prof_mod is not None and hasattr(prof_mod, '_resolve_base_flash_home'):
        try:
            prof_mod._DEFAULT_HERMES_HOME = prof_mod._resolve_base_flash_home()
        except Exception:
            pass
...
# ── Flash agent discovery (mirrors api/config._discover_agent_dir) ───────
def _discover_agent_dir() -> pathlib.Path:
    candidates = [
        os.getenv('HERMES_WEBUI_AGENT_DIR', ''),
        str(HERMES_HOME / 'flash-agent'),
        str(REPO_ROOT.parent / 'flash-agent'),
        str(HOME / '.flash' / 'flash-agent'),
        str(HOME / 'flash-agent'),
    ]
    for c in candidates:
        if not c:
            continue
...
# Tests that require flash-agent modules (cron, skills, approval, chat/stream)
...
def _check_agent_modules():
    """Verify flash-agent Python modules are actually importable."""
    if not HERMES_AGENT:
        return False
    try:
        import importlib
...
# pytest marker: skip tests that need flash-agent when it's not present
requires_agent = pytest.mark.skipif(
    not AGENT_AVAILABLE,
    reason="flash-agent not found (skipping agent-dependent test)"
)
requires_agent_modules = pytest.mark.skipif(
    not AGENT_MODULES_AVAILABLE,
    reason="flash-agent Python modules not importable (cron, skills_tool)"
)

def pytest_configure(config):
    config.addinivalue_line("markers", "requires_agent: skip when flash-agent dir is not found")
    config.addinivalue_line("markers", "requires_agent_modules: skip when flash-agent Python modules are not importable")
...
# Background: when flash-agent's bedrock_adapter / botocore credential chain
...
# the guard the flash_cli/doctor.py command already uses in its parallel-probe
# block).
#
...
# raises OSError("flash test network isolation").  Tests that deliberately
...
import socket as _flash_test_socket
_REAL_CREATE_CONNECTION = _flash_test_socket.create_connection
_REAL_SOCKET_CONNECT = _flash_test_socket.socket.connect


def _flash_addr_is_local(host: str) -> bool:
...
def _flash_blocked_create_connection(address, *a, **kw):
    try:
        host = address[0]
    except (TypeError, IndexError):
        host = ""
    if _flash_addr_is_local(host):
        return _REAL_CREATE_CONNECTION(address, *a, **kw)
    raise OSError(
        f"flash test network isolation: outbound socket to {address!r} is blocked. "
...
def _flash_blocked_socket_connect(self, address):
    try:
        host = address[0]
    except (TypeError, IndexError):
        host = ""
    ...
    raise OSError(
        f"flash test network isolation: socket.connect to {address!r} is blocked."
    )


_flash_test_socket.create_connection = _flash_blocked_create_connection
_flash_test_socket.socket.connect = _flash_blocked_socket_connect
...
def allow_outbound_network(monkeypatch):
    ...
    """
    monkeypatch.setattr(_flash_test_socket, "create_connection", _REAL_CREATE_CONNECTION)
    monkeypatch.setattr(_flash_test_socket.socket, "connect", _REAL_SOCKET_CONNECT)
    yield
...
def pytest_collection_modifyitems(config, items):
    """Auto-skip agent-dependent tests when flash-agent is not available.

    Instead of requiring markers on every test function, we pattern-match
    test names to known categories that depend on flash-agent modules.
    ...
    # Exact list of tests known to fail without flash-agent.
    ...
    skip_marker = pytest.mark.skip(reason="requires flash-agent (not installed)")
    skipped = 0
    ...
    if skipped:
        print(f"\nWARNING: flash-agent not found; {skipped} agent-dependent tests will be skipped\n")
...
def test_server():
    ...
    env.update({
        ...
        # ~/.flash can redirect _get_active_flash_home() out of the sandbox,
        # causing onboarding writes (config.yaml, .env) to land in the production
        # ~/.flash/profiles/webui/ and overwrite real API keys.
    ...
    import tempfile as _tempfile
    _server_log = pathlib.Path(_tempfile.gettempdir()) / f"flash-webui-test-server-{TEST_PORT}.log"
    ...
    for _attempt in range(1, boot_attempts + 1):
        ...
        # 45s (up from 20s): server.py imports the full flash-agent, which is
...
# ── Per-test flash_cli module integrity guard ───────────────────────────────
# Several tests simulate "flash_cli unavailable / CI without the package" by
# swapping sys.modules['flash_cli'] for a stub whose __path__ is [] (e.g.
# test_byok_model_dropdown's _install_provider_model_ids sets
# `flash_cli.__path__ = []`), by monkeypatch.delitem-ing it, or by installing a
# meta-path finder that raises ImportError for flash_cli.* imports. monkeypatch
# usually restores these, BUT once the REAL flash_cli module object has its
...
# `import flash_cli.profiles` can no longer find the subpackage (empty __path__)
...
# flash_cli" TLS-test failure — all pass in isolation, all fail only after one
# of the poisoners has run earlier in the suite.
#
# This autouse guard captures the genuine on-disk flash_cli package once, and
# after every test restores it if sys.modules has been left with a stub, a
# missing entry, or an emptied __path__ — and purges any poisoned flash_cli.*
# submodule entries so the next importer re-imports them cleanly from disk.
_REAL_HERMES_CLI = sys.modules.get("flash_cli")
_REAL_HERMES_CLI_PATH = (
    list(getattr(_REAL_HERMES_CLI, "__path__", []) or [])
    if _REAL_HERMES_CLI is not None
    else []
)
# flash_state is a sibling top-level module in the SAME agent dir as flash_cli
# (…/flash-agent/flash_state.py). The same "simulate agent-package
# unavailable" tests that poison flash_cli also leave flash_state unimportable
# (test_v050259_sessiondb_fd_leak's `from flash_state import SessionDB` fails
# only in the full suite, never alone), so it needs the same restore guard.
_REAL_HERMES_STATE = sys.modules.get("flash_state")
...
# PYTHONPATH and the child can't import flash_cli — the chronic
...
# flash_cli._subprocess_compat import ...`). Snapshot the agent-path env + the
...
def _flash_cli_is_healthy() -> bool:
    mod = sys.modules.get("flash_cli")
...
@pytest.fixture(autouse=True)
def _restore_flash_cli_module():
    """Restore the real flash_cli / flash_state packages + agent-path env after
    ...
    flash_cli.profiles`, `import flash_state`, or spawn a server subprocess
    that can't import the agent at all.
    """
    yield
    if _REAL_HERMES_CLI is not None and not _flash_cli_is_healthy():
        # Restore the genuine package object + its real __path__.
        try:
            _REAL_HERMES_CLI.__path__ = list(_REAL_HERMES_CLI_PATH)
        except Exception:
            pass
        sys.modules["flash_cli"] = _REAL_HERMES_CLI
        # Drop poisoned submodule entries (stubs / partially-imported) so the
        # next `import flash_cli.<sub>` re-imports the real module from disk.
        for _name in [n for n in list(sys.modules) if n.startswith("flash_cli.")]:
            _sub = sys.modules.get(_name)
            _subfile = getattr(_sub, "__file__", None)
            if not _subfile or "flash_cli" not in str(_subfile):
                sys.modules.pop(_name, None)
    # Restore flash_state if a test swapped/removed it for a stub.
    if _REAL_HERMES_STATE is not None:
        if sys.modules.get("flash_state") is not _REAL_HERMES_STATE:
            sys.modules["flash_state"] = _REAL_HERMES_STATE
