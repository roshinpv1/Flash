"""Tests for fix: onboarding wizard must not fire when Flash is already configured.

Issue #420 — existing Flash users (config.yaml present + chat_ready) were
...
def _make_status(
    *, tmp_path: pathlib.Path, config_exists: bool, chat_ready: bool, onboarding_done: bool = False
    ...
    runtime = {
        ...
        "env_path": str(tmp_path / ".flash_test" / ".env"),
    }
...
class TestOnboardingGate:
    def test_config_exists_and_chat_ready_returns_completed_true(self, tmp_path):
        ...
        assert result["completed"] is True, (
            "Wizard fired for existing Flash user! "
...
class TestApplyOnboardingSetupGuard:
    ...
    def test_setup_allowed_with_confirm_overwrite(self, tmp_path):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_home:
                ...
                # Also redirect _get_active_flash_home so .env writes go to the temp dir,
                # never to the real ~/.flash/.env.
                with mock.patch.object(mod, "_get_active_flash_home", return_value=tmp_home_path):
    ...
    def test_setup_allowed_when_no_config_exists(self, tmp_path):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_home:
                tmp_home_path = pathlib.Path(tmp_home)
                # Redirect both config path and flash home into temp dirs so the
                # test never touches the real ~/.flash/.env.
                with (
                    mock.patch.object(mod, "_get_config_path", return_value=fake_config_path),
                    mock.patch.object(mod, "_get_active_flash_home", return_value=tmp_home_path),
                ):
                    result = mod.apply_onboarding_setup(
                        {
                            "provider": "openrouter",
...
def _server_flash_home() -> pathlib.Path:
...
class TestOnboardingGateIntegration:
    ...
    @pytest.fixture(autouse=True)
    def _clean(self):
        flash_home = _server_flash_home()
        for rel in ("config.yaml", ".env"):
            (flash_home / rel).unlink(missing_ok=True)
    ...
    def test_existing_config_and_chat_ready_skips_wizard(self):
        ...
        # — but only when flash_cli imports are available
        data, _ = _http_get("/api/onboarding/status")
        try:
            if data["system"]["flash_found"] and data["system"]["imports_ok"]:
            ...
            (flash_home / "config.yaml").unlink(missing_ok=True)
            (flash_home / ".env").unlink(missing_ok=True)
            _http_post("/api/settings", {"onboarding_completed": False})
    @_needs_yaml
