...
def test_worktree_metadata_round_trips_through_session_file(_isolate_sessions):
    s = Session(
        ...
        workspace=str(_isolate_sessions.parent / "repo" / ".worktrees" / "flash-1234"),
        worktree_path=str(_isolate_sessions.parent / "repo" / ".worktrees" / "flash-1234"),
        worktree_branch="flash/flash-1234",
    ...
    assert Path(raw["worktree_path"]).as_posix().endswith(".worktrees/flash-1234")
    assert raw["worktree_branch"] == "flash/flash-1234"
...
def test_new_session_with_worktree_info_persists_immediately(_isolate_sessions):
    ...
    assert s.worktree_branch == "flash/flash-abcd1234"
...
def test_empty_worktree_session_remains_visible_in_sidebar(_isolate_sessions):
    repo = _isolate_sessions.parent / "repo"
    worktree = repo / ".worktrees" / "flash-visible"
    worktree.mkdir(parents=True)
...
def test_create_worktree_for_workspace_calls_agent_setup_with_repo_root(tmp_path, monkeypatch):
    ...
    def fake_setup(repo_root):
        ...
        return {
            "path": str(repo / ".worktrees" / "flash-test"),
            "branch": "flash/flash-test",
            "repo_root": str(repo),
        }
    ...
    assert Path(info["path"]).as_posix().endswith(".worktrees/flash-test")
    assert info["branch"] == "flash/flash-test"
...
def test_session_new_route_creates_worktree_backed_session(tmp_path, monkeypatch):
    ...
    repo = tmp_path / "repo"
    worktree = repo / ".worktrees" / "flash-route"
    repo.mkdir()
    worktree.mkdir(parents=True)
    ...
    assert session["worktree_branch"] == "flash/flash-route"
