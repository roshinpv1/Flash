...
def _run_session_identity_probe() -> dict:
    ...
    workspace_prompt = f"[Workspace::v1: /tmp/flash-webui]\n{prompt}"
    legacy_workspace_prompt = f"[Workspace: /tmp/flash-webui]\n{prompt}"
    attached_prompt = f"{prompt}\n\n[Attached files: /tmp/a.txt]"
    forced_prompt = (
        "[USER OVERRIDE] You MUST follow the skill 'flash-webui-coordinator' "
        "content provided below before responding to the next message.\n\n"
        "[FORCED SKILL CONTEXT: flash-webui-coordinator]\n"
