...
provider in ``flash_cli.auth.PROVIDER_REGISTRY``, so the agent's auxiliary
...
3. ``set_flash_default_model()`` refuses to persist ``"local"`` going
...
# ── 3. set_flash_default_model never persists 'local' ───────────────────


class TestSetFlashDefaultModelNeverPersistsLocal:
