...
def test_restart_active_profile_gateway_success_uses_active_profile_home(monkeypatch):
    ...
    monkeypatch.setattr(gateway_restart, "get_active_flash_home", lambda: "/mock/flash/home")
    ...
    assert called["args"] == ["/mock/bin/flash", "--profile", "default", "gateway", "restart"]
    assert called["env"]["HERMES_HOME"] == "/mock/flash/home"
...
def test_restart_active_profile_gateway_pins_explicit_default_profile(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda profile: "/mock/flash/default" if profile == "default" else "/mock/flash/profiles/work",
    )
    monkeypatch.setattr(gateway_restart.shutil, "which", lambda cmd: "/mock/bin/flash")
    monkeypatch.setattr(gateway_restart.subprocess, "Popen", fake_popen)
    ...
    assert result["status"] == "completed"
    assert called["args"] == ["/mock/bin/flash", "--profile", "default", "gateway", "restart"]
    assert called["env"]["HERMES_HOME"] == "/mock/flash/default"
...
def test_restart_active_profile_gateway_omits_profile_for_isolated_default_home(monkeypatch):
    ...
    monkeypatch.setattr(
        gateway_restart,
        "get_flash_home_for_profile",
        lambda profile: "/mock/flash/profiles/default",
    )
    monkeypatch.setattr(gateway_restart.shutil, "which", lambda cmd: "/mock/bin/flash")
    monkeypatch.setattr(gateway_restart.subprocess, "Popen", fake_popen)
    ...
    assert result["status"] == "completed"
    assert called["args"] == ["/mock/bin/flash", "gateway", "restart"]
    assert called["env"]["HERMES_HOME"] == "/mock/flash/profiles/default"
...
def test_restart_active_profile_gateway_accepts_renamed_root_alias(monkeypatch):
    ...
    monkeypatch.setattr(
        gateway_restart,
        "get_flash_home_for_profile",
        lambda profile: "/mock/flash/root" if profile == "rootalias" else "/mock/flash/other",
    )
    monkeypatch.setattr(
        gateway_restart,
        "_is_root_profile",
    ...
    assert result["status"] == "completed"
    assert called["args"] == ["/mock/bin/flash", "--profile", "default", "gateway", "restart"]
    assert called["env"]["HERMES_HOME"] == "/mock/flash/root"
...
def test_restart_active_profile_gateway_timeout_releases_lock_after_background_wait(monkeypatch):
    ...
    proc = MockPopen(
        ...
        env={"HERMES_HOME": "/mock/flash/home"},
    )
