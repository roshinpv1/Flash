...
def test_handoff_summary_does_not_call_removed_agent_get_response():
    """Current Flash Agent exposes run_conversation/private transports, not get_response."""
...
def test_handoff_summary_persistence_targets_both_backends_for_messaging_session(tmp_path, monkeypatch):
    ...
    mock_home = tmp_path / "flash_home"
    mock_home.mkdir()
    mock_sessions = tmp_path / "sessions"
    mock_sessions.mkdir()

    monkeypatch.setattr(profiles, "get_active_flash_home", lambda: mock_home)
