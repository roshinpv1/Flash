"""Tests for GET /api/commands -- exposes flash-agent COMMAND_REGISTRY."""
import io
import json
import urllib.error
...
def _install_fake_codex_runtime_switch(monkeypatch):
    import sys
    flash_cli_pkg = sys.modules.get("flash_cli") or ModuleType("flash_cli")
    # Restore the real flash_cli.__path__ on teardown instead of emptying it in
    # place: `sys.modules.get(...)` grabs the REAL package object, so a bare
    # `__path__ = []` permanently strands it (later `import flash_cli.<sub>`
    # fails for the rest of the suite). monkeypatch.setattr snapshots and restores.
    monkeypatch.setattr(flash_cli_pkg, "__path__", [], raising=False)
    codex_runtime_switch = ModuleType("flash_cli.codex_runtime_switch")
    calls = []
    ...
    monkeypatch.setitem(sys.modules, "flash_cli", flash_cli_pkg)
    monkeypatch.setitem(sys.modules, "flash_cli.codex_runtime_switch", codex_runtime_switch)
    return calls
...
def test_credits_command_renders_shared_credits_view(monkeypatch):
    """`/credits` should reuse the shared Flash credits view in WebUI output."""
...
def test_credits_command_returns_not_logged_in_message(monkeypatch):
    ...
    assert output == "Not logged into Nous. Run `flash auth login nous` in Flash CLI, then try /credits again."
...
def test_reload_skills_error_is_generic(monkeypatch):
    ...
    def reload_skills():
        raise RuntimeError("secret_path=C:/Users/Rod/.flash/skills/private")
...
def test_list_commands_returns_empty_for_empty_registry():
    ...
    flash_cli is missing (the empty-or-missing case)."""
    from api.commands import list_commands
    assert list_commands(_registry=[]) == []


def test_list_commands_degrades_when_agent_missing(monkeypatch):
    """If flash_cli.commands is not importable, list_commands() returns []
    ...
    import sys
    monkeypatch.setitem(sys.modules, 'flash_cli.commands', None)
