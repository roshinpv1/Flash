"""Extension theme-registration capability (window.registerFlashSkin).
...
def test_register_api_exposed_on_window():
    assert "function registerFlashSkin(descriptor)" in BOOT_JS, (
        "registerFlashSkin API missing from boot.js"
    )
    assert "window.registerFlashSkin=registerFlashSkin" in BOOT_JS, (
        "registerFlashSkin must be exposed on window for extensions to call"
    )
...
const endMarker = "window.registerFlashSkin=registerFlashSkin;";
...
`;
eval(prelude + region);
// In non-strict eval, `function registerFlashSkin` leaks into this scope.

const results = {};

// 1. valid skin registers
results.valid = registerFlashSkin({
...
results.injection = registerFlashSkin({
