"""Regression tests for WebUI handling of Flash CLI-only slash commands."""
...
def test_cli_only_response_mentions_webui_and_cli_scope():
    ...
    assert "Flash CLI-only command" in COMMANDS_JS
...
def test_browser_cli_only_response_explains_server_side_browser_tools():
    ...
    assert "`/browser` itself only works in `flash chat`" in response
...
def test_cli_only_response_helper_uses_canonical_command_name():
    ...
    assert "`/browser` is a Flash CLI-only command" in result["response"]
