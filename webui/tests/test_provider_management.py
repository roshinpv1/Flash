...
class TestGetProviders:
    ...
    def test_openai_codex_provider_card_prefers_live_catalog(self, monkeypatch, tmp_path):
        ...
        /api/models already uses flash_cli/Codex cache discovery for Codex.  The
        ...
        fake_models = sys.modules["flash_cli.models"]
        fake_models.provider_model_ids = lambda pid: (
...
class TestSetProviderKey:
    ...
    def test_set_key_writes_to_env_file(self, monkeypatch, tmp_path):
        """Setting a key should write the env var to ~/.flash/.env."""
        _install_fake_flash_cli(monkeypatch)
        monkeypatch.setattr(profiles, "get_active_flash_home", lambda: tmp_path)
...
class TestIssue1410OllamaEnvVarBleed:
    ...
    flash_cli/runtime_provider.py only consumes OLLAMA_API_KEY when the
