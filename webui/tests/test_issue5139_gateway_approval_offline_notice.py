...
def test_gateway_chat_emits_offline_warning_for_unreachable_probe():
    ...
    assert warnings[0][1]["message"] == "Gateway connection failed. Check that the connected Flash gateway is running and reachable."
...
def test_gateway_chat_keeps_unsupported_warning_for_reachable_older_gateway():
    ...
    assert warnings[0][1]["message"] == "Approvals require a newer gateway. Upgrade the connected Flash gateway to enable this."
    assert any(isinstance(item, tuple) and item[0] == "done" for item in events)
    assert not any(
...
def test_gateway_chat_source_mentions_offline_warning_type():
    ...
    assert "approval_message = \"Gateway connection failed. Check that the connected Flash gateway is running and reachable.\"" in GATEWAY_CHAT
