...
poll), every poll leaked one or more open file descriptors against `~/.flash/state.db`.
...
def test_get_cli_session_messages_closes_connection(tmp_path, tracking_sqlite, monkeypatch):
    ...
    # Stub get_active_flash_home so the tmp_path is used regardless of profile state.
    import api.profiles
    monkeypatch.setattr(api.profiles, "get_active_flash_home", lambda: str(tmp_path))
