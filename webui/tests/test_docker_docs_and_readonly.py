...
1. The `flash-agent-src` named volume is mounted READ-ONLY on the WebUI
...
3. `docs/docker.md` documents the agent-image upgrade procedure (`docker volume
   rm flash-agent-src`) — the root cause of #1416.
"""
...
# ── 1: flash-agent-src must be read-only on the WebUI mount ────────────────
...
def test_two_container_webui_mounts_agent_src_readonly():
    ...
    assert (
        "flash-agent-src:/home/flashwebui/.flash/flash-agent:ro" in src
    ), (
        "two-container: the WebUI must mount flash-agent-src with :ro. "
...
def test_three_container_webui_mounts_agent_src_readonly():
    ...
    assert (
        ...
        "three-container: the WebUI must mount flash-agent-src with :ro."
    )
...
def test_agent_service_keeps_writable_agent_src_mount():
    ...
    for fn in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        # The agent's mount is `flash-agent-src:/opt/flash` (no :ro suffix).
        # Look for the line that has /opt/flash without :ro.
        agent_lines = [
            line for line in src.splitlines()
            if "flash-agent-src:/opt/flash" in line
        ]
        assert agent_lines, f"{fn}: agent must mount flash-agent-src at /opt/flash"
        for line in agent_lines:
            assert not line.rstrip().endswith(":ro"), (
                f"{fn}: agent's flash-agent-src mount must be writable "
                f"(it populates /opt/flash on first run): {line!r}"
            )
...
def test_docker_md_documents_agent_image_upgrade():
    """The `flash-agent-src` named volume caches the agent source on first
    ...
    assert "flash-agent-src" in docs
...
# which touches `flash_agent.egg-info/` *inside the source tree* even under
...
def test_docker_init_stages_agent_source_for_writable_install():
    ...
    for line in install_lines:
        assert '"$_agent_src[all]"' not in line, (
            ...
            "directly — this fails with EROFS when the flash-agent volume "
...
def test_docker_init_makes_staged_dir_writable_after_ro_mount_copy():
    """Regression test for the docker-init "could not create flash_agent.egg-info:
    Permission denied" failure on :ro multi-container mounts.

    `rsync -a` / `cp -a` preserve the source tree's mode bits, so a flash-agent
    source mounted mode 555 leaves the staged copy also mode 555 even though the
    staging dir itself was created writable by flashwebui. setuptools then can't
    ...
    assert re.search(r"chmod\s+-R\s+u\+w\s+\"?\\?\$?_stage_src\"?", stage_block), (
        ...
        "after the rsync/cp copy. Without it, a :ro flash-agent mount leaves "
        "the staged tree mode 555 and `uv pip install` fails with "
        "'could not create flash_agent.egg-info: Permission denied' under "
