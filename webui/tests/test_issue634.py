...
Users with older flash-agent versions (missing 'source' column in state.db) got
...
class TestCliSessionsErrorSurface:
    ...
    def test_missing_source_column_suggests_upgrade(self):
        """Warning message must suggest upgrading flash-agent."""
        assert "Upgrade flash-agent" in combined_src or "upgrade flash-agent" in combined_src.lower()
