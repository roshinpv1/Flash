...
`~/.flash/profiles/<name>` shaped HERMES_HOME. But the Flash Agent launcher exports
...
def named_profile_home():
    """A normal single-user layout: base ~/.flash with several profiles, active = `webui`.

    The active profile's home is `~/.flash/profiles/webui` — exactly the
    `*/profiles/<name>` shape the Flash Agent launcher exports for a named profile, and
...
class TestIssue4586NamedProfileIsNotIsolated:
    ...
    def test_profiles_tab_is_not_clamped_to_single_profile(self, named_profile_home):
        ...
        unlike exercising the full flash_cli-backed discovery machinery (which depends on
        ...
        with mock.patch.dict(os.environ, {"HERMES_HOME": str(active)}, clear=False):
            with mock.patch("api.profiles._INITIAL_HERMES_HOME", str(active)):
                with mock.patch("api.profiles._DEFAULT_HERMES_HOME", base):
                    with mock.patch("api.profiles._resolve_base_flash_home", return_value=base):
...
class TestIssue4586ExplicitOptInStillWorks:
    ...
    def test_flag_without_profile_shape_stays_off(self, named_profile_home):
        ...
        base = named_profile_home["base"]  # base ~/.flash, NOT a */profiles/<name> path
