...
def test_legacy_loop_checks_flash_approval_request_event():
    """Legacy SSE loop must handle `flash.approval.request` events."""
    loop = _extract_legacy_sse_loop()
    assert '"flash.approval.request"' in loop, (
        "Legacy SSE loop must check for flash.approval.request event name"
    )
...
def test_legacy_loop_resets_sse_event_after_approval():
    ...
    loop = _extract_legacy_sse_loop()
    approval_idx = loop.find('"flash.approval.request"')
