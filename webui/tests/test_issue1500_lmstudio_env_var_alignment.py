...
Settings detection read it. The agent CLI runtime (flash_cli/auth.py:182,
...
def _install_fake_flash_cli(monkeypatch):
...
class TestIssue1500EnvVarAlignment:
    def test_onboarding_supported_provider_setup_uses_lm_api_key(self):
        ...
        assert meta["env_var"] == "LM_API_KEY", (
            ...
            f"(matching flash_cli/auth.py:182 api_key_env_vars=('LM_API_KEY',)). "
    ...
    def test_onboarding_writes_canonical_name_only(self, monkeypatch, tmp_path):
        ...
        # ~/.flash — pattern from webui-onboarding-provider-readiness skill.
        from api import onboarding as ob
        monkeypatch.setattr(ob, "_get_active_flash_home", lambda: tmp_path)
