...
Note (cross-tool): flash-agent's ``openai-codex`` provider config
...
def test_openai_api_key_env_var_path_detects_openai_codex(monkeypatch):
    ...
    The primary OAuth detection path (flash_cli.auth) handles Codex for
    users who ran `flash auth login openai-codex`. This test covers the
    fallback path for environments where flash_cli is not available or
