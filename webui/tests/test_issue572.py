...
Covers:
  1. _provider_api_key_present returns True for minimax-cn when
     MINIMAX_CN_API_KEY is in env (via flash_cli.auth.get_auth_status)
  ...
  4. The flash_cli import failure path is safe (falls back gracefully)
"""
...
def _inject_flash_cli_auth(get_auth_status_return):
    """Inject a minimal flash_cli.auth stub into sys.modules.

    CI doesn't install flash_cli (it's a separate package).  Tests that
    exercise the flash_cli fallback path must inject the module themselves
    rather than relying on mock.patch('flash_cli.auth.get_auth_status')
    which fails with ModuleNotFoundError when the module isn't installed.
    """
    mock_auth = types.ModuleType("flash_cli.auth")
    mock_auth.get_auth_status = mock.MagicMock(return_value=get_auth_status_return)
    mock_flash_cli = types.ModuleType("flash_cli")

    return mock.patch.dict(sys.modules, {
        "flash_cli": mock_flash_cli,
        "flash_cli.auth": mock_auth,
    })
...
class TestProviderApiKeyPresentFallback:
    ...
    def test_unsupported_provider_logged_out_returns_false(self):
        ...
        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            ...
            with _inject_flash_cli_auth({"logged_in": False}):
                result = _call_provider_api_key_present("deepseek")
                assert result is False

    def test_flash_cli_import_failure_is_safe(self):
        """If flash_cli is unavailable, falls back silently to False."""
        import builtins
        real_import = builtins.__import__

        def _block_flash_cli(name, *args, **kwargs):
            if name.startswith("flash_cli"):
                raise ImportError("flash_cli not available")
            return real_import(name, *args, **kwargs)

        with mock.patch("api.onboarding._SUPPORTED_PROVIDER_SETUPS", {
            "openrouter": {}, "anthropic": {}, "openai": {}, "custom": {}
        }):
            with mock.patch("builtins.__import__", side_effect=_block_flash_cli):
                result = _call_provider_api_key_present("minimax-cn")
...
class TestStatusFromRuntimeUnsupportedProvider:
    ...
    def _run(self, provider: str, model: str, api_key_present: bool, oauth_present: bool = False):
        ...
        with (
            ...
            mock.patch("api.onboarding._get_active_flash_home", return_value=pathlib.Path("/tmp")),
...
class TestOnboardingStatusUnsupportedProvider:
    ...
    def _make_status(self, chat_ready: bool, provider: str = "minimax-cn"):
        ...
        with (
            ...
            mock.patch.object(mod, "verify_flash_imports", return_value=(True, [], {})),
