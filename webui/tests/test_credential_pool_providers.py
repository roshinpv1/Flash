...
def _install_fake_flash_cli(monkeypatch, *, with_load_pool: bool = False, pool_data: dict | None = None):
    """Stub flash_cli modules so tests are deterministic and offline.

    When *with_load_pool* is True, also stubs flash_cli.credential_pool with a
    ...
    # not the live ~/.flash/auth.json via the real venv module.
...
def _call_get_available_models(monkeypatch, tmp_path, auth_payload, *, with_load_pool: bool = False):
    """Call get_available_models() with auth.json pinned to a temp Flash home."""
...
def test_ollama_cloud_empty_catalog_skips_group(monkeypatch, tmp_path):
    """When flash_cli returns no models for ollama-cloud, the group is omitted.
    ...
    """
    _install_fake_flash_cli(monkeypatch)

    # Override the stub to return empty for ollama-cloud.
    import sys as _sys
    _sys.modules["flash_cli.models"].provider_model_ids = lambda pid: []
...
def test_ambient_gh_cli_not_detectable_by_provider_has_key(monkeypatch, tmp_path):
    ...
    _install_fake_flash_cli(monkeypatch, with_load_pool=True, pool_data={
        "copilot": [
...
def test_has_explicit_pool_credentials_import_error_is_false(monkeypatch, tmp_path):
    ...
    _install_fake_flash_cli(monkeypatch)  # no with_load_pool — agent.credential_pool is removed
    monkeypatch.setattr(profiles, "get_active_flash_home", lambda: tmp_path)
    config._CREDENTIAL_POOL_CACHE.clear()
