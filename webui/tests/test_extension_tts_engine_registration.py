"""Extension TTS-engine registration capability (window.registerFlashTtsEngine).
...
def test_public_api_present():
    assert "window.registerFlashTtsEngine=function" in BOOT_JS
    assert "window._flashTtsSynth=function" in BOOT_JS
    assert "window._flashTtsIsRegistered=function" in BOOT_JS
    assert "window._flashTtsEngineOptions=function" in BOOT_JS
...
def test_both_playback_paths_check_registry():
    ...
    assert "_flashTtsIsRegistered(engine)" in BOOT_JS, "voice-mode path must check the registry"
    assert "_flashTtsIsRegistered(engine)" in UI_JS, "per-message path must check the registry"


def test_settings_panel_readds_registered_options():
    assert "_flashTtsEngineOptions" in PANELS_JS, (
...
def test_registration_behavior():
    ...
    end = BOOT_JS.index("window._flashTtsSynth=function")
    ...
    harness = textwrap.dedent(
        ...
        const results = {};
        // valid registration
        results.validOk = window.registerFlashTtsEngine({
          id: 'voicevox', label: 'VOICEVOX', synthesize: () => new ArrayBuffer(4)
        });
        results.isRegistered = window._flashTtsIsRegistered('voicevox');
        // reserved key rejected
        results.reservedRejected = (window.registerFlashTtsEngine({
          id: 'edge', label: 'x', synthesize: () => new ArrayBuffer(1) }) === false);
        results.openaiReservedRejected = (window.registerFlashTtsEngine({
          id: 'openai', label: 'x', synthesize: () => new ArrayBuffer(1) }) === false);
        // bad id rejected
        results.badIdRejected = (window.registerFlashTtsEngine({
          id: 'Bad Id!', label: 'x', synthesize: () => new ArrayBuffer(1) }) === false);
        // missing synthesize rejected
        results.noSynthRejected = (window.registerFlashTtsEngine({ id: 'nosynth', label: 'x' }) === false);
        // options list reflects the registered engine
        results.optionListed = window._flashTtsEngineOptions().some(e => e.id === 'voicevox');
        // synth coerces ArrayBuffer through
        window._flashTtsSynth('voicevox', 'hi', {}).then(buf => {
          results.synthReturnsArrayBuffer = (buf instanceof ArrayBuffer);
          // unregistered engine returns null
          results.unregisteredNull = (window._flashTtsSynth('ghost', 'hi', {}) === null);
