...
Patrick-81 reported a symlinked flash-agent install that produced a bare
"AIAgent not available -- check that flash-agent is on sys.path" error with
...
class TestAIAgentImportErrorDetail:
    ...
    def test_preserves_original_message_for_log_scrapers(self):
        ...
        assert first == "AIAgent not available -- check that flash-agent is on sys.path", (
    ...
    def test_lists_sys_path_entries_when_relevant(self, monkeypatch):
        """If sys.path contains entries mentioning flash/agent, the diagnostic
        ...
        monkeypatch.syspath_prepend("/fake/flash-agent")
        out = helper()
        assert "/fake/flash-agent" in out

    def test_handles_no_relevant_sys_path_entries(self, monkeypatch):
        """If sys.path has NO flash/agent-related entries, the diagnostic must
        say so explicitly — this is itself a strong diagnostic signal.
        """
        helper = _import_helper()
        # Replace sys.path with entries that mention neither flash nor agent.
        ...
        out = helper()
        assert "no entries mention flash or agent" in out, (
