...
get_flash_home_for_profile() resolves a HERMES_HOME path from a name without
...
# ── R19: get_flash_home_for_profile ─────────────────────────────────────────

def test_get_flash_home_for_profile_returns_default_for_none():
    """R19a: None / empty string / 'default' all return the base home."""
    import api.profiles as p
    base = p._DEFAULT_HERMES_HOME
    assert p.get_flash_home_for_profile(None) == base
    assert p.get_flash_home_for_profile('') == base
    assert p.get_flash_home_for_profile('default') == base


def test_get_flash_home_for_profile_returns_profile_subdir(tmp_path, monkeypatch):
    ...
    result = p.get_flash_home_for_profile('alice')
    assert result == profile_dir


def test_get_flash_home_for_profile_returns_profile_path_for_missing_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(p, '_DEFAULT_HERMES_HOME', tmp_path)
    result = p.get_flash_home_for_profile('ghost')
    assert result == tmp_path / 'profiles' / 'ghost'


def test_get_flash_home_for_profile_does_not_mutate_globals():
    """R19d: get_flash_home_for_profile() must never change _active_profile or os.environ."""
    import api.profiles as p

    before_active = p._active_profile
    before_flash_home = os.environ.get('HERMES_HOME')

    p.get_flash_home_for_profile('some-other-profile')

    assert p._active_profile == before_active, (
        "get_flash_home_for_profile() must not mutate _active_profile"
    )
    assert os.environ.get('HERMES_HOME') == before_flash_home, (
        "get_flash_home_for_profile() must not mutate os.environ['HERMES_HOME']"
    )
...
foo_home = p.get_active_flash_home()
explicit_foo_home = p.get_flash_home_for_profile('foo')
foo_runtime = p.get_profile_runtime_env(explicit_foo_home)
model_home = m._get_profile_home('foo')
explicit_bar_home = p.get_flash_home_for_profile('bar')
p.set_request_profile('bar')
active_bar_home = p.get_active_flash_home()
print(json.dumps({
...
def test_flash_base_home_named_profile_matches_cookie_without_doubling(tmp_path):
    ...
    logical cookie flash_profile=foo.  Both active-profile and explicit
...
def test_flash_base_home_named_profile_nonmatching_cookie_uses_sibling_profile_path(tmp_path):
...
def test_get_flash_home_for_profile_rejects_path_traversal():
    """R19j: get_flash_home_for_profile() must reject names that don't match
    ...
    import api.profiles as p
    base = p._DEFAULT_HERMES_HOME
    assert p.get_flash_home_for_profile('../../etc') == base
    assert p.get_flash_home_for_profile('../escape') == base
    assert p.get_flash_home_for_profile('/absolute/path') == base
    assert p.get_flash_home_for_profile('has spaces') == base
    assert p.get_flash_home_for_profile('UPPERCASE') == base
    ...
    assert p.get_flash_home_for_profile('alice') == base / 'profiles' / 'alice'
    assert p.get_flash_home_for_profile('my-profile') == base / 'profiles' / 'my-profile'
    assert p.get_flash_home_for_profile('profile_1') == base / 'profiles' / 'profile_1'
    ...
    assert p.get_flash_home_for_profile('valid\n') == base
    assert p.get_flash_home_for_profile('a\n') == base
    # - Length boundaries: 64 chars (max valid: 1 + 63 suffix) routes to profile path,
    #   65 chars rejected.
    assert p.get_flash_home_for_profile('a' * 64) == base / 'profiles' / ('a' * 64)
    assert p.get_flash_home_for_profile('a' * 65) == base
    # - Single-char name is the minimum valid form.
    assert p.get_flash_home_for_profile('a') == base / 'profiles' / 'a'
    # - Non-ASCII / Unicode-trick names are rejected by the ASCII-only charset.
    assert p.get_flash_home_for_profile('voilà') == base
    assert p.get_flash_home_for_profile('名前') == base
