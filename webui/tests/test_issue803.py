...
PR #805 extends the fix to ALL endpoints: profile switches set a flash_profile
...
Covers:
  ...
  3. get_active_flash_home() routes via thread-local
...
class TestProfileCookieHelpers:
    ...
    def test_build_profile_cookie_sets_value(self):
        ...
        assert 'flash_profile=alice' in s
    ...
    def test_build_profile_cookie_default_persists(self):
        ...
        assert 'flash_profile=default' in s
    ...
    def test_get_profile_cookie_extracts_valid_name(self, monkeypatch):
        ...
        handler = MagicMock()
        handler.headers.get = lambda k, d='': 'flash_profile=alice' if k == 'Cookie' else d
    ...
    def test_get_profile_cookie_requires_session_bound_signature_when_auth_enabled(self, monkeypatch):
        ...
        handler = MagicMock()
        handler.headers.get = lambda k, d='': (
            f'flash_session={session_cookie}; flash_profile={signed_profile}' if k == 'Cookie' else d
        )
        assert get_profile_cookie(handler) == 'alice'
    ...
    def test_get_profile_cookie_rejects_unsigned_profile_when_auth_enabled(self, monkeypatch):
        ...
        handler = MagicMock()
        handler.headers.get = lambda k, d='': (
            'flash_session=session-token.session-sig; flash_profile=alice' if k == 'Cookie' else d
        )
        assert get_profile_cookie(handler) is None
    ...
    def test_get_profile_cookie_rejects_profile_signed_for_another_session(self, monkeypatch):
        ...
        handler = MagicMock()
        handler.headers.get = lambda k, d='': (
            f'flash_session={current_session}; flash_profile={signed_profile}' if k == 'Cookie' else d
        )
        assert get_profile_cookie(handler) is None
    ...
    def test_build_profile_cookie_binds_to_auth_session_when_auth_enabled(self, monkeypatch):
        ...
        monkeypatch.setattr('api.auth.verify_session', lambda cookie: cookie == session_cookie)
        handler = MagicMock()
        handler.headers.get = lambda k, d='': f'flash_session={session_cookie}' if k == 'Cookie' else d

        cookie = build_profile_cookie('alice', handler)
        value = cookie.split('flash_profile=', 1)[1].split(';', 1)[0]
        assert value != 'alice'
    ...
    def test_get_profile_cookie_accepts_default(self, monkeypatch):
        ...
        monkeypatch.setattr('api.auth.is_auth_enabled', lambda: False)
        handler = MagicMock()
        handler.headers.get = lambda k, d='': 'flash_profile=default' if k == 'Cookie' else d
    ...
    def test_get_profile_cookie_rejects_injection(self, monkeypatch):
        ...
        for bad in ('../etc', 'a/b', 'name;DROP', 'WithCaps', 'has space', '.hidden'):
            handler = MagicMock()
            handler.headers.get = lambda k, d='', v=bad: f'flash_profile={v}' if k == 'Cookie' else d
    ...
    def test_profile_cookie_name_defaults_to_flash_profile(self, monkeypatch):
        from api.helpers import build_profile_cookie
    ...
    def test_profile_cookie_name_can_be_isolated_per_webui_instance(self, monkeypatch):
        ...
        monkeypatch.setenv('WEBUI_PROFILE_COOKIE_NAME', 'flash_profile_social')
        monkeypatch.setattr('api.auth.is_auth_enabled', lambda: False)

        s = build_profile_cookie('writer')
        assert 'flash_profile_social=writer' in s
        assert 'flash_profile=writer' not in s

        handler = MagicMock()
        handler.headers.get = lambda k, d='': (
            'flash_profile=wrong; flash_profile_social=writer' if k == 'Cookie' else d
        )
    ...
    def test_configured_profile_cookie_ignores_default_cookie_name(self, monkeypatch):
        ...
        monkeypatch.setenv('WEBUI_PROFILE_COOKIE_NAME', 'flash_profile_main')
        monkeypatch.setattr('api.auth.is_auth_enabled', lambda: False)

        handler = MagicMock()
        handler.headers.get = lambda k, d='': 'flash_profile=social_profile' if k == 'Cookie' else d
        assert get_profile_cookie(handler) is None
...
class TestProfileCookieNameResolution:
    ...
    def test_canonical_env_overrides_default(self, monkeypatch):
        ...
        monkeypatch.delenv('WEBUI_PROFILE_COOKIE_NAME', raising=False)
        monkeypatch.setenv('HERMES_WEBUI_PROFILE_COOKIE_NAME', 'flash_profile_alt')
        assert get_profile_cookie_name() == 'flash_profile_alt'

    def test_legacy_env_still_honoured(self, monkeypatch):
        from api.helpers import get_profile_cookie_name
        monkeypatch.delenv('HERMES_WEBUI_PROFILE_COOKIE_NAME', raising=False)
        monkeypatch.setenv('WEBUI_PROFILE_COOKIE_NAME', 'flash_profile_legacy')
        assert get_profile_cookie_name() == 'flash_profile_legacy'
    ...
    def test_legacy_deprecation_warns_only_once(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger='api.helpers'):
            for _ in range(3):
                assert helpers.get_profile_cookie_name() == 'flash_profile_legacy'
...
# ── 3. get_active_flash_home routes through TLS ─────────────────────────────

def test_get_active_flash_home_respects_tls(tmp_path, monkeypatch):
    import api.profiles as p
    ...
    try:
        p.set_request_profile('alice')
        assert p.get_active_flash_home() == profile_dir
        p.set_request_profile('default')
        assert p.get_active_flash_home() == tmp_path
    finally:
        p.clear_request_profile()
...
def test_switch_profile_process_wide_false_does_not_mutate_global():
    ...
    # Use 'default' — switch_profile accepts it without requiring flash_cli.
    try:
