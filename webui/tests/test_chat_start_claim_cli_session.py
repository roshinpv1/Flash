...
def test_chat_start_sanitises_500_error():
    ...
    # Maintainer follow-up (nesquena-flash, 2026-06-14): pin that
...
def _make_state_db(path: Path, sid: str, *, message_count: int = 2,
    ...
    Schema mirrors flash_state.SessionDB closely enough for
...
# ---------------------------------------------------------------------------
# Residual gap from nesquena-flash review 2026-06-10 03:58Z:
...
def test_helper_denylist_includes_gateway_and_unknown():
    ...
    # project checked out, not just at /opt/flash-webui/.
    src = ROUTES_PY.read_text(encoding="utf-8")
