...
def test_gateway_http_401_with_key_suggests_key_mismatch():
    ...
    assert event["type"] == "gateway_auth_error"
    assert event["hint"] == "Check that HERMES_WEBUI_GATEWAY_API_KEY matches the Flash Gateway API_SERVER_KEY."
...
def test_gateway_chat_worker_translates_sse_and_persists_session(tmp_path, monkeypatch):
    ...
    class FakeResponse:
        ...
        def __iter__(self):
            yield b'event: flash.tool.progress\n'
    ...
    assert captured["headers"]["X-flash-session-id"] == s.session_id
    assert captured["headers"]["X-flash-session-key"] == f"webui:{s.session_id}"
