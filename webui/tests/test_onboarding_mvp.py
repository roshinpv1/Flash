...
Python environment (the agent venv). They are skipped when flash-agent is
...
def _server_flash_home() -> pathlib.Path:
    """Get the flash home path the test server is actually using.
    ...
    return flash_home / "webui-mvp-test"


@pytest.fixture(autouse=True)
def clean_flash_config_files():
...
def test_onboarding_setup_openrouter_writes_real_config_and_env():
    ...
    if data["system"]["imports_ok"] and data["system"]["flash_found"]:
    ...
    cfg_text = (_server_flash_home() / "config.yaml").read_text(encoding="utf-8")
    env_text = (_server_flash_home() / ".env").read_text(encoding="utf-8")
...
def test_onboarding_setup_detects_incomplete_saved_provider():
    ...
    (_server_flash_home() / ".env").unlink(missing_ok=True)
...
def test_onboarding_complete_persists_flag():
    ...
    settings = json.loads(
        (_server_flash_home() / "settings.json").read_text(encoding="utf-8")
    )
