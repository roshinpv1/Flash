...
@pytest.fixture(autouse=True)
def _mock_flash_modules(monkeypatch):
...
def test_process_wakeup_pause_revalidates_when_credential_state_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(models, "_get_profile_home", lambda _profile: flash_home)
    session = Session(
...
def test_process_wakeup_pause_revalidates_named_profile_sanitized_env_pool_recovery(
    ...
    fake_auth_module = types.ModuleType("flash_cli.auth")
    ...
    monkeypatch.setitem(sys.modules, "flash_cli.auth", fake_auth_module)
    fake_flash_cli = sys.modules.get("flash_cli")
    if fake_flash_cli is not None:
        fake_flash_cli.auth = fake_auth_module
