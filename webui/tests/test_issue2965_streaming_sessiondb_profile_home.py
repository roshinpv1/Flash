...
def test_streaming_sessiondb_uses_session_profile_state_db(tmp_path, monkeypatch):
    """WebUI streaming must not rely on flash_state.DEFAULT_DB_PATH.

    ``flash_state.DEFAULT_DB_PATH`` is frozen at import time. A non-default
    ...
    profile_home = tmp_path / "flash-home" / "profiles" / "zhangtingban"
    ...
    fake_flash_state.SessionDB = fake_session_db
    ...
    monkeypatch.setattr(profiles, "get_flash_home_for_profile", lambda _profile: profile_home)
    ...
    monkeypatch.setitem(sys.modules, "flash_cli.runtime_provider", fake_runtime_module)
    monkeypatch.setitem(sys.modules, "flash_state", fake_flash_state)
