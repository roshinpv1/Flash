    ...
    *,
    ...
    if hook_throws:
        hook_setup += textwrap.dedent(
            """
            ctx.window.__flashHandlePetSlashCommand = async payload => {
...
def test_pet_help_routes_to_install_guidance_when_companion_is_missing():
    ...
    assert "https://github.com/franksong2702/flash-webui-desktop-companion#after-gallery-install" in message
...
def test_pet_slash_intercept_bypasses_generic_agent_execution():
    ...
    assert "Flash CLI-only command" not in pet["messages"][1]["content"]
    assert pet["commandExecCalls"] == []
    assert pet["remainingInput"] == ""

    assert [item["role"] for item in browser["messages"]] == ["user", "assistant"]
    assert "`/browser` is a Flash CLI-only command" in browser["messages"][1]["content"]
