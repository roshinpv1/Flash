...
2. workspace directory setup must happen before the server drops privileges;
   bind-mount dirs created by Docker as root are unwritable by flashwebui.
...
class TestDockerfileUvPreinstall:
    ...
    def test_dockerfile_uv_installed_system_wide(self):
        ...
        # (so the default install location is accessible to flashwebui user)
        has_system_dir = "/usr/local/bin" in uv_install_line or "UV_INSTALL_DIR=/usr/local/bin" in DOCKERFILE
        assert has_system_dir, (
            "uv must be installed to /usr/local/bin (system-wide) so flashwebui user "
            "can find it. Installing as flashwebuitoo puts it in /home/flashwebuitoo/.local/bin "
            "which is NOT on flashwebui's PATH."
        )
    ...
    def test_dockerfile_uv_installed_as_root_or_before_user_switch(self):
        ...
        If installed as flashwebuitoo, it lands in ~flashwebuitoo/.local/bin,
        which the flashwebui user at runtime can't see.
        """
        ...
        assert user_before == "root", (
            ...
            "Installing as flashwebuitoo puts uv in /home/flashwebuitoo/.local/bin "
            "which is not accessible to the flashwebui runtime user."
        )
...
class TestInitScriptUvSkip:
    ...
    def test_init_script_path_includes_flashwebui_local_bin(self):
        """PATH must include /home/flashwebui/.local/bin for fallback runtime install."""
        assert "/home/flashwebui/.local/bin" in INIT_SCRIPT, (
            "docker_init.bash must include /home/flashwebui/.local/bin in PATH "
...
class TestWorkspacePermissions:
    ...
    def test_workspace_uses_root_init_mkdir(self):
        ...
        leaving them unwritable by flashwebui. The production image no longer
    ...
    def test_workspace_uses_root_init_chown(self):
        ...
        assert 'chown flashwebui:flashwebui "$HERMES_WEBUI_DEFAULT_WORKSPACE"' in root_section, (
    ...
    def test_workspace_mkdir_before_chown(self):
        ...
        chown_pos = INIT_SCRIPT.find('chown flashwebui:flashwebui "$HERMES_WEBUI_DEFAULT_WORKSPACE"')
