...
def test_lost_response_recovered_on_second_read(flash_home):
...
def test_concurrent_get_session_serializes_lazy_journal_retry(flash_home, monkeypatch):
    sid = "retry_lock_sid"
...
def test_still_arriving_journal_does_not_consume_retry_budget(flash_home, monkeypatch):
...
def test_sealed_empty_journal_consumes_retry_budget_and_demotes_at_max(flash_home, monkeypatch):
...
def test_marker_demotes_after_max_attempts_with_sealed_empty_journal(flash_home, monkeypatch):
    sid = "retry_max_sid"
...
def test_marker_demotes_after_giveup_seconds(flash_home, monkeypatch):
    base = 1_779_000_000
...
def test_repair_stale_pending_skips_pre_compression_snapshot_parent(flash_home):
...
def test_repair_stale_pending_skips_parent_when_continuation_exists(flash_home):
...
def test_get_session_syncs_sidecar_from_newer_state_db_even_when_stream_not_terminal(monkeypatch):
    ...
    turn, while the underlying Flash agent has already written the current
