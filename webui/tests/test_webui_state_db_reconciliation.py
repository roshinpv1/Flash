...
def _install_test_session(monkeypatch, tmp_path, sid, sidecar_messages):
    ...
    monkeypatch.setattr(profiles, "get_active_flash_home", lambda: tmp_path, raising=False)
...
def test_api_sessions_overlays_webui_state_db_summary_after_desktop_append(monkeypatch, tmp_path):
    ...
    # Simulate the official Flash Desktop App continuing the same WebUI-origin
    # Flash Agent session and settling its final rows into state.db. The second
