"""
Regression tests for #895 (set_flash_default_model strips @nous: prefix + blocks on live fetch)
...
from api.config import resolve_model_provider, set_flash_default_model
...
# ── #895: set_flash_default_model persists @provider: prefix ──────────────
...
class TestSetDefaultModelPreservesAtPrefix:
    ...
    def test_at_nous_prefix_strips_to_bare_for_cli_compatibility(self, tmp_path, monkeypatch):
        """set_flash_default_model must persist the RESOLVED bare/slash form, not the
        `@provider:` prefix. The `@provider:` syntax is a WebUI-internal routing hint;
        the flash-agent CLI reads `config.yaml -> model.default` directly and passes
        ...
        Storing `@nous:anthropic/...` would break any user who runs `flash` in the
        ...
        result = set_flash_default_model("@nous:anthropic/claude-opus-4.6")
        ...
        assert saved["model"]["default"] == "anthropic/claude-opus-4.6", (
            f"Config must persist the resolved bare form so the flash-agent CLI "
    ...
    def test_settings_picker_applies_saved_default_via_smart_matcher(self):
        ...
        # Find the block that sets _settingsFlashDefaultModelOnOpen
        anchor = "_settingsFlashDefaultModelOnOpen=(models&&models.default_model)||"
        idx = js.find(anchor)
    ...
    def test_save_does_not_return_full_model_catalog(self, tmp_path, monkeypatch):
        """set_flash_default_model must return a lightweight ack, not call get_available_models (#895)."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        ...
        result = set_flash_default_model("openai/gpt-5.4-mini")
        # Must be a simple dict with ok+model, NOT the full catalog (which has "groups")
        assert result.get("ok") is True
        assert "groups" not in result, (
            "set_flash_default_model must not return the full model catalog — "
