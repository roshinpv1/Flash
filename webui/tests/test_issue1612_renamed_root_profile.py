"""Tests for issue #1612: renamed root profile must resolve to ~/.flash,
not ~/.flash/profiles/<name>.

A renamed root/default Flash profile (`is_default=True` on the agent side
but with a display name like `kinni`) was being treated as a named profile
directory under `~/.flash/profiles/kinni`, which doesn't exist. Every
...
def test_is_root_profile_caches_results(monkeypatch):
    ...
    def fake_list():
        calls['n'] += 1
        return [{'name': 'kinni', 'is_default': True, 'path': '/tmp/.flash'}]
...
def test_is_root_profile_invalidation_drops_stale(monkeypatch):
    ...
    seq = [
        [{'name': 'kinni', 'is_default': True, 'path': '/tmp/.flash'}],
        [{'name': 'noblepro', 'is_default': True, 'path': '/tmp/.flash'}],
    ]
...
def test_is_root_profile_handles_list_profiles_failure(monkeypatch):
    ...
    def boom():
        raise RuntimeError("flash_cli explosion")
...
# ── get_active_flash_home: returns _DEFAULT_HERMES_HOME for renamed root ──


def test_get_active_flash_home_returns_default_for_renamed_root(tmp_path, monkeypatch):
    ...
    result = p.get_active_flash_home()
    assert result == tmp_path, f"Expected {tmp_path}, got {result}"


def test_get_active_flash_home_returns_named_for_real_named_profile(tmp_path, monkeypatch):
