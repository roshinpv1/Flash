...
def _claude_fixture_rows() -> list[dict]:
    return [
        ...
        {"timestamp": "2026-04-18T12:00:01Z", "message": {"role": "user", "content": [{"type": "text", "text": "Can Flash show this Claude Code history read-only?"}]}},
...
def test_get_claude_code_sessions_reads_fixture_jsonl_without_real_home(tmp_path):
    ...
    assert messages == [
        {"role": "user", "content": "Can Flash show this Claude Code history read-only?", "timestamp": 1776513601.0},
...
def test_get_cli_sessions_reuses_short_ttl_cache(monkeypatch, tmp_path):
    ...
    flash_home.mkdir()
    monkeypatch.setattr(profiles, "get_active_flash_home", lambda: str(flash_home))
