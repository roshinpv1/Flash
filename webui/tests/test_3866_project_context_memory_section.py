...
def test_project_context_prefers_flash_md_and_reports_shadowed_agents(tmp_path):
    workspace = tmp_path / "priority"
    workspace.mkdir()
    (workspace / "HERMES.md").write_text("# Flash Rules\n\nFlash wins.", encoding="utf-8")
    (workspace / "AGENTS.md").write_text("# Agent Rules\n\nAgents lose.", encoding="utf-8")

    data = project_context_for(workspace)

    assert "Flash wins." in data["content"]
...
def test_project_context_walks_flash_md_to_git_root_but_not_agents_md(tmp_path):
    root = tmp_path / "repo"
    child = root / "src" / "pkg"
    child.mkdir(parents=True)
    (root / ".git").mkdir()
    (root / ".flash.md").write_text("# Root Flash\n\nRoot project rules.", encoding="utf-8")
    ...
    assert data["path"].endswith(".flash.md")
...
def test_project_context_content_is_redacted_in_memory_response(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(api.profiles, "get_active_flash_home", lambda: home)
...
let _memoryData = {
  ...
  memory_path: 'C:/Users/Rod/.flash/memories/MEMORY.md',
  memory_mtime: 1712345678,
  user: 'User memory body',
  user_path: 'C:/Users/Rod/.flash/memories/USER.md',
  user_mtime: 1712345678,
  soul: 'Soul memory body',
  soul_path: 'C:/Users/Rod/.flash/SOUL.md',
  soul_mtime: 1712345678,
  project_context: 'Project context body',
  project_context_path: 'D:/Repos/flash-webui/AGENTS.md',
...
def test_memory_detail_renders_path_for_non_project_sections():
    ...
    assert "C:/Users/Rod/.flash/memories/MEMORY.md" in rendered["memoryHtml"]
    assert "USER.md" in rendered["userHtml"]
    assert "C:/Users/Rod/.flash/memories/USER.md" in rendered["userHtml"]
    assert "SOUL.md" in rendered["soulHtml"]
    assert "C:/Users/Rod/.flash/SOUL.md" in rendered["soulHtml"]
    assert "AGENTS.md · D:/Repos/flash-webui/AGENTS.md" in rendered["projectHtml"]
...
def test_memory_section_list_renders_hover_path_titles():
    ...
    assert rendered["memory"] == "C:/Users/Rod/.flash/memories/MEMORY.md"
    assert rendered["user"] == "C:/Users/Rod/.flash/memories/USER.md"
    assert rendered["soul"] == "C:/Users/Rod/.flash/SOUL.md"
    assert rendered["project_context"] == "D:/Repos/flash-webui/AGENTS.md"
