...
def test_docker_docs_warn_sudo_changes_home_bind_mount():
    """#3006: sudo can render ${HOME}/.flash as /root/.flash."""
    assert "`sudo docker compose up -d` can make `${HOME}` expand to the root user's home" in README
    assert "Docker mounts the wrong `.flash` directory instead of your real `~/.flash`" in README
    assert "HERMES_HOME=/home/you/.flash" in README

    assert "sudo` often changes `$HOME` to `/root`" in DOCKER_MD
    assert "`${HERMES_HOME:-${HOME}/.flash}` becomes `/root/.flash`" in DOCKER_MD
    assert "HERMES_HOME=/home/youruser/.flash" in DOCKER_MD
