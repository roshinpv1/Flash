...
mid-body, so the agent config reader (`flash_cli.config.get_config_path` /
`load_config`, which read `get_flash_home()`) resolves the WRONG profile's
config — intermittent turn-init failures referencing another profile's provider.

Fix (#5567): when flash-agent >= v0.18.0 exposes the context-local home
override (`flash_constants.set_flash_home_override`), the worker scope installs
it so `get_flash_home()` resolves THIS task's profile home from task-local state,
immune to the process-global clobber — without serializing workers.

Per #2321's acceptance criteria, this exercises the REAL
`flash_cli.config.load_config()` against a non-default profile with an
...
config_mod = pytest.importorskip("flash_cli.config")
flash_constants = pytest.importorskip("flash_constants")

HAS_OVERRIDE = hasattr(flash_constants, "set_flash_home_override") and hasattr(
    flash_constants, "get_flash_home"
)
...
@pytest.mark.skipif(
    not HAS_OVERRIDE,
    reason="flash-agent < v0.18.0: no set_flash_home_override; WebUI degrades to the os.environ mirror",
)
def test_load_config_resolves_worker_profile_despite_env_clobber(tmp_path, monkeypatch):
    ...
    # under test; the reader is the real flash_cli.config below.
    monkeypatch.setattr(profiles_api, "get_flash_home_for_profile", lambda name: home_a)
...
def test_override_is_cleared_after_worker_exits(tmp_path, monkeypatch):
    ...
    assert flash_constants.get_flash_home_override() is None
    with profiles_api.profile_env_for_background_worker("alpha", "test worker"):
        assert flash_constants.get_flash_home_override() == str(home_a)
...
def test_graceful_degradation_resolver_is_optional():
    ...
    mod = profiles_api._resolve_flash_home_override()
    if HAS_OVERRIDE:
        assert mod is not None and hasattr(mod, "set_flash_home_override")
    else:
