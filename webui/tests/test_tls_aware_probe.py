...
def _load_bootstrap():
    spec = importlib.util.spec_from_file_location("flash_bootstrap_probe", BOOTSTRAP)
...
def test_helper_scheme_http_without_tls():
    out = subprocess.run(
        ["bash", "-c", f". {HEALTH_PROBE}; flash_webui_probe_scheme"],
...
def test_ctl_status_survives_readonly_env_vars(tmp_path: Path):
    ...
    (repo / ".env").write_text("UID=1000\nGID=1000\nHERMES_WEBUI_PORT=8787\n",
                               encoding="utf-8")
    env = os.environ.copy()
    env["HERMES_HOME"] = str(tmp_path / ".flash")
    res = subprocess.run(
        ["bash", str(repo / "ctl.sh"), "status"],
        capture_output=True, text=True, env=env, timeout=30,
    )
    assert res.returncode == 0, res.stderr
    assert "flash-webui" in res.stdout
