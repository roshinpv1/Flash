...
def test_status_returns_summary(cleanup_test_sessions):
    ...
    assert r['flash_home'] == str(TEST_STATE_DIR)
...
def test_status_returns_profile_specific_flash_home(cleanup_test_sessions):
    ...
    r = _get(
        ...
        headers={'Cookie': 'flash_profile=research'},
    )

    assert r['profile'] == 'research'
    assert r['flash_home'] == str(TEST_STATE_DIR / 'profiles' / 'research')
