...
def test_visible_progress_token_reasoning_and_interim_are_deduped(cleanup_test_sessions):
    """Progress text can arrive through three Flash callbacks; WebUI must show it once.
    ...
    fake_flash_cli.__dict__["runtime_provider"] = fake_runtime_module
    fake_flash_state = types.ModuleType("flash_state")
    fake_flash_state.__dict__["SessionDB"] = mock.Mock(return_value=None)
    injected = {
...
def test_final_answer_prefix_reasoning_echo_is_not_journaled_or_merged(cleanup_test_sessions):
    ...
    final_answer = (
        "已按 Flash WebUI workflow 在独立 worktree 完成，本地 review-ready；"
        "没有 push、没有开 PR、没有改真实 cron/config，也没有在主 checkout 实现。\n\n"
        "## 位置\n\n"
        "| 项 | 值 |\n|---|---|\n"
        "| Worktree | `/Users/xuefusong/flash-webui-worktrees/example` |\n"
        "| Branch | `fix/example` |\n\n"
        "## 根因\n\n"
