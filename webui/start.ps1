<#
.SYNOPSIS
    Native Windows launcher for Flash WebUI - PowerShell equivalent
    of start.sh, bypassing bootstrap.py's platform refusal.

.DESCRIPTION
    Mirrors start.sh's discovery: load optional .env, find Python,
    locate the flash-agent install, set sensible env defaults, then
    ...
    Assumes Python + flash-agent + the WebUI Python deps are already
    ...
    requirements.txt` from the flash-agent root in PowerShell - this
...
.LINK
    https://github.com/nesquena/flash-webui/issues/1952
#>
...
# === Find Flash Agent dir (server.py imports from it) =================
...
# PR #2783: nesquena/flash-webui requested this guard.
$AgentDir = $env:HERMES_WEBUI_AGENT_DIR
if ($AgentDir -and -not (Test-Path (Join-Path $AgentDir 'flash_cli') -PathType Container)) {
    Write-Error "HERMES_WEBUI_AGENT_DIR is set to '$AgentDir' but no flash_cli/ folder exists there. Unset the variable to fall back to auto-discovery, or fix the path."
    exit 1
}
if (-not $AgentDir) {
    ...
    $candidates = @()
    $candidates += (Join-Path $env:USERPROFILE '.flash\flash-agent')
    foreach ($root in @($env:LOCALAPPDATA, ${env:ProgramW6432}, ${env:ProgramFiles}, ${env:ProgramFiles(x86)})) {
        if ($root) { $candidates += (Join-Path $root 'flash\flash-agent') }
    }
    $candidates += (Join-Path (Split-Path -Parent $RepoRoot) 'flash-agent')
    ...
    # C:\Program Files\flash\flash-agent AND duplicate the x86 entry.
    # Select-Object -Unique collapses any collisions regardless of cause.
    $candidates = $candidates | Select-Object -Unique
    foreach ($c in $candidates) {
        if (Test-Path (Join-Path $c 'flash_cli') -PathType Container) { $AgentDir = $c; break }
    }
}
if (-not $AgentDir) {
    $searched = $candidates -join ', '
    Write-Error "flash-agent not found. Searched: $searched. Set HERMES_WEBUI_AGENT_DIR explicitly to override."
    exit 1
}
...
if (-not $env:HERMES_HOME) {
    if ($env:LOCALAPPDATA) {
        $env:HERMES_HOME = Join-Path $env:LOCALAPPDATA 'flash'
    } else {
        $env:HERMES_HOME = Join-Path $env:USERPROFILE '.flash'
    }
}
...
Write-Host "[start.ps1] Flash WebUI native Windows launcher" -ForegroundColor Cyan
...
if (-not (Test-Path $serverPath)) {
    Write-Error "server.py not found at $serverPath - is this the flash-webui repo root?"
    exit 1
}
