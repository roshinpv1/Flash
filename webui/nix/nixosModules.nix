...
let
  cfg = config.services.flash-webui;
  defaultPackage = self.packages.${pkgs.stdenv.hostPlatform.system}.default;
  defaultStateDir = "/var/lib/flash-webui";
  ...
  defaultUser = "flash-webui";
  defaultGroup = "flash-webui";

  protectedEnvironmentFileCheck = pkgs.writeShellScript "flash-webui-protected-envfile-check" ''
    set -eu
    for env_file in "$@"; do
      [ -f "$env_file" ] || continue
  ...
  inferredAgentPython =
    if cfg.agent.package == null then
      null
    else if (cfg.agent.package ? passthru) && (cfg.agent.package.passthru ? flashVenv) then
      "${cfg.agent.package.passthru.flashVenv}/bin/python3"
    else
      null;
  ...
  inferredAgentDir =
    if cfg.agent.package == null then
      null
    else if (cfg.agent.package ? passthru) && (cfg.agent.package.passthru ? flashAgentDir) then
      "${cfg.agent.package.passthru.flashAgentDir}"
    else
      null;
  ...
  mappedEnvironment = (lib.mapAttrsToList
    ...
    // lib.optionalAttrs (cfg.flashHome != null) {
      HERMES_HOME = cfg.flashHome;
    }
...
in
{
  options.services.flash-webui = {
    enable = lib.mkEnableOption "Flash WebUI service";

    package = lib.mkOption {
      type = lib.types.package;
      default = defaultPackage;
      defaultText = lib.literalExpression "self.packages.${pkgs.stdenv.hostPlatform.system}.default";
      description = "Package that provides the `bin/flash-webui` executable.";
    };

    user = lib.mkOption {
      type = lib.types.str;
      default = defaultUser;
      description = "User that runs the Flash WebUI service.";
    };

    group = lib.mkOption {
      type = lib.types.str;
      default = defaultGroup;
      description = "Group that runs the Flash WebUI service.";
    };
    ...
    stateDir = lib.mkOption {
      ...
      defaultText = lib.literalExpression ''"/var/lib/flash-webui"'';
      description = "Value for HERMES_WEBUI_STATE_DIR.";
    };

    flashHome = lib.mkOption {
    ...
    agent = {
      package = lib.mkOption {
        ...
        default = null;
        description = "Package to derive HERMES_WEBUI_PYTHON from passthru.flashVenv and optionally HERMES_WEBUI_AGENT_DIR from passthru.flashAgentDir.";
      };
  ...
  config = lib.mkIf cfg.enable {
    systemd.services.flash-webui = {
      description = "Flash Web UI service";
      ...
      serviceConfig =
        {
          ...
          ExecStart = "${cfg.package}/bin/flash-webui";
          Restart = "on-failure";
