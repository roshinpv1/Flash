# Flash Web UI: Developer and Architecture Guide

> This document is the canonical reference for anyone (human or agent) working on the
> Flash Web UI. It covers the exact current state of the code, every design decision and
...
>
> Notable architecture state: the bootstrap and first-run onboarding flow own setup discovery; the default WebUI state directory is `~/.flash/webui`; `ctl.sh` provides a daemon wrapper for homelab installs; chat streaming is still WebUI-owned SSE with stream-ownership guards, cancellation, async manual compression, and turn-journal audit plumbing; provider/model discovery is profile-aware with live-model cache invalidation and custom-provider scoping. (Version/test-count numbers above are a periodic snapshot — the authoritative source is the latest git tag and `pytest --collect-only`.)

---

## 1. Overview and Purpose

The Flash Web UI is a lightweight web application that gives you a browser-based
interface to the Flash agent that is functionally equivalent to the CLI. It is modeled on
...
Flash-level chrome is intentionally consolidated: the sidebar has no dedicated brand header.
Instead, the footer exposes a single "Flash WebUI" launch button that opens one tabbed
...
Log file:
    ...
    ~/.flash/webui/bootstrap-8787.log   start.sh/bootstrap background server log
    ~/.flash/webui.log                  ctl.sh daemon log
...
- The venv has all Flash agent dependencies (run_agent, tools/*, cron/*)
...
- The server imports Flash modules via sys.path.insert(0, parent_dir)
...
Environment variables controlling behavior:
    ...
    HERMES_CONFIG_PATH             Path to ~/.flash/config.yaml
    ...
    HERMES_HOME                    Base directory for Flash state (~/.flash by default)

Test isolation environment variables (set by conftest.py):

    HERMES_WEBUI_TEST_PORT=...                         Optional pinned test port
    HERMES_WEBUI_TEST_STATE_DIR=/tmp/flash-webui-tests/* Optional pinned test state (default: OS temp dir; must be outside ~/.flash)
...
The approval system uses the existing Flash gateway module at tools/approval.py.
...
Three-panel layout (in static/index.html):
    ...
    <aside class="sidebar">    Left panel: session list, nav tabs, sidebar-footer Flash WebUI trigger
...
Boot IIFE:
    localStorage key 'flash-webui-session' stores last session_id
...
9.  setBusy(true), setStatus('Flash is thinking...')
...
The api/ modules in turn import Flash internals:
...
On startup, server.py reads ~/.flash/config.yaml:
...
1. Structured JSON logging: COMPLETE (Sprint 1). Per-request JSON is printed to the active launcher log (`~/.flash/webui/bootstrap-8787.log` for `start.sh`, `~/.flash/webui.log` for `ctl.sh`).
...
Endpoint that calls Flash Python modules:
...
## 12. Common Debugging Commands
    ...
    tail -f ~/.flash/webui/bootstrap-8787.log
    tail -f ~/.flash/webui.log  # when launched through ctl.sh
    ...
    ls -lt ~/.flash/webui/sessions/
    cat ~/.flash/webui/sessions/SESSION_ID.json | python3 -m json.tool
    ...
    ls ~/.flash/webui/sessions/ | xargs -I{} python3 -c "
    import json, sys
    d = json.load(open('~/.flash/webui/sessions/{}'))
...
This section is specifically for agents (Flash instances, subagents, Codex, etc.) that
...
~/.flash/memories/, and SOUL.md from ~/.flash/), renders both as markdown via renderMd() with timestamps.
...
The symlink means all existing import paths (sys.path.insert for flash-agent modules)
continue working unchanged. start.sh updated to reference new canonical path.

Safe from: git pull, git reset --hard, git stash on flash-agent repo.
...
WORKSPACES_FILE = ~/.flash/webui-mvp/workspaces.json
LAST_WORKSPACE_FILE = ~/.flash/webui-mvp/last_workspace.txt
