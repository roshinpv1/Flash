# Agent instructions for Flash WebUI
...
- do not delete or overwrite a real `~/.flash` directory without explicit
  approval
...
- For Docker build changes in `docker_init.bash`, mirror directory exclusions
  in both the `rsync` and `cp -a` paths — `/opt/flash` may contain subdirectories
  with restricted permissions (e.g. `.playwright/`).

## Local state and secrets

Flash WebUI can read and write real agent state, sessions, workspaces,
...
```bash
HERMES_HOME=/tmp/flash-webui-agent-home \
HERMES_WEBUI_STATE_DIR=/tmp/flash-webui-agent-state \
