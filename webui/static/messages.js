...
if (typeof window !== 'undefined') {
  window.__flashSetBackgrounded = (value) => {
  ...
  try {
    ...
    _runOptionalPostStartUiStep('post-start ui/bookkeeping', () => {
      ...
      if (startData && startData.effective_model && S.session) {
        ...
        localStorage.setItem('flash-webui-model', startData.effective_model);
...
function attachLiveStream(activeSid, streamId, uploaded = [], options = {}) {
  ...
  const _anchorApi = (typeof window !== 'undefined' && window.FlashAssistantTurnAnchors)
    ? window.FlashAssistantTurnAnchors
    : null;
  ...
  function _streamFadeNextText(targetText) {
    ...
    // appear to accelerate. Flash has a playout buffer, so track incoming word
          ...
          if (S.session && S.session.session_id === activeSid) {
            const _wasFollowingAtCancelFb = ((typeof _isMessagePaneNearBottom === 'function')
              ...
              && !((typeof _isMessageReaderUnpinned === 'function')
                ...
                : (typeof _messageUserUnpinned !== 'undefined' && _messageUserUnpinned));
            clearLiveToolCards(); if (!assistantText) removeThinking();
            const cancelAgentName = (assistantDisplayName() + '').trim() || 'Flash';
...
function transcript() {
  const lines = [`# Flash session ${S.session?.session_id || ''}`, ``,
...
const _DISMISSED_APPROVALS_KEY = 'flash_dismissed_approvals';
...
function startSessionStream(sid) {
  ...
  if (typeof document !== 'undefined' && !document._flashSessionStreamVisibilityHook) {
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
    ...
    document._flashSessionStreamVisibilityHook = true;
  }
...
function _stashClarifyDraft(reason) {
  ...
  const key = `flash-clarify-draft-${sid}-${_clarifySignature || "unknown"}`;
  try {
...
function _notificationOptions(body, options = {}) {
  ...
  return { body: body || '', tag: sid ? `flash-${sid}` : 'flash-webui', renotify: false, icon: 'static/favicon-192.png', badge: 'static/favicon-32.png', data: { url } };
}
