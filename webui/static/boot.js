...
(function () {
  ...
  try { localStorage.removeItem('flash-webui-server-stopped'); } catch (_) { }
  // Listen for shutdown broadcast from other tabs
  try {
    var _stopChan = new BroadcastChannel('flash-webui-shutdown');
...
function _syncWorkspacePanelInlineWidth() {
  ...
  const saved = localStorage.getItem('flash-panel-w');
  if (!saved) return;
...
function _setWorkspacePanelMode(mode) {
  ...
  try { localStorage.setItem('flash-webui-workspace-panel', open ? 'open' : 'closed'); } catch (_) { }
...
async function _maybeBindFreshDefaultWorkspaceSession(prefillIntent = null) {
  ...
  try {
    ...
    console.warn('[flash] failed to bind fresh default workspace session', e);
    return false;
  }
}
...
const _SIDEBAR_COLLAPSED_KEY = 'flash-webui-sidebar-collapsed';
...
(function () {
  ...
  // Prefer Flash server-side STT (MediaRecorder -> /api/transcribe) only
  ...
  let _rawAudioMode = localStorage.getItem('flash-raw-audio-mode') === 'true';
  ...
  let _dictationAppend = localStorage.getItem('flash-dictation-append') !== 'false';
  ...
  function _applyRawAudioModePreference(enabled) {
    ...
    try { localStorage.setItem('flash-raw-audio-mode', _rawAudioMode ? 'true' : 'false'); } catch (_) { }
  ...
  function _applyDictationAppendPreference(enabled) {
    ...
    try { localStorage.setItem('flash-dictation-append', _dictationAppend ? 'true' : 'false'); } catch (_) { }
  ...
  // utterance). An explicit flash-mic-continuous flag wins in both directions,
  // mirroring the flash-voice-continuous pattern: 'true' opts a desktop in,
  ...
  function _micDictationContinuous() {
    try {
      const flag = localStorage.getItem('flash-mic-continuous');
...
// into localStorage — the same synchronous-source pattern used by flash-lang /
// flash-theme — so the very first send after a reload honors the saved choice.
...
const _LEGACY_DEFAULT_MESSAGE_MODE_KEY = 'flash-busy-input-mode';
const _DEFAULT_MESSAGE_MODE_KEY = 'flash-default-message-mode';
...
// ── Extension TTS-engine registry (registerFlashTtsEngine) ──────────────────
...
// handles selection, the dropdown option, and playback. Mirrors registerFlashSkin.
//
//   window.registerFlashTtsEngine({
...
function _flashTtsValidId(id) { return typeof id === 'string' && /^[a-z0-9][a-z0-9_-]{0,31}$/.test(id); }
function _flashAddTtsOption(id, label) {
...
window.registerFlashTtsEngine = function (desc) {
  try {
    if (!desc || typeof desc !== 'object') return false;
    var id = String(desc.id || '').toLowerCase();
    if (!_flashTtsValidId(id)) return false;
    ...
    _flashAddTtsOption(id, label);
    return true;
  } catch (_) { return false; }
};
window._flashTtsIsRegistered = function (id) { return !!_HERMES_TTS_ENGINES[id]; };
// List registered engines (for the settings panel to re-add options on render).
window._flashTtsEngineOptions = function () {
...
window._flashTtsSynth = function (id, text, opts) {
...
(function () {
  ...
  function _voiceModePrefEnabled() {
    try { return localStorage.getItem('flash-voice-mode-button') === 'true'; }
    catch (_) { return false; }
  }
  ...
  //   flash-voice-silence-ms, pause duration before auto-send (ms, default 1800)
  //   flash-voice-continuous, keep mic open across natural pauses ("true"/"false", default false)
  function _voiceSilenceMs() {
    const _silenceMsRaw = parseInt(localStorage.getItem('flash-voice-silence-ms'), 10);
  ...
  function _startListening() {
    ...
    _recognition.continuous = localStorage.getItem('flash-voice-continuous') === 'true';
  ...
  function _speakResponse() {
    ...
    // Extension-registered TTS engine (window.registerFlashTtsEngine): synth
    // via the extension, then play through the same Audio lifecycle as edge.
    if (typeof window._flashTtsIsRegistered === 'function' && window._flashTtsIsRegistered(engine)) {
      _ttsSpeaking = true;
      const _opts = {
        voice: localStorage.getItem("flash-tts-voice") || '',
        rate: parseFloat(localStorage.getItem("flash-tts-rate")),
        pitch: parseFloat(localStorage.getItem("flash-tts-pitch")),
      };
      Promise.resolve(window._flashTtsSynth(engine, clean, _opts))
        .then(function (buf) {
    ...
    if (engine === "edge") {
      const voice = localStorage.getItem("flash-tts-voice") || "zh-CN-XiaoxiaoNeural";
      const savedRate = parseFloat(localStorage.getItem("flash-tts-rate"));
      const savedPitch = parseFloat(localStorage.getItem("flash-tts-pitch"));
      let rate = '', pitch = '';
    ...
    const savedVoice = localStorage.getItem('flash-tts-voice');
...
$('btnDownload').onclick = () => {
  ...
  a.download = `flash-${S.session.session_id}.md`; a.click(); URL.revokeObjectURL(a.href);
};
$('btnExportJSON').onclick = () => {
  if (!S.session) return;
  const url = `/api/session/export?session_id=${encodeURIComponent(S.session.session_id)}`;
  const a = document.createElement('a'); a.href = url;
  a.download = `flash-${S.session.session_id}.json`; a.click();
};
$('btnShareSession').onclick = async () => {
  ...
  try {
    ...
    if (typeof _syncFlashPanelSessionActions === 'function') _syncFlashPanelSessionActions();
...
function exportSessionHTML(session) {
  ...
  const a = document.createElement('a'); a.href = url;
  a.download = `flash-${sid}.html`; a.click();
}
...
$('modelSelect').onchange = async () => {
  ...
  else try { localStorage.setItem('flash-webui-model', modelState.model) } catch { }
  if (!S.session) {
  ...
  // Warn if selected model belongs to a different provider than what Flash is configured for
...
(function () {
  ...
  window._initResizePanels = function () {
    ...
    initResize('sidebarResize', sidebar, 'right', SIDEBAR_MIN, SIDEBAR_MAX, 'flash-sidebar-w');
    initResize('rightpanelResize', rightpanel, 'left', PANEL_MIN, PANEL_MAX, 'flash-panel-w');
  };
})();
...
//   3. Native WKWebView wrappers (e.g.flash-swift-mac) that read this attribute as
...
function _syncThemeColorMeta() {
  try {
    ...
    if (!bg) return;
    const known = document.getElementById('flash-theme-color');
    if (known) {
...
function _pickTheme(name) {
  const currentSkin = localStorage.getItem('flash-skin');
  const appearance = _normalizeAppearance(name, currentSkin);
  localStorage.setItem('flash-theme', appearance.theme);
  localStorage.setItem('flash-skin', appearance.skin);
...
function _pickSkin(name) {
  const appearance = _normalizeAppearance(localStorage.getItem('flash-theme'), name);
  localStorage.setItem('flash-theme', appearance.theme);
  localStorage.setItem('flash-skin', appearance.skin);
  _applyTheme(appearance.theme);
  _applySkin(appearance.skin);
...
function _pickFontSize(size) {
  localStorage.setItem('flash-font-size', size);
  _applyFontSize(size);
...
function _buildSkinPicker(activeSkin) {
  ...
  for (const skin of _SKINS) {
    ...
    // label/name (registerFlashSkin descriptor) can never inject markup into
...
// extension calls window.registerFlashSkin(descriptor); core validates +
...
const _EXT_SKIN_STYLE_ID = 'flashExtensionSkinStyles';
...
function registerFlashSkin(descriptor) {
  try {
    ...
    if (document.getElementById('skinPickerGrid')) {
      _buildSkinPicker((localStorage.getItem('flash-skin') || 'default').toLowerCase());
    }
    // If the user had previously selected this (now-available) skin, apply it.
    if ((localStorage.getItem('flash-skin') || '').toLowerCase() === key) {
      _applySkin(key);
    }
    return true;
  } catch (_) { return false; }
}
if (typeof window !== 'undefined') window.registerFlashSkin = registerFlashSkin;
...
function _mirrorSpeechSettingsFromServer(s) {
  ...
  if (hasServerValue('raw_audio_mode')) {
    const rawAudioMode = resolveBool('raw_audio_mode', 'flash-raw-audio-mode');
    if (typeof window._applyRawAudioModePreference === 'function') {
      window._applyRawAudioModePreference(rawAudioMode);
    } else {
      try { localStorage.setItem('flash-raw-audio-mode', rawAudioMode ? 'true' : 'false'); } catch (_) { }
    }
  }
}
...
(async () => {
  ...
  try {
    ...
    window._botName = s.bot_name || 'Flash';
    ...
    const lsTheme = (localStorage.getItem('flash-theme') || '').trim().toLowerCase();
    const lsSkin = (localStorage.getItem('flash-skin') || '').trim().toLowerCase();
    ...
    // skin (registerFlashSkin) whose extension script hasn't registered it yet
    // at this point in boot. Preserve it verbatim instead of normalizing it away
    // to 'default' — the extension's registerFlashSkin() will inject the CSS and
    ...
    _applyTheme(theme);
    localStorage.setItem('flash-skin', skin);
    _applySkin(skin);
    ...
    const fontSize = (s.font_size || localStorage.getItem('flash-font-size') || 'default');
    localStorage.setItem('flash-font-size', fontSize);
    _applyFontSize(fontSize);
    if (typeof setLocale === 'function') {
      const _lang = typeof resolvePreferredLocale === 'function'
        ? resolvePreferredLocale(s.language, localStorage.getItem('flash-lang'))
        : (s.language || localStorage.getItem('flash-lang') || 'en');
      setLocale(_lang);
    ...
    if (typeof _applyTtsEnabled === 'function') _applyTtsEnabled(localStorage.getItem('flash-tts-enabled') === 'true');
  } catch (e) {
    window._sendKey = 'enter';
    ...
    _applyComposerControlOrder(window._composerControlOrder);
    window._botName = 'Flash';
    _bootSettings = { check_for_updates: false };
    if (typeof setLocale === 'function') {
      const _lang = typeof resolvePreferredLocale === 'function'
        ? resolvePreferredLocale(null, localStorage.getItem('flash-lang'))
        : (localStorage.getItem('flash-lang') || 'en');
      setLocale(_lang);
      if (typeof applyLocaleToDOM === 'function') applyLocaleToDOM();
    }
    // Apply voice-mode visibility BEFORE computing the divider so the
  ...
  if (_testUpdates || (_bootSettings.check_for_updates !== false && !sessionStorage.getItem('flash-update-checked') && !sessionStorage.getItem('flash-update-dismissed'))) {
    const _checkUrl = 'api/updates/check' + (_testUpdates ? '?simulate=1' : '');
    api(_checkUrl, { method: _testUpdates ? 'GET' : 'POST', body: _testUpdates ? undefined : JSON.stringify({ force: false }) }).then(d => { if (!_testUpdates) sessionStorage.setItem('flash-update-checked', '1'); if ((d.webui && d.webui.behind > 0) || (d.agent && d.agent.behind > 0)) _showUpdateBanner(d); }).catch(() => { });
  }
  const _bootActiveProfileUnauthRedirectBudget = (() => {
    const markerKey = 'flash-webui-active-profile-bootstrap-401';
    let consumed = false;
  ...
  const _savedLocalBeforeProfileSwitch = localStorage.getItem('flash-webui-session');
  ...
  const _hydrateModelDropdown = ({ redirectIfUnauth = null } = {}) => populateModelDropdown({
    ...
    const savedState = (typeof _readPersistedModelState === 'function')
      ...
      : (localStorage.getItem('flash-webui-model') ? { model: localStorage.getItem('flash-webui-model'), model_provider: null } : null);
    ...
    if (savedModel && $('modelSelect')) {
      ...
      if (!applied && sessionModelState && typeof _ensureModelOptionInDropdown === 'function') {
        ...
        if (typeof _clearPersistedModelState === 'function') _clearPersistedModelState();
          ...
          localStorage.removeItem('flash-webui-model');
          localStorage.removeItem('flash-webui-model-state');
        }
      }
  ...
  const pwaLaunchAction = (window.FlashPWA && typeof window.FlashPWA.launchAction === 'function')
    ? window.FlashPWA.launchAction()
    : null;
  ...
  if (_profileQueryBlocksSavedLocal && _profileSwitchCompleted && _profileSwitchChangedProfile) {
    try {
      if (localStorage.getItem('flash-webui-session') === _savedLocalBeforeProfileSwitch) localStorage.removeItem('flash-webui-session');
    } catch (_) { }
  }
  const savedLocal = localStorage.getItem('flash-webui-session');
  ...
  if (saved) {
    try {
      ...
      if (savedSidebarOnlyState && savedSidebarOnlyState.sidebarOnly) {
        if (savedSidebarOnlyState.archived) {
          try { localStorage.removeItem('flash-webui-session'); } catch (_) { }
        }
      ...
      if (_rootPrefillNeedsFreshComposer(urlSession, savedLocal, prefillIntent)) {
        S.session = null; S.messages = []; S.activeStreamId = null; S.busy = false;
        S._bootReady = true;
        const _ephPanelPref = localStorage.getItem('flash-webui-workspace-panel-pref') === 'open'
          || localStorage.getItem('flash-webui-workspace-panel') === 'open';
      ...
      const panelPref = localStorage.getItem('flash-webui-workspace-panel-pref') === 'open'
        || localStorage.getItem('flash-webui-workspace-panel') === 'open';
    ...
    catch (e) { localStorage.removeItem('flash-webui-session'); }
  }
  ...
  const _freshPanelPref = localStorage.getItem('flash-webui-workspace-panel-pref') === 'open'
    || localStorage.getItem('flash-webui-workspace-panel') === 'open';
  ...
  console.error('[flash] boot failed', e);
...
window.addEventListener('pageshow', async (event) => {
  ...
  if (typeof _isSidebarCollapsed === 'function' && typeof toggleSidebar === 'function') {
    try {
      const _want = localStorage.getItem('flash-webui-sidebar-collapsed') === '1';
...
async function shutdownServer() {
  const ok = await showConfirmDialog({
    title: (typeof t === 'function' ? t('settings_shutdown_confirm_title') : 'Stop Flash WebUI'),
    message: (typeof t === 'function' ? t('settings_shutdown_confirm_message') : 'Stop the Flash WebUI server?'),
    confirmLabel: (typeof t === 'function' ? t('settings_shutdown_confirm_btn') : 'Stop'),
    danger: true,
  });
  if (!ok) return;
  localStorage.setItem('flash-webui-server-stopped', '1');
  try { var bc = new BroadcastChannel('flash-webui-shutdown'); bc.postMessage('stop'); bc.close(); } catch (_) { }
  _showServerStopped();
