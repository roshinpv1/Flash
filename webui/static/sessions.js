...
const NEW_CHAT_DRAFT_SESSION_KEY = 'flash-new-chat-draft-session';
...
const SESSION_VIEWED_COUNTS_KEY = 'flash-session-viewed-counts';
const SESSION_COMPLETION_UNREAD_KEY = 'flash-session-completion-unread';
const SESSION_OBSERVED_STREAMING_KEY = 'flash-session-observed-streaming';
...
const SESSION_PROFILE_COUNTS_KEY = 'flash-session-profile-counts';
...
async function newSession(flash, options = {}) {
  ...
  _newSessionInFlight = (async () => {
    ...
    try { localStorage.setItem('flash-webui-session', S.session.session_id); } catch (_) { }
...
function _setSessionSourceFilter(filter) {
  ...
  try { localStorage.setItem('flash-session-source-filter', next); } catch (_e) { }
  renderSessionListFromCache();
  void renderSessionList({ deferWhileInteracting: false });
}
...
function _restoreSessionSourceFilter() {
  try {
    const raw = localStorage.getItem('flash-session-source-filter');
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'flash-show-all-profiles';
  ...
  deleteBtn.onclick = async () => {
    ...
    try {
      ...
      if (S.session && ids.includes(S.session.session_id)) {
        S.session = null; S.messages = []; S.entries = []; localStorage.removeItem('flash-webui-session');
...
function _showBatchProjectPicker() {
  ...
  none.onclick = async () => {
    picker.remove();
    try {
      await Promise.all(ids.map(sid => api('/api/session/move', { method: 'POST', body: JSON.stringify({ session_id: sid, project_id: null }) })));
  ...
  for (const p of (_allProjects || [])) {
    ...
    item.onclick = async () => {
      picker.remove();
      try {
        await Promise.all(ids.map(sid => api('/api/session/move', { method: 'POST', body: JSON.stringify({ session_id: sid, project_id: p.project_id }) })));
...
async function _archiveSession(session, archived = true, beforeListRender = null) {
  ...
  try {
    ...
    try { if (archived && session.session_id && localStorage.getItem('flash-webui-session') === session.session_id) localStorage.removeItem('flash-webui-session'); } catch (_) { }
...
function ensureActiveSessionExternalRefreshPoll() {
  ...
  if (typeof document !== 'undefined' && !document._flashExternalRefreshVisibilityHook) {
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) void refreshActiveSessionIfExternallyUpdated('visible');
    });
    document._flashExternalRefreshVisibilityHook = true;
  }
  if (typeof window !== 'undefined' && !window._flashExternalRefreshFocusHook) {
    window.addEventListener('focus', () => { void refreshActiveSessionIfExternallyUpdated('focus'); });
    window._flashExternalRefreshFocusHook = true;
  }
}
...
function _installSidebarSseFocusHook() {
  ...
  if (document._flashSidebarSseFocusHook) return;
  document._flashSidebarSseFocusHook = true;
...
function ensureSessionEventsSSE() {
  if (typeof document !== 'undefined' && !document._flashSessionEventsVisibilityHook) {
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        _closeSessionEventsSSE();
      } else {
    ...
    document._flashSessionEventsVisibilityHook = true;
  }
...
function startGatewaySSE() {
  ...
  if (typeof document !== 'undefined' && !document._flashGatewaySSEVisibilityHook) {
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        stopGatewaySSE();
      } else {
    ...
    document._flashGatewaySSEVisibilityHook = true;
  }
  _installSidebarSseFocusHook();
...
function _sessionTitleIsDefaultWebUI(rawTitle) {
  ...
  return title === 'Flash WebUI' || /^Flash WebUI #\d+$/.test(title);
}
...
function renderSessionListFromCache() {
  ...
  let _groupCollapsed = {};
  try { _groupCollapsed = JSON.parse(localStorage.getItem('flash-date-groups-collapsed') || '{}'); } catch (e) { }
  const _saveCollapsed = () => { try { localStorage.setItem('flash-date-groups-collapsed', JSON.stringify(_groupCollapsed)); } catch (e) { } };
...
async function _handleActiveSessionStorageEvent(e) {
  if (!e || e.key !== 'flash-webui-session') return;
...
async function deleteSession(sid, beforeDelete = null) {
  ...
  if (S.session && S.session.session_id === sid) {
    ...
    if (typeof _hydrateTodosFromSession === 'function') _hydrateTodosFromSession(null);
    localStorage.removeItem('flash-webui-session');
