...
function _beginSettingsPanelSession() {
  ...
  _settingsThemeOnOpen = localStorage.getItem('flash-theme') || 'dark';
  _settingsSkinOnOpen = localStorage.getItem('flash-skin') || 'default';
  _settingsFontSizeOnOpen = localStorage.getItem('flash-font-size') || 'default';
...
function _cronGatewayNoticeHtml(status) {
  ...
  const body = notConfigured
    ? 'In Flash WebUI, scheduled jobs require the Flash gateway daemon. If this is a single-container Docker install, jobs can be created and run manually here, but scheduled ticks need a gateway container or `flash gateway` running outside the WebUI.'
    : isStaleMetadata
      ...
      : isRemoteUnreachable
        ...
        : 'In Flash WebUI, scheduled jobs require the Flash gateway daemon to be running. Start the gateway container or `flash gateway` before relying on offline scheduled runs.';
  const docsHref = 'https://github.com/nesquena/flash-webui/blob/master/docs/docker.md#scheduled-jobs-and-the-gateway-daemon';
...
function _cronPanelExpandKey(jobId, suffix) {
  return `flash-webui-cron-${suffix}-expanded-${encodeURIComponent(String(jobId || ''))}`;
}
...
function _cronScriptCardHtml(job) {
  ...
  return `<div class="detail-card cron-script-card">
        ...
        ${workdirRow}
        <div class="detail-hint cron-script-card-hint">${esc(t('cron_script_path_hint') || 'Resolved under ~/.flash/scripts/ unless an absolute path. Edit the script file on the server to change behavior.')}</div>
      </div>`;
}
...
function _renderCronForm({ name, schedule, prompt, deliver, profile, toast_notifications = true, no_agent = false, script = '', model = '', provider = '', isEdit }) {
  ...
  const scriptBlock = isNoAgent ? `
        <div class="detail-form-row">
          ...
          <div class="detail-form-hint">${esc(t('cron_script_path_hint') || 'Resolved under ~/.flash/scripts/ unless an absolute path. Edit the script file on the server to change behavior.')}</div>
        </div>` : '';
...
function _kanbanRenderMarkdown(source) {
  ...
  while (i < lines.length) {
    ...
    if (/^```/.test(trimmed)) {
      ...
      out.push(lang
        ? `<pre class="flash-kanban-code"><code class="language-${_kanbanRenderMarkdownInline(lang)}">${codeHtml}</code></pre>`
        : `<pre class="flash-kanban-code"><code>${codeHtml}</code></pre>`);
      continue;
    }
    ...
    if (taskMatch) {
      ...
      const items = [];
      items.push(`<li class="flash-kanban-task${checked ? ' checked' : ''}"><input type="checkbox"${checked ? ' checked' : ''} disabled> ${_kanbanRenderMarkdownInline(text)}</li>`);
      i++;
      ...
      while (i < lines.length) {
        ...
        if (nextTask) {
          const c = nextTask[1] !== ' ';
          items.push(`<li class="flash-kanban-task${c ? ' checked' : ''}"><input type="checkbox"${c ? ' checked' : ''} disabled> ${_kanbanRenderMarkdownInline(nextTask[2])}</li>`);
          i++;
        } else if (nextLi) {
  ...
  return `<div class="flash-kanban-md">${out.join('\n')}</div>`;
}
...
async function runKanbanDispatcher() {
  ...
  // (one `flash -p <assignee>` per claimed row, up to max=8 per call).
...
//
// The assignee field auto-completes against the union of (a) live Flash
...
async function _kanbanPopulateAssigneeSelect(currentValue) {
  ...
  let html = '';
  if (profiles.length) {
    html += `<optgroup label="${esc(t('kanban_assignee_profiles_label') || 'Flash profiles')}">`;
...
const KANBAN_BOARD_LS_KEY = 'flash-kanban-active-board';
...
function _renderLlmWikiStatus(d) {
  ...
  const rawDocsUrl = status.docs_url || 'https://flash-agent.nousresearch.com/docs/user-guide/skills/bundled/research/research-llm-wiki';
  ...
  const toggleNote = status.toggle_available
    ? 'Toggle available from configured Flash Agent setting.'
...
const PROFILE_DROPDOWN_CACHE_KEY = 'flash-webui-profile-dropdown-cache-v1';
...
async function switchToProfile(name) {
  ...
  try {
    ...
    else localStorage.removeItem('flash-webui-model');
    _skillsData = null;
    _workspaceList = null;
...
let _settingsFlashDefaultModelOnOpen = '';
let _settingsFlashDefaultModelProviderOnOpen = null;
...
const _HIDDEN_TABS_LS_KEY = 'flash-webui-hidden-tabs';
const _TAB_ORDER_LS_KEY = 'flash-webui-tab-order';
const _COMPOSER_CONTROL_ORDER_LS_KEY = 'flash-webui-composer-control-order';
...
function _renderDashboardVisibilityChip(container) {
  ...
  chip.setAttribute('data-tab-panel', '__flash_dashboard__');
...
function _syncFlashPanelSessionActions() {
...
function _appearancePayloadFromUi() {
  ...
  return {
    theme: ($('settingsTheme') || {}).value || localStorage.getItem('flash-theme') || 'dark',
    skin: ($('settingsSkin') || {}).value || localStorage.getItem('flash-skin') || 'default',
    font_size: ($('settingsFontSize') || {}).value || localStorage.getItem('flash-font-size') || 'default',
...
function _rememberAppearanceSaved(payload) {
  if (!payload) return;
  _settingsThemeOnOpen = payload.theme || localStorage.getItem('flash-theme') || 'dark';
  _settingsSkinOnOpen = payload.skin || localStorage.getItem('flash-skin') || 'default';
  _settingsFontSizeOnOpen = payload.font_size || localStorage.getItem('flash-font-size') || 'default';
}
...
async function _autosaveAppearanceSettings(payload) {
  try {
    ...
    if (saved && saved.font_size) {
      localStorage.setItem('flash-font-size', saved.font_size);
    }
    if (saved) {
...
const _SETTINGS_SPEECH_STORAGE_KEYS = {
  tts_enabled: 'flash-tts-enabled',
  tts_auto_read: 'flash-tts-auto-read',
  tts_engine: 'flash-tts-engine',
  tts_voice: 'flash-tts-voice',
  tts_rate: 'flash-tts-rate',
  tts_pitch: 'flash-tts-pitch',
  voice_mode_button: 'flash-voice-mode-button',
  voice_continuous: 'flash-voice-continuous',
  voice_silence_ms: 'flash-voice-silence-ms',
  raw_audio_mode: 'flash-raw-audio-mode',
};
...
function _speechPreferencesPayloadFromUi() {
  ...
  _setOwnedSpeechPayload(payload, 'raw_audio_mode', rawAudioCb ? rawAudioCb.checked : localStorage.getItem('flash-raw-audio-mode') === 'true');
  _setOwnedSpeechPayload(payload, 'voice_continuous', localStorage.getItem('flash-voice-continuous') === 'true');
  const voiceSilence = parseInt(localStorage.getItem('flash-voice-silence-ms'), 10);
...
function _rememberPreferencesSaved(payload) {
  if (!payload) return;
  if (payload.send_key !== undefined) localStorage.setItem('flash-pref-send_key', payload.send_key);
  if (payload.language !== undefined) localStorage.setItem('flash-pref-language', payload.language);
}
...
async function _autosavePreferencesSettings(payload) {
  try {
    ...
    const modelDirty = !!(
      modelSel && (
        (modelState.model || '') !== (_settingsFlashDefaultModelOnOpen || '') ||
        ((modelState.model_provider || null) !== (_settingsFlashDefaultModelProviderOnOpen || null))
      )
    );
...
async function loadSettingsPanel() {
  try {
    ...
    const skinVal = (localStorage.getItem('flash-skin') || settings.skin || 'default').toLowerCase();
    const skinSel = $('settingsSkin');
    if (skinSel) skinSel.value = skinVal;
    if (typeof _buildSkinPicker === 'function') _buildSkinPicker(skinVal);
    const fontSizeVal = settings.font_size || localStorage.getItem('flash-font-size') || 'default';
    localStorage.setItem('flash-font-size', fontSizeVal);
    ...
    // Uses a separate key (flash-webui-workspace-panel-pref) so that
    // closing the panel via toolbar X does not clear the user's preference.
    const wsPanelCb = $('settingsWorkspacePanelOpen');
    if (wsPanelCb) {
      wsPanelCb.checked = localStorage.getItem('flash-webui-workspace-panel-pref') === 'open';
      wsPanelCb.onchange = function () {
        const open = this.checked;
        localStorage.setItem('flash-webui-workspace-panel-pref', open ? 'open' : 'closed');
        // Also sync the runtime key so the current session reflects the change
        localStorage.setItem('flash-webui-workspace-panel', open ? 'open' : 'closed');
    ...
    const resolvedLanguage = (typeof resolvePreferredLocale === 'function')
      ? resolvePreferredLocale(settings.language, localStorage.getItem('flash-lang'))
      : (settings.language || localStorage.getItem('flash-lang') || 'en');
    ...
    if (modelSel) {
      ...
      _settingsFlashDefaultModelOnOpen = (models && models.default_model) || '';
      _settingsFlashDefaultModelProviderOnOpen = (models && models.active_provider) || null;
      // Use the smart matcher so a saved bare form like "anthropic/claude-opus-4.6"
      // (what the CLI's `flash model` command writes) still selects the matching
      ...
      if (typeof _applyModelToDropdown === 'function') {
        _applyModelToDropdown(_settingsFlashDefaultModelOnOpen, modelSel, (models && models.active_provider) || window._activeProvider || null);
      } else {
        modelSel.value = _settingsFlashDefaultModelOnOpen;
      }
    ...
    if (showCliCb) {
      showCliCb.addEventListener('change', function () {
    ...
    const rtlCb = $('settingsRtl');
    if (rtlCb) {
      const saved = !!settings.rtl || localStorage.getItem('flash-rtl') === 'true';
      rtlCb.checked = saved;
      try { localStorage.setItem('flash-rtl', saved ? 'true' : 'false'); } catch (_) { }
      document.documentElement.classList.toggle('chat-content-rtl', saved);
      rtlCb.addEventListener('change', () => {
        const on = rtlCb.checked;
        try { localStorage.setItem('flash-rtl', on ? 'true' : 'false'); } catch (_) { }
    ...
    const rawAudioCb = $('settingsRawAudio');
    if (rawAudioCb) {
      rawAudioCb.checked = _speechBool('raw_audio_mode', 'flash-raw-audio-mode', false);
      rawAudioCb.onchange = function () {
        ...
        else localStorage.setItem('flash-raw-audio-mode', this.checked ? 'true' : 'false');
        _schedulePreferencesAutosave();
      };
    }
    const voiceContinuous = _speechBool('voice_continuous', 'flash-voice-continuous', false);
    _syncSpeechPreferenceCache('voice_continuous', voiceContinuous ? 'true' : 'false');
    const voiceSilence = parseInt(_speechSetting('voice_silence_ms', 'flash-voice-silence-ms', 1800), 10);
    ...
    const ttsEnabledCb = $('settingsTtsEnabled');
    if (ttsEnabledCb) { ttsEnabledCb.checked = _speechBool('tts_enabled', 'flash-tts-enabled', false); ttsEnabledCb.onchange = function () { _markSpeechPreferenceChanged('tts_enabled'); localStorage.setItem('flash-tts-enabled', this.checked ? 'true' : 'false'); _applyTtsEnabled(this.checked); _schedulePreferencesAutosave(); }; }
    const ttsAutoReadCb = $('settingsTtsAutoRead');
    if (ttsAutoReadCb) { ttsAutoReadCb.checked = _speechBool('tts_auto_read', 'flash-tts-auto-read', false); ttsAutoReadCb.onchange = function () { _markSpeechPreferenceChanged('tts_auto_read'); localStorage.setItem('flash-tts-auto-read', this.checked ? 'true' : 'false'); _schedulePreferencesAutosave(); }; }
    ...
    const voiceModeCb = $('settingsVoiceModeEnabled');
    if (voiceModeCb) {
      voiceModeCb.checked = _speechBool('voice_mode_button', 'flash-voice-mode-button', false);
      voiceModeCb.onchange = function () {
        _markSpeechPreferenceChanged('voice_mode_button');
        localStorage.setItem('flash-voice-mode-button', this.checked ? 'true' : 'false');
    ...
    const ttsEngineSel = $('settingsTtsEngine');
    if (ttsEngineSel) {
      // Re-add any extension-registered TTS engines (window.registerFlashTtsEngine)
      ...
      if (typeof window._flashTtsEngineOptions === 'function') {
        window._flashTtsEngineOptions().forEach(function (e) {
      ...
      const saved = String(_speechSetting('tts_engine', 'flash-tts-engine', 'browser') || 'browser');
      ...
      ttsEngineSel.onchange = function () {
        ...
        localStorage.setItem('flash-tts-engine', this.value);
    ...
    window._populateTtsVoices = function () {
      ...
      const engine = localStorage.getItem('flash-tts-engine') || 'browser';
      const current = String(_speechSetting('tts_voice', 'flash-tts-voice', '') || '');
    ...
    if (ttsVoiceSel && 'speechSynthesis' in window) {
      ...
      ttsVoiceSel.onchange = function () { _markSpeechPreferenceChanged('tts_voice'); localStorage.setItem('flash-tts-voice', this.value); _schedulePreferencesAutosave(); };
    }
    // TTS rate/pitch sliders
    const ttsRateSlider = $('settingsTtsRate');
    const ttsRateValue = $('settingsTtsRateValue');
    if (ttsRateSlider) {
      const savedRate = _speechSetting('tts_rate', 'flash-tts-rate', 1);
      ...
      ttsRateSlider.oninput = function () { _markSpeechPreferenceChanged('tts_rate'); if (ttsRateValue) ttsRateValue.textContent = parseFloat(this.value).toFixed(1) + 'x'; localStorage.setItem('flash-tts-rate', this.value); _schedulePreferencesAutosave(); };
    }
    const ttsPitchSlider = $('settingsTtsPitch');
    const ttsPitchValue = $('settingsTtsPitchValue');
    if (ttsPitchSlider) {
      const savedPitch = _speechSetting('tts_pitch', 'flash-tts-pitch', 1);
      ...
      ttsPitchSlider.oninput = function () { _markSpeechPreferenceChanged('tts_pitch'); if (ttsPitchValue) ttsPitchValue.textContent = parseFloat(this.value).toFixed(1); localStorage.setItem('flash-tts-pitch', this.value); _schedulePreferencesAutosave(); };
    }
    const notifCb = $('settingsNotificationsEnabled');
    ...
    const botNameField = $('settingsBotName');
    if (botNameField) {
      botNameField.value = settings.bot_name || 'Flash';
...
function _configureExtensionSettingsFromStatus(data) {
  if (!window.FlashExtensionSettings || !data || !Array.isArray(data.extensions)) return;
  window.FlashExtensionSettings.primeFromStatus({ extensions: data.extensions });
}
...
function _extensionSettingsControls(entry) {
  ...
  const settingsApi = window.FlashExtensionSettings && id ? window.FlashExtensionSettings.settingsForExtension(id) : null;
...
function _fillExtensionSettingsForm(row, id) {
  if (!window.FlashExtensionSettings) return;
  const values = window.FlashExtensionSettings.settingsForExtension(id).values;
  row.querySelectorAll('[data-extension-setting-input]').forEach(input => {
    const key = input.dataset.extensionSettingInput || '';
    const type = input.dataset.extensionSettingType || '';
...
function handleExtensionSettingsSave(btn) {
  ...
  if (!id || !row || !window.FlashExtensionSettings) return;
  const api = window.FlashExtensionSettings.settingsForExtension(id);
...
function handleExtensionSettingsReset(btn) {
  ...
  const row = btn && btn.closest('[data-extension-id]');
  if (!id || !row || !window.FlashExtensionSettings) return;
  window.FlashExtensionSettings.settingsForExtension(id).reset();
  _fillExtensionSettingsForm(row, id);
...
function handleExtensionStorageClear(btn) {
  ...
  if (!id || !window.FlashExtensionSettings) return;
  window.FlashExtensionSettings.storageForExtension(id).clear();
...
function _extensionRegistrySourceUrl(entryPath) {
  ...
  return 'https://github.com/flash-webui/flash-webui-extensions/tree/main/' + folder.map(encodeURIComponent).join('/');
}
...
async function handleExtensionInstall(btn, entry) {
  ...
  try {
    const result = await api('/api/extensions/install', {
      method: 'POST', body: JSON.stringify({
        id: entry.id,
...
async function _loadPluginPage(path, label) {
  ...
  // Security note: plugins are locally-installed (~/.flash/plugins/), similar
...
function _providerQuotaPoolShouldDefaultOpen(pool) {
  try {
    const saved = localStorage.getItem('flash-provider-quota-pool-open');
...
function _buildProviderQuotaCard(status) {
  ...
  if (poolDetails) {
    poolDetails.addEventListener('toggle', () => {
      try { localStorage.setItem('flash-provider-quota-pool-open', poolDetails.open ? '1' : '0'); } catch (e) { }
    });
  }
  return card;
}
...
function _buildProviderCard(p) {
  ...
  // key_source can be 'oauth' (flash auth), 'config_yaml' (token in config.yaml), or 'none'.
  ...
  if (isOauth) {
    ...
    if (p.key_source === 'config_yaml') {
      hint.textContent = t('providers_oauth_config_yaml_hint') || 'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run flash auth.';
      ...
      hint.textContent = t('providers_oauth_not_configured_hint') || 'Not authenticated. Run flash auth in the terminal to configure this provider.';
  ...
  if (p.configurable) {
    ...
    hint.textContent = p.is_custom
      ? 'Custom provider loaded from config.yaml / flash model. Edit it from the CLI or config file.'
...
async function registerPasskey() {
  ...
  try {
    ...
    await api('/api/auth/passkey/register', {
      method: 'POST', body: JSON.stringify({
...
function _applySavedSettingsUi(saved, body, opts) {
  ...
  window._botName = body.bot_name || 'Flash';
  if (typeof applyBotName === 'function') applyBotName();
  ...
  _settingsFontSizeOnOpen = fontSize || localStorage.getItem('flash-font-size') || 'default';
  const bar = $('settingsUnsavedBar');
  if (bar) bar.style.display = 'none';
  _settingsFlashDefaultModelOnOpen = body.default_model || _settingsFlashDefaultModelOnOpen || '';
  if (Object.prototype.hasOwnProperty.call(body, 'default_model_provider')) _settingsFlashDefaultModelProviderOnOpen = body.default_model_provider || null;
...
function _openAuxAdvancedOptions(taskCfg, cfg) {
  ...
  if (body) {
    ...
    const timingFields = isMain ? '' : (
      _auxAdvancedInputHtml('auxAdvancedTimeout', t('settings_aux_advanced_timeout') || 'Timeout seconds', _auxAdvancedValue(cfg, 'timeout'), t('settings_aux_advanced_timeout_desc') || 'Request timeout for this auxiliary task. Blank uses Flash default.', 'number', 'inputmode="numeric" min="1" step="1"') +
...
async function saveSettings(andClose) {
  ...
  const modelChanged = (model || '') !== (_settingsFlashDefaultModelOnOpen || '') || ((modelState.model_provider || null) !== (_settingsFlashDefaultModelProviderOnOpen || null));
  ...
  const fontSize = ($('settingsFontSize') || {}).value || localStorage.getItem('flash-font-size') || 'default';
...
window.addEventListener('flash:cron_created', () => {
