...
const DESKTOP_COMPANION_SETUP_GUIDE_URL = 'https://github.com/franksong2702/flash-webui-desktop-companion#after-gallery-install';
...
async function handlePetSlashCommand(rawCommandText, meta) {
  ...
  const hook = typeof window !== 'undefined' && window.__flashHandlePetSlashCommand;
  if (typeof hook === 'function') {
    try {
      ...
      if (typeof console !== 'undefined' && console.error) {
        console.error('[flash] Desktop Companion /pet hook error:', _e);
      }
...
function cliOnlyCommandResponse(cmdName, meta) {
  ...
  let extra = '';
  if (name === 'browser') {
    extra = '\n\nBrowser tools in WebUI must be configured server-side with the agent/browser environment. Once configured, ask the model to use browser tools directly; `/browser` itself only works in `flash chat`.';
  }
  return `\`/${name}\` is a Flash CLI-only command and cannot run inside the WebUI.${detail}${extra}`;
}
...
async function cmdTheme(args) {
  ...
  if (themes.includes(val) || legacyThemes.includes(val)) {
    const appearance = _normalizeAppearance(
      val,
      legacyThemes.includes(val) ? null : localStorage.getItem('flash-skin')
    );
  ...
  if (skins.includes(val)) {
    const appearance = _normalizeAppearance(localStorage.getItem('flash-theme'), val);
    localStorage.setItem('flash-theme', appearance.theme);
    localStorage.setItem('flash-skin', appearance.skin);
    _applyTheme(appearance.theme);
    _applySkin(appearance.skin);
 ...
 * or doesn't support steer (older flash-agent versions). The failed steer text
...
function cmdReasoning(args) {
  ...
  const BRAIN = '\uD83E\uDDE0';
  // Matches flash_constants.VALID_REASONING_EFFORTS + 'none' (CLI parity).
  // Keep this WebUI effort list in sync with flash-agent#29248.
