    ...
    try {
      ...
      const requestPromise = (async () => {
        ...
        if (!res.ok) {
          ...
          // and for subpath mounts like /flash/, where /login escapes to the site root.
          if (res.status === 401) {
...
function _wsExpandKey() {
  ...
  return ws ? 'flash-webui-expanded:' + ws : null;
}
...
function _workspaceRouteForPath(path, kind, opts = {}) {
  ...
  // mount like /flash/. A bare "/api/…" string resolves to the server root
...
// Canonical Flash mutators plus MCP filesystem aliases that can create/edit files.
...
async function toggleEditMode() {
  ...
  if (editing) {
    ...
    try {
      const saved = await api(_previewSaveRoute || '/api/file/save', {
