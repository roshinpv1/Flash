...
// Handles iOS PWA standalone mode and keeps subpath mounts like /flash/ from
...
function _renderUserFencedBlocks(text) {
  ...
  const stashSelectedContextBlocks = (value) => {
    ...
    const marker = '<!-- flash-selected-context -->';
    const out = [];
...
async function startCompressionRecovery(btn) {
  ...
  try {
    ...
    try { localStorage.setItem('flash-webui-session', sid); } catch (_) { }
...
function openFlashDashboard(event) {
...
const MEDIA_PLAYBACK_STORAGE_KEY = 'flash-media-playback-rate';
...
const MODEL_STATE_KEY = 'flash-webui-model-state';
const PENDING_SESSION_MODEL_PREFIX = 'flash-webui-pending-session-model:';
...
function _readPersistedModelState() {
  ...
  const legacy = localStorage.getItem('flash-webui-model');
...
function _writePersistedModelState(model, modelProvider) {
  ...
  localStorage.setItem('flash-webui-model', value);
  try {
...
async function _fetchLiveModels(provider, sel, requestSeq = null) {
  ...
  try {
    ...
    if (added > 0) {
      if (typeof syncModelChip === 'function') syncModelChip();
      console.debug('[flash] Live models loaded for', provider + ':', added, 'new models added');
    }
  } catch (e) {
    console.debug('[flash] Live model fetch failed for', provider, e.message);
  } finally {
 ...
 * currently configured in Flash. Returns a warning string if mismatched,
...
function _checkProviderMismatch(modelId) {
  ...
  if (norm(modelProvider) !== norm(ap)) {
    return (window.t ? window.t('provider_mismatch_warning', modelId, ap) :
      `"${modelId}" may not work with your configured provider (${ap}). Send anyway or run \`flash model\` to switch.`);
  }
  return null;
}
...
function speakMessage(btn) {
  ...
  // Extension-registered TTS engine (window.registerFlashTtsEngine). Synthesize
...
function autoReadLastAssistant() {
  ...
  const pref = localStorage.getItem('flash-tts-auto-read');
  if (pref !== 'true') return;
  ...
  // Extension-registered TTS engine (window.registerFlashTtsEngine): synth via
...
const INFLIGHT_KEY = 'flash-webui-inflight'; // localStorage key for in-flight session tracking
const INFLIGHT_STATE_KEY = 'flash-webui-inflight-state'; // localStorage snapshots for mid-stream reload recovery
...
// ── Flash agent/gateway heartbeat alert (#716) ──
...
function _showAgentHealthAlert(payload) {
  ...
  if (!banner) return;
  if (title) title.textContent = 'Flash agent is not responding';
...
function _formatManualUpdateInstruction(info) {
  ...
  return t('settings_update_manual_docker', 'docker pull ghcr.io/nesquena/flash-webui:latest');
}
...
const WHATS_NEW_SUMMARY_STORAGE_KEY = 'flash-whats-new-generated-summaries';
...
function dismissUpdate() {
  ...
  sessionStorage.setItem('flash-update-dismissed', '1');
}
...
async function applyUpdates() {
  ...
  try {
    ...
    sessionStorage.removeItem('flash-update-checked');
    sessionStorage.removeItem('flash-update-dismissed');
...
function syncTopbar() {
  ...
  // active profile (the flash_profile cookie, set only by /api/profile/switch).
...
function _setLatestAssistantTurnLandmark(turn, isLatest) {
  if (!turn) return;
  const label = 'Latest Flash response';
  if (isLatest) {
    if (typeof document !== 'undefined') {
...
function _syncTransparentEventControls(turn) {
  ...
  // Wire the Flash chat name tag toggle for the live turn.
...
// ── Transparent turn-level collapse (Flash chat name tag) ───────────────
...
const _activityDisclosureStoragePrefix = 'flash-activity-disclosure:';
...
function _projectLiveAnchorActivitySceneForStream(streamId, mode) {
  const api = (typeof window !== 'undefined') ? window.FlashAssistantTurnAnchors : null;
...
function _startLiveRunStatusTimer(sid, startedAt) {
  ...
  _liveRunStatusTimers[sid] = {
    startedAt, interval: setInterval(() => {
      const el = $('liveRunStatus');
...
function renderMessages(options) {
  ...
  if (!S.busy && (hasMessageToolMetadata || !S.toolCalls || !S.toolCalls.length)) {
    ...
    S.messages.forEach((m, rawIdx) => {
      if (!m) return;
      // OpenAI / Flash CLI format: role=tool with tool_call_id
      if (m.role === 'tool') {
  ...
  // Transparent mode per-turn wiring: collapsible Flash chat name tag, old-event
  ...
  row.innerHTML = `
    <div class="${cardClass}">
      ...
      ${hasDetail ? `<div class="tool-card-detail">
        ${detailLead}
        ${visibleArgs.length ? `<div class="tool-card-args">${visibleArgs.map(([k, v]) => {
    let sv = String(v);
    if (typeof _redactToolTargetLabel === 'function') { try { sv = _redactToolTargetLabel(sv); } catch (e) { } }
...
async function submitEdit(msgIdx, newText) {
  ...
  try {
    await api('/api/session/truncate', {
    ...
    if (!_mermaidLoading) {
      ...
      script.onload = () => {
        if (typeof mermaid !== 'undefined') {
          mermaid.initialize({
            startOnLoad: false, theme: document.documentElement.classList.contains('dark') ? 'dark' : 'default', themeVariables: {
...
function toggleWorkspaceHiddenFiles(value) {
  ...
  try { localStorage.setItem('flash-workspace-show-hidden-files', S.showHiddenWorkspaceFiles ? '1' : '0'); } catch (_) { }
  _syncWorkspaceHiddenToggle();
  renderFileTree();
}
try { S.showHiddenWorkspaceFiles = localStorage.getItem('flash-workspace-show-hidden-files') === '1'; } catch (_) { }
...
async function _performWorkspaceMove(srcPath, destDir, isDir) {
  ...
  try {
    const data = await api('/api/file/move', {
      method: 'POST', body: JSON.stringify({
