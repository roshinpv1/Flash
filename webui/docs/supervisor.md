# Running Flash Web UI under a process supervisor
...
**Important (launchd on macOS):** if the ``com.parantoux.flash-webui`` LaunchAgent is enabled, treat launchd as the single source of truth for WebUI lifecycle. Do **not** also run ``./ctl.sh start``, ``bash start.sh``, ``python bootstrap.py``, or ``python server.py`` against the same state dir/port, or you can create a second WebUI instance and trigger port-8787 restart churn.
...
``~/Library/LaunchAgents/com.example.flash-webui.plist``:
...
<dict>
    <key>Label</key>
    <string>com.example.flash-webui</string>

    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>/Users/yourname/flash-webui/start.sh</string>
        <string>--foreground</string>
    </array>

    <key>WorkingDirectory</key>
    <string>/Users/yourname/flash-webui</string>
    ...
    <string>/Users/yourname/.flash/webui/launchd-stdout.log</string>

    <key>StandardErrorPath</key>
    <string>/Users/yourname/.flash/webui/launchd-stderr.log</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.example.flash-webui.plist
launchctl print gui/$(id -u)/com.example.flash-webui   # check state
```

Reload after editing the plist:

```bash
launchctl unload ~/Library/LaunchAgents/com.example.flash-webui.plist
launchctl load   ~/Library/LaunchAgents/com.example.flash-webui.plist
```
...
``~/.config/systemd/user/flash-webui.service``:
...
[Service]
Type=simple
WorkingDirectory=%h/flash-webui
ExecStart=/bin/bash %h/flash-webui/start.sh --foreground
Restart=on-failure
RestartSec=5
...
systemctl --user enable --now flash-webui.service
journalctl --user -u flash-webui.service -f
```
...
``/etc/supervisor/conf.d/flash-webui.conf``:

```ini
[program:flash-webui]
command=/bin/bash /home/youruser/flash-webui/start.sh --foreground
directory=/home/youruser/flash-webui
user=youruser
autostart=true
autorestart=true
stopsignal=TERM
stopwaitsecs=10
stdout_logfile=/var/log/flash-webui.out.log
stderr_logfile=/var/log/flash-webui.err.log
...
sudo supervisorctl status flash-webui
```
...
Flash Web UI exposes two health levels:
...
- ``/health?deep=1`` — readiness probe that briefly acquires the stream lock,
  reads the sidebar/session path, reads projects state, and touches Flash
...
``systemctl --user restart flash-webui.service`` on failure.
