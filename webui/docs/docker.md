# Flash WebUI — Docker setup guide
...
| **All-in-one image** (community fork — third-party, not maintained by us) | Podman 3.4 / multi-arch / supervisord-style preference. | [sunnysktsang/flash-suite](https://github.com/sunnysktsang/flash-suite) — see [#1399](https://github.com/nesquena/flash-webui/issues/1399) for the original discussion |

> **Note (v0.14+):** If you use `docker-compose.three-container.yml`, both
> `flash-agent` and `flash-dashboard` initialise from the same image and write
> to the same `flash-home` volume simultaneously. This can cause overlapping lock
...
> avoids this by running a single `flash-agent` process that serves both the
...
Flash WebUI assumes one operator controls the container, mounted Flash home, and workspace.
The image does **not** install `sudo`, does not add runtime users to a sudo group, and does not
grant `NOPASSWD` escalation. If an agent/tool process gains a shell as `flashwebui`, it should
...
UID/GID alignment and ownership preparation before the app can read `~/.flash`, `/workspace`,
`/app`, and `/uv_cache`. After that setup, `docker_init.bash` re-execs itself as the unprivileged
`flashwebui` user and starts the server there. Init scratch files under `/tmp/flashwebui_init`
...
That's it for a real personal Docker install. Your existing `~/.flash`
...
them manually from the Tasks panel. In Docker, scheduled jobs require the Flash gateway daemon
to tick while you are away. If System Settings shows `Gateway not configured`,
use `docker-compose.two-container.yml`,
`docker-compose.three-container.yml`, or run `flash gateway` separately before
...
your real `~/.flash` unless you intentionally want to test real state. Use an
isolated Flash home and follow
[`docs/onboarding-agent-checklist.md`](onboarding-agent-checklist.md) instead.

> **Linux note**: run Compose as the user who owns the Flash home. The command
> `sudo docker compose up -d` can make Compose expand `${HOME}` as `/root`, so
> the default `${HOME}/.flash` bind mount becomes `/root/.flash` instead of
> your user's real Flash directory. Prefer adding your user to the `docker group`
...
The default Flash WebUI Docker image stays CPU-only. GPU user-space packages
are installed only when you build a custom image with the opt-in build arg:

```bash
docker build --build-arg INSTALL_GPU_LIBS=1 -t flash-webui:gpu .
```
...
`flashwebui` runtime user, so the WebUI process keeps access to `/dev/dri`.
...
```bash
docker run --rm --gpus all flash-webui:gpu nvidia-smi
```
...
and container toolkit setup before debugging Flash WebUI. The container image
...
The three-container layout adds the dashboard but is otherwise the same shape. If you must stay single-container, you can run `flash gateway` inside the container as a long-lived background process, but the compose split is sturdier.
...
services:
  flash-webui:
    environment:
      - HERMES_API_URL=http://flash-agent:8642
      # HERMES_WEBUI_GATEWAY_BASE_URL=http://flash-agent:8642 also works.
```
...
services:
  flash-webui:
    environment:
      ...
      - HERMES_WEBUI_GATEWAY_BASE_URL=http://flash-agent:8642
...
```bash
export GATEWAY_BASE_URL="${HERMES_API_URL:-${HERMES_WEBUI_GATEWAY_BASE_URL:-http://flash-agent:8642}}"
docker compose -f docker-compose.two-container.yml exec flash-agent flash gateway status
...
Since v0.14, `flash-agent` can serve the gateway API and the built-in dashboard
...
single writer to `flash-home`, eliminating the concurrent-init write conflicts
that occur when `flash-agent` and `flash-dashboard` both start from the same
...
| Service | Image | Ports |
|---|---|---|
| `flash-agent` | `nousresearch/flash-agent:latest` | 8642 (gateway), 9119 (dashboard) |
| `flash-webui` | `ghcr.io/nesquena/flash-webui:latest` | 8787 (chat UI) |
...
services:
  flash-agent:
    image: nousresearch/flash-agent:latest
    container_name: flash-agent
    command: gateway run
    ports:
      - "127.0.0.1:8642:8642"
      - "127.0.0.1:9119:9119"
    volumes:
      - flash-home:/home/flash/.flash
      - flash-agent-src:/opt/flash
    environment:
      - HERMES_HOME=/home/flash/.flash
      - HERMES_UID=${UID:-1000}
      - HERMES_GID=${GID:-1000}
  ...
  flash-webui:
    image: ghcr.io/nesquena/flash-webui:latest
    container_name: flash-webui
    depends_on:
      - flash-agent
    ports:
      - "127.0.0.1:8787:8787"
    volumes:
      - flash-home:/home/flashwebui/.flash
      - flash-agent-src:/home/flashwebui/.flash/flash-agent:ro
      - ${HERMES_WORKSPACE:-${HOME}/workspace}:/workspace
    environment:
      - HERMES_WEBUI_HOST=0.0.0.0
      - HERMES_WEBUI_PORT=8787
      - HERMES_WEBUI_STATE_DIR=/home/flashwebui/.flash/webui
      - WANTED_UID=${UID:-1000}
      - WANTED_GID=${GID:-1000}
    restart: unless-stopped
    networks:
...
Check `flash gateway run --help` for the exact flag names for your agent release —
...
`flash-dashboard` to `flash-agent` with a `condition: service_healthy` healthcheck
...
Until the compatibility boundary work in [#1925](https://github.com/nesquena/flash-webui/issues/1925) and [#2491](https://github.com/nesquena/flash-webui/issues/2491) land, the WebUI and Flash Agent deployment should be treated as a release pair: the WebUI release is tested against its matching agent release and should be upgraded/pinned together.

If you use `latest`, use it consistently on both sides and avoid mixing a fixed tag with `latest`:
- fixed WebUI tag + `flash-agent:latest`
- `flash-webui:latest` + fixed `flash-agent` tag
...
If you see behavior issues after a mixed-version upgrade, capture both WebUI and flash-agent versions and the compose layout in the issue.

### 1. "Permission denied" at startup

**Symptom**: Container starts but immediately crashes, logs show:
```
PermissionError: [Errno 13] Permission denied: '/home/flashwebui/.flash/...'
```
...
```
!! WARNING: flash-agent source not found.
!!   Looked in: /home/flashwebui/.flash/flash-agent
!!              /opt/flash
```

**Cause**: The agent's source (`/opt/flash` inside the agent container) needs to be exposed to the WebUI container via a shared volume. The two-container compose file does this via `flash-agent-src` named volume, but if you're using bind mounts incorrectly the path won't resolve.

**Fix**: Use the named volumes that ship with `docker-compose.two-container.yml` — don't replace them with bind mounts unless you know what you're doing. The agent container writes its source to `/opt/flash`, and the WebUI mounts that volume at `/home/flashwebui/.flash/flash-agent`.

If you must use a bind mount: pick a host path, then mount it to `/opt/flash` in the agent container AND `/home/flashwebui/.flash/flash-agent` in the WebUI container.
...
- **Combined image** ([sunnysktsang/flash-suite](https://github.com/sunnysktsang/flash-suite)) — community fork that ships agent+webui+dashboard in one container

### 6. "config.yaml not loaded"

**Symptom**: You have a `config.yaml` in your host `~/.flash/`, but the WebUI shows "no model configured" or doesn't pick up your custom providers.

**Cause**: Either the file isn't readable (UID/GID issue, see #1) or it's not in the expected path inside the container.

**Fix**:
- Verify: `docker exec flash-webui ls -la /home/flashwebui/.flash/config.yaml`
...
### 7. "On Podman: can't share .flash between containers"
...
**Fix**: Either upgrade to Podman 4+ (which fixes this), or use the [single-container setup](#5-minute-quickstart-single-container), or use the [community all-in-one image](https://github.com/sunnysktsang/flash-suite).

### 8. "API base URL set to localhost fails from Docker" (#3012)

**Symptom**: A provider, local model server, webhook, or custom API works on the host at `http://localhost:<port>`, but fails when the same URL is configured in Flash WebUI running in Docker.
...
The WebUI container doesn't ship with the agent's Python deps — at startup it runs `uv pip install /home/flashwebui/.flash/flash-agent` to install them from the shared volume. The WebUI mount is read-only; the agent container is the only writer.

## Upgrading the agent container

The `flash-agent-src` named volume is initialised from the agent image's `/opt/flash` on first `up`. Docker reuses the volume verbatim on every subsequent `up` — **even after `docker pull` of a newer agent image**. The cached volume content masks the new image's source tree, so a fresh `docker pull` of `nousresearch/flash-agent:latest` does not by itself give you the new agent code, dependencies, or entrypoint.

This is the root cause of [#1416](https://github.com/nesquena/flash-webui/issues/1416): the symptom looked like a missing entrypoint, but the entrypoint was actually present in the new image and hidden behind the stale named volume.
...
docker volume rm <project>_flash-agent-src
...
Replace `<project>` with your Compose project name (the parent directory by default; check with `docker volume ls`). The `flash-home` volume (config, sessions, state) is left untouched — only `flash-agent-src` (the agent's installed Python source) is recreated.

> The single-container setup (`docker-compose.yml`) does not use `flash-agent-src` and is not affected by this upgrade pattern — pulling a newer WebUI image and `docker compose up -d --force-recreate` is sufficient.
...
- The gateway API (port 8642) is bound by the agent service only; the WebUI cannot bind it. Other containers reach the gateway via the `flash-net` Docker network.
...
- **Filesystem boundary.** Both services share `flash-home` (config, sessions, state), and the WebUI mounts the agent's installed source from `flash-agent-src`. The WebUI mount is read-only (since v0.51.84), but the agent service still has write access, and both services share the home volume.
...
- **Trust boundary on the agent source.** The WebUI installs Python dependencies from the shared `flash-agent-src` volume at startup. The read-only mount means a compromised WebUI cannot rewrite the agent source, but it does run code from that volume.
...
The direct source mount is a compatibility bridge, not the long-term API contract. The current source/API boundary inventory and decoupling task list live in [`docs/rfcs/agent-source-boundary.md`](rfcs/agent-source-boundary.md) for [#2453](https://github.com/nesquena/flash-webui/issues/2453). If you customize the compose files with bind mounts, keep the WebUI-side agent source mount read-only unless you are intentionally doing local development; `docker_init.bash` warns at startup when that path is writable.

## Bind-mount migration (advanced)

If you really need to bind-mount an existing host `~/.flash` (e.g. you're keeping config in dotfiles, sharing with a non-Docker `flash` install, etc.):
...
volumes:
  flash-home:
    ...
    driver_opts:
      type: none
      o: bind
      device: /home/youruser/.flash
  flash-agent-src:
    driver: local
    driver_opts:
      type: none
      o: bind
      device: /opt/flash-agent-source
```

**Critical requirements**:

1. The host directory MUST be readable by your container UID. Run `id -u` on the host and ensure `~/.flash` is owned by that UID (or readable via group bits).
2. ALL containers sharing the volume must run as the SAME UID/GID. Set `UID=$(id -u)` and `GID=$(id -g)` in `.env`.
3. If you run Compose with sudo, do not rely on `${HOME}` defaults: `sudo` often changes `$HOME` to `/root`, so `${HERMES_HOME:-${HOME}/.flash}` becomes `/root/.flash`. Prefer running Docker as your user; otherwise pass absolute paths with `sudo -E`, for example `HERMES_HOME=/home/youruser/.flash HERMES_WORKSPACE=/home/youruser/workspace sudo -E docker compose up -d`, and confirm the rendered bind mount with `docker compose config`.
...
- #1416 — agent-image upgrade requires removing `flash-agent-src` named volume (see [Upgrading the agent container](#upgrading-the-agent-container))
...
- #3006 — `sudo docker compose` can mount `/root/.flash` instead of the user's Flash home
- #3243 — optional GPU runtime image/docs for containerized acceleration workloads
- #858 — two-container `/opt/flash` path confusion
...
If you hit a new failure mode not covered here, please [open an issue](https://github.com/nesquena/flash-webui/issues/new) with:

1. Which compose file you used
2. The error from `docker logs flash-webui`
3. `docker exec flash-webui id` output
4. `docker exec flash-webui ls -la /home/flashwebui/.flash` output
