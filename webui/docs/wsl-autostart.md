...
Flash WebUI runs well under WSL2, but native Windows login does not automatically start Linux user processes. This guide covers two supported options:
...
```text
scripts/wsl/flash_webui_autostart.sh
```
...
| `HERMES_WEBUI_LOG_DIR` | `$HOME/.flash/webui/logs` | Autostart and WebUI logs |
...
| `HERMES_WEBUI_PID_FILE` | `$HERMES_WEBUI_LOG_DIR/flash-webui.pid` | pid file used for duplicate prevention |
| `HERMES_WEBUI_REQUIRE_AGENT_PROCESS` | `0` | Optional: set to `1` only if your local setup requires a separate Flash process before WebUI starts |

Make the script executable once inside WSL:

```bash
cd /path/to/flash-webui
chmod +x scripts/wsl/flash_webui_autostart.sh
```
...
```text
$HOME/.flash/webui/logs/webui_autostart.log
$HOME/.flash/webui/logs/flash_webui.log
```
...
```bash
if [ -x "$HOME/flash-webui/scripts/wsl/flash_webui_autostart.sh" ]; then
  HERMES_WEBUI_REPO="$HOME/flash-webui" \
    "$HOME/flash-webui/scripts/wsl/flash_webui_autostart.sh" >/dev/null 2>&1 &
fi
```
...
.\scripts\windows\setup_webui_autostart.ps1 `
  -WslScriptPath "/home/your-user/flash-webui/scripts/wsl/flash_webui_autostart.sh" `
  -Distro "Ubuntu"
```
...
- The default task name is `FlashWebUIAutoStart`; pass `-TaskName` if you need a different name.
...
```powershell
Get-ScheduledTask -TaskName FlashWebUIAutoStart
Unregister-ScheduledTask -TaskName FlashWebUIAutoStart -Confirm:$false
```
...
```bash
tail -n 80 "$HOME/.flash/webui/logs/webui_autostart.log"
tail -n 80 "$HOME/.flash/webui/logs/flash_webui.log"
```
...
| Health check fails but pid exists | WebUI is still booting or the port differs | Check `HERMES_WEBUI_PORT` and `flash_webui.log` |
