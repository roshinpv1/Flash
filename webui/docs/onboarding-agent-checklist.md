...
debug Flash WebUI onboarding. It does not replace the human first-run wizard.
...
- approving any cleanup of a real Flash home
...
- Do not delete, move, or overwrite the real `~/.flash` directory unless the
...
non-secret keys needed to understand the active Flash home, WebUI state
directory, port, or host.

## Isolated local trial

Use an isolated Flash home and WebUI state directory for a reinstall or support
...
```text
~/flash-onboarding-test/webui/bootstrap-8789.log
```
...
find ~/flash-onboarding-test -maxdepth 3 -type f | sort
tail -n 120 ~/flash-onboarding-test/webui/bootstrap-8789.log
```
...
- `system.flash_found` and `system.imports_ok` match the expected bootstrap
  state.
...
mean the wizard correctly points them to `flash model` or `flash auth` rather
...
- confirm the bootstrap found or installed Flash Agent
