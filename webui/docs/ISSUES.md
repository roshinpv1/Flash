...
### Root Cause
The check lives in **flash-agent** (upstream), not flash-webui:

```
flash-agent/tools/browser_tool.py → check_browser_requirements()
```
...
### Fix required
The fix must happen in `flash-agent/tools/browser_tool.py`:
