# Flash Run Adapter Contract and Migration Gates
...
- **Created:** 2026-05-11
- **Revised:** 2026-05-23
- **Tracking issue:** [#1925](https://github.com/nesquena/flash-webui/issues/1925)
...
- future Flash runtime APIs cannot be adopted cleanly because WebUI lacks a
...
- Identify future backend mapping: existing Flash runtime API, missing Flash
...
- Do not depend on Flash Agent shipping a WebUI-specific runtime connector before
...
future WebUI-managed runner, or a future Flash `/v1/runs` backend.
...
| queue / continue | append follow-up work according to Flash semantics | runner/runtime control plane |
...
|---|---|---|---|---|---|
| `STREAMS` / `STREAMS_LOCK` | `api.state_sync` process memory | adapter API surface + presentation fan-out | WebUI runner or future Flash run observation API | keep live path; mirror events into journal | Must stop being authoritative for active run existence. |
...
| cached `AIAgent` objects / `AGENT_INSTANCES` | `api/config.py` process memory | runner process | runner-owned Flash integration | unchanged | Moving this is deferred until after journal proof. |
...
| command capability metadata | WebUI command registry + Flash command assumptions | adapter API surface | runtime command/capability metadata | unchanged | Unknown command support should not be guessed by WebUI. |
...
| Multi-profile isolation | profile-specific runs write/read the correct Flash home and memory | protects #2134-family isolation concerns | runner/profile slice |
| Queue/continue | follow-up input during live/resumable work obeys Flash semantics | prevents parallel continuation model | control migration slice |
...
- future Flash CLI/Python/local API or `/v1/runs` backends can be evaluated
  behind the adapter.
...
4. **Restart/reattach success criterion.** Start a long-running run, restart only
   `flash-webui.service`, reload the session, rediscover the active or terminal
...
- no dependency on Flash Agent shipping `/v1/runs` before WebUI can validate the
...
- carry the runtime API gap matrix forward: missing Flash-owned capabilities
...
- no permanent WebUI-owned active-run discovery cache when the missing capability
  belongs in a runner or future Flash Runtime API;
...
Flash runtime families such as `token.delta`, `tool.started`, and `done` before
...
- no permanent WebUI-owned active-run discovery cache that duplicates runner or
  future Flash Runtime API responsibility.
...
1. **Process ownership moved.** A local runner process, not `flash-webui`, owns
...
2. **Restart/reattach with a real runner.** Start a non-trivial `runner-local`
   run, restart only `flash-webui`, reload the session, rediscover the active or
...
- no server-side queue scheduler just for adapter symmetry;
- no broad WebUI product-surface migration;
- no claim that this is the canonical Flash Agent Runtime API; if Flash Agent
...
- If Flash Agent later ships a durable `/v1/runs` API, which adapter fields map
