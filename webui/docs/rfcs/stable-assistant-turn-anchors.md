...
- **Created:** 2026-06-10
- **Tracking issue:** [#3926](https://github.com/nesquena/flash-webui/issues/3926)
- **Parent contract:** [Live-to-Final Assistant Replies](./live-to-final-assistant-replies.md) ([#3400](https://github.com/nesquena/flash-webui/issues/3400))
- **Related RFCs:** [Transparent Stream](./transparent-stream-activity-mode.md), [Flash Run Adapter Contract](./flash-run-adapter-contract.md), [WebUI Run State Consistency Contract](./webui-run-state-consistency-contract.md), [Turn Journal](./turn-journal.md), [Pending Intent Controls](./webui-pending-intent-controls.md)
...
- Insulate the presentation layer from the in-progress RuntimeAdapter substrate
  ...
  Flash runtime changes the anchor's input source, not the UI.

## Non-goals

- Do not replace the Live-to-Final product model from #3400.
- Do not implement Transparent Stream in this RFC.
- Do not implement a RuntimeAdapter, runner process, sidecar, or Flash runtime
  API.
...
`flash-run-adapter-contract.md` draws the runtime ownership boundary:
...
emits now and a runner or Flash runtime will emit later. `run_id` is the durable
...
`flash-run-adapter-contract.md`) as its identity source, not a new scheme. That
envelope — `event_id = "run_id:seq"`, monotonic `seq`, `run_id` — is already
emitted by the WebUI run journal today and will be emitted by a runner or Flash
...
from the WebUI run path to a runner or Flash runtime changes the anchor's input
