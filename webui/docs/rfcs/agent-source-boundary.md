...
- **Status:** Proposed
- **Created:** 2026-05-17
- **Tracking issue:** [#2453](https://github.com/nesquena/flash-webui/issues/2453)

## Problem

The WebUI currently depends on Flash Agent Python source being importable at
runtime. In local installs this usually means a neighboring checkout; in the
multi-container Docker setup it means the WebUI reads the `flash-agent-src`
...
Flash Agent internal module layout and makes the multi-container setup look more
isolated than it really is.

## Current safety posture

- The multi-container compose files mount `flash-agent-src` read-only into the
...
`flash_cli`/`agent` modules being importable. Each item should eventually move
...
|---|---|---|---|
| Browser chat execution | `run_agent.AIAgent` imported by `api/streaming.py` | Run lifecycle API: start, observe, status, cancel, approval, clarify, final usage | Covered by the runtime-adapter migration in [#1925](https://github.com/nesquena/flash-webui/issues/1925), but still source-backed today. |
...
| Profile list/create/delete/seed | `flash_cli.profiles` from `api/profiles.py` | Profile management API with profile metadata, env/runtime context, seed/delete operations, and validation errors | WebUI has fallback filesystem handling for some operations, but feature parity follows Flash CLI internals. |
| Goal command state | `flash_cli.goals` from `api/goals.py` | Goal CRUD/control API: get, save, pause/resume/clear, and status | Should preserve current `/goal` WebUI behavior without direct module import. |
| Slash command registry and plugin commands | `flash_cli.commands` and `flash_cli.plugins` from `api/commands.py` | Command/plugin capability discovery API scoped by active profile | WebUI should render command help from a stable capability response. |
| Provider/auth/model catalogs | `flash_cli.models`, `flash_cli.auth`, and `agent.credential_pool` from `api/config.py` | Provider registry, model catalog, auth status, OAuth/credential-pool status APIs | WebUI has static fallbacks, but exact parity and custom provider state come from Agent internals. |
...
1. Keep the Docker default safe: WebUI-side `flash-agent-src` stays read-only in
...
- Do not replace the runtime adapter or Flash Agent API in this document-only
  inventory slice.
