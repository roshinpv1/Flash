...
This guide explains what happens the first time Flash WebUI starts, which
...
Flash WebUI is only the browser interface. The actual agent runtime, memory,
skills, config, cron jobs, and provider credentials belong to Flash Agent.
...
[#1952](https://github.com/nesquena/flash-webui/issues/1952), including:

- [Native Windows guide](https://github.com/markwang2658/flash-windows-native-guide)
- [Native Windows setup scripts](https://github.com/markwang2658/flash-windows-native)
...
Do not delete `~/.flash` just to see the wizard again. That directory can hold
your real Flash config, credentials, memory, skills, profiles, sessions, and
cron state.

For a clean local trial, use an isolated Flash home and WebUI state directory:

```bash
mkdir -p ~/flash-onboarding-test
HERMES_HOME=~/flash-onboarding-test/.flash \
HERMES_WEBUI_STATE_DIR=~/flash-onboarding-test/webui \
...
- Flash Agent importability: whether WebUI can import and run `AIAgent`.
...
through the wizard or run `flash model` in the same machine environment that
will run WebUI.
...
For API-key providers, the wizard writes the key to the active Flash `.env`
...
first-class built-in Flash provider id. Configure it under the
...
the onboarding flow when your Flash config selects the corresponding provider.
If the wizard points you back to `flash model`, use that CLI flow first, then
refresh WebUI.
...
The workspace is the filesystem location Flash should use for new sessions.
...
- Active Flash `config.yaml`: provider, default model, and Base URL when
  relevant.
- Active Flash `.env`: provider API keys when you entered one.
...
- Flash Agent state: Windows `%LOCALAPPDATA%\flash`; POSIX `~/.flash`
- WebUI state: `$HERMES_HOME/webui` (Windows default `%LOCALAPPDATA%\flash\webui`, POSIX default `~/.flash/webui`)
