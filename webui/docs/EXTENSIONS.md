...
Flash WebUI supports a small, opt-in extension surface for self-hosted installs.
...
panels that should not live in core Flash WebUI.
...
> **[flash-webui/flash-webui-extensions](https://github.com/flash-webui/flash-webui-extensions)**.
> "In the registry == vetted." That repo holds the entries, the authoring
> conventions ([`docs/extension-entry.md`](https://github.com/flash-webui/flash-webui-extensions/blob/main/docs/extension-entry.md)),
...
- change Flash Agent permissions, models, memory, or tools unless they call
...
`~/.flash/webui/extensions/`) and installs into it; gallery-installed
...
write your `~/.flash` state) can place code there. The trust model below still
...
```bash
cat > ~/.flash/webui-extension-bundle/extensions.json <<'JSON'
{
  "extensions": [
    {
      "id": "templates",
...
HERMES_WEBUI_EXTENSION_DIR=~/.flash/webui-extension-bundle \
...
```js
const settings = window.FlashExtensionSettings.settingsForExtension("desktop-companion");
const value = settings.get("show_badge");
settings.set("show_badge", false);

const storage = window.FlashExtensionSettings.storageForExtension("desktop-companion");
storage.set("lastPanel", "settings");

const sameSettings = window.flashExt.settings.forExtension("desktop-companion");
const sameStorage = window.flashExt.storage.forExtension("desktop-companion");
```
...
switcher. Call `window.registerFlashSkin(descriptor)` from your extension
script:

```javascript
window.registerFlashSkin({
...
`window.registerFlashTtsEngine(descriptor)`:
...
**[flash-webui/flash-webui-extensions](https://github.com/flash-webui/flash-webui-extensions)**
following [`docs/extension-entry.md`](https://github.com/flash-webui/flash-webui-extensions/blob/main/docs/extension-entry.md)
...
```bash
mkdir -p ~/.flash/webui-extension
cat > ~/.flash/webui-extension/app.css <<'CSS'
.my-extension-badge {
  position: fixed;
  right: 12px;
  bottom: 12px;
...
CSS
cat > ~/.flash/webui-extension/app.js <<'JS'
(() => {
...
```bash
HERMES_WEBUI_EXTENSION_DIR=~/.flash/webui-extension \
...
storage actions call `window.FlashExtensionSettings`; they do not call backend
