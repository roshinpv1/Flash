...
`FlashAssistantTurnAnchors.stateLayers` so tests can pin the current authority
order.

## Slice 2 Normalizer Helper

`FlashAssistantTurnAnchors.normalizeAssistantTurnAnchorSourceEvent()` converts a
...
`FlashAssistantTurnAnchors.normalizeAssistantTurnAnchorSourceEvents()` applies
...
`FlashAssistantTurnAnchors.createAssistantTurnAnchorRegistry()` creates a local
...
`FlashAssistantTurnAnchors.applyAssistantTurnAnchorSourceEvent()` and
...
`FlashAssistantTurnAnchors.createAssistantTurnAnchorShadowSnapshot()` is the
...
`FlashAssistantTurnAnchors.terminalStates` exposes the RFC terminal-state enum:
...
`FlashAssistantTurnAnchors.projectAssistantTurnAnchorSettledMessageFinalAnswer()`
...
`FlashAssistantTurnAnchors.projectAssistantTurnAnchorActivityScene()` projects
...
`FlashAssistantTurnAnchors.applyAssistantTurnAnchorSourceEvent()`. This is a
...
`FlashAssistantTurnAnchors.reconcileAssistantTurnAnchorActivityScene()` compares
...
`FlashAssistantTurnAnchors.createAssistantTurnAnchorRendererSnapshot()` turns
...
`FlashAssistantTurnAnchors.reconcileAssistantTurnAnchorRendererSnapshot()` is
