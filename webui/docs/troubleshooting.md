...
Concrete diagnostic flows for the most common failure modes when running Flash WebUI. Each entry has the symptom, the diagnostic commands you should run *before* opening an issue, and the fix that has worked for past reporters.

If your symptom isn't listed and the diagnostics don't narrow it down, file a bug at https://github.com/nesquena/flash-webui/issues — include the relevant command output after redacting secrets, private paths, full `.env` files, full `auth.json` files, cookies, tokens, and password hashes.

---

## "AIAgent not available -- check that flash-agent is on sys.path"
...
**Why it happens.** The WebUI imports the agent class at chat time via `from run_agent import AIAgent`. That import only succeeds if the running Python's `sys.path` contains either the flash-agent checkout or a pip-installed copy of the agent. Three common failure modes:

1. **Agent installed but not on `sys.path`.** Most common. The agent is checked out somewhere (e.g. `~/Programmes/flash-agent`), the WebUI was launched with a Python that doesn't know about it, and there's no `pip install -e .` linking the two.
...
4. **Agent installed as root, under the FHS layout.** When the Flash Agent installer runs as root on Linux it places the agent at `/usr/local/lib/flash-agent` (CLI linked into `/usr/local/bin`), not `~/.flash/flash-agent`. Older `bootstrap.py` didn't probe that path, so it built a WebUI-only `.venv` and failed at launch with **"Python environment cannot import both WebUI dependencies and Flash Agent."** `git pull` to update the WebUI (current `bootstrap.py` auto-discovers the FHS layout and follows the `flash` launcher to the agent), or set `HERMES_WEBUI_PYTHON=/usr/local/lib/flash-agent/venv/bin/python` and relaunch.

### Step 1 — confirm the agent location

```bash
# If you have ~/flash-agent (the default location):
ls -la ~/flash-agent
readlink ~/flash-agent          # if it's a symlink, where does it resolve?
ls ~/flash-agent/agent/__init__.py 2>&1
```
...
```bash
cd ~/flash-webui && ./start.sh 2>&1 | grep -iE 'agent|python|flash_webui_python' | head -20
```
...
```bash
export HERMES_WEBUI_AGENT_DIR=/absolute/path/to/flash-agent
...
```bash
cd /path/to/flash-agent          # the directory holding pyproject.toml + the agent/ module
...
```bash
PYTHONPATH=/path/to/flash-agent $HERMES_WEBUI_PYTHON -c "from run_agent import AIAgent; print('ok')"
```
...
If after running steps 1-4 the import still fails *and* `pip install -e .` succeeded *and* `PYTHONPATH=... python -c "from run_agent import AIAgent"` succeeds — that's a real WebUI bug. File at https://github.com/nesquena/flash-webui/issues with:
...
The on-disk locations below assume the default `~/.flash/webui` state directory. If you override it via `HERMES_WEBUI_STATE_DIR`, substitute that path for `~/.flash/webui` in every step.

1. Identify the affected session id and stream id from the marker. The marker JSON lives at `~/.flash/webui/sessions/<sid>.json`; after the fix it shows them on the `_journal_retry_stream_id` key. Pre-fix sessions only carry the legacy wording, with no retry meta.
2. Check whether the run-journal contains real events:
   ```bash
   ls -la ~/.flash/webui/sessions/_run_journal/<sid>/<stream_id>.jsonl
   head -2 ~/.flash/webui/sessions/_run_journal/<sid>/<stream_id>.jsonl
   ```
...
1. Open the session JSON under your WebUI state directory, for example:
   ...
   jq '.recommended_recovery_action, .compression_recovery' \
     ~/.flash/webui/sessions/<session_id>.json
   ```
...
**Fix.** Prefer WebUI's own password for installed PWAs. If you keep proxy basic auth, configure it so the same-origin service-worker and shell update fetches can complete. If the installed shell is already blank, clear site data for the Flash origin, then reopen or reinstall the PWA after that site-scoped cleanup.
