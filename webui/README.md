...
[Flash Agent](https://flash-agent.nousresearch.com/) is a sophisticated autonomous agent that lives on your server, accessed via a terminal or messaging apps, that remembers what it learns and gets more capable the longer it runs.

Flash WebUI is a lightweight, dark-themed web app interface in your browser for [Flash Agent](https://flash-agent.nousresearch.com/).
...
**Flash Control Center** (launcher at the sidebar bottom).

Setup Flash so you can access it natively on every device:
...
This gives you nearly **1:1 parity with Flash CLI from a convenient web UI** which you can access securely through an SSH tunnel from your Flash setup. Single command to start this up, and a single command to SSH tunnel for access on your computer. Every single part of the web UI uses your existing Flash agent and existing models, without requiring any additional setup.
...
](https://get-flash.ai/setup/) 

- [Why Flash](#why-flash) — what it is and how it compares
...
Flash retains context across sessions, runs scheduled jobs while you're offline, and gets
smarter about your environment the longer it runs. It uses your existing Flash agent setup,
...
- **Persistent memory** — user profile, agent notes, and a skills system that saves reusable
  procedures; Flash learns your environment and does not have to relearn it
...
- **Self-improving skills** — Flash writes and saves its own skills automatically from experience;
...
**vs. the field** *(landscape is actively shifting — see [docs/why-flash.md](docs/why-flash.md) for the full breakdown)*:

| | OpenClaw | Claude Code | Codex CLI | OpenCode | Flash |
|---|---|---|---|---|---|
...
with memory, cron, and messaging. The key differences: Flash writes and saves its own skills
automatically as a core behavior (OpenClaw's skill system centers on a community marketplace);
Flash is more stable across updates (OpenClaw has documented release regressions and ClawHub
has had security incidents involving malicious skills); and Flash runs natively in the Python
ecosystem. See [docs/why-flash.md](docs/why-flash.md) for the full side-by-side.
...
```bash
git clone https://github.com/nesquena/flash-webui.git flash-webui
cd flash-webui
...
```bash
./ctl.sh start              # background daemon, PID at ~/.flash/webui.pid
./ctl.sh status             # PID, uptime, bound host/port, log path, /health
./ctl.sh logs --lines 100   # tail ~/.flash/webui.log
./ctl.sh restart
./ctl.sh stop
```

`ctl.sh start` runs the bootstrap in foreground/no-browser mode behind the daemon wrapper, writes logs to `~/.flash/webui.log`, and respects `.env` plus inline overrides such as `HERMES_WEBUI_HOST=0.0.0.0 ./ctl.sh start`.

> **Stopping the server.** Each launch method has its own stop path because only `ctl.sh start` writes a PID file (`~/.flash/webui.pid`):
>
...
> **How chat runs by default.** WebUI runs the Flash agent in-process, reading
> your `HERMES_HOME` config directly. It does not connect to an external
> Flash/agent OpenAI-compatible API server to run chat. `HERMES_API_URL` is only
...
> 2. **Route chat through a Flash Gateway API server** (supported today via
>    `HERMES_WEBUI_CHAT_BACKEND=gateway`): see [`docs/advanced-chat-setup.md`](docs/advanced-chat-setup.md).
>    Full agent-loop delegation is not yet shipped; tracked in [#1925](https://github.com/nesquena/flash-webui/issues/1925).

### Advanced: dynamic recall prefill & Gateway-backed chat

Two optional, self-hosted-deployment features — attaching dynamic **session-recall prefill** to browser turns (Joplin/Obsidian/Notion/llm-wiki routers), and routing browser chat through a running **Flash Gateway** — are documented in [`docs/advanced-chat-setup.md`](docs/advanced-chat-setup.md). Most users need neither.

The bootstrap will:

1. Detect Flash Agent and, if missing, attempt the official installer (`curl -fsSL https://raw.githubusercontent.com/NousResearch/flash-agent/main/scripts/install.sh | bash`).
...
A community-maintained native Windows setup is documented at [@markwang2658/flash-windows-native-guide](https://github.com/markwang2658/flash-windows-native-guide) (companion setup repo: [@markwang2658/flash-windows-native](https://github.com/markwang2658/flash-windows-native)). Notes from the community report in [#1952](https://github.com/nesquena/flash-webui/issues/1952):
...
- **Native Windows setup:** install Python 3.11+, then from the flash-agent root in PowerShell: `python -m venv venv` → `pip install -r requirements.txt` → `pwsh .\start.ps1` (it auto-discovers `venv\Scripts\python.exe`).
...
If provider setup is still incomplete after install, the onboarding wizard will point you to finish it with `flash model` instead of trying to replicate the full CLI setup in-browser.
...
- Multi-provider model support -- any Flash API provider (OpenAI, Anthropic, Google, DeepSeek, Nous Portal, OpenRouter, MiniMax, Xiaomi MiMo, Z.AI); dynamic model dropdown populated from configured keys
...
- File attachments persist across page reloads and are stored outside the active workspace by default (`~/.flash/webui/attachments/<session_id>/`, or `HERMES_WEBUI_ATTACHMENT_DIR/<session_id>/` when configured)
...
- CLI session bridge -- CLI sessions from flash-agent's SQLite store appear in the sidebar with a gold "cli" badge; click to import with full history and reply normally
...
- **Flash Control Center** (sidebar launcher button) -- Conversation tab (export/import/clear), Preferences tab (model, send key, theme, language, all toggles), System tab (version, password)
...
|---|---|
| Flash agent dir | `HERMES_WEBUI_AGENT_DIR`, then known checkout paths, the `flash` launcher on `PATH`, and finally the installed `run_agent` module exposed by `HERMES_WEBUI_PYTHON` |
| Python executable | Agent venv first, then `.venv` in this repo, then system `python3` |
| State directory | `HERMES_WEBUI_STATE_DIR` env, then `$HERMES_HOME/webui` (Windows default `%LOCALAPPDATA%\flash\webui`, POSIX default `~/.flash/webui`) |
...
```bash
export HERMES_WEBUI_AGENT_DIR=/path/to/flash-agent
...
|---|---|---|
| `HERMES_WEBUI_AGENT_DIR` | auto-discovered | Path to the Flash Agent source or installed module root |
| `HERMES_WEBUI_PYTHON` | auto-discovered | Python executable |
| `HERMES_WEBUI_HOST` | `127.0.0.1` | Bind address (`0.0.0.0` for all IPv4, `::` for all IPv6, `::1` for IPv6 loopback) |
| `HERMES_WEBUI_PORT` | `8787` | Port |
| `HERMES_WEBUI_STATE_DIR` | `$HERMES_HOME/webui` (Windows default `%LOCALAPPDATA%\flash\webui`, POSIX default `~/.flash/webui`) | Where sessions and state are stored. **Note (upgrade):** the default now follows `HERMES_HOME` — if you previously relocated `HERMES_HOME` to a non-default base **without** setting `HERMES_WEBUI_STATE_DIR`, your WebUI state now resolves to `$HERMES_HOME/webui` instead of the old platform-default `~/.flash/webui`. To keep using the old location, set `HERMES_WEBUI_STATE_DIR` to it (or move the directory). Installs with `HERMES_HOME` unset or at the default base are unaffected. |
| `HERMES_WEBUI_DEFAULT_WORKSPACE` | `~/workspace` | Default workspace |
| `HERMES_WEBUI_DEFAULT_MODEL` | *(provider default)* | Optional model override; leave unset to use the active Flash provider default |
...
| `HERMES_HOME` | Windows: `%LOCALAPPDATA%\flash`; POSIX: `~/.flash` | Base directory for Flash state (affects all paths) |
| `HERMES_CONFIG_PATH` | `$HERMES_HOME/config.yaml` | Path to Flash config file |
...
Flash WebUI has a Nix flake package and a NixOS service module so you can run it declaratively.

Install the latest package with:

```bash
nix shell github:nesquena/flash-webui#default
```

Use this flake input in your system configuration:

```nix
inputs.flash-webui.url = "github:nesquena/flash-webui";
```
...
```nix
{
  inputs = {
    ...
    flash-agent.url = "github:NousResearch/flash-agent";
    flash-webui.url = "github:nesquena/flash-webui";
  };

  outputs = { self, nixpkgs, flash-agent, flash-webui, ... }: {
    nixosConfigurations.<host> = nixpkgs.lib.nixosSystem {
      modules = [
        flash-agent.nixosModules.default
        flash-webui.nixosModules.default
        ({ pkgs, ... }: {
          services.flash-agent.enable = true;
          services.flash-webui = {
            ...
            user = "flash";
            group = "flash";
            flashHome = "/var/lib/flash/.flash";
            agent.package = flash-agent.packages.${pkgs.stdenv.hostPlatform.system}.default;
            environmentFiles = [ "/run/secrets/flash-webui.env" ];
          };
        })
      ];
    };
...
The published Flash Agent package exposes `passthru.flashVenv`, so the module derives `HERMES_WEBUI_PYTHON` from its interpreter. Bootstrap then locates the installed `run_agent.py` through that interpreter without importing Agent code and exports its parent as `HERMES_WEBUI_AGENT_DIR`. Packages may expose `passthru.flashAgentDir` as a direct path instead. Use `agent.dir` and `agent.python` as explicit overrides for custom package layouts.

When WebUI reads shared Flash Agent state, run the service as a user that can already read that state. For a co-located Flash Agent service, set `user` and `group` to the agent service account; the module only creates the default `flash-webui` account and never changes ownership of an existing `flashHome`.
...
```bash
cd /path/to/flash-agent          # or wherever sys.path can find Flash modules
HERMES_WEBUI_PORT=8787 venv/bin/python /path/to/flash-webui/server.py
```

Note: use the agent venv Python (or any Python environment that has the Flash agent dependencies installed). System Python will be missing `openai`, `httpx`, and other required packages.
...
```bash
git clone https://github.com/nesquena/flash-webui
cd flash-webui
...
Run Compose as the user who owns your Flash home. `sudo docker compose up -d` can make `${HOME}` expand to the root user's home, so Docker mounts the wrong `.flash` directory instead of your real `~/.flash` and the WebUI starts with `config.yaml (not found, using defaults)`. Prefer adding your user to the Docker group and running `docker compose up -d`; if you must use sudo, set absolute paths first, for example `HERMES_HOME=/home/you/.flash HERMES_WORKSPACE=/home/you/workspace sudo -E docker compose up -d`, then verify with `docker compose config`.

The container auto-detects your UID/GID from the mounted `~/.flash` volume so files written by the agent stay readable by you on the host.
...
```bash
docker pull ghcr.io/nesquena/flash-webui:latest
docker run -d \
  -e WANTED_UID=$(id -u) -e WANTED_GID=$(id -g) \
  -v ~/.flash:/home/flashwebui/.flash \
  -e HERMES_WEBUI_STATE_DIR=/home/flashwebui/.flash/webui \
  -v ~/workspace:/workspace \
  -p 127.0.0.1:8787:8787 \
  ghcr.io/nesquena/flash-webui:latest
```

### Build locally

```bash
docker build -t flash-webui .
docker run -d \
  -e WANTED_UID=$(id -u) -e WANTED_GID=$(id -g) \
  -v ~/.flash:/home/flashwebui/.flash \
  -e HERMES_WEBUI_STATE_DIR=/home/flashwebui/.flash/webui \
...
> **Known limitation (#681)**: in the two-container setup, tools triggered from the WebUI run in the **WebUI container**, not the agent container. If you need git/node/etc. on the WebUI's filesystem, either use the single-container setup, extend the WebUI Dockerfile, or use the community [all-in-one image](https://github.com/sunnysktsang/flash-suite).
>
> **Source boundary note (#2453)**: the multi-container setup mounts `flash-agent-src` read-only into the WebUI by default. This prevents WebUI-side source rewrites but is still an implementation-coupling bridge, not a stable Agent API boundary. See [`docs/rfcs/agent-source-boundary.md`](docs/rfcs/agent-source-boundary.md) for the current source/API decoupling inventory.
...
| WebUI can't find agent source | `flash-agent-src` volume misconfigured | Use the named volumes from compose files as-is |
| Podman shared `.flash` fails | Podman 3.4 `keep-id` limitation | Use Podman 4+ or single-container |
...
| WebUI can't see `~/.flash` after `sudo docker compose` | `${HOME}` expanded to the root user's home (#3006) | Run Compose as your user, or pass absolute `HERMES_HOME`/`HERMES_WORKSPACE` with `sudo -E` |
...
Tests discover the repo and the Flash agent dynamically -- no hardcoded paths.
...
api/
  ...
  profiles.py     Profile state management, flash_cli wrapper
...
State lives outside the repo at `~/.flash/webui/` by default
...
The WebUI is still coupled to Flash Agent internals for runtime execution, provider/model access, and state/schema usage until the stable agent boundary work in [#1925](https://github.com/nesquena/flash-webui/issues/1925) and [#2491](https://github.com/nesquena/flash-webui/issues/2491) land. In practice, the WebUI imports Agent modules directly (`api/config.py`, `api/providers.py`, `api/streaming.py`) and reads Agent state layout directly, so version skew can cause import or behavior drift.

**Compatibility policy**
- WebUI release branches are tested against the matching Flash Agent release available at that WebUI release time.
- **Upgrade both together**: upgrade or pin WebUI and flash-agent together (same release train/version/date), especially before enabling production traffic.
- Running pinned older/newer combinations is **untested and unsupported** until the stable API boundary work in [#1925](https://github.com/nesquena/flash-webui/issues/1925) / [#2491](https://github.com/nesquena/flash-webui/issues/2491) is in place.
- Record the full `flash-agent` + `flash-webui` versions in issue reports when upgrade mismatches are suspected.

**Docker users**: pin both image tags (or corresponding pinned source revisions) rather than using `latest` on one side and a fixed tag on the other. When upgrading the multi-container setup, follow the agent-image upgrade procedure in [`docs/docker.md`](docs/docker.md) (which requires dropping the `flash-agent-src` volume before recreating). The current source-boundary status is tracked in [`docs/rfcs/agent-source-boundary.md`](docs/rfcs/agent-source-boundary.md).
...
**Start here**
- [`docs/why-flash.md`](docs/why-flash.md) — why Flash, the mental model, and a detailed comparison to Claude Code / Codex / OpenCode / Cursor
...
Flash WebUI is built with help from the open-source community. Every PR — whether merged directly, absorbed into a batch release, or salvaged from a larger proposal — shapes the project, and we're grateful to everyone who has taken the time to contribute.
...
```
git@github.com:nesquena/flash-webui.git
```
