#!/usr/bin/env python3
"""
Flash WebUI MCP Server — exposes project and session management
...
MCP config for Flash Agent (add to config.yaml):
    mcp_servers:
      flash-webui:
        command: /path/to/venv/bin/python3
        args: [/path/to/flash-webui/mcp_server.py]
        env:
          HERMES_WEBUI_PASSWORD: your_password

Profile override (optional):
        args: [/path/to/flash-webui/mcp_server.py, --profile, myprofile]

AI-authoring disclosure: this file was rewritten by MILO (Flash Agent)
...
def _api_auth() -> str | None:
    ...
    try:
        ...
        if cookie:
            _auth_cookie = cookie.split(";")[0]  # "flash_session=VALUE; ..."
