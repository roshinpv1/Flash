...
Credential storage mirrors every other Nyxo channel:

    * runtime SDK creds  -> ``~/.nyxo/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.nyxo/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a nyxo dependency
...
# registers Nyxo as its own client_id.
...
# Default name of the project Nyxo provisions for the operator.
DEFAULT_PROJECT_NAME = "Nyxo Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of nyxo-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.nyxo/auth.json`` honouring the active Nyxo profile."""
    try:
        from nyxo_constants import get_nyxo_home
        return Path(get_nyxo_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.nyxo")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.nyxo/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.nyxo/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.nyxo/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: nyxo_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `nyxo photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `nyxo photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `nyxo photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `nyxo photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `nyxo photon setup`)"
