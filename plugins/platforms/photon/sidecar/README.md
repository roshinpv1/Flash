...
Small Node helper that bridges Nyxo Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Nyxo is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Nyxo-Sidecar-Token`)
...
The Nyxo plugin's `nyxo photon setup` command runs `npm install`
here automatically.
