...
This plugin connects Nyxo Agent to iMessage (and other Spectrum
...
TypeScript-only, Nyxo runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Nyxo-Sidecar-Token`.
...
There is no separate `login` command; like every other Nyxo channel,
...
Run `nyxo photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.nyxo/.env` (the same place every other
...
Management metadata lives in `~/.nyxo/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `nyxo photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `nyxo photon status`, a DM and a group roundtrip,
