"""
LINE Messaging API platform adapter for Nyxo Agent.
...
to Nyxo Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NYXO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nyxo_home = Path.home().joinpath(".nyxo").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nyxo status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nyxo status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nyxo setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nyxo/.env`` via ``nyxo_cli.config``.
    """
    print()
    ...
    try:
        from nyxo_cli.config import get_env_var, set_env_var
    except ImportError:
        print("nyxo_cli.config not available; set LINE_* vars manually in ~/.nyxo/.env")
        return
