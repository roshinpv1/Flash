"""
Microsoft Teams platform adapter for Flash Agent.
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``flash cron`` running as a
    separate process from ``flash gateway``).  Without this hook,
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Flash"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; FlashAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        flash_action = data.get("flash_action", "")
        session_key = data.get("session_key", "")

        if not flash_action or not session_key:
        ...
        choice = choice_map.get(flash_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="flash_approve",
                    data={**btn_data_base, "flash_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="flash_approve",
                    data={**btn_data_base, "flash_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="flash_approve",
                    data={**btn_data_base, "flash_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="flash_approve",
                    data={**btn_data_base, "flash_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    from flash_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_info,
        print_success,
    ...
    print_info("  teams app create --name \"Flash\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.flash/.env")
    print_info("Install the app in Teams:  teams app install --id <teamsAppId>")
    print_info("Restart the gateway:       flash gateway restart")
