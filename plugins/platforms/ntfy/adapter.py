"""ntfy platform adapter (Flash plugin).
...
This adapter ships as a Flash platform plugin under
``plugins/platforms/ntfy/``. The Flash plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "flash-in"              # subscribe topic (incoming)
          publish_topic: "flash-out"     # optional — defaults to topic
...
_ECHO_TAG = "flash-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``flash cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Flash plugin system at startup."""
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Flash dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `flash gateway status` without
