"""ntfy platform adapter (Nyxo plugin).
...
``plugins/platforms/ntfy/``. The Nyxo plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "nyxo-in"              # subscribe topic (incoming)
          publish_topic: "nyxo-out"     # optional — defaults to topic
...
_ECHO_TAG = "nyxo-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``nyxo cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Nyxo dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `nyxo gateway status` without
