...
# ``model_tools`` import time, so ``nyxo status``, ``nyxo chat``, etc.
...
# ``nyxo_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Nyxo is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nyxo MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NYXO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _nyxo_home = _get_nyxo_home()
        except (ModuleNotFoundError, ImportError):
            _nyxo_home = _Path.home() / ".nyxo"
        self._thread_count_store = _ThreadCountStore(
            _nyxo_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Nyxo is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NYXO_HOME", str(_Path.home() / ".nyxo"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NYXO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nyxo MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nyxo is thinking…' marker message.
        ...
        "Nyxo is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Nyxo is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nyxo is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nyxo is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nyxo is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nyxo setup``.

    The setup wizard at ``nyxo_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nyxo/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nyxo-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nyxo/.env")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nyxo plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nyxo-agent[google_chat]'",
        ...
        # the "Nyxo is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nyxo is thinking…' marker message "
