...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("NYXO_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "NYXO_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "NYXO_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Nyxo-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Nyxo' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Nyxo must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `nyxo update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Nyxo or OpenClaw instance is running "
            "with this token, then restart the gateway with 'nyxo gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Nyxo process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("NYXO_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("NYXO_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("NYXO_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("NYXO_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("NYXO_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            disable_fallback = (os.getenv("NYXO_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from nyxo_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  Nyxo defaults to 60 to
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``nyxo model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from nyxo_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from nyxo_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.nyxo/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".nyxo" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@nyxo_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Nyxo bot profiles. A message like
...
# nyxo_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("NYXO_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in nyxo_cli/gateway.py.
    """
    from nyxo_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'nyxo-agent[telegram]'",
