...
Auto-tracks and cleans up ephemeral files created during Nyxo Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NYXO_HOME` and `/tmp/nyxo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NYXO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NYXO_HOME` or `/tmp/nyxo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NYXO_HOME/disk-cleanup/` is itself excluded
- `$NYXO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
