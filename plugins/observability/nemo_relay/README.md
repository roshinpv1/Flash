...
Optional Flash observability plugin that maps Flash observer hooks to
...
not replace Flash Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Flash emit NeMo Relay lifecycle events for
the work Flash already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Flash Agent can:

- Preserve Flash execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
flash plugins enable observability/nemo_relay
```
...
```bash
env HERMES_HOME=/tmp/flash-nemo-relay-test \
  flash plugins enable observability/nemo_relay
```
...
`HERMES_HOME` is the Flash profile/config home used by both
`flash plugins enable ...` and the later `flash chat ...` run. If unset,
Flash uses the user's default home, usually `~/.flash`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export HERMES_HOME=/tmp/flash-nemo-relay-test
flash plugins enable observability/nemo_relay
flash chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `flash` command you run is built from the
...
uv run flash plugins enable observability/nemo_relay
uv run flash chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/flash_agent-*.whl
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Flash does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Flash keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Flash routes LLM and tool
...
For the full generic Flash middleware contract, see
...
export HERMES_HOME=/tmp/flash-nemo-relay-docs/flash-home
mkdir -p "$HERMES_HOME"
...
This run starts a parent Flash session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/flash-nemo-relay-docs/subagent/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/flash-nemo-relay-docs/subagent/atif
export HERMES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Flash Agent E2E'
...
{"kind":"mark","name":"flash.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Flash Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Flash dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/flash-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/flash-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/flash-nemo-relay-docs/workdir/beta.txt
cd /tmp/flash-nemo-relay-docs/workdir

export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/flash-nemo-relay-docs/parallel/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/flash-nemo-relay-docs/parallel/atif
...
- Flash sessions map to `agent` scopes.
- Flash API request hooks map to `llm` scope start/end events.
- Flash tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Flash execution middleware to hand
...
```bash
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/flash-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Flash routes execution
...
Flash provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Flash provider adapter next_call(...)

Flash tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Flash tool dispatcher next_call(...)
```
...
middleware for a local Flash run. This path requires a NeMo Relay runtime that
...
```bash
export HERMES_HOME=/tmp/flash-middleware-test/flash-home
mkdir -p "$HERMES_HOME" /tmp/flash-middleware-test/nemo-relay
...
cat > /tmp/flash-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/flash-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/flash-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Flash Middleware E2E"
agent_version = "local"
...
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/flash-middleware-test/nemo-relay/plugins.toml
