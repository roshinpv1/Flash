...
Optional Nyxo observability plugin that maps Nyxo observer hooks to
...
not replace Nyxo Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Nyxo emit NeMo Relay lifecycle events for
the work Nyxo already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Nyxo Agent can:

- Preserve Nyxo execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
nyxo plugins enable observability/nemo_relay
```

The `NYXO_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `NYXO_HOME` that the
agent run will use:

```bash
env NYXO_HOME=/tmp/nyxo-nemo-relay-test \
  nyxo plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`NYXO_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`NYXO_HOME` is the Nyxo profile/config home used by both
`nyxo plugins enable ...` and the later `nyxo chat ...` run. If unset,
Nyxo uses the user's default home, usually `~/.nyxo`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export NYXO_HOME=/tmp/nyxo-nemo-relay-test
nyxo plugins enable observability/nemo_relay
nyxo chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `nyxo` command you run is built from the
...
uv run nyxo plugins enable observability/nemo_relay
uv run nyxo chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/nyxo_agent-*.whl
...
The plugin can configure exporters directly from `NYXO_NEMO_RELAY_*`
...
```bash
export NYXO_NEMO_RELAY_ATOF_ENABLED=1
export NYXO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export NYXO_NEMO_RELAY_ATIF_ENABLED=1
export NYXO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `NYXO_NEMO_RELAY_ATOF_FILENAME`
- `NYXO_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `NYXO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `NYXO_NEMO_RELAY_ATIF_AGENT_NAME`
- `NYXO_NEMO_RELAY_ATIF_AGENT_VERSION`
- `NYXO_NEMO_RELAY_ATIF_MODEL_NAME`
- `NYXO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Nyxo at it:

```bash
export NYXO_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Nyxo Agent"
agent_version = "local"
```

When `NYXO_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`NYXO_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`NYXO_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Nyxo does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Nyxo keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Nyxo routes LLM and tool
...
For the full generic Nyxo middleware contract, see
...
export NYXO_HOME=/tmp/nyxo-nemo-relay-docs/nyxo-home
mkdir -p "$NYXO_HOME"

cat > "$NYXO_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Nyxo session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export NYXO_NEMO_RELAY_ATOF_ENABLED=1
export NYXO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nyxo-nemo-relay-docs/subagent/atof
export NYXO_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export NYXO_NEMO_RELAY_ATOF_MODE=overwrite
export NYXO_NEMO_RELAY_ATIF_ENABLED=1
export NYXO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nyxo-nemo-relay-docs/subagent/atif
export NYXO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export NYXO_NEMO_RELAY_ATIF_AGENT_NAME='Nyxo Agent E2E'
export NYXO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export NYXO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"nyxo.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Nyxo Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Nyxo dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/nyxo-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/nyxo-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/nyxo-nemo-relay-docs/workdir/beta.txt
cd /tmp/nyxo-nemo-relay-docs/workdir

export NYXO_NEMO_RELAY_ATOF_ENABLED=1
export NYXO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nyxo-nemo-relay-docs/parallel/atof
export NYXO_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export NYXO_NEMO_RELAY_ATOF_MODE=overwrite
export NYXO_NEMO_RELAY_ATIF_ENABLED=1
export NYXO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nyxo-nemo-relay-docs/parallel/atif
export NYXO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export NYXO_NEMO_RELAY_ATIF_AGENT_NAME='Nyxo Agent E2E'
export NYXO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Nyxo sessions map to `agent` scopes.
- Nyxo API request hooks map to `llm` scope start/end events.
- Nyxo tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Nyxo execution middleware to hand
...
```bash
export NYXO_NEMO_RELAY_PLUGINS_TOML=/tmp/nyxo-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Nyxo routes execution
...
Nyxo provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Nyxo provider adapter next_call(...)

Nyxo tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Nyxo tool dispatcher next_call(...)
```
...
middleware for a local Nyxo run. This path requires a NeMo Relay runtime that
...
```bash
export NYXO_HOME=/tmp/nyxo-middleware-test/nyxo-home
mkdir -p "$NYXO_HOME" /tmp/nyxo-middleware-test/nemo-relay
...
cat > /tmp/nyxo-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/nyxo-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/nyxo-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Nyxo Middleware E2E"
agent_version = "local"
...
export NYXO_NEMO_RELAY_PLUGINS_TOML=/tmp/nyxo-middleware-test/nemo-relay/plugins.toml
