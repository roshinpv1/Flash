"""langfuse — Flash plugin for Langfuse observability.

Traces Flash conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Flash plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``flash plugins enable
observability/langfuse`` or ``flash tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``flash tools`` or ~/.flash/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Flash plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Flash paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Flash branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Flash also has a turn-scoped
    ...
    # messages is a list only for legacy Flash branches that fired
    # pre_llm_call with API messages directly. Current Flash fires
...
def register(ctx) -> None:
    ...
    # Flash versions.  pre_api_request / post_api_request fire per API
