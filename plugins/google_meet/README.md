...
Let the nyxo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where nyxo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `nyxo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `nyxo meet` CLI |
...
| `cli.py` | `nyxo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NYXO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `nyxo meet node {run,list,approve,remove,status,ping}` |
...
nyxo meet install                                      # pip + Chromium
nyxo meet setup                                        # preflight
nyxo meet auth                                         # optional
nyxo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
nyxo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.nyxo/.env
nyxo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
nyxo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
nyxo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.nyxo/.env
nyxo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, nyxo will **not** switch your system audio input automatically — the
...
nyxo plugins enable google_meet
nyxo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
