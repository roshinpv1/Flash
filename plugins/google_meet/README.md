...
Let the flash agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where flash runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `flash meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `flash meet` CLI |
...
| `cli.py` | `flash meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `flash meet node {run,list,approve,remove,status,ping}` |
...
flash meet install                                      # pip + Chromium
flash meet setup                                        # preflight
flash meet auth                                         # optional
flash meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
flash meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.flash/.env
flash meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
flash meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
flash meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.flash/.env
flash meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, flash will **not** switch your system audio input automatically — the
...
flash plugins enable google_meet
flash meet node run --display-name my-mac --host 0.0.0.0 --port 18789
