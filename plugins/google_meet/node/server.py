...
Launched by ``flash meet node run``.
...
via ``flash meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_flash_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "flash-meet-node",
    ) -> None:
        self.host = host
        self.port = port
