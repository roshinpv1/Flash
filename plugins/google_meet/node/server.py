...
Launched by ``nyxo meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$NYXO_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``nyxo meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_nyxo_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "nyxo-meet-node",
    ) -> None:
        self.host = host
        self.port = port
