...
flash meet install                 # pip deps + Chromium (transcribe only)
flash meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
flash meet auth                    # optional; skips guest-lobby wait
flash meet setup                   # preflight checks
```

`flash meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.flash/.env
```
...
pip install playwright websockets && python -m playwright install chromium
flash plugins enable google_meet
flash meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
flash meet node approve my-mac ws://<mac-ip>:18789 <token>
flash meet node ping my-mac                   # confirm reachable
```

Run `flash meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Flash agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `flash meet auth` avoids this.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `flash_meet_src`). |
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `flash meet node approve`.
