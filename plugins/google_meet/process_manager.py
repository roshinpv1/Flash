...
session-scoped state file under ``$NYXO_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $NYXO_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_nyxo_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this nyxo install, leave it first —
    ...
    env = os.environ.copy()
    env["NYXO_MEET_URL"] = url
    env["NYXO_MEET_OUT_DIR"] = str(out)
    env["NYXO_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["NYXO_MEET_HEADED"] = "1"
    if auth_state:
        env["NYXO_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["NYXO_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if NYXO_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["NYXO_MEET_MODE"] = mode
    if realtime_model:
        env["NYXO_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["NYXO_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["NYXO_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["NYXO_MEET_REALTIME_KEY"] = realtime_api_key
