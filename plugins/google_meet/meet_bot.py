...
``$HERMES_HOME/workspace/meetings/<meeting-id>/``. The main flash process
...
# and exposes ``window.__flashMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__flashMeetInstalled) return;
  window.__flashMeetInstalled = true;
  window.__flashMeetQueue = [];
  ...
  window.__flashMeetDrain = () => {
    const out = window.__flashMeetQueue.slice();
    window.__flashMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "flash_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    guest_name = os.environ.get("HERMES_MEET_GUEST_NAME", "Flash Agent")
    ...
    try:
        with sync_playwright() as pw:
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__flashMeetDrain && window.__flashMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__flashMeetInstalled) {
