# Nyxo Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: nyxo-achievements plugin
...
### `GET /api/plugins/nyxo-achievements/achievements`
...
### `POST /api/plugins/nyxo-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/nyxo-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/nyxo-achievements/overview`
...
- Persist plugin data under:
  - `~/.nyxo/plugins/nyxo-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.nyxo/plugins/nyxo-achievements/scan_checkpoint.json`
...
- `~/.nyxo/plugins/nyxo-achievements/`
