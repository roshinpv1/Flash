...
``flash_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``flash_cli/web_server.py``.

This means ``flash dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from flash_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``flash_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from flash_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``flash_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from flash_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from flash_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from flash_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``flash kanban
...
# spawn failures, stuck-blocked). See flash_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``flash kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``flash kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``flash kanban specify <task_id>``.
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from flash_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``flash kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.flash/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Flash profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.flash/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from flash_constants import get_flash_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_flash_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``flash profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from flash_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``flash kanban decompose <task_id>``.
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from flash_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.flash/config.yaml.
