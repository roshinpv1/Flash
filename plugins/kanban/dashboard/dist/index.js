/**
 * Flash Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.flash/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "flash-kanban-dot-triage",
    todo: "flash-kanban-dot-todo",
    ready: "flash-kanban-dot-ready",
    running: "flash-kanban-dot-running",
    blocked: "flash-kanban-dot-blocked",
    done: "flash-kanban-dot-done",
    archived: "flash-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-flash-task";
  ...
  const DOCS_URL = "https://flash-agent.flashorg.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://flash-agent.flashorg.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "flash.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="flash-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])flash-kanban-md-code\1/i.test(attrs)) {
      return ' class="flash-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "flash-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "flash-kanban-md",
  ...
  // standard `drop` event and our `flash-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("flash-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("flash-kanban-column--drop");
          if (target) target.classList.add("flash-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("flash-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("flash-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("flash-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "flash-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "flash-kanban-attention",
        "flash-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "flash-kanban-attention-bar" },
        h("span", { className: "flash-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "flash-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "flash-kanban-attention-list" },
          diagTasks.map(function (task) {
            ...
            return h("div", {
              key: task.id,
              className: cn(
                "flash-kanban-attention-row",
                "flash-kanban-attention-row--" + sev,
              ),
            },
              h("span", { className: "flash-kanban-attention-row-sev" },
                sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
              h("span", { className: "flash-kanban-attention-row-id" }, task.id),
              h("span", { className: "flash-kanban-attention-row-title" },
                task.title || tx(t, "untitled", "(untitled)")),
              h("span", { className: "flash-kanban-attention-row-meta" },
                task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                " \u00b7 ",
                kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
              ),
              h("button", {
                className: "flash-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "flash-kanban-diag-action-btn",
      action.suggested ? "flash-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " flash-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".flash-kanban-drawer-comment-row input, .flash-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "flash-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("flash-kanban-diag", sevClass) },
      h("div", { className: "flash-kanban-diag-header" },
        h("span", { className: "flash-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
            diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "flash-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "flash-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "flash-kanban-diag-data" },
          Object.keys(diag.data).map(function (k) {
            const v = diag.data[k];
            if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
              v[0].indexOf("t_") === 0) {
              // Task-id list — render as chips.
              return h("div", { key: k, className: "flash-kanban-diag-data-row" },
                h("span", { className: "flash-kanban-diag-data-key" }, k + ":"),
                v.map(function (x) {
                  return h("code", {
                    key: x, className: "flash-kanban-event-phantom-chip",
                  }, x);
                }),
              );
            }
            return h("div", { key: k, className: "flash-kanban-diag-data-row" },
              h("span", { className: "flash-kanban-diag-data-key" }, k + ":"),
              h("span", { className: "flash-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "flash-kanban-diag-reassign-row" },
          h("span", { className: "flash-kanban-diag-reassign-label" },
            tx(t, "reassignTo", "Reassign to:")),
          h("select", {
            className: "flash-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "flash-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
          className: cn(
            "flash-kanban-diag-msg",
            msg.ok ? "flash-kanban-diag-msg--ok" : "flash-kanban-diag-msg--err",
          ),
        }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "flash-kanban-section" },
      h("div", { className: "flash-kanban-section-head-row" },
        h("span", { className: "flash-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "flash-kanban-section-head-warning" },
              `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "flash-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "flash-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "flash-kanban-docs-link",
      title: "Open Flash Kanban docs in a new tab",
      "aria-label": "Flash Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `flash kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "flash-kanban-msg-ok" : "flash-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "flash-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "flash-kanban-boardswitcher" },
      h("div", { className: "flash-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "flash-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "flash-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "flash-kanban-dialog-title" },
        ...
        h("div", { className: "flash-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", {
        className: "flex flex-col gap-1",
        title: "Filter by assigned Flash profile. Profiles are the named agent identities that claim and work on tasks."
      },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "flash-kanban-bulk" },
      h("span", { className: "flash-kanban-bulk-count" },
      ...
      h("div", {
        className: "flash-kanban-bulk-priority",
      ...
      h("div", {
        className: "flash-kanban-bulk-reassign",
        title: "Reassign selected tasks to a different Flash profile. Pick a profile (or unassign) and click Apply."
      },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "flash-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("flash-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("flash-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "flash-kanban-trash",
        dragOver ? "flash-kanban-trash--drop" : "",
        props.draggingTaskId ? "flash-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "flash-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "flash-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".flash-kanban-card")) return true;
      if (target.closest && target.closest(".flash-kanban-column-add")) return true;
      if (target.closest && target.closest(".flash-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("flash-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("flash-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".flash-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "flash-kanban-columns",
        isScrollable ? "flash-kanban-columns--scrollable" : "",
        isPanning ? "flash-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("flash-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("flash-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "flash-kanban-column",
        dragOver ? "flash-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
      ...
      h("div", {
        className: "flash-kanban-column-header",
        title: colHelp || ""
      },
        h(Checkbox, {
          className: "flash-kanban-col-check",
        ...
        h("span", { className: cn("flash-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "flash-kanban-column-label" },
          colLabel || props.column.name),
        h("span", {
          className: "flash-kanban-column-count",
          title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column`
        },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "flash-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "flash-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "flash-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "flash-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
              return h("div", { key: lane.assignee, className: "flash-kanban-lane" },
                h("div", { className: "flash-kanban-lane-head" },
                  h("span", { className: "flash-kanban-lane-name" }, lane.assignee),
                  h("span", { className: "flash-kanban-lane-count" }, lane.tasks.length),
                ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red) return "flash-kanban-card--stale-red";
    if (age >= tier.amber) return "flash-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".flash-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "flash-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "flash-kanban-card",
        props.selected ? "flash-kanban-card--selected" : "",
        props.failed ? "flash-kanban-card--failed" : "",
        props.draggingSource ? "flash-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "flash-kanban-card-content" },
          h("div", { className: "flash-kanban-card-row" },
            h("label", {
              className: "flash-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "flash-kanban-card-check",
            ...
            h("span", {
              className: "flash-kanban-card-id",
              title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or flash kanban show.`
            }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                className: cn(
                  "flash-kanban-warning-badge",
                  "flash-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                ),
                title: (
            ...
            progress
              ? h("span", {
                className: cn(
                  "flash-kanban-progress",
                  progress.done === progress.total ? "flash-kanban-progress--full" : "",
                ),
            ...
            needsAssignee
              ? h(Badge, {
                variant: "outline",
                className: "flash-kanban-needs-assignee",
          ...
          h("div", { className: "flash-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "flash-kanban-card-row flash-kanban-card-meta" },
            t.assignee
              ? h("span", {
                className: "flash-kanban-assignee",
                title: `Assigned to Flash profile @${t.assignee}`
              }, "@", t.assignee)
              : h("span", {
                className: "flash-kanban-unassigned",
                title: needsAssignee
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "flash-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Flash profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Flash profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "flash-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "flash-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "flash-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "flash-kanban-drawer-close",
        ...
        data ? h("div", { className: "flash-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "flash-kanban-section" },
      h("div", { className: "flash-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
          return h("div", {
            ...
            h("button", {
              type: "button",
              className: "flash-kanban-attachment-link truncate",
              title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "flash-kanban-drawer-body" },
      h("div", { className: "flash-kanban-drawer-title" },
        h("span", { className: cn("flash-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
            initial: t.title || "",
          ...
          : h("span", {
            className: "flash-kanban-drawer-title-text",
            title: tx(i18n, "clickToEdit", "Click to edit"),
            onClick: function () { props.setEditing(true); },
          }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "flash-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "flash-kanban-section" },
        h("div", { className: "flash-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "flash-kanban-section" },
        h("div", { className: "flash-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
            return h("div", { key: c.id, className: "flash-kanban-comment" },
              h("div", { className: "flash-kanban-comment-head" },
                h("span", { className: "flash-kanban-comment-author" }, c.author || "anon"),
                h("span", { className: "flash-kanban-comment-ago" },
      ...
      h("div", { className: "flash-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "flash-kanban-event",
              isDiag ? "flash-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "flash-kanban-event-header" },
                h("span", { className: "flash-kanban-event-warning-icon" }, "⚠"),
                h("span", { className: "flash-kanban-event-warning-label" },
                  getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                h("span", { className: "flash-kanban-event-ago" },
                  timeAgo ? timeAgo(e.created_at) : ""),
              )
              : h("div", { className: "flash-kanban-event-header-plain" },
                h("span", { className: "flash-kanban-event-kind" }, e.kind),
                h("span", { className: "flash-kanban-event-ago" },
                  timeAgo ? timeAgo(e.created_at) : ""),
              ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "flash-kanban-event-phantom-row" },
                h("span", { className: "flash-kanban-event-phantom-label" },
                  tx(i18n, "phantomIds", "Phantom ids:")),
                phantoms.map(function (pid) {
                  return h("code", {
                    key: pid,
                    className: "flash-kanban-event-phantom-chip",
                  }, pid);
                }),
              )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "flash-kanban-event-payload" },
    ...
    return h("div", { className: "flash-kanban-section" },
      h("div", { className: "flash-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
            type: "button",
            onClick: function () { setExpanded(true); },
            className: "flash-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `flash-kanban-run--${r.outcome || r.status || "ended"}`
          : "flash-kanban-run--active";
        return h("div", { key: r.id, className: cn("flash-kanban-run", outcomeClass) },
          h("div", { className: "flash-kanban-run-head" },
            h("span", { className: "flash-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "flash-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "flash-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "flash-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "flash-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "flash-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
              ...
              return h("details", {
                className: "flash-kanban-run-meta-block",
                open: !collapsed,
              },
                h("summary", { className: "flash-kanban-run-meta-label" }, "Metadata"),
                h("code", { className: "flash-kanban-run-meta" }, json),
              );
            })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "flash-kanban-pre flash-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "flash-kanban-meta-row" },
      h("span", { className: "flash-kanban-meta-label" }, props.label),
      h("span", { className: "flash-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "flash-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "flash-kanban-meta-row" },
        h("span", { className: "flash-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "flash-kanban-meta-value flash-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "flash-kanban-meta-row" },
        h("span", { className: "flash-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "flash-kanban-meta-value flash-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "flash-kanban-section" },
      h("div", { className: "flash-kanban-section-head-row" },
        h("span", { className: "flash-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
          className: "flash-kanban-textarea",
          value: v,
          rows: 8,
          onChange: function (e) { setV(e.target.value); },
        })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "flash-kanban-section" },
      h("div", { className: "flash-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "flash-kanban-deps-row" },
        h("span", { className: "flash-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "flash-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "flash-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
              return h("span", { key: id, className: "flash-kanban-dep-chip" },
                id,
                h("button", {
                  type: "button",
                  className: "flash-kanban-dep-chip-x",
      ...
      h("div", { className: "flash-kanban-deps-row" },
        h("span", { className: "flash-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "flash-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "flash-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "flash-kanban-section" },
      ...
      h("div", { className: "flash-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "flash-kanban-home-sub flash-kanban-home-sub--on"
              : "flash-kanban-home-sub",
          }, label);
        })
      )
    );
