# Flash Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: flash-achievements plugin
...
### `GET /api/plugins/flash-achievements/achievements`
...
### `POST /api/plugins/flash-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/flash-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/flash-achievements/overview`
...
- Persist plugin data under:
  - `~/.flash/plugins/flash-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.flash/plugins/flash-achievements/scan_checkpoint.json`
...
- `~/.flash/plugins/flash-achievements/`
