...
``{title, url, description, position}`` rows every other Nyxo web
provider produces.
...
prefers Nyxo-managed xAI Grok OAuth (via ``nyxo auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.nyxo/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``nyxo tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `nyxo auth` to sign in with "
