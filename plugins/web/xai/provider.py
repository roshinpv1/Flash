...
``{title, url, description, position}`` rows every other Flash web
provider produces.
...
prefers Flash-managed xAI Grok OAuth (via ``flash auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.flash/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``flash tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `flash auth` to sign in with "
