...
A standards-compliant OpenID Connect Relying Party for the ``flash dashboard``
OAuth gate. Unlike the bundled ``flash`` provider (which encodes FlashPortal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-flash-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~flash_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``flash_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~flash_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``flash``
provider verifies its *access* token because FlashPortal mints a custom JWT
...
same precedence convention as the ``flash`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.flash/.env over config.yaml.
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the flash
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the flash plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "flash_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Flash does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the flash provider.
        """
...
def register(ctx) -> None:
    ...
    # canonical home is the env var / ~/.flash/.env; config.yaml is supported
