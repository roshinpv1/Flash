...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``flash-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``flash_session_pkce``
cookie.
...
_DEFAULT_PORTAL_URL = "https://portal.flashorg.com"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"flash_session_pkce\"]``
...
def register(ctx) -> None:
    ...
    engagement layer (``flash_cli.web_server.should_require_auth`` +
