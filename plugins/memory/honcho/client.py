...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Flash instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Flash profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Flash profile.

    Resolution order:
      1. HERMES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``flash.<profile>``
      3. Fallback: ``"flash"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.flash/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_flash_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_flash_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Flash system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.flash
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Flash profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.flash block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Flash session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Flash session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'flash honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run flash honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `flash honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                honcho_cfg = flash_cfg.get("honcho", {})
