...
``flash://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Flash OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "flash-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``flash-desktop`` / ``flash-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Flash.</div>"
)
