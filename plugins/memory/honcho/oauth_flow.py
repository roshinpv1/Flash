...
``nyxo://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Nyxo OAuth client. IP-literal so
...
def begin_authorization(
    ...
    *,
    ...
    (``nyxo-desktop`` / ``nyxo-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Nyxo.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local NYXO_HOME override can't reach.
