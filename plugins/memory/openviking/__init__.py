...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Flash peer ID in OpenViking (default: flash)
...
_DEFAULT_AGENT = "flash"
_AGENT_PROMPT_LABEL = "Flash peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Flash slash-skill scaffolding before sending content to OpenViking.
...
def _load_flash_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("HERMES_HOME", "")).expanduser() if os.environ.get("HERMES_HOME") else Path.home() / ".flash"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Flash session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Flash only", "write values only to Flash .env"),
    ...
    _save_flash_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Flash .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See flash-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_flash_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Flash peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, flash_home: str, config: dict) -> None:
        ...
        from flash_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        flash_home_path = Path(flash_home)
        env_path = flash_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_flash_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Flash run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Flash' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Flash canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See flash-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/flash/.overview.md).
