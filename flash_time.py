"""
Timezone-aware clock for Flash.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.flash/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Flash never
...
from flash_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Flash requires 3.9+)
