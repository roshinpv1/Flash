"""Centralized logging setup for Flash Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.flash/logs/`` (profile-aware via ``get_flash_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``flash_cli.web_server``,
    ``flash_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Flash (TUI, gateway, ``hy_memory`` server, MCP
...
from flash_constants import get_config_path, get_flash_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_flash_session_injector", False):
    ...
    _session_record_factory._flash_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``flash logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``flash logs --component gateway``.
    "gateway": ("gateway", "flash_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("flash_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "flash_cli.web_server",
        "flash_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Flash logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    flash_home
        Override for the Flash home directory.  Falls back to
        ``get_flash_home()`` (profile-aware).
    log_level
    ...
    home = flash_home or get_flash_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_flash_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._flash_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the flash group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from flash_cli.config import is_managed
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._flash_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="flash-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_flash_queue", False):
