...
本指南带你从头到尾完成在 [Nous Portal](https://portal.flashorg.com) 订阅下运行 Flash Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [Nous Portal 集成页面](/integrations/nous-portal)。本页是操作步骤脚本。
...
3. 在 `~/.flash/config.yaml` 中设置 `model.provider: nous`
...
```bash
flash auth logout nous       # 清除本地 refresh token
