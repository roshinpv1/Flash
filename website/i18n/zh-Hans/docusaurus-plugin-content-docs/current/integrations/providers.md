...
| 提供商 | 配置方式 |
|----------|-------|
| **Nous Portal** | `flash model`（OAuth，订阅制） |
...
[Nous Portal](https://portal.flashorg.com) 是 Flash Org 的统一订阅网关，也是**运行 Flash Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [Nous Chat](https://chat.flashorg.com)——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。

```bash
flash setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
flash model              # 已有安装——从列表中选择"Nous Portal"
...
**完整详情：** 参见专属的 [Nous Portal 集成页面](/integrations/flash-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Nous Portal 运行 Flash Agent](/guides/run-flash-with-flash-portal)。
...
:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Flash 将这些任务路由到你的**主聊天模型**——即你在 `flash model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 Nous Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`flash setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `flash model` 或 `flash tools` 按工具启用。随时使用 `flash portal info` 查看路由状态。
:::
