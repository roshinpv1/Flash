...
在 `~/.flash/config.yaml` 中直接设置 `use_gateway`：
...
在 `flash tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
若使用自建或自定义网关，可在 `~/.flash/.env` 中用环境变量覆盖端点：

```bash
TOOL_GATEWAY_DOMAIN=flashorg.com     # 网关路由基础域名
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.flashorg.com/manage-subscription) 或通过 `flash tools` 改回直连 Key。
