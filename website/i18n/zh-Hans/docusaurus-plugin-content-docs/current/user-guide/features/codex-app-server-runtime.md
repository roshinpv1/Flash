...
Flash 可以选择将 `openai/*` 和 `openai-codex/*` 的轮次交由 [Codex CLI app-server](https://github.com/openai/codex) 处理，而不是运行自己的工具循环。启用后，终端命令、文件编辑、沙箱隔离以及 MCP 工具调用均在 Codex 的运行时内执行——Flash 成为其外层 shell（会话数据库、斜杠命令、gateway、记忆与技能审查）。

此功能**仅限手动启用**。除非你主动切换该标志，否则 Flash 的默认行为不变。Flash 不会自动将你路由到此运行时。
...
- **原生 Codex 插件**——Linear、GitHub、Gmail、Calendar、Canva 等——通过 `codex plugin` 安装后，会自动迁移并在你的 Flash 会话中激活。
- **Flash 的丰富工具一并可用**——web_search、web_extract、浏览器自动化、视觉、图像生成、技能和 TTS 通过 MCP 回调提供。Codex 会回调 Flash 获取其自身没有内置的工具。
- **记忆与技能提示持续生效**——Codex 的事件被投影为 Flash 的消息格式，使自我改进循环看到正常的对话记录。
...
这些工具随 `codex app-server` 本身一起提供——无需 Flash 介入，无需 MCP，无需插件。运行时启动后，以下五个工具立即可用：
...
- **`update_plan`** — Codex 的内部待办/计划跟踪器。等同于 Flash 的 `todo` 工具，但完全在 Codex 运行时内部管理。
- **`view_image`** — 将本地图像文件加载到对话中，使模型能够查看它。
- **`web_search`** — 配置后 Codex 拥有自己的内置网络搜索。Flash 也通过下方的回调暴露 `web_search`（基于 Firecrawl）；模型会选择其偏好的那个。

因此，**任何你通过终端完成的操作——读/写/搜索/查找/运行——Codex 都能原生处理**。沙箱配置文件（启用运行时时默认为 `:workspace`）控制可写范围。

### 2. 原生 Codex 插件（从你的 `codex plugin` 安装中自动迁移）

启用运行时时，Flash 会查询 Codex 的 `plugin/list` RPC，并为你已安装的每个插件写入一条 `[plugins."<name>@openai-curated"]` 配置项。插件本身由 Codex 管理，并通过 Codex 自己的 UI 完成一次性授权。
...
### 3. Flash 工具回调（MCP server，注册在 `~/.codex/config.toml` 中）

Flash 将自身注册为 MCP server，以便 Codex 能够回调获取 Codex 自身未内置的工具。通过回调可用的工具：
...
- **`image_generate`** — 通过 Flash 的 image_gen 插件链生成图像。
- **`skill_view` / `skills_list`** — 读取 Flash 的技能库。
- **`text_to_speech`** — 通过 Flash 配置的提供商进行 TTS。

当模型需要其中某个工具时，Codex 通过 stdio MCP 生成 `flash_tools_mcp_server` 子进程，调用通过 `model_tools.handle_function_call()` 分发（与 Flash 默认运行时的代码路径相同），结果像其他 MCP 响应一样返回给 Codex。

### 此运行时上不可用的工具

以下四个 Flash 工具需要运行中的 AIAgent 上下文（循环中间状态）才能分发，无状态的 MCP 回调无法驱动它们。需要这些工具时，请切换回默认运行时（`/codex-runtime auto`）：

- **`delegate_task`** — 生成子 agent
- **`memory`** — Flash 的持久记忆存储
- **`session_search`** — 跨会话搜索
- **`todo`** — Flash 的待办存储（Codex 的 `update_plan` 是运行时内的等效工具）
...
**需要注意的一点：** 每个续接提示都是一次全新的 Codex 轮次，这意味着 Codex 会从头重新评估命令审批策略。如果你在执行包含大量写操作的长期目标，预期会看到比单次会话内任务更多的审批提示。设置 `default_permissions = ":workspace"`（启用运行时时 Flash 会自动设置）可避免简单的工作区写操作触发提示。

### Kanban（多 agent 工作树分发）

**在此运行时上可用，但有一个细微依赖。** Kanban 分发器将每个 worker 生成为独立的 `flash chat -q` 子进程，该子进程读取用户配置——这意味着如果全局设置了 `model.openai_runtime: codex_app_server`，worker 也会在 Codex 运行时上启动。
...
- 用于 browser_*、vision、image_gen、技能、TTS 的 Flash 工具回调
...
|  | Flash 默认运行时 | Codex app-server（可选启用） |
|---|---|---|
...
2. **Codex OAuth 登录。** Codex 子进程读取 `~/.codex/auth.json`。有两种方式填充它：
   ...
   ```
   Flash 自己的 `flash auth login codex` 写入 `~/.flash/auth.json`——那是独立的会话。**如果你还没有运行过 `codex login`，请单独运行它。**

3. **（可选）安装你想要的 Codex 插件。** 启用运行时时，Flash 会自动迁移你已通过 Codex CLI 安装的所有精选插件：
   ```bash
   codex plugin marketplace add openai-curated
   # 然后通过 Codex 的 TUI 安装 Linear / GitHub / Gmail 等
   ```
   Flash 会自动发现它们并将 `[plugins."<name>@openai-curated"]` 条目写入 `~/.codex/config.toml`。
...
- 将用户 MCP server 从 `~/.flash/config.yaml` 迁移到 `~/.codex/config.toml`。
- **发现并迁移已安装的原生 Codex 插件**（Linear、GitHub、Gmail、Calendar、Canva 等），通过查询 Codex 的 `plugin/list` RPC 实现。
- **将 Flash 自身的工具注册为 MCP server**，以便 Codex 子进程能够回调获取 Codex 未内置的工具。
...
你也可以在 `~/.flash/config.yaml` 中手动设置：
```yaml
model:
  openai_runtime: codex_app_server   # 默认值为 "auto"（= Flash 运行时）
```

## 自我改进循环（记忆 + 技能提示）

Flash 的后台自我改进在计数器达到阈值时触发：

- 每 10 个用户 prompt（提示词）→ 一个分叉的审查 agent 查看对话，决定是否有内容应保存到记忆中。
- 单次轮次内每 10 次工具迭代 → 同样的逻辑，但针对技能（`skill_manage` 写入）。

**两者在 Codex 运行时上均持续生效。** Codex 路径将每个已完成的 `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` 事件项投影为合成的 `assistant tool_call` + `tool` 结果消息，因此审查运行时看到的格式与在默认 Flash 运行时上看到的相同。
...
一个细节：审查分叉本身需要调用 Flash 的 agent 循环工具（`memory`、`skill_manage`），这需要 Flash 自身的分发。因此，当父 agent 处于 `codex_app_server` 时，审查分叉会**降级为 `codex_responses`**——相同的 OAuth 凭据，相同的 `openai-codex` 提供商，但直接与 OpenAI 的 Responses API 通信，使 Flash 拥有循环控制权，agent 循环工具得以正常工作。这对用户不可见。

最终效果：启用 Codex 运行时后，你的记忆 + 技能提示计数器与之前完全一样持续触发。

## 审批流程

Codex 在执行命令或应用 patch 之前会请求审批。这些请求会被转换为 Flash 标准的"危险命令"提示：
...
对于 `apply_patch`（文件编辑）审批，当 Codex 通过对应的 `fileChange` 事件项提供数据时，Flash 会显示变更摘要（`1 add, 1 update: /tmp/new.py, /tmp/old.py`）。

## 权限配置文件

Codex 有三个内置权限配置文件：
- `:read-only` — 禁止写入；每条 shell 命令都需要审批
- `:workspace` — 允许在当前工作区内写入而无需提示（启用运行时时 Flash 的默认值）
- `:danger-no-sandbox` — 完全不使用沙箱（除非你清楚其含义，否则不要使用）

你可以在 Flash 管理块之外的 `~/.codex/config.toml` 中覆盖默认值：

```toml
default_permissions = ":read-only"
```

（只要你的覆盖配置位于 `# managed by flash-agent` 标记之外，Flash 在重新迁移时会保留它。）

## 辅助任务与 ChatGPT 订阅 token 消耗

当此运行时与 `openai-codex` 提供商一起开启时，**辅助任务（标题生成、上下文压缩、视觉自动检测、后台自我改进审查分叉）默认也会通过你的 ChatGPT 订阅流转**，因为 Flash 的辅助客户端在没有设置每任务覆盖时使用主提供商/模型。

这并非 `codex_app_server` 特有——现有的 `codex_responses` 路径也是如此——但在这里更为明显，因为你是在明确选择订阅计费。

要将特定辅助任务路由到更便宜/不同的模型，请在 `~/.flash/config.yaml` 中设置显式覆盖：
...
自我改进审查分叉通过 `_current_main_runtime()` 继承主运行时，Flash 会自动将其从 `codex_app_server` 降级为 `codex_responses`（以便分叉能够实际调用 `memory` 和 `skill_manage`——Flash 自身的 agent 循环工具）。除非你已将辅助任务路由到其他地方，否则该分叉仍使用你的订阅认证。

## 安全编辑 `~/.codex/config.toml`

Flash 将其管理的所有内容包裹在两个标记注释之间：
...
默认情况下，无论哪个 Flash 配置文件处于活跃状态，Flash 都将 Codex 子进程指向 `~/.codex/`。这意味着 `flash -p work` 和 `flash -p personal` 共享相同的 Codex 认证、插件和配置。对大多数用户来说这是正确的行为——与直接运行 `codex` CLI 的效果一致。

如果你需要按配置文件隔离 Codex（独立的认证、独立的已安装插件、独立的配置），请为每个配置文件显式设置 `CODEX_HOME`。最简洁的方式是指向你 `HERMES_HOME` 下的某个目录：

```bash
# 在 work 配置文件中，你可以这样包装 flash：
CODEX_HOME=~/.flash/profiles/work/codex flash chat
```

你需要在设置了该 `CODEX_HOME` 的情况下重新运行一次 `codex login`，以便 OAuth token 落入配置文件范围的位置。之后，`flash -p work` 将在隔离的 Codex 状态下运行。

我们不自动限定此范围，因为移动现有用户的 `~/.codex/` 会静默地使其 Codex CLI 认证失效——任何已运行过 `codex login` 的用户都需要重新认证。选择加入比给用户带来意外更安全。

## HOME 环境变量透传

Flash 在生成 Codex app-server 子进程时**不会**重写 `HOME`（我们使用 `os.environ.copy()`，仅覆盖 `CODEX_HOME` 和 `RUST_LOG`）。这意味着：
...
Flash 的 `mcp_servers` 配置会自动转换为 Codex 所需的 TOML 格式。迁移在每次启用运行时时运行，且是幂等的——重新运行会替换管理块，但保留用户编辑的 Codex 配置。
...
未迁移的内容：
- Flash 特有的键，如 `sampling`（Codex 的 MCP 客户端没有等效项——这些会被丢弃并附带每个 server 的警告）。

## 原生 Codex 插件迁移

通过 `codex plugin` 安装的插件（Linear、GitHub、Gmail、Calendar、Canva 等）通过 Codex 的 `plugin/list` RPC 被发现。对于每个 `installed: true` 的插件，Flash 会写入一个 `[plugins."<name>@openai-curated"]` 块，在你的 Flash 会话中启用它。

这意味着：当你的朋友说"我在 Codex CLI 中设置了 Calendar 和 GitHub"，他们启用 Flash 的 Codex 运行时后，Flash 会自动激活这些插件。无需重新配置。
...
- 插件 OAuth——你在 Codex 本身中对每个插件授权一次；Flash 不接触凭据。

## Flash 工具回调（新 MCP server）

Codex 的内置工具集涵盖 shell/文件操作/patch，但没有网络搜索、浏览器自动化、视觉、图像生成等功能。为了在 Codex 轮次中保持这些工具可用，Flash 在 `~/.codex/config.toml` 中将自身注册为 MCP server：
...
当模型调用 `web_search`（或其他暴露的 Flash 工具）时，Codex 通过 stdio 生成 `flash_tools_mcp_server` 子进程，请求通过 `model_tools.handle_function_call()` 分发，结果像其他 MCP 响应一样投影回 Codex。
...
**不可用的工具：** `delegate_task`、`memory`、`session_search`、`todo`。这些工具需要运行中的 AIAgent 上下文（循环中间状态）才能分发，无状态的 MCP 回调无法驱动它们。需要这些工具时，请使用默认 Flash 运行时（`/codex-runtime auto`）。
...
此运行时为**可选启用的 beta 功能**。以下功能在 Flash Agent 2026.5 + Codex CLI 0.130.0 上已验证可用：

- 多轮对话
- 通过 Flash UI 进行 `commandExecution` 和 `fileChange`（apply_patch）审批
- MCP 工具调用（已针对 `@modelcontextprotocol/server-filesystem` 和新的 `flash-tools` 回调验证）
- 原生 Codex 插件迁移（已针对 Linear / GitHub / Calendar 清单验证）
- 拒绝/取消路径
- 开关切换循环
- 记忆和技能提示计数器（已通过集成测试实时验证）
- 通过 Codex 使用 Flash web_search（已实时验证："OpenAI Codex CLI – Getting Started" 端到端返回结果）

已知限制：

- **Flash 认证和 Codex 认证是独立的会话。** 为获得最佳体验，你需要同时运行 `codex login` 和 `flash auth login codex`（运行时使用 Codex 的会话进行 LLM 调用）。这是 Flash `_import_codex_cli_tokens` 中的有意设计——Flash 不会与 Codex CLI 共享 OAuth 状态，以避免在 token 刷新时相互覆盖。
- **`delegate_task`、`memory`、`session_search`、`todo` 在此运行时上不可用。** 它们需要运行中的 AIAgent 上下文，无状态的 MCP 回调无法提供。需要这些工具时，请使用 `/codex-runtime auto`。
- **当 Codex 未跟踪变更集时，审批提示中没有内联 patch 预览。** Codex 的 `fileChange` 审批参数并不总是携带变更集。Flash 会尽可能从对应的 `item/started` 通知中缓存数据，但如果审批在事件项流式传输完成之前到达，提示会回退到 Codex 提供的 `reason`。
- **亚秒级取消无法保证。** 流式传输中途的中断（Codex 响应时按 Ctrl+C）通过 `turn/interrupt` 发送，但如果 Codex 已经刷新了最终消息，你仍会收到该响应。

如果你发现 bug，请[提交 issue](https://github.com/FlashOrg/flash-agent/issues)，附上 `flash logs --since 5m` 的输出。在标题中注明 `codex-runtime` 以便于分类处理。
...
有关实现细节，请参阅 [PR #24182](https://github.com/FlashOrg/flash-agent/pull/24182) 和 [Codex app-server 协议 README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md)。