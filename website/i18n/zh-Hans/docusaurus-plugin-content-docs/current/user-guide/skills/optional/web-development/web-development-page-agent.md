...
- 用户希望 **Flash 本身驱动浏览器** → 使用 Flash 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
