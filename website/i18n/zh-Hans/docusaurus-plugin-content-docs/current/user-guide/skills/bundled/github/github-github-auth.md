...
- 点击"New SSH key"
- 粘贴公钥内容
- 填写标题，如"flash-agent-&lt;machine-name>"
...
elif [ -f ~/.flash/.env ] && grep -q "^GITHUB_TOKEN=" ~/.flash/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.flash/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
