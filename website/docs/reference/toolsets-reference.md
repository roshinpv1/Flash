...
description: "Reference for Flash core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `flash-cli` is the default for interactive CLI sessions)
...
```bash
flash chat --toolsets web,file,terminal
flash chat --toolsets debugging        # composite — expands to file + terminal + web
flash chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - flash-cli          # default for CLI
  # - flash-telegram   # override for Telegram gateway
```

### Interactive management

```bash
flash tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Flash tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `flash-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `flash-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `flash-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `flash tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `flash-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `flash-cli`:

| Toolset | Differences from `flash-cli` |
|---------|-------------------------------|
| `flash-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, and clarify, plus the `safe` (read-only) bundle. |
| `flash-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `flash-api-server` | Drops `clarify` and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `flash-cron` | Same as `flash-cli`. |
| `flash-telegram` | Same as `flash-cli`. |
| `flash-discord` | Adds `discord` and `discord_admin` on top of `flash-cli`. |
| `flash-slack` | Same as `flash-cli`. |
| `flash-whatsapp` | Same as `flash-cli`. |
| `flash-signal` | Same as `flash-cli`. |
| `flash-matrix` | Same as `flash-cli`. |
| `flash-mattermost` | Same as `flash-cli`. |
| `flash-email` | Same as `flash-cli`. |
| `flash-sms` | Same as `flash-cli`. |
| `flash-bluebubbles` | Same as `flash-cli`. |
| `flash-dingtalk` | Same as `flash-cli`. |
| `flash-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `flash-qqbot` | Same as `flash-cli`. |
| `flash-wecom` | Same as `flash-cli`. |
| `flash-wecom-callback` | Same as `flash-cli`. |
| `flash-weixin` | Same as `flash-cli`. |
| `flash-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `flash-cli`. |
| `flash-homeassistant` | Same as `flash-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `flash-webhook` | Same as `flash-cli`. |
| `flash-gateway` | Internal gateway orchestrator toolset — union of every `flash-<platform>` toolset; used when the gateway needs to accept any message source. |
...
## Relationship to `flash tools`

The `flash tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
