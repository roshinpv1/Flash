...
of fields, and Flash schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Flash asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.flash.blueprint` block in its
