...
The Portal's own [model info page](https://portal.flashorg.com/info) carries the same warning, so this isn't a Hermes-side opinion — it's the official guidance from Nous Research.
...
If a model is genuinely missing, [open an issue](https://github.com/FlashOrg/hermes-agent/issues) — we surface the Portal's catalog to Hermes and gaps usually mean a routing config we can update.
