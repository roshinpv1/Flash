...
If you only run one Nyxo agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Nyxo agents that should all
...
(`ai.nyxo.gateway-<name>.plist`) or systemd user service
(`nyxo-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
nyxo profile create coder
nyxo profile create personal-bot
nyxo profile create research
...
Equivalently, in the default profile's `~/.nyxo/config.yaml`:
...
You do **not** run `nyxo gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `nyxo gateway start` / `run` is a
...
home). `nyxo status` reports the multiplexer and the profiles it serves;
`nyxo status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/nyxo-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: nyxo-gateways {start|stop|restart|status|list}"
}
...
```bash
nyxo-gateways start      # start every configured profile
nyxo-gateways stop       # stop every configured profile
nyxo-gateways restart    # restart all
nyxo-gateways status     # status across all
nyxo-gateways list       # delegates to `nyxo gateway list`
```

:::tip
The `default` profile is targeted with `nyxo gateway <action>` (no `-p`),
not `nyxo -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `nyxo -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.nyxo.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/nyxo-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.nyxo.gateway.plist` /
`nyxo-gateway.service`.
...
```bash
# Default profile
tail -f ~/.nyxo/logs/gateway.log
tail -f ~/.nyxo/logs/gateway.error.log

# Named profile
tail -f ~/.nyxo/profiles/<name>/logs/gateway.log
tail -f ~/.nyxo/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.nyxo/logs/gateway.log ~/.nyxo/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
nyxo logs -f                  # follow default profile
nyxo -p coder logs -f         # follow one profile
nyxo logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
nyxo profile list             # profiles + model + gateway state
nyxo-gateways status          # full status across every profile
launchctl list | grep nyxo    # macOS — PIDs and labels
systemctl --user list-units 'nyxo-gateway-*'   # Linux — units
```
...
The default profile uses `~/.nyxo/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
nyxo config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.nyxo/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=nyxo --why="gateways running" \
  sleep infinity &
...
`nyxo-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.nyxo/.env ~/.nyxo/profiles/*/.env
```

## Updating the code

`nyxo update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `nyxo gateway start` after a previous `nyxo gateway stop`. The
...
```bash
ps -ef | grep "nyxo_cli.*-p <profile>"
cat ~/.nyxo/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.nyxo.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.nyxo.gateway-<profile>.plist

# Linux
systemctl --user restart nyxo-gateway-<profile>.service
```

### Health check

```bash
nyxo doctor                  # default profile
nyxo -p <profile> doctor     # one profile
```
