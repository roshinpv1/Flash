...
If you only run one Flash agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Flash agents that should all
...
(`ai.flash.gateway-<name>.plist`) or systemd user service
(`flash-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
flash profile create coder
flash profile create personal-bot
flash profile create research
...
Equivalently, in the default profile's `~/.flash/config.yaml`:
...
You do **not** run `flash gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `flash gateway start` / `run` is a
...
home). `flash status` reports the multiplexer and the profiles it serves;
`flash status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/flash-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: flash-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    flash gateway "$action"
  else
    flash -p "$profile" gateway "$action"
  fi
}
...
```bash
flash-gateways start      # start every configured profile
flash-gateways stop       # stop every configured profile
flash-gateways restart    # restart all
flash-gateways status     # status across all
flash-gateways list       # delegates to `flash gateway list`
```

:::tip
The `default` profile is targeted with `flash gateway <action>` (no `-p`),
not `flash -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `flash -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.flash.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/flash-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.flash.gateway.plist` /
`flash-gateway.service`.
...
```bash
# Default profile
tail -f ~/.flash/logs/gateway.log
tail -f ~/.flash/logs/gateway.error.log

# Named profile
tail -f ~/.flash/profiles/<name>/logs/gateway.log
tail -f ~/.flash/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.flash/logs/gateway.log ~/.flash/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
flash logs -f                  # follow default profile
flash -p coder logs -f         # follow one profile
flash logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
flash profile list             # profiles + model + gateway state
flash-gateways status          # full status across every profile
launchctl list | grep flash    # macOS — PIDs and labels
systemctl --user list-units 'flash-gateway-*'   # Linux — units
```
...
The default profile uses `~/.flash/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
flash config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.flash/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=flash --why="gateways running" \
  sleep infinity &
...
`flash-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.flash/.env ~/.flash/profiles/*/.env
```

## Updating the code

`flash update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `flash gateway start` after a previous `flash gateway stop`. The
...
```bash
ps -ef | grep "flash_cli.*-p <profile>"
cat ~/.flash/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.flash.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.flash.gateway-<profile>.plist

# Linux
systemctl --user restart flash-gateway-<profile>.service
```

### Health check

```bash
flash doctor                  # default profile
flash -p <profile> doctor     # one profile
```
