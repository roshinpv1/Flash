...
Flash Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Flash checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.flash/config.yaml`:
...
| `timeout` | `60` | Seconds Flash waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `flash --yolo` or `flash chat --yolo`
...
When YOLO is active, Flash shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Flash refuses to run them **regardless** of:
...
Configure the timeout in `~/.flash/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.flash/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.flash/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` flash/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.flash/config.yaml`:
...
:::tip
Use `flash config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Flash controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.flash/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.flash/.env to allow open access,
...
For more flexible authorization, Flash includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `flash pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.flash/config.yaml`:
...
flash pairing approve telegram ABC12DEF

# Revoke a user's access
flash pairing revoke telegram 123456789

# Clear all pending codes
flash pairing clear-pending
```

:::tip Docker users: run pairing commands as the `flash` user
The official Docker image runs the gateway as the unprivileged `flash` user
...
```bash
docker exec -u flash flash-agent flash pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/FlashOrg/flash-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.flash/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Flash applies strict security hardening to every container.
...
Container resources are configurable in `~/.flash/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.flash/sandboxes/docker/<task_id>/`
...
When loaded, Flash checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Flash infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Flash infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Flash integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Flash under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.flash/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.flash/logs/` for unauthorized access attempts
10. **Keep updated** — run `flash update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.flash/.env`:
...
TERMINAL_SSH_USER=flash
TERMINAL_SSH_KEY=~/.ssh/flash_agent_key
```
...
Flash ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `flash_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `flash doctor` for the full remediation.
- **`flash doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Flash installs these **lazily** on first use rather than eagerly under `flash-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `flash tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `flash tools`.
