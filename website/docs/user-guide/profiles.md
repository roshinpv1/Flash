...
Run multiple independent Flash agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Flash home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Flash state.
...
```bash
flash profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Flash profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `flash setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
flash profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `flash profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.flash/profiles/work/.env` for different API keys, or `~/.flash/profiles/work/SOUL.md` for a different personality.
...
```bash
flash profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every flash subcommand — it's just `flash -p <name>` under the hood.
...
```bash
flash -p coder chat
flash --profile=coder doctor
flash chat -p coder -q "hello"    # works in any position
```

### Sticky default (`flash profile use`)

```bash
flash profile use coder
flash chat                   # now targets coder
flash tools                  # configures coder's tools
flash profile use default    # switch back
```

Sets a default so plain `flash` commands target that profile. Like `kubectl config use-context`.
...
- **`flash profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Flash its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Flash was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.flash/profiles/coder/.env

# Edit assistant's tokens
nano ~/.flash/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates flash-gateway-coder systemd/launchd service
assistant gateway install     # creates flash-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `flash profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `flash -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.flash/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `flash profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`flash update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
flash profile list           # show all profiles with status
flash profile show coder     # detailed info for one profile
flash profile rename coder dev-bot   # rename (updates alias + service)
flash profile export coder   # export to coder.tar.gz
flash profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
flash profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `flash profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.flash`). To remove everything, use `flash uninstall`.
:::
...
```bash
# Bash
eval "$(flash completion bash)"

# Zsh
eval "$(flash completion zsh)"
```
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.flash/profiles/coder` before launching flash. Since 119+ files in the codebase resolve paths via `get_flash_home()`, Flash state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `HERMES_HOME` is the profile boundary. It controls Flash config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Flash
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Flash keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Flash launches tool
...
Flash also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.flash` itself. No migration needed — existing installs work identically.
