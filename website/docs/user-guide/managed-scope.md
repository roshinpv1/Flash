...
`~/.flash/config.yaml`, `~/.flash/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/flash`:

```text
/etc/flash/
├── config.yaml     # managed config layer (wins over ~/.flash/config.yaml)
└── .env            # managed env layer (wins over ~/.flash/.env + shell)
```
...
files. It is **never persisted** to any `.env` by Flash.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export HERMES_MANAGED_DIR=/opt/org/flash-policy
```
...
left user-settable. `flash doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/flash/config.yaml` (managed) | `/etc/flash/.env` (managed) |
| 2 | `~/.flash/config.yaml` (user) | `~/.flash/.env` (user) |
...
```bash
flash config        # shows a header naming the managed source + the pinned keys
flash doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Flash refuses and names the source:

```bash
$ flash config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/flash/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `flash config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/flash
...
sudo tee /etc/flash/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/flash
sudo chmod 0644 /etc/flash/config.yaml /etc/flash/.env
```

Changes take effect on the next Flash start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`flash doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Flash as `root`), managed scope is advisory.
