...
`~/.nyxo/config.yaml`, `~/.nyxo/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/nyxo`:

```text
/etc/nyxo/
├── config.yaml     # managed config layer (wins over ~/.nyxo/config.yaml)
└── .env            # managed env layer (wins over ~/.nyxo/.env + shell)
```
...
The location can be relocated with the `NYXO_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `NYXO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Nyxo.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export NYXO_MANAGED_DIR=/opt/org/nyxo-policy
```

:::warning
A user who can set `NYXO_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `nyxo doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/nyxo/config.yaml` (managed) | `/etc/nyxo/.env` (managed) |
| 2 | `~/.nyxo/config.yaml` (user) | `~/.nyxo/.env` (user) |
...
```bash
nyxo config        # shows a header naming the managed source + the pinned keys
nyxo doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Nyxo refuses and names the source:

```bash
$ nyxo config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/nyxo/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `nyxo config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/nyxo

# Pin some config values for every user on this machine
sudo tee /etc/nyxo/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/nyxo/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/nyxo
sudo chmod 0644 /etc/nyxo/config.yaml /etc/nyxo/.env
```

Changes take effect on the next Nyxo start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`nyxo doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Nyxo as `root`), managed scope is advisory.
