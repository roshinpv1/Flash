...
:::tip
Inside the container, run `flash setup --portal` once — the refresh token persists in the mounted `~/.flash` volume. See [Nous Portal](/integrations/flash-portal).
:::
...
- **OAuth (Nous Portal)** — for hosted/public deploys: the `dashboard_auth/flash` provider activates whenever `HERMES_DASHBOARD_OAUTH_CLIENT_ID` is set.
