...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Flash Agent terminal interface — commands, keybindings, personalities, and more"
---
...
One command — `flash setup --portal` — and you're ready to `flash chat`. See [FlashPortal](/integrations/flash-portal).
:::

:::tip
Flash also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `flash --tui` — see the [TUI](tui.md) guide.
:::
...
flash chat --provider flash        # Use FlashPortal
...
flash -s flash-agent-dev,github-auth
flash chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
flash --continue             # Resume the most recent CLI session (-c)
flash --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
flash chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
flash -w                         # Interactive mode in worktree
flash -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Flash CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Flash CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `flash --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`flash -c` or `flash --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Flash to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Flash supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Flash replies |
...
```bash
flash -s flash-agent-dev,github-auth
flash chat -s github-pr-workflow -s github-auth
```

Flash loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.flash/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.flash/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Flash..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Flash recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Flash — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Flash is working, Flash prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Flash to the background — just like any terminal process. The shell prints a confirmation:

```
Flash Agent has been suspended. Run `fg` to bring Flash Agent back.
```
...
Resume this session with:
  flash --resume 20260225_143052_a1b2c3
...
```bash
flash --continue                          # Resume the most recent CLI session
flash -c                                  # Short form
flash -c "my project"                     # Resume a named session (latest in lineage)
flash --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
flash --resume "refactoring auth"         # Resume by title
flash -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `flash sessions rename <id> <title>` from the command line. Use `flash sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Flash's SQLite state database under `~/.flash/state.db`. The database keeps:
...
Flash immediately confirms the task and gives you back the prompt:
