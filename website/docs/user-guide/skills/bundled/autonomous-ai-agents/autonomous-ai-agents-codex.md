...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`nyxo-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nyxo-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Nyxo terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Nyxo itself, `model.provider: openai-codex` uses Nyxo-managed Codex
OAuth from `~/.nyxo/auth.json` after `nyxo auth add openai-codex`. For the
...
When invoking the Codex CLI from a Nyxo gateway/service context (for example,
