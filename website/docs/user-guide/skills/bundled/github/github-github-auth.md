...
- Give it a name like "flash-agent"
- Select scopes:
...
- Give it a title like "flash-agent-&lt;machine-name>"
...
elif _flash_env="${HERMES_HOME:-$HOME/.flash}/.env"; [ -f "$_flash_env" ] && grep -q "^GITHUB_TOKEN=" "$_flash_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_flash_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
