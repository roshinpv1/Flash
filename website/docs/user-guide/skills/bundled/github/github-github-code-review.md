...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _nyxo_env="${NYXO_HOME:-$HOME/.nyxo}/.env"; [ -f "$_nyxo_env" ] && grep -q "^GITHUB_TOKEN=" "$_nyxo_env"; then
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Nyxo Agent\",
    \"comments\": [
...
```bash
source "${NYXO_HOME:-$HOME/.nyxo}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Nyxo Agent. Code looks clean — good test coverage, no security concerns."
