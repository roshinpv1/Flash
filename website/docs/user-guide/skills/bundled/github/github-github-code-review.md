...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _flash_env="${HERMES_HOME:-$HOME/.flash}/.env"; [ -f "$_flash_env" ] && grep -q "^GITHUB_TOKEN=" "$_flash_env"; then
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Flash Agent\",
    \"comments\": [
...
```bash
source "${HERMES_HOME:-$HOME/.flash}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Flash Agent. Code looks clean — good test coverage, no security concerns."
