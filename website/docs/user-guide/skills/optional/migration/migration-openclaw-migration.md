---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Flash Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Flash Agent"
---
...
Migrate a user's OpenClaw customization footprint into Flash Agent. Imports Flash-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Related skills | [`flash-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-flash-agent) |
...
# OpenClaw -> Flash Migration

Use this skill when a user wants to move their OpenClaw setup into Flash Agent with minimal manual cleanup.
...
```bash
flash claw migrate              # Full interactive migration
...
flash claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `flash setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_flash.py` to:

- import `SOUL.md` into the Flash home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Flash memory entries
- merge OpenClaw command approval patterns into Flash `command_allowlist`
- migrate Flash-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Flash working-directory configuration
- copy OpenClaw skills into `~/.flash/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Flash workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.flash/tts/`
- archive non-secret docs that do not have a direct Flash destination
...
- `scripts/openclaw_to_flash.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py`

Do not guess a shorter path like `~/.flash/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.flash/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Flash-compatible secrets, currently:
...
Flash CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Flash. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Flash workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Flash skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.flash/skills/migration/openclaw-migration/scripts/openclaw_to_flash.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Flash unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Flash targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Flash destination. Unsupported auth blobs must still be reported as skipped.
...
- Flash persona state imported
- Flash memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.flash/skills/openclaw-imports/`
