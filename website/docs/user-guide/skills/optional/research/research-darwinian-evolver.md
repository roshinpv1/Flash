...
| | |
|---|---|
| Source | Optional — install with `nyxo skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Nyxo Agent |
| License | MIT |
...
import upstream classes into Nyxo itself.
...
```bash
mkdir -p ~/.nyxo/cache/darwinian-evolver && cd ~/.nyxo/cache/darwinian-evolver
...
```bash
cd ~/.nyxo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.nyxo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.nyxo/skills/research/darwinian-evolver
DE_DIR=~/.nyxo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Nyxo core.
   Custom driver scripts under `~/.nyxo/skills/...` are user-side and fine.
