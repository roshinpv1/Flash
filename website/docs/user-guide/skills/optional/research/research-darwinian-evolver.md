...
| | |
|---|---|
| Source | Optional — install with `flash skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Flash Agent |
| License | MIT |
...
import upstream classes into Flash itself.
...
```bash
mkdir -p ~/.flash/cache/darwinian-evolver && cd ~/.flash/cache/darwinian-evolver
...
```bash
cd ~/.flash/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.flash/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.flash/skills/research/darwinian-evolver
DE_DIR=~/.flash/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Flash core.
   Custom driver scripts under `~/.flash/skills/...` are user-side and fine.
