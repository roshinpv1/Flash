...
| | |
|---|---|
| Source | Optional — install with `nyxo skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Nyxo Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Nyxo terminal calls (desktop app flow only)
...
### Service Account (recommended for Nyxo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${NYXO_HOME:-~/.nyxo}/.env` (the skill will prompt for this on first load).
...
## Nyxo Execution Pattern (desktop app flow)

Nyxo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nyxo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nyxo-op.sock"
