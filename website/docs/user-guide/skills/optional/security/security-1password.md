...
| | |
|---|---|
| Source | Optional — install with `flash skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Flash Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Flash terminal calls (desktop app flow only)
...
### Service Account (recommended for Flash)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HERMES_HOME:-~/.flash}/.env` (the skill will prompt for this on first load).
...
## Flash Execution Pattern (desktop app flow)

Flash terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/flash-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/flash-op.sock"
