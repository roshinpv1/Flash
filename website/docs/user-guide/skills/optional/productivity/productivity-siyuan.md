...
| | |
|---|---|
| Source | Optional — install with `flash skills install official/productivity/siyuan` |
...
3. Store it in `${HERMES_HOME:-~/.flash}/.env`:
   ```
...
```yaml
# In ~/.flash/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
