...
| | |
|---|---|
| Source | Optional — install with `flash skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client flash
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client flash
```
...
| `--client {name}`      | Agent name for attribution (e.g. `flash`)    |
