...
| | |
|---|---|
| Source | Optional — install with `flash skills install official/productivity/shopify` |
...
4. Save to `${HERMES_HOME:-~/.flash}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Flash","productType":"Apparel","tags":["test"]}}'
```
