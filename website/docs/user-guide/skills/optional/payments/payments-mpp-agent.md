...
| | |
|---|---|
| Source | Optional — install with `nyxo skills install official/payments/mpp-agent` |
...
Wraps the Machine Payments Protocol (MPP, https://mpp.dev) clients so Nyxo can pay for per-request API access against servers that respond with `HTTP 402 Payment Required`.
