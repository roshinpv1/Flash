...
| | |
|---|---|
| Source | Optional — install with `nyxo skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Nyxo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Nyxo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nyxo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nyxo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Nyxo integration, either:

- configure the server in `~/.nyxo/config.yaml` using the `native-mcp` skill, or
...
### Nyxo cannot see the deployed server

The server-building part may be correct while the Nyxo config is not. Load the `native-mcp` skill and configure the server in `~/.nyxo/config.yaml`, then restart Nyxo.
