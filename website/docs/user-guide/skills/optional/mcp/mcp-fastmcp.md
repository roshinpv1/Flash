...
| | |
|---|---|
| Source | Optional — install with `flash skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Flash or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Flash. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.flash/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.flash/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Flash integration, either:

- configure the server in `~/.flash/config.yaml` using the `native-mcp` skill, or
...
### Flash cannot see the deployed server

The server-building part may be correct while the Flash config is not. Load the `native-mcp` skill and configure the server in `~/.flash/config.yaml`, then restart Flash.
