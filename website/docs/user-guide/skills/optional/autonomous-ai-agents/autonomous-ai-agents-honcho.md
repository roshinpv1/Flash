...
description: "Configure and use Honcho memory with Nyxo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Nyxo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nyxo skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Nyxo

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Nyxo profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/nyxo#running-honcho-locally-with-nyxo

### Verify

```bash
nyxo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Nyxo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Nyxo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Nyxo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Nyxo run |
| `global` | Single session across all directories |

Manual override: `nyxo honcho map my-project-name`
...
Each Nyxo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nyxo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nyxo.coder` host block in `honcho.json`
...
```bash
nyxo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Nyxo when Honcho memory is active.
...
Config file: `$NYXO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nyxo honcho setup`. Ensure `memory.provider: honcho` is in `~/.nyxo/config.yaml`.

### Memory not persisting across sessions
Check `nyxo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nyxo profile create <name> --clone`. For existing profiles: `nyxo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nyxo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nyxo honcho status` | Show resolved config, connection test, peer info for active profile |
| `nyxo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nyxo honcho disable` | Disable Honcho for the active profile |
| `nyxo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nyxo honcho peers` | Show peer identities across all profiles |
| `nyxo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nyxo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nyxo honcho sessions` | List known directory-to-session-name mappings |
| `nyxo honcho map <name>` | Map current working directory to a Honcho session name |
| `nyxo honcho identity` | Seed AI peer identity or show both peer representations |
| `nyxo honcho sync` | Create host blocks for all Nyxo profiles that don't have one yet |
| `nyxo honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Nyxo + Honcho |
| `nyxo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nyxo memory status` | Show active memory provider and config |
| `nyxo memory off` | Disable external memory provider |
