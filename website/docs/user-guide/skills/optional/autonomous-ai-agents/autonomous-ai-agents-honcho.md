...
description: "Configure and use Honcho memory with Flash -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Flash -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `flash skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Flash

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Flash profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/flash#running-honcho-locally-with-flash

### Verify

```bash
flash honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Flash profile's AI peer
...
Honcho models conversations as interactions between **peers**. Flash creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Flash instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Flash run |
| `global` | Single session across all directories |

Manual override: `flash honcho map my-project-name`
...
Each Flash profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block flash.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `flash.coder` host block in `honcho.json`
...
```bash
flash honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Flash when Honcho memory is active.
...
Run `flash honcho setup`. Ensure `memory.provider: honcho` is in `~/.flash/config.yaml`.

### Memory not persisting across sessions
Check `flash honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `flash profile create <name> --clone`. For existing profiles: `flash honcho sync`.
...
| Command | Description |
|---------|-------------|
| `flash honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `flash honcho status` | Show resolved config, connection test, peer info for active profile |
| `flash honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `flash honcho disable` | Disable Honcho for the active profile |
| `flash honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `flash honcho peers` | Show peer identities across all profiles |
| `flash honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `flash honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `flash honcho sessions` | List known directory-to-session-name mappings |
| `flash honcho map <name>` | Map current working directory to a Honcho session name |
| `flash honcho identity` | Seed AI peer identity or show both peer representations |
| `flash honcho sync` | Create host blocks for all Flash profiles that don't have one yet |
| `flash honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Flash + Honcho |
| `flash memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `flash memory status` | Show active memory provider and config |
| `flash memory off` | Disable external memory provider |
