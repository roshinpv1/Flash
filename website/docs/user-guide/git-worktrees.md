...
title: "Git Worktrees"
description: "Run multiple Flash agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Flash Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Flash so each session has a clean, isolated working directory.

## Why Use Worktrees with Flash?

Flash treats the **current working directory** as the project root:

- CLI: the directory where you run `flash` or `flash chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.flash/config.yaml`
...
git worktree add ../repo-feature feature/flash-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/flash-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Flash there:

```bash
cd ../repo-feature

# Start Flash in the worktree
flash
```
...
git worktree add ../repo-experiment-a feature/flash-a
git worktree add ../repo-experiment-b feature/flash-b
```
...
- Works on its own branch (`feature/flash-a` vs `feature/flash-b`).
...
- Flash checkpoint data under `~/.flash/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Flash experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/flash-checkpoints-docs`, `feature/flash-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Flash from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `flash -w` (Automatic Worktree Mode)

Flash has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `flash/flash-<hash>`).
...
```bash
flash -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `flash -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Flash session its own clean checkout.
