...
title: "Git Worktrees"
description: "Run multiple Nyxo agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Nyxo Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Nyxo so each session has a clean, isolated working directory.

## Why Use Worktrees with Nyxo?

Nyxo treats the **current working directory** as the project root:

- CLI: the directory where you run `nyxo` or `nyxo chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.nyxo/config.yaml`
...
git worktree add ../repo-feature feature/nyxo-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nyxo-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Nyxo there:

```bash
cd ../repo-feature

# Start Nyxo in the worktree
nyxo
```
...
git worktree add ../repo-experiment-a feature/nyxo-a
git worktree add ../repo-experiment-b feature/nyxo-b
```
...
- Works on its own branch (`feature/nyxo-a` vs `feature/nyxo-b`).
...
- Nyxo checkpoint data under `~/.nyxo/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Nyxo experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nyxo-checkpoints-docs`, `feature/nyxo-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Nyxo from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nyxo -w` (Automatic Worktree Mode)

Nyxo has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nyxo/nyxo-<hash>`).
...
```bash
nyxo -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nyxo -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Nyxo session its own clean checkout.
