...
The `execute_code` tool lets the agent write Python scripts that call Flash tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Flash over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from flash_tools import ...`
2. Flash generates a `flash_tools.py` stub module with RPC functions
3. Flash opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Flash
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from flash_tools import web_search, web_extract
...
```python
from flash_tools import search_files, read_file
import json
...
```python
from flash_tools import search_files, read_file, patch
...
```python
from flash_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.flash/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Flash's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Flash's own python) |
...
(plus `HERMES_RPC_DIR` / `HERMES_RPC_SOCKET` / `TZ` / `HOME`, which Flash
...
the secret-stripping guarantee (Flash-managed provider credentials can never
...
variables, Flash emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`flash logs --level
DEBUG`, or check `~/.flash/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted HERMES_* var(s)` if a script behaves
as though a `HERMES_*` variable is missing.

Flash always writes the script and the auto-generated `flash_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Flash tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
