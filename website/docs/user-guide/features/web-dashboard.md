...
:::tip
Hosted-mode auth uses FlashPortal OAuth; if you also want the dashboard to talk to a real backend, `flash setup --portal` wires up the model and tool gateway too. See [FlashPortal](/integrations/flash-portal).
:::
...
```bash
# Custom port
flash dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
flash dashboard --host 0.0.0.0

# Start without opening browser
flash dashboard --no-open
```
...
processes (manage them via `flash -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `flash-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.flash/flash-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.flash/flash-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `flash dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `flash dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `flash --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.flash/flash-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.flash/.env
ExecStart=/path/to/venv/bin/python -m flash_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.flash/.env` containing:
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `flash config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Flash instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `flash profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.flash/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `flash skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `flash mcp` reads from.
...
**Catalog:** browse the Flash-approved MCP servers (the bundled `optional-mcps/`
...
`flash mcp catalog` / `flash mcp install` use.
...
Connect Flash to any messaging platform from the browser — full parity with
`flash setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.flash/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Flash versions, CPU core count + utilization, memory, disk usage of the Flash home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Flash version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `flash update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **FlashPortal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `flash portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `flash curator`.
...
![System admin page — host stats and FlashPortal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.flash/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/mcp/catalog` | Browse the Flash-approved MCP catalog |
...
| `GET /api/flash/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | FlashPortal auth + Tool Gateway routing (read-only) |
...
- **[OAuth (FlashPortal)](#default-provider-flash-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Flash Desktop connection](#connecting-flash-desktop-to-a-remote-backend). Every login is verified against your Flashaccount, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No FlashPortal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `flash dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `flash dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Flashplugin, no custom plugin), `flash dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
Because every login is verified against FlashPortal and protected by your Flashaccount, **the Flashprovider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Flashprovider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `flash dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Flashlogin (run `flash setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.flash/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
|---------|-----------|--------|----------------|
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `flash dashboard register` |

Per the Flash Agent convention (`~/.flash/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • flash: HERMES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The FlashPortal
...
From a logged-in Flash install to a Flash-gated dashboard in three steps.

**1. Log in and register the dashboard.** `flash dashboard register` uses your existing Flashlogin to provision an OAuth client and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.flash/.env` for you:

```bash
flash setup            # if you're not already logged into FlashPortal
flash dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.flash/.env
```
...
Like the Flashprovider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback/`--insecure` operators are unaffected.
...
**1. Set credentials in `~/.flash/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no FlashPortal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the Flashprovider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback / `--insecure` dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/flash/   # required
      client_id: flash-dashboard                              # required
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-flash.json` — it defines a `flash` realm, a **public PKCE client** (`flash-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "flash-dashboard",
      "name": "Flash Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-flash.json:/opt/keycloak/data/import/realm-flash.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/flash/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/flash`). The admin console is at
...
```bash
export HERMES_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/flash"
export HERMES_DASHBOARD_OIDC_CLIENT_ID="flash-dashboard"
...
> admin console (Clients → flash-dashboard → Settings). The same pattern works
...
```yaml
dashboard:
  public_url: "https://dashboard.example.com/flash"
```
...
The provider implements the [FlashPortal OAuth contract v1](https://github.com/FlashOrg/flash-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `flash_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `flash_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `flash_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
To plug a non-FlashOAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.flash/plugins/dashboard-auth-myidp/__init__.py
from flash_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# 1. Set the dashboard login credentials in ~/.flash/.env (secrets file, 0600).
cat >> ~/.flash/.env <<'EOF'
HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
flash dashboard --no-open --host 0.0.0.0 --port 9119
```
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/flash` are supported if you front it with a reverse proxy)
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `flash_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `flash update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `flash dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Flash Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Flash Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
