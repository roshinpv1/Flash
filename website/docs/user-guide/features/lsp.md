...
Nyxo runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Nyxo
...
1. Nyxo captures a baseline of current diagnostics for the file.
...
…). Nyxo auto-detects the binary on PATH or in
`<NYXO_HOME>/lsp/bin/`.
...
`node_modules` tree — Nyxo installs both packages together when you
run `nyxo lsp install typescript` or auto-install fires on first
use.

## CLI

```
nyxo lsp status          # service state + per-server install status
nyxo lsp list            # registry, optionally --installed-only
nyxo lsp install <id>    # eagerly install one server
nyxo lsp install-all     # try every server with a known recipe
nyxo lsp restart         # tear down running clients
nyxo lsp which <id>      # print resolved binary path
```

`nyxo lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <NYXO_HOME>/lsp/bin
...
When `install_strategy: auto`, Nyxo installs binaries into
`<NYXO_HOME>/lsp/bin/`. NPM packages land in
`<NYXO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Nyxo-owned and is
...
Servers are kept alive for the life of the Nyxo process. There's
...
**`nyxo lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<NYXO_HOME>/lsp/bin/`. Run
`nyxo lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `nyxo lsp status`**
...
When `nyxo lsp status` shows a `Backend warnings` section, install
...
Check `~/.nyxo/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `nyxo lsp restart` to clear the set;
