...
Flash can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), Flash renders the sprite at full fidelity when your
...
flash pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `flash pets list [query] [--limit N]` |
| List installed pets | `flash pets list --installed` |
| Install a pet | `flash pets install <slug> [--select] [--force]` |
| Set the active pet | `flash pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `flash pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `flash pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `flash pets off` |
| Remove an installed pet | `flash pets remove <slug>` |
| Diagnose setup | `flash pets doctor` |

`flash pets show` flags:
...
Beyond installing pre-made pets from the gallery, Flash can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `flash pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Flash state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **FlashPortal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Flashrun a quality-first model chain by default.

- Resolution order prefers FlashPortal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `flash tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
...
always-on-top desktop window. Out there it stays visible while Flash is
...
Run `flash pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Flash uses its own
  profile-scoped `<HERMES_HOME>/pets/` instead — install through `flash pets`.
