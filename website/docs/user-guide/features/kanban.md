...
description: "Durable SQLite-backed task board for coordinating multiple Flash profiles"
---
...
Flash Kanban is a durable task board, shared across all your Flash profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.flash/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.flash/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `flash kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `flash kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/flash-kanban-v1-spec.pdf` in the repository.
...
called `default` (DB at `~/.flash/kanban.db` for back-compat). Users who
only want one stream of work never need to know about boards; the feature
is opt-in.

Per-board isolation is absolute:

- Separate SQLite DB per board (`~/.flash/kanban/boards/<slug>/kanban.db`).
...
flash kanban boards list

# Create a new board.
flash kanban boards create atm10-server \
    --name "ATM10 Server" \
...
flash kanban --board atm10-server list
flash kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
flash kanban boards switch atm10-server
flash kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
flash kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.flash/kanban/current` — the slug persisted by `flash kanban
   boards switch`.
4. `default`.
...
`flash dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<flash-home>/kanban/attachments/<task_id>/` for the default board, or
  `<flash-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
...
flash kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `flash kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `flash gateway
...
until one comes up — `flash kanban create` warns about this at creation
time.

Running `flash kanban daemon` as a separate process is **deprecated**;
...
flash kanban create "nightly ops review" \
    --assignee ops \
...
```bash
flash kanban complete t_abc t_def t_hij --result "batch wrap"
flash kanban archive  t_abc t_def t_hij
flash kanban unblock  t_abc t_def
flash kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `flash kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `flash kanban` CLI.
...
### Why tools instead of shelling to `flash kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `flash kanban complete` *inside* the container, where `flash` isn't installed and `~/.flash/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.flash/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `flash chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/flash_cli/test_kanban_db.py -q"],
...
```bash
flash kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

flash kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `flash skills list` to see what's available); there's no runtime install.
...
```bash
flash kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `flash kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `flash kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `flash profile create --description "..."`, `flash profile describe <name> --text "..."`, `flash profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.flash/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `flash profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.flash/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `flash dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.flash/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `flash -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard Flash dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
flash kanban init                                     # create kanban.db + print daemon hint
flash kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
flash kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
flash kanban show <id> [--json]
flash kanban assign <id> <profile>                    # or 'none' to unassign
flash kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
flash kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
flash kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
flash kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
flash kanban diagnostics [--json]                     # board health snapshot (alias: diag)
flash kanban link <parent_id> <child_id>
flash kanban unlink <parent_id> <child_id>
flash kanban claim <id> [--ttl SECONDS]
flash kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
flash kanban complete <id>... [--result "..."]
flash kanban block <id> "<reason>" [--ids <id>...]
flash kanban unblock <id>...
flash kanban archive <id>...

flash kanban tail <id>                                # follow a single task's event stream
flash kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
flash kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
flash kanban runs <id> [--json]                       # attempt history (one row per run)
flash kanban assignees [--json]                       # profiles on disk + per-assignee task counts
flash kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
flash kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `flash gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
flash kanban stats [--json]                           # per-status + per-assignee counts
flash kanban log <id> [--tail BYTES]                  # worker log from ~/.flash/kanban/logs/
flash kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
flash kanban notify-list [<id>] [--json]
flash kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
flash kanban context <id>                             # what a worker sees
flash kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
flash kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
flash kanban create "nightly backup audit" \
...
Every `flash kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `flash chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `flash_cli.kanban.run_slash()` entry point that reuses the `flash kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `flash kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.flash/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`flash kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `flash kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/flash-kanban-v1-spec.pdf`.
...
```bash
flash kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
flash kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
flash kanban notify-list
flash kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
flash kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
flash kanban runs t_abcd
...
**Bulk close caveat.** `flash kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`flash kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `flash kanban heartbeat $TASK` to signal liveness during long operations. |
...
`flash kanban tail <id>` shows these for a single task. `flash kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.flash/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/flash-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
