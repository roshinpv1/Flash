...
All skills live in **`~/.flash/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Flash at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `flash update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.flash` profile):

```bash
curl -fsSL https://flash-agent.flashorg.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
flash profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
flash skills opt-out            # stop future seeding — nothing on disk is touched
flash skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
flash skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `flash update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `flash skills opt-in`) to re-enable.

:::note Safe by default
`flash skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Flash installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Flash to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.flash/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
flash chat --toolsets skills -q "What skills do you have?"
flash chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Flash-tool framing, no invented
commands).
...
/home/user/.flash/cache/chart-q4-2025.png
...
When a missing value is encountered, Flash asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `flash setup` or `~/.flash/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `flash config migrate` prompts for unconfigured settings, and `flash config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.flash/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Flash scans the
...
If you maintain skills outside of Flash — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Flash to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.flash/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.flash/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Flash process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Flash ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.flash/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.flash/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `flash bundles list`.
...
flash bundles create research

# Overwrite an existing bundle
flash bundles create backend-dev --skill ... --force

# Delete a bundle
flash bundles delete backend-dev

# Re-scan ~/.flash/skill-bundles/ and report changes
flash bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.flash/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.flash/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.flash/pending/skills/` and are
...
```bash
flash skills browse                              # Browse all hub skills (official first)
flash skills browse --source official            # Browse only official optional skills
flash skills search kubernetes                   # Search all sources
flash skills search react --source skills-sh     # Search the skills.sh directory
flash skills search https://mintlify.com/docs --source well-known
flash skills inspect openai/skills/k8s           # Preview before installing
flash skills install openai/skills/k8s           # Install with security scan
flash skills install official/security/1password
flash skills install skills-sh/vercel-labs/json-render/json-render-react --force
flash skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
flash skills list --source hub                   # List hub-installed skills
flash skills check                               # Check installed hub skills for upstream updates
flash skills update                              # Reinstall hub skills with upstream changes when needed
flash skills audit                               # Re-scan all hub skills for security
flash skills uninstall k8s                       # Remove a hub skill
flash skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
flash skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
flash skills publish skills/my-skill --to github --repo owner/repo
flash skills snapshot export setup.json          # Export skill config
flash skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Flash. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `flash skills search <query> --source skills-sh`. Flash resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Flash currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Flash repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Flash can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
flash skills search https://mintlify.com/docs --source well-known
flash skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
flash skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Flash can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
flash skills install openai/skills/k8s
flash skills tap add myorg/skills-repo
```
...
[Skills Hub](https://flash-agent.flashorg.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Flash-side changes required.
...
- Flash source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Flash supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Flash source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Flash can search and convert agent entries from LobeHub's public catalog into installable Flash skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Flash source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Flash integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Flash' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Flash source id: `browse-sh`
- Trust level: `community`

```bash
flash skills search airbnb --source browse-sh
flash skills inspect browse-sh/airbnb.com/search-listings-ddgioa
flash skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Flash also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Flash source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Flash does not enumerate or copy unrelated files from the host.

```bash
flash skills install https://sharethis.chat/SKILL.md
flash skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
flash skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `flash skills update` re-fetches from the same URL automatically when you want to refresh.
...
`flash skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Flash | Always trusted |
...
```bash
flash skills check          # Report which installed hub skills changed upstream
flash skills update         # Reinstall only the skills with updates available
flash skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Flash users add with `flash skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.flash.tags`, `version`, `author`, `platforms`, `metadata.flash.config`).
...
Flash discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Flash user can subscribe and install:

```bash
flash skills tap add my-org/flash-skills
flash skills search deploy
flash skills install my-org/flash-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.flash/.hub/taps.json`:
...
The `flash skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `flash skills tap list` shows the effective path per tap.
...
```bash
flash skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.flash/.hub/taps.json` (created on demand).

## Bundled skill updates (`flash skills reset`)

Flash ships with a set of bundled skills in `skills/` inside the repo. On install and on every `flash update`, a sync pass copies those into `~/.flash/skills/` and records a manifest at `~/.flash/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Flash recomputes the hash of your local copy and compares it to the origin hash:
...
flash skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `flash -p coder skills reset <name>` only affects that profile.
:::
