...
description: "Build themes and plugins for the Flash web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Flash web dashboard (`flash dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.flash/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.flash/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.flash/dashboard-themes
```

```yaml
# ~/.flash/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-flash-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Flash Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Flash Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Flash Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.flash/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Flash plugins in `~/.flash/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
```bash
mkdir -p ~/.flash/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.flash/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.flash/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
#### Calling built-in Flash endpoints
...
| `header-left` | Before the Flash brand in the top bar. |
...
```json
// ~/.flash/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.flash/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
A reference plugin (`example-dashboard` in [`flash-example-plugins`](https://github.com/FlashOrg/flash-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.flash/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing Flash internals

Backend routes run inside the dashboard process, so they can import from the flash-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.flash/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.flash/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
The [`strike-freedom-cockpit`](https://github.com/FlashOrg/flash-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `flash-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/FlashOrg/flash-example-plugins.git

# Theme
cp flash-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.flash/dashboard-themes/

# Plugin
cp -r flash-example-plugins/strike-freedom-cockpit ~/.flash/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Flash Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
Check that the file is in `~/.flash/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.flash/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.flash/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
2. Restart `flash dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.flash/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.flash/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
