...
description: "Customize Flash Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Flash Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Flash is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Flash now seeds a default `SOUL.md` automatically in:

```text
~/.flash/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Flash with a custom home directory, it will use:
...
- Flash creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Flash loads `SOUL.md` only from `HERMES_HOME`
- Flash does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Flash falls back to a built-in default identity
...
If Flash loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Flash instance itself.

That also makes it easier to teach users:
- "Edit `~/.flash/SOUL.md` to change Flash' default personality."
...
- how Flash should handle uncertainty, disagreement, or ambiguity
...
## What Flash injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Flash falls back to a built-in default identity ("You are Flash Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Flash ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Flash, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Flash its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.flash/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.flash/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-flash)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Flash speaks
- `display.skin` and `/skin` affect how Flash looks in the terminal
