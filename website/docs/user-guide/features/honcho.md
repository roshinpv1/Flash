...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Nyxo's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Nyxo instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
nyxo memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `nyxo` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Nyxo syncs them back at session init.
...
The `nyxo honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `nyxo memory setup honcho` (or run `nyxo memory setup` and pick it from the list); the `nyxo honcho` subcommand then appears on the next invocation.

```bash
nyxo memory setup honcho    # Configure Honcho directly (works before activation)
nyxo honcho status          # Connection status, config, and key settings
nyxo honcho setup           # Redirects to `nyxo memory setup` (post-activation alias)
nyxo honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
nyxo honcho peer            # Show or update peer names + dialectic reasoning level
nyxo honcho mode            # Show or set recall mode (hybrid/context/tools)
nyxo honcho tokens          # Show or set token budget for context and dialectic
nyxo honcho identity        # Seed or show the AI peer's Honcho identity
nyxo honcho sync            # Sync Honcho config to all existing profiles
nyxo honcho peers           # Show peer identities across all profiles
nyxo honcho sessions        # List known Honcho session mappings
nyxo honcho map             # Map current directory to a Honcho session name
nyxo honcho enable          # Enable Honcho for the active profile
nyxo honcho disable         # Disable Honcho for the active profile
nyxo honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `nyxo honcho`

If you previously used the standalone `nyxo honcho setup`:
...
No re-login or re-setup needed. Run `nyxo memory setup` and select "honcho" — the wizard detects your existing config.
