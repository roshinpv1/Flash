...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Flash's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Flash instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
flash memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `flash` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Flash syncs them back at session init.
...
The `flash honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `flash memory setup honcho` (or run `flash memory setup` and pick it from the list); the `flash honcho` subcommand then appears on the next invocation.

```bash
flash memory setup honcho    # Configure Honcho directly (works before activation)
flash honcho status          # Connection status, config, and key settings
flash honcho setup           # Redirects to `flash memory setup` (post-activation alias)
flash honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
flash honcho peer            # Show or update peer names + dialectic reasoning level
flash honcho mode            # Show or set recall mode (hybrid/context/tools)
flash honcho tokens          # Show or set token budget for context and dialectic
flash honcho identity        # Seed or show the AI peer's Honcho identity
flash honcho sync            # Sync Honcho config to all existing profiles
flash honcho peers           # Show peer identities across all profiles
flash honcho sessions        # List known Honcho session mappings
flash honcho map             # Map current directory to a Honcho session name
flash honcho enable          # Enable Honcho for the active profile
flash honcho disable         # Disable Honcho for the active profile
flash honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `flash honcho`

If you previously used the standalone `flash honcho setup`:
...
No re-login or re-setup needed. Run `flash memory setup` and select "honcho" — the wizard detects your existing config.
