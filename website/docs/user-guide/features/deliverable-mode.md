...
When Nyxo Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.nyxo/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.nyxo/config.yaml` (switchable per session
via `/personality`).
...
If you use Nyxo' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.nyxo/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Nyxo Agent's deliverable
