...
When Flash Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.flash/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.flash/config.yaml` (switchable per session
via `/personality`).
...
If you use Flash' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.flash/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Flash Agent's deliverable
