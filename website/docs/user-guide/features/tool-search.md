...
**Tool Search** is Flash' opt-in progressive-disclosure layer for that
...
:::info Built-in Flash tools never defer
The tools that make up Flash' core capability set (`terminal`,
...
When the model invokes `tool_call`, Flash **unwraps the bridge** and
...
- **No JS sandbox.** Flash uses the simpler "structured tools" mode
