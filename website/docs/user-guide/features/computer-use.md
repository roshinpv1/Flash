...
Flash Agent can drive your desktop — clicking, typing, scrolling,
...
on macOS/Linux, `install.ps1` on Windows. Use `flash computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `flash tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Flash app). `flash computer-use doctor` will tell you which permission is missing. |
...
```
flash -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.flash/config.yaml`.

## `flash computer-use doctor` — your first triage stop

`flash computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ flash computer-use doctor
...
agent is acting here." Each Flash run declares its own cua-driver
**session id** (something like `flash-3a7b9c14d2e8`); the cursor's
...
Flash intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Flash-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Flash skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Flash
...
Flash applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.flash/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Flash applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Flash agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Flash agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Flash warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Flash from inside the RDP / console
...
upstream. **Flash disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Flash sets
...
When it's on, `flash computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Flash at a binary you built from source instead
of the published release. Flash resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Flash testing (see notes).
...
### Option B — point Flash straight at the built binary (fastest loop)
...
### Confirm Flash is using your build

- `flash computer-use status` prints the resolved binary path and
  version.
- `flash computer-use doctor` confirms the binary is reachable and
...
- **Flash spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Flash
  change needed either way.
- **Local builds skip the version check.** Flash warns when the
...
**First action when anything's off: run `flash computer-use doctor`.**
...
Run `flash computer-use install` to fetch the cua-driver binary, or
run `flash tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `flash computer-use doctor` will
...
- **Flash-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Flash `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Flash (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Flash among them)
