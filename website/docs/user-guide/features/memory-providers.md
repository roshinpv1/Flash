...
Flash Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
flash memory setup      # interactive picker + configuration
flash memory status     # check what's active
flash memory off        # disable external provider
```

You can also select the active memory provider via `flash plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.flash/config.yaml`:
...
When a memory provider is active, Flash automatically:
...
**Setup Wizard:**
```bash
flash memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `flash honcho setup` command still works (it now redirects to `flash memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$HERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HERMES_HOME/honcho.json` > `~/.flash/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/FlashOrg/flash-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/flash).
...
:::tip Migrating from `flash honcho`
If you previously used `flash honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Flash profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Flash profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Flash profile. Host key `flash` → default; `flash.<profile>` for others. |
...
```bash
flash profile create coder --clone
```

`--clone` creates a `flash.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Flash profile, creates host blocks for any profile without one, inherits settings from the default `flash` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `flash memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/FlashOrg/flash-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/flash).
...
flash memory setup    # select "openviking"
# Or manually:
flash config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.flash/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.flash/.env
```
...
`OPENVIKING_AGENT` is Flash' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
flash memory setup    # select "mem0" → "Platform"
# Or manually:
flash config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.flash/.env
```

**Setup (OSS):**
```bash
flash memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
flash memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
flash memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
flash memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
flash memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.flash/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.flash/.env
```
...
**Config:** `$HERMES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.flash/.env`.
...
| `user_id` | `flash-user` | User identifier |
| `agent_id` | `flash` | Agent identifier |
...
**Switching modes:** Re-run `flash memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
flash memory setup    # select "hindsight"
# Or manually:
flash config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.flash/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p flash ui start`
...
| `bank_id` | `flash` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchroflashly on the server |
...
See [plugin README](https://github.com/FlashOrg/flash-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
flash memory setup    # select "holographic"
# Or manually:
flash config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.flash-memory-store`
...
**Setup:**
```bash
flash memory setup    # select "retaindb"
# Or manually:
flash config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.flash/.env
```
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=flash) |
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `flash` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `flash-{identity}` → `flash-coder`) to isolate memories per Flash profile
...
| **Requires** | `pip install flash-memori` + `flash-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install flash-memori
flash-memori install
flash config set memory.provider memori
flash memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `flash-memori` | Tool-aware memory + structured recall |
