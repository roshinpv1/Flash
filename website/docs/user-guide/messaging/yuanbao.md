...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Flash Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Flash to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.flash/.env`:
...
All standard Flash commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.flash/logs/gateway.log`
...
Yuanbao has a maximum message size. Flash automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
HERMES_LOG_LEVEL=debug flash gateway
```
...
```bash
flash chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
