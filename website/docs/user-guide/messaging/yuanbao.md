...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Nyxo Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Nyxo to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.nyxo/.env`:
...
All standard Nyxo commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.nyxo/logs/gateway.log`
...
4. Consider enabling verbose logging: `NYXO_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Nyxo automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
NYXO_LOG_LEVEL=debug nyxo gateway
```
...
```bash
nyxo chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
