...
Connect Flash to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Flash supports two WeCom integration modes:
...
> Run `flash gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Flash decrypts the message, queues it for the agent
...
(Use `flash gateway start` only after `flash gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Flash and the token the
...
in `~/.flash/.env` values around `=` will also break signature checks. After
fixing, restart `flash gateway run`.
...
Check `flash gateway run` logs for the bound host/port. If the adapter bound to
