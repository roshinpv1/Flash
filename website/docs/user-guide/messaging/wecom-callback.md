...
Connect Nyxo to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Nyxo supports two WeCom integration modes:
...
> Run `nyxo gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Nyxo decrypts the message, queues it for the agent
...
(Use `nyxo gateway start` only after `nyxo gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Nyxo and the token the
...
in `~/.nyxo/.env` values around `=` will also break signature checks. After
fixing, restart `nyxo gateway run`.
...
Check `nyxo gateway run` logs for the bound host/port. If the adapter bound to
