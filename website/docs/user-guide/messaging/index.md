...
description: "Chat with Flash from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Flash](/guides/use-voice-mode-with-flash).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [FlashPortal](/integrations/flash-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Flash Gateway"]
...
For group chats, hooks, and automation flows, Flash supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Flash keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Flash does not hide failures just because the text resembles a silence token.
...
```bash
flash gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
flash gateway              # Run in foreground
flash gateway setup        # Configure messaging platforms interactively
flash gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo flash gateway install --system   # Linux only: install a boot-time system service
flash gateway start        # Start the default service
flash gateway stop         # Stop the default service
flash gateway status       # Check default service status
flash gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Flash Agent to the latest version |
...
with the `session_reset` section in `~/.flash/config.yaml`:
...
Configure per-platform overrides in `~/.flash/gateway.json`:
...
flash pairing approve telegram XKGH5N7P

# Other pairing commands:
flash pairing list          # View pending + approved users
flash pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Flash appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.flash/config.yaml`:
...
Off by default. When enabled, Flash prepends a human-readable timestamp
...
Each `/background` prompt spawns a **separate agent instance** that runs asynchroflashly:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.flash/config.yaml`:
...
```bash
flash gateway install               # Install as user service
flash gateway start                 # Start the service
flash gateway stop                  # Stop the service
flash gateway status                # Check status
journalctl --user -u flash-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo flash gateway install --system
sudo flash gateway start --system
sudo flash gateway status --system
journalctl -u flash-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Flash installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit flash-gateway` (or `sudo systemctl edit flash-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `flash update`. When `flash update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart flash-gateway` command (it never blocks on an interactive password prompt).
...
```bash
flash gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `flash update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo flash update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/flash-gateway`:

```
flash ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed flash-gateway*, /usr/bin/systemctl --no-ask-password start flash-gateway*, /usr/bin/systemctl --no-ask-password restart flash-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Flash will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Flash installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.flash` uses `flash-gateway`; other installations use `flash-gateway-<hash>`. The `flash gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::

### macOS (launchd)

```bash
flash gateway install               # Install as launchd agent
flash gateway start                 # Start the service
flash gateway stop                  # Stop the service
flash gateway status                # Check status
tail -f ~/.flash/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.flash.gateway.plist`. It includes three environment variables:
...
- **HERMES_HOME** — scopes the gateway to your Flash installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `flash gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERMES_HOME` directory gets its own launchd label. The default `~/.flash` uses `ai.flash.gateway`; other installations use `ai.flash.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `flash-cli` | Full access |
| Telegram | `flash-telegram` | Full tools including terminal |
| Discord | `flash-discord` | Full tools including terminal |
| WhatsApp | `flash-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `flash-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `flash-slack` | Full tools including terminal |
| Google Chat | `flash-google_chat` | Full tools including terminal |
| Signal | `flash-signal` | Full tools including terminal |
| SMS | `flash-sms` | Full tools including terminal |
| Email | `flash-email` | Full tools including terminal |
| Home Assistant | `flash-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `flash-mattermost` | Full tools including terminal |
| Matrix | `flash-matrix` | Full tools including terminal |
| DingTalk | `flash-dingtalk` | Full tools including terminal |
| Feishu/Lark | `flash-feishu` | Full tools including terminal |
| WeCom | `flash-wecom` | Full tools including terminal |
| WeCom Callback | `flash-wecom-callback` | Full tools including terminal |
| Weixin | `flash-weixin` | Full tools including terminal |
| BlueBubbles | `flash-bluebubbles` | Full tools including terminal |
| QQBot | `flash-qqbot` | Full tools including terminal |
| Yuanbao | `flash-yuanbao` | Full tools including terminal |
| Microsoft Teams | `flash-teams` | Full tools including terminal |
| API Server | `flash-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `flash-webhook` | Full tools including terminal |
| Raft | `flash-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.flash/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
