...
description: "Set up Flash Agent as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Flash can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Flash gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Flash limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Flash degrades gracefully if absent.
...
2. **Add** → name (e.g. `flash-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.flash/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Flash to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Flash gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.flash/.env`.
3. Start the Flash gateway: `flash gateway`.
...
## Allowlist (Flash-side)

In addition to Meta's recipient whitelist, Flash has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.flash/.env`:
...
The `flash whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.flash/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, Flash uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Flash acknowledges inbound messages immediately:
...
Flash TTS produces MP3. Two paths:
...
Flash warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Flash. If you need it, please [open an issue](https://github.com/FlashOrg/flash-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Flash's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Flash doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.flash/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `flash gateway` is up.
- **App Secret not set** — without it, Flash refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Flash.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `flash tools list` and verify the platform is using `flash-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `flash_cli/platforms.py` for the platform → default toolset mapping.
...
```bash
flash config set stt.provider openai
flash config set stt.use_gateway true
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Flash will accept as authentic.
...
| | Baileys (`flash whatsapp`) | Cloud API (`flash whatsapp-cloud`) |
|---|---|---|
...
Most users running Flash for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
