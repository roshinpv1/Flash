...
Connect Flash to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Flash — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Flash runs it in a
...
flash photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Flash Agent` project on your account.
...
Runtime credentials are written to `~/.flash/.env`
...
(device token, dashboard project id) lives in `~/.flash/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Flash
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Flash replies with a pairing code. Approve it with:

```bash
flash pairing approve photon <CODE>
```

Use `flash pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.flash/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.flash/.env`):
...
By default Flash responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Flash` and
`@Flash agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Flash will reply.
...
assigned iMessage line Flash uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run flash photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `flash photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `flash photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `flash photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Flash sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Flash wake words  | JSON list / comma / newline regex patterns for group mentions |
