...
Connect Nyxo to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Nyxo — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Nyxo runs it in a
...
nyxo photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Nyxo Agent` project on your account.
...
Runtime credentials are written to `~/.nyxo/.env`
...
(device token, dashboard project id) lives in `~/.nyxo/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Nyxo
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Nyxo replies with a pairing code. Approve it with:

```bash
nyxo pairing approve photon <CODE>
```

Use `nyxo pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.nyxo/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.nyxo/.env`):
...
By default Nyxo responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Nyxo` and
`@Nyxo agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Nyxo will reply.
...
assigned iMessage line Nyxo uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run nyxo photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `nyxo photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `nyxo photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `nyxo photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Nyxo sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Nyxo wake words  | JSON list / comma / newline regex patterns for group mentions |
