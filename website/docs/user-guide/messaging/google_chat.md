...
sidebar_position: 12
title: "Google Chat"
description: "Set up Nyxo Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Nyxo Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Nyxo
...
> Run `nyxo gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Nyxo can read (e.g.,
`~/.nyxo/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a nyxo restart)
...
- Principal: `nyxo-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Nyxo calls
...
- **App name**: whatever you want users to see ("Nyxo" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/nyxo-chat-events`.
...
`ADDED_TO_SPACE` event that Nyxo uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Nyxo

Add the Google Chat section to `~/.nyxo/.env`:
...
Install the dependencies the Google Chat adapter needs (no Nyxo extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Nyxo is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Nyxo detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Nyxo session.
...
3. Download the JSON. Move it onto the host that runs Nyxo.
4. Register the client with Nyxo (run under the profile you want it scoped to):
...
nyxo -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Nyxo home (e.g.
`~/.nyxo/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.nyxo/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.nyxo/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Nyxo isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `nyxo gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Nyxo will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.nyxo/google_chat_user_tokens/<sanitized_email>.json` (filesystem
