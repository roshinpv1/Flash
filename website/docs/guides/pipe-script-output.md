...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `flash send`."
---

# Pipe Script Output to Messaging Platforms

`flash send` is a small, scriptable CLI that pushes a message to any
messaging platform Flash is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | flash send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `flash
...
flash send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
flash send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
flash send --list

# Filter by platform
flash send --list telegram
```
...
Any platform Flash ships adapters for works as a target:
...
`flash send` resolves the message body in this order:

1. **Positional argument** — `flash send --to telegram "hi"`
2. **`--file PATH`** — `flash send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | flash send --to telegram`

When stdin is a TTY (no pipe), Flash does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  flash send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `flash send` reuses your Flash config, the same script works on
any host where Flash is installed — no need to export bot tokens into
...
`flash send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  flash send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | flash send \
...
```bash
./train.py --epochs 200 && \
  flash send --to telegram "training done" || \
  flash send --to telegram "training failed (exit $?)"
```
...
flash send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(flash send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `flash send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `flash send` calls
the platform's REST endpoint directly using credentials from
`~/.flash/.env` and `~/.flash/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `flash gateway start` and retry.
...
# Machine-readable
flash send --list --json
```

The listing is built from `~/.flash/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`flash gateway
...
| Approach | Multi-platform | Reuses Flash creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `flash send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`flash send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `flash send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `flash send` shares with cron delivery.
