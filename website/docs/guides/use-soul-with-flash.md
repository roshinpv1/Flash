---
sidebar_position: 7
title: "Use SOUL.md with Flash"
description: "How to use SOUL.md to shape Flash Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Flash

`SOUL.md` is the **primary identity** for your Flash instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Flash to feel like the same assistant every time you talk to it — or if you want to replace the Flash persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Flash should be
- what Flash should avoid stylistically
- how Flash should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Flash is and how Flash speaks
...
Flash now uses only the global SOUL file for the current instance:

```text
~/.flash/SOUL.md
```

If you run Flash with a custom home directory, it becomes:
...
Flash automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Flash does not overwrite it
- if the file exists but is empty, Flash adds nothing from it to the prompt

## How Flash uses it

When Flash starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Flash feels.
...
Flash already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Flash should sound.

# Avoid
What Flash should not do.

# Defaults
How Flash should behave when ambiguity appears.
```
...
Then restart Flash or start a new session.
...
### I edited SOUL.md but Flash still sounds the same

Check:
- you edited `~/.flash/SOUL.md` or `$HERMES_HOME/SOUL.md`
...
### Flash is ignoring parts of my SOUL.md
