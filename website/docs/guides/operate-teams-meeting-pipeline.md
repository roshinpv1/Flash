...
```bash
flash teams-pipeline token-health
flash teams-pipeline token-health --force-refresh
```
...
```bash
flash teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
flash teams-pipeline maintain-subscriptions
flash teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Flash cron (recommended if you already run the Flash gateway)

Flash ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.flash/scripts/`. First create the script:

```bash
mkdir -p ~/.flash/scripts
cat > ~/.flash/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec flash teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.flash/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
flash cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/flash-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Flash Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/flash teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/flash-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Flash Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now flash-teams-pipeline-maintain.timer
systemctl list-timers flash-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/flash teams-pipeline maintain-subscriptions >> /var/log/flash/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.flash/.env` at the top of a wrapper script that crontab calls.
...
```bash
flash teams-pipeline subscriptions   # should show expirationDateTime advanced
flash teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
flash teams-pipeline list
flash teams-pipeline list --status failed
flash teams-pipeline show <job-id>
```

### Replay a stored job

```bash
flash teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
flash teams-pipeline fetch --meeting-id <meeting-id>
flash teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `flash teams-pipeline maintain-subscriptions --dry-run`
- inspect `flash teams-pipeline list --status failed`
...
- run `flash teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `flash teams-pipeline run`
...
- [ ] `flash teams-pipeline validate` returns an OK snapshot
- [ ] `flash teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Flash cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
