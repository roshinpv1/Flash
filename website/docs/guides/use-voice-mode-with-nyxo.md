---
sidebar_position: 8
title: "Use Voice Mode with Nyxo"
description: "A practical guide to setting up and using Nyxo voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Nyxo
...
- you want Nyxo sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Nyxo.
...
## Step 1: make sure normal Nyxo works first
...
Nyxo supports both local and cloud speech stacks.
...
### If you use `nyxo setup`

If you choose NeuTTS in the setup wizard, Nyxo checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Nyxo transcribes and responds
...
- asking Nyxo to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Nyxo loop.

## Tuning CLI behavior

### Silence threshold

If Nyxo starts/stops too aggressively, tune:
...
Nyxo stays a normal chat bot, but can speak replies.
...
- you want Nyxo to function like a portable research or ops assistant
...
Nyxo joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Nyxo detects speech boundaries
- transcripts are posted in the associated text channel
- Nyxo responds in text and audio
...
1. get text Nyxo working
2. install `nyxo-agent[voice]`
