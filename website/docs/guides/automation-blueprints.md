...
Copy-paste blueprints for common automation patterns. Each blueprint uses Nyxo's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`nyxo webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `nyxo webhook subscribe`) |
...
```bash
nyxo cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/nyxo-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/nyxo-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
nyxo webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
nyxo cron create "0 9 * * 1" \
  "Scan the NousResearch/nyxo-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/nyxo-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (nyxo_cli/commands.py, nyxo_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (nyxo_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
nyxo cron create "0 6 * * *" \
  "Run a dependency security audit on the nyxo-agent project.

1. cd ~/.nyxo/nyxo-agent && source .venv/bin/activate
...
```bash
nyxo webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
nyxo webhook subscribe alert-triage \
...
```python title="~/.nyxo/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Nyxo-Monitor/1.0"})
...
nyxo cron create "every 30m" \
  ...
  --script ~/.nyxo/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
nyxo cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Nyxo Agent development." \
...
```bash
nyxo webhook subscribe github-issues \
  --events "issues" \
...
```bash
nyxo webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
nyxo webhook subscribe stripe-payments \
...
```bash
nyxo cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the nyxo-agent codebase.
