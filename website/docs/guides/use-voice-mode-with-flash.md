---
sidebar_position: 8
title: "Use Voice Mode with Flash"
description: "A practical guide to setting up and using Flash voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Flash
...
:::tip
[Nous Portal](/integrations/flash-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Flash sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Flash.
...
## Step 1: make sure normal Flash works first
...
Flash supports both local and cloud speech stacks.
...
### If you use `flash setup`

If you choose NeuTTS in the setup wizard, Flash checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Flash transcribes and responds
...
- asking Flash to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Flash loop.

## Tuning CLI behavior

### Silence threshold

If Flash starts/stops too aggressively, tune:
...
Flash stays a normal chat bot, but can speak replies.
...
- you want Flash to function like a portable research or ops assistant
...
Flash joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Flash detects speech boundaries
- transcripts are posted in the associated text channel
- Flash responds in text and audio
...
1. get text Flash working
2. install `flash-agent[voice]`
