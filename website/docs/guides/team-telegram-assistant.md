...
This tutorial walks you through setting up a Telegram bot powered by Nyxo Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Nyxo Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nyxo/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Nyxo itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Nyxo Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nyxo_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Nyxo Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u nyxo-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nyxo gateway install` to update the plist.
:::
...
```bash
nyxo gateway stop && nyxo gateway start
```
...
nyxo pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nyxo/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nyxo/config.yaml`:
...
Customize how the bot communicates by editing `~/.nyxo/SOUL.md`:

For a full guide, see [Use SOUL.md with Nyxo](/guides/use-soul-with-nyxo).
...
```bash
# From the CLI
nyxo cron list          # View all scheduled jobs
nyxo cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nyxo-gateway` (Linux) or `~/.nyxo/logs/gateway.log` (macOS) |
| Cron job output | `~/.nyxo/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nyxo/cron/jobs.json` |
| Pairing data | `~/.nyxo/pairing/` |
| Session history | `~/.nyxo/sessions/` |
