...
This tutorial walks you through setting up a Telegram bot powered by Flash Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Flash Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.flash/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Flash itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Flash Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_flash_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Flash Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Flash Gateway...
...
# View live logs
journalctl --user -u flash-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `flash gateway install` to update the plist.
:::
...
```bash
flash gateway stop && flash gateway start
```
...
flash pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.flash/.env`:
...
Control how much detail the bot shows when using tools. In `~/.flash/config.yaml`:
...
Customize how the bot communicates by editing `~/.flash/SOUL.md`:

For a full guide, see [Use SOUL.md with Flash](/guides/use-soul-with-flash).
...
```bash
# From the CLI
flash cron list          # View all scheduled jobs
flash cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u flash-gateway` (Linux) or `~/.flash/logs/gateway.log` (macOS) |
| Cron job output | `~/.flash/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.flash/cron/jobs.json` |
| Pairing data | `~/.flash/pairing/` |
| Session history | `~/.flash/sessions/` |
