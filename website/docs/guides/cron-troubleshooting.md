...
description: "Diagnose and fix common Nyxo cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`nyxo gateway` for foreground, or `nyxo gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `nyxo cron tick`.
...
```bash
date
nyxo cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.nyxo/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.nyxo/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.nyxo/.env` |
...
| `local` | Write access to `~/.nyxo/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `nyxo cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `nyxo skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `nyxo skills list`.
...
2. `~/.nyxo/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `nyxo cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Nyxo config directory). Verify:
```bash
ls ~/.nyxo/scripts/your-script.py   # Must exist
nyxo cron edit <job_id> --script ~/.nyxo/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `nyxo skills install <skill-name>`.
...
Jobs are stored in `~/.nyxo/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.nyxo/cron/jobs.json
chmod 600 ~/.nyxo/cron/jobs.json   # Your user should own it
```
...
```bash
nyxo cron list                    # Show all jobs, states, next_run times
nyxo cron run <job_id>            # Schedule for next tick (for testing)
nyxo cron edit <job_id>           # Fix configuration issues
nyxo logs                         # View recent Nyxo logs
nyxo skills list                  # Verify installed skills
```
...
1. Run the job with `nyxo cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.nyxo/logs/agent.log` for scheduler messages and `~/.nyxo/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/nyxo-agent](https://github.com/NousResearch/nyxo-agent) with:
