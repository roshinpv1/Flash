...
description: "Diagnose and fix common Flash cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`flash gateway` for foreground, or `flash gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `flash cron tick`.
...
```bash
date
flash cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.flash/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.flash/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.flash/.env` |
...
| `local` | Write access to `~/.flash/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `flash cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `flash skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `flash skills list`.
...
2. `~/.flash/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `flash cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Flash config directory). Verify:
```bash
ls ~/.flash/scripts/your-script.py   # Must exist
flash cron edit <job_id> --script ~/.flash/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `flash skills install <skill-name>`.
...
Jobs are stored in `~/.flash/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.flash/cron/jobs.json
chmod 600 ~/.flash/cron/jobs.json   # Your user should own it
```
...
```bash
flash cron list                    # Show all jobs, states, next_run times
flash cron run <job_id>            # Schedule for next tick (for testing)
flash cron edit <job_id>           # Fix configuration issues
flash logs                         # View recent Flash logs
flash skills list                  # Verify installed skills
```
...
1. Run the job with `flash cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.flash/logs/agent.log` for scheduler messages and `~/.flash/logs/errors.log` for warnings
3. Open an issue at [github.com/FlashOrg/flash-agent](https://github.com/FlashOrg/flash-agent) with:
