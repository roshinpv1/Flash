---
sidebar_position: 7
title: "Use SOUL.md with Nyxo"
description: "How to use SOUL.md to shape Nyxo Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Nyxo

`SOUL.md` is the **primary identity** for your Nyxo instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Nyxo to feel like the same assistant every time you talk to it — or if you want to replace the Nyxo persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Nyxo should be
- what Nyxo should avoid stylistically
- how Nyxo should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Nyxo is and how Nyxo speaks
...
Nyxo now uses only the global SOUL file for the current instance:

```text
~/.nyxo/SOUL.md
```

If you run Nyxo with a custom home directory, it becomes:
...
Nyxo automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Nyxo does not overwrite it
- if the file exists but is empty, Nyxo adds nothing from it to the prompt

## How Nyxo uses it

When Nyxo starts a session, it reads `SOUL.md` from `NYXO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Nyxo feels.
...
Nyxo already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Nyxo should sound.

# Avoid
What Nyxo should not do.

# Defaults
How Nyxo should behave when ambiguity appears.
```
...
Then restart Nyxo or start a new session.
...
### I edited SOUL.md but Nyxo still sounds the same

Check:
- you edited `~/.nyxo/SOUL.md` or `$NYXO_HOME/SOUL.md`
...
### Nyxo is ignoring parts of my SOUL.md
