...
description: "How to add a new inference provider to Nyxo Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Nyxo can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `nyxo model` menu entries
...
1. `nyxo_cli/auth.py` decides how credentials are found.
2. `nyxo_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `nyxo_cli/models.py` and `nyxo_cli/main.py` make the provider show up in the CLI. (`nyxo_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `nyxo_cli/auth.py`
2. `nyxo_cli/models.py`
3. `nyxo_cli/runtime_provider.py`
4. `nyxo_cli/main.py`
...
:::tip
`nyxo_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `nyxo setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$NYXO_HOME/plugins/model-providers/` both get picked up.
...
7. `nyxo model` menu includes the provider
8. `nyxo setup` wizard delegates to `main.py` automatically
...
User plugins at `$NYXO_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `nyxo model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `nyxo_cli/auth.py`
- `_PROVIDER_LABELS` in `nyxo_cli/models.py`
- `_PROVIDER_ALIASES` in both `nyxo_cli/auth.py` and `nyxo_cli/models.py`
- CLI `--provider` choices in `nyxo_cli/main.py`
...
## Step 2: Add auth metadata in `nyxo_cli/auth.py`
...
- What env vars should Nyxo check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `nyxo_cli/models.py`
...
## Step 4: Resolve runtime data in `nyxo_cli/runtime_provider.py`
...
Be careful with API-key precedence. Nyxo already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `nyxo_cli/main.py`

A provider is not discoverable until it shows up in the interactive `nyxo model` flow.

Update these in `nyxo_cli/main.py`:
...
:::tip
`nyxo_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `nyxo model` and `nyxo setup` automatically.
:::
...
When you add a native provider, double-check that Nyxo is only sending fields that provider actually understands.
...
- `tests/nyxo_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/nyxo_cli/test_model_switch_custom_providers.py` (and adjacent `tests/nyxo_cli/test_model_switch_*.py`)
- `tests/nyxo_cli/test_setup_model_provider.py`
...
python -m pytest tests/nyxo_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/nyxo_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m nyxo_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `nyxo_cli/auth.py`
- [ ] aliases added in `nyxo_cli/auth.py` and `nyxo_cli/models.py`
- [ ] model catalog added in `nyxo_cli/models.py`
- [ ] runtime branch added in `nyxo_cli/runtime_provider.py`
- [ ] CLI wiring added in `nyxo_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `nyxo model` but not `nyxo setup`
