...
description: "How to add a new inference provider to Flash Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Flash can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `flash model` menu entries
...
1. `flash_cli/auth.py` decides how credentials are found.
2. `flash_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `flash_cli/models.py` and `flash_cli/main.py` make the provider show up in the CLI. (`flash_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `flash_cli/auth.py`
2. `flash_cli/models.py`
3. `flash_cli/runtime_provider.py`
4. `flash_cli/main.py`
...
:::tip
`flash_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `flash setup`.
:::
...
7. `flash model` menu includes the provider
8. `flash setup` wizard delegates to `main.py` automatically
...
- OAuth or token refresh (FlashPortal, Codex, Qwen Portal, Copilot)
...
- Provider-specific `flash model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `flash_cli/auth.py`
- `_PROVIDER_LABELS` in `flash_cli/models.py`
- `_PROVIDER_ALIASES` in both `flash_cli/auth.py` and `flash_cli/models.py`
- CLI `--provider` choices in `flash_cli/main.py`
...
## Step 2: Add auth metadata in `flash_cli/auth.py`
...
- OAuth / auth-store path: Flash, OpenAI Codex

Questions to answer here:

- What env vars should Flash check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `flash_cli/models.py`
...
## Step 4: Resolve runtime data in `flash_cli/runtime_provider.py`
...
Be careful with API-key precedence. Flash already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `flash_cli/main.py`

A provider is not discoverable until it shows up in the interactive `flash model` flow.

Update these in `flash_cli/main.py`:
...
:::tip
`flash_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `flash model` and `flash setup` automatically.
:::
...
When you add a native provider, double-check that Flash is only sending fields that provider actually understands.
...
- `tests/flash_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/flash_cli/test_model_switch_custom_providers.py` (and adjacent `tests/flash_cli/test_model_switch_*.py`)
- `tests/flash_cli/test_setup_model_provider.py`
...
python -m pytest tests/flash_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/flash_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m flash_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `flash_cli/auth.py`
- [ ] aliases added in `flash_cli/auth.py` and `flash_cli/models.py`
- [ ] model catalog added in `flash_cli/models.py`
- [ ] runtime branch added in `flash_cli/runtime_provider.py`
- [ ] CLI wiring added in `flash_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `flash model` but not `flash setup`
