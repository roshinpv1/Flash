...
The ACP adapter wraps Flash' synchroflash `AIAgent` in an async JSON-RPC stdio server.
...
```text
flash acp / flash-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.flash/.env
  -> configure stderr logging
  -> construct FlashACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'flash-agent[acp]==<version>' flash-acp`, pointed at the `flash-agent` PyPI release.
...
- `allow_once` -> Flash `once`
- `allow_always` -> Flash `always`
- reject options -> Flash `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Flash tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["flash-acp"])
...
Instead it reuses Flash' runtime resolver:

- `acp_adapter/auth.py`
- `flash_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Flash provider/credentials. It also always advertises a terminal setup auth method (`flash-setup`, args `--setup`) so first-run registry clients can open Flash' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.flash/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `flash-acp` toolset definition
- `flash_cli/main.py` — `flash acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `flash-acp` script
