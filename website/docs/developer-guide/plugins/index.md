...
title: "Build a Flash Plugin"
description: "Step-by-step guide to building a complete Flash plugin with tools, hooks, data files, and skills"
---

# Build a Flash Plugin

This guide walks through building a complete Flash plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Flash has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.flash/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `flash skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `FlashOrg/flash-agent`. Users install them into `~/.flash/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Flash Org Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/FlashOrg/flash-agent/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.flash/plugins/calculator
cd ~/.flash/plugins/calculator
```
...
This tells Flash: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Flash may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `flash my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Flash continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 flash plugins list
```
...
The same logs are always written to `~/.flash/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
flash logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `flash plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.flash/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.flash/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.flash/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.flash/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `flash plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Flash into installing arbitrary packages — only specs Flash itself ships are eligible |
...
auditable in `~/.flash/logs/agent.log`. Plugins load after built-in
...
- **The system prompt is Flash's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `flash <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for flash my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: flash my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for flash my-plugin."""
...
After registration, users can run `flash my-plugin status`, `flash my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `flash name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `flash chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:
...
For running a full `flash <subcommand>` (e.g. `flash kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "flash kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Flash from a hook.
...
Flash has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.flash/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `flash gateway status` without SDK instantiation.
...
Flash also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Flash without any Python plugin. Declare them in `~/.flash/config.yaml`:
...
Flash connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.flash/hooks/<name>/`:

```yaml
# ~/.flash/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.flash/hooks/long-task-alert/handler.py
...
flash skills search my-workflow --source myorg/skills-repo
flash skills install myorg/skills-repo/my-workflow
```
...
```toml
# pyproject.toml
[project.entry-points."flash_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install flash-plugin-calculator
# Plugin auto-discovered on next flash startup
```
...
```python
# Wrong — will break if Flash passes extra context
def handler(args):
    ...
