...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Flash Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Flash Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Flash core? Start with [Build a Flash Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Flash itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Flash instance (throwaway HERMES_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m flash_cli.main
scripts/dev-sandbox.sh --persistent python -m flash_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.flash/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.flash` — use `get_flash_home()` from `flash_constants` for code paths and `display_flash_home()` for user-facing messages. See [AGENTS.md](https://github.com/FlashOrg/flash-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/FlashOrg/flash-agent/blob/main/LICENSE).
