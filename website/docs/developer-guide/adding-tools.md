...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Flash Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Flash tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Flash core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Flash Plugin](/developer-guide/plugins)
...
If your tool requires an API key, add it to `flash_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `flash_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `flash chat -q "Use the weather tool for London"`
