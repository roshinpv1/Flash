...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Nyxo Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Nyxo tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Nyxo core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Nyxo Plugin](/guides/build-a-nyxo-plugin)
...
2. **`toolsets.py`** — add tool name to `_NYXO_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `nyxo_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `nyxo_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `nyxo chat -q "Use the weather tool for London"`
