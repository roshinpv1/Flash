...
sidebar_position: 1
title: "Architecture"
description: "Nyxo Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Nyxo Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ nyxo_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NyxoCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── nyxo_state.py           # SQLite session/state database with FTS5
├── nyxo_constants.py       # NYXO_HOME, profile-aware paths
...
│
├── nyxo_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `nyxo` subcommands (large file)
...
│   ├── skills_config.py      # nyxo skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # nyxo tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # nyxo gateway start/stop
│
...
```text
User input → NyxoCLI.process_input()
...
Three discovery sources: `~/.nyxo/plugins/` (user), `.nyxo/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `nyxo plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-nyxo-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Nyxo as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`nyxo -p <name>`) gets its own NYXO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
