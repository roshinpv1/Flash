...
description: "How Flash detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Flash' browser tooling:
...
`window.alert`/`confirm`/`prompt` with a synchroflash XHR to a magic host
(`flash-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Flash `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `flash-acp`, `flash-api-server`, and core toolsets (gated on CDP reachability)
- `flash_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
