...
description: "Run Flash Agent directly on an Android phone with Termux"
---

# Flash on Android with Termux
...
Flash Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Flash from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Flash now ships a Termux-aware installer path:
...
- links `flash` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `flash` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/flash" "$PREFIX/bin/flash"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `flash` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.flash/.env`.
...
### `flash doctor` says ripgrep or Node is missing
