...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Flash Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Flash setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Flash Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Flash Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Flash for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Flash working on my machine | `flash setup` | Run a real chat and verify it responds |
| I already know my provider | `flash model` | Save the config, then start chatting |
| I want a bot or always-on setup | `flash gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `flash model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `flash model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Flash cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Flash Agent
### With the Flash Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Flash Desktop installer](https://flash-agent.flashorg.com/) from our website and run it.

### Without Flash Desktop:
For a command-line only install without Flash Desktop, run:
...
The single most important setup step. Use `flash model` to walk through the choice interactively:

```bash
flash model
```

:::tip Easiest path: FlashPortal
...
That logs you in, sets Flashas your provider, and turns on the Tool Gateway in one command.
:::

:::info Setup modes
On a fresh install, `flash setup` offers three modes:

- **Quick Setup (FlashPortal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `flash update`. Re-enable anything later with `flash tools`, seed skills with `flash skills opt-in --sync`, or tune settings with `flash setup agent`.
:::
...
|----------|-----------|---------------|
| **FlashPortal** | Subscription-based, zero-config | OAuth login via `flash model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `flash model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `flash model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `flash_cli/models.py` may change between releases) | `flash model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `flash model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `flash model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `flash model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `flash model` (requires `copilot` CLI + `copilot login`) |
...
Flash Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `flash model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Flash separates secrets from normal config:

- **Secrets and tokens** → `~/.flash/.env`
- **Non-secret settings** → `~/.flash/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
flash config set model anthropic/claude-opus-4.6
flash config set terminal.backend docker
flash config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
flash            # classic CLI
flash --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Flash ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `flash` vs `flash --tui`.
:::
...
```bash
flash --continue    # Resume the most recent session
flash -c            # Short form
```
...
```bash
flash gateway setup    # Interactive platform configuration
```
...
- `flash tools` — tune tool access per platform
- `flash skills` — browse and install reusable workflows
...
```bash
flash config set terminal.backend docker    # Docker isolation
flash config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Flash install directory (the curl installer placed it at
# ~/.flash/flash-agent on Linux/macOS or %LOCALAPPDATA%\flash\flash-agent on Windows):
cd ~/.flash/flash-agent
...
Skills are on-demand instruction documents that teach Flash how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Flash ships with a catalog of bundled skills already installed in `~/.flash/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
flash skills browse                      # list everything available
flash skills search kubernetes           # find skills by keyword
flash skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `flash skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Flash ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.flash/flash-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Flash opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `flash model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `flash gateway setup` and check `flash gateway status` |
| `flash --continue` can't find old session | Switched profiles or session never saved | Check `flash sessions list` and confirm you're in the right profile |
...
| `flash doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `flash model`
3. `flash setup`
4. `flash sessions list`
5. `flash --continue`
6. `flash gateway status`
...
| `flash tools` | Configure which tools are enabled per platform |
| `flash setup` | Full setup wizard (configures everything at once) |
| `flash doctor` | Diagnose issues |
| `flash update` | Update to latest version |
| `flash gateway` | Start the messaging gateway |
| `flash --continue` | Resume last session |
