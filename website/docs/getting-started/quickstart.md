...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Nyxo Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Nyxo setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Nyxo Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Nyxo Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Nyxo for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Nyxo working on my machine | `nyxo setup` | Run a real chat and verify it responds |
| I already know my provider | `nyxo model` | Save the config, then start chatting |
| I want a bot or always-on setup | `nyxo gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `nyxo model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `nyxo model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Nyxo cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Nyxo Agent
### With the Nyxo Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Nyxo Desktop installer](https://nyxo-agent.nousresearch.com/) from our website and run it.

### Without Nyxo Desktop:
For a command-line only install without Nyxo Desktop, run:
...
The single most important setup step. Use `nyxo model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `nyxo setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `nyxo update`. Re-enable anything later with `nyxo tools`, seed skills with `nyxo skills opt-in --sync`, or tune settings with `nyxo setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `nyxo model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `nyxo model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `nyxo model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `nyxo_cli/models.py` may change between releases) | `nyxo model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `nyxo model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `nyxo model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `nyxo model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `nyxo model` (requires `copilot` CLI + `copilot login`) |
...
Nyxo Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `nyxo model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Nyxo separates secrets from normal config:

- **Secrets and tokens** → `~/.nyxo/.env`
- **Non-secret settings** → `~/.nyxo/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
nyxo config set model anthropic/claude-opus-4.6
nyxo config set terminal.backend docker
nyxo config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
nyxo            # classic CLI
nyxo --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Nyxo ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `nyxo` vs `nyxo --tui`.
:::
...
```bash
nyxo --continue    # Resume the most recent session
nyxo -c            # Short form
```
...
```bash
nyxo gateway setup    # Interactive platform configuration
```
...
- `nyxo tools` — tune tool access per platform
- `nyxo skills` — browse and install reusable workflows
...
```bash
nyxo config set terminal.backend docker    # Docker isolation
nyxo config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Nyxo install directory (the curl installer placed it at
# ~/.nyxo/nyxo-agent on Linux/macOS or %LOCALAPPDATA%\nyxo\nyxo-agent on Windows):
cd ~/.nyxo/nyxo-agent
...
Skills are on-demand instruction documents that teach Nyxo how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Nyxo ships with a catalog of bundled skills already installed in `~/.nyxo/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
nyxo skills browse                      # list everything available
nyxo skills search kubernetes           # find skills by keyword
nyxo skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `nyxo skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Nyxo ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.nyxo/nyxo-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Nyxo opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `nyxo model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `nyxo gateway setup` and check `nyxo gateway status` |
| `nyxo --continue` can't find old session | Switched profiles or session never saved | Check `nyxo sessions list` and confirm you're in the right profile |
...
| `nyxo doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `nyxo model`
3. `nyxo setup`
4. `nyxo sessions list`
5. `nyxo --continue`
6. `nyxo gateway status`
...
| `nyxo tools` | Configure which tools are enabled per platform |
| `nyxo setup` | Full setup wizard (configures everything at once) |
| `nyxo doctor` | Diagnose issues |
| `nyxo update` | Update to latest version |
| `nyxo gateway` | Start the messaging gateway |
| `nyxo --continue` | Resume last session |
