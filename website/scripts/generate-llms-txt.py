#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Flash docs site.
...
Both publish at:
  https://flash-agent.flashorg.com/docs/llms.txt
  https://flash-agent.flashorg.com/docs/llms-full.txt
...
SITE_BASE = "https://flash-agent.flashorg.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Flash as a Python Library", None),
        ("guides/use-mcp-with-flash", "Use MCP with Flash", None),
        ("guides/use-voice-mode-with-flash", "Use Voice Mode with Flash", None),
        ("guides/use-soul-with-flash", "Use SOUL.md with Flash", None),
        ("guides/build-a-flash-plugin", "Build a Flash Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Flash"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "backends. Works with FlashPortal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/FlashOrg/"
        "flash-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/FlashOrg/flash-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Flash Agent — Full Documentation\n",
        (
            "This file is the entire Flash Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Flash, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://flash-agent.flashorg.com/docs\n",
        "Short index: https://flash-agent.flashorg.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
