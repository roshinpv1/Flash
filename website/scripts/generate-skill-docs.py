...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/FlashOrg/flash-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    flash_meta = (fm.get("metadata") or {}).get("flash") or {}
    tags = flash_meta.get("tags") or []
    related = flash_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `flash skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Flash Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Flash ships with a large built-in skill library copied into `~/.flash/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Flash also syncs bundled skills on `flash update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.flash/skills/` tree, it is still shipped with Flash; restore it with `flash skills reset <name> --restore`.",
        "",
