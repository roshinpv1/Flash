...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/nyxo-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nyxo_meta = (fm.get("metadata") or {}).get("nyxo") or {}
    tags = nyxo_meta.get("tags") or []
    related = nyxo_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nyxo skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Nyxo Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Nyxo ships with a large built-in skill library copied into `~/.nyxo/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Nyxo also syncs bundled skills on `nyxo update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.nyxo/skills/` tree, it is still shipped with Nyxo; restore it with `nyxo skills reset <name> --restore`.",
        "",
