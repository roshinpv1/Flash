...
NYXO_HOME_DIR="${NYXO_HOME:-$HOME/.nyxo}"
NYXO_CFG="${NYXO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Nyxo config has twozero_td MCP entry ──
if [[ ! -f "$NYXO_CFG" ]]; then
    echo -e " ${FAIL} Nyxo config not found at ${NYXO_CFG}"
    manual_steps+=("Create ${NYXO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NYXO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Nyxo config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Nyxo config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NYXO_CFG} manually"); }
    manual_steps+=("Restart Nyxo session to pick up config change")
fi
