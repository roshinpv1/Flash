#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Flash Agent.
...
from _flash_home import display_flash_home, get_flash_home
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Flash profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Flash Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Flash Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'flash-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Flash home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_flash_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Flash")
