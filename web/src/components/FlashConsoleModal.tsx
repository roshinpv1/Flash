...
interface FlashConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function FlashConsoleModal({ open, onClose }: FlashConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("flash> ");
  const inputPromptRef = useRef("flash> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "flash> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Flash Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="flash-console-title"
    >
      <div
        className={cn(
          themedBody,
