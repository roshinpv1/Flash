/**
 * Flash Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 ...
 *      (``@/lib/api``, ``@flash-research/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``FlashPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@flash/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @flash-research/ui types into the contract) or is the loose
...
export interface FlashPluginSDK {
   ...
   * ``Promise<T>``, a few return a URL string synchroflashly); plugins call the
...
declare global {
  interface Window {
    __HERMES_PLUGIN_SDK__?: FlashPluginSDK;
