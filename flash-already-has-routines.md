# Flash Agent Has Had "Routines" Since March
...
Flash equivalent — works today:
```bash
flash webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Flash implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Flash Agent |
|---|---|---|
...
## Things Flash Does That Routines Can't
...
Flash has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Flash Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
flash gateway setup    # enable webhooks
flash webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [flash-agent.flashorg.com/docs/reference/automation-blueprints-catalog](https://flash-agent.flashorg.com/docs/reference/automation-blueprints-catalog)

Documentation: [flash-agent.flashorg.com](https://flash-agent.flashorg.com)

GitHub: [github.com/FlashOrg/flash-agent](https://github.com/FlashOrg/flash-agent)
