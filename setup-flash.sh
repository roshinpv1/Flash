...
# 5. Symlinks the 'flash' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Flash Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/flash-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink flash into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up flash command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/flash"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/flash"
echo -e "${GREEN}✓${NC} Symlinked flash → $COMMAND_LINK_DISPLAY_DIR/flash"
...
# ============================================================================
# Seed bundled skills into ~/.flash/skills/
# ============================================================================

HERMES_SKILLS_DIR="${HERMES_HOME:-$HOME/.flash}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.flash/skills/ ..."
...
echo "Other commands:"
echo "  flash status        # Check configuration"
if is_termux; then
    echo "  flash gateway       # Run gateway in foreground"
else
    echo "  flash gateway install # Install gateway service (messaging + cron)"
fi
echo "  flash cron list     # View scheduled jobs"
echo "  flash doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m flash_cli.main setup
fi
