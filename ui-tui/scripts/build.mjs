...
await build({
  ...
  // Skip the prebuilt @nyxo/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@nyxo/ink': resolve(root, 'packages/nyxo-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The nyxo_cli launcher
