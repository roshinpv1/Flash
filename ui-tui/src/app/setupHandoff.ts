import type { RunExternalProcess } from '@flash/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`flash ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching flash: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`flash ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
