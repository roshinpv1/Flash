import type { RunExternalProcess } from '@nyxo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nyxo ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nyxo: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nyxo ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
