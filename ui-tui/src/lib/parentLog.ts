...
const logDir = join(process.env.HERMES_HOME?.trim() || join(homedir(), '.flash'), 'logs')
...
// a real ~/.flash (tests must stay hermetic — see AGENTS.md).
...
// or add noticeable blocking on the synchroflash append. Mirrors the spirit of
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('flash-tui: parent lifecycle log unavailable\n')
    }
  }
}
