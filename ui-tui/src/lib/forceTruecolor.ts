...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.NYXO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Nyxo on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via NYXO_TUI_TRUECOLOR=1.
