...
React + Ink terminal UI for Nyxo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NYXO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `nyxo setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.nyxo/.nyxo_history` or under `NYXO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `nyxo setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/nyxo-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external nyxo setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.nyxo/tui-parent.log
    ...
    types/
      nyxo-ink.d.ts            type declarations for @nyxo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NyxoCLI subprocess for slash commands
```