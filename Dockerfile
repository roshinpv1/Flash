...
# Do not write .pyc files at runtime: /opt/flash is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/flash/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when flash
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# flash process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main flash process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Flash WebUI' catalog). The image has moved to s6-overlay /init\
...
RUN useradd -u 10000 -m -d /opt/data flash
...
#
# ui-tui/packages/flash-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/flash-ink/ ui-tui/packages/flash-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as flash-ink above).
...
# lock on the @flash/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/flash/.venv at first
...
# gives the non-root flash user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link flash-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/flash are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the flash user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/flash/bin && \
    cp /opt/flash/docker/flash-exec-shim.sh /opt/flash/bin/flash && \
    chmod 0755 /opt/flash/bin/flash && \
    printf 'docker\n' > /opt/flash/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $HERMES_HOME. $HERMES_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``flash update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the flash user via
# `s6-setuidgid flash` in its run script. If HERMES_UID is unset, services
# run as the default flash user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `flash dump` reports
...
# /opt/flash/.flash_build_sha at build time, and have
# flash_cli/build_info.py read it at runtime.  Both `flash dump` and
...
RUN if [ -n "${HERMES_GIT_SHA}" ]; then \
        printf '%s\n' "${HERMES_GIT_SHA}" > /opt/flash/.flash_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-flash + dashboard.
...
# `exec flash`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/flash/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-flash-setup && \
    chmod +x /etc/cont-init.d/01-flash-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV HERMES_WEB_DIST=/opt/flash/flash_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/flash/ui-tui/dist/entry.js`) and skip the
...
ENV HERMES_TUI_DIR=/opt/flash/ui-tui
ENV HERMES_HOME=/opt/data
ENV HERMES_WRITE_SAFE_ROOT=/opt/data
ENV HERMES_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/flash (root-owned, read-only) so a runtime
...
# is seeded + chowned to the flash user by docker/stage2-hook.sh and lives
...
# `docker exec <c> flash ...` they default to root, and any file the
# command writes under $HERMES_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/flash/bin/flash, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid flash`
# when invoked as root. Non-root callers (supervised processes,
# `--user flash`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/flash/.venv/bin/flash). See the shim source for
...
# bin globally so `docker exec <container> flash ...` and any
# subprocess that doesn't activate the venv first still find flash.
#
# /opt/flash/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/flash/bin:/opt/flash/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. flash
# subcommand vs. no-args), drops to the flash user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/flash/docker/main-wrapper.sh" ]
CMD [ ]
